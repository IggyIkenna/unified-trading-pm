...
| -- | --------- | ----- | ------- | --- | ------ |
| F-23 | ALC-05 | CODEX-DRIFT | **Opus re-verify → DOWNGRADED P0→P2.** Cross-client fund movement is **unexpressable by construction**: `TransferIntent` (transfer_events.py:68-116) has a SINGLE `client_id` (source+dest both belong to it — no separate source/dest client fields), and the execution-coordinator additionally raises `CrossClientTransferForbiddenError` if `intent.client_id ≠` the process's bound client (transfer_coordinator.py:241). The safety invariant HOLDS robustly. What's missing is only the *literal* codex/CLAUDE wording ("3 raising layers + UAC-validator-rejects-cross-client"): UAC has no raising validator (enforcement is structural), strategy-emit has no separate raise (worker constructs intents with its own client_id). Action: reconcile the HARD-RULE doc to the actual mechanism, OR add a UAC validator for belt-and-suspenders + to satisfy the required-test. | P2 | CONFIRMED |
