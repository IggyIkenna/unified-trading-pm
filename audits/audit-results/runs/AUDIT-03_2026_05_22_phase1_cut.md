...
- **CUT-01 downgraded P0→P1**: the sub-agent flagged APD→"CEFI" as a P0 DEX-feature-starvation bug. But the May-23 APD
  MVP is the **funding-rate-dispersion** variant (CeFi 6-perp, the APD-13 live gate), for which a CEFI bucket is correct.
  The single-category mapping is a **latent hybrid-archetype** limitation (also affects carry→DEFI missing its CeFi perp-
  funding hedge feature). Diagnose-before-fix: confirm whether features merge downstream for the MVP path before treating
  as a blocker. Composes the DeFi+CeFi hybrid architecture rule.
