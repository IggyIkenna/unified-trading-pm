...
| DQ-05 | 2026-05-25 | `measure_honest_coverage.py` hardcodes `-prd` per AG, but CeFi tick still writes the **flat** bucket — writing-flat is WORKING-AS-DEFERRED (`mtds_backfill_phase3` Deferred-work → `bucket_name_ssot` Phase 2.6 migration; NOT a bug). So the cron measures the near-empty `-prd` for cefi → wrong cefi %. Fix = reader tracks writers' bucket via `get_write_bucket_name`, not hardcoded `-prd`. Issue: `plans/active/issues/cefi_tick_bucket_ssot_divergence_2026_05_25.md`.                                                                                                                                                                                                                                                              | OPEN — fix coverage reader (small)         |
