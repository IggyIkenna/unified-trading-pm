...
- **Manifest pruning is non-functional today.** TRADFI MDPS rows in `_index/availability_index.parquet` only cover 2
  dates (2026-01-02 + 2026-04-10) despite ~24 days of GCS parquet existing. MDPS writer is underfilling. Until fixed,
  **listing-pruning** (one `list_blobs` per candidate day) is the right interim approach — adds 15410ms for a 30-day
  window, well under the savings.
- Single-file cold p50 = 663ms is high vs parent-doc target (≤ 200ms cold). Likely region mismatch — bench ran from a
  workstation outside the bucket region. In a co-located backend (same region as the bucket), expect 50–100ms cold and
  20–50ms warm.
