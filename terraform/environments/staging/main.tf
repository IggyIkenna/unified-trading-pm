# Terraform: uts-staging-ikenna GCP Project
# Purpose: Isolated Cloud Build + Artifact Registry for staging branch builds
...
terraform {
  ...
  backend "gcs" {
    ...
    prefix = "unified-trading/staging"
  }
}
...
resource "google_artifact_registry_repository" "unified_trading" {
  ...
  location      = var.region
  format        = "DOCKER"
  description   = "Unified trading system Docker images (staging branch builds)"
...
resource "google_service_account" "cloud_build" {
  ...
  display_name = "Cloud Build Service Account (staging)"
}
