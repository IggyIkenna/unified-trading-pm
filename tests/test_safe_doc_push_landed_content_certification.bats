...
@test "fallback success when a peer landed identical content mid-run (control -- remote check must not over-fire)" {
  # THE HONEST case the "nothing to stage" fallback exists for: the caller had a real diff
  # at entry, and while the run was in flight a concurrent session landed IDENTICAL content
  # on origin. The fallback must report success -- and the new remote-ref check
  # (_sdp_assert_remote_has_entry_content) must confirm origin/$BRANCH actually carries the
  # caller's entry content, not merely that HEAD moved (P0, safe_doc_push_isolation_drops_
  # rename_deletions_2026_08_10).
  git clone -q "${BATS_TEST_TMPDIR}/origin.git" "${BATS_TEST_TMPDIR}/peer" >/dev/null 2>&1
  ( cd "${BATS_TEST_TMPDIR}/peer" && \
      git config user.email "peer@example.com" && \
      git config user.name "peer" && \
      echo "my genuine change" > doc.md && \
      git add doc.md && \
      git commit -q -m "peer: land the same change" && \
      git push -q origin HEAD:live-defi-rollout )

  # Local clone still at "base" (behind origin); caller edits doc.md to the same content
  # the peer just landed -- real diff at entry, converges to the remote mid-run.
  echo "my genuine change" > doc.md

  run bash "$SCRIPT" "docs: change doc" --files "doc.md"

  [ "$status" -eq 0 ]
  [[ "$output" == *"already on origin/live-defi-rollout"* ]]
