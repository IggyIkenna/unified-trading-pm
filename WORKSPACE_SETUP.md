...
See: [INDEX_MIGRATION.md](INDEX_MIGRATION.md) for details.
...
| Script | Purpose |
|--------|---------|
| `scripts/setup-workspace-root.sh` | Main setup script (this guide) |
| `scripts/cleanup-agent-chats.sh` | Clean old Cursor agent transcripts |
| `scripts/sync-rules-pull.sh` | Pull latest cursor rules from PM repo |
| `scripts/sync-workspace.sh` | Check rule sync status |
