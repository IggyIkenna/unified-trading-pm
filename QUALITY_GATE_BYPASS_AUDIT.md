...
**Additional:** `scripts/cicd/tier_c_promotion_gate.py` — `_overlay_firestore_ci_status()` catches
`Exception` so Firestore unavailability (SDK missing, no credentials, network error) never blocks
the promotion gate. Falling back to the manifest ci_status values on any error is the intended
safe-default behaviour. Added 2026-06-11.
