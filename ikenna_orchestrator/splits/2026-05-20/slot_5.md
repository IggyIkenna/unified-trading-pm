...
| -------------------------------------------------------------------- | ------------------------------------------------ | ------------------------------------------------------------- |
| **-2** Strategy/ML consolidation finish (Bucket 3 stale-ref cleanup) | 🟡 IN PROGRESS — dispatched per-slot             | **Read `ikenna_orchestrator/pings/slot_5.md` for your slice** |
...
**Phase 6 + 7 prep + dep-prop QG work** (all can start ahead of Phase 5 GREEN):

(1) **Phase 6 prep — writer fleet Docker staleness diagnosis** — per mega-audit, 0% of 7.4M prod rows are at v8 even
though code constant says 8. Identify which writers still emit pre-v8 rows. Read actual `schema_version` column
distribution per bucket. Plan-of-record: `writegate_honest_coverage_endtoend_2026_05_06.md` § Phase 7.A (2) **Phase 7
prep — DIVERGENT_EMPTY triage CSV** for the 765 cells from A3. Per-cell decision: label-flip vs Phase-11-rebackfill. NO
data backfill yet — only label/schema correctness on existing rows. (3) **dep-prop QG** —
`check_dependency_fail_propagation.py` (per A5 mega-audit findings). Build the QG step. (4) **Phase -2 Bucket 3** — read
...
- **Phase 6** Docker rebuild + redeploy — needs Phase 5 GREEN. Verification: 100 newest manifest rows per bucket at
  `schema_version=8`.
- **Phase 7** Manifest v8 backfill + label-flip — needs Phase 6 GREEN. Verification: 100% v8 + 0 NULL across 10 buckets.
- **Phase 10** QG enforcement upgrade (with slot 2/3) — new QG steps for manifest v8 + dep-fail-propagation.
- **Phase 14** Strategy + execution topology cleanup (with slot 8) — needs Phase 13 GREEN.
- **Phase 11** v8-correctness verification + per-batch sample-check (assert all new rows at v8) — needs Phase 10 GREEN.
- **Phase 12** Live writer parity (with slot 4) — needs Phase 11 batch-side GREEN.

**Single-walk discipline HARD RULE** — Phase 2.2 GCS migration walks every parquet ONCE. Any post-Phase-2 plan proposing
another whole-corpus GCS walk is review-blocking.
