...
| -------------------------------------------------------------------- | ------------------------------------------------ | ------------------------------------------------------------- |
| **-2** Strategy/ML consolidation finish (Bucket 3 stale-ref cleanup) | 🟡 IN PROGRESS — dispatched per-slot             | **Read `ikenna_orchestrator/pings/slot_7.md` for your slice** |
...
**A3 Sports + A2 off-season gap remediation** — original theme `simulation_scenarios + defi_master P2-3` is FROZEN until
Phase 8 unfreeze. Until then:

- Plan: `plans/audit/results/mega_audit_phase_a_issues_human_readable_2026_05_20.md` § A3 Sports + A2 off-season
- Audit every Sports `MISSING_EXPECTED` cell:
  - Confirm `is_in_known_gap()` correctly classifies off-season ranges as `empty_confirmed[reason=EXPECTED_OFF_SEASON]`
    (not `MISSING_EXPECTED`)
  - For real in-season gaps: re-run adapter, backfill OR record typed empty reason
- UAC helpers: `clip_dates_to_source_coverage()`, `is_in_known_gap()`
- Sports + Prediction parallel to DeFi (not blocked by DeFi gate)

**Parallel: Phase -2 Bucket 3** — read `ikenna_orchestrator/pings/slot_7.md`

---

## Future phases (when Phase -2 + dependencies GREEN)

- **Phase 11** Operational backfill (with slots 6, 9) — needs Phase 10 GREEN. Your slice = Sports + Prediction
  MISSING_EXPECTED close-out
