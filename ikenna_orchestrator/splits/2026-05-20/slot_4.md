...
| -------------------------------------------------------------------- | ------------------------------------------------ | ------------------------------------------------------------- |
| **-2** Strategy/ML consolidation finish (Bucket 3 stale-ref cleanup) | 🟡 IN PROGRESS — dispatched per-slot             | **Read `ikenna_orchestrator/pings/slot_4.md` for your slice** |
...
(1) **Phase -2 Bucket 3 cleanup** — read `ikenna_orchestrator/pings/slot_4.md` for your Phase 11f slice (likely
market-data + execution consumers of archived services)

(2) **api_keys + defi_recursive_borrow live-adapter prep** — Phase 12 work that can start AHEAD of Phase 5 (live adapter
scaffold, error-code wiring) without blocking on bucket migration:

- `ApiKeyReloader` from UTL — hot-reload across every adapter. SSOT:
  `codex/06-coding-standards/config-reloader-pattern.md`
- defi_recursive_borrow: 7 RECURSIVE_LOOP codes in UAC `DefiErrorCode`. Reference:
...
- **Phase 5** AWS bucket migration — needs Phase 4 GREEN. SSOT: `aws_migration_defi_first_2026_05_07.md` + master coord
  § Phase 5
- **Phase 12** Live-data adapter completion (with slot 5) — needs Phase 11 batch-side GREEN

**Wallet keys** (and kill-switch / force-push to main / 1.0.0 / cross-client funds) are hard-stop human-only —
/blocked + escalate, do NOT autonomously edit.
