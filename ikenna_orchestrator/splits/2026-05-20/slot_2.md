...
| -------------------------------------------------------------------- | ------------------------------------------------ | ------------------------------------------------------------- |
| **-2** Strategy/ML consolidation finish (Bucket 3 stale-ref cleanup) | 🟡 IN PROGRESS — dispatched per-slot             | **Read `ikenna_orchestrator/pings/slot_2.md` for your slice** |
...
**Phase 1 — AWS↔GCP bucket-name symmetry audit + fix** (paired with slot 3). Phase 0 is GREEN so this is unblocked NOW.

- Read `unified-trading-pm/plans/active/bucket_name_ssot_canonicalisation_2026_05_10.md` § extension
- Read master coordinator § "Phase 1 — bucket symmetry"
- Coordinate with slot 3 (also Phase 1 owner) via /progress or slot ping

**Parallel: R19 UAC import surface enforcement** — close mega-audit R19 (consumers using deep `canonical.*` /
`normalize_utils.*` paths). SSOT: `cursor-rules/imports/uac-import-surface-enforcement.mdc`. Workspace-wide grep +
rewrite to top-level `from unified_api_contracts import X`.

---

## Future phases (when Phase -2 + dependencies GREEN)

- **Phase 3** VM fleet drain (with slot 3) — needs Phase 2 (code freeze) active
- **Phase 4** GCS bucket migration (with slot 3) — needs Phase 3 GREEN
- **Phase 10** QG enforcement upgrade (with slot 5, slot 3) — needs Phase 8 GREEN
