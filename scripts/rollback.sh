...
HISTORICAL_MANIFEST=""
COMMITS=$(git -C "$WORKSPACE_ROOT" log --format="%H" -- "$MANIFEST" 2>/dev/null || true)
FOUND_COMMIT=""
for commit in $COMMITS; do
  VER=$(git -C "$WORKSPACE_ROOT" show "$commit:$MANIFEST" 2>/dev/null | jq -r --arg repo "$REPO_NAME" '.versions[$repo] // .repositories[$repo].version // empty' 2>/dev/null || true)
...
if [ -z "$FOUND_COMMIT" ]; then
  ...
  HISTORICAL_MANIFEST=$(git -C "$WORKSPACE_ROOT" show "$FOUND_COMMIT:$MANIFEST" 2>/dev/null)
fi
echo
