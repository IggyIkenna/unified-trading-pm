...
ResultDict: TypeAlias = dict[str, str | list[str]]
...
CHECKS = [
    ...
    },
    {
        ...
        "code": (
            ...
            '; echo "$EL_OLD" | head -3; ((V++)); }'
            ' || log_success "Event logging imports from'
    ...
    },
    {
        ...
        "code": (
            "\n# pip install anywhere other than bootstrap"
            ...
            '; echo "$PIP" | head -3; ((V++)); }'
            ' || log_success "No bare pip install"'
        ),
    },
    {
        "id": "SWALLOWED_ERRORS",
        ...
        "code": (
            "\n# Swallowed errors — except that silently"
            " passes/returns None\n"
            'SWALLOWED=$(rg "except (ConnectionError, TimeoutError,'
    ...
    # ── Security checks ───────────────────────────────────────────────────────
    {
        ...
        "anchor": 'log_success "No hardcoded project ID in production"',
        "code": (
            "\n# Security: no service account JSON files"
    ...
    },
    {
        ...
        "code": (
            ...
            'PRIV_KEY=$(rg "BEGIN RSA PRIVATE KEY'
            "|BEGIN PRIVATE KEY"
    ...
    },
    {
        ...
        "code": (
            "\n# Security: .gitignore must exclude"
            " credential JSON patterns\n"
            'if [[ -f ".gitignore" ]]; then\n'
            "    CRED_EXCLUDED=$(grep -E "
            '"credentials.*\\.json'
            "|\\*service.account\\*"
            '|\\*\\.key\\.json"'
            ...
            ' || log_success ".gitignore excludes'
            ' credential files"\nfi'
        ),
    },
    # ── Test quality checks ───────────────────────────────────────────────────
    {
        ...
        "code": (
            ...
            "    # @pytest.mark.skip must have a reason"
            ...
            "'# reason: ...' above\""
            '; echo "$SKIP_NO_REASON" | head -3; exit 1; }\n'
            '    log_success "All pytest.mark.skip have'
            ' reason comments"'
        ),
    },
    # ── CI/CD hygiene ─────────────────────────────────────────────────────────
    {
        ...
        "code": (
            "\n# CI/CD hygiene: ||true bypasses"
            " in quality gate scripts\n"
            'BYPASS=$(rg "\\\\|\\\\|true|\\\\|\\\\| true"'
            ' --glob "**/quality-gates.sh"'
            ...
            " ((V++)); }"
            ' || log_success "No ||true quality gate'
            ' bypasses"'
        ),
    },
    # ── id_conventions: must come from unified-config-interface, not UCS ─────
    {
        ...
        "sentinel": (
            "ID_CONVENTIONS_UCS"
            "|id_conventions.*unified_trading_services"
            "|generate_strategy_id.*from unified_trading_services"
        ),
        "anchor": 'log_success "No ||true quality gate bypasses"',
        "code": (
            "\n# ID conventions (generate_strategy_id,"
            " generate_config_id, etc.) live in"
            " unified-config-interface\n"
            "ID_CONV=$(rg 'from unified_trading_services"
            " import[^#]*?(generate_strategy_id"
            "|generate_config_id|parse_strategy_id"
            "|parse_config_id|validate_strategy_id"
            "|validate_config_id|get_gcs_config_path"
            ...
            ' echo "$ID_CONV" | head -3; ((V++)); }'
            ' || log_success "ID convention imports'
            ' from unified_config_interface"'
        ),
    },
    # ── security module: deleted — use UEI log_event instead ─────────────────
    {
        ...
        "code": (
            "\n# unified_trading_services.security module"
            " was deleted Feb 2026 — use log_event() from"
            " unified_events_interface\n"
            "SEC=$(rg 'from unified_trading_services"
            ".*security|from unified_trading_services"
            " import.*[Aa]uth[Ff]ailure|SecurityLogger"
            ...
            ' unified_events_interface";'
            ' echo "$SEC" | head -3; ((V++)); }'
            ' || log_success "No deleted security'
            ' module imports"'
        ),
    },
    # ── Domain layer: Service→UDS→UCS boundary ────────────────────────────────
    {
        ...
        "code": (
            ...
            "UCS_DOMAIN=$(rg 'from"
            " unified_trading_services"
            " import[^#]*?(InstrumentsDomainClient"
            ...
            " unified_domain_client, not"
            ' unified_trading_services";'
            ' echo "$UCS_DOMAIN" | head -5; ((V++)); }'
            ' || log_success "Domain clients imported'
            ' from unified_domain_client"'
        ),
    },
    # ── GCP auth: ADC pattern enforcement ─────────────────────────────────────
    {
        ...
        "anchor": 'log_success "No ||true quality gate bypasses"',
        "code": (
            "\n# GCP auth: tests must use"
            " google.auth.default() — never pytest.skip"
            " for missing credential file\n"
            "BAD_AUTH_SKIP=$(rg 'pytest\\.skip"
            ".*[Cc]redential|pytest\\.skip"
            ".*GOOGLE_APPLICATION_CREDENTIALS"
            "|if not.*gcp_credentials.*pytest\\.skip"
            "|if not.*cred_file.*pytest\\.skip' \\\n"
            "    --type py tests/ 2>/dev/null \\\n"
            '    | grep -v "_skip_integration_without_creds'
            ...
            ' echo "$BAD_AUTH_SKIP" | head -5; ((V++)); }'
            ' || log_success "No credential-file skip'
            ' patterns in tests"'
        ),
    },
    {
        ...
        "sentinel": (
            "GOOGLE_APPLICATION_CREDENTIALS.*\\.env\\.example|No GOOGLE_APPLICATION_CREDENTIALS in .env.example"
        ),
        "anchor": 'log_success "No credential-file skip patterns in tests"',
        "code": (
            "\n# .env.example must not contain"
            " GOOGLE_APPLICATION_CREDENTIALS"
            " (use ADC / GH token / Cloud SA)\n"
            '[[ -f ".env.example" ]] &&'
            ' rg "GOOGLE_APPLICATION_CREDENTIALS"'
...
def update_repo(repo_path: Path, dry_run: bool) -> dict[str, str | list[str]]:
...
def main() -> None:
    ...
    repo_name: str | None = getattr(args, "repo", None)

    repos = [WORKSPACE / r for r in ALL_REPOS]
    if repo_name:
        repos = [WORKSPACE / repo_name]

    prefix = "[DRY RUN] " if dry_run else ""
    print(f"\n{prefix}Rolling out quality gate checks to {len(repos)} repos\n")
    ...
    for repo_path in repos:
        ...
        qg_result: ResultDict = update_repo(repo_path, dry_run)
        bp_result = update_basedpyright(repo_path, dry_run)
    ...
    # Summary
    updated: list[ResultDict] = [r for r in results if r["status"] == "UPDATED"]
    up_to_date: list[ResultDict] = [r for r in results if r["status"] == "UP_TO_DATE"]
    skipped_repos: list[ResultDict] = [r for r in results if r["status"] in ("SKIP", "MISSING")]

    print(f"\n{'=' * 60}")
    print(f"Updated:    {len(updated)} repos")
    print(f"Up-to-date: {len(up_to_date)} repos")
    skipped_names = ", ".join(str(r["repo"]) for r in skipped_repos)
    print(f"Skipped:    {len(skipped_repos)} repos ({skipped_names})")
    print(f"{'=' * 60}\n")
