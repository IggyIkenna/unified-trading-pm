...
for _clone in "${_ws_root}"/*/ "${_ws_root}"/.tabs/*/*/; do
    ...
    # PREK_HOME per-slot scoping for the pre-commit hook itself (2026-08-17,
    # safe_doc_push_prek_patch_not_restored_recurrence_2026_08_17.md). The 2026-08-16 per-slot
    # PREK_HOME fix (safe_doc_push_shared_prek_home_across_ao_vm_slots_2026_08_16.md) only scoped
    # scripts/dev/safe-doc-push.sh and scripts/quickmerge.sh -- but the RULES.md-sanctioned raw
    # `git commit` + `git push` path for a cross-repo PM plan-flip (worker.md's own ship loop,
    # the single highest-frequency PM mutation) invokes NEITHER wrapper, so it always fell through
    # to prek's unscoped host-global ~/.cache/prek/patches. Any two slots landing a raw plan-flip
    # commit concurrently share that one cache dir, and prek's own stash/restore of each other's
    # unstaged out-of-scope edits can interleave and silently drop one side's WIP -- reproducing,
    # for the raw-commit path specifically, the exact collision the 08-16 fix closed for the two
    # wrapper scripts. Patching the hook itself (rather than the two scripts) closes the gap for
    # every commit path, not just the two already-covered ones. CONTENT-gated (a marker string),
    # not existence-gated, so it converges within one 5-min sweep the same way the pre-push guard
    # below does, and re-heals if a future `prek install` regenerates the hook without it.
    if [[ -f "${_pc_hook}" ]] && grep -q 'generated by prek' "${_pc_hook}" 2>/dev/null \
        && ! grep -q 'PREK_HOME per-slot scoping' "${_pc_hook}" 2>/dev/null; then
        _pc_slot_num="$(printf '%s' "${_hooks_dir}" | sed -nE 's#.*/\.tabs/([0-9]+)/[^/]+/\.git/hooks$#\1#p')"
        if [[ -n "${_pc_slot_num}" ]]; then
            _pc_patch_file="$(mktemp)"
            {
                echo ""
                echo "# PREK_HOME per-slot scoping (2026-08-17, safe_doc_push_prek_patch_not_restored_recurrence_2026_08_17.md)"
                echo "# -- gives this slot's raw \`git commit\` calls a private prek patches/ dir, mirroring"
                echo "# scripts/dev/safe-doc-push.sh / scripts/quickmerge.sh's own PREK_HOME scoping."
                echo 'if [ -z "${PREK_HOME:-}" ]; then'
                echo "  _pc_home=\"\$HOME/.cache/prek-slot-${_pc_slot_num}\""
                echo '  mkdir -p "$_pc_home" 2>/dev/null'
                echo '  for _pc_item in repos hooks tools cache config-tracking.json; do'
                echo '    [ -e "$HOME/.cache/prek/$_pc_item" ] || continue'
                echo '    ln -sfn "$HOME/.cache/prek/$_pc_item" "$_pc_home/$_pc_item" 2>/dev/null'
                echo '  done'
                echo '  export PREK_HOME="$_pc_home"'
                echo "fi"
                echo ""
            } >"${_pc_patch_file}"
            if awk -v patchfile="${_pc_patch_file}" '
                /^exec "\$PREK"/ { while ((getline line < patchfile) > 0) print line; close(patchfile) }
                { print }
            ' "${_pc_hook}" >"${_pc_hook}.tmp" 2>/dev/null && [[ -s "${_pc_hook}.tmp" ]]; then
                mv "${_pc_hook}.tmp" "${_pc_hook}" && chmod +x "${_pc_hook}"
            else
                rm -f "${_pc_hook}.tmp" 2>/dev/null
            fi
            rm -f "${_pc_patch_file}" 2>/dev/null
        fi
    fi
