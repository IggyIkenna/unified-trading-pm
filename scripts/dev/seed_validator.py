...
def _validate_file(parquet_path: Path, strict: bool) -> FileReport:
    ...
    try:
        ...
        if schema_type == "unknown":
            if strict:
                errors.append("Cannot determine schema type from path — unknown directory structure")
        else:
            # Required columns
...
def _parse_args(argv: list[str]) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Validate seed data Parquet files against UAC/UIC schemas.")
