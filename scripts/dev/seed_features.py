...
def compute_onchain(df: pd.DataFrame) -> pd.DataFrame:
    ...
    for symbol, grp in df.groupby("symbol"):
        ...
        n = len(grp)
        result = grp[["timestamp", "symbol", "venue"]].copy()
        result["active_addresses_norm"] = np.clip(0.5 + np.cumsum(rng.standard_normal(n)) * 0.01, 0.0, 1.0)
...
def run(mode: str, input_dir: Path, output_dir: Path, project: str | None, dry_run: bool) -> int:
    ...
    if df.empty:
        log.error("No OHLCV seed data found in %s — run generate_synthetic_data.py first", input_dir)
        return 1
    ...
    if mode == "quick":
        ...
        log.info("Quick mode: filtered to %d rows across %d symbols", len(df), df["symbol"].nunique())
...
def _parse_args(argv: list[str]) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Seed pre-computed feature data for the unified-trading dev project.")
    ...
    parser.add_argument(
        "--output",
        type=Path,
        default=Path("/tmp/seed_features"),
