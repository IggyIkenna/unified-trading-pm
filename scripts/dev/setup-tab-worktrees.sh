...
# husky's .husky/_/ shims with its own pre-commit, breaking husky/lint-staged for those repos
# (same install-order race documented in slot-cron-ff-pull.sh and
# deployment_ui_prettier_version_skew_blocks_quickmerge_2026_07_21.md). Skip clone-time install
# there and leave the repo entirely to husky.
install_prek_precommit_hook() {
    ...
    hooks_dir="$(git -C "${clone_dir}" rev-parse --path-format=absolute --git-path hooks 2>/dev/null)" || return 0
