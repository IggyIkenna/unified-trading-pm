...
PYTEST_UNIT_DIR="tests/unit/ scripts/quality_gates/ scripts/cicd/ scripts/docs/ scripts/plan-hygiene/ cursor-configs/skills/"
...
# ── Post-gates: Evidence-backed completion (runtime-green / prod data-mutation claims cite VERIFIED evidence) ──
...
# sub-rule B (baselined ratchet) flags a runtime-green claim with no evidence ref. A `- [x]` todo claiming a prod
# DATA-mutation (restamp/backfill/purge row-count, GCS object rename/delete, tofu/terraform state op) MUST similarly
# cite `Evidence: manifest-delta=<path>|vm-log=<path>|gcs-op=<id>|state-list=<before>,<after>`; sub-rule C (baselined
# ratchet, same discipline as B) flags a mutation claim with no such ref. Re-baseline B/C with --baseline-write.
...
if [ -f "$EVIDENCE_CHECKER" ] && [ -n "${WORKSPACE_ROOT:-}" ]; then
    ...
    if python3 "$EVIDENCE_CHECKER" --workspace-root "$WORKSPACE_ROOT" >/dev/null; then
        ...
        echo "   build/deploy/promote-green claim has no 'Evidence: cloudbuild=<id>' ref, OR a new prod data-mutation" >&2
        echo "   claim has no 'Evidence: manifest-delta=|vm-log=|gcs-op=|state-list=' ref. See plans/PLAN_FORMAT.md §§ 8b, 8d." >&2
        echo "   Re-baseline sub-rules B/C after intentional debt: python3 ${EVIDENCE_CHECKER} --workspace-root \$WORKSPACE_ROOT --baseline-write" >&2
...
# ── Post-gates: Codex doc freshness — MOVED OFF the commit path (2026-08-15) ──
# check_codex_doc_freshness.py used to run here as a blocking post-gate step. Removed per
# ci_satellite_ao_dispatch_batch14_2026_08_15.md's operator-approved todo: even after the
# 2026-08-12 AGENCY SPLIT made pure calendar-staleness advisory-only, an authoring-defect
# violation on ANY scanned doc (not just one you touched) could still fail Pass 1 for a
# commit that never touched codex/ at all, once a doc drifted stale enough to trip on the
# calendar — see codex_freshness_ratchet_trips_on_calendar_blocking_all_pm_code_commits_2026_08_11.md.
# The check now runs async, daily, via .github/workflows/codex-freshness-sweep.yml — a
# violation files a real AO escalation (wall_type=codex_freshness_stale, resolves on the
# sweep's next clean run) instead of blocking a stranger's commit. SSOT:
