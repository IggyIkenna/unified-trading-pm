...
def generate_svg(data: JsonDict) -> str:
    ...
    ln(
        f'  <text x="40" y="38" class="title">'
        f"Workspace Manifest - Topological DAG ({len(levels)} levels)</text>"
    )
    ln(
        f'  <text x="40" y="58" class="subtitle">{total} repos.'
        " SSOT: workspace-manifest.json."
        " Generated by unified-trading-pm/scripts/generate_workspace_dag.py</text>"
    )
    ln(
    ...
    for lvl in sorted(levels.keys()):
        ...
        ln(
            f'  <rect x="40" y="{y}" width="2320"'
            f' height="{bh}" class="level-bg" />'
        )
        ln(
            f'  <rect x="55" y="{y + 8}" width="80"'
            f' height="22" fill="{color}" class="level-badge" />'
        )
        ln(
            f'  <text x="95" y="{y + 24}"'
            f' text-anchor="middle" class="level-label">L{lvl}</text>'
        )
        ln(
            f'  <text x="150" y="{y + 24}"'
            f' class="desc">{esc_desc} ({count} repos)</text>'
        )
    ...
    footer_y = y + 20
    counts_str = ", ".join(
        f"{v} {k}s" for k, v in sorted(type_counts.items())
    )
    ...
    ln(
        f'  <text x="60" y="{footer_y + 25}"'
        ' class="section">Versioning + Summary</text>'
    )
    ln(
    ...
    ln(
        f'  <text x="60" y="{footer_y + 60}" class="small">'
        f"{total} repos: {counts_str}</text>"
    )
    ln(
