...
#
# Uses `exec -a <marker>` to give each throwaway `sleep` process a distinguishing argv0
# so `pkill -f` has something concrete to match/not-match, without touching any real
# quality-gates.sh process.
#
...
victim1=$(spawn_marked "$LTMP/quality-gates.sh")
sleep 0.2
pkill -f "quality-gates.sh" >/dev/null 2>&1
rc=$?
...
target2=$(spawn_marked "$LTMP/.tabs/5/mtds/quality-gates.sh")
sleep 0.2
(cd "$LTMP/.tabs/5/mtds" && pkill -f "$LTMP/.tabs/5/mtds/quality-gates.sh") >/dev/null 2>&1
rc=$?
...
victim3=$(spawn_marked "$LTMP/.tabs/2/mtds/quality-gates.sh")
sleep 0.2
(cd "$LTMP/.tabs/5/mtds" && pkill -f "$LTMP/.tabs/2/mtds/quality-gates.sh") >/dev/null 2>&1
rc=$?
...
victim4=$(spawn_marked "$LTMP/some_generic_name")
sleep 0.2
