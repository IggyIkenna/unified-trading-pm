...
# `# QG-allow: legacy-bucket-name-migration` — the latter is the legacy-bucket
# marker also honored by STEP 5.96 (legacy-concat checker). 5.31 and 5.96 overlap
# on the same `instruments-/features-/ml-` legacy patterns; honoring the migration
# marker in BOTH avoids forcing a line to carry two redundant markers (which would
# overflow the 120-char E501 limit). The marker declares a sanctioned legacy bucket
# pending the bucket-name SSOT migration (resolve_bucket_name emits env-tiered names,
# not behaviour-preserving for these flat legacy buckets yet).
# ============================================================
...
# the 5.7x/5.9x ratchets — fallback-imports 5.94, DTZ/TID251 5.95, etc.) still does
# `V=$(( V + 1 ))` on failure, but V is NEVER re-evaluated afterward → a red ratchet step
