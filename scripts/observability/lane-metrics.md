...
and was removed 2026-06-11 (cicd_workflow_sprawl_audit). Cassette-replay coverage is provided by
`cassette-drift-check.yml`

- the per-commit `tests/test_cassette_schema_parity.py` in unified-api-contracts. **Scope (if re-introduced):** Replays
  VCR cassettes against current Pydantic models. **Counter:** `contract_replay_total{repo, cassette, result}`
  **Labels:** `result` in `{pass, fail, skip}`
...
**Trigger:** Nightly via `cassette-drift-check.yml`. **Scope:** Re-records cassettes and diffs against committed
