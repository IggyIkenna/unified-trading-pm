...
for repo_path in "$WORKSPACE_ROOT"/*/; do
  ...
  # Detect tracked files that should be ignored, respecting negation patterns.
  # GIT_INDEX_FILE=/dev/null = empty index (git sees no tracked files) so it
  # evaluates .gitignore from scratch including ! negations — unlike --no-index
  # which skips negations and produces false positives on intentional exceptions.
  hits=$(
    git -C "$repo_path" ls-files 2>/dev/null \
      | GIT_INDEX_FILE=/dev/null git -C "$repo_path" check-ignore --stdin 2>/dev/null \
      || true
  )
