...
EXCLUDE_DIRS = {
    ".venv", "venv", ".venv-workspace", "__pycache__", ".git", ".github",
    "node_modules", "build", "dist", ".egg-info", ".eggs", ".tox",
    ".mypy_cache", ".pytest_cache", ".ruff_cache", "tests", "examples",
    "docs", "scripts",
}
...
def get_stdlib_modules() -> set[str]:
    ...
    known_stdlib = {
        ...
        "bisect", "builtins", "calendar", "cgi", "cgitb", "codecs", "collections",
        ...
        # C-level modules
        "_thread", "_io", "_signal", "_weakref", "_abc", "_collections",
        "_functools", "_operator", "_stat", "_string", "_struct",
    ...
    for import_name in sorted(all_imports):
        ...
        if import_name in internal_packages:
            ...
            if internal_repo not in declared_pkg_names and import_name not in declared_import_names:
                violations.append(
                    f"  INTERNAL: imports '{import_name}' (repo: {internal_repo}) "
                    f"but not declared in pyproject.toml"
                )
            elif verbose:
        ...
        known_namespace_packages = {
            "google": {"google-cloud-storage", "google-cloud-pubsub", "google-auth",
                       "google-api-python-client", "google-cloud-build",
                       "google-cloud-compute", "google-cloud-bigquery",
                       "google-cloud-secret-manager", "google-cloud-logging",
                       "google-cloud-monitoring", "gcloud-aio-storage",
                       "google-cloud-run"},
            "grpc": {"grpcio"},
            "attr": {"attrs"},
            "yaml": {"pyyaml"},
        ...
        violations.append(
            f"  UNDECLARED: imports '{import_name}' - not in pyproject.toml, "
            f"not in workspace constraints, not stdlib"
        )
...
def main() -> int:
    ...
    for repo_name in sorted(repos.keys()):
        ...
        violations = check_repo(
            repo_name, repo_dir, internal_packages,
            constraints_packages, stdlib_modules, verbose,
        )
        repos_checked += 1
