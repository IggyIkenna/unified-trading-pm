...
if [ -L "$SYMLINK_PATH" ]; then
  ...
  if [ "$CANONICAL_TARGET" = "$TARGET_DIR" ] || [ "$CURRENT_TARGET" = "$TARGET_DIR" ]; then
    ...
    echo ""
    echo "Plan files:"
    ls -la "$TARGET_DIR"/*.plan.md 2>/dev/null || echo "(none)"
    exit 0
  fi
  rm "$SYMLINK_PATH"
...
if [ -d "$SYMLINK_PATH" ]; then
  ...
  if [ -n "$(find "$SYMLINK_PATH" -maxdepth 1 -name "*.plan.md" 2>/dev/null | head -1)" ]; then
    echo "[MOVE] Moving .plan.md files to target..."
    for f in "$SYMLINK_PATH"/*.plan.md; do
      [ -e "$f" ] && mv "$f" "$TARGET_DIR/"
    done
  fi
