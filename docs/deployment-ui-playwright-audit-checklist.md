...
| 3.8 | Every active VM's `category` column must be a real category (`CEFI` / `TRADFI` / `DEFI` / `SPORTS` / `PREDICTION`), **never `UNKNOWN`**. An `UNKNOWN` value indicates the launcher dropped the `--asset-group` arg or the deployment-events emitter stripped it — both are regressions. |
