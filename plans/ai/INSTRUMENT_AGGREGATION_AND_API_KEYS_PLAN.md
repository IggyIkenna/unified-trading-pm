...
**Status:** Draft  
**Scope:** instruments-service, unified-cloud-services, unified-trading-deployment-v3, market-tick-data-handler, market-data-processing-service  
...
| **unified-cloud-services** | `get_secret_with_fallback(secret_name, project_id, fallback_env_var)` — **only** way to resolve API keys |
...
- **Scripts that need API keys:** Must use `get_secret_with_fallback`. No direct `os.environ.get` for API keys.
...
**InstrumentsDomainClient** (unified-domain-services) is the **only** canonical way to read instruments.
...
- **unified-cloud-services** `InstrumentsDomainClient` — migrate to unified-domain-services. UDS is the canonical implementation.
...
| **unified-domain-services** | Expose `InstrumentsDomainClient.get_aggregated_instruments(category)` — reads latest `aggregated/aggregated_instruments_*.parquet` |
...
┌─────────────────────────────────────────────────────────────────────────┐
│ API Keys                                                                 │
│   Secret Manager → get_secret_with_fallback (UCS) → services             │
...
- `unified-cloud-services/unified_cloud_services/domain/clients.py`
