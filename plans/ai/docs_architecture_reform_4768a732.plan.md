...
1. **Quickmerge SSOT:** Template (7 stages) vs deployed repos (3-4 stages) — neither is complete. Template has pre-flight audit + Act simulation (not deployed); deployed repos have venv activation + dep installation (not in template). `unified-cloud-services` is the most advanced deployed version.
...
Before writing ANY new function, class, or connector, search for it:
  ...
  4. unified-cloud-services — storage, secrets, pubsub, ConfigStore
  5. unified-config-interface — config schema, BaseConfig
  6. unified-events-interface — event logging
  7. unified-domain-services — instruments, domain clients
...
- **High-junk targets**: `unified-cloud-services` (41 .md), `features-delta-one-service` (61 .md + archive/), `strategy-service` (56 .md), `unified-trade-execution-interface` (11 junk files)
...
#### Template 3: Library (unified-cloud-services is canonical reference)
...
unified-cloud-services, unified-config-interface, unified-events-interface, unified-domain-services, unified-market-interface, unified-ml-interface, unified-trade-execution-interface, unified-defi-execution-interface
...
├── unified-cloud-services/     ← gitignored (own git repo)
...
REPOS=(instruments-service unified-cloud-services unified-config-interface ...)
...
- `from unified_cloud_services import ConfigStore, get_storage_client, get_secret_client` — persistence + cloud
- `from unified_events_interface import setup_events, log_event` — events
- `from unified_domain_services import InstrumentsDomainClient` — domain data
