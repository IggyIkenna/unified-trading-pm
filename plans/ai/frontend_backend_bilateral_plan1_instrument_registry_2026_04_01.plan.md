...
todos:
  ...
  - id: p1-5-market-data-assumption
    content: |
      - [ ] [AGENT] P1. For every instrument in the snapshot, the UI can assume market data exists (even if GCS doesn't have it yet). Add a `hasMarketData: true` flag to all snapshot instruments. The mock handler will use this to generate synthetic tickers/candles/orderbook for any instrument in the snapshot. This decouples UI development from actual GCS data availability.
    status: todo
  - id: p1-6-enhance-openapi-reference
    content: |
      - [ ] [AGENT] P1. Audit what's in `ui-reference-data.json` vs what UAC/UIC/instruments-service actually exports. If there are enums, config schemas, or registry entries that the generate_ui_reference_data.py script misses, add extraction steps. Specifically check: strategy configs from system-topology.json (37 strategies), execution algo definitions, sports bookmaker registry, DeFi protocol capabilities, TradFi exchange calendars. Everything the UI needs for realistic mocking should be accessible from the registry.
    status: todo
  - id: p1-7-tests
    content: |
      - [ ] [AGENT] P0. Add tests: (1) snapshot generation script produces valid JSON with expected structure, (2) all 5 asset classes represented, (3) venue names match VENUE_CATEGORY_MAP in ui-reference-data.json, (4) instruments.ts exports compile without errors, (5) snapshot contains 10,000+ instruments (no artificial limits). Run QG on unified-trading-pm and unified-trading-system-ui.
    status: todo
---
