...
copy, signup form labels, codex SSOT, and admin tooling; lacks a per-prospect tailored pre-demo prep layer between
Strategy Evaluation and the Platform walkthrough (`/strategy-review` does not exist); and exposes long-form content
(FAQ, founder story) on public pages that should be gated. Funnel order is now: Questionnaire → Briefings → Initial call
→ Strategy Evaluation → **Strategy Review (pre-demo prep pack)** → Platform walkthrough → **Commercial Tailoring** →
Signup → Client Dashboard.
...
| 4   | Catalogue exposure                                | Curated examples (a small handful) at Strategy Review; full catalogue at Commercial Tailoring (§2.6, was "Bespoke Tailoring") post-walkthrough. Strategy Review v1 ships with the curated-examples scaffolding.                           |
...
| File                                                                         | Phase | Action                                                                                                                                                                                                                                                                   |
| ---------------------------------------------------------------------------- | ----- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `unified-trading-pm/codex/08-workflows/signup-signin-workflow.md`            | 0     | Insert Strategy Review + Platform walkthrough stages (§2.4b/§2.5 — Eval → Strategy Review → Walkthrough → Commercial Tailoring); rename Regulatory Umbrella → Regulated Operating Models in marketing-context table cells; keep IM/Reg legal labels in §2.7.2 path table |
...
- [x] [AGENT] P0. Update `signup-signin-workflow.md`: insert new §2.4b "Strategy Review" stage between current §2.4
      (Strategy Evaluation) and §2.5 (Tailored demo). Update funnel diagram (lines 30-72) to nine stages.
- [x] [AGENT] P0. **2026-04-26 reorder.** Swap §2.4b ↔ §2.5 and rename: §2.4b is now "Platform walkthrough" (was
      "Tailored demo") and §2.5 is now "Strategy Review" (was the new §2.4b). Rationale: Strategy Review is more useful
      AFTER the prospect has seen the platform than before. Public homepage rail bumped 6 → 7 visible stages;
      engagement-route process strips bumped 5 → 6. Strategy Review's "demo preparation" section reframed as
      "walkthrough follow-up" since the demo precedes the Review.
- [x] [AGENT] P0. **2026-04-26 ROLLBACK + Commercial Tailoring rename** (Funnel Coherence plan, Workstream G). Revert
      the morning's swap: §2.4b is back to "Strategy Review" (pre-walkthrough), §2.5 is back to "Platform walkthrough",
      §2.6 is renamed from "Bespoke tailoring" to "Commercial Tailoring". Strategy Review reframed as a **pre-demo
      tailored review and walkthrough preparation pack** — proposed route hypothesis, briefing excerpts, demo agenda,
      workflows likely to be shown, curated examples, missing-info checklist, route-specific risks. NOT a final
      commercial proposal (that's Commercial Tailoring at §2.6). Public homepage rail stays 7 stages (Onboarding
      implicit). Engagement-route process strips on `/investment-management`, `/platform`, `/regulatory` all mirror the
      same 7 stages. Strategy Review `_client.tsx` schema added 7 new optional fields (proposedRouteHypothesis ·
      briefingExcerpts · demoAgenda · workflowsShown · curatedExamples · missingInformation · routeRisks); legacy fields
      kept as fallbacks. Codex change-log keeps both 2026-04-26 entries (the morning's swap AND this rollback) for
      honest history.
