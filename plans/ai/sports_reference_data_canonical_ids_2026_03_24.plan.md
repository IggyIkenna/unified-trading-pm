...
- repo: unified-sports-reference-interface code: C0 notes: "Canonical ID mapping across adapters"
- repo: unified-reference-data-interface code: C0 notes: "URDI sports adapters must use canonical IDs from UAC"
...
URDI for instruments. URDI calls `api_football` and `betfair` adapters. The output should be `InstrumentRecord[]` where
each record represents a betting market / fixture available to trade.
...
| Sports data adapters (api_football, odds_api, footystats, transfermarkt, understat)                             | `unified_sports_reference_interface.adapters.*`                                     |
| `TeamNormalizer` (raw name → canonical)                                                                         | `unified_reference_data_interface.adapters._sports_normalizer`                      |
...
User clarified: historical odds come from **Odds API** (`odds-api-key` in Secret Manager). USRI already has
`unified_sports_reference_interface.adapters.odds_api`. The question is: **which data source does instruments-service
use for SPORTS historical reference data?**
...
- `URDI CANONICAL_VENUE_TO_ADAPTER` currently maps `BETFAIR` → `betfair` and `API_FOOTBALL` → `api_football`
...
### 4. URDI Capability Registry Not Updated for Sports

The `refdata_preflight_skip` warning fires for all sports venues: `"venue=api_football not in capability registry"`.
URDI's preflight check has a capability registry that needs to include sports adapters.

**Where:** `unified_reference_data_interface` capability registry (whichever file controls which adapters are "live").
...
- The `TeamNormalizer` in URDI — it works and maps to canonical names
- The api_football adapter in URDI — it already produces canonical fixture keys

## Recommended Action Order

1. **Define canonical ID format** in UAC docstrings (no code changes — documentation only)
2. **Audit adapters** in USRI and URDI to verify they produce IDs in that format
3. **Add `SeasonDefinition`** to UAC
4. **Fix URDI capability registry** to include sports adapters
