...
- **Others (no changes)**: unified-cloud-services, unified-config-interface, unified-market-interface, unified-trade-execution-interface, unified-domain-services
...
1. **API Keys Standardization**:
  - All API keys via `get_secret_with_fallback()` (no `os.environ.get`)
...
**Current**: `MemoryMonitor` exists in unified-cloud-services but services duplicate DataFrame memory tracking.
...
**Keep in UCS**: System-level `MemoryMonitor` (cross-platform, threshold checking) stays in unified-cloud-services. UEI adds DataFrame-specific helpers.
...
from unified_cloud_services import BaseDependencyChecker
...
| ---------------------------------- | --------------------------------------------------- | -------------------------------------------- |
| **UCS** (unified-cloud-services)   | Storage clients, error handling, retries, auth      | `get_storage_client()`, `@handle_api_errors` |
...
**Issue**: All 4 handlers import `unified_cloud_services` inside functions (whitelisted in quality gates).
...
For system memory monitoring, use unified-cloud-services:

```python
from unified_cloud_services import get_memory_monitor
