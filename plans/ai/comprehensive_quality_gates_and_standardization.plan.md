...
todos:
  - id: lib-pyright-fix
    content: "Phase 1A: Fix 2 libraries still using pyright (unified-domain-services, unified-trade-execution-interface)"
    status: pending
...
2. ✅ unified-cloud-services — Uses `basedpyright`, passes all quality gates
...
- ✅ instruments-service uses `get_secret_with_fallback` (2 script exceptions documented)
...
- ⚠️ unified-domain-services: Uses `pyright` (not `basedpyright`) — line 80-116 in quality-gates.sh
...
- unified-domain-services — Coverage 14% (need 35%), uses `pyright`
...
$TYPE_CHECKER unified_domain_services/ --level warning
```
...
- Agent 1: unified-domain-services (pyright → basedpyright)
...
- unified-cloud-services ✅ (done)
- unified-config-interface ✅ (done)
- unified-events-interface ✅ (done)
- unified-domain-services ⚠️ (needs pyright fix)
...
1. **Libraries first** (merge to main):
  - unified-domain-services, unified-trade-execution-interface (Phase 1A)
...
- name: Type check (basedpyright)
  run: basedpyright unified_domain_services/ --level warning
```

**Test locally:**

```bash
cd unified-domain-services
bash scripts/quality-gates.sh --no-fix
```
...
rg "_load_instruments_by_venue" --type py \
    --glob "!unified-cloud-services/**" \
    --glob "!unified-domain-services/**"
...
- ✅ instruments-service uses get_secret_with_fallback
