...
| ------------------------------ | ----------------- | -------------------------------- |
| **unified-cloud-services**     | ~16,467           | -6,097 (restructure)             |
| **unified-config-interface**   | 4,348             | —                                |
| **unified-events-interface**   | 1,293             | —                                |
| **unified-domain-services**    | ~6,293            | +1,563 (domain clients from UCS) |
| **unified-market-interface**   | 20,885            | —                                |
| **unified-trade-execution-interface**    | 2,698             | —                                |
| **execution-algo-library**     | 1,396             | —                                |
...
- **Action:** Delete [unified-cloud-services/unified_cloud_services/domain/data_source_mapping.py](unified-cloud-services/unified_cloud_services/domain/data_source_mapping.py) (129 lines)
...
- **Action:** Move [unified-cloud-services/unified_cloud_services/domain/clients.py](unified-cloud-services/unified_cloud_services/domain/clients.py) (1,563 lines) to UDS. UDS is canonical per dependency matrix ("Domain clients, date validation").
...
- **Action:** Refactor [StandardizedDomainCloudService](unified-cloud-services/unified_cloud_services/domain/standardized_service.py) to use `get_storage_client`, `CloudTarget`, and cloud primitives directly instead of UnifiedCloudService. Delete [unified_cloud_service.py](unified-cloud-services/unified_cloud_services/core/unified_cloud_service.py) (1,750 lines) and [domain/factories.py](unified-cloud-services/unified_cloud_services/domain/factories.py) (76 lines).
- **Repos to update:**
  - unified-cloud-services (core refactor)
  - unified-domain-services (factories re-export — update or remove)
  - execution-services (test_ucs_integration.py — update or remove)

### 0.4 Migrate UnifiedCloudServicesConfig to UCI

- **Action:** Migrate remaining services to UCI `UnifiedCloudConfig`. Remove [unified-cloud-services/unified_cloud_services/core/config.py](unified-cloud-services/unified_cloud_services/core/config.py) UnifiedCloudServicesConfig (669 lines). Keep `get_config`, `unified_config` if still needed; otherwise migrate to UCI.
...
- **Action:** Migrate callers to UEI `setup_events`. UEI can use UCS for cloud-specific wiring (e.g. Cloud Logging) if needed. Remove or thin [core/logging.py](unified-cloud-services/unified_cloud_services/core/logging.py) setup_cloud_logging (~458 lines).
...
- **Action:** Ensure UCS build/package excludes `clients/`. UMI has canonical Tardis, Databento, TheGraph, etc. UMI clients correctly use UCS for `get_secret_with_fallback`, `get_config` — no code carryover needed.

### 0.7 Move ML Module to unified-ml-interface

- **Action:** Create new `unified-ml-interface` library. Move [unified-cloud-services/unified_cloud_services/ml/](unified-cloud-services/unified_cloud_services/ml/) (~1,428 lines) — ModelRegistry, ModelVariantConfig, ModelMetadata, config_schema.
...
**File:** [unified-cloud-services/unified_cloud_services/domain/clients.py](unified-cloud-services/unified_cloud_services/domain/clients.py) (until moved to UDS)
...
# After
from unified_cloud_services.core.market_category import get_instruments_bucket_for_category
...
- [unified-cloud-services/.cursorrules](unified-cloud-services/.cursorrules) — cloud-only scope
- [unified-cloud-services/docs/ARCHITECTURE.md](unified-cloud-services/docs/ARCHITECTURE.md) — remove domain clients, config, ML
- [unified-cloud-services/README.md](unified-cloud-services/README.md) — scope
- [unified-domain-services/README.md](unified-domain-services/README.md) — domain clients canonical
...
| -------------------- | ----------- | ----------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------- |
| UnifiedCloudService  | pending     | **Removed** (file gone; refs in comments only)                                            | Fixed test that imported it → use get_secret_with_fallback; removed file-size bypass; Dockerfile healthcheck → get_storage_client, CloudTarget |
| DataSourceMapping    | completed   | **Gone** (no file)                                                                        | None                                                                                                                                           |
| domain/clients.py    | completed   | **In UDS** (unified_domain_services/clients.py)                                           | None                                                                                                                                           |
...
| API clients in build | pending     | **Excluded** (pyproject.toml exclude unified_cloud_services/clients*)                     | Marked completed                                                                                                                               |
...
| Item                           | Actual state                                                          |
| ------------------------------ | --------------------------------------------------------------------- |
| Domain clients                 | In unified_domain_services/clients.py (InstrumentsDomainClient, etc.) |
