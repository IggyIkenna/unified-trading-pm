...
**Strengths:** Quality gates in instruments-service include security checks (credential negation, hardcoded project ID, private keys, Dockerfile secrets). Production code uses `get_secret_client` for API keys. No SQL injection, command injection, or unsafe deserialization found.
...
**Standard:** Use `get_secret_client()` from unified-trading-services. Primary = Secret Manager; env fallback only for local dev.

**Remediation:** Migrate to `get_secret_client(secret_name, project_id, fallback_env_var="DATABENTO_API_KEY")`.
...
**Risk:** Overly broad `*.json` can accidentally exclude config; `!package.json` etc. create negation complexity. Audit (Feb 2022) flagged `!central-element-323112-e35fb0ddafe2.json` — current instruments-service .gitignore does NOT have this; unified-trading-services .gitignore also clean. **Verify no repo has credential negation.**

**Status:** instruments-service and unified-trading-services .gitignore reviewed — no `!central-element` or `!*credentials*.json`. Quality gates check: `rg "!central-element|!.*credentials.*\.json" .gitignore`.
...
**Status:** `.env` is in .gitignore for instruments-service and unified-trading-services. `.env_template` exists at workspace root (template only). **Recommendation:** Run `git ls-files | grep '\.env'` in each repo to confirm no .env is tracked.
...
| P1 | H1: Migrate scripts to get_secret_client | 4–8 hrs | High |
...
- `.cursor/rules/instruments-domain-and-api-keys.mdc` — API key via get_secret_client
