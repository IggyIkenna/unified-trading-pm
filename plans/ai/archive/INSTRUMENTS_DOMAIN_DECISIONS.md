...
**Decision:** All instrument reads go through `InstrumentsDomainClient` from unified-domain-client. No duplicate `_load_instruments_by_venue`, no direct GCS reads, no service-specific implementations.
...
**Decision:** If moving to InstrumentsDomainClient would reduce functionality for a service, add that functionality to InstrumentsDomainClient (unified-domain-client). Never add fallbacks, bypasses, or backward-compat layers in the service.
...
**Decision:** All API keys via `get_secret_client`. No `os.environ.get` for API keys in production code or scripts.
...
| Use `os.environ.get` for API keys in scripts | Use `get_secret_client` |
