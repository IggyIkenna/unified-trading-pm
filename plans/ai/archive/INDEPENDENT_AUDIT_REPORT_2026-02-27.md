...
| 2 | SECURITY | GOOGLE_CLOUD_PROJECT as primary / fallback | `market_data_tick_handler/config.py:31,46`; `config_base.py:25,40,62`; `execution_service/venues/initializer.py:57`; `dependency_checker.py:215`; `grid_generator.py:1723`; `instruction_api.py:47`; scripts (manage-secrets, download_upbit, data_catalog, etc.) |
...
| 4 | SECURITY | Config: required via typed class; no empty fallbacks | Same as above; `execution_service/venues/initializer.py:127-128` (`api_key or ""` pattern) |
| 5 | CODE QUALITY | Bare `except:` or `except Exception: pass` | `unified-trading-codex/.../unified_logging_service.py:281,289,300`; `execution_service/results/extractor.py:196-197`; `execution_service/venues/deribit.py:181-182,229-230,315-316,443-444`; `execution_service/backtest/preflight.py:1312-1313`; `twap_adapter.py:290-291`; `execution_service/backtest/engine.py:185-186,1285,2541-2542`; `evaluator.py` (many); `unified_market_interface/adapters/defi/euler_adapter.py:379-380,402-403,409-410` |
| 6 | CODE QUALITY | try/except ImportError fallback imports | `features-delta-one-service/.../orchestrator.py:296,324`; `market_data_tick_handler/.../tardis_client.py:34`; `futures_orchestrator.py:23`; `dependency_checker.py:256`; `instruments_service/.../cloud_instrument_storage.py:34`; `execution_service/strategy_instructions/gcs.py:17`; `execution_service/venues/initializer.py:79`; `instrument_resolver.py:35`; `execution_service/results/serializer.py:24`; scripts (ensure_test_buckets, cleanup_gcs_bucket, etc.) |
| 7 | CODE QUALITY | Files >1500 lines (≥900 = FAIL per codex) | `execution-service/execution_service/backtest/engine.py` (2827+ lines) |
| 8 | CODE QUALITY | `: Any` / `-> Any` in non-test source | features-delta-one-service, market_data_tick_handler, instruments_service, execution_service (runner, results, config_builder, backtest engine, evaluator, etc.), unified_domain_client, unified_trading_services, api-contracts, execution_results_api (see agent 1 table) |
| 9 | CODE QUALITY | `from typing import List/Dict/Tuple` | 46+ files (instruments_service, execution_service, features-delta-one-service, market_data_tick_handler, etc.) |
...
| 11 | CODE QUALITY | `print(` in production (non-test) | market-tick-data-handler/scripts/run_quality_gates.py (50+); manage-secrets.py; execution-service/scripts/utils/prefetch_data.py:210 |
| 12 | CODE QUALITY | datetime.now() / utcnow() without timezone.utc | instruments-service (cloud_data_provider, orchestrator_helpers); execution-service validation.py; strategy-service (output_builders, cloud_data_provider); ml-training-service; unified-trading-deployment-v3 (reporting, status_service, reporting_handler, deployment_handler, maintenance_handler, calculation_handler, deployment_validation) |
| 13 | CODE QUALITY | pip install in scripts/Dockerfiles | market-tick-data-handler (setup.sh, quality-gates.sh); execution_service (serializer, checker, live_execution_handler, execute_handler); execution-service scripts (cleanup_gcs_bucket, check_tradfi_data) |
| 14 | CODE QUALITY | Hardcoded project ID / central-element | market_data_tick_handler/config.py; scripts (manage-secrets, data_catalog, migrate_gcs_structure, analyze_gcs_file_sizes); execution-service (instruction_api, upload_backtest_results_to_gcs, run_fresh_backtest, run_backtest_with_log, gcs_cache_helper, grid_generator, backtest.py); tests (test_catalog_gcs_simple.py, test_gcs_service.py) — use `test-project` |
...
| 17 | SCHEMA | validate_timestamp_date_alignment before writes / no silent pass-through | `execution_service/strategy_instructions/gcs.py:85`; `execution_service/results/serializer.py:626`; `execution_service/results/result_formatter.py:479` — write allowed when validator missing/optional |
...
| 19 | TESTS | test-project vs real project ID | `unified-trading-services/tests/conftest.py:102` ("central-element-323112"); execution-service tests (test_catalog_gcs, test_gcs_service) — use `test-project` |
...
| CODE QUALITY | time.sleep in live path | execution_service/engine/modes/live/data_sink.py:202,205 |
...
| except Exception: pass | FAIL | execution_service (extractor, deribit, backtest, evaluator, etc.); codex; UMI euler_adapter |
...
| pip install | FAIL | market-tick-data-handler, execution_service scripts/Dockerfiles |
...
| print( | FAIL | market-tick-data-handler scripts; execution-service script |
...
4. **P1 — Code quality**
   - Split `execution_service/backtest/engine.py` (2827 lines) per SRP (max 900 lines).
...
6. **P1 — Schema & validation**
   - Ensure `validate_timestamp_date_alignment()` is called before every GCS write in execution-service; remove optional skip (gcs.py, serializer.py, result_formatter.py).
