...
4. **API Keys via `os.environ.get`**
   ...
   - **Fix:** Use `get_secret_client` from unified-trading-services
...
1. **Test Infrastructure Failure**
   ...
   - **Root cause:** `unified-trading-services` circular import in `MarketDataDomainClient`
...
4. **Top 10 Untested Critical Functions**
   ...
   6. Storage/secret client factory (unified-trading-services)
...
1. **`os.getenv` in Production Code (~40 instances)**
   - **Locations:** unified-trading-services (~25), market-tick-data-handler (~15)
...
1. **Test Infrastructure Failure** (18+ repos blocked)
   - Fix circular import in `unified-trading-services.MarketDataDomainClient`
...
| Task | Effort | Owner |
|------|--------|-------|
| Fix circular import in unified-trading-services | 2 days | Backend |
...
1. **Test Infrastructure Failure**
   ...
   - **Mitigation:** Fix circular import in unified-trading-services (P0)
...
2. **Fix circular import** in unified-trading-services (blocks 18+ repos)
