...
| `unified-trading-services/unified_trading_services/__init__.py` | 37–49 | `except ImportError: pass` for dotenv, gcsfuse (optional deps) |
| `unified-trading-services/unified_trading_services/core/async_gcp_clients.py` | 24–31 | `except ImportError:` → `GCLOUD_AIO_AVAILABLE = False` (lazy optional dep) |
...
**Assessment:** ImportError fallbacks for optional deps (dotenv, gcloud-aio) are acceptable. The `batch_handler` fallback for `should_skip_date`/`get_earliest_valid_date` hides missing `unified-domain-client` and returns `False`/`"1970-01-01"` — **P1**: should fail or log clearly.
...
| `unified-trading-services/unified_trading_services/core/async_gcp_clients.py` | 236–237 | `upload_batch`: `return [not isinstance(r, Exception) for r in results]` — failures not logged |
| `unified-trading-services/unified_trading_services/core/async_gcp_clients.py` | 250–252 | `download_batch`: `return [None if isinstance(r, Exception) else r for r in results]` — failures not logged |
...
| 6 | `unified-trading-services/.../async_gcp_clients.py` | 236 | `upload_batch` — failures not logged | **P1** |
| 7 | `unified-trading-services/.../async_gcp_clients.py` | 251 | `download_batch` — failures not logged | **P1** |
...
| 19 | `unified-trading-services/.../async_gcp_clients.py` | 78–79 | `AsyncGCSBlob.exists` returns False on error — no log | P2 |
...
```python
try:
    from unified_domain_client.date_utils import get_earliest_valid_date, should_skip_date
except ImportError:
...
**Problem:** If `unified-domain-client` is missing, code silently uses stubs. Date filtering and validation may be wrong.

**Fix:** Fail at import or log a clear warning and fail at runtime when these are used.

### 3.4 unified-trading-services async_gcp_clients upload_batch/download_batch
...
1. **P1 – Fix immediately**
   ...
   - `features-volatility-service` batch_handler: Remove ImportError fallback or fail clearly when `unified-domain-client` is missing.
   - `unified-trading-services` `upload_batch`/`download_batch`: Log each failed task with blob path and exception.
