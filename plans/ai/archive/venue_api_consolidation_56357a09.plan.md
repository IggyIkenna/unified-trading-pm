...
**Problem:** Only unified-trading-services has auto-version-bump pre-commit hook. Without it, code changes without version bumps cause Artifact Registry publish failures ("entity already exists").
...
(unified-trading-services already has it)

### Implementation per Repo

**1. Copy bump script:**

Copy [unified-trading-services/scripts/bump-library-version.sh](unified-trading-services/scripts/bump-library-version.sh) → `scripts/bump-library-version.sh` in each repo.
...
Copy the 21 venue-specific fields from [unified-trading-services/unified_trading_services/core/config.py](unified-trading-services/unified_trading_services/core/config.py) lines ~320-466.

### 1.2: Remove Venue Fields from UnifiedCloudServicesConfig

**File:** [unified-trading-services/unified_trading_services/core/config.py](unified-trading-services/unified_trading_services/core/config.py)
...
2. **unified-trading-services:** 1.9.0 → **2.0.0** (removed 21 venue config fields - BREAKING)
...
2. unified-trading-services 2.0.0 (lean config)
...
graph TB
    ...
    subgraph UCS[unified-trading-services v2.0.0]
