...
**Root cause of most failures:** `unified-cloud-services` has circular import (`MarketDataDomainClient`), and several repos have `IndentationError` in conftest (line 40). Fixing these will unblock ~18 repos.
...
| Repo | Error |
|------|-------|
| unified-cloud-services | Circular import: `cannot import name 'MarketDataDomainClient'` |
| unified-config-interface | 9 collection errors (unified_cloud_services dependency) |
| unified-events-interface | 5 collection errors |
| unified-domain-services | 9 collection errors |
...
| unified-cloud-services | Storage/secret client factory | Used by all services |
...
- **unified-cloud-services**: No `test_event_logging.py` (library; codex requires it for services only)

### 5.2 conftest.py Presence

| Has conftest | Repos |
|--------------|-------|
| ✅ Yes | instruments-service, unified-cloud-services, risk-and-exposure-service, pnl-attribution-service, market-tick-data-handler, strategy-service |
...
- **Undermocking:** Several services import real `unified_cloud_services` in conftest — causes cascade failures when UCS is broken
...
2. **instruments-service, market-data-processing-service, position-balance-monitor-service**
   - Fix IndentationError in conftest.py line 40 (likely from `unified_cloud_services` import)
...
1. Fix unified-cloud-services circular import and IndentationError — unblocks 18+ repos
