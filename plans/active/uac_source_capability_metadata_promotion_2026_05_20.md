...
- [ ] **P0. Inventory** the 70 SourceCapability instances. Output:
...
- [ ] **P0. Harvest from venue_launch_dates.py** — map venue → coverage_start dict per data_type. Output appended to
      inventory CSV.
- [ ] **P0. Harvest from data_source_continuity.py** — map (source, data_type) → start_date. Cross-reference + flag
      mismatches with venue_launch_dates.py.
- [ ] **P1. Per-venue web research for gaps** — for each venue with no coverage_start in either file, probe the venue's
      REST API for the earliest available data (canonical pattern from Extended research). Cap at 30 venues; rest go to
      a follow-up.
...
- [ ] **P1. Document the integration** in `codex/02-data/availability-manifest-and-data-status.md` § "Reason taxonomy" —
      link the new field to the existing `EXPECTED_PRE_SOURCE_COVERAGE_START` reason. (Phase 5)

### Phase 5 — Codex SSOT + post-migration cleanup

- [ ] **P0. Update** `codex/02-data/contracts-scope-and-layout.md` to document the 4 new fields + their canonical
      values.
...
- [ ] **P1. Prune duplicated info** in capability_declarations docstrings — once Phase 4 confirms structured fields are
      ...
      human-readable context paragraphs.
- [ ] **P1. SUPERSEDED banner** on `venue_launch_dates.py` if Phase 4 fully replaces its read sites. If not, leave it as
      a secondary index — both `venue_launch_dates.py` and `SourceCapability.coverage_start` can co-exist with
      `coverage_start` as the SSOT (cite the SSOT in venue_launch_dates docstring).
