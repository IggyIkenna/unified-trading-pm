...
| `dex_pools_handler.py`'s parallel Solana writer — still scheduled?                                                        | Not done — unverified whether this second, cruder (always-bare-address) writer is still actively cron'd; NOT covered by the symbol-collision fix above (that only touches `_solana_row_symbol`, which this second writer never calls).                                                                                                                                                     | Nobody — needs a quick read of `main.py`'s CLI dispatch table to resolve.                                                                                             |
