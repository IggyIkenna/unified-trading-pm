...
| ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Glued-id manifest rebuild verify + delete `_migrated_` markers                                                                                                                                                                                                                            | **REBUILD FULLY COMPLETE 2026-07-23 -- reconciled two parallel editing sessions on this exact row (see below), both were the same overall effort viewed from different agent instances, not two separate owners.** All shards verified terminal with rc=0: 2020-01-01..2022-04-29 (original run), 2022-04-29..2024-10-07 (`-194751`), 2024-10-08..2024-11-19 (`-075807`, 532,261 shards), 2024-11-20..2024-12-31 (`-075900`, 510,508 shards), 2025-01-01..2025-06-30 (`-075224` on-demand, 4,957,616 shards), 2025-07-01..2025-08-09 (the ORIGINAL `-073738` run, BEFORE its preemption -- verified via its log: completed cleanly through date=2025-08-09, 25,400 shards that day -- the "unclaimed gap" flagged in an earlier edit of this row was a false alarm, it was never actually a gap), 2025-08-10..2025-12-31 (`-080250` resume, 3,578,528 shards), 2026 full year (`-083424` after one auto-recovered preemption, 1,910,446 shards). No date range is missing. **Corrected auto-recovery finding** (softening an earlier overclaim in this log): the real root cause has TWO independent parts, not one -- (a) the `dp-exit-code-monitor` cron dead ~83h (fixed this session, `deployment-service@e6ae98c5b`), AND (b) `launch-canonical-migration-vm.sh` never called `lc_write_preemption_signal_file` at all, so the durable PREEMPTED blob the `RelaunchPreemptedVm` actuator keys on was never written for ANY canonical-migration VM regardless of (a) -- found + fixed by the concurrent work on this same plan, `deployment-service@a32360a` (also fixed a STOP-vs-DELETE termination-action mismatch that would have made a same-name relaunch fail with "already exists"). Full writeup: `issues/vm_fleet_preemption_autorecovery_gap_2026_07_23.md`. Given (b) was still landing partway through this session's preemptions, some of the earlier-observed correct-checkpoint relaunches in this log were most likely manual (by the concurrent agent instance working this same plan) rather than genuinely automated -- this will only be cleanly verifiable on the next real preemption after both fixes are live. **Bonus finding**: the 2025-H1 rebuild's per-data_type breakdown surfaced live counts for the legacy `dex_pools` (241,247 rows) and `perp_mark_price` (198 rows) data_types flagged in the 2026-07-23 census conversation -- confirms those legacy-named objects genuinely exist on disk for this range, and the rebuild (scanning the GCS hive path directly, not the original writer's manifest calls) retroactively gave `perp_mark_price` real manifest visibility as a side effect, independent of the separately-tracked derivative_ticker migration.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          | Next: verify 0 glued ids in the fresh consolidated manifest, then delete `_migrated_*` markers (both gated on this rebuild, now unblocked).                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
