...
- [ ] [CODE] P1. **NEW 2026-07-23 — the "writer emits the wrapped filename" gap above is not cosmetic: it is an ACTIVE
      data-conflation bug for Solana concentrated-liquidity pools, confirmed with live evidence.** Found while
      ...
      "symbol" key), always falling back to the bare pool address; unclear whether this second writer is still actively
      scheduled (not verified this session). **Scope of fix**: wire `_solana_row_symbol` (and/or `write_defi_rows`'s
      DeFi/POOL `instrument_id` construction generally) to glue in the same fee/tick-spacing discriminator the catalogue
      already computes — either by consulting IS's `build_pool_identity` directly, or by replicating its discriminator
      rule locally against the row's own `tick_spacing`/`fee_rate_bps` columns (note: `fee_rate_bps` alone is
      insufficient for Raydium — 2 of the 3 live `AKE-USDC` pools sampled share an identical `fee_rate_bps`/`pool_type`;
      only the pool-address-anchored discriminator is fully unique). Confirm/resolve whether `dex_pools_handler.py`'s
      writer is still live and needs the same fix or should be retired in favor of one writer. **Full scope, not just
      the code fix (self-audit 2026-07-23 — these 3 were originally missing)**: (a) **migration for ALREADY-WRITTEN data
      — CHECKED 2026-07-23, CLEAN: zero production shards corrupted, because zero were ever written.**
      `uts-prod-mtds-collect-solana-defi-cron` is `PAUSED` (confirmed via
      ...
      there is nothing to re-split. **This becomes an explicit pre-resume gate, not a closed item**: added to the
      "Resume the paused DeFi crons" todo (Track 8) so the symbol fix must land before `collect-solana-defi` resumes, or
      day-one of resumed capture starts producing the exact conflation this todo describes; (b) **manifest impact** —
      once the symbol/`instrument_id` convention changes, existing `capture_status` cells keyed by the OLD bare-symbol
      form no longer match the new discriminated form; this is a de-facto key rename and needs the SAME
      backfill/re-registration treatment as any other instrument-id migration in this plan (see the POOL/LENDING
      fallback-elimination item above, step 5, for the pattern), not a silent drop; (c) **naming-doc update** —
      `codex/02-data/defi-canonical-naming-ssot.md` documents the current bare-symbol filename grammar and must be
      updated once the discriminated form ships, or the SSOT drifts from the code the moment this lands. (repos:
      market-tick-data-service)
...
| ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Glued-id manifest rebuild verify + delete `_migrated_` markers                                                                                                                                                                                                                            | **FULL 2020-2026 REBUILD COMPLETE (prior tick) -- verify/delete pass 2026-07-23 (this tick), sub-agent, PARTIAL + one important data-safety finding.** **Part 2a DONE**: exact total `_migrated_*` count via server-side `match_glob` listing (`google-cloud-storage list_blobs(..., match_glob=".../day={D}/**/_migrated_*.parquet")`, day-sharded 2020-01-01..2026-07-23, 40 workers, 139s wall) = **356,314 markers** (2,392/2,396 days have >=1). By `data_type`: dex_pool_state 125,937, liquidations 68,263, lending_indices 50,338, dex_swaps 31,770, dex_pool_swaps 21,010, lst_rates 19,178, gas_fees 14,868, perp_funding 8,065, vault_share_price 7,327, oracle_prices 7,216, perp_daily_ctx 1,109, risk_params 854, perp_mark_price 316, dex_pools 63; top venues AAVE_V3 57,647 / UNISWAP_V3 42,002 / BALANCER 34,403 / COMPOUND_V3 34,049 / GMX 27,505. **Part 1 (0-glued-id check) IN PROGRESS, real measured partial evidence, not yet complete**: row-group-projected scan (pyarrow native `GcsFileSystem`, 8-of-42 columns, `asset_group=defi`+`capture_status=captured`) applying the migration script's OWN `is_bundled_batch_leaf()` predicate to every row's `instrument_id` -- as of last check **100/191 row groups (~52%) scanned, 12,258,098 defi-captured rows checked, 0 glued found so far**; still running, no ETA committed (the consolidated index is under active concurrent rewrite this session -- row/row-group counts shift slightly between opens, and one earlier attempt hit a hard `NOT_FOUND` mid-scan at row group 21/192 for exactly that reason; current attempt is a fresh-retry-on-failure wrapper past that point cleanly). **Part 2b sample safety-verify DONE (250-marker stratified sample by year x data_type)**: SAFE (real sibling leaves, row-count-verified) 132, SAFE_FILECOUNT_ONLY (sibling count > 30, capped per this task's own file-count-is-the-primary-signal guidance) 71, FLAGGED_NO_SIBLINGS (zero same-directory per-instrument leaves) 47 -- **all 47 individually re-investigated by hand, not left as an unexplained flag**: 36/47 are genuinely 0-ROW empty placeholder bundles (nothing to lose), 2/47 are real non-trivial dex_pool_state content (263 + 289 rows, TRADER_JOE_V2/AVALANCHE day=2023-08-24 + VELODROME_V2/OPTIMISM day=2024-05-17) fully content-matched byte-for-byte in the shared `_needs_attribution/day={D}/{data_type}_{D}.parquet` object, and 9/47 are 1-row GMX perp_funding market-aggregate snapshots (`instrument_id` blank -- an unattributable venue-wide funding-rate/OI record, not a per-instrument row) of which **4/9 have a matching `_needs_attribution` twin but 5/9 do NOT** (days 2022-01-25, 2022-04-11, 2022-05-23, 2023-01-10, 2023-04-01) -- **for those 5, the `_migrated_*` marker is the ONLY surviving copy of that row's content**, most likely because a VM run was killed/preempted after `_process_cell` renamed the source but before that run's single end-of-run `_flush_needs_attribution()` call ever executed. **DATA-SAFETY FINDING (flagging per findings-triage, not just noting)**: this proves a same-directory-sibling-only check would have been WRONG to treat "zero siblings" as uniformly either safe or unsafe -- some zero-sibling markers are trivially safe (empty) or safely covered elsewhere (needs_attribution), but a genuine minority (~5/250 = 2% of this representative sample, order-of-magnitude ~7,000 of the 356,314-marker corpus if the rate holds, NOT yet corpus-verified) hold content with no confirmed backup anywhere. The absolute stakes are low (1-row market-wide DeFi perp-funding aggregates, not user funds/critical trading data, plausibly re-derivable from GMX's subgraph/API if historical depth exists) but this is a real, previously-undiscovered gap in the R3 migration's `_needs_attribution` flush reliability across its many separately-launched/preempted VM chunks, worth its own follow-up (not scoped further this tick). **Shipped + re-tested**: `market-tick-data-service/scripts/one_offs/delete_migrated_defi_markers_2026_07_23.py` -- now a proper 3-leg check (0-row short-circuit -> same-directory sibling+row-count -> `_needs_attribution` coverage fallback before ever calling a zero-sibling marker safe), dry-run default, `--apply` gated, discovers via `match_glob`, deletes ONLY `SAFE`/`SAFE_NEEDS_ATTRIBUTION_COVERED` markers via UTL `gcs_delete_object`, resumable via a JSONL log; ruff-clean, and re-verified against all 4 example cases above (0-row SAFE, dex_pool_state SAFE_NEEDS_ATTRIBUTION_COVERED, the 2022-01-25 GMX FLAGGED_NO_SIBLINGS_NO_BACKUP, the 2021-09-03 GMX SAFE_NEEDS_ATTRIBUTION_COVERED) -- all matched expected disposition exactly. Not yet committed as of this edit (landing in the same commit as this row). **HARD-STOP FINDING (read before any `--apply` run)**: `codex/02-data/gcs-and-manifest-delete-safety-protocol.md` hard-stop #1 ("Any prod-bucket delete ... There is no confidence level at which an agent deletes from prod") plus CLAUDE.md's own "prod-bucket deletes are human-only" line mean an agent may **never** invoke `--apply` against this `-prd-` bucket autonomously, regardless of verification confidence -- even the doc's top disposition tier (`yes-twin-confirmed`) is "Human executes; agent suggests." This directly conflicts with this task's original framing ("execute the deletes"); resolved in favor of the HARD RULE -- **no deletes were executed this session, at any confidence level, for any marker**. **Update 2026-07-23 (a different thread on this same row, running concurrently with the Part 1/Part 2 work above -- reconciling, not overwriting)**: independently found + fixed a DIFFERENT, narrower glued-id defect: a fresh full 2020-2026 `defi-rebuild` manifest rebuild (this session, 5-VM parallel, completed earlier this tick) surfaced **34 previously-unindexed captured rows** matching the OTHER glued-id issue's regex (`(_\d{8}_\d{6} | _\d{10})$`, tracked in `issues/defi_lst_oracle_timestamp_glued_instrument_id_2026_07_20.md`) -- all `lending_indices`/`liquidations`(KAMINO/SOLANA x22, 12x single-row protocol/chain liquidations), dated 2026-05-28, i.e. pre-existing GCS objects the manifest simply hadn't indexed yet as of the 2026-07-21 sweep (same 'rebuild surfaces previously-invisible objects' mechanism as this row's own`perp_mark_price`/`dex_pools` bonus finding, NOT a forward-write-path regression -- that fix (`mtds@4ca2640d`) is separately re-verified clean). Fixed via a new, fleet-registered `defi-glued-reshard` launcher category (`deployment-service@3345bf9`, extends `launch-canonical-migration-vm.sh`rather than hand-rolling a VM) running`reshard_glued_defi_ids_2026_07_21.py --apply`on a fresh in-region VM with pinned tarballs: 34/34 processed, 242 new per-instrument twins written, 968 present-twins skipped, 0 unattributable/missing/err, 34 originals retired to`_migrated_`. A follow-up full-corpus manifest rebuild (6-way parallel, gap-checked 2020-01-01..2026-07-22) is running now to reflect this in the index. **Endorses this row's own HARD-STOP finding independently**: no `_migrated_`marker was deleted by this thread either -- retirement-to-marker is the reshard tool's own behavior (proof-gated, same as R3), not a`delete_migrated_defi_markers`invocation, so it does not touch the prod-delete hard-stop. **Environment note for future sessions**: this laptop's network path to GCS was measured too slow this session for any ~100MB+ direct read/download (a 47.8MB single-shard download timed out at 60s; a 923MB full-index download broke mid-stream with`ChunkedEncodingError` after 266MB) -- an in-region GCE VM (same project/zone as the bucket) is the reliable path for this class of check now, not just a nice-to-have. | Part 1's background scan finishing (real progress each check -- next tick reports the final glued-id count over the full ~23.4M-row consolidated index). A **human/operator** then runs `python -m scripts.one_offs.delete_migrated_defi_markers_2026_07_23 --apply` (default full 2020-01-01..today range) from the mtds repo root -- gated on hard-stop #1, cannot be agent-executed regardless of how clean Part 1 comes back. Separately: the 5/9-of-250-sample GMX `_needs_attribution` flush-gap finding above deserves its own small follow-up (either accept the low-materiality loss on `--apply`, since FLAGGED markers are never deleted by this tool regardless, or a targeted backfill/recovery decision) -- operator call, not scoped further here. |
