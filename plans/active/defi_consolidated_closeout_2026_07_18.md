...
| Fake-history relabel-forward migration (todo 3, `defi_solana_dex_pools_fake_history_recurrence_prd_bucket_2026_07_23.md`) | **IN PROGRESS 2026-07-24** — script shipped + validated + a real 429-crash bug found+fixed (`market-tick-data-service@b48a0a4d`, see that issue doc for full detail) + relaunched on ON_DEMAND after 2 SPOT-related losses. Not yet verified complete.                                                                                                                                                                                                                          | Elapsed time — monitor via the real GCS destination-object count (day=2026-05-04/05 canonical `dex_pool_state`), not VM presence (see issue doc for the exact check). |
...
**Recommended next action**: verify the relabel migration to actual completion (real GCS object count, not VM presence —
the VMs have disappeared twice now without a clean terminal signal, most likely SPOT/heartbeat quirks, now on ON_DEMAND
to remove that risk for the remainder). Once complete, the pending-delete report needs human review. Separately, resolve
whether `dex_pools_handler.py` is still scheduled (quick check, no dependencies) before deciding whether
`collect-solana-defi` is safe to resume.
