...
| ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Glued-id manifest rebuild verify + delete `_migrated_` markers                                                                                                                                                                                                                            | **In progress, 2026-07-23 continued (tick 4).** Parallelized the rebuild across 5 date-range-scoped `defi-rebuild` VMs (one each for 2026, 2025 H1, 2025 H2, and 2024 Q4 split in two) instead of one sequential sweep -- safe because the script forces `per_vm_shards=True` (each VM writes its own `_index/per_vm/<vm-name>.parquet`, no CAS race) and the `*/1 * * * *` consolidator cron merges them (confirmed healthy: last attempt 18s before check, canonical index updated 4 min prior). Both 2024 Q4 splits COMPLETED cleanly (`-075807`: 2024-10-08..11-19, 532,261 shards, rc=0, 140.6s; `-075900`: 2024-11-20..12-31, 510,508 shards, rc=0, 137.0s). **Real auto-recovery finding**: `canonical-migration-defi-` IS registered in `VM_PREFIX_TO_BUCKET` (`vm_prefix_registry.py:818`) -- registration was never the gap. The actual gap was the `dp-exit-code-monitor` cron (separately root-caused + fixed this session, `deployment-service@e6ae98c5b`) -- its ~83h outage meant the full `DP_VM_PREEMPTED` -> `escalation.py::route_finding` -> `RelaunchPreemptedVm` auto-recovery chain was silently dark for every preemption in this closeout, forcing manual detection+resume all session. **Confirmed working again live**: a 2025-H2 preemption (`-073738`, completed through 2025-08-09 cleanly) was auto-relaunched as `-080250` scoped `2025-08-10..2025-12-31` -- the EXACT measured resume point, independently matching manual derivation, which only a real checkpoint-reader would produce. **Operational lesson**: manually deleting the ORIGINAL VM (`-194751`, mid-run, no clean `EXIT_STATUS`) made the monitor treat it as an ambiguous/incomplete termination and auto-relaunch a broad catch-all covering its full original remaining range (`2024-10-08..2026-12-31`) -- and deleting THAT relaunch (as redundant with the dedicated splits) triggered ANOTHER one (`-080749`, same range). Don't fight this loop: the redundant catch-all is harmless (idempotent, same per-VM-shard isolation), just leave it running rather than repeatedly deleting it.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              | The four still-running VMs (2026 `-073604`, 2025-H1 `-075224` on-demand after 3 consecutive SPOT preemptions on that slot, 2025-H2-resume `-080250`, and the redundant catch-all `-080749`) reaching terminal state, then verify 0 glued ids + delete `_migrated_*` markers. Separately: still worth adding a PROGRESS.json checkpoint to `defi-rebuild`/`defi-pi-range` specifically (distinct from the now-confirmed-working VM-level preemption auto-recovery) so a resumed VM doesn't need to re-derive its own start point from log content.                                                                                                                                                                                                          |
