...
| ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Glued-id manifest rebuild verify + delete `_migrated_` markers                                                                                                                                                                                                                            | **REBUILD COMPLETE 2026-07-23 (tick 4).** Parallelized the rebuild across 5 date-range-scoped `defi-rebuild` VMs (2026, 2025 H1, 2025 H2, and 2024 Q4 split in two) instead of one sequential sweep -- safe because the script forces `per_vm_shards=True` (each VM writes its own `_index/per_vm/<vm-name>.parquet`, no CAS race) and the `*/1 * * * *` consolidator cron merges them (confirmed healthy: last attempt 18s before check, canonical index updated 4 min prior). All 5 exited rc=0: 2024 Q4a (`-075807`, 43 days, 532,261 shards, 140.6s), 2024 Q4b (`-075900`, 42 days, 510,508 shards, 137.0s), 2025 H2 (`-080250` after one resumed preemption, 144 days, 3,578,528 shards, 2.7h), 2026 (`-083424` after one resumed preemption, 120 days through 2026-07-22/today, 1,910,446 shards, 39min), 2025 H1 (`-075224` on-demand after 3 consecutive SPOT preemptions on that slot, 181 days, 4,957,616 shards, ~5h). Combined ~11.5M shards, full 2020-01-01..2026-07-22 coverage. **Real auto-recovery finding, confirmed via `gcloud compute operations list`**: `canonical-migration-defi-` IS registered in `VM_PREFIX_TO_BUCKET` (`vm_prefix_registry.py:818`) -- registration was never the gap. The actual gap was the `dp-exit-code-monitor` cron (separately root-caused + fixed this session, `deployment-service@e6ae98c5b`) -- its ~83h outage meant the `DP_VM_PREEMPTED` -> `escalation.py::route_finding` -> `RelaunchPreemptedVm` auto-recovery chain was silently dark for every preemption in this closeout, forcing manual detection+resume all session. **Confirmed working again live**: a 2025-H2 preemption (`-073738`, completed through 2025-08-09 cleanly) was auto-relaunched as `-080250` scoped `2025-08-10..2025-12-31` -- the EXACT measured resume point, independently matching manual derivation, which only a real checkpoint-reader would produce. **Operational lesson**: manually deleting the ORIGINAL VM (`-194751`, mid-run, no clean `EXIT_STATUS`) made the monitor treat it as an ambiguous/incomplete termination and auto-relaunch a broad catch-all covering its full original remaining range -- and deleting THAT relaunch (as redundant with the dedicated splits) triggered ANOTHER one (`-080749`, verified via `gcloud compute operations list`: inserted then deleted ~3.5 min later, harmless). Lesson for future sessions: once the auto-recovery system is live, don't fight it by deleting a VM it just relaunched -- it will just relaunch again; either let a redundant catch-all run to completion (idempotent, harmless) or stop chasing it.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          | None -- done. Next: verify 0 glued ids in the now-consolidated index, then delete `_migrated_*` markers. Separately: still worth adding a PROGRESS.json checkpoint to `defi-rebuild`/`defi-pi-range` specifically (distinct from the now-confirmed-working VM-level preemption auto-recovery) so a resumed VM doesn't need to re-derive its own start point from log content.                                                                                                                                                                                                                                                                                                                                                                       |
