...
- **2026-07-22 (session 3, continued) — shipped `available_at` broader fix; recovered a stalled SPOT VM.**
  ...
  **Lesson (5)**: a "MISSING manifest" launch warning is not automatically a real absence — `gsutil stat` the exact path
  ...
  separately this time, so a genuine future stall can't hide behind a ticking heartbeat again).
...
| -------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| Glued-id manifest rebuild verify + delete `_migrated_` markers                                                                                           | Cannot be done yet — root cause found + fixed after 4 recoveries (all idempotent, zero progress lost): an untimed `gcsfs` read inside a `ThreadPoolExecutor` fan-out hung regardless of `--workers` (16/8/1 all hung at the identical year=2025 boundary). Fixed `market-tick-data-service@7dc8dcd6` (stall-timeout + shard-level abandonment, QG-green 6797/0). CURRENT VM is `canonical-migration-defi-per-instrument-20260722-164109` (`WORKERS=8`, on the fixed code), watchdog `bh0ima9ph` (tracks real vs heartbeat-only progress separately)       | The VM reaching a terminal `EXIT_STATUS`. Any future stall now self-resolves via the shipped 600s cell-abandon instead of hanging the whole VM                           |
