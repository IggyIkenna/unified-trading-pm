...
- [ ] [DATA] P0. **Pin the flat canonical path shape** (venue-before-chain, lowercase itype, `pipeline_mode=`); ~~DELETE
      ...
      **2026-07-22 — root cause found, fix implemented + verified, ship pending an external QG precondition (NOT > my
      code, see "Deferred work" table for full evidence).** The historical bare-`0x<address>.parquet` batch > writer
      suspected by `issues/defi_dexpool_second_writer_path_and_zero_capture_2026_07_10.md` was ALREADY fixed >
      2026-07-09 (`mtds@0713c01a`/`0ce28623`) — confirmed dead via a narrow live-GCS read (`day=2026-07-18` CURVE >
      `dex_pool_state` objects are real `TOKEN0-TOKEN1.parquet` symbol names, not addresses). The ACTUAL live second >
      writer: `market_tick_data_service.live.websocket_runner.live_tick_blob_path` (`mtds@3043f2dc1`, 2026-06-26) >
      spliced `chain=` BEFORE `venue=` for every non-cefi asset_group — the reverse of the canonical batch order >
      (`unified_api_contracts.build_defi_partition_path`: `venue={V}/chain={C}/...`) — for the SAME > (asset_group=defi,
      venue, chain, data_type, day) shard. Undetected for ~1 month because > `canonical_path_violations` parsed
      partition segments into a `key→value` dict and never validated ORDER (only > presence/values) — proven empirically
      (a hand-built reversed-order path returned the identical violation list > as the correct order). Fix (in working
      tree, uncommitted): (1) `live_tick_blob_path` reordered to > venue-before-chain; (2) a new defi-scoped structural
      check added to `canonical_path_violations` > (venue-before-chain, lowercase `instrument_type`, `pipeline_mode=`
      position) so this drift class fails loud > going forward — mathematically safe against the real writer (its
      template is unconditional/fixed, verified > across every pipeline_mode × instrument_type × data_type combination +
      the fixed live path, zero violations). > **Why not shipped this session**: `quickmerge --agent` requires a
      fully-green whole-tree `quality-gates.sh` > (HARD RULE) and neither repo can produce one right now for reasons
      independent of this fix — MTDS: a LIVE > concurrent sibling agent's uncommitted `unified-api-contracts` registry
      edit (METEORA/LIFINITY/PHOENIX > phase downgrade, `defi_venues.py`/`venue_adapter_keys.py`/`expected_coverage.py`)
      drifted the pinned > `_PER_AG_SHARD_COUNTS["DEFI"]` test (2673→2592) via the shared editable dependency; UAC: a
      PRE-EXISTING, > unrelated standing defect — `tests/internal/unit/test_archetype_capability_manifest_parity.py` (3
      failures, > codex markdown missing sections for 29 archetypes added by the Phase-9 regen `uac@e5dc6e7f`
      2026-07-21) — the > stale `.qg_last_passed_sha` sentinel (predates `e5dc6e7f`) proves NO ONE has had a clean UAC
      agent-mode QG run > since, branch-wide. **Recommended next step**: retry `quickmerge --agent` for MTDS once the
      concurrent > sibling's WIP lands (imminent — was actively running its own QG at investigation end); the UAC half
      needs > either a wait for the archetype-doc gap to be fixed separately (out of scope here — strategy/archetype >
      domain, not DeFi) or a dedicated follow-up.
...
| ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| Glued-id manifest rebuild verify + delete `_migrated_` markers                                                                                                                                                                                                                            | In progress — fix confirmed WORKING on the real poison cells: year=2025 self-abandoned 157 stuck cells (concentrated ORCA/SOLANA/solana_amm_pool/dex_pool_state, days 2025-12-23..30) after 600s instead of hanging, wrote 3205 real instruments/127019 rows, and the launcher correctly continued into year=2026 (climbing normally). BUT year=2025's errors=157 means `rc_all=1`, so the auto-chained `rebuild_defi_manifest` step will be SKIPPED at the end — needs a manual run once 2026 finishes. Separately need to investigate why those 157 specific cells hang (concentrated cluster, not random) before deciding whether to retry them                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               | year=2026 finishing, then (a) manually run `rebuild_defi_manifest` over the full 2020-2026 range since the auto-chain will skip it, (b) investigate the 157 stuck ORCA/SOLANA cells specifically                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       |
...
| Path shape pin (code portion) + kill second dexpool writer                                                                                                                                                                                                                                | **Root cause found + fix implemented + verified (2026-07-22), NOT YET SHIPPED.** The suspected historical bare-`0x<address>.parquet` writer was already fixed 2026-07-09 (confirmed dead via live-GCS read). The REAL second writer: `live_tick_blob_path` (LIVE DeFi path, `mtds@3043f2dc1` 2026-06-26) put `chain=` before `venue=`, diverging from the canonical batch order — undetected ~1 month because `canonical_path_violations` never validated segment ORDER. Fix in working tree (uncommitted): reordered `live_tick_blob_path` + added a defi order/case/position structural check to the UAC oracle; both verified via targeted pytest (126 UAC + 17 MTDS tests green, incl. 5 new regression tests) and proven mathematically safe against the real single-writer template.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       | Cannot pass the mandatory whole-tree `quality-gates.sh` gate right now for reasons independent of this fix — MTDS: a live concurrent sibling agent's uncommitted UAC registry edit (METEORA/LIFINITY/PHOENIX phase downgrade) drifted the pinned `_PER_AG_SHARD_COUNTS["DEFI"]` test via the shared editable dep; UAC: a PRE-EXISTING, unrelated standing defect (`test_archetype_capability_manifest_parity.py`, codex missing sections for the Phase-9 archetype regen `uac@e5dc6e7f` 2026-07-21 — stale `.qg_last_passed_sha` proves no clean run since, branch-wide). Retry `quickmerge --agent` for MTDS once the sibling's WIP lands (was actively running its own QG at investigation end); UAC needs that archetype-doc gap fixed separately first (out of scope — strategy domain, not DeFi). |
...
| DeFi MVP backfill to 100%                                                                                                                                                                                                                                                                 | **Cannot be done yet — gate re-verified 2026-07-22, genuinely still open, NOT stale.**                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           | C-GREEN gated on Track 1 (LENDING migration + canon walk + case/venue-unify rows above, all open) + Track 2 (path-shape-pin row above) + Track 3 (the "Purge 1.79M dup…" row above, sequenced after the running migration VM, PLUS the non-POOL-EU-reconciliation `[DECISION]` — the **one genuine operator/architecture decision-gate** in all of Track 1–3; everything else cited is execution work, not an open decision). Backlog task `mvp_backfill_defi_onchain_v10-001` stays parked (`priority: 999`) until `defi_onchain_v10_universe_v2_seed_or_backfill_progressed` flips true per Track 5's own unpark note — not yet, the historical migration (R3) is still RUNNING.                                                                                                                     |
| Non-POOL EU terminal-state decision + oracle dead-venue (METEORA/LIFINITY/PHOENIX) honest-empty path                                                                                                                                                                                      | Not done                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         | None structurally — note METEORA/LIFINITY/PHOENIX are separately tracked as measured-dead upstreams in `issues/uac_is_defi_oracle_dex_adapter_drift_2026_07_20.md`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
| Checker collect-* fleet-wide real-VM-launch verification                                                                                                                                                                                                                                  | **SCOPED 2026-07-22, GO — not executed** (sub-agent scoped to recommend, not execute). Evidence the dirty-tarball blocker is ALREADY GONE: `gs://deployment-scripts-central-element-323112/vm/setup-data-pipeline-vm.sh` (live fleet-shared script, uploaded 2026-07-22T16:30:56Z) is byte-identical to local deployment-service HEAD `521f0643`, which has the collect-\* fix `56a451f8` as a confirmed ancestor; `mtds-code.manifest.json` shows `commit_sha=ce773ee4`/`git_status_clean=true`, exactly matching local MTDS HEAD. "Fleet-wide" is the fix's blast radius (the fleet-shared setup script), not a 2403-shard sweep — the issue doc's own scoped ask is 2 commands (~3 short-lived self-deleting VMs): (1) `bash scripts/vm/launch-mtds-backfill-vm.sh --asset-group DEFI --venues AAVE_V3 --data-types lending_indices --start 2025-03-12 --end 2025-03-12 --test-run` confirming `run.log` shows `op=collect-evm-defi` (not the `Skipping 98 DeFi venues` line), then (2) `/data-pipeline-check-mtds --asset-group DEFI --venue AAVE_V3` (force+skip legs). Verified SAFE alongside the running canonical-migration VM(s) (`canonical-migration-defi-per-instrument-20260722-174809` + `canonical-migration-defi-cdlap-20260722-175209`, both RUNNING at check time): different `VM_TASK` (`mtds-backfill` vs `canonical-migration`, no shared code branch), different bucket (`IS_TEST_RUN=true` → `market-data-tick-test-defi`, isolated from the prod `market-data-tick-defi-prd` bucket the migration walks, so no manifest/index race), and DeFi collect-\* is TheGraph/RPC-sourced (not Tardis — the N=1 Tardis cap doesn't apply). **Run now, no need to wait for the migration VM.**                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    | None — ready to execute from any slot; see Progress Log entry 2026-07-22 (checker collect-* verification scoped) for full evidence.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    |
| `uac_is_defi_oracle_dex_adapter_drift_2026_07_20.md` stashed WIP disposition (instruments-service `stash@{0}`)                                                                                                                                                                            | Not done — needs a human/agent to diff-confirm superseded, then drop                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             | None structurally; owned by that issue doc                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             |
