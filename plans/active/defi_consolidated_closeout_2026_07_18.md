...
| Item                                                                                                                      | State / why deferred                                                                                                                                                                                                                                                                                                                                                                                                                                                                             | Blocked on                                                                                                                          |
| ------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ----------------------------------------------------------------------------------------------------------------------------------- |
| Orphan-sweep VM (`orphan-sweep-defi-20260723-043605`) reaching ACCEPTANCE                                                 | **DONE 2026-07-23/24** — completed, wrote 15,865,384 orphan-E + 1,080 legacy-B = 15,866,464 actionable rows to `orphan_sweep_defi.parquet`. VM self-deleted per `VM_SHUTDOWN_ON_COMPLETION=true` (`exit_code=1` is the tool's OWN by-design convention for "found actionable rows", not a crash — see `migration_orphan_sweep.py::main()`'s `return 0 if (n_e==0 and unknown==0) else 1`).                                                                                                       | —                                                                                                                                   |
| Fake-history relabel-forward migration (todo 3, `defi_solana_dex_pools_fake_history_recurrence_prd_bucket_2026_07_23.md`) | **DONE 2026-07-24 ~12:09 UTC** — all 4 ON_DEMAND shards terminated cleanly (`exit_code=0`), processed counts sum to exactly 241,281 (the full measured population, proving no shard silently under-ran). Real destination footprint: 28,435 distinct objects (most of the 17 fake-day duplicates converge onto the same true dates, as hypothesized). Pending-delete report verified present with all 241,281 rows logged — needs HUMAN review before any old-object deletion (never automated). | —                                                                                                                                   |
| Get TRUE final fake-history scope (todo 2, same doc)                                                                      | **DONE 2026-07-23** — a targeted bounded walk across the original bug's own documented date range proved exactly 34 (day,venue) combinations, independent of the sweep. Real object count also measured directly: 241,281.                                                                                                                                                                                                                                                                       | —                                                                                                                                   |
| cefi/prediction timestamp-provenance audit (todo 5, same doc)                                                             | **DONE 2026-07-23** — 10 cefi + 30 prediction samples checked against real `uri` (not `dest_path`), zero fabrication evidence in either.                                                                                                                                                                                                                                                                                                                                                         | —                                                                                                                                   |
| Solana AMM symbol-collision code fix (Track 8 `[CODE] P1` todo above)                                                     | **DONE 2026-07-24** `market-tick-data-service@0d83a8a9` — `_pool_fee_discriminator()` wired into `_solana_row_symbol`, 8 new tests, QG green. The `collect-solana-defi` pre-resume gate is now cleared; resuming the cron itself is still an operator decision.                                                                                                                                                                                                                                  | Manifest re-registration (b) + naming-doc update (c) from the todo above remain open, but are moot until the cron actually resumes. |
| `dex_pools_handler.py`'s parallel Solana writer — still scheduled?                                                        | **RESOLVED 2026-07-24** — `DexPoolsHandler` maps to CLI op `collect-dex-pools`, its OWN scheduler job `uts-prod-mtds-collect-dex-pools-cron` is `PAUSED` (same 2026-06-08 pause as `collect-solana-defi`) — still registered/live, NOT retired, zero immediate risk while paused. NOT covered by the symbol-collision fix (only touches `_solana_row_symbol`, which this handler never calls) — needs its own fix (or a retire decision) before its cron is safe to resume too.                  | Nobody further — resolved; fix-vs-retire is a design call for whoever resumes `collect-dex-pools`.                                  |

**Recommended next action**: every item in this table is now DONE or RESOLVED — the only remaining human-owned step is
reviewing `_index/audit/dex_pools_fake_history_pending_delete.parquet` (241,281 rows) before deciding whether/when to
delete the old mislabeled objects, and separately deciding whether to resume `collect-solana-defi` / `collect-dex-pools`
now that their code-side gates are cleared (both are live-production cron restarts — operator decisions, not something
...
**Follow-up to the parallel-write fix above (mtds@2ba4226e): two more iterations needed before it was genuinely fixed.**
Production verification exposed real gaps the isolated single-cell test couldn't:

1. **v1 (mtds@2ba4226e) confirmed on ONE isolated cell** (day=2025-12-23, ORCA/SOLANA/dex_pool_state, 14,093
   instruments): `DONE cells=1 ... errors=0 wall=314.5s` — clean. But relaunching the remaining 8 poison days
   (2025-12-24..31) TOGETHER re-stalled ALL of them: `STALL: 8 cell(s) exceeded 600s with no completion`. Root cause:
   `_process_cell` created a FRESH `ThreadPoolExecutor(max_workers=16)` per large cell, and the OUTER cell-level pool
   (`--workers 16`) dispatched all 8 concurrently — up to 8×16=128 threads oversubscribing one VM.
2. **v2 (mtds@57c05125)** replaced the per-cell pool with ONE shared, run-lifetime `leaf_pool`
   (`_LEAF_WRITE_POOL_SIZE = 16`) threaded through `_run_cells`→`_process_cell`. Re-verified on the same 8-day range:
   IMPROVED but still partial — `DONE cells=3 ... instruments_written=42279 errors=5 wall=1999.3s`. Root cause:
   `ThreadPoolExecutor.map()` submits a cell's ENTIRE batch (~14k items) to the shared pool in one eager call; whichever
   cell's batch hit the FIFO queue first monopolized all 16 workers for a long time, starving the other cells' outer
   futures at ZERO completions (classic batch head-of-line blocking).
3. **v3 (mtds@826cd6c1)** chunked each cell's own submission into `_LEAF_WRITE_CHUNK_SIZE = 8`-item waves instead of one
   giant `.map()` call, so concurrent cells interleave fairly through the shared pool. Re-verified on the same range at
   the DEFAULT 600s stall timeout: WORSE-looking on paper — `DONE cells=0 ... errors=5 wall=600.4s` (STALL fired at
   almost exactly 600s). This was NOT a new bug: fair-sharing 16 workers across 5 concurrent giant cells genuinely takes
   each cell longer than running alone (my isolated 314s baseline had the FULL pool to itself), so the 600s default —
   calibrated for a SINGLE large cell — was simply too tight for 5 CONCURRENT large cells sharing it. Relaunched the
   identical range with `--stall-timeout-seconds 3600` (the CLI knob shipped earlier this session, `mtds@5218837e`,
   built exactly for this):
   **`DONE cells=5 files_scanned=5 files_split=5 instruments_written=70465 rows=70465 needs_attribution=0 errors=0 wall=1573.9s`**
   — every remaining cell split cleanly, zero errors.

**Net result — VERIFIED on real production infrastructure, not just unit tests**: the ORCA/SOLANA/dex_pool_state
day=2025-12-23 cell (14,093 instruments, v1 alone) + the day=2025-12-24..31 cells (5 with actual data; the other 3 of
the 8 calendar days had zero ORCA/SOLANA dex_pool_state objects, hence never appeared in any run's cell count) all now
split with 0 errors — 84,558 total instruments written across the two verification runs. The remaining discrepancy
against the manifest audit's earlier "9 residual" count (1 + 5 = 6 confirmed-and-fixed here) is not chased further — the
true count is whatever `rebuild_defi_manifest` finds on its next run now that these source bundles are fixed.

**Lesson for future large-cell migration work**: a shared, size-capped worker pool alone is necessary but not sufficient
for fairness across concurrent big batches — it also needs CHUNKED submission (bound how much of the pool any one caller
can occupy at once), and the stall-timeout must scale with how many large cells are expected to run concurrently, not
just with one cell's own size. All three fixes + this finding are captured in
`issues/mtds_defi_migration_cell_stall_untimed_gcs_read_2026_07_22.md`.

Next: re-run `rebuild_defi_manifest` and re-check the glued-id count for these specific days now that their source
bundles are genuinely split (folds into the existing "verify 0 glued ids" + "purge phantom rows" todos above).

**"Next" step above DONE**: ran `rebuild_defi_manifest --start-date 2025-12-23 --end-date 2025-12-31` (VM
`canonical-migration-defi-rebuild-20260724-113048-rbverify`, pinned `mtds@826cd6c1`) — completed cleanly (`exit_code=0`,
`elapsed_s=854.1`, 226,884 total shards scanned across all 9 days, 198 write_errors unrelated to this scope — see the
per-venue breakdown: ORCA alone contributed 126,837 rows). Applied the migration tool's own `is_bundled_batch_leaf()`
predicate to this VM's per-VM manifest shard, scoped to `venue=ORCA, chain=SOLANA, data_type=dex_pool_state` (126,837
rows): **0 glued-id rows** — confirms the v1→v3 fix is clean end-to-end, from GCS object names through to the rebuilt
manifest index, for every one of the 9 previously-poison days. This closes out the ORCA/SOLANA residual-cell thread
started earlier this session; only the separate, already-tracked 34-row phantom-index case and the 12-row `liquidations`
empty-marker case (both documented in `issues/defi_lst_oracle_timestamp_glued_instrument_id_2026_07_20.md`) remain open
against the broader "0 glued ids" goal, and neither is a migration-tool defect — they're the phantom-row-purge and
empty-marker-cron cases respectively.

**The `liquidations` empty-marker case (12 rows) is code-fixed and ready to ship** — 6 handlers' timestamp-glued
empty-marker filenames dropped, `ROWCOUNT_VERIFIED_DATA_TYPES` expanded so the manifest rebuild routes future 0-row
shards to honest-absence instead of CAPTURED (`market-tick-data-service` commit local, not yet pushed — see the
`/autonomous` session note below). Once shipped + a single-day (`2026-07-22`) targeted rebuild runs to reclassify the 12
already-existing rows, the closeout's "0 glued ids" goal should be fully met (ORCA: done above; liquidations: fix ready;
the 34-row phantom-index case is structurally separate, gated on the phantom-row purge below, not blocking).

**Phantom-row purge (1.79M dup + ~219.5K phantom rows) — assessed this session, NOT attempted; genuine pre-requisite
measurement work needed first, not a "run a known command" task.** Traced the provenance of this figure:
`issues/defi_expected_unattempted_backlog_1m_2026_07_03.md` documents a related-but-DISTINCT incident
(`defi_manifest_consolidator_duplicate_race_2026_07_10.md`, now archived/resolved) where the manifest consolidator's
incremental merge had zero self-dedup on untouched canonical rows — root-caused + fixed
(`unified-trading-library@0de04b6e`), verified "zero genuine duplicates in the live defi manifest, 14,023,022 rows" AS
OF 2026-07-10. That was two weeks and a full 6-VM corpus rebuild ago — the CURRENT 1.79M dup / 219.5K phantom figures
this plan cites are a LATER, more recent measurement (exact source script not identified this session) and may or may
not still be accurate post-rebuild. Compounding this: the doc's own history shows this exact area (expected_unattempted
backlog scale) has moved 46× between two operator-approved checkpoints (1.38M → 63.9M) purely from organic catalog
growth during the SAME investigation window — i.e., this is a demonstrated moving target, not a stable number to purge
against blind. **Decision: do not attempt the purge or the 63.9M seed this session.** This needs a dedicated pass that
(1) re-measures the current dup/phantom counts fresh against the post-rebuild manifest, (2) confirms the purge tool/
script (not identified this session — no `reconcile_phantom_manifest_rows_all.py`-shaped script was found already scoped
to defi dup+phantom rows specifically), and (3) re-confirms the 63.9M seed figure is still current before any write.
Given the stakes (tens of millions of manifest row writes) and this area's proven history of stale-scale surprises, this
is exactly the kind of judgment call that should get a fresh measurement, not a rushed unattended execution — left as
`- [ ]` for a dedicated follow-up, per AUTONOMOUS_AGENT_RULES rule 1 (least-bad path documented, not silently skipped).
