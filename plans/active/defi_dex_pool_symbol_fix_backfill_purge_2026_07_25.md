...
> **🟢 COMPLETE 2026-08-02 — ARCHIVED.** All 5 todos landed (query fix, live-indexing test, backfill, both purge
> categories) and independently re-verified. Reconciled + archived by
> `/plans/archive/2026_08/defi_dex_pool_symbol_fix_backfill_purge_finalize_2026_07_25.md`; moved to
...
- **2026-07-28 (slot-14) — category-2 apply run CONFIRMED complete + independently re-verified; category-1 (marker
  purge) dry-run in progress.**
  - Followed the checkpoint's own numbered resume recipe exactly: (1) `gcloud config set account unified-trading-sa@...`
    first (the monitoring-lesson fix); (2)
    `gcloud compute instances describe canonical-migration-defi-dex-pool-leaf-purge-20260728-134427` returned a genuine
    "not found" (not a permission error); (3) `gcloud compute operations list` confirmed clean `insert`→`delete` only,
    no `compute.instances.preempted` — a real self-delete, not a preemption; (4) `run.log` tail confirmed
    `DEPLOYMENT_COMPLETED ... (exit_code=0)` and the `=== SUMMARY ===` block: `SAFE: 190955`,
    `FLAGGED_NO_MATCHING_REPLACEMENT: 541890` — EXACT match to the dry-run's counts; (5) downloaded the apply resume-log
    (12,005 shard entries, 190,955 leaves with `action=deleted`, matching the SUMMARY exactly) and spot- checked 5
    random `deleted`-action leaf paths across 3 venues (CURVE/ETHEREUM, TRADER_JOE_V2/AVALANCHE, SUSHISWAP/ARBITRUM×3)
    via direct `gcloud storage ls` — **5/5 confirmed genuinely absent from GCS**, not just marked deleted in the log.
    Category-2 (address-keyed leaf purge) is DONE and independently verified, not just trusted from the log.
  - **Category-1 (redundant `_migrated_*` markers) started, scoped dry-run running** (not yet complete at this
    checkpoint):
    `delete_migrated_defi_markers_2026_07_23.py --venues CURVE,SUSHISWAP,VELODROME_V2,TRADER_JOE_V2 --data-types dex_pool_state --discover-workers 8 --verify-workers 8`
    (reduced from the script's own default 32/24 workers — this shared host was measured at sustained load-average 57-75
    on 16 cores / 87-93% swap utilization this session, see
    `issues/footystats_migration_bg_workers_killed_externally_2026_07_28.md`'s latest entry; a lightweight GCS-listing
    discovery pass is much less likely to trigger the same OOM-class kill that hit 4 consecutive local
    `pytest`+`basedpyright` QG runs, but reducing worker count is a cheap additional precaution). Ran locally (not VM) —
    same precedent as todo 1's lst_rates marker purge, which also ran this script directly without a VM. Log:
    `/tmp/dex_pool_marker_dryrun.log` (session-local).
  - **Todo 5's checkbox intentionally left UNCHECKED** — its own literal done-when requires BOTH the leaf purge (now
    verified done) AND `delete_migrated_defi_markers_2026_07_23.py`'s dry-run showing near-zero FLAGGED for these venues
    (category-1, in progress) — per the prior checkpoint's own explicit permission ("unless split into its own follow-up
    todo"), NOT splitting yet — will flip once category-1's dry-run + apply are both verified, in the same session if
    the dry-run completes cleanly.

- **2026-07-28 (later) — category-1 dry-run interrupted by a HOST-LEVEL crisis (session death), resumable, NOT a bug in
  this todo's work.** The dry-run reached 8,500/23,588 markers ({'SAFE': 8050, 'FLAGGED_ROWCOUNT_SHORTFALL': 98,
  'FLAGGED_NO_SIBLINGS_NO_BACKUP': 308, 'SAFE_NEEDS_ATTRIBUTION_COVERED': 44} — no unexplained FLAGGED categories,
  matching the expected disposition shape) when this agent's ENTIRE SESSION died mid-task (not just the one background
  process) — required a full resume. `cat /proc/loadavg` immediately after resume: **185.17 259.50 252.63** on this
  16-core host (~16x oversubscribed), materially worse than the 57-75 already flagged earlier this session. Escalated
  P2→P0 in `issues/footystats_migration_bg_workers_killed_externally_2026_07_28.md` (this is now a fleet-wide
  host-capacity concern, not specific to this todo). **No data/progress lost**: the script's own
  `--resume-log /tmp/dex_pool_marker_purge.dry.resume.jsonl` survived on local disk with 8,903 entries (a few more than
  the last printed checkpoint), and `git status` across every repo in this slot is clean — nothing uncommitted was at
  risk. **Waiting for host load to decline meaningfully before resuming** (checked post-escalation: 1-min load dropping
  185→124→104 while 5/15-min remain elevated from the recent peak — trending down but not yet safe to relaunch even a
  lightweight process) rather than immediately retrying into the same crisis. Next step once load subsides: re-run the
  SAME command with `--resume-log /tmp/dex_pool_marker_purge.dry.resume.jsonl` (the script's own resumable design skips
  the 8,903 already-processed markers, per the same contract already verified working for todo 1's lst_rates purge and
  the address-keyed leaf-purge VM run).

- **2026-07-28 (slot-14, later) — category-1 dry-run resumed; root-caused the resume-attempt kills to `nohup`+`disown`
  detachment (not host load); now running healthily to completion.** Three `nohup ... & disown` resume attempts (worker
  counts 4/4, 4/4, 2/2 across the attempts) each died silently within ~1-3 minutes of the verification phase starting,
  regardless of host load level (which ranged 49-95 across the attempts, not correlated with which one died
  first/fastest) — full diagnostic detail logged to
  `issues/footystats_migration_bg_workers_killed_externally_2026_07_28.md`. Switched to running the script directly
  under the agent harness's own tracked `run_in_background` mechanism (no `nohup`/`disown` at all) — this survived 30+
  minutes with steady progress and zero kills, confirming the nohup-detachment (likely a session/cgroup boundary reap of
  orphaned processes), not host load itself, was the proximate trigger for this specific lightweight-script class of
  kill. Once durability was confirmed, restarted once more at doubled concurrency
  (`--discover-workers 8 --verify-workers 8`, no `--limit`) to process the full remaining scope in one run, since the
  earlier worker-count reductions were mitigating the wrong theory. **Fresh delete-safety re-check performed ahead of
  the eventual `--apply`** (not deferred to apply-time): `gcs_bucket_soft_delete_retention_seconds` on
  `market-data-tick-defi-prd-central-element-323112` returned `604800` (7 days), re-confirming the
  reversibility-qualified path from `/codex/02-data/gcs-and-manifest-delete-safety-protocol.md` §3a is still valid.
  **Status at this checkpoint**: dry-run healthy and progressing (12,166/23,588 processed, dispositions sane —
