...
- **2026-07-28 -- todo 4 backfill IN PROGRESS (checkpoint, not yet closed): 2 scoped SPOT VMs launched with the fixed
  ...
  - **batch1 SPOT-preempted at ~07:38 UTC, recovered per the preemption-recovery HARD RULE (two attempts, first one
    ...
    - **As of this checkpoint (2026-07-28, session still monitoring)**: the third launch is RUNNING, healthy, correctly
      scoped, manifest entries climbing normally. Remaining range is ~480 days (2025-04-03..2026-07-27). Todo 4 is NOT
      ...
      for RUNNING/gone: if gone, check `gcloud compute operations list` for preemption (again) before assuming
      completion, and if preempted again, relaunch scoped to the measured last day (same recipe as above -- do NOT
      replay from 2020-01-20 or even 2025-01-10); (2) if it completed cleanly (`DEPLOYMENT_COMPLETED exit_code=0`), do
      the manifest spot-check for the remaining 3 protocols and finish the todo.
