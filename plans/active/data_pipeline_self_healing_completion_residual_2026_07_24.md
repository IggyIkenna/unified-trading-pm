...
- [ ] [CODE] P2. **Vectorize `detect_manifest_divergence.py::_build_oracle_expected`'s oracle-mode row generation**
      (`unified-trading-library/scripts/detect_manifest_divergence.py`) instead of the current nested
      `for day in date_range: for (venue, data_type) in get_expected_pairs(ag): rows.append({...})` Python-dict-list
      build. Root cause of `uts-prod-dp-manifest-hygiene-changed` (DP-WATCHER-006/DP_CLOUD_RUN_JOB_FAILED, escalation
      agt-11a922, 2026-08-15) OOM-killing at 4vCPU/16Gi on every 2026-08-11..08-15 daily run — the row count grows with
      elapsed days x venue/data_type coverage, so it will exceed the just-applied 8vCPU/32Gi ceiling too as more days
      and venues accrue, same growth-past-headroom shape as the digest job's `.to_pandas()` OOM
      (`dp_meta_watchers_oom_at_32gi_2026_08_13.md`). Unlike that fix, this path has no column-pruning lever (it's
      row-COUNT growth from the nested loop, not column-width); needs either a vectorized/batched generation (e.g.
      `itertools.product` + a single `pd.DataFrame` construction from arrays, not per-row dict appends) or migrating to
      a DuckDB-based aggregation. Repo: unified-trading-library. Immediate mitigation shipped:
      `deployment-service/terraform/gcp/data_pipeline_audit_scheduler.tf` `dp_manifest_hygiene_changed_job` bumped to
      8vCPU/32Gi (matches `dp_daily_digest_job`'s ceiling).
