...
- [x] ✅ [INFRA] P2. **Rebuild e2e-audit:latest from clean LDR** so the daily reprobe cron loads all 5 per-AG hooks
      ...
      `/plans/archive/2026_08/issues/dp_reprobe_empty_oom_regression_unbounded_manifest_read_2026_08_09.md` (ARCHIVED
      2026-08-10 — resolved, all todos done)). — e2e-testing, deployment-service. Evidence:
      cloudbuild=1057b974-93b2-4d54-8540-a9c18757f43a
...
- [ ] [CODE] P1. **Streaming/DuckDB rewrite of `detect_manifest_divergence.py`'s manifest reader — defi's index is 160M
      rows, exceeds Cloud Run's 32Gi ceiling regardless of in-process optimization.** Full investigation + 3 options
      (rewrite reader / dedicated defi compute path / investigate row-count at the source) in
      `/plans/archive/2026_08/issues/dp_manifest_hygiene_defi_index_scale_oom_2026_08_15.md` (archived 2026-08-15,
      Option-C investigation done, findings recorded there) — read that doc first, do not re-derive. Short version:
      `uts-prod-dp-manifest-hygiene-changed` (DP-WATCHER-006/DP_CLOUD_RUN_JOB_FAILED, escalation agt-11a922, 2026-08-15)
      OOM-killed daily 2026-08-11+; bumping to 8vCPU/32Gi (Cloud Run gen2's ceiling, `deployment-service@6c98fbacc9`,
      live) AND vectorizing `_aggregate_manifest`'s groupby (`unified-trading-library@2945ab584c`, 17/17 tests pass) are
      BOTH shipped, BOTH real improvements, BOTH KEPT — and BOTH CONFIRMED INSUFFICIENT via 2 live re-executions that
      still OOM'd. Measured root cause: `defi`'s `availability_index.parquet` is 159,774,844 rows / 6.75GB (13-33x every
      other AG); even a column-pruned pandas read of just that file OOM-killed a local repro on this shared host at
      14GB+ RSS and climbing. No further Cloud Run memory bump is possible (32Gi is the platform max) — needs a genuine
      reader redesign (streaming row-group iteration or DuckDB pushdown), which is real engineering scope beyond a
      one-shot escalation fix. Repo: unified-trading-library.
...
- **na-eligibility-audit 2026-08-17** [body-hash:54553f2d3ef07332]: KEEP-NA, valid -- Residual actuator-wiring plan forked from the (mostly-shipped) data-pipeline self-healing hardening plan; extensive Progress Log shows near-total completion via many verified commits. Of the 3 remaining open todos: the registry-mode verbose->active flip was explicitly assessed by TWO prior audit passes (na-eligibility-audit 2026-08-07, and the round11 RECLASSIFY+satellite-extraction sweep 2026-08-09) and BOTH times deliberately NOT extracted, citing a genuine per-tier-wiring dependency gate -- a standing prior assessment, not re-derived fresh here. The stretch launch-spec-persist item is explicitly marked '(stretch)' with 'no forcing function.' The DuckDB/streaming manifest-reader rewrite is live and actively re-confirmed as still needed via a 2026-08-16 fresh escalation recurrence note citing an explicit operator ruling (BLK-b06d255a, 2026-08-15) and requires a genuine design choice among 3 stated architecture options (streaming rewrite / dedicated compute path / investigate row-count at source) -- not mechanically bounded.
