...
- [ ] [CODE] P1. **Streaming/DuckDB rewrite of `detect_manifest_divergence.py`'s manifest reader — defi's index is 160M
      ...
      `plans/active/issues/dp_manifest_hygiene_defi_index_scale_oom_2026_08_15.md` — read that doc first, do not
      re-derive. Short version: `uts-prod-dp-manifest-hygiene-changed` (DP-WATCHER-006/DP_CLOUD_RUN_JOB_FAILED,
      escalation agt-11a922, 2026-08-15) OOM-killed daily 2026-08-11+; bumping to 8vCPU/32Gi (Cloud Run gen2's ceiling,
      `deployment-service@6c98fbacc9`, live) AND vectorizing `_aggregate_manifest`'s groupby
      (`unified-trading-library@2945ab584c`, 17/17 tests pass) are BOTH shipped, BOTH real improvements, BOTH KEPT — and
      BOTH CONFIRMED INSUFFICIENT via 2 live re-executions that still OOM'd. Measured root cause: `defi`'s
      `availability_index.parquet` is 159,774,844 rows / 6.75GB (13-33x every other AG); even a column-pruned pandas
      read of just that file OOM-killed a local repro on this shared host at 14GB+ RSS and climbing. No further Cloud
      Run memory bump is possible (32Gi is the platform max) — needs a genuine reader redesign (streaming row-group
      iteration or DuckDB pushdown), which is real engineering scope beyond a one-shot escalation fix. Repo:
