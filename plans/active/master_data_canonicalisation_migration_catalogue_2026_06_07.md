...
> **Extracted verbatim 2026-08-03 →
> `/plans/archive/2026_08/master_data_canonicalisation_migration_catalogue_progress_log_history_2026_08_03.md`** (second
> line-cap remediation pass) — R1-backfill, R2-schema, R3-verdicts, R4-IS-freeze, R5-service-smoke, and R6-codex (all
> six ✅ DONE) now live there verbatim.
...
> **Extracted verbatim 2026-08-03 →
> `/plans/archive/2026_08/master_data_canonicalisation_migration_catalogue_progress_log_history_2026_08_03.md`** (second
> line-cap remediation pass) — the probe method note, the BLOCKED-shards table, the GREEN-per-AG table, the
> cross-cutting findings, and the promotion-to-main snapshot (pure evidence record, no checkboxes) now live there
> verbatim. The "R5 remediation todos" subsection below (which carries the one still-open R5-fix-7 item) stays here.
...
1. (line ~1247, todo `G1.run-prediction`) "Prediction `expected_unattempted` must seed at the cqg-bundle grain
   ...
   `predictions_master` Phase 3 (cqg could-exist)." **✅ NO LONGER `BLOCKED-OPERATOR-DECISION` (retagged 2026-07-28)** —
   decision 338 is ratified (classifier change landed `unified-api-contracts@d4523602`, `ClassifierConfidenceLow` now
   0.0000% both venues, re-confirmed `autonomous_session_operator_decisions_2026_07_25.md` entry #14). Remaining work is
   unblocked implementation (wire the cqg grain into the loader), tracked at `prediction_cqg_residual_2026_07_24.md`
   todo 2 ("249-b") — the live successor.
