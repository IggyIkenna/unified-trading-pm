...
- **na-eligibility-audit 2026-08-21** (defi tranche, wave 2): KEEP-NA, valid — re-confirmed; both remaining open items (#1 CEX-spot contiguity backfill, #4 lst_yields feature computation) stay explicit operator-owned boundaries per repeatedly-reaffirmed rulings (blind-VM-relaunch caution; `backfill_lst_yields_30day.sh`'s own `owner: operator` marker forbidding CI/agent execution). Doc stays `assigned_vm: NA`.
