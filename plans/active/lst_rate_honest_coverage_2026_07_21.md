...
| Phase 5 #2 `dex_pool_swaps` backfill                                                | **Cannot be done yet** — scaled to a 3-VM date-sharded on-demand fleet (`mtds-dex-swaps-backfill-1/2/3`, covering the exact measured 570-day gap `2024-10-07→2026-07-21`) after the single-VM version preempted 4 times (confirmed genuine SPOT reclaim, not a code bug — decreasing survival time each cycle). Targeting ~20-30h total. All 3 confirmed healthy as of the last check.                                                                                                                                                                                                                                                                                          | Elapsed time only — check `gcloud compute instances list --filter="name~mtds-dex-swaps-backfill"` + each VM's run.log; if any vanishes, just relaunch that exact chunk's command again (idempotent by design) |
...
| MTDS `lst_rates` `pipeline_mode` mislabels Solana Tier-4 rows                       | **Not done** — found while code-tracing the 4-rate audit; real but low-risk, Solana-only, not blocking the ETH-side STAKING build.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              | Nothing — pick it up when convenient                                                                                                                                                                          |
