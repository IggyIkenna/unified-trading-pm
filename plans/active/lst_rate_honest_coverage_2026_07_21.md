...
| ------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| ✅ Phase 5 #4 `lst_rates` backfill (`mtds-lst-rates-20260722-181845`) reaching `2026-07-22` | **DONE 2026-07-23 01:57 UTC** — full `2021-08-17→2026-07-22` range completed in ONE continuous run, no preemption. `Batch complete: 1801 results collected`, `exit_code=0`, clean self-delete. Manifest closed at 29,273 entries for this VM's segment.                                                                                                                                                                                                                                                        | n/a — closed. Next: run the `lst_yields` FEATURE computation over this now-complete source history (see the `#4 lst_yields backfill` todo above — a features-service compute step, separate from this raw-data backfill).    |
| Phase 5 #2 `dex_pool_swaps` backfill (`mtds-dex-swaps-backfill`)                            | **Cannot be done yet** — PREEMPTED once (~01:20 UTC, no clean exit marker, confirmed via `get-serial-port-output` → instance not found), RESUMED via a plain relaunch (same `--start 2023-01-01 --end 2026-07-22`, no `--force`/date-math needed — this collector has genuine per-day `ManifestFreshnessCache` skip logic, confirmed by reading `dex_swaps_handler.py`). Resume verified working: manifest climbed 63,850→69,205 immediately post-relaunch (not restarting from zero). Still running, healthy. | Elapsed time only — check `gsutil cat gs://deployment-scripts-central-element-323112/vm-logs/mtds-dex-swaps-backfill/run.log \| tail -50`; if it vanishes again, just relaunch the same command again (idempotent by design) |
| Sanctum reconciliation follow-up                                                            | ✅ **DONE** — shipped `mtds@52c5ff02` + `instruments-service@4b82310a` earlier this session.                                                                                                                                                                                                                                                                                                                                                                                                                   | n/a — closed                                                                                                                                                                                                                 |
