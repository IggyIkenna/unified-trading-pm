...
todos:
  ...
  - id: risk-circuit-breaker
    content: "Implement circuit breaker in execution-service: if order submission to a venue fails N consecutive times within M seconds, pause that venue's order flow and emit CIRCUIT_BREAKER_OPEN event via log_event(). After cooldown period, emit CIRCUIT_BREAKER_HALF_OPEN and allow a single test order. On success: CIRCUIT_BREAKER_CLOSED. Pattern must live in execution-service/engine/ (not a venue adapter). Unit test all three states."
    status: pending
  - id: risk-kill-switch
...
| ------------------------------------------------ | ------------- | -------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Phase 3 service hardening not complete           | `[PLAN_TODO]` | [phase3_service_hardening_integration.plan.md](phase3_service_hardening_integration.plan.md) § todo `p3-service-hardening` | Execution-service architecture must be clean before adding circuit breaker; avoid wiring new safety controls into structurally broken code                                    |
