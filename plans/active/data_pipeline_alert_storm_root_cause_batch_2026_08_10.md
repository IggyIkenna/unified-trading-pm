...
- [ ] [DATA] P0. Populate `contract_size` for CeFi instruments in instruments-service and get it to MDPS through a
      channel a T4 service may use (the tick payload or a UAC registry — NOT a direct instruments-service call). This is
...
| **Wrong inverse notional already on GCS** (~4,113 shards)                                                                                                                           | **Operator-owned.** Prod-bucket delete is human-only. Fail-closing stopped new wrong values; these keep `captured` rows and stay readable downstream. See the P0 todo for the per-venue breakdown.                                                                                                                                                                                                                                                                                                                                                                                                                           | operator                                                |
| **`contract_size` population for CeFi**                                                                                                                                             | **Not done.** The single blocker for inverse liquidation notional — the field is declared by instruments-service's sink and the UAC schema but absent from all 40 catalogue columns. Needs a T4-legal delivery channel to MDPS too (tick payload or UAC registry, never a service call).                                                                                                                                                                                                                                                                                                                                     | nobody — highest-value next item                        |
| **VM fleet wedge + blind monitor**                                                                                                                                                  | **Partly done.** 393 wedged VMs deleted under operator authorisation (651→250 running). The CAUSE is untouched: ~398 VMs hung mid-shutdown inside one hour, and `uts-prod-dp-exit-code-monitor` times out at 1800 s so it verdicted only 18 VMs in 6 h — none of them live. Duplicates are still being produced. Full write-up + 7 todos in the issue doc.                                                                                                                                                                                                                                                                   | nobody — see the issue doc                              |
