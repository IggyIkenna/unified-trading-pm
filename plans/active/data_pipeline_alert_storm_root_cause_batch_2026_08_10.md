...
| item                                                                                     | state / why deferred                                                                                                                                                                                                                                                                                                                                                                                                                                                           | blocked-on                                                                       |
| ---------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | -------------------------------------------------------------------------------- |
| **Wrong inverse notional already on GCS** (~5,232 shards, population moves — re-measure) | **Attempted, FAILED — 0% fixed.** A scoped `--force` re-derive VM ran to completion (2223 dates, 4.5h) but 0/33,686 write attempts succeeded. Root cause split into two: (1) 1d timeframe (majority of the population) almost never even gets attempted — a genuine, unsolved code-path question, data-availability ruled out; (2) 15m/1h/4h fail 100% via a sparse-liquidation-events OHLC density bug (upgraded P2→P1). Full evidence + next-step plan in the P0 todo above. | nobody — highest-value next item, now a code investigation not an infra/ops task |
...
**Recommended NEXT item (updated 2026-08-12): trace why the 1d timeframe almost never gets attempted under `--force`.**
This is now a code investigation, not an infra/ops task — the VM launcher, the fix deployment, and the fleet cleanup are
all done and verified. Data-availability is ruled out (see the P0 todo's evidence). Start from
