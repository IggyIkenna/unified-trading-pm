...
- [x] ✅ [AGENT] P0. Replace the claude-spawn-based usage poller with a direct **Anthropic API call** to the usage/limits
      endpoint (per Anthropic docs). Same data, no subprocess churn. Implementation: `httpx.post` against
      `https://api.anthropic.com/v1/messages` (minimal 1-token probe); rate-limit data returned in response headers
      (`anthropic-ratelimit-unified-5h-utilization`, `7d-utilization`, `5h-reset`, `7d-reset`).
      Token extracted from env file without subprocess. — agent-orchestrator@ad28879
