...
**2026-08-21 — slot 14 exact generator-scoped rerun (RED, terminal).** Driver `pipeline-e2e-check-mtds-20260821-053311-24ca62` enumerated exactly four rows (`KALSHI`/`POLYMARKET` × `trades`/`book_snapshot_5`) and completed at `2026-08-21T06:01:33Z` with exit `1`: 12 leg cells, 0 passed, 8 failed, 0 ambiguous, and 4 explicitly skipped. The durable report records venue-scoped test-bucket paths for every force/skip result, genuine skip proofs for both trades rows, live-only skip classification for both book snapshots, and canonical-index checks for each row. The failures are concrete data/canonical results (`no_parquet_under`, `canonical_no_matching_rows`, or `noncanonical-instrument_type:'None'`), not cross-venue sampling. Evidence: `gs://deployment-scripts-central-element-323112/pipeline-e2e-check-reports/data_pipeline_e2e_check_mtds/2026-08-20/data_pipeline_e2e_check_mtds_2026_08_20_prediction.md`; MTDS quality gate passed (`11,117 passed, 28 skipped, 1 xpassed`).
