...
- [ ] [RECOVERY] P1. **PREMISE CHANGED → NOT a bulk-delete (verified 2026-06-24 slot-2): the 21 `tab/*` branches hold ORPHANED UNMERGED WORK that never reached LDR.** Fleet sweep: 21 `tab/*` branches across 14 repos, **ALL `ahead_by≥1` of `live-defi-rollout`** (none cleanly contained), and **content-verified NOT in LDR** for the sample: strategy-service `tab/rootm/7` = `kill_switch_bus_subscriber.py` (**safety-critical kill-switch code**), deployment-service `tab/rootm/2` = **32 files** (config_loader / sports scheduler / consolidator watchdog / bucket-migration infra), unified-api-contracts `tab/rootm/7` = **27 files** (alerting thresholds / circuit_breaker / incident module / cme_polymarket_link), market-tick-data-service `tab/rootm/2` = **15 files** (solana/dex/lending handlers + orchestrator), client-reporting-api `tab/ikennaigboaka/8` = 5 files (attribution route / invoices / tests). The Path-B migration (2026-06-08) only preserved *uncommitted* WIP to `wip-preserve/*` — these COMMITTED-but-unmerged tab commits were orphaned when the tab model was retired. **DO NOT DELETE.** Task is now: per-branch triage — for each of the 21, content-diff vs LDR → (a) superseded/redundant → safe to delete after confirming, (b) genuinely-orphaned real work → cherry-pick/re-evaluate for merge to LDR (the kill-switch fix + the 32/27/15-file sets first). **Operator awareness required** (orphaned safety-critical code). (release_machinery ▸ sprawl; finding 2026-06-24 slot-2)
