...
- [ ] [SCRIPT] P3. Plan-hygiene gate GAP: `check_superseded_in_active.sh` only catches filename-`SUPERSEDED` + superseded-parent-epic — it does NOT flag a plan whose slug appears in another ACTIVE plan's `supersedes:`/`parent_consolidation:` frontmatter while still `status: active`. That gap let the 7 source plans masquerade as active for a day (caught by operator 2026-06-25, not tooling). Add the check: scan every active plan's `supersedes:` list → any listed slug still `status: active` in `plans/active/` = flag. (consolidation closeout)
...
- [ ] [DOCS] P3. **NEW (3-agent sweep 2026-06-24): codex SSOT `codex/07-security/service-to-service-auth.md` is STALE** — last updated 2026-05-08, it still describes the per-service local `verify_service_token` in `{service}/auth_s2s.py` as the canonical pattern and **does NOT mention the UTL factory `create_s2s_auth_dependency` at all**, despite 17 service modules already using it fleet-wide. Update the doc to mandate the factory as canonical (and mark local hand-rolled copies as the anti-pattern the contract_hardening #3 item is retiring). (quality_gates ▸ contract_hardening #3 follow-up)
