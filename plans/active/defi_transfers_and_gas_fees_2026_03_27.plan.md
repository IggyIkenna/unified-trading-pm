...
todos:
  # ============================================================
  # PHASE 1: UAC Schema Foundation (PARALLEL within phase)
  # ============================================================
  - id: uac-transfer-types
    content: |
      ...
      - `TransferRecord(BaseModel)`: transfer_id, transfer_type, status, source_chain_id, dest_chain_id, source_address, dest_address, token, amount_wei, tx_hash, bridge_tx_hash, gas_used, gas_price_gwei, gas_cost_eth, gas_cost_usd, submitted_at, confirmed_at, bridge_protocol, error_code, error_message
  ...
  - id: uac-gas-fee-types
    content: |
      ...
      - `GasFeeSnapshot(BaseModel)`: chain_id, timestamp, block_number, base_fee_gwei, priority_fee_gwei, eth_price_usd, gas_estimates (dict[GasCostAction, int] — gas units per operation)
  ...
  - id: exec-transfer-handler-live
    content: |
      ...
      - Add `_execute_cex_withdrawal(instruction)` — calls CEX API adapter (Binance/OKX/Bybit/Coinbase) using existing venue adapter pattern
  ...
  - id: exec-eth-balance-tracker
    content: |
      ...
      - `deduct_gas(wallet, chain_id, gas_cost_eth)` → EthBalanceImpact
      - `check_eth_sufficiency(wallet, chain_id, estimated_gas_eth)` → bool
      - If gas deduction would create negative ETH balance:
        - Emit ETH_BALANCE_DEBT event
        - Set `creates_eth_debt = True` on EthBalanceImpact
        - Log warning with wallet, chain, debt amount
        - Do NOT block execution (debt is tracked, not prevented)
  ...
  - id: exec-bridge-connector
    content: |
      ...
      - `BaseBridgeConnector(BaseConnector)` abstract class:
        ...
        - `estimate_bridge_fee(source_chain, dest_chain, token, amount)` → BridgeFeeEstimate
      ...
      - `SocketBridgeConnector(BaseBridgeConnector)` — Socket/Bungee API integration (aggregates bridges)
        - Uses Socket API to find best route across Stargate, Across, Hop, etc.
  ...
  - id: pnl-gas-attribution
    content: |
      ...
      - Add gas cost breakdown to `StrategyPnL`: `gas_cost_by_operation` (dict[str, Decimal]), `gas_cost_by_chain` (dict[int, Decimal])
      - `net_pnl = gross_pnl - total_gas_cost - trading_fees` (already exists, verify gas_cost is populated from real data)
  ...
  - id: strategy-transfer-instructions
    content: |
      ...
      - PortfolioRebalancer currently generates SWAP/TRADE only — add TRANSFER generation for:
        - Cross-venue rebalancing (move funds from one wallet to another)
        - CEX withdrawal (move from exchange to on-chain wallet for DeFi)
        - Sweep (consolidate small balances to main wallet)
  ...
  - id: pbms-transfer-reconciliation
    content: |
      ...
      - On periodic schedule: scan all tracked wallets via `get_wallet_transfer_history()` for unexpected inflows/outflows
