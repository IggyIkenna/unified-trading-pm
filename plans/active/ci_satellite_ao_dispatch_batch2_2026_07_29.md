...
status:
  complete # (was: active) 2026-07-31 archival: all 14 todos done; all 15 Deferred items (E1-E15) re-verified before
  # archival — E1/E3/E4/E5(mixed)/E6/E7/E8/E9/E10/E13/E14 remain live open todos in their own source docs, E2 and E11
  # corrected from "still open" to RESOLVED-AS-MOOT on fresh re-verification, E12 already resolved, E15 already
  # closed-as-no-action. Archived via the standard 6-step ritual alongside its gated finalize.
nature: process
asset_group: [ci]
stage: [meta]
repos:
...
> **🟢 ARCHIVED 2026-07-31** — all 14 todos done + all 15 Deferred items (E1-E15) verified before archival: E1 / E3 / E4
> / E5 (mixed) / E6 / E7 / E8 / E9 / E10 / E13 / E14 remain live open todos in their own source docs (nothing lost); E2
> and E11 were corrected from "still open"/"still unclaimed" to RESOLVED-AS-MOOT on fresh re-verification (both had gone
> stale — see their rows below); E12 was already resolved; E15 was already closed-as-no-action in its own doc. The 3
> operator questions (`## Escalated to the operator`, tied to E6/E8/E14) remain open in their own source docs — not a
> blocker on this plan's own archival, since none is this plan's own unfinished work. Closed via the standard 6-step
> ritual (`/codex/12-agent-workflow/plan-completion-and-archival-discipline.md`) — gated finalize:
...
| E2  | ~~`ci_test_content_and_tooling_speed_findings_2026_07_28.md`'s `--durations=25` addition to `base-library.sh`~~ **RESOLVED — re-verified 2026-07-31 (finalize todo 4, archival pass).** Stale: the source doc's fleet-wide-leverage todo 1 already bundled BOTH files in one commit — `unified-trading-pm@3ed0fc99d` added `--durations=25` to `base-service.sh:771` AND `base-library.sh:391` together. Confirmed live: `grep -n PARGS scripts/quality-gates-base/base-library.sh` shows `--durations=25` present at line 459 today, and `git show 3ed0fc99d --stat` touches both files. Finalize todo 2's "ready for batch-3 extraction" note was based on the unverified batch2 table text, not a live-code check — corrected here. No batch-3 follow-up needed. | Todo 11 owns `base-library.sh` this round (todo 1 already carries the `base-service.sh` half of this same item) — moot: both halves shipped together, see resolution.                                                                                 |
...
| E7  | `mtds_deployment_env_monkeypatch_leak_blocks_quickmerge_2026_07_23.md` — root-causing the 14+ confirmed DEPLOYMENT_ENV leak occurrences is explicitly NOT bounded as currently framed (5 independent investigation sessions have failed to pin the mechanism). This is the SAME underlying bug as `mtds_deployment_env_race_survives_single_worker_2026_07_23.md` (D3(3), already parked pending its own cascade-instrumentation step) — both docs explicitly say "read together, do not duplicate investigation." Stays parked on that single shared blocker. **Re-verified 2026-07-31 (finalize todo 3): still open, unchanged** — the sibling doc's cascade-instrumentation todo (`- [ ] [INFRA] P2`) is still unchecked; that doc's own 2026-07-31 na-eligibility-audit verdict independently confirms E7 unchanged.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  |
...
| E11 | `check_strict_quickmerge_blind_to_dirty_deps_carveout_2026_07_23.md`'s Option 2 sibling docs (`mutable_git_sha_tag_restamping_cloudbuild_2026_07_13.md`, `sports_odds_ownership_registry_split_brain_...`, others) that reference the SAME dirty-deps carve-out class — out of scope for todo 5 above, which is scoped to the checker + recipe only. **Re-verified 2026-07-31 (finalize todo 3): still unclaimed** — the two named sibling docs are archived/resolved for unrelated reasons (their own primary fixes), not because anyone migrated them to the new `Quickmerge: direct-carveout-dirty-deps` trailer; no new doc targets that migration. **RESOLVED AS MOOT — re-verified 2026-07-31 (finalize todo 4, archival pass), superseding the "still unclaimed" framing above.** Finalize todo 3 confirmed the two named siblings are archived for unrelated reasons but stopped short of checking whether "migration" is actually meaningful work. Fresh corpus-wide grep for `dirty-deps carve-out`/`dirty deps carve-out`/`direct-carveout-dirty-deps` (13 hits outside batch1/batch2/finalize/the source doc) shows every hit is a Progress-Log-style HISTORICAL citation ("shipped via the dirty-deps carve-out direct push") describing a past action at the time it happened — none teaches the pre-2026-07-29 convention as forward-looking instruction. The one doc that IS forward-looking guidance, `/codex/08-workflows/ci-cd-flow.md`, already documents the current trailer (shipped alongside `unified-trading-pm@bbe9a9871`). Rewriting historical citations to reference a trailer that didn't exist when those commits landed would be revisionist, not a fix. No live doc needs migrating — no bounded remaining work exists, so no follow-up todo is created. |

### Role-mismatch (needs a `[UI]`-capable slot, not `cicd`)

| id  | Item                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        |
| --- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| E12 | ~~`ci_test_content_and_tooling_speed_findings_2026_07_28.md` — UI/deployment-ui test-tooling speed items.~~ **RESOLVED — re-verified 2026-07-31 (finalize todo 3).** All 5 constituent `[UI]` todos are `[x]` done (Playwright `--project=chromium` scoping + browser-binary `actions/cache`, ESLint `archive/**` ignore-list + `--cache`, `deployment-ui@de5b7af2` npm→pnpm, `vitest.config.ts` jsdom→happy-dom) and the source doc is archived at `plans/archive/issues/ci_test_content_and_tooling_speed_findings_2026_07_28.md` — a `[UI]`-capable slot picked this up between batch-2's drafting (2026-07-29) and this finalize pass. No follow-up needed at archival. |
| E13 | `monitoring_control_plane_master_2026_06_10.md` — Rollout-ratchet panels (deployment-api reader + deployment-ui panel) + G4 (folds into the same panel work). **Re-verified 2026-07-31 (finalize todo 3): still open, unchanged** — both line-251 (Rollout-ratchet panels) and line-455 (G4) remain `- [ ]` in the source doc; corroborated by that doc's own 2026-07-30 na-eligibility-audit verdict (KEEP-NA valid, still needs a `[UI]`-capable role).                                                                                                                                                                                                                   |
