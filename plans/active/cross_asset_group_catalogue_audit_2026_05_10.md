...
- [ ] [AGENT] P0. **1C — GMX + DRIFT dual-classification resolution (REVERTED 2026-05-13)**. Prior
      axis_override approach (UAC@`7c8482e`) reverted by UAC@`949185c`: perp-eligibility is now a CAPABILITY
      check (`venue.has_perp_funding`), not an asset_group filter. GMX-ARBITRUM/AVALANCHE/DRIFT-SOLANA
      classified as defi-axis (added to `MTDS_DEFI_VENUES`); strategy-service hedge-leg selectors and MTDS
      `perp_funding_handler` updated to asset_group-agnostic capability check (slot 8 sub-agents B+C —
      see commit SHAs in plan-of-record). Re-opens DF-3/CF-1/CF-2/CF-9/CF-10 under capability-check banner.
...
| Phase 1C (GMX/DRIFT dual-classification)                  | 🔁 REVERTED 2026-05-13 (harsh-slot8) — UAC@`949185c`; capability-check approach replaces axis_override        | `DEFI_VENUE_AXIS_OVERRIDES` approach reverted; GMX-ARBITRUM/AVALANCHE/DRIFT-SOLANA moved to `MTDS_DEFI_VENUES` (defi axis). Perp-eligibility is now a `perp_funding` capability check. strategy-service (slot 8 sub-agent B) + MTDS perp_funding_handler (slot 8 sub-agent C) in flight.                                                                                                                                                                                                                     |
...
| Phase 1C — GMX/DRIFT dual-classification                         | 🔁 REVERTED 2026-05-13 — capability-check refactor in flight (slot 8 sub-agents A+B+C)                                  | axis_override approach (UAC@7c8482e) reverted; capability-check approach landed UAC@949185c; strategy-service + MTDS handler refactors in flight                                    |
