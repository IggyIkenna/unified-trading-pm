...
- [ ] [AGENT] P0. **1B — Spark + Radiant SSOT consolidation**. (a) Build out Spark instruments-service adapter + MTDS
...
- [ ] [AGENT] P1. **1F-extend — chain-set fragmentation reconciliation (extend half).** Catalogue-audit DF-7 still open:
...
| Phase 1F-extend (chain-set fragmentation)                 | ☐ TODO — PRE_CUTOVER (owner=ikenna)             | new 1F-extend todo captured @PM`021e945f` — reconcile DF-7 (`MAINNET_CHAIN_IDS=19` vs `CHAIN_GENESIS_DATES=21` vs `CHAIN_CONFIGS=35` vs `GAS_FEE_CHAIN_START_DATES=14`; "22 chains" claim matches none) + correct workspace-wide "22 chains" wording + add QG ratchet                                                                                                                                                                                                                                        |
