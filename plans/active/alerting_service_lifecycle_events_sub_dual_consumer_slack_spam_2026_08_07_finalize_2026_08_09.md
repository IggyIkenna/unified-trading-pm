...
summary: >-
  ...
  `uts-prod-alerting-paging` Cloud Run Job + its `-cron` Cloud Scheduler trigger, now live-confirmed stable/idle for
  ~2 days) is done. Reconciles the source doc's checkbox once shipped (citing the deletion evidence), then archives it
  via the standard 6-step ritual. Authored 2026-08-09 as part of the round-9 combined RECLASSIFY + satellite-extraction
...
source: >-
  ...
  -> planning after a fresh live gcloud check confirmed the doc's own "once confirmed stable for a few days" time-gate
  had cleared (scheduler PAUSED, zero Job executions in ~2 days). Conflict-checked clean against every active plan
...
- [ ] [DATA] P3. **Reconcile.** Confirm `uts-prod-alerting-paging` (Cloud Run Job) and `uts-prod-alerting-paging-cron`
      (Cloud Scheduler) are both deleted (or Terraform-destroyed) — `gcloud run jobs describe uts-prod-alerting-paging`
      and `gcloud scheduler jobs describe uts-prod-alerting-paging-cron` should both return NOT_FOUND. If the source
      doc's todo used the Terraform path, confirm `terraform state list` no longer carries either resource. **Done
      when**: both `gcloud describe` calls confirm NOT_FOUND, and the source doc's `[SCRIPT] P3` checkbox is flipped
      `[x]` citing the verification. Repo: unified-trading-pm (checkbox) + deployment-service (the actual delete,
      already done by the source doc's own todo).
- [ ] [DOC] P3. **Archive** `issues/alerting_service_lifecycle_events_sub_dual_consumer_slack_spam_2026_08_07.md` via
      ...
      (`/codex/05-infrastructure/data-pipeline-alerts.md` — confirm it doesn't still reference the deleted legacy Job
      as a live consumer), update every corpus referrer, `git mv` to `plans/archive/2026_08/issues/`. Repo:
