...
- **2026-07-27 — Phase 3b, 3d, and most of Phase 3c shipped in one session; the Phase 3c live-launch verification is IN
  ...
  - **IN PROGRESS, do not restart from scratch — resume here**: the Phase 3c live-launch verification (todo below Phase
    3c's shell-edit todo). **First attempt (`qg-snapshot-20260727-232216`) deleted pre-work** — GCS copy of
    `setup-data-pipeline-vm.sh` was stale (predated this session's fix); deleted before it did any work, then
    republished both the setup script and the stale `deployment-service`/`unified-api-contracts` tarballs
    ...
    commit_sha its manifest says, not which specific commit that is. **Next steps to close this out unchanged from the
    plan's Phase 3c live-launch todo**: (1) codex T+10min check (`codex/15-runbooks/vm-log-observability-verify.md`) —
    `run.log` growing, heartbeat fresh, eventually `EXIT_STATUS`; (2) once it self-deletes or `EXIT_STATUS` shows rc=0,
    find its registry row (`deployments/active/` or `deployments/archive/`) and assert `git_commit` is non-empty; (3)
    spot-check that value against the actually -installed tarball's manifest `commit_sha` from run.log (read it, don't
    assume which repo); (4) only then flip the live-launch checkbox with the VM name + evidence. If this VM ALSO fails
    for an unrelated reason, the next fallback is a minimal hand-verified path: launch any registered-`VM_SERVICE`
    EPHEMERAL_BATCH VM at all, or as a last resort SSH into a fresh e2-small and manually run
    `setup-data-pipeline-vm.sh`'s tarball-download block to confirm the export in isolation — do not keep burning real
    launches on launchers with unrelated pre-existing bugs.
...
| **Phase 3c live-launch verification (the shell edit's own verification gate)**            | 🟡 **IN PROGRESS, NOT DONE** — 2 launches so far: `qg-snapshot-20260727-232717` FAILED for an unrelated pre-existing bug (unregistered VM_SERVICE → install-all-28-repos pip conflict, never reached the new code); pivoted to `measure-honest-coverage-20260727-234251` (VM_SERVICE=instruments_service, registered). See the dated Progress Log entry above ("resume here") for exact next steps | this VM completing + the T+10min check |
