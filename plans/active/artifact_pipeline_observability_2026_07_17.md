...
- [x] [UI] P2. ✅ **Console deep-links — `deployment-ui@74c0a7d` (2026-07-27).** Wrote the missing Artifact Registry /
      ...
      AWS/ECR rows, plus the Deployments deep-link href). GCP Cloud Build / AWS CodeBuild build-log links were already
      live (`row.log_url`, Pipeline tab drawer) — no new work needed there, confirmed by grep before starting. Gap that
...
- **2026-07-24 (later) — operator review of the open-decisions list; 6 items resolved/clarified, 1 issue doc filed, 1
  ...
  - **Not addressed this round** (still open, no new instruction given): the CPU-throttling test itself (still paused)
    ...
    states them.
- **2026-07-27 — Phase 3b, 3d, and most of Phase 3c shipped in one session; the Phase 3c live-launch verification is IN
  PROGRESS (a real VM is running right now — see "resume here" below).** Operator dispatch: complete the plan's
  remaining todos autonomously. Shipped, each verified landed on `origin/live-defi-rollout` by content (not just exit
  code — this repo was under heavy concurrent-agent contention all session; several pushes needed 2-4 pull-rebase-retry
  cycles, and one retry silently dropped a staged edit that a content-diff check caught before it would have shipped an
  incomplete commit):
  ...
  - **IN PROGRESS, do not restart from scratch — resume here**: the Phase 3c live-launch verification (todo below Phase
    3c's shell-edit todo). A real VM, `qg-snapshot-20260727-232717`, was launched via
    `deployment-service/scripts/vm/launch-qg-snapshot-vm.sh` (e2-small, asia-northeast1-c, launched ~2026-07-27T23:27:17
    JST / ~14:27 UTC) specifically because its startup-script-url IS `setup-data-pipeline-vm.sh` — the exact file this
    plan's fix touches — and it self-shuts-down when done (`VM_SHUTDOWN_ON_COMPLETION=true`), so it needs no manual
    cleanup once verification is complete. **A first launch attempt (`qg-snapshot-20260727-232216`) had to be deleted**
    — the launcher warned the GCS copy of `setup-data-pipeline-vm.sh` was STALE (predated this session's fix), so that
    VM would have run pre-fix logic and proven nothing; deleted before it did any work, then
    `gcloud storage cp scripts/vm/setup-data-pipeline-vm.sh gs://deployment-scripts-central-element-323112/vm/` +
    `bash create-code-tarballs.sh --include unified-api-contracts --include deployment-service` republished BOTH the
    setup script and the stale tarballs; the launcher's own freshness check confirmed "setup script fresh" + "all 3
    tarball(s) current" before this second, valid launch. **As of last check (~T+9min)**: VM status RUNNING; no
    `run.log`/heartbeat blob yet at
    `gs://deployment-scripts-central-element-323112/vm-logs/qg-snapshot-20260727-232717/` — serial-console output shows
    it is genuinely still working, NOT hung: `VM_SERVICE=qg_snapshot` matches no entry in `SERVICE_TARBALLS`, so
    `setup-data-pipeline-vm.sh` fell back to its "installing all available tarballs" path (confirmed installing
    features-cross-instrument-service, market-data-processing-service, ml-inference-service, alerting-service,
    execution-service, … in sequence) — a much bigger install than a normal qg-snapshot run, which is why the T+2min
    "log should be streaming" guideline in the runbook doesn't apply cleanly here. This also means the eventual
    `GIT_COMMIT` value will be whichever tarball is LAST in that install loop (consistent with the audited design's
    honest "last-processed tarball" semantics — not a bug), not necessarily `deployment-service`'s own SHA
    (`dae295f916b5ecfea92d5f1d70ee87cacd9e9a92`, confirmed fresh in its manifest) — **the spot-check step must read
    whichever tarball's manifest was actually last-installed from the run.log, not assume it's deployment-service's**.
    **Next steps to close this out** (per the plan's Phase 3c live-launch todo, unchanged): (1) re-run the codex T+10min
    check (`codex/15-runbooks/vm-log-observability-verify.md`) —
    `gsutil ls -l .../vm-logs/qg-snapshot-20260727-232717/run.log` (should exist by now), heartbeat blob, and eventually
    `EXIT_STATUS`; (2) once the VM self-deletes (or `EXIT_STATUS` shows rc=0), find its registry row (check both
    `deployments/active/` and `deployments/archive/` — a self-deleted VM's entry may have already moved) and assert
    `git_commit` is non-empty; (3) spot-check that value against the LAST tarball's manifest `commit_sha` from run.log
    (not assumed); (4) only then flip the live-launch todo checkbox with the VM name + evidence. If the VM is found
    STOPPED/TERMINATED with no `EXIT_STATUS` and no registry row when picked back up, that's a genuine failed-boot
    signal — check the serial console
    (`gcloud compute instances get-serial-port-output qg-snapshot-20260727-232717 --zone=asia-northeast1-c`) before
    relaunching, don't assume and retry blind.
...
**Recommended NEXT (updated 2026-07-27): finish the Phase 3c live-launch verification already in flight** — a real VM
(`qg-snapshot-20260727-232717`) is running RIGHT NOW; see the dated 2026-07-27 Progress Log entry above ("IN PROGRESS,
do not restart from scratch — resume here") for the exact resume steps. Phase 3b, 3d, and the Phase 3c shell edit are
all DONE as of 2026-07-27 — this live-launch check is the ONE remaining piece of the entire plan below Phase 7. Once it
closes, the only work left is: Phase 7's CPU-throttling test (still **operator-paused** since 2026-07-24 — "let's check
the locally running one first"), the Phase 6 stretch items (all P3, optional), and AWS resume (operator/credits-gated).
...
| **Phase 3c live-launch verification (the shell edit's own verification gate)**         | 🟡 **IN PROGRESS, NOT DONE** — VM `qg-snapshot-20260727-232717` launched 2026-07-27 ~14:27 UTC, RUNNING as of last check (~T+9min, no run.log yet but confirmed genuinely mid-install via serial console, not hung). See the dated Progress Log entry above ("resume here") for the exact next steps + what to spot-check against | VM completing + the T+10min check     |
