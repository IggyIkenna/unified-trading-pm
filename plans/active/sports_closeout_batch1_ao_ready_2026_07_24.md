...
- [x] [CLEANUP] P3. ✅ All 3 sub-parts resolved (full writeup + evidence:
      ...
      `markets`/`outcomes`/`settlements`/`arbitrage_opportunity` GCS scaffolding — CLOSED as verified-absent (main
      ruling on `BLK-7aa96c0a`): bounded search across all 6 sports buckets + code found nothing, consistent with the
      2020-06 data floor's pre-floor wipe; no purge needed since nothing exists to purge. (2) `SPORTS_INSTRUMENTS.md`'s
      stale "lineups player-id strip" claim corrected — `instruments-service@97fbea22`. (3) Non-ASCII junk-symbol guard
      added — `unified-api-contracts@a6346f95`.
...
- **2026-07-24 (slot-8, in-flight — code DONE, not yet shipped)** — Todo "Wire `is_promotion_relegation`": implemented
  in full. `features-service`'s `_compute_league_batch` (`derived_features_helpers.py`) now derives the real flag from
  standings home/away league position vs. the league's measured team count (bottom-3 = a promotion/relegation battle,
  matching `season_context.compute_season_context_features`'s own existing-but-unused default); a new
  `_promo_relegation.py` leaf module holds the calc (kept `derived_features_helpers.py` at the 900-line cap) and
  `_run_events_lineup_derived_calculators` merges the flag into `season_context`'s input before that calculator runs, so
  `points_at_stake`'s multiplier reflects it too. Unknown team-count is honest NaN, never fabricated False. 33 new/
  updated unit tests added (`test_derived_features_helpers.py`, `test_derived_features_exporter.py`); `uv run pytest` on
  both files passes clean (186/186), ruff lint+format clean, basedpyright shows only pre-existing baseline noise (no new
  errors). Two commits made locally in `.tabs/8/features-service`: `6f835b8b` (new module) + `05a756e3` (wiring + tests)
  — **NOT yet pushed** (ahead=2 of origin/live-defi-rollout).
  - **Blocked on**: a single clean `bash scripts/quality-gates.sh` pass to get the `.qg_last_passed_sha` sentinel
    `quickmerge --agent` requires. ~12 consecutive attempts this session were killed before completing, root-caused to
    TWO distinct causes, both now understood: (a) severe multi-slot host contention (load avg spiked 30+ on an 8-core/
    30GB shared host — confirmed via `ps`/`uptime`, NOT a code defect: the full pytest suite itself completed cleanly
    THREE separate times, always 17796 passed/0 failed) and (b) `PYRIGHT_TIMEOUT` defaults to 120s in `base-service.sh`,
    too low for this repo's TYPE CHECK step even under moderate load (killed at exit=143) — filed as a todo on
    `qg_host_adaptive_resource_governor_2026_07_14.md` (PM@b3ed4272d, pushed). The 12th attempt
    (`PYRIGHT_TIMEOUT=600 QG_MEM_CAP=0 bash scripts/quality-gates.sh --no-fix`, setsid-detached, log at
    `<slot-8 scratchpad>/qg_run13.log`) had passed TESTS cleanly and was mid-TYPE-CHECK when this session paused for a
    context checkpoint.
  - **Next step for whoever resumes**: check if `qg_run13.log` (or a fresh run) completed — if `.qg_last_passed_sha` now
    matches HEAD, run
    `bash scripts/quickmerge.sh "feat(sports): wire is_promotion_relegation from standings positions" --agent --files 'features_service/sports/exporters/_promo_relegation.py features_service/sports/exporters/derived_features_helpers.py features_service/sports/exporters/derived_features_exporter.py tests/sports/unit/test_derived_features_helpers.py tests/sports/unit/test_derived_features_exporter.py'`
    from `.tabs/8/features-service` (or any slot with those 2 commits — `git log --oneline -2` confirms), then flip this
    todo's checkbox with the resulting SHA. If no clean run exists yet, retry
    `PYRIGHT_TIMEOUT=600 QG_MEM_CAP=0 bash scripts/quality-gates.sh --no-fix` when host load is reasonable (`uptime`
    load1 < ~15 on this box).
