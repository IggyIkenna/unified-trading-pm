...
- **na-eligibility-audit 2026-08-21** (cross-cutting tranche): KEEP-NA, valid — reassessed the 2026-08-19 MISCLASSIFIED_LIKELY_AO_ELIGIBLE flag on 1/4 items (the 'fix the stale scheduled-jobs table' item): it is thematically bundled into the §1 build recipe's step 5 ('do this at the same time as adding this job's row') alongside items 1-2, which remain explicitly framed by the doc's own §5 banner as 'build-phase — not yet scoped for AO dispatch; each needs its own sizing pass' for a brand-new production job with no operator go-ahead on the build itself recorded — splitting the doc-table fix alone loses that context without adding real dispatch value. Downgrading the flag; doc stays whole KEEP-NA, valid. 4 open todos unchanged.
