...
|---|---|---|
| `data_status_drilldown_shard_atom_alignment_2026_05_07.plan.md` | Phase 1 UAC SSOT | `48162308` |
| `data_status_drilldown_shard_atom_alignment_2026_05_07.plan.md` | Phase 1 deployment-api math + wiring | `60b3c258` |
| `data_status_drilldown_shard_atom_alignment_2026_05_07.plan.md` | Phase 1+2+4+5 (endpoint, UI, CLI, codex docs) | `6957d202` |
| `data_status_drilldown_shard_atom_alignment_2026_05_07.plan.md` | Phase 3 deploy-missing-preview | `c1e93f47` |

## New plans created this session (plans/ai/)

| Plan | Purpose |
|---|---|
| `deploy_missing_auto_launch_2026_05_07.plan.md` | Successor to drilldown Phase 3 — promote preview mode to auto-launch (deployment-api → gcloud direct) after security review + tarball-refresh wiring + UI confirmation modal. |
| `launcher_scripts_consolidation_into_deployment_service_2026_05_07.plan.md` | Migrate the 30 ad-hoc VM launchers into `deployment-service/scripts/vm/` per the new CLAUDE.md SSOT rule. |
...
|---|---|
| 30 ad-hoc launcher migration into `deployment-service/scripts/vm/` | `plans/ai/launcher_scripts_consolidation_into_deployment_service_2026_05_07.plan.md` (newly written; promote to active when ready) |
| Deploy-Missing auto-launch (API directly invokes gcloud / aws ec2) | `plans/ai/deploy_missing_auto_launch_2026_05_07.plan.md` (newly written; promote to active after security review) |
| AWS S3 bucket parity + ECR + EC2 launcher work | `plans/active/aws_migration_defi_first_2026_05_07.plan.md` (existing — covers the bigger AWS work; the AWS/GCP toggle audit findings from this session roll into its Phase N) |
| deployment-api GCS-only call sites (`shard_detail.py`, `data_status_hierarchical.py`, `storage_facade.py`) | Same: `plans/active/aws_migration_defi_first_2026_05_07.plan.md` — the unified storage facade is its scope. Inventory recorded in the launcher-consolidation plan's "Pre-audit blast radius" section. |
| DataStatusTab.tsx visual smoke walk for every (service, asset_group) | `plans/active/data_status_drilldown_shard_atom_alignment_2026_05_07.plan.md` Phase 2 — explicitly DEFERRED; local stack now renders the hierarchy at <http://localhost:5183/> for operator-doable manual smoke. |
...
| Date-range / venue / instrument backfills (full coverage validation) | Asset-group umbrella plans: `cefi_master`, `defi_master`, `tradfi_master`, `sports_master`, `predictions_master` (all `2026_05_07`) cover per-asset_group full backfill scope; the `master_to_live_defi_2026_05_23.plan.md` is the cross-cutting target. |
| Schema / event / config migrations | `manifest_migration_master_2026_05_07.plan.md` + `writegate_honest_coverage_endtoend_2026_05_06.plan.md` cover schema-side; `feature_dag_uac_ssot_and_features_coverage_2026_05_06.plan.md` covers feature DAG. |
| Validation gates per record_captured (4 pillars) | `writegate_honest_coverage_endtoend_2026_05_06.plan.md` Phase 1A. |
...
* The 30-launcher migration is captured in
  `plans/ai/launcher_scripts_consolidation_into_deployment_service_2026_05_07.plan.md` with full
