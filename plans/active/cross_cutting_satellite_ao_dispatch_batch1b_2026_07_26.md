...
- [ ] [SCRIPT] P2. **BLOCKED-OPERATOR-DECISION (added 2026-08-09, plan_reconciler agt-733350, self-resolved per
      BLK-fea4bd0f [WORKER REC] after 2h with no operator reply).** The classify/delete/relocate portion below is gated
      on `repo_scripts_governance_audit_2026_06_18.md` Phase 1 human-judgment execution (see that paragraph for why) —
      using `BLOCKED-OPERATOR-DECISION` specifically (not a bespoke marker) because it is the token
      `agent-orchestrator/server/regen_backlog_from_plan.py`'s `_BLOCKED_TOKEN_RE` actually recognizes as
      non-dispatchable; a prior `BLOCKED-PREREQUISITES` marker elsewhere in this tranche was found this same run to be
      silently non-functional for the identical reason.** features-service coverage/script-canon cleanup — three bounded
      follow-ups from the 2026-06-10 coverage session: (1) ~~fix the per-module `pytest --cov=features_service.<module>`
      scipy/numpy/pytest-cov double-import crash on Python 3.13~~ **DONE 2026-07-31 — features-service@60992d3e** (see
...
- [x] ✅ [CODE] P2. **features-service — fix `odds_features_exporter.py` velocity-accel fallback NaN/math semantics
      ...
      pending owner sign-off on NaN/fallback semantics — operator ruling 2026-07-27, see
...
- [x] ✅ [CODE] P2. **features-service — make `_make_session`'s aiohttp resolver construction lazy/loop-safe (latent
      ...
      pending owner sign-off on the DNS-resolver config change — operator ruling 2026-07-27, see
      `/plans/active/june_2026_vintage_audit_findings_2026_07_27.md` §5-RESOLVED item 35: this is now agent-owned scoped
...
- [x] ✅ [DATA] P1. **Close out remaining perp-funding data-semantics/cadence CeFi work: exact discrete funding, cadence
      ...
      **Status 2026-07-29 (slot 9, data_engineering) — leg (e) now FULLY resolved (registry corrected, not just
      ...
      both this plan and
      `/plans/active/issues/aster_perp_funding_backfill_stale_launcher_and_genesis_conflict_2026_07_28.md` — only
      unrelated commits landed; no operator ruling recorded there). The 2026-07-31 corpus-wide ownership-conflict sweep
      added a premise-correction banner to the issue doc (about Finding 1's HL/ASTER-funding-equivalence claim being
      reversed) but explicitly left this todo's genesis-date question `[OPERATOR]`-gated and unaffected.
      `POST /api/backlog/.../blockers` confirms no formal blocker record tied to this task_id (the gate lives on the
      issue doc's own todo, not the backlog dispatcher), and a `/progress` call returned no operator message. No
      unblocked action available — same as the 2026-07-29 status. Released via
