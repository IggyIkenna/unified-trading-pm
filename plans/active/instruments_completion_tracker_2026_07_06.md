...
| **D5**  | Deribit options stance (`options_chain` effectively uncaptured)                                                                      | **not a standalone decision** — resolves via Stage-3 re-measure (§E.1: A18 indeterminate until a live DERIBIT `options_chain` measure) | ✅ **RESOLVED**   | **Measurement-gated, not a fork** — the honest DERIBIT `options_chain` number falls out of the Stage-3 re-measure (cefi_deribit_bundle GCS scan already found the old "138 captured" were phantoms). MVP "don't widen beyond BTC/ETH `options_chain`" stance STANDS. Mechanical residual (annotate the Layer-1 gap + gate spot-checks behind ">0 captured") folds into D2 / cefi_deribit_bundle. |
