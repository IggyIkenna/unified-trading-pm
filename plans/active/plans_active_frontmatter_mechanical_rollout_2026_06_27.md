...
- [ ] [SCRIPT] P1. **Seed in place.** Run `python3 scripts/docs/seed_frontmatter.py --apply plans/active/*.md`
      ...
      changed (`git status` clean of other paths).
- [ ] [SCRIPT] P1. **Validator-green (HARD).** Run `python3 scripts/docs/docspec.py --check plans/active/*.md`; it must
      ...
      **Gate**: zero HARD violations across all top-level active-plan docs; SOFT-only remainder is the deferred content
      pass, not this plan.
- [ ] [SCRIPT] P2. **Ship + flip.** Commit the seeded docs to `live-defi-rollout` (docs-only → direct-push carve-out)
      and flip these checkboxes in the same turn with the `docs(plans):` prefix. **Gate**: pushed; the three checkboxes
      above are flipped to `[x]` with the commit sha cited.
