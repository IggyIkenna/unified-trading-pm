...
- [ ] [BACKEND] P1. **Kill the false-failure in the dispatch script.** `curl --max-time 30` against a measured 55-56s
      synchronous endpoint. Prefer making `POST /api/plan-health/dispatch` return **202 immediately** and spawn in the
      background (the caller does not need the spawn result); if that is too invasive, raise `--max-time` to ≥180s as
      the fallback. Also distinguish `HTTP 000` (timeout/connect failure) from a real error code in the logged verdict.
      The script is generated by `scripts/install-plan-reconciler-timer.sh` — **edit the generator, not the deployed
      copy**, then re-run it. **Gate**: a timer run logs an unambiguous DISPATCHED / NO-CAPACITY / FAILED verdict that
      matches what actually happened; no `exit 1` on a successful dispatch.
