...
| **Data-status tab (REAL path = `/api/data-status/manifest`)** | `/api/data-status/manifest` (IS/MTDS/MDPS), drilldown, downloads                             | **⚑ THE OOM: IS full-hist 90s/18GB · MTDS full 81GB · MDPS 3mo 56GB — all uncached hot (index cached, cell-grid NOT); default range = full history → 1 click = kill** | **DECIDED 2026-07-14**: full cell-grid bound/stream re-architecture stays DEFERRED (per operator's 2026-07-13 Slack ruling — Ikenna isn't using this tab right now). Near-term instead: a fail-fast/refuse-large (or serve-stale-as-last-resort) OOM guard on the live-build fallback so one request can no longer take down the whole container — see Phase B todo. | **DECIDED**: change the UI's default initial range from full-history (2018-01-01) to a bounded recent window (90 days); full history stays reachable via an explicit "Load full history" action, not the silent default. |
