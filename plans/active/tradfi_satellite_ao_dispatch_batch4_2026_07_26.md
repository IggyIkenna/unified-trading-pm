...
- [ ] [OPERATOR] P1. Flip `tradfi_satellite_ao_dispatch_batch4_2026_07_26_finalize.md`'s frontmatter `status` from
      `draft` to `active` — its `depends_on`+`gate_on_depends: true` condition on this plan is now satisfied (all 8
      todos above are done), but nothing has triggered the draft→active flip, so its reconciliation-then-archive todos
      (per this plan's own "## Reconciliation" section) have never dispatched; this plan must not be archived directly.
