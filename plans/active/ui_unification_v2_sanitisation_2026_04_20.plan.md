...
todos:
  ...
  - id: p6-admin-nav-wiring
    content: |
      - [ ] [AGENT] P1. Extend `ADMIN_TABS` in `components/shell/service-tabs.tsx` to include new admin subroutes (Users, Apps, Audit Log, Firebase Users, GitHub, Groups, Health Checks, Notifications, Onboard, Questionnaires, Requests, Templates). Gate entire admin area with `user?.role === "admin"` check at layout level — no per-page gates. **DEFERRED post-merge to avoid racing Wave 2-A ADMIN_TABS edits.** Wave 2-A is concurrently restructuring `lib/lifecycle-mapping.ts` + `components/shell/lifecycle-nav.tsx` + `components/shell/service-tabs.tsx`. Migrated pages ARE reachable at `/ops/admin/...` URLs already under the existing layout-level `role === "admin"` gate — tab-bar entries just need ADMIN_TABS mutation which would race Wave 2-A. Pick up once Wave 2-A settles.
    status: todo
