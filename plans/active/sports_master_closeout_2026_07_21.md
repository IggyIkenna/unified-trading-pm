...
- [x] [DATA] P0. ✅ **K2 — manifest-swap PLAN + EXECUTE COMPLETE.** Report: 373,296/373,297 objects row-counted (1
      ...
      Progress Log below.
- [ ] [DATA] P1. **NEW FINDING (2026-07-22, found while scoping K2 completeness): 1,266,874
      `pipeline_mode=batch_api_football` rows in the MTDS sports manifest** (incl. 7,248 genuinely `captured` real
      data), `attempted_at` reaching 2026-07-13 — ~19+ days after the 2026-06-24 operator-ruled wipe of this exact
      wrong-source population (`wipe_api_football_sports_odds_2026_06_24.py`: "api_football is NOT a sanctioned
      bookmaker-odds source... WIPE EVERYTHING source=api_football"). Writer never disabled — re-accumulating. NOT part
      of K1/K2 scope (different writer, different pipeline_mode — see the issue doc for why). Exact write-path not yet
      pinpointed. Full evidence + next steps:
