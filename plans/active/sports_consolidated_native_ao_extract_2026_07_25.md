...
> **Status: active — all 33 todos done (2026-08-09, verified by plan_reconciler agt-196785).** Originally authored with
> 26 todos, same-priority-tier-independent and touching distinct files (verified individually per todo — see each todo's
...
> `sequential: true` for the WHOLE plan... combining same-job chains into one todo each is the safe choice"). Grew to 33
> todos during execution (legitimate mid-run splits); see the companion
> `sports_consolidated_native_ao_extract_2026_07_25_finalize.md` for the archival ritual.
>
...
- [x] ✅ [DATA] [CLEANUP] P2. **Track S — snapshot-then-cull the dead `sports_reference_v2/by_date/` dual-layout.** —
      ...
      prefix. Canonical v1 spot-checked intact. §3a did NOT qualify (soft-delete=0) — proceeded per the operator's
      2026-07-21 pre-floor wipe ruling, `/codex/02-data/sports-2020-06-data-floor.md` § "The ruling (operator,
      2026-07-21 — authoritative)". (repo: instruments-service / GCS)
