...
- [ ] [P0][MTDS] **CATALOGUE-AS-FILTER capture (THE EU-conversion lever — measured root cause resume #20)**: the dex_pool_state/dex_pool_swaps handlers must query the subgraph BY the catalogue pool-address set per (venue,chain,date) (`pool_in:`/`id_in:` filter, paginated past 1000), NOT the broad top-1000-by-TVL — so captured (pool,date) cells share the EU cell-key (today per-(pool,date) overlap=1/date → 0 EU conversion). Includes: add the missing-venue subgraphs (TRADER_JOE_V2/UNISWAP_V4/ORCA/KAMINO/VELODROME_V2/RAYDIUM — ~1,082 catalogue pools never queried), reconcile the Balancer `pool_id` (0x+16hex 66-char) ↔ catalogue `pool_address` (40-hex) id form, and `record_empty(SOURCE_RETURNED_ZERO/EXPECTED_NOT_ENOUGH_TVL)` for any catalogue pool the source returns nothing for. The cursor fix (08b45468) is necessary (full-day, no skip-cap) but insufficient alone (doesn't change WHICH pools are queried). — market-tick-data-service
- [ ] [P0][DECISION][instruments-service] **EU-seed-set vs per-date-snapshot-set vs capture-set 3-way mismatch (cross-repo, BIG finding — needs the operator/coordinator design call)**: EU is seeded from the cumulative `prod/catalog.parquet` roll-up (1,425 UNISWAP_V3/ETH pools for 02-23, clipped to available_from/to → every available day), but the per-date `by_date/day=D/` snapshot the MTDS handler reads lists only **316** that day, and the subgraph top-N captures 4,531 — three non-aligned sets. DECIDE the SSOT pool-set the capture filters on + the EU seeds against (cumulative-catalogue vs per-date-snapshot), so seed-set == capture-set == the key the manifest reconciles on. This gates the catalogue-as-filter P0 above (which set to filter by). — instruments-service (enumerator + catalogue) + market-tick-data-service
