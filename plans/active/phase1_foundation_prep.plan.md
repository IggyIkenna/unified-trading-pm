...
todos:
  ...
  - id: ci-pipeline-wiring
    content: "DONE (2026-03-06): B1 DEP_BRANCH — all 19 quality-gates.yml files with git clone already use DEP_BRANCH fallback pattern (DEP_BRANCH env var + fallback to main). Only unified-trading-pm itself uses git clone for bats-core (third-party, not a dep-repo, no DEP_BRANCH needed). B2 version-bump.yml — SSOT at unified-trading-pm/.github/workflows/version-bump.yml is complete with correct semver: BREAKING CHANGE/bang-prefix → major, feat → minor, fix → patch, chore/docs/refactor → no bump (skip). All repos already have version-bump.yml copied from SSOT (A1 task). Both sample repos (unified-cloud-interface, instruments-service) verified present. B3 Cloud Build feature branch trigger — template created at unified-trading-pm/templates/cloudbuild-feature.yaml: triggers feature/* and feat/* branches, runs quality-gates.sh --no-fix --quick inside Docker, adds +feat.SHORT_SHA PEP 440 local version suffix, no push step."
    status: done
  ...
  - id: arch-market-tick-data-node-cleanup
    content: "NEW VIOLATION (found 2026-03-06 by arch-ui-audit-full): market-tick-data-service has package.json at root declaring tardis-dev (^14.1.2) JS dependency. No tsx/jsx files. Assess: if tardis-dev JS is still needed (e.g. for some script), move to a dedicated tools/ dir with explanation; if unused, remove package.json entirely."
    status: pending
  ...
  - id: arch-ui-audit-full
    content: "DONE (2026-03-06): Full audit of all 21 service repos complete. Results in unified-trading-pm/plans/active/ui-audit-results.md. Findings: (1) strategy-service VIOLATION — full React/TypeScript frontend/ dir (16 tsx files, package.json, vite config); action required: arch-strategy-ui-extract; (2) market-tick-data-service PARTIAL VIOLATION — package.json at root declaring tardis-dev JS dep, no tsx/jsx files; action required: arch-market-tick-data-node-cleanup; (3) execution-service CLEAN — visualizer-ui/visualizer-api already extracted (archive/execution-visualizer-ui exists), only empty package-lock.json remnant; (4) all other 18 repos clean. htmlcov/index.html are coverage artifacts, not embedded UI."
    status: done
  ...
  - id: ci-cloudbuild-audit
    ...
    status: done
  - id: ci-aws-parity
    content: "DONE 2026-03-06: AWS parity tasks complete. aws-cloudbuild-parity DONE (2026-03-05) — buildspec.aws.yaml added to all 45 repos. aws-compute-stubs-wire DONE — aws_batch.py already had get_status_batch; aws_ec2.py was missing get_status_batch, added (commit 49dc640). aws-secret-naming-parity: bybit_api_key underscore name is an SM secret name only, no code references found — rename tracked as SM-level op (no code change needed)."
    status: done
isProject: true
---
