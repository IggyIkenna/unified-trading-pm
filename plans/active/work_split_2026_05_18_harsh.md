...
- [x] ✅ **14. MEGA RESERVE — features-service Wave 60+/61+/62+/63+ rolling coverage waves** — Wave 60: aave_risk 87%→~97% + aave_utilization 88%→~98% (features-service@e68575d8); Wave 61: cryptoquant_exchange_flow 80.2%→~96% (bug fix: tz-naive/tz-aware comparison) + pool_invariant_drift 89.5%→~97% (balancer edge cases + _coerce_list_decimals + calculate_features empty/missing-cols) features-service@59e1b1ce; Wave 62: defillama_tvl _fetch_chain_tvls + _fetch_stablecoin_data (5 tests), macro_sentiment fetch_data session path + exception handler (3 tests) features-service@567b0ab4; Wave 63: block_priority_gas source_name+empty-calc (2), lst_staking no-pools branch (1), fear_greed validate_response+no-records (2) features-service@dafa61d9
