...
- [ ] **S1. SUSTAIN — per-repo basedpyright strictness audit + uplift** [execution-service / workspace audit] — audit all 26+ active Python repos for `reportUnknown* = "error"` strictness; identify gaps; close per-repo. Done-def: per-repo strict mode verified + zero unresolved violations. **~10 cal-days** (~0.4 days × 26 repos).
- [ ] **S2. SUSTAIN — workspace-wide test coverage uplift to 80%** [workspace test surfaces] — ~10 repos currently below 80% coverage. Per-repo: identify uncovered branches, add 5-10 targeted tests, verify QG green. Done-def: 10 repos at ≥80% + per-repo QG green. **~15 cal-days**.
