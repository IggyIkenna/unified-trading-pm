...
- [x] ✅ **17. MEGA RESERVE — semver-agent label + tag audit across 21 Python repos** — build on item 14 (semver-agent label audit). Verify per-repo `feat/fix/feat!` label-to-commit mapping is correct; cross-check shipped tags vs commit history. Done-def: audit report + 2+ corrections. **~2 cal-days**.
      — Audit: 24 worktree repos scanned. 0 manifest↔pyproject version mismatches. All use semver_policy=agent. Correction 1: unified-trading-api missing semver-agent.yml (no .github/workflows/ at all) → created (unified-trading-api@a4ccb75). Correction 2: unified-trading-api missing semver_policy in workspace-manifest.json → added (PM@388ed980). Stale-tag finding: UTL has orphan v1.0.0+v1.2.0 tags; instruments-service has orphan v1.1.0/v1.2.0/v1.3.0 tags — all from pre-semver-agent GitHub Packages publishing workflows; semver-agent uses pyproject.toml not tags, so no functional impact.
