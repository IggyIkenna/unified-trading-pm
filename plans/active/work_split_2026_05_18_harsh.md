...
- [x] ✅ **15. MEGA RESERVE — codex/04-architecture/ drift audit** — sweep `batch-live-architecture.md` +
      ...
      Done-def: 3+ docs audited + drift fixes. **~3 cal-days**. — PM@9874c794 (runtime-deployment-topology.md: URDI
...
- [ ] **S3. SUSTAIN — deployment-api integration test coverage uplift to 90%** [deployment-api] — **IN PROGRESS
      2026-05-18 slot-7**: 74.38% after user_management (deployment-api@720a03e); 75.06% after
      data_status_mock + service_status_fast_data + service_status_execution helpers (deployment-api@93c3668, 69 new
      tests); 75.6% after epics route tests (deployment-api@edeb30e, 32 new tests). Gap to 90%: ~2697 lines. Next
      targets: routes/data_status.py (346 missed), services/shard_detail.py (268 missed), routes/deployment_state.py
      (179 missed), routes/service_status.py (156 missed). Done-def: deployment-api coverage ≥90% + QG green.
      **~10 cal-days**.
...
- [x] ✅ **4. codex/06 cross-link sweep (slot 8 item 20)** — sweep codex/06-coding-standards/ for stale cross-links
      ...
      (unified_trading_services→unified_trading_library x10, URDI→instruments-service x9). PM worktree@8fa773c4.
