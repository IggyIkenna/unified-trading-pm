...
- [x] ✅ **S1. SUSTAIN — per-repo basedpyright strictness audit + uplift** [alerting-service + batch-live-recon] — alerting-service@69a9f4a: 4 settings promoted warning→error; 0 existing warnings. batch-live-reconciliation@983e4ad: pyrightconfig.json created (full strict, stage0 excluded for pyarrow lazy imports), 2 `cast()` fixes + `reportMissingTypeStubs=false`. 16 errors → 0. BIG FINDING: QG `|| true` bug silently swallows basedpyright errors workspace-wide — issue filed: `issues/qg_basedpyright_or_true_bug_2026_05_18.md`. Slot 2 owns workspace sweep + QG fix.
...
- [x] ✅ **5. features-service Phase B coverage waves continuation (Wave 59)** — eigen_rewards_calculator
      65.9%→100% (14 new tests: _derive_eth_price, MTDS success path, fetch_data, _eigen_pools_to_records,
      _fetch_from_defillama); aave_rate_impact_calculator 71.9%→97.8% (5 new tests: fetch_data + exception branches).
      50 tests passing. — features-service@68074c9b
...
- [x] ✅ **14. MEGA RESERVE — features-service Wave 60+/61+/62+/63+/64+/65+/66+/67+/68+/69+/70+/71+ rolling coverage waves** — Wave 60: aave_risk 87%→~97% + aave_utilization 88%→~98% (features-service@e68575d8); Wave 61: cryptoquant_exchange_flow 80.2%→~96% + pool_invariant_drift 89.5%→~97% (features-service@59e1b1ce); Wave 62: defillama_tvl + macro_sentiment fetch_data (features-service@567b0ab4); Wave 63: block_priority_gas + lst_staking + fear_greed (features-service@dafa61d9); Wave 64: protocol_rewards + vault_share_price_apy (features-service@a6afbd78); Wave 65: concentrated_liquidity + chainlink_peg (features-service@fc8ba425); Wave 66: onchain_regime_calculator (features-service@44543910); Wave 67: cryptoquant_adapter 74.3%→100% + defillama_adapter 93.2%→100% + defillama_yields_adapter 26.6%→100% + mtds_canonical_reader 93.0%→100% — 42 new tests (features-service@f5fa85f6); Wave 68: live_handler→100% + lst_seasonal_rewards→100% + lst_seasonal_rewards_live→100% + feature_builder_registry cycle-detect + dependency_checker module-level delegation — 207 lines (features-service@ad645883); Wave 69: base.py line 104 + aave_rate_impact lines 100-101 + lending_features broad-except + lst_features lines 30/33/83-85/101-140 — 12 tests (features-service@c96b2cd5); Wave 70: LSTSeasonalRewardsCollector orchestrator — EmptyChainEventScanner lines 118-119, __init__ lines 152-153, collect_for_day lst_filter+logger lines 164-185, _collect_one_stream scanner-None+except lines 193-221 — 11 tests (features-service@a55c053b); Wave 71: lst_seasonal_rewards_collector line 170 (CARRY_BASE skip) + cli/parser line 142 (validate_args incremental→live) — 2 files, 2 near-100% modules closed (features-service@4d1a6647); Wave 72: batch_handler.py 64.6%→~85% — _handle_dependency_report/check_dependencies/_process_groups/_preflight_guard/_log_run_error/run except + _initialize_services/_process_feature_group — 15 new tests (features-service@bc212b1c); Wave 73: feature_writer.py 66%→~84% — Utf8/Int64 timestamp branches + _handle_write_error + _run_write_gate alignment-fail + _apply_emission_gate both suppressed paths + _validate_alignment invalid + write_seasonal_rewards empty+nonempty + check_exists — 11 new tests (features-service@c3ef28af)
...
- [x] ✅ **S3. SUSTAIN — risk-and-exposure-service test coverage gap (recovery_loop 76%→≥85%, pre_crash_checkpoint 84%→≥90%)** — 9 tests added covering: tick_recovery exception handler (180-181), _schedule_next_tick daemon Timer (186-192), _ticker_body success+exception-reschedule (196-202), start_recovery_ticker first-call+idempotent-alive (211-217), reset with live timer cancel (225-226), NoSuchProcess watchdog break (71-74), high-RSS checkpoint trigger (75-77). QG green. risk-and-exposure-service@da8ede0.
- [x] ✅ **S4. SUSTAIN — pnl-attribution-service coverage gap closure (sports_pnl persist_to_gcs 0%→100%, _to_float 0%→100%)** — 7 new tests: persist_to_gcs empty-no-upload + records-calls-upload (lines 112-128); _to_float None/Decimal/int/float/unknown-type paths (lines 154, 157-159). QG green. pnl-attribution-service@68b5317.
- [x] ✅ **S5. SUSTAIN — execution-service market_hours coverage gap (align/validate/check_open 72%→≥90%)** — 17 new tests in TestAlignTradfiTimeWindow (8: non-tradfi passthrough, within-hours, start-before-open, start-after-close, end-after-close, same-day-before-open ValueError, different-day-before-open prev-close, equal-after-alignment ValueError), TestValidateTradfiDataWindow (5: no bounds, before-available, after-available, start-before/end-after overlap), TestCheckTradfiMarketOpen (6: non-tradfi, Saturday, Sunday, before-open, after-close, during-hours). Covers lines 22-107, 165-292. QG: 7274 passed (30 pre-existing unchanged). execution-service@ba60562e.
- [x] ✅ **S6. SUSTAIN — execution-service execution_policies 86.8%→100% (18 new tests, all 20 previously-uncovered lines)** — registry edge paths (lines 146, 156-157, 162, 165, 168): get() version-not-found, get_by_ref() non-integer version, all_versions(), policy_ids(), __len__; operator paths (lines 183, 189, 198, 206, 210, 212): <=, string coerce passthrough, in/not_in non-collection ValueError, unknown-op; _match_when (lines 219, 224-228, 230): any_of/all_of non-list, all_of match/fail, missing key; is_applicable urgency gate (line 280). QG: 7292 passed (30 pre-existing unchanged). execution-service@36730ff1.
- [x] ✅ **S7. SUSTAIN — execution-service sports_adapter + sports_factory coverage gaps (4 new tests, lines 81-83/91/148-149/170)** — get_odds via betting_adapter fallback when no odds_adapter registered (148-149), get_balance ValueError for unknown bookmaker (170), live-mode unified_trading_library ImportError → empty SportsAdapter (81-83), successful venue registration via SportsExecutionRouter.get_adapter() (91). QG: 7296 passed (30 pre-existing unchanged). execution-service@eb7031ff.
- [x] ✅ **S8. SUSTAIN — execution-service leveraged_leg_controller + atomic_bundle_executor coverage gaps (13 new tests)** — leveraged_leg_controller: _bps zero-target (82), total_target_lev=0 fallback (202), reward_inflow credit (161-173), reward_inflow not-found warning (174-179), ALWAYS_SWEEP skip zero-delta (244), min_leg_notional filter (246), all-filtered empty return (267), verify_net_delta zero-equity (343) + non-zero-target bps (347), _should_emit ALWAYS_SWEEP (366) + PERIODIC (368); atomic_bundle_executor: mismatched-count mock (74-77), borrow-after-repay mock (97). QG: 7309 passed (30 pre-existing unchanged). execution-service@295680ef.
- [x] ✅ **S9. SUSTAIN — execution-service freshness_gate + drain_mode unit coverage (14 new tests, freshness 0%→≥95%, drain 0%→100%)** — freshness_gate: stale→DataStalenessError+FEED_UNHEALTHY (lines 44-63), warn→DATA_STALE (65-76), fresh passes, unknown-venue skip (35-42), case-insensitive; drain_mode: activate() sets flag+emits DRAIN_MODE_ACTIVATED (37-51), deactivate() clears+emits DRAIN_MODE_DEACTIVATED (57-66), get_status() dict (71). QG: 7323 passed (30 pre-existing unchanged). execution-service@a9cbf5c4.
- [x] ✅ **S10. SUSTAIN — execution-service single-line coverage gaps (3 tests: sor:181/cost_models:210/mev_router:94)** — sor single-venue validate_config() logs WARNING (line 181), cost_model zero-commission when no flat/win/tiers (line 210 → Decimal("0")), mev_router KeyError on empty-policies registry (line 94). QG: 7326 passed (30 pre-existing unchanged). execution-service@51395abf.
- [x] ✅ **S11. SUSTAIN — execution-service calculators coverage gaps (3 tests: vwap:89/pov:90/twap:105)** — vwap_calculator zero-weight-sum raises ValueError (line 89, mocked _calculate_u_shape_weights), pov_dynamic_calculator zero participation_rate → MAX_BUCKETS (line 90), tradfi/twap.py tiny quantity skips zero-base_qty slices (line 105 continue). QG: 7329 passed (30 pre-existing unchanged). execution-service@d201117e.
- [x] ✅ **S12. SUSTAIN — execution-service exit_algo + benchmark_registry coverage gaps (4 tests)** — exit_algo line 42 (trigger_price=None raises ValueError in phase1), line 74 (on_mark_price early-return after complete); benchmark_registry line 104 (registered() sorted names), line 131 (register_benchmark_fill convenience wrapper). QG: 7333 passed (30 pre-existing unchanged). execution-service@0ff550f2.
...
- [x] ✅ **S1. SUSTAIN — deployment_ui_lifecycle_tabs implementation sweep (30 cal-days budget)** [deployment-api/ui] — c2/c3/c4/c6 Phase C @f585227. Phase H complete: h1 codex doc (PM@af609ad7) + h2 env-tier scoping (deployment-api@78b68c4; env:str on all 4 Monitor responses) + h3 UI env badge (deployment-ui@75202df; green DEV/amber STAGING/red PROD). **7 items shipped. h4 BLOCKED-OPERATOR (DNS/IAM).** Plan: [`deployment_ui_lifecycle_tabs_2026_05_08.md`](deployment_ui_lifecycle_tabs_2026_05_08.md).
...
- [ ] **S3. SUSTAIN — deployment-api integration test coverage uplift to 90%** [deployment-api] — **IN PROGRESS 2026-05-18 slot-7**: 26 monitor route tests shipped (deployment-api@0bffd18, QG green). Current coverage: 72.4%. Gap to 90%: ~3300 lines. Next: cost auth+rate-limit, deploy-ready, snapshots. Build on item 20 (Firebase auth integration). Add tests for: cost endpoint auth + rate-limit, VM events filter pagination, treasury rollup Firebase-vs-API-key parity, deploy-ready tracking endpoint, snapshot endpoints. Done-def: 20+ tests + deployment-api coverage ≥90% + QG green. **~10 cal-days**.
- [x] ✅ **S4. SUSTAIN — deployment-ui playwright e2e coverage extension** [deployment-ui] — **SHIPPED 2026-05-18 slot-7** — deployment-ui@bfcc361: 22 new smoketests: env-tier DEV badge (green text + color class + tooltip open/close/content), mobile hamburger (open/close/link/close-on-nav), nav routing (DART/ML/Strategy/ExecBT/LiveOps), per-page no-crash smoke (7 routes). 22/22 passing. pnpm build green. Done-def: 12+ smoketests + pnpm green. **~10 cal-days**.
- [x] ✅ **S5. SUSTAIN — deployment-api Firebase auth + RBAC role-check audit** [deployment-api] — **SHIPPED 2026-05-18 slot-7** — deployment-api@71ba947: added `require_permission(Permission.DEPLOY_TRIGGER)` RBAC guard to 4 launch endpoints (backfill, ml_experiment, strategy_backtest, execution_backtest); extended test_rbac_endpoint_guards.py with 8 new tests (4 classes × VIEWER=403 + OPERATOR=pass); all 19 RBAC tests green; QG passes. **~5 cal-days**.
- [x] ✅ **S6. SUSTAIN — deployment-api OpenAPI spec generation + refresh** [deployment-api] — **SHIPPED 2026-05-18 slot-7** — deployment-api@e1fa23d: docs/specs/openapi.json generated (170 paths, 181 endpoints, all with summary/description, 663KB); scripts/generate_openapi.py added; fixed 2 duplicate OperationId warnings in client_treasury.py. QG passes. **~5 cal-days**.
- [x] ✅ **S7. SUSTAIN — deployment-ui accessibility audit (WCAG AA)** [deployment-ui] — **SHIPPED 2026-05-18 slot-7** — deployment-ui@c8cab31: installed @axe-core/playwright; wrote tests/smoke/accessibility_audit.spec.ts (7 pages, WCAG2A/AA tags, color-contrast deferred to S11 dark-mode audit); fixed 13 critical violations (9 select + 4 input elements missing aria-label across Dart/MlExperiments/StrategyBacktests/ExecutionBacktests); 7/7 axe scans 0 critical/serious; 54/54 smoke tests pass. **~5 cal-days**.
- [x] ✅ **S8. SUSTAIN — deployment-api rate-limiting per endpoint audit** [deployment-api] — **SHIPPED 2026-05-18 slot-7** — deployment-api@5f3b3f1: new rate_limiting.py (sliding-window per-(client_ip, path) dep factory, no-op under DISABLE_AUTH); applied to 6 endpoints: /costs/daily (20/min), /events×2+/logs/{vm} (30/min), /treasury/rollup (10/min), kill-switch arm+disarm (5/min); 8 unit tests (within-limit, 429, DISABLE_AUTH bypass, IP/path isolation, missing-client fallback); QG passes. **~3 cal-days**.
- [x] ✅ **S9. SUSTAIN — VM zombie watchdog test hardening (continuation)** [deployment-service] — **SHIPPED 2026-05-18 slot-7** — deployment-service@0f16556: 19 new tests across 4 classes (dual-staleness verdict, TestNewPrefixRegistration×8 [exec-alpha/synbench/batch-live-recon/strategy-backtest-grid/prediction-fwd/strategy-test/ml-train/heartbeat-only check], TestLifecycleClassCompleteness×2 [all-valid + EPHEMERAL_BATCH cefi], TestForwardPollIdleThresholds×3 [cefi-fwd/defi-fwd/mdps-features-live wider windows]); QG 0 new failures. **~3 cal-days**.
- [x] ✅ **S10. SUSTAIN — deployment-api/ui dashboard widget cross-check** [deployment-ui] — **SHIPPED 2026-05-18 slot-7** — deployment-ui@6d3d082: 8 widgets audited (cost/DailyCosts, cost-estimate/VmCostEstimatePanel, kill-switch/KillSwitchTab+Panel, data-status/DataStatusTab, vm-events/VmEventsTimeline, vm-health/VmHealthBadge, treasury/TreasuryTab, risk/RiskTab); 6 fixes: (1) VmHealthBadge hardcoded Tailwind→CSS vars (dark-mode); (2) DailyCosts loading skeleton; (3) VmEventsTimeline loading indicator + truncation notice; (4) KillSwitchTab spurious loading flash on subview toggle; (5) TreasuryTab refresh button. QG green (67 tests). **~3 cal-days**.
- [x] ✅ **S11. SUSTAIN — deployment-ui dark-mode coverage audit** [deployment-ui] — **SHIPPED 2026-05-18 slot-7** — deployment-ui@0ee228f: 8 components fixed (ArchetypeMatrix, HealthFactorMonitorTile, BreakdownsAccordion, RecursiveBorrowDrilldown, LiveFreshnessPanel, HonestCoverageCard, plus VmHealthBadge from S10); all hardcoded Tailwind bg-gray-*/bg-white/text-slate-*/border-slate-* replaced with var(--color-*) tokens; LiveFreshnessPanel.test.tsx updated (removed fragile .toHaveClass() color assertions); QG green. **~3 cal-days**.
- [x] ✅ **S12. SUSTAIN — deployment-api audit-log emission coverage** [deployment-api] — **SHIPPED 2026-05-18 slot-7** — deployment-api@9f47791: 11 mutation endpoints audited; 7 missing audit emissions added (user_management: USER_CREATED/USER_UPDATED/USER_DEACTIVATED/USER_ROLE_ASSIGNED, vm_admin: VM_CANCEL_REQUESTED/VM_PAUSE_REQUESTED/VM_RESUME_REQUESTED); 19 new tests (test_user_management.py 7 + test_vm_admin.py TestAuditEmissions 3 + 9 existing). QG green (3155 passed). **~3 cal-days**.
- [x] ✅ **S13. SUSTAIN — deployment-ui mobile-responsive smoketest extension** [deployment-ui] — **SHIPPED 2026-05-18 slot-7** — deployment-ui@1a6f2d1: 10 Playwright tests in tests/smoke/mobile_responsive.spec.ts across iPhone SE (375x667) and iPhone Pro (414x896); covers home/deployments/data-status/costs/kill-switch load + hamburger + horizontal overflow. TypeScript clean; 719 Vitest pass. **~3 cal-days**.
...
- [x] ✅ **11. DEEP RESERVE — MTDS additional adapter coverage (databento + polymarket_clob)** — extend MTDS handler coverage to databento_l1_l2_l3 + polymarket_clob handlers. Same shape as item 1 (3-handler pickup). Done-def: 2 adapters above 90% + MTDS QG green. — mtds@66f4f3f (backfilled 2026-05-18)
- [x] ✅ **12. DEEP RESERVE — MDPS canonical_writer error classification audit** — audit MDPS canonical_writer paths for missing `classify_venue_error()` + `record_failed()` emission. Done-def: audit report + 3+ classification gaps fixed. — mdps@b71bb75 (backfilled 2026-05-18)
- [x] ✅ **13. DEEP RESERVE — PBM mode parity edge cases** — build on item 8 (TestDegradedConditionModeParity, mdps@92d9be5). Add: sparse data (90%+ NaN rows), partial-shard captures, recovery from BATCH_TARDIS → LIVE_WEBSOCKET transition mid-shard. Done-def: 4+ tests + PBM QG green. — mdps@1d67851 (backfilled 2026-05-18)
...
- [x] ✅ **15. MEGA RESERVE — MTDS adapter coverage continuation (deribit + binance + bybit handlers)** — after item 11 (databento + polymarket_clob), extend coverage on perp venues. Same Tier-1 SPL stake-pool / multi-call architecture pattern. Done-def: 3 adapters above 90% + MTDS QG green. **~3 cal-days**. — mtds@0978e58 (1683 passed, deribit+binance-futures+bybit WSFeedConnectors ≥90%; backfilled 2026-05-18)
- [x] ✅ **16. MEGA RESERVE — MDPS canonical_writer error classification audit (continuation of deep reserve 12)** — extend item 12. Audit all canonical_writer paths for `classify_venue_error()` + `record_failed()` emission completeness; fix gaps. Done-def: audit report + 5+ classification gaps fixed. — mdps@b0c5f68 (11 new tests: strict=False SchemaContractNotFound, open/close streaming error paths, _publish_emission_check OSError, record_expected_unattempted swallow, resolve_pipeline_mode unknown; 1321 passed)
- [x] ✅ **17. MEGA RESERVE — PBM canonical_writer mode parity + cross-archetype dispatch tests** — build on item 5 (PBM canonical_writer integration tests, mdps@4ad6060). Add: cross-archetype dispatch under degraded conditions; manifest re-sync after pause/resume. Done-def: 6+ tests + PBM QG green. **~3 cal-days**. — mdps@dead5de (8 new tests: TestCrossArchetypeDispatch ×6 CEFI/TRADFI/DEFI × batch/live modes + degraded data + partial shard; TestManifestResyncAfterPauseResume ×2 pause/resume lifecycle + idempotency guard; 1321+ passed)
