...
- **na-eligibility-audit 2026-08-21** (cross-cutting tranche): KEEP-NA, valid — 6 open items remain, each with a dated, measured annotation from the 2026-08-20 T4 dispatch rather than a guessed fix: the sub-candle-rung wiring and route-through-then_params items both need real `ChildOrder`/`MarketData` plumbing this session explicitly didn't map; the population-measurement item is a data/manifest query outside this dispatch's code-only tooling; the PB.8-into-sub-candle correction is a genuine `SubCandleBar` design question flagged 'not safe to guess in safety-adjacent fill-fidelity code'; the new HandlerRegistry production-caller item needs an unmapped `ExecutionOrchestrator`/`algorithm_factory` subsystem identified first; retire-campaign-scripts lives in a different repo (e2e-testing) outside this doc's 2-repo scope. Safety-adjacent execution/fill-model code — conservative KEEP-NA is correct, none of the 6 is safely mechanical.
