...
- [ ] [INFRA] P1. deployment-service — once the foreign `paper_week_determinism_scheduler.tf` duplicate-`blrs_image` local is removed by its owner, `tofu apply` the gas-fees consolidator job+cron (deployment-service@f0f7ded), then relaunch `launch-mtds-gas-fees-backfill-vm.sh` and verify it climbs past 2021-01-01 (no `ManifestConsolidatorStaleError`). Target repo: deployment-service.
