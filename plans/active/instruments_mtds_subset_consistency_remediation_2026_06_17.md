---
doc_type: plan
title: Instruments â MTDS subset + consistency remediation
summary:
status: active
nature: process
...
source:
  [
    "plans/audit/results/instruments_mtds_subset_and_consistency_audit_2026_06_17.md (findings F1âF7, full-index walk)",
    operator 2026-06-17 (deep-dive audit dispatch),
  ]
drift_direction: advance-code
---

# Instruments â MTDS subset + consistency remediation

> **ð´ PRE-`--apply` BLOCKER GATE (2026-06-17).** The dry-run projections that `--apply` will materialise STILL carry
> these defects, and a reconcile `--apply` over uncovered path shapes flips real `captured`â`attempted_failed`
> (CLAUDE.md hard rule). **Do NOT `--apply` until these are fixed + the projection regenerated + re-eyeballed:** (1)
> **prefix_tpls coverage** â prove `ASSET_GROUP_CONFIG[ag]["prefix_tpls"]` covers ALL coexisting shapes
...
> after/parallel): F1, F3, N2, N4, F6, N8. **Apply order: pred â tradfi (clean) â cefi â sports â defi; never all-AG at
> once.**

Findings of record + method: `plans/audit/results/instruments_mtds_subset_and_consistency_audit_2026_06_17.md`.

> **ð¢ SCRIPT-COVERAGE MAP (2026-06-17) â every blocker is a GAP in the existing rebuild scripts, not unscripted.** The
> rebuild scripts ARE the migration: fix the gap â regenerate the dry-run projection â improved beta â `--apply`
> (path-schema) â backfills. Per finding: **prefix_tpls** â `canonical_path_templates(ag)` covers all shapes (sports
> `[""]` â verify only); **N3** â ï¸ `rebuild_sports_manifest_v9` never extracts `league_id`/`league` from the MTDS
> object path into the row_key (canonicalizer `_canonicalize_row_key_league_id` then gets null); **N1** â ï¸
> `rebuild_cefi` CF-11 dedup key mismatches (empty re-emit has blank `instrument_type` vs captured populated â both
> survive); **N5** â `rebuild_defi` emits `captured`/row_count=0 on file PRESENCE without opening (0-row/pre-launch â
> false captured) â route via `record_zero_rows`; **N6** â ï¸ `rebuild_defi._split_legacy_venue_chain` lacks
> instrument_type case-norm + lets pairs leak into `chain` + incomplete venue-dedup; **F3** â `rebuild_cefi` passes
> legacy `attempted_failed` reasons through un-reclassified; **N2** â instruments enumerator marks CME weekend
> carry-forward as `SOURCE_RETURNED_ZERO`. **Each Phase-A/B/D todo below = a scoped fix to the named script â regen that
> AG's dry-run projection â re-audit the fixed dimension.** Phase-1 (manifest-level, full v9-projected-index walk) is
> DONE; Phase-2 (file-level cross-year manifest-vs-reality sampling) is IN PROGRESS via per-AG sub-agents â findings
> fold back into the audit doc + new todos here.

> **ð´ð´ GCS DELETE SAFETY INVARIANT â READ BEFORE DELETING ANY OBJECT (codified 2026-06-18; HARD RULE).** **The
> manifest migration RELABELLED nothing â it is a CELL-KEYED rewrite (`_index` rows keyed by
> `(date,venue,data_type,instrument_type,instrument_id,underlying)`, NOT by GCS path); the v9 data was physically COPIED
> to canonical paths by `migrate_*_v9_canonical` (COPY not MOVE â the legacy bare `asset_group=`/`category=`/top-level
> `day=` shapes are DUPLICATES that still exist).** Therefore an agent must **NEVER assume a legacy object is safe to
> delete.** A legacy object is delete-safe **ONLY IF a twin exists already in CANONICAL format** â
> `raw_tick_data/by_date/day={D}/pipeline_mode={mode}_{source}/asset_group={ag}/â¦` (defi: + normalized venue/itype) â
...
> vs MIGRATE-FIRST (no canonical twin â COPY to canonical, THEN it becomes delete-safe).** Delete-list + migrate-first
> list + the 48h e2e-research-data accounting are built by the read-only rescan â
...
> (inspectâconfirmâdelete); migrate-first MUST complete first.**

## GCS delete safety â path/schema migration prerequisite map (DONE-before-DELETE)
...
1. **`pipeline_mode={mode}_{source}/` prepend** (primary) â every bare `â¦/day={D}/asset_group={ag}/â¦` object needs a
   `â¦/day={D}/pipeline_mode={mode}_{source}/asset_group={ag}/â¦` twin (mode/source via UTL
   `derive_pipeline_mode_for_row`). Tool: `migrate_{cefi_flat,defi_full,tradfi}_to_v9_canonical.py` (COPY).
   MIGRATE-FIRST = bare objects the rescan finds with no `pipeline_mode=` twin â run the migrate to create the twin.
2. **`category=`â`asset_group=`** â DONE (0 `category=` objects remain on cefi/defi/sports; verified).
3. **DeFi venue/itype canonicalization in the PATH** (N5r/N6r) â the canonical defi twin must be at the NORMALIZED venue
   (`UNISWAP_V3` not `UNISWAPV3`, `_canonical_venue` SSOT) + lowercase `instrument_type` (`pool` not `POOL`). An object
   at an un-normalized venue/itype path is NOT a canonical twin â migrate it (copy to the normalized canonical path)
...
4. **per-AG verification** â after migrate-first, re-run the rescan's twin-verify per AG; require **100% canonical-twin
...
canonical-only reads to miss ZERO orphans, every captured cell must resolve to a canonical-format object â which is
exactly what migrate-first guarantees. **Sequence: rescan â migrate-first the untwinned (â 100% canonical-twin coverage,
verified per AG) â canonical-only reads are orphan-free â THEN delete legacy (operator-gated).** Deleting before 100%
coverage would orphan the un-migrated cells under canonical-only reads â the invariant above prevents exactly that.

## Execution sequence (end-to-end â the autonomous worker drives this in order)

Each script-fix step = fix â `quality-gates.sh`-green â `quickmerge --agent --files` â **regenerate that AG's dry-run
projection** (`rebuild_{ag}_manifest.py --dry-run --projection _index/audit/projected_index_{ag}.parquet`) â **re-audit
the fixed dimension** (the `/tmp/audit_subset.py` pattern or a per-AG file re-check) â flip the todo + journal
before/after numbers. Order:

1. **CeFi script** `rebuild_cefi_manifest.py` â **N1** dedup key (captured suppresses its blank-type empty shadow) +
   **F3** reclassify legacy `attempted_failed` recon-noise (~1.3M) vs keep genuine ~88k. Verify: no captured+empty
   double-rows; attempted_failed â ~88k.
2. **Sports script** `rebuild_sports_manifest_v9.py` â **N3** extract `league_id`/`league` from the MTDS object path +
...
3. **DeFi script** `rebuild_defi_manifest.py` â **N6** normalize instrument_type case (pool/POOL), keep pool-pairs OUT
   of `chain` (only known chain tokens), collapse venue dups; **N5** route 0-row/pre-launch files through
   `DefiManifestRecorder.record_zero_rows` (venue-launch-date-aware) instead of presenceâcaptured. Verify: no
   token-pairs in chain, single-case instrument_type, no pre-launch captured-0-row vault cells.
4. **Instruments enumerator** (instruments-service) â **N2** CME/TradFi weekend carry-forward = honest carry-forward
   (not `SOURCE_RETURNED_ZERO`); de-dup 2Ã-per-cell index rows.
5. **prefix_tpls VERIFY** (`reconcile_phantom_manifest_rows_all.py` `ASSET_GROUP_CONFIG`) â prove
   ...
   templates. **APPLY FOOT-GUN â uncovered shape â apply flips real capturedâattempted_failed.** Block apply for any AG
   whose coverage isn't proven.
6. **Regenerate ALL projections â re-audit** = the IMPROVED beta. Confirm F1âF7 + N1âN8 resolved/honestly-classified;
   record before/after in the audit-doc Progress Log.
7. **PRE-MIGRATION DRAIN GATE (HARD, CLAUDE.md)** â before ANY `--apply`: gracefully stop ALL running VMs (GCP+AWS) +
...
8. **`--apply` AG-by-AG, safest first: pred â tradfi â cefi â sports â defi.** Per AG: prefix_tpls green + projection
   re-audited clean â run the real path-schema migration â verify the live `_index` matches the projection (NO mass
   capturedâfailed flip) â next AG. **Never all-AG at once.** Mass-flip â STOP + diagnose prefix_tpls, do not continue.
9. **Backfills** â F1 (Kraken+ instruments history), the ~88k genuine cefi failed cells (legacy `error_reason`
   `VENUE_FETCH_FAILED`; **`[A16 NOTE]` retired from live EMISSION â `classify_venue_error()` else
   `UNCLASSIFIED:{code}`, verified in MTDS `engine/orchestrator/sentinels.py:267-269`; 482,518 historical rows still
   carry the label so it is a VALID historical selector â task unchanged, wording only**), any real captured-absent
   cells. Run to completion (manifest-verified rows). **â The FULL path to 100% (could-exist enumeration + per-AG
...
gate â STOP+document, don't apply that AG. Genuine human hard-stops unchanged: live wallet keys, `1.0.0` graduation.

## ð¢ AUTONOMOUS COMPLETION PLAN (2026-06-18) â drive ALL to verified-working, EXCEPT the delete (operator-gated)

> Operator `/autonomous` 2026-06-18: complete everything (~1-2h, parallelise) to a working+verified state; **the ONLY
> thing NOT to do is DELETE the old data** â but size it all up so the delete is ready. Loop until done; journal each
> tick.

**State now:** cefi legacy delete DONE (9.98 TB, recoverable). cefi fully migrated. defi/tradfi/sports/pred
object-migration was a broken VM run (0 twins) â VMs STOPPED â focused defi diagnosis+fix sub-agent `acb89f8f6b5c9a943`
IN FLIGHT (gets defi producing twins e2e via a copy-driver off the audit parquet's pre-computed
`legacy_pathâcanonical_twin_path`, then reports recipe + tradfi/sports/pred feasibility). Manifests (all 5) already
canonicalized + reconciled (cell-keyed, correct).

**Ordered completion (drive in this order; parallelise within a step):**

1. **defi migration working e2e** (sub-agent acb89f8f) â verify migrate-firstâ0 for defi (twin-audit). [GATE: proves the
   recipe]
2. **Fan out the recipe** to tradfi/sports/pred (parallel per-AG copy-drivers off each
   `legacy_dup_delete_list_{ag}.parquet`). tradfi dash-separated/pred-restructure shapes that are UN-mappable â
   re-download-or-bespoke (decide+document, don't fake).
3. **B3 â research-data copy across**: HL `perp_funding`/`perp_daily_ctx` (`perp-funding-*`) + LST (`lst-rates-*`) â
   canonical placement (+ manifest record_captured) + e2e doc (oldâcanonical mapping so e2e scripts repoint).
   Independent â can run â¥.
4. **Manifests reflect canonical** â re-verify all 5 live `_index` are cell-correct post-migration (already
   canonicalized; confirm no regression).
5. **Orphan check** â per AG, every captured cell has a canonical object (twin-audit migrate-firstâ0). THE gate for
   delete-safe.
6. **data_type + schema checking** â the migrated canonical objects carry the right data_type partition + parquet schema
   (sample-open per AGÃdata_type; confirm canonical objects == legacy content/schema, not just present).
7. **Reader cutover** â repoint deployment-api drilldown + MTDS readers to canonical `pipeline_mode=` ONLY, remove ALL
   legacy fallbacks / multiple-SSOT (safe once 5/6 complete per AG). cefi can cut over now.
8. **SIZE UP the final delete for ALL AGs** â re-run the twin-audit â per-AG SAFE-TO-DELETE delete-lists (legacy objs
...
**Hard-stop (operator):** the final DELETE of old data â prepare+size it, never execute.

## Progress Log â B0/B1/B2 autonomous run (2026-06-18, dispatch)

> Operator `/autonomous` 2026-06-18: NEW Databento key live in SM `databento-api-key`, Tardis public, DeFi creds exist â
> no credential blockers. Drive B0âB1âB2 to verified completion. This log is the loop's handoff memory (no summary doc).

**Discovery (read-first, 2026-06-18):**

- **Data state** (instrument_availability/by_date latest day): tradfi=2026-06-11, defi=2026-06-11, cefi=2026-06-17 â
...
- **B2 â MVP IS ALREADY CODIFIED**: `unified_api_contracts.canonical.crosscutting.mvp_scope.MVP_SCOPE` + `is_mvp()`
  (config v3) â catalogue (`build_instrument_catalogue.py`) + enumerator already consume it. The "total reasonable
  ...
  explicit + distinct.
- No code tarball in `gs://deployment-scripts-â¦/code/instruments-code.tar.gz` (need `create-code-tarballs.sh` first).
- No instr-backfill VM currently running.

**Discovery â instrument-store state per AG (read-first, 2026-06-18 22:30 UTC; IS@02cb876 + UAC@aeae389 + subscription
guard installed):** ran the IS `--operation status` + read each `instruments-store-{ag}-prd`
`_index/availability_index.parquet`:

- **tradfi â ALREADY FULLY BACKFILLED to date (B0 effectively done for tradfi):** 11,418 captured / 256 empty_confirmed,
  cov 1.0, **0 attempted_failed, 0 date gaps.** 6 venues continuous DAILY: CME/FX/ICE/CBOE 2020-01-01â2026-06-18,
  NASDAQ/NYSE 2023-04-15(subscription start)â2026-06-18 (distinct-days == calendar-span â no missing day). The new
  3-dataset subscription guard (`assert_databento_request_allowed`, dataset-level shard-isolation) is installed on the
  IS `definition` fetch but matters only for FUTURE/forced fetches â existing tradfi instrument rows are already the
  ...
  keep-green).**
- **cefi â cov 0.999 (28,552 captured / 22 attempted_failed); real F1/F2 gaps confirmed:** KRAKEN-SPOT/KRAKEN-FUTURES
  have only 2 days (2026-06-17/18) vs earliest_venue_date 2020-01-01 â **~6yr backfill needed**; LIGHTER-ZKSYNC
...
- **defi â cov 0.998 (75,706 captured / 172 attempted_failed):** 95 venues, 2020-01-20â2026-06-18. 172 failed to
  diagnose.
- **sports â high cov on most entities;** RED-by-design: SFI_LEAGUES/SFI_STANDINGS/TRANSFERMARKT_LEAGUES cov 0.000 (all
  attempted_failed â credentialed/blocked sources, see F-track). INJURIES/ODDS ~0.96.
- **prediction â NO per-AG instruments-store entry in the bucket SSOT** (`Available: CEFI/DEFI/SPORTS/TRADFI`); resolves
  to the FLAT kind `instruments-store-pred-prd-central-element-323112`. 500 captured POLYMARKET,
  2025-03-14â**2026-06-09** (9-day stale; the `--operation status` path can't read the flat-kind bucket â status-CLI
...
- **The IS CLI is idempotent + manifest-driven:** a re-run on a date already fresh in the manifest SKIPs ("all N
  venues/entities already fresh â use --force"). So a backfill targets dates NOT in the manifest (the absent venues /
...
**B0 EXECUTION â 2026-06-18 ~23:00 UTC (monitored local CLI, log dir `/tmp/is_backfill_logs/`):**

- **tradfi B0 = COMPLETE** (already; no action). cov 1.0, 0 gaps, 2020-01-01â2026-06-18, 3-dataset contract.
- **cefi F1 â Kraken 6yr backfill**:
  `instruments-service --asset-group cefi --venues KRAKEN-SPOT KRAKEN-FUTURES --start-date 2020-01-01 --end-date 2026-06-18`
  â RUNNING in background (Tardis source, ~40 records/day across both venues). The LONG leg (~2,360 days Ã 2 venues,
  ~10s/day) â ETA a few hours; left to run to completion + reports its state. Idempotent (skips fresh days). Log:
  `kraken_f1.log`.
- **cefi F1 â 3 absent venues backfilled DONE/near-done**: LIGHTER-ZKSYNC (2024-08-01â, 198 instr/day),
  EXTENDED-STARKNET (2024-10-01â, 103/day), PACIFICA-SOLANA (2025-06-01â2026-06-18 â **DONE**, 10/day). All via
  Tardis/native adapters, creds present, 0 errors.
- **cefi F2 â BITGET 5 missing days**: re-fetched 2024-11-08â2026-06-18 (`--venues BITGET-FUTURES BITGET-SPOT`),
  near-done, fills the F2 gap.
- **prediction freshness â â DONE**: `--asset-group prediction --start-date 2026-06-09 --end-date 2026-06-18` wrote
  **12,720 records across 30 venues** (POLYMARKET CLOB full scan ~1.4M markets + KALSHI 2000) to
  ...
  asset_group entry) â a status-CLI display gap, NOT a backfill blocker (the WRITE path resolves via
  `resolve_instruments_store_kind`).
- **defi**: 95 venues, cov 0.998, 2020-01-20â2026-06-18 â already broadly complete; 172 attempted_failed
  (MORPHO-ETH/BASE 41 each, DRIFT-SOLANA 41, AAVE_V3-OPTIMISM 41, TRADER_JOE_V2-AVALANCHE 6, SUSHISWAP_V3-BASE 2, all
  UNCLASSIFIED_ADAPTER_ERROR, 2026-05-09â06-18).
- **sports**: most entities high-cov; SFI_LEAGUES/SFI_STANDINGS/TRANSFERMARKT_LEAGUES cov 0.000 (all attempted_failed â
  credentialed/blocked scraper sources, tracked in sports_master DEFERRED-INDEFINITELY scraper set). Not a B0 gap.

- [ ] [CODE] P3. **`--operation status --asset-group prediction` can't read the flat-kind bucket** â
      ...
      `resolve_instruments_store_kind`â`instruments-store-pred`). Teach the status path to use
      `_get_instruments_bucket_for_asset_group` (the same resolver the write path uses) so prediction status renders.
      Display-only gap; the backfill WRITE path already works. â instruments-service
- [ ] [DATA] P2. **Stale `attempted_failed` rows survive a failedâcaptured retry in the consolidated `_index` (manifest
      dedup blank-column edge â KNOWN, already tracked)** (surfaced 2026-06-18 while backfilling the fixed venues).
      After re-fetching a previously-`attempted_failed` shard to `captured`, the consolidated
      `_index/availability_index.parquet` carries BOTH rows for the same (date, venue) â e.g. DERIBIT-COMBO 2026-05-23
      ...
      `record_failed` leaves them blank while the captured retry populates them â populated-vs-blank delta keeps BOTH
      ...
      IS instruments-store indices â but do NOT hand-edit the dedup machine here (deliberate design tradeoff with a
      named owner). â unified-trading-library (dedup) â cross-link
      `pipeline_mode_source_batch_live_replay_standardisation_2026_06_05`
- [x] â [DATA] P2. **Diagnosed all 172 defi attempted_failed cells (2026-05-09â06-18) â 4 of 6 venues fixed, 2 are
      deeper upstream changes (split below)**. Each was UNCLASSIFIED_ADAPTER_ERROR from a distinct upstream API
      change: - **MORPHO-ETHEREUM (41) + MORPHO-BASE (41) â â ADAPTER FIXED + RE-FETCHED**
      (instruments-service@ec3fd3a): Morpho renamed `Market.uniqueKey`â`marketId` (HTTP 400 "Cannot query field
      uniqueKey"). Live verify: 968 markets fetched (was 0); re-fetched 2026-05-09â06-18 (164 captured rows written
      2026-06-18 23:1x). **The captured rows land under the CANONICAL bare venue `MORPHO`** (the writer keys the shard
      by the adapter's `venue` property `"morpho"`â`MORPHO`, NOT the per-record chain-suffixed `venue_tag`
      `MORPHO-ETHEREUM`) â and `MORPHO` already has **1,669 captured rows 2024-01-08â2026-06-18** (the historical
      canonical capture). So the 41+41 `MORPHO-ETHEREUM`/ `MORPHO-BASE` `attempted_failed` rows are an ANOMALOUS
      chain-suffixed venue-naming VARIANT, NOT a genuine data gap â the morpho lending markets ARE captured + current
      under `MORPHO`. (Same multi-source venue-naming drift the manifest-canonicalisation track owns â see the
      venue-naming P2 below.) - **TRADER_JOE_V2-AVALANCHE (6) + SUSHISWAP_V3-BASE (2) â â SELF-RECOVERED + canonical-tag
      captured**: both fetch 1000 pool instruments cleanly (transient subgraph rate-limits, not a code bug). Re-fetched;
      captured under the canonical bare `TRADER_JOE_V2` (74 captured 2026-05-09â06-18) + `SUSHISWAP_V3` (2,606 captured
      2023-04-05â06-18). The `-AVALANCHE`/`-BASE` chain-suffixed `attempted_failed` rows are the same anomalous variant
      â data captured under the canonical bare venue. - **DRIFT-SOLANA (41) + AAVE_V3-OPTIMISM (41)**: genuine deeper
      upstream changes â split to the two P2 todos below. â instruments-service
- [x] â [DATA] P2. **DeFi manifest venue-naming drift â `_index` reconcile DONE (grain DECIDED = PROTOCOL-CHAIN)**
      ...
      already-canonical `AAVE_V3`+chain). **GRAIN DECISION: PROTOCOL-CHAIN** â the UAC SSOT `ALL_DEFI_VENUES` is 150/159
      protocol-chain, so the canonical instrument venue grain is `PROTOCOL-CHAIN` (`AAVE_V3-ETHEREUM`), NOT bare. The
      live `_index` venue column was canonicalised 91â58 venues (71,799 rows re-pointed via the reader-SSOT
      `VenueMapping.normalize_defi_venue` resolver) + 861 captured legacyâcanonical spelling-dedup, folded into the v9
      column-population walk (instruments-service@7a63be9 â APPLIED). Captured preserved 75,942â75,081 (â861
      all-captured twins, 0 captured cell shadowed). â instruments-service
- [ ] [CODE] P2. **DeFi venue-grain â align the ADAPTER/writer shard key to the decided PROTOCOL-CHAIN grain** (the
      ...
      no re-reconcile needed. â instruments-service / unified-trading-library (manifest shard key) â composes with the
      `*_manifest_canonicalisation_*` + `source=` provenance tracks
- [x] [DATA] P2. **DRIFT-SOLANA instrument adapter â `data.api.drift.trade/stats/markets` now 404** (diagnosed
      2026-06-18). The Drift Data API endpoint moved: `/stats/markets`â404, `/markets`â403, `/contracts`/`/perpMarkets`
      â403 (auth-gated), `dlob.drift.trade`â502. Find Drift's current PUBLIC markets endpoint (docs at
      ...
      UAC `get_solana_protocol_url("drift","api_url")` â update the registry value, not a hardcode), classify the breach
      properly, backfill 2026-05-09â06-18. â instruments-service / unified-api-contracts (registry URL) â SHIPPED
      2026-06-19: rewrote `drift.py` to parse Drift SDK TypeScript constants on GitHub
      (`MainnetPerpMarkets`/`MainnetSpotMarkets`) via regex bracket-depth walk â 55 active perps + 73 spots. SDK URLs in
      UAC registry at `sdk_perp_markets_url`/`sdk_spot_markets_url`. Backfill ran 2026-05-09â2026-06-19 (42 dates, 40
      instruments/day). Manifest now shows `DRIFT` + `chain=SOLANA` = `captured` (42 rows). IS@87099cc, UAC@74509df.
- [x] [DATA] P2. **AAVE_V3-OPTIMISM IS instruments adapter must route to the RPC fallback (KNOWN abandoned subgraph â
      NOT a subgraph-ID hunt)** (diagnosed 2026-06-18). The instruments adapter queries the subgraph
      `3RWFxWNstn4nP3dXiDfKi9GgBoHx7xzc7APkXs1MLEgi` which raises `Type Query has no field reserves` â attempted_failed.
      **This is the DOCUMENTED operator policy (UAC `_defi.py` aave_v3 OPTIMISM comment, decision 2026-05-30): Aave
      ...
      â never leave it attempted_failed (a known-policy state masquerading as a fetch failure). The sibling chains
      (ETH/ARB/POLY/BASE/AVALANCHE) work fine. â instruments-service (NOT a UAC subgraph-ID change) â SHIPPED
      2026-06-19: added static 7-reserve fallback (`_AAVE_V3_OPTIMISM_STATIC_RESERVES`) with DERIVED citations per STEP
      5.97. `get_instruments()` shortcircuits for OPTIMISM chain before subgraph call â returns 12 instruments (5
      borrowing-enabled Ã 2 = 10 + 2 non-borrowing Ã 1 = 2). Backfill 2026-05-09â2026-06-19 (42 dates). Manifest:
      `AAVE_V3` + `chain=OPTIMISM` = `captured` (42 rows). IS@87099cc.

**DERIBIT-COMBO â fixed a NEVER-WORKING venue (4 stacked breaks, found during cefi diagnosis) â â SHIPPED:** cefi's 22
attempted_failed were ALL DERIBIT-COMBO (0 captured days since added 2026-05-23). Root cause = 4 stacked bugs, all
fixed: (1) Deribit retired `get_instruments?kind=combo` (HTTP 400) â switched to `public/get_combos`; (2) adapter tagged
records `venue=DERIBIT` but batch canonical is `DERIBIT-COMBO` â URDI venue-tag filter dropped all rows â fixed the
venue property; (3) legs were always empty (`_parse_combo_legs` always returned `[]`) and validation rejects leg-less
COMBOs â build `InstrumentLeg` from `get_combos` structured legs; (4) `DERIBIT-COMBO` was absent from UAC
`VENUES_BY_ASSET_GROUP["cefi"]` + `CEFI_VENUE_LAUNCH_DATES` â validation rejected "unknown venue" â registered it.
...
days running (`deribit_combo.log`) â combos active today land captured; expired-combo days the get_combos endpoint no
longer returns stay honest (the API only returns currently-active combos â historical combo state is not retrievable, an
upstream limitation, NOT a silent placeholder).

> **Dependency order (operator 2026-06-18):** (B0) backfill instruments to NO-MISSING first â (B1) regen the instrument
> catalogue (it aggregates instruments) â (B2) codify MVP-universe vs total-reasonable-universe (so the backfill config
>
> - data-status "could-exist" are correct) â these gate/inform each other. Research-data canonical-copy (B3) is
>   independent. Cross-links: `path_to_100pct_backfill_mtds_is_2026_06_17.md` (the backfill-to-100% home).

- [ ] [INFRA] P1. **B3 â copy e2e research data to CANONICAL placement + e2e doc**: HL `perp_funding`/`perp_daily_ctx`
      ...
      home per data_type â the dedicated `-prd-` bucket (`lst-rates-prd`, exists) vs the
      ...
      in-region) the research objects â canonical placement (+ manifest `record_captured` so the `_index` reflects
      them). (c) Write `e2e-testing/docs/` (or the e2e README) a note: research reads MUST migrate to the canonical
      sources â list the oldâcanonical bucket/path mapping so the e2e funding scripts
      (`staked_basis_funding_scan`/`colocated_engine`/etc.) update their fetch paths. Then the research buckets become
      deletable (operator-gated). â instruments-service/deployment-service + e2e-testing(doc)
- [ ] [INFRA] P1. **B1 â instrument catalogue regen + un-pause (aggregation/dedup; "has this instrument ever existed" +
      ...
      **PAUSED** + last ran ~2026-06-11/15 (STALE, pre-backfill). AFTER B0 (instrument backfill no-missing): re-run the
      regen jobs per AG â verify the catalogue reflects the full deduped instrument lifecycle
      (genesis/first-seen/last-seen per instrument) â decide cadence + un-pause the daily schedulers (or keep manual).
      data-status "could-exist" + the expected_unattempted enumerator (`enumerate_expected_universe.py`) read this â
      stale catalogue = wrong could-exist universe. â instruments-service/deployment-service
- [x] â [DESIGN] P1. **B2 â codify MVP-universe vs TOTAL-REASONABLE-universe (NOT codified anywhere â confirmed gap)**:
      ...
      data-status read from. â unified-api-contracts/instruments-service â **UAC SSOT SHIPPED:
      unified-api-contracts@b654eb6** â `canonical/crosscutting/total_universe.py` (`TOTAL_UNIVERSE_AXES` per-AG
      ...
      classifier MVPâTOTAL; config-version descriptor) + 9 unit tests, all exported from the UAC root facade. The
...
- [ ] [DATA] P0. **B0 â backfill instruments to NO-MISSING (prereq for B1 catalogue + all expected-universe
      ...
      available/expected â this runs FIRST. â instruments-service

## GCS object-migration COMPLETE + delete-list sizing (2026-06-18) â DELETE IS OPERATOR-GATED
...
Re-audit (`audit_legacy_gcs_dup_delete_list.py --ag defi,tradfi,sports,pred`) confirms **migrate-first â 0 on every
mappable cell** â every SAFE-TO-DELETE legacy object has a `gcs_describe`-verified canonical twin. Delete-lists written
to each AG `_index/audit/legacy_dup_delete_list_{ag}.parquet`.

| AG        | copied twins  | SAFE-TO-DELETE | reclaimable   | unmappable residue (NO twin â stays legacy, NOT delete-safe) |
| --------- | ------------- | -------------- | ------------- | ------------------------------------------------------------ |
...
target â excluded from every delete-list (tracked P2 residual below). **DELETE of the 168.72 GB SAFE-TO-DELETE set is
operator-gated â sized + inspect-ready, NEVER auto-executed.**

**Reader cutover DONE (deployment-api@0e267be):** data-status/drilldown readers now read canonical `pipeline_mode=`
paths ONLY â `storage_facade.list_objects` is pipeline_mode-aware (fans out canonical layers + dedups â no
...
already-canonical v9 manifest (not GCS paths) â unaffected. **Schema/data_type preservation:** established by
...
## MARKET-DATA `_index` v9 COLUMN POPULATION â APPLIED to ALL 5 AGs (2026-06-18)
...
> â the v9 column population was never run on the live index.

**DECISION â in-place populator, NOT the object-scan rebuild (root-cause diagnosis).** The `rebuild_{ag}_manifest.py`
...
projections carry massive true duplicates â `projected_index_cefi_v2` 1.08M dup rows, defi 0.67M, sports 0.73M â and
`projected_index_prediction` REGRESSES captured 16,918â7,116. Applying any would CORRUPT the deduped live index
(capturedâfailed mass-flip / dup inflation = the exact gate-fail the BLOCKER GATE forbids). Instead built
`market-tick-data-service/scripts/populate_v9_index_columns_inplace.py` (mtds@6b9f4b5): reads the live deduped `_index`,
fills `pipeline_mode` via UTL `derive_pipeline_mode_for_row` (100% derivable, verified all 5 AGs; source-aware â tradfi
splits barchart/massive/databento), `source` via UAC `source_string_for(PipelineMode)`, `asset_group` constant,
`schema_version=9` â ROW-PRESERVING, so captured is provably preserved. defi additionally picks up the **46,866
...
**Per-AG result (beforeâafter, captured-preserved gate honored absolutely):**

| AG     | rows beforeâafter     | schema v9 | pipeline_mode | source | asset_group | captured (Î)                   | snapshot |
| ------ | --------------------- | --------- | ------------- | ------ | ----------- | ------------------------------ | -------- |
| pred   | 19,299 â 19,299       | 100%      | 100%          | 100%   | 100%        | 16,918 (+0)                    | â        |
| tradfi | 144,314 â 144,314     | 100%      | 100%          | 100%   | 100%        | 96,811 (+0)                    | â        |
| cefi   | 2,167,688 â 2,167,688 | 100%      | 100%          | 100%   | 100%        | 1,311,984 (+0)                 | â        |
| sports | 803,796 â 803,796     | 100%      | 100%          | 100%   | 100%        | 202,087 (+0)                   | â        |
| defi   | 1,578,922 â 1,625,788 | 100%      | 100%          | 100%   | 100%        | 344,564 â 391,430 (+46,866 V4) | â        |
...
pred â tradfi â cefi â sports â defi (safest first). Tool is a `scripts/` oneoff (ruff-lint-clean + runtime-verified via
5 applies; lifecycle marker present).

- [x] â [SCRIPT] P1. **MARKET-DATA `_index` v9 column population `--apply` for ALL 5 AGs** â DONE 2026-06-18.
      ...
      dup-inflated/captured-regressing â would corrupt the deduped index). â market-tick-data-service

## MARKET-DATA `_index` venue/instrument_type SPELLING canonicalisation (N6r) â DONE for defi; pred/tradfi/cefi/sports VERIFIED-CLEAN (2026-06-18)

> Operator dispatch 2026-06-18: "everything needs to be canonical, fix it all" â now UNBLOCKED by the legacy-duplicate
> delete (2,874,085 objects / 168.72 GB freed â GCS holds ONLY canonical-spelling objects). The live `_index` still
...
(`pipeline_mode=*/asset_group=*/venue=*/` delimiter-walk) â the ACTUAL venue/itype spelling SET on disk; diffs vs the
...
| ------ | --------------- | -------------------- | ------------------------------------------- | ----------------- | ---------------------------- |
| pred   | 2               | 1 (POLYMARKET)       | `UNKNOWN`(21)                               | 0                 | CLEAN + 1 stragglerâtodo     |
| tradfi | 6               | 6                    | 0                                           | 0                 | **CLEAN â no action**        |
| cefi   | 21              | 18                   | `COINBASE`(7)/`OKX`(7)                      | 0                 | CLEAN + tiny stragglersâtodo |
| sports | 29              | 27                   | `UNIBET_EU`(11)/`UNKNOWN`(3)                | 0                 | CLEAN + tiny stragglersâtodo |
| defi   | 30              | 30                   | **50 venue spellings (256k captured rows)** | 0                 | **REBUILT (N6r)**            |

**DeFi (the headline) â `canonicalize_mtds_index.py` extended with N6r venue-spelling-canon + dedup-merge (mtds, this
run):** the prior "defi venue NOT normalised (STOP)" block (object-desync risk under COPY-not-MOVE) is now SATISFIED
_by_ the rewrite â a legacy-spelling index row (`UNISWAPV3`/`AAVEV3-ETHEREUM`) points at a DELETED object spelling and
MUST be re-pointed to the canonical spelling (`UNISWAP_V3`/`AAVE_V3`) whose object is the only one left on disk.
**GCS-VERIFIED remap rule** (NOT blind `_canonical_defi_venue`): remap `V`âcanon(V) ONLY when canon(V)â V AND the
LITERAL `V` venue dir is ABSENT from canonical GCS; a venue still live on GCS is KEPT. This protected the two genuine
coexisting-distinct-data exceptions â `SUSHISWAP` (captured rows resolve 12/12 to `venue=SUSHISWAP` objects, 0/12 to
`SUSHISWAP_V3`) and `YEARNV3` â which a blind canon would have desynced/false-merged. Dedup-merge after the remap keeps
the CAPTURED row over any non-captured twin.

**Content gate (CRITICAL) â PASSED:** dry-run on the live defi `_index`: 48 spellings remapped (254,812 captured rows
re-pointed; SUSHISWAP/YEARNV3 kept), 24,280 duplicate cell-keys collapsed (23,866 all-captured legacyâcanonical twins +
414 all-non-captured; **0 keys mix captured+non-captured** â no captured cell ever shadowed). captured 391,430 â 367,564
rows = exactly the legitimate legacyâcanonical captured-twin dedup (â23,866); **every distinct captured cell-key
...
- [x] â [SCRIPT] P1. **DeFi `_index` venue-spelling canon + dedup-merge (N6r) `--apply`** â DONE 2026-06-18.
      `canonicalize_mtds_index.py` N6r (GCS-verified venue remap: literal-goneâcanon-exists; KEEP SUSHISWAP/YEARNV3
      still-live; captured-first dedup-merge). Snapshot `_index/snapshots/pre_canonical_rebuild_defi_2026_06_18.parquet`
      (1,625,788 rows) â `--apply` â live defi `_index` now **1,601,508 rows** (48 spellings remapped, 254,812 captured
      rows re-pointed, 24,280 dup cell-keys collapsed). **Independently re-verified post-apply** (300-day GCS venue
      walk): captured 391,430â**367,564** (= legitimate â23,866 legacyâcanonical captured-twin dedup; every distinct
      captured cell-key preserved), **0 captured venue spellings absent from canonical GCS** (KAMINO/MARGINFI/etc. are
      genuine canon venues whose objects exist â the audit-sample "stragglers" are false-positives of a narrow day
      ...
      mislabeled-captured remnants tracked as the 3 todos below. â market-tick-data-service **Ship-path note
      (2026-06-18):** the 2 scripts (`canonicalize_mtds_index.py` N6r + new `audit_index_vs_gcs_spellings.py`) shipped
      to LDR via the **dirty-deps + foreign-WIP carve-out** (mtds@6db7713, direct push) â quickmerge was structurally
...
- [ ] [DATA] P2. **pred `_index`: 21 captured `UNKNOWN`-venue `trades` cells (2025-03-14..)** â GCS has
      ...
      label drift). â market-tick-data-service
- [ ] [DATA] P2. **cefi `_index`: `COINBASE`(7)+`OKX`(7) captured rows with BLANK data_type/instrument_type** â GCS has
      `venue=COINBASE-SPOT`/`OKX-SPOT`/`OKX-SWAP` (market-type-suffixed), NOT bare `COINBASE`/`OKX`. These are malformed
      blank-shard-dim aggregate captured rows with no concrete object; the bareâsuffixed map is AMBIGUOUS (SPOT vs
      ...
      (sample miss, no action). â market-tick-data-service
- [x] â [DATA] P2. **sports `_index`: `UNIBET_EU`(11)+`UNKNOWN` captured rows under wrong `pipeline_mode`** â DONE
      2026-06-19 (mtds@ba21ee5 `recover_sports_mtds_index_leagues_2026_06_19.py`, APPLIED+verified live). GCS-verified
      remap: the captured UNIBET_EU objects live under `pipeline_mode=batch_odds_api/venue=UNIBET_EU/league_id=<L>/` â
      re-stamped pipeline_modeâbatch_odds_api + sourceâodds_api + recovered league_id (was null). Folded into the
      combined N3a/F4/N9 recovery (one snapshot, one apply). â market-tick-data-service

## INSTRUMENTS-STORE buckets â legacy GCS audit + `_index` canonicalisation COMPLETE (2026-06-18)

> Operator-granted: delete legacy twins once 100%-verified canonical twin (`gcs_describe`). This is the
> **instruments-store** (reference-data) analogue of the market-data delete work above â a DISTINCT bucket set
> (`instruments-store-{cefi,defi,tradfi,sports}-prd-â¦` + `instruments-store-pred-prd-â¦`), NOT the `market-data-tick-*`
> buckets (those are mid-delete and were NOT touched here).

**KEY FINDING â instruments-store has NO `pipeline_mode=`/`asset_group=` twin model.** It is reference data (one AG per
...
`instruments-service/scripts/audit_instruments_store_legacy_gcs_delete_list.py` (per-AG delete-list parquet â
...
| ---------- | --------- | ------- | ---------------------- | -------------- | -------------------------------- | ----------------- |
| cefi       | 28,524    | 7       | 0, 0                   | 0              | 0                                | canonical â       |
| defi       | 66,894    | 6       | 0, 0                   | 0              | 0                                | canonical â       |
| tradfi     | 11,648    | 8       | 0, 0                   | 0              | 0                                | canonical â       |
| sports     | 110,138   | 778,010 | 2, 9,721               | 0              | 9,723 (0.146 GB)                 | canonical â       |
| prediction | 4,932     | 50      | 0, 0                   | 0              | 0                                | canonical â       |

**Migrate-first + delete: nothing to do (safe outcome).** cefi/defi/tradfi/pred are 100% canonical â zero legacy
...
delete-list, reported only â never deleted):

- bare top-level `day=2026-03-21/venue=BETFAIR/<hash>.parquet` (n=2): a BETFAIR odds-instrument write (different venue
  than the canonical `venue=API_FOOTBALL_FIXTURES`, no `league=` key â no canonical-rename twin).
- `instrument_availability/by-date/day-{D}/{soccer_slug}/instruments.parquet` (n=9,721, dates 2020-06-06..2025-12-15, 52
  ...
  `selection` schema; venues ONEXBET/PADDYPOWER/PINNACLE/BETFAIR/UNIBET â from `oddspapi_historical_backfill.py`). The
  canonical `by_date/` (underscore) shape is api-football FIXTURES reference (different data source + schema + a
  non-translatable `soccer_germany_bundesliga`â`BUNDESLIGA` league-slug map), and it COVERS the same date range + is
  written more recently (tip 2026-05-13 > legacy tip 2025-12-15) â the dash shape is superseded, not a twin. Deleting it
  needs an operator/explicit decision (it is orphan stale data, but has no canonical replacement to verify against).
  **MIGRATE-FIRST=0 mappable** â the only "legacy" is unmappable by construction (no canonical-rename target exists).

**`_index` (manifest) canonicalisation â now COMPLETE for ALL 5 AGs** (the N2 2x-per-cell + blank-status v4 shadow
defect). `canonicalize_instruments_store_index.py` was already applied to tradfi+sports; I applied it to **cefi**
(15,933 blanks classified, 12 dups dropped â 28,586â28,574 rows) and **defi** (127,140 blanks classified, 121,109 dups
dropped â 196,987â75,878 rows), and re-ran tradfi idempotently (dropped 3 residual dups â 11,674). Re-verified: **every
AG now has blank_status=0 AND dup_cells=0.** prediction was already clean (500 rows, 0 blank, 0 dup).

- [x] â [SCRIPT] P1. **instruments-store legacy GCS audit + per-AG delete-list** â DONE
      instruments-service@`audit_instruments_store_legacy_gcs_delete_list.py`. cefi/defi/tradfi/pred 100% canonical (0
      legacy); sports 9,723 unmappable-superseded (excluded, reported); SAFE-TO-DELETE=0 fleet-wide â no delete needed.
      Delete-list parquets written to each `_index/audit/`. â instruments-service
- [x] â [SCRIPT] P1. **instruments-store `_index` blank-status/dedup canonical for ALL 5 AGs** â DONE (cefi+defi+tradfi
      `--apply`'d; sports/prediction already clean). Every AG blank_status=0 + dup_cells=0. **â ï¸ This was the DEDUP
      pass, NOT the v9 COLUMN pass â see the new v9-column item below.** â instruments-service
- [x] â [SCRIPT] P1. **instruments-store `_index` v9 COLUMN-population for cefi/defi/tradfi/prediction** (the dedup pass
      above was NOT this â audited 2026-06-19, the live IS `_index` was a v4/v8/v9 MIX with `source` 0%, `asset_group`
      column ABSENT, `pipeline_mode` mostly blank). `populate_is_index_v9_2026_06_19.py` row-preservingly stamps
      schema*version=9 + asset_group + pipeline_mode (blankâ`batch_instruments_service`) + source (DERIVED PER CELL via
      `source_string_for(pipeline_mode)`, NOT a default). DeFi additionally venue-canonicalised 91â58 (PROTOCOL-CHAIN
      SSOT) + 861 captured spelling-dedup. **APPLIED cefi/defi/prediction** (verified live: schema_v9=100%,
      source/asset_group/pipeline_mode=100%; captured preserved â cefi 36,062 / pred 791 / defi 75,081 = â861 legitimate
      spelling-dedup). **tradfi v9-column apply DEFERRED until the running DBEQ/CBOE per-date backfills finish** (avoid
      clobbering their in-flight per-VM-shard writes; the consolidator merges them). Snapshots â
      ...
      on blank batch producer rows (C-#6-identity-safe; +3 regression tests). â instruments-service@7a63be9 +
      unified-trading-library@f8ec9096
- [ ] [SCRIPT] P3. **`canonicalize_instruments_store_index.py` can't resolve the prediction bucket** â `_bucket_for`
      ...
      is already canonical â 500 rows, 0 blank, 0 dup â nothing to canonicalize), but the `--asset-group prediction`
      choice is a dead path. Fix `_bucket_for` to route prediction â
      `kind="instruments-store-prediction",     asset_group=None` if prediction ever needs re-canonicalisation.
      **NICE-TO-HAVE** (provenance: 2026-06-18 instruments-store audit). â instruments-service

## CME event contracts (binary-settlement EC\* series) â FINISH the backfill (operator 2026-06-19)

CONFIRMED in Databento: all 9 `EC*` event contracts (ECES/ECNQ/ECRTY/ECYM/ECGC/ECCL/ECNG/EC6E/**ECBTC** â BTC binary,
...
- [ ] [DATA] P1. **CME EC\* event-contract backfill â v9-certification dependency only** (execution owned by
      ...
      CERTIFICATION explicitly checks EC\* coverage (esp. ECBTC). Do NOT launch a duplicate EC\* backfill here â defer
      to the plan-of-record. â market-tick-data-service / instruments-service

## Forthcoming credentials (operator 2026-06-19 â note now, unblock on arrival)

Operator is acquiring these â record as pending-credential so the backfill runs the moment the keys land (NOT memory;
tracked here per the durable-facts rule):

- [x] â **Kalshi credential UPLOADED 2026-06-19** â `kalshi-api-credentials` v1 in Secret Manager (JSON
      `api_key_id`/`key_id` + RSA `private_key` PEM; account has no funds, market-data-only). The credential-registry
      already maps `"kalshi" â kalshi-api-credentials`.
- [ ] [CODE] P1. **Wire Kalshi into the pipeline (hist + live market data)** â the credential is stored; now wire the
      ...
      Kalshi backfill. â mtds / instruments-service (prediction)
- [x] â **Extended public market data needs NO API key â "operator applying for API" was a FALSE blocker for the data
...
- [x] â **IS Extended adapter: per-instrument genesis (honest `available_from`)** â instruments-service@9bb7cdfd. Probes
      ...
      2024-07-26â2026-05-22; **50/103 markets have candle history pre-dating their `createdAt`** (BTC/ETH from
      2024-07-26 testnet vs createdAt 2025-07-18 mainnet-migration bulk-stamp), so neither a global constant nor
      `createdAt` is honest â only the probed candle-genesis is. Fix produces 58 distinct `available_from` dates (was
      1). basedpyright clean; IS QG green (88.24% cov).
- [ ] [DATA] P2. **Run the Extended public instrument + perp backfill (UNBLOCKED â no key needed)** â IS daily-listing
      CLI for EXTENDED-STARKNET (genesis-accurate now) + MTDS OHLCV/funding capture over 2024-07-26âyesterday (funding
      only from 2025-08-01 mainnet). Verify honest coverage converts `expected_unattempted`â`captured`. â mtds /
      instruments-service (defi/cefi perp)
- [ ] [CODE] P2. **Harden MTDS Extended candle sharp edge (silent truncation)** â the live `_umi_extended.py` candle
      fetch sends `{interval, limit:1440, endTime}` with NO `startTime`; the API caps a single response at ~2800â3000
      ...
      `limit` + a LOUD truncation warning so a multi-day/finer-interval call can never under-capture silently. â mtds
- [ ] [CODE] P3. **Align/consolidate the two parallel Extended candle paths** â the live path is
      ...
      per-instrument) OR delete it (no live importers). Parallel-paths anti-pattern per Delete-Deprecated-Code. â mtds
- Tardis: `tardis-api-key` (+ `-backup`, `-full`) already in SM â provisioned (not a gap).

## Databento SUBSCRIPTION CONTRACT (operator 2026-06-18 â supersedes PAYG model)

**No longer PAYG** â subscription + ~$150 credits (more than enough to stream all instruments). **ONE API key**
(`databento-api-key`, single-key â operator chose collapse-to-single-key) across **exactly 3 datasets**: `GLBX.MDP3`
(CME) + `DBEQ.BASIC` (Databento US Equities) + `CFE` (CBOE Futures). Any other dataset â reject.

Schema â free-window entitlement (a request's `start` must be â¥ `today â window`; clip/reject otherwise):

| Level | Schemas                                          | Free window | Guard                 |
| ----- | ------------------------------------------------ | ----------- | --------------------- |
| L0    | `ohlcv-1s`, `definition`, `statistics`, `status` | 16 years    | start â¥ today â 16y  |
| L1    | `trades`, `tbbo`, `mbp-1`, `bbo-*`               | 1 year      | start â¥ today â 365d |
| L2    | `mbp-10`                                         | 1 month     | start â¥ today â ~30d |
| L3    | `mbo`                                            | 1 month     | start â¥ today â ~30d |

Codify this (schemaâwindow table + 3-dataset allowlist) as the SSOT (UAC) + enforce as a pre-request guard in the
Databento adapter(s) â replaces the PAYG-cost-blocker framing (cost emission stays as credit-burn telemetry; the hard
guard is now entitlement window + dataset, surfaced as 403/entitlement not 402/payment). Instruments = `definition`
schema = L0 (16y window) â the instrument backfill can pull the FULL universe within the 3 datasets, cost-free within
credits. Tracked todos below.

- [x] â [CODE] P1. **Databento subscription cutover (MTDS+UAC+IS)** â DONE: single-key config
      (`use_multi_key_rotation=False`, `num_api_keys=1`, `DEFAULT_NUM_API_KEYS=1`; num_keys-asserting test fixed to read
      `get_num_api_keys()`; transitional secret `databento-api-key-1` DELETED â only `databento-api-key` remains) +
      schemaâfree-window + 3-dataset allowlist SSOT (`databento_subscription_allowlist.py` @31db3b0) enforced as the
      pre-request guard at the MTDS get_range chokepoint AND the IS `definition`-schema fetch (with dataset-level shard
      isolation so an off-allowlist dataset doesn't hard-fail siblings). â market-tick-data-service@88d1c65e /
      instruments-service@86ecc67b / unified-api-contracts@3b76c0bc | all QG green.
- [x] â [SCRIPT] P1. **B0 instrument backfill within contract**: backfill `definition` (L0, 16y) for GLBX.MDP3 +
      DBEQ.BASIC + CFE â full universe (credits cover it). â instruments-service â **ALREADY COMPLETE** (verified
      ...
      2020-01-01â2026-06-18, NASDAQ/NYSE 2023-04-15(subscription start)â2026-06-18 (distinct-days == calendar-span â no
      missing day). The 3-dataset subscription guard (`assert_databento_request_allowed`, dataset-level shard-isolation)
      is installed on the `definition` fetch â banned datasets isolate (not hard-fail), existing rows are already the
      right 6-venue universe (no banned datasets present). Forward daily keep-green only. See Progress Log "B0
      EXECUTION".
- [ ] [CODE] P1. **`ohlcv-1s` has NO `BarTimeframe` member â OHLCV close-edge conversion raises** (surfaced 2026-06-18
      during the subscription cutover): the contract fetches ONLY `ohlcv-1s`, but `_OHLCV_DATA_TYPE_TIMEFRAME` (mtds
      `databento_adapter.py`) + the UAC `BarTimeframe` closed set (`bar_boundary.py` â smallest unit is `15s`) have no
      ...
      ALL in one commit). Until landed, the OHLCV path must AGGREGATE `ohlcv-1s`â1m/15m/24h before the close-edge stamp
      ...
      no longer depend on this gap.) â unified-api-contracts / market-tick-data-service / features-service

## Autonomous-run residuals (2026-06-18, surfaced during the migration drive)

- [x] â [CODE] P2. **Batch-query GCS scanner is a second canonical-path SSOT** â DONE deployment-api@c003271.
      `path_combinatorics.to_gcs_prefix` â `to_gcs_prefixes` (list) now builds the canonical
      ...
      (ac60e4a). Direct-LDR push (dirty-deps carve-out: UAC dep had live foreign Databento WIP). â deployment-api

- [x] â [CODE] P1. **e2e funding scripts hardcode legacy research buckets â repoint to `resolve_bucket_name`** â DONE
      ...
      `gcs_copy_object` import = runtime ImportError â restored to canonical `cloud_interface` form). e2e QG green
      (foreign untracked `verify_unmappable_legacy_content_aware.py` set aside â its `qg-cloud-sdk` noqa trips the
      TID251 ratchet, see N-residual below). Direct-LDR push (dirty-deps carve-out: live foreign UAC WIP). â e2e-testing
- [ ] [INFRA] P2. **Research `-prd-` buckets carry NO `_index/` â move the availability index off the legacy
      `perp-funding`/`lst-rates` buckets before they can be deleted** (DIAGNOSED 2026-06-18, larger than a config edit â
      ...
      `manifest_consolidator_buckets` covers only `instruments-store-*` + `market-data-tick-*`, env-tiered + legacy â
      ...
      `INDEX_BUCKET = "perp-funding-central-element-323112"` (LEGACY flat) â the live `_index` sits only in the legacy
      bucket; there is no consolidator cron for these stores. **EXACT STEPS:** (1) add 4 entries to
      `manifest_consolidator_buckets` (gcp) + the AWS Batch/EventBridge equivalent â
      `perp-funding-prd-${env}-${project}`, `lst-rates-prd-${env}-${project}` (the canonical `-prd-` homes; resolve via
      the `perp-funding`/`lst-rates` `cloud-providers.yaml` keys, NOT new hardcodes) â with a `*/1` cron + IAM
      ...
      writer if/when one exists â `kind="lst-rates"`); (3) one-shot seed: copy the legacy
      `perp-funding-central-element-323112/_index/` + `lst-rates-central-element-323112/_index/` â their `-prd-` twins
      ...
      `e2e-testing/docs/defi/research_data_canonical_sources_2026_06_18.md`. â deployment-service/e2e-testing

- [x] â [CODE] P3. **e2e `verify_unmappable_legacy_content_aware.py` non-ruff `# noqa: qg-cloud-sdk`** â FIXED by the
      ...
      `google.cloud.storage` import â so there is NO TID251 site at all (e2e `tid251` stays == baseline 10), no `# noqa`
      needed, and `ruff check` + the STEP 5.95 ratchet + full e2e QG all pass clean. â e2e-testing

### Migration unmappable residue â DIAGNOSED 2026-06-18 (the 10,250 `MIGRATE-FIRST` objects)

**SSOT confirmed**: the per-AG audit parquet `_index/audit/legacy_dup_delete_list_{ag}.parquet`
(`classification â {SAFE-TO-DELETE, MIGRATE-FIRST}`) IS the source of truth. `MIGRATE-FIRST` /
`reason=no_venue_or_data_type_in_path` rows = exactly **defi 5,332 + tradfi 1,102 + sports 3,816 + pred 0 = 10,250**,
all `twin_exists=False` / empty `canonical_twin_path`. They are NOT junk â sampled file contents (smallest-first,
...
non-hive directory layout (`equities/NYSE/`) â the canonical target is **derivable from FILE CONTENTS**, not the path.

**Per-AG path-shape distribution + contents:**

| AG     | shape (count)                                                                                                              | rows/file                          | content columns (derive canonical from these)                                                                                                    |
| ------ | -------------------------------------------------------------------------------------------------------------------------- | ---------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------ |
| defi   | `by_date/day=*/asset_group=defi/venue={VENUE}-ETHEREUM/ticks*.parquet` (5,332)                                             | 1 (631 tiny) â 44k (4,701 bundles) | `data_type` (e.g. `oracle_prices`), `source` (`aave_oracle`), `instrument_key` (`ETHENA-ETHEREUM:YIELD_BEARING:sUSDe`), `timestamp`, `price_usd` |
| tradfi | `by_date/day=*/data_type=ohlcv_*/{equities,etf,futures_chain,indices,spot,options_chain}/{EXCH}/{INSTR}.parquet` (**995**) | 0â697k                             | already canonical-shape OHLCV (`timestamp,open,high,low,close,volume[,symbol,instrument_key]`); path carries data_type+exch+instr                |
| tradfi | bare `by_date/day=*/venue={CME,ICE,NYSE,NASDAQ}/ticks.parquet` (**107**) + 15 are 0-row empties                            | 0âsmall                            | legacy pre-canonical day bundles                                                                                                                 |
| sports | `by_date/day=*/source=ODDS_API[/league=*]/ticks.parquet` (3,245) + `venue=ODDS_API[/league=*]/ticks.parquet` (571)         | 6â8.9k                             | `venue` (bookmaker: pinnacle/fanduel/â¦), `data_type` (`odds`), `instrument_id`, `sport_key`, `event_id`, `league_id` â full odds rows           |

**â ï¸ DIAGNOSIS CORRECTED 2026-06-18 (CONTENT-VERIFIED, supersedes the path-only verdict above).** The prior "unique
...
(`migrate_defi_full_v9_canonical.py`) ALREADY consumed â not unique unmigrated data.** Sampled coverage (decisive):

- **defi**: 303,394 / 303,394 legacy rows across 4 random days (2024-09..2025-12) have their exact
  `(data_type, instrument_key)` present in same-day canonical content â `0` uncovered. The ETHENA `oracle_prices` bundle
  ...
  fan-out RESHAPES the path (venue `ETHENA-ETHEREUM`â`ETHENA`, adds `chain=`/`instrument_type=`, per-instrument stem,
...
- **sports**: 58,910 / 58,910 sampled legacy `instrument_id` present in same-day canonical `pipeline_mode=batch_*`
  content (canonical splits across `trades`/`odds`; the legacy bundle has no `data_type` column â match against the
...
NOT others (2025-03/04/06 sampled â zero canonical UNISWAP_V4 that day). The 359 legacy `venue=UNISWAPV4-ETHEREUM`
objects (`swaps`+`liquidity`, ~9.7k rows/day) carry the human-pair `instrument_key`
(`UNISWAPV4-ETHEREUM:POOL:ETH-USDC:3000`) where canonical V4 uses the `pool_id` hash stem â so on the un-captured days
...
- **TWIN-VERIFIED-SAFE (â9,891 of 10,250 â everything except UNISWAP_V4)**: NOT unique data; their content already lives
  in canonical. â reclassify SAFE-TO-DELETE (operator-gated deletion, joins the legacy delete-list). NO migration needed
  â migrating would DUPLICATE existing canonical data.
- **UNISWAP_V4 (â¤359 objects, the genuine gap)**: content-aware fan-out the legacy V4 `swaps`/`liquidity` rows that
  have NO same-day canonical V4 twin â reshape to the canonical V4 schema (`venue=UNISWAP_V4`/`chain=ETHEREUM`/
  `instrument_type=pool`/`data_type=dex_pool_swaps|dex_pool_state`, `pipeline_mode=batch_onchain_subgraph`, conform to
  `_CANONICAL_UNION`) keyed by the legacy `pool_address`â`pool_id` stem; manifest-verify each migrated cell.
...
- [x] â [DATA] P2. **Content-aware verification of the 10,250 unmappable legacy objects** â SHIPPED the content-aware
      ...
      against the same-day canonical `pipeline_mode=` content UNION â reclassifies TWIN-VERIFIED-SAFE / MIGRATE-NEEDED /
      ...
      PRE-CANONICAL SOURCE the v9 fan-out ALREADY consumed â migrating them would DUPLICATE existing canonical data.
      ...
      delete-list, the migrate/delete DECISION is already determined by the verifier's logic + sampling. â e2e-testing
- [x] â [DATA] P2. **UNISWAP_V4 content-aware fan-out (the genuine residual) â MIGRATED + VERIFIED**
      (`e2e-testing/scripts/defi/migrate_uniswap_v4_legacy_to_canonical.py` @9bd18bf, RAN `--apply` 2026-06-18). Of
      4,919,235 legacy V4 rows across 359 bundles, **3,237,107 genuinely-uncovered rows â 31,773 canonical
      `pool_id`-stem parquets written** (1,682,128 rows already in canonical â skipped no-dup; 84 objects fully-covered;
      **0 errors**). Reshape column-parity vs an existing canonical V4 file = EXACT (37-col set identical). Verified:
      ...
      walk / consolidator run (folded into the N5r/N6r rebuild-for-real-replace below â do NOT run a competing partial
      rebuild). â e2e-testing
- [ ] [DATA] P3. **Verify-then-delete the ~122 genuinely-legacy-only tradfi stragglers** â REFRAMED 2026-06-18: the 107
      bare `venue={CME,ICE,NYSE,NASDAQ}/ticks.parquet` are recent (2026-02+) RAW `data_type=trades` ticks (Databento
      market-by-order), NOT pre-canonical OHLCV â and `data_type=trades` IS a canonical tradfi data_type
      ...
      bare-107, 100% covered). So they are NOT a separate straggler class â the verify parquet's TWIN-VERIFIED-SAFE set
      is the delete-safe list (operator-gated). 0-row/empty legacy = honest no-data, delete-safe once confirmed empty. â
      e2e-testing

## Phase A â subset violations (MTDS data with no instrument backing)

- [ ] [DATA] P1. **F1 â backfill instruments-service for CEFI venues MTDS has but instruments lacks historically**:
      `KRAKEN-SPOT`/`KRAKEN-FUTURES` (added to instruments only at day=2026-06-17 â ~6yr gap), `LIGHTER-ZKSYNC`,
      `PACIFICA-SOLANA`, `EXTENDED-STARKNET`. Re-run the IS daily-listing CLI across the MTDS-covered date range per
      venue (never copy between dates). Verify the cefi (venue,date) subset closes. â instruments-service â **ð¢ IN
      PROGRESS / mostly DONE (2026-06-18 autonomous run):** LIGHTER-ZKSYNC (2024-08-01â06-18 â), PACIFICA-SOLANA
      (2025-06-01â06-18 â), EXTENDED-STARKNET (2024-10-01â06-18 â) all backfilled via the IS daily CLI (Tardis/native, 0
      errors). **KRAKEN-SPOT/FUTURES 6yr backfill RUNNING** to completion in background (2020-01-01â2026-06-18, Tardis,
      ~40 records/day; at 2024-06 as of 23:20, ETA ~1h; `/tmp/is_backfill_logs/kraken_f1.log`, monitored). Flip fully â
      once Kraken reaches 2026-06-18 (the monitor reports completion).
- [x] â [DATA] P2. **F2 â backfill 5 missing BITGET-FUTURES + 5 BITGET-SPOT instrument-days** that MTDS captured but
      instruments is absent for. â instruments-service â DONE 2026-06-18: re-ran the IS daily CLI
...
- [x] â [DATA] P1. **F4 â SPORTS: captured MTDS cells with NULL `league_id`** â DONE 2026-06-19 (subset of N3a below;
      same combined recovery mtds@ba21ee5). league_id recovered from the GCS object path for every backed cell; unbacked
      â honest `empty_confirmed`/SOURCE_RETURNED_ZERO. No captured cell remains league-less. â market-tick-data-service
- [ ] [DATA] P3. **F7 â DEFI: 19 Ethereum MTDS cells pre-instruments-genesis (2020-01-01..19)**. Confirm instruments
      defi genesis should start earlier, or mark those MTDS cells spurious. â instruments-service

## Phase B â instruments internal consistency

- [ ] [DATA] P0. **F3 â CEFI: 1.40M `attempted_failed` MTDS cells (36%)**. Break down by venueÃdata_type; diagnose the
      failing adapters/venues; backfill. (Data-pipeline-correctness heartbeat â no deferral.) â market-tick-data-service
- [ ] [CODE] P2. **F6 â TRADFI: 182k blank `instrument_type` + thin options (`options_chain` 3,287 vs `futures_chain`
      ...
      capture gap if real. â market-tick-data-service / instruments-service
- [x] â [DATA] P2. **F5 â SPORTS INSTR index hygiene** â FIXED instruments-service@7b7d3a3 (new
      `scripts/canonicalize_instruments_store_index.py`). sports v2 projection: blank `capture_status` **6,869â0**
      (6,869 malformed blank-data_type+blank-league rows dropped as no-shard-identity), `date='all'` **preserved (2,
      by-design reference entities)**, grain intact (35 league_ids in captured TEAMS). Verified independently. â
      instruments-service

## Phase C â file-level verification (Phase-2 sub-agents)

- [x] â [AUDIT] P1. **Cross-year file sampling per AG â DONE** (5 per-AG sub-agents opened real GCS parquets across
      ...
      captured (CME 8,602 opts/day, ES options_chain 20,956 rows) â the "thinness" is a typing artifact, REFUTED; **F5**
      `date='all'` (2 rows) is by-design reference entities. Discarded one false sub-agent claim (cefiâ tradfi).

## Phase D â file-level correctness findings (Phase-2 sub-agents, NEW)

- [x] â [DATA] P1. **N1 â CEFI phantom `empty_confirmed` shadow rows** â FIXED mtds@aaeada9. `_rebuild_cefi_cf11.py` now
      ...
      suppressed**; re-audit shows **capturedâ©empty shadow cells = 0** (was ~63k) + **capturedâ©failed = 0**. 33 unit
      tests (6 new). â market-tick-data-service
- [x] â [DATA] P1. **N2 â TRADFI CME weekend dishonest-empty + 2Ã-per-cell dup** â FIXED instruments-service@7b7d3a3.
      ROOT CAUSE: the v8/v9 re-emit APPENDED a row per cell instead of replacing the stale `schema_version=4` legacy row
      â every cell carried captured v8/v9 + a blank-status v4 shadow (`instrument_id=None` vs `""` hid the dup). New
      `canonicalize_instruments_store_index.py` does grain-aware de-dup + classify (count>0âcaptured incl CME
      carry-forward; count==0âempty via `non_trading_day_reason` EXPECTED_WEEKEND/HOLIDAY). tradfi v2: rows
      **20,404â11,630**, blank capture_status **11,301â0**, 2-row cells **8,774â0**, CME weekends = EXPECTED_WEEKEND
      (183), **SOURCE_RETURNED_ZERO=0**. Verified independently. â instruments-service
- [x] â [DATA] P0. **N3 â SPORTS league_id dropped** â FIXED mtds@aaeada9. REFRAME: the audit's "100% NULL-league"
      ...
      `_source_from_row` now resolves sports `trades`â`odds_api` + case-insensitive bridge. `projected_index_sports_v2`:
      null-league **202,087â32,707**, NULL source **73.7k+â6** (202,081 stamped odds_api). 28 tests (3 new). Residual
      tail (32,707 genuinely-null in LIVE + 6 null-source) â N3a/N3b below. â market-tick-data-service
- [x] â [DATA] P2. **N4 â SPORTS instruments `instrument_count==0`** â CONFIRMED NOT-A-DEFECT
      ...
      Left untouched â no fabricated counts (per-league grain + companion rows preserved in the v2 projection). â
      instruments-service
- [x] â [DATA] P1. **N5 â DEFI pre-launch `vault_share_price`** â FIXED (code mtds@3f5cc6e: rebuild routes pre-launch +
      ...
      97 ETHENA pre-launch (2023-11â2024-02) reclassified capturedâempty. **Residual** (rebuild-for-real-replace,
      tracked N5r below): the VAULT 2020-2022 0-row phantoms (~1,113) need the per-object rebuild applied to live (the
      index-walk can't open files). â market-tick-data-service
- [x] â [CODE] P1. **N6 â DEFI dimension normalization** â itype case FIXED (live: `POOL`â`pool`, 2,450 collapsed via
      canonicalize_mtds_index@d7b04b2 APPLIED). venue-spelling dedup CODE shipped (mtds@cf63cf6: `_canonical_defi_venue`
      replicates the migrator so manifest venue==object-path venue â SAFE only in the per-object rebuild, NOT the
      index-walk). **Residual N6r below.** â market-tick-data-service
- [ ] [DATA] P2. **N5r/N6r â DEFI rebuild-for-real-replace to land venue-dedup + VAULT-0-row + 496 chain-pollution on
      ...
      `pool_id` cells (3.24M rows) the content-aware fan-out wrote 2026-06-18** â the rebuild walks all canonical
      objects, so these get their `availability_index` rows on this same wholesale-replace run (no separate V4 manifest
      pass needed). â market-tick-data-service
- [x] â [DATA] P0. **F3 (reframed) â CEFI re-classify legacy-recon `attempted_failed`** â FIXED mtds@aaeada9.
      ...
      `LegacyBlankErrorReasonError`â`UNCLASSIFIED_ADAPTER_ERROR` preserved (visible/backfill-worthy). attempted_failed
      **1.40Mâ782,005** in `projected_index_cefi_v2`. Genuine `VENUE_FETCH_FAILED`(83,975)+`HTTP_429`(3,652) preserved â
      backfill Step 9. The ~698k UNCLASSIFIED reconcile-to-expected_unattempted is N1b (depends Step 4). â
      market-tick-data-service
- [ ] [CODE] P2. **F6 (reframed) â TRADFI option/instrument_type encoding**: unify the two options encodings
      (`instrument_type=options_chain` vs `data_type=options_chain` w/ blank type) + stamp instrument_type on the 182k
      blank-type cells (legacy path shapes). Not missing data â a typing fix. â market-tick-data-service
- [x] â [INFRA] P3. **N7 / Step-5 prefix_tpls VERIFY â DONE (no code change needed)**:
      `reconcile_phantom_manifest_rows_all.py` `prefix_tpls = canonical_path_templates(ag)` (CF-15/V0 UAC SSOT) for
      cefi/defi/tradfi/prediction â VERIFIED complete: enumerates every coexisting shape
      (`pipeline_mode=batch_<source>/`, bare `asset_group=`, legacy `category=`, top-level `day=`, defi
      `venue=PROTOCOL-CHAIN` overload + bare-venue). **Sports `[""]` is NOT a foot-gun** â sports routes to the
      dedicated `_audit_sports` + UAC `candidate_parquet_paths` SSOT (bucket kind=instruments-store), and ALL 17
      captured instruments-store-sports data_types (STANDINGS/TEAMS/FIXTURES/ODDS/â¦) resolve â¥1 candidate path.
      `--apply` will NOT mass-flip on any AG from a prefix-coverage gap. â instruments-service
- [ ] [DATA] P3. **N8 â PRED index data_type label drift** (`prediction_canonical_question_group` vs GCS
      `prediction_trades`/`trades`) + 1 blank-reason attempted_failed cell. Confirm intentional rollup label vs drift;
      type the blank reason. â market-tick-data-service
- [ ] [DATA] P1. **N1b â CEFI: reconcile the ~698k `UNCLASSIFIED_ADAPTER_ERROR` (ex-`LegacyBlankErrorReasonError`,
      ...
      failed row; genuine in-coverage listed-instrument gaps stay attempted_failed â backfill (Step 9). DEPENDS on
      Step 4. (Provenance: Step-1 fix kept them visible rather than hide a gap; final fate is
      enumerator+reconcile-driven.) â market-tick-data-service
- [x] â [DATA] P2. **N3a â SPORTS: 32,707 captured cells genuinely NULL-league in the LIVE index** â DONE 2026-06-19
      ...
      `league=`-or-`league_id=`). Result: captured-null-league **32,707 â 0**; the null aggregates EXPLODED to **177,118
      new per-league captured cells** (captured 202,087 â 346,498) â each backed by a verified GCS object (30/30
      sampled). Empty per-league shadow rows SUPERSEDED by the recovered captured (11,327). Genuinely-unbacked null
      cells (319) â `empty_confirmed`/SOURCE_RETURNED_ZERO (30/30 verified object-free). Snapshot
      pre_sports_league_recovery_20260619. â market-tick-data-service
- [x] â [DATA] P2. **N9 â MTDS SPORTS 17,288 blank-capture_status rows** â DONE 2026-06-19 (mtds@ba21ee5,
      APPLIED+verified live; blank/non-4-state capture_status **17,288 â 0**). All were `data_type=ODDS` venue=ODDS_API
      (footystats odds) + 6 MDPS-computed UNKNOWN. Classified by GCS backing in the SAME combined recovery: **3,852
      resolved to a footystats `venue=ODDS_API/data_type=odds/league=<L>` object â captured (+ recovered league)**; 319
      genuinely-unbacked â `empty_confirmed`/SOURCE_RETURNED_ZERO. No silent placeholder; captured never lost. â
      market-tick-data-service
- [x] â [CODE] P1. **N9b â DISPLAY-side bug: legacy `"None"`/NaN/non-4-state `capture_status` SILENTLY DROPPED from the
      ...
      legacy v4 sports `"None"`-status rows (N9) â plus any blank/NaN â were dropped from BOTH numerator and
      ...
      three to coerce any non-4-state token â `captured`: `data_status/coverage.py::_build_coverage_for_cat`
      ...
      should still be re-classified to a real status by the sports-MTDS canonicalize pass â this is the display-side
      defence so they read as captured (not vanished) in the meantime. â deployment-api
- [x] â [CODE] P0. **READER-SHAPE GAP â deployment-api drilldown now reads canonical `pipeline_mode=` (RESOLVED by the
      @0e267be/@c003271 cutover; verified 2026-06-18 data-status audit)**: `_shard_core._mtds_shard_path` builds the
      probe prefix `raw_tick_data/by_date/day={D}/pipeline_mode={mode}_{source}/asset_group={ag}/â¦` (fans across
      ...
      `pipeline_mode=batch_tardis/` with zero dupes; the legacy bare twin is NOT read â no double-count, no post-delete
      orphan. â deployment-api@d956a6e (audit verify; the cutover itself @0e267be/@c003271)
  ...
  > migrated.** cefi = 1,077,672 SAFE-TO-DELETE (~9.98 TB, byte-identical `pipeline_mode=` twins â I spot-verified 5/5
  > size-match) + 15 migrate-first. **defi (352,062) / tradfi (1,706,332, incl VIX) / sports (252,318) / pred (573,451)
  > = ALL MIGRATE-FIRST (~179 GB, NO canonical twin â verified 3/3 tradfi have twin_exists=False)** â their
  > canonicalisation never completed / was a RESTRUCTURE (pred renamed keys+stems; tradfi bulk is dash-separated
  > non-hive never canonicalised), so the legacy objects are the LIVE copy â deleting them LOSES DATA. **Only cefi is
  > delete-safe today.** e2e 48h research data: CLEAN â HL perp_funding/perp_daily_ctx in standalone `perp-funding-*`
  > bucket + LST in `lst-rates-*` (BOTH out of the 5 in-scope buckets); cefi funding reads the canonical
  > `pipeline_mode=batch_tardis` (the safe-delete list is their legacy twin â delete preserves reads); Aster/Drift
  ...
  > under canonical-only until their objects are migrated â the per-AG OBJECT migration is now a hard prerequisite for
...
- [ ] [INFRA] P0. **Migrate-first the 4 un-migrated AGs' OBJECTS to canonical `pipeline_mode=` shape
      (defi/tradfi/sports/ pred, ~2.88M objects / 179 GB)** â their canonical migration never completed (tradfi never
      ...
      workers=32) to create the canonical twins, then re-run the twin-audit â 0 migrate-first per AG. ONLY THEN are
      those AGs' canonical-only reads orphan-free + their legacy objects delete-safe. cefi needs NONE of this (already
      twinned). â market-tick-data-service / deployment-service
- [x] â [INFRA] P1. **Phase D rescan + delete-list â DONE + verified.** cefi SAFE-TO-DELETE list ready for operator
      ...
      OPERATOR-GATED (inspectâconfirmâdelete).
- [ ] [INFRA] P1. **Phase D â DELETE legacy GCS dupes (OPERATOR-GATED, cefi-only today)**: the bare
      ...
      canonical-only). Procedure: per AG, list bare `day=*/asset_group=` objects â verify each has a `pipeline_mode=`
      twin (via `gcs_describe_object`) â write the delete-list to `_index/audit/legacy_dup_delete_list_{ag}.txt` â
      **OPERATOR INSPECTS + confirms** â `gcs_delete_object` the confirmed bare twins (in-region VM, workers=32).
      Storage reclamation only; do NOT delete any bare object lacking a canonical twin (that would be unmigrated â
      migrate it first). â instruments-service/deployment-service
- [ ] [DATA] P2. **N9c â MTDS `_index` is NOT yet v9 for any of the 5 AGs; `pipeline_mode` column 100% BLANK
      ...
      `_apply_pipeline_mode_filter` chip (`coverage.py`) narrows to ZERO on any `batch_*` filter â the manifest rows
      have no pipeline*mode to match â even though the GCS objects ARE canonically
      ...
      non-blank > 0 post-rebuild per AG. â market-tick-data-service
- [x] â [DATA] P3. **N3b â SPORTS: captured cells still NULL source** â DONE 2026-06-19. Live-index audit shows captured
      NULL-source = **0** (already resolved on the live `_index`; the v9 source-stamp populated every captured cell â
      verified `source` nonblank 100%/803,796 pre-recovery). The combined recovery (mtds@ba21ee5) derives `source` from
      the recovered pipeline_mode for every emitted/re-stamped cell, so it stays 0. â market-tick-data-service

## SPORTS E2E audit + twin-migration drive (2026-06-19, autonomous dispatch) â Progress Log
...
- **MTDS `market-data-tick-sports-prd` `_index` = FULLY v9** â: 803,796 rows 100% schema_version=9,
...
- **MTDS remnants (OPEN)**: `UNIBET_EU`(11 captured) + `UNKNOWN`(3 captured) carry `pipeline_mode=batch_api_football`
  but are odds bookmaker venues â should be `batch_odds_api`. captured NULL-league = **32,707** (F4 subset =
  ODDS_API/ODDS 2,127 + odds_horizon_bucket 1,813; rest = bookmaker `trades` per-book rows). N9 17,288 blank-status NA
  rows.
- **IS `instruments-store-sports-prd` `_index`**: blank_status=0 + dup=0 â (the "v9-canonical" canonicalize DID run).
  ...
  5 AGs â DONE" OVERCLAIMS** â it only ran blank/dedup; the v9-COLUMN population (schema_version=9 + source +
  ...
  IS â in-place `_index` rewrite would race.)

**TWIN-COVERAGE (operator's core ask) â characterised across BOTH sports buckets:**
...
- **MD `legacy_dup_delete_list_sports.parquet`**: 252,318 objs = 248,502 SAFE-TO-DELETE (canonical_twin_verified) +
  **3,816 MIGRATE-FIRST** (`source=ODDS_API[/league]/ticks.parquet` 3,245 + `venue=ODDS_API[/league]/ticks.parquet` 571;
  ...
  ids in canonical) â confirm + write authoritative verify parquet.
- **IS `instruments_store_legacy_delete_list_sports.parquet`**: **9,723 UNMAPPABLE / twin_exists=False** = 9,721
  ...
  - **DECISIVE (corrects the plan's "superseded, MIGRATE-FIRST=0" verdict)**: canonical `venue=odds_api` in the `_index`
    ...
    matchbook/coral/betsson/skybet). Recent canonical IS days (2026-05-13) carry `venue=API_FOOTBALL` ONLY â **no
    canonical odds_api instruments exist**. â the odds-api instrument universe is GENUINELY-UNIQUE legacy data (backs
    the odds-api MARKET data in market-data-tick-sports) â **must be MIGRATED (canonical twin), not declared
    unmappable**.
  - **Migration = PATH canonicalisation** (data is fine):
    `instrument_availability/by-date/day-{D}/{soccer_slug}/ instruments.parquet` â canonical hive
    `instrument_availability/by_date/day={D}/league={canonical_league}/ venue=ODDS_API/instruments.parquet` (canonical
    IS shape confirmed = `by_date/day={D}/league={L}/venue={V}/`), via a soccer_slugâcanonical-league map (the-odds-api
    sport_keys). Untranslatable slugs preserved (no data loss). Then re-audit â 0 migrate-first â operator-gated delete
    is safe.
...
`transfermarkt-api-key` EXIST in SM. cov 0.000 â keys likely expired/invalid â BLOCKED-CREDENTIALS (validate/rotate),
NOT build. SFI_LEAGUES/SFI_STANDINGS/TRANSFERMARKT_LEAGUES are RETIRED data_types (runtime-only UAC catalog, cov-0
by-design â not a gap).

**REMAINING DRIVE (this dispatch):** [twin-migrate IS 9,721 + MD 3,816 â 0 migrate-first] â [MTDS UNIBET/UNKNOWN +
F4/N3a NULL-league + N9 classify] â [IS v9-column populate sports (coordinate af95b962)] â [credential asks] â
[shard-atom D] â [report + flips].

### Sports drive â Progress update (2026-06-19, autonomous tick 2)

**SHIPPED + VERIFIED:**

- **IS odds-api legacy twin-migration COMPLETE (operator's "make twins so delete loses nothing" ask)** â all **9,723**
  ...
  `LEAGUE_CLASSIFICATION_DATA_A/B[*]["odds_api_league_name"]`; added 4 missing display-name variants to the UAC dict â
  Liga-Profesional-Argentina/MLS/Superliga/accented-Primera-DivisiÃ³n). 7,721 twins (2,002 collisions = same-league
  two-source pairs with DISJOINT instrument_keys â read+concat+drop_dup UNION, no row loss; 2 bare BETFAIR
  hash-stem-preserved). 45/45 twins verified present+sized, 3/3 parity. Migration parquet
  `gs://instruments-store-sports-prd-â¦/_index/audit/sports_legacy_oddsapi_twin_migration_2026_06_19.parquet` (every
  delete-list legacy_path twinned). Shipped UAC@2224818 + instruments-service@308013f. **CORRECTS the plan's earlier
  "9,723 unmappable/superseded, MIGRATE-FIRST=0" verdict** â they were genuinely-unique odds-api INSTRUMENT data (canon
  `venue=odds_api` was empty_confirmed-only to 2020-06-05) backing the odds-api MARKET data â migrated, not abandoned.
- **MTDS sports `_index` recovery COMPLETE** (N3a/F4/N9/N3b/UNIBET) â mtds@ba21ee5, APPLIED+verified to live
  `market-data-tick-sports-prd`: captured **202,087 â 346,498** (per-league grain recovered/exploded from GCS, 177,118
  new backed per-league cells), **captured-null-league 32,707 â 0**, **blank capture_status 17,288 â 0**, NULL-source 0,
...
**STILL OPEN (this tick â next):** MD 3,816 twin-verify (sub-agent parked â finishing); IS sports `_index` v9-column
populate (schema_version=9 + source + asset_group â FLEET-WIDE gap, coordinate af95b962); IS
...
- [x] â [SCRIPT] P1. **Sports instruments-store `_index` v9-COLUMN populate** â DONE 2026-06-19
      (instruments-service@5d7f6f0 `populate_sports_is_index_v9_2026_06_19.py`, APPLIED+verified live
      `instruments-store-sports-prd`). schema_version **mix(v4/5/6/8, 75k/2.6M v9) â 100% v9**, asset_group **13k/2.6M â
      100% sports**, source **0 â 93.4%** (2,435,436 via the EXISTING UAC SSOT
      `unified_api_contracts.sports.get_source_for_data_type`; 171,227 / 6.6% blank = SSOT-unmapped catalog/retired
      data_types LEAGUES/VENUES/SFI_LEAGUES/SFI_STANDINGS/TRANSFERMARKT_LEAGUES â honest, the SSOT defines what is
      source-attributable). ROW-PRESERVING â captured 659,693 unchanged. pipeline_mode left blank (reference data, no
      mode model). Snapshot pre_sports_is_v9_20260619. â instruments-service
- [ ] [DATA] P2. **FLEET-WIDE: instruments-store `_index` v9-COLUMN populate for cefi/tradfi/defi (+ prediction
      source)** â the plan's earlier "instruments-store `_index` v9-canonical for ALL 5 AGs â DONE" (line ~575)
      ...
      column. Sports is now populated (above); apply the same per-AG (`get_source_for_data_type` analogue per AG â
      cefi/defi need a UAC source SSOT; tradfi has databento/massive in UAC SOURCE_PRIORITY). The live-WRITER
      source-auto-stamp (so NEW rows carry source, not just the historical backfill) is the larger scope. â homed under
      `data_source_provenance_all_asset_groups_2026_06_01.md` (the named owning plan for the source RED gap). â
...
- [x] â [DATA] P2. **SFI + Transfermarkt sports keys â UNBLOCKED + backfill launched** â DONE 2026-06-19. Operator
      provisioned the RapidAPI subscription (new key `840373â¦` on BOTH `soccer-football-info-api-key` v2 +
      `transfermarkt-api-key` v4, same key). **LIVE-SMOKED 2026-06-19 (slot-6, instruments-service .venv, real GCP)**:
      (a) SFI `get_match_descriptors_for_date(2025-03-01)` â HTTP 200, 1525 completed matches; `_fetch_sfi_data`
      end-to-end wrote **21,014 SFI_PROGRESSIVE_STATS rows** + manifest per-VM shard. (b) Transfermarkt RapidAPI
      `competitions/standings` GB1/2024 â HTTP 200 (NOT apify path);
      `_fetch_transfermarkt_data(PLAYER_VALUES, GB1, 2024)` â 20 player_values rows + master/snapshot tables + manifest
      shard. Prior 403 "not subscribed" is RESOLVED. Backfill VMs launched (auto-shutdown-on-completion, per-VM shards):
      4Ã `sfi-backfill-chunk-{1..4}of4-20260619-161036` (2020-01-01â2026-06-19, SFI 4 req/s; backfills ~69.7k
      expected_unattempted SFI cells) + 1Ã `tm-backfill-20260619-161123` (PLAYER_VALUES 2015-01-01â2026-06-19;
      ...
      SFI_LEAGUES/SFI_STANDINGS/TRANSFERMARKT_LEAGUES stay RETIRED (runtime-only UAC catalog). â instruments-service +
      deployment-service VM launchers
- [ ] [DATA] P2. **Verify SFI+TM backfill VMs ran to completion + manifest cells flipped** â the 5 backfill VMs (run-id
      ...
      re-read the sports availability index â `expected_unattempted` for `sourceâ{soccer_football_info,transfermarkt}`
      should drop sharply as cells flip to `captured`/`empty_confirmed`. â instruments-service [VM RUNNING]

### Sports A2/A3/D read-only verification â ALL PASS (2026-06-19, autonomous tick 4)

- [x] â [AUDIT] P1. **Sports A2/A3/D e2e consistency verification** â DONE 2026-06-19 (read-only sub-agent):
  - **A2a catalogue PASS** â `gs://instruments-store-sports-prd/prod/catalog.parquet` = 789-league roll-up, FRESH
    (rebuilt 2026-06-19T08:38Z), correct columns (instrument_id/league_id/available_from/mvp). available_to 100% null =
    open-ended-active (correct). Finding â todo below.
  - **A2b MVP vs TOTAL_UNIVERSE PASS** â sports present in `TOTAL_UNIVERSE_AXES["sports"]` (2 axes: fixtures
    DOWNLOAD_DERIVED + data_type HARDCODED_GENESIS) AND `MVP_SCOPE["sports"]` (`SportsMvpRule` 4 leagues EPL/LA_LIGA/
    NFL/NBA Ã 6 data_types); `universe_membership()` classifies MVPâTOTAL correctly (total_universe.py:241-254,
  ...
  - **A3 paths PASS** â all 6 representative data_types (FIXTURES/STANDINGS/ODDS/PLAYER_VALUES/SFI_PROGRESSIVE_STATS/
    WEATHER) resolve to actual GCS objects via `candidate_parquet_paths()`. No reader-shape drift.
  - **D shard-atom PASS** â `(data_type, league_id, date)` atom IDENTICAL across IS SSOT
    ...
    surface drops league_id. â verified read-only
- [ ] [DATA] P3. **Sports catalogue `mvp` column is 100% False (numeric league IDs vs is_mvp() canonical strings)** â
      `prod/catalog.parquet` `league_id` holds NUMERIC provider IDs (`'10'`/`'100'`) while `is_mvp()`'s SportsMvpRule
      keys canonical strings (`EPL`/`LA_LIGA`/`NFL`/`NBA`) â no sports league ever tags `mvp=True`. The catalogue
      builder should map the provider league_id â canonical league_id (UAC `league_data`/`provider_league_ids`) before
      the `is_mvp()` check, so the MVP subset is tagged. Low-risk display/classification fix (MVP tag unused downstream
      today). Provenance: 2026-06-19 sports A2a catalogue verify. â instruments-service
...
### Sports MD (market-data-tick-sports) twin-coverage â verify + fan-out (2026-06-19, autonomous tick 5)

Operator "make twins for ALL sports data lacking one so the delete loses nothing" â the MD bucket half.

- [x] â [DATA] P1. **MD legacy MIGRATE-FIRST twin-verification** â DONE 2026-06-19 (e2e-testing@1b07bcb
      ...
      TWIN-VERIFIED-SAFE** (content already canonical â delete-safe) + **700 MIGRATE-NEEDED** (genuinely-unique odds-api
      odds, days 2022-03..2023-04, where the day carries ONLY the legacy `source=ODDS_API` shape â the v9 fan-out never
      ...
      (58,910/58,910 sampled)" â that was a 6-file sample; the FULL run found the 700 gap.** â e2e-testing
- [x] â [DATA] P1. **MD 700 MIGRATE-NEEDED content-aware fan-out to canonical** â DONE 2026-06-19 (e2e-testing@1b07bcb
      `migrate_sports_md_unmappable_to_canonical_2026_06_19.py --apply`, RAN: 700/700 objects â 41,206 canonical cells /
      10,111,734 rows written). **RE-VERIFIED: the full twin-verifier now reports 3,816 TWIN-VERIFIED-SAFE / 0
      MIGRATE-NEEDED / 1,962,770 of 1,962,770 ids covered (100.0%)** â every MD legacy object is delete-safe. fans the
      700 genuinely-unique odds objects â canonical
      ...
      overwrite-lose, never delete legacy). On completion re-run the verify â MIGRATE-NEEDED must reach 0 â all 3,816
      delete-safe. Flip once re-verify == 0. â e2e-testing
...
once the fan-out completes + re-verifies. **Delete stays OPERATOR-GATED â never executed by the agent.**

## SPORTS E2E audit + remediation â FINAL REPORT (rule 9, autonomous run COMPLETE 2026-06-19)
...
executed). Concurrent agent af95b962 (IS coverage backfill) never collided â all my IS work was index-canonicalise +
...
| --------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------- |
| Twin coverage IS (operator ask)   | 9,723 legacy odds-api instrument objects â ALL canonical-twinned, delete-safe                                                                            | UAC@2224818 + IS@308013f; `sports_legacy_oddsapi_twin_migration_2026_06_19.parquet` |
...
| MTDS `_index` v9 + canonical      | 100% v9; null-league 32,707â0; blank 17,288â0; captured 202,087â346,498 (per-league grain recovered)                                                     | mtds@ba21ee5                                                                        |
...
| IS catalogue + MVP/total_universe | PASS â 789-league catalogue fresh; sports in TOTAL_UNIVERSE_AXES + MVP_SCOPE; universe_membership MVPâTOTAL                                              | sub-agent verify                                                                    |
| IS GCS paths                      | PASS â all 6 data_types resolve via candidate_parquet_paths()                                                                                            | sub-agent verify                                                                    |
| Shard-atom (D)                    | PASS â (data_type, league_id, date) identical IS/MTDS/data-status/UI                                                                                     | data_status_axis_matrix.py:70,105                                                   |
...
1. **Plan claim corrections** (both surfaced + fixed honestly): (a) "9,723 unmappable/superseded, MIGRATE-FIRST=0" was
   WRONG â they were genuinely-unique odds-api instrument data (canon venue=odds_api was empty_confirmed-only to
   2020-06-05) â migrated, not abandoned. (b) "instruments-store `_index` v9-canonical for ALL 5 AGs â DONE" OVERCLAIMED
   â it ran only blank/dedup; the v9-COLUMN populate was never run for ANY AG â done for sports here, fleet-wide gap
...
2. **MD 700 genuine gap**: the prior "all 3,816 TWIN-VERIFIED-SAFE (58,910 sampled)" was a 6-file sample; the FULL
   verifier found 700 genuinely-unique 2022-2023 odds objects on days with ZERO canonical content â fanned out (not
...
3. **3 captured-preservation bugs** caught by adversarial pre-apply verification before the MTDS recovery `--apply`
   (existing_keys captured-only + supersede; processed/ root; footystats `league=`/lowercase-`odds`) â would have
...
4. **Source-column scope split**: sports IS source backfilled now (UAC SSOT); the live-writer auto-stamp + cefi/tradfi/
   ...
   RED-gap owner) â not a sports deferral.

**Remaining open (all properly homed â NO sports-data-correctness deferral):** (1) FLEET-WIDE IS v9 for the OTHER AGs
...
### SPORTS â independent LIVE re-certification (2026-06-19, verify-not-redo dispatch)

A follow-up dispatch (verify the prior sports drive, finish any remainder, certify 100% twin-coverage). Read-only
re-verified EVERY claim against the LIVE prd buckets (no redo â all prior work confirmed APPLIED + correct). **Material
...
| --------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ------------------------------------------------------------------------------------------ |
| MTDS sports `_index` league-recovery APPLIED  | â captured **346,498** (== projection), captured-null-league **0**, blank-status **0**, NULL-source **0**, schema_version **100% v9**                                                                        | direct read `market-data-tick-sports-prd/_index/availability_index.parquet`                |
| IS sports `_index` v9 column-populate APPLIED | â schema **100% v9** (2,606,663 rows), asset_group **100% sports**, source **93.4%** (171,227 blank = SSOT-unmapped retired/catalog data_types â honest), blank-status **0**, captured **659,693** preserved | direct read `instruments-store-sports-prd/_index/availability_index.parquet`               |
| IS 9,723 odds-api twin-migration              | â 9,723/9,723 mapped (0 unmapped), 7,721 unique twins (5,719 MIGRATED + 4,004 MIGRATED-UNION, 2,368,129 rows, no row loss); twin sample 25/25 present on disk                                                | `sports_legacy_oddsapi_twin_migration_2026_06_19.parquet` + `gcs_describe` sample          |
| MD sports twin coverage                       | â 252,318 ALL delete-safe (248,502 path-twin `canonical_twin_verified` + 3,816 content-twin `TWIN-VERIFIED-SAFE`, the 700 MIGRATE-NEEDED fan-out re-verified 0)                                              | `legacy_dup_delete_list_sports.parquet` + `sports_md_unmappable_verify_2026_06_19.parquet` |
| Legacy DELETE executed (BOTH buckets)         | â IS legacy sample 0/25 still present (deleted, permanent), MD per-object `gcs_describe` twin re-verify before each delete                                                                                   | e2e-testing@0f1d761 `delete_sports_legacy_twinned_2026_06_19.py`                           |
| captured-preserved throughout                 | â MTDS 202,087â346,498 (per-league grain explode, never lost); IS 659,693 unchanged                                                                                                                          | both `_index` reads                                                                        |

**Delete-ready manifest â SPORTS row (now HISTORICAL â already deleted):** IS 9,723 legacy odds-api instrument objects
(0.146 GB) + MD 252,318 legacy objects (4.78 GB) â all twin-verified, operator-authorized, **DELETED 2026-06-19**. No
agent delete performed in this dispatch (delete was already done by the operator-authorized run).

**Sports is FOLDED INTO 100% twin-coverage on both buckets** â every captured cell is backed by a canonical-path object,
...
- [x] â [DATA] P2. **Residual sports MTDS bookmaker-`trades` pipeline_mode/source mislabel â re-stamped 559 cells** â
      ...
      CORRECT** â their object genuinely lives under
      `â¦/pipeline_mode=batch_api_football/â¦/data_source=ODDS_API/venue={V}/     league_id={L}/â¦/data_type=trades/`
      ...
      `batch_api_football`). Re-stamped only those 559 â `pipeline_mode=batch_odds_api` + `source=odds_api` (day-map
      distinguishes the two via `batch_api_football in     modes`). ROW-PRESERVING â captured **346,498 â 346,498** (0
      ...
      `pre_sports_bookmaker_restamp_20260619_130152`. â market-tick-data-service
...
- [x] â [INFRA] P1. **Operator-authorized DELETE of the fully-twinned sports legacy objects (BOTH buckets)** â DONE
      ...
      deleted earlier; sports completes the sports-bucket legacy cleanup. â e2e-testing
- [x] â [DATA] P2. **SFI + Transfermarkt keys LIVE-TESTED (operator "check if they work")** â DONE 2026-06-19. Both
      secrets hold the SAME valid RapidAPI key (`22380b4aâ¦`); both APIs return HTTP 403
      `{"message":"You are not subscribed to this API."}`. **Root cause = RapidAPI SUBSCRIPTION GAP, not a bad/expired
      key** (control: api-football `c820a404â¦` + footystats `b1d5bc90â¦` are distinct keys with working subscriptions).
      ...
      (adapter auto-detects).** Stays BLOCKED-CREDENTIALS (subscription, not rotation). â ping slot_1.md UPDATE. â
      instruments-service [BLOCKED-CREDENTIALS]

### Progress Log â tradfi IS-defs VM fan-out (2026-06-19, operator "use more servers")
...
- B1 catalogue) was replaced with a 9-VM sharded fleet for ~9x wall-clock speedup. Stopped the local serial runners
  ...
  `deployment-service/scripts/vm/launch-tradfi-is-defs-sharded.sh` (new, shellcheck-clean, lifecycle:campaign) â 9 GCE
  VMs `instr-backfill-tradfi-{cboe-a/b/c,nasdaq-a/b,nyse-a/b,cme-a/b}-20260619-141559` (asia-northeast1-c,
  e2-standard-4, run-ts 20260619-141559), each a disjoint (venue, date-window) shard over 2010-06-19â2026-06-19,
  `VM_VENUE` scoped to the 3 paid datasets (CME/NASDAQ/NYSE/CBOE; ICE/FX excluded â off the Databento billing
  ...
  (12471 rows): schema_v9=13.8%, sourceâ0%, asset_group ABSENT. Post-fleet sequence (pending VM self-shutdown):
  consolidator Cloud Run Job `uts-prod-manifest-consolidator-instruments-tradfi` â
  `populate_is_index_v9_2026_06_19.py --asset-group tradfi --apply` (row-preserving, aborts if captured drops) â
  `build_instrument_catalogue.py --asset-group tradfi` â delete VMs.

### Progress Log â close-out drive + LIVE certification (2026-06-19, autonomous)

**VM diagnosis (4 running at 19:30Z; freshness = per-VM SHARD update, NOT the lagging GCS log-tee):**

- `instr-backfill-tradfi-cme-b` â **WORKING**, climbing (date=2021-07-14 of its 2020-01-01â2026-06-19 window). The 8
  sibling tradfi IS-def shards (cboe-a/b/c, nasdaq-a/b, nyse-a/b, cme-a) **already self-deleted**
  (`VM_SHUTDOWN_ON_COMPLETION`) â only CME-b remains. Genuine multi-year CME GLBX.MDP3 daily-definitions backfill â many
  hours ETA.
- `af-backfill` (sports MTDS api-football coverage) â **WORKING**, log fresh 19:33Z (multi-season league sweep; many
  `Fetched 0 teams` = off-season/no-data, normal honest absence).
- `mtds-gas-fees` (defi gas_fees 2021â2026 multi-chain RPC) â **WORKING** (initially misread as stalled: GCS log-tee
  ...
  previous membership set") â writes continue; root cause is that bucket has **no consolidator Cloud Run job** (only a
  2026-05-20 `_index`), which does NOT block the backfill. Load ~0.05 = RPC-bound, not hung. Long backfill.
- `sfi-backfill-chunk-2of4` â **DELETED** (no-op). sshd-dead (port 22 backend fail), log frozen 3h21m, wrote ZERO data
...
could-exist grid â capturedâtotal is correct there).

**NOT-100% honest reasons (no false 100% claims):**

- **tradfi IS 37.6% v9 / 0% ag = the ONE open cell** â awaits CME-b finish â
  `populate_is_index_v9 --asset-group tradfi --apply` â `build_instrument_catalogue --asset-group tradfi`. IN PROGRESS.
- **Low honest-cov% on defi/tradfi/cefi MTDS (13.7/11.1/50.5) = expected_unattempted dominating, BY DESIGN** â the huge
...
### Delete-ready manifest (2026-06-19, OPERATOR-FACING â no agent delete performed this session)
...
| defi MTDS                | 352,234   | 346,902               | **5,332**     | 5,332 MIGRATE-FIRST = no canonical twin yet â NOT delete-safe (migrate first)                                     |
| tradfi MTDS              | 1,706,332 | 1,705,230             | **1,102**     | 1,102 MIGRATE-FIRST not delete-safe                                                                               |
| sports MTDS              | 252,318   | 248,502               | 3,816         | **ALREADY EXECUTED 2026-06-19** (3,816 content-twin verified safe at delete time) â list is pre-delete/historical |
| pred MTDS                | 573,451   | 573,451               | 0             | all twin-verified safe (canonical = `-pred-prd`)                                                                  |
| sports IS                | 9,723     | (UNMAPPABLEâmigrated) | â             | **ALREADY EXECUTED 2026-06-19** (odds-api twins migrated then legacy deleted)                                     |
| cefi/defi/tradfi/pred IS | 0         | â                     | â             | no legacy IS dups listed                                                                                          |
...
**prediction legacy-flat BUCKETS** `instruments-store-prediction-â¦` (stale 2026-06-08) +
`market-data-tick-prediction-â¦` are SUPERSEDED by canonical `-pred-prd` (which is live + 100%/97.8% certified) â
...
**NOT delete-safe (MIGRATE-FIRST first):** defi MTDS 5,332 + tradfi MTDS 1,102 objects have no canonical twin â must be
...
1. **tradfi IS v9 = the ONE genuinely-open cell** (37.6% v9, 0% asset_group) â gated on `instr-backfill-tradfi-cme-b`
   (CME GLBX.MDP3 daily-defs 2020â2026, ~108 days/h, **ETA ~17h from 19:48Z**). On its TERMINATION the close-out runs:
   consolidator â `populate_is_index_v9 --asset-group tradfi --apply` â
   `build_instrument_catalogue --asset-group tradfi` â verify 100% v9. Tracked waiter armed (`/tmp/wait_cme_b.sh`). NOT
   a code/decision blocker â pure backfill wall-clock.
...
4. **defi/tradfi MTDS low honest-cov (13.7/11.1%) = expected_unattempted BY DESIGN** â huge could-exist universe (every
   IS instrument Ã every post-genesis day) is honest absence (the 4th state working), not pipeline failure. captured is
   real.
5. **prediction MTDS 96.5% v9 / 93.9% src** â near-complete; 50 failed + 338 expU residual. Not a blocker.
6. The **legacy-flat `_index` reads (prediction 0% v9, etc.) were a measurement artifact** â the CANONICAL
...
## Close-out continuation (2026-06-19 ~20:20Z) â Progress Log

- **MTDS fallback-import ratchet 3â2 SHIPPED** (operator ask): `no_fallback_imports_baseline.yaml` lowered;
  `check_no_fallback_imports.py` confirms `market-tick-data-service: 2 (== baseline)` PASS; MTDS tree has no uncommitted
  `.py` (count durable on committed tree). **PM@953bc18fc** on LDR â standing PR #432 â main. Locks the import-pattern
  improvement against regression.
- **batch+LIVE smoke matrix DONE** (af55592b): `e2e-testing@c92d50f` harness, 3401 cells Ã 5 AGs â **754 batch-pass / 0
...
- **SFI CONFLICT DEFINITIVELY RESOLVED** â the _new_ `soccer-football-info-api-key` works: sfi-backfill-chunk-3of4 log
...
- **gas-fees re-launch VERIFIED CLIMBING** on the fixed log-streamer (BSC gas blocks, 2021 dates, 200 pts/chain/day) â
...
- **CME-b (tradfi IS v9, the ONE open cell)**: `instr-backfill-tradfi-cme-b-20260619-141559` RUNNING + writing CME
  ...
  poll of VM state + hourly climbing-metric breadcrumb + 2h-flat stall-trip + 20h cap). On terminal â re-invokes main
  loop to run: consolidator â `populate_is_index_v9 --asset-group tradfi --apply` â
  `build_instrument_catalogue --asset-group tradfi` â verify tradfi IS 100% v9. (Replaces the sub-agent-owned waiter
  that died when its parent came to rest â per CLAUDE.md "main loop owns the waiter".)
...
## gas-fees + sfi backfill diagnosis (2026-06-19 ~20:50Z) â Progress Log

The operator-flagged "frozen" gas-fees + sfi VMs were re-investigated to a definitive root cause (NOT the prior
"climbing on fixed streamer" reading â that was the gsutil-tee daemon firing every 60s while the _work process_ was
silent; the run.log object mtime stayed frozen at 19:58 â 4min post-launch â for 46+min on BOTH).

- **gas-fees â ROOT-CAUSED + FIXED.** The collector writes per-VM shards to the dedicated
  `gas-fees-central-element-323112` reference bucket and read-preflights it via `assert_consolidator_healthy()`, but
  **no consolidator job covered that bucket** (35 jobs exist; gas-fees absent) â the index was always >120s stale â
  `ManifestConsolidatorStaleError` raised (the earlier run 151404 shows the identical traceback after reaching
  2021-01-26; the close-out agent's "non-fatal warning" claim was WRONG â it is fatal). **Fix:
  ...
  provisions both the Cloud Run job + the `*/1` cron; ~13 \_index shards â default 4vCPU/16Gi/300s).
- **ð© OPERATOR FLAG â foreign TF blocks ALL gcp IaC apply.** `tofu plan/apply` in `deployment-service/terraform/gcp/`
  currently errors `Duplicate local value definition: blrs_image` â defined in BOTH `audit03_cron_provisioning.tf:17`
...
- **sfi-chunk-3of4 â hung, different cause.** Log frozen 46min at the same 19:58 (was actively skipping off-season dates
  at 19:57:33, then stopped mid-processing â a hang, not a startup crash). It writes to the consolidated sports bucket
  (consolidator IS covered) so it is NOT the gas-fees failure mode; root cause unknown (likely a hung SFI API request or
  a manifest-write stall). The SFI _key itself works_ (200, 50 leagues, honest-absence empties for off-season) â this is
  a runtime hang, not blocked-credentials.
- Both hung VMs DELETED (STOPPING) to stop compute waste.

### Follow-up todos (tracked)

- [x] â [INFRA] P1. deployment-service â gas-fees consolidator job+cron APPLIED + VERIFIED. Foreign
      ...
      sampling, 2021-01-01/02, **no ManifestConsolidatorStaleError**) â deployment-service@f0f7ded.
- [ ] [SCRIPT] P2. market-tick-data-service / deployment-service â diagnose the sfi backfill mid-processing hang (log
...
- [ ] [SCRIPT] P2. **DEFERRED** â the silent-worker watchdog (already a pending residual) is the systemic fix for the
...
## gas-fees FIX VERIFIED + sfi relaunch (2026-06-19 ~21:18Z) â Progress Log

- **Foreign TF blocker RESOLVED by its owner** â `paper_week_determinism_scheduler.tf`'s duplicate `blrs_image` local
...
- **gas-fees consolidator cron APPLIED + FIX VERIFIED.** Targeted `tofu apply` (2 add / 0 change / 0 destroy) created
  ...
  preflight that crashed the prior run** â log shows ETHEREUM gas sampling + BSC block resolution for 2021-01-01/02 with
  **no `ManifestConsolidatorStaleError` and no traceback**. Root cause (missing consolidator coverage) is genuinely
  closed.
- **sfi â HTTP-layer hang ruled OUT; relaunched to reproduce-or-clear.** The SFI adapter base
  (`instruments-service/.../adapters/sports/adapters/base.py`) ALREADY sets a bounded `aiohttp.ClientTimeout`
  (`_HTTP_TOTAL_TIMEOUT` + sock bounds) and retries `asyncio.TimeoutError` â so a stalled SFI request CANNOT hang the
  ...
  reproducer to root-cause** (NOT HTTP). Honest status: sfi root cause is NOT yet pinned to a code defect â relaunch is
  the reproduce-or-clear step, not a claimed fix.

## Backfill "freeze" ROOT CAUSE + fix shipped; rate-limit-vs-internal verdict (2026-06-19 ~21:45Z) â Progress Log

**Definitive root cause (local faulthandler repro, per operator "run local, VM is slow"):** the "frozen backfill log" is
NOT a hang â it is slow work + sparse logging. `gas_fee_client.get_historical_fees` sampled ~288 blocks STRICTLY
...
**Operator's question â rate-limited vs internally self-slowed â answered with evidence:**

- **defi/gas = INTERNAL self-throttle (sequential), NOT rate-limited.** Parallel run hit 16 concurrent with ZERO 429s
  and scaled ~14Ã (86sâ6s). FIXED in code.
- **sfi = GENUINELY rate-limit bound (external).** VM log shows repeated
  `Rate limited (429) ... sleeping 60s to next minute` EVEN at the adapter's 0.34s self-pace â ~one 60s sleep/minute, ~a
  handful of matches/min effective. Parallelizing would worsen it; fix is a higher RapidAPI tier.

**Fix shipped:** `gas_fee_client.get_historical_fees` parallelized â `ThreadPoolExecutor(max_workers=16)` (I/O-bound
fleet default), first-block probe preserves the `use_fallback` mode, logs every 50, output sorted by block.
**market-tick-data-service@7421693** on LDR (QG-green, sentinel 6b9af8f; ruff+basedpyright clean; local re-run 86sâ6s
...
cassette loader) absent from the ignore-block; added it (non-exploitable â own fixtures, vcrpy pinned by aiohttp-3.14
deadlock). **unified-trading-pm@78a4615d2**.

### Follow-up todos (tracked)

- [ ] [SCRIPT] P2. instruments-service / market-tick-data-service â apply the same parallelization pattern to the
...
- [x] â [CREDENTIALS] P1. SUPERSEDED â NOT a tier ask. Operator 2026-06-19: SFI RapidAPI is **4 req/s (max tier 6
      req/s) + 100k req/day**, so a tier upgrade is negligible (4â6). The 429s were SELF-INFLICTED: we ran **4
      chunk-parallel VMs sharing ONE RapidAPI key** (each self-pacing 0.34sâ2.94/s â 4Ã2.94â11.8/s vs the 4/s ACCOUNT
      limit) â constant 429 collisions â 60s back-off sleeps â aggregate throughput WORSE than one clean stream. **Fix =
...
- [x] â [INFRA] P3. deployment-service / unified-trading-pm â cosmetic `qg-common.sh:159` bug: `stat` output leaks into
      an arithmetic `(( ))` expression in the pip-audit deps-hash cache check â "syntax error in expression" + a
      redundant full pip-audit run (non-fatal). Fix the cache-hash comparison. Target repo: unified-trading-pm
      (qg-common.sh SSOT). â unified-trading-pm `qg-common.sh:162` (GNU-first guarded
      `stat -c %Y 2>/dev/null || stat -f %m` shipped).

## sfi EFFICIENCY â corrected root cause (2026-06-19 ~22:18Z) â Progress Log

Operator clarified the SFI RapidAPI limits: **4 req/s (max 6), 100k/day**. This INVALIDATES the "needs a higher tier"
framing â 4â6 rps is negligible and the per-day 100k is the true ceiling. The real bug: **the chunk-parallel backfill
ran 4 VMs against ONE shared RapidAPI key**, so 4 Ã the per-instance 2.94/s â 11.8/s vs a 4/s ACCOUNT limit â 429 storms
â 60s back-offs â effective throughput far BELOW a single clean stream. (Verified: all 4 chunks of run 211603 were
...
under the 4/s cap â no collisions). The chunk-parallel approach is fundamentally wrong for a per-account-rate-limited
vendor.

### Follow-up todos (corrected)

- [ ] [SCRIPT] P2. deployment-service â `launch-sfi-backfill-vm.sh` must DEFAULT SFI to a single stream (or refuse
      `--chunks N>1`) because the RapidAPI key's 4/s limit is PER-ACCOUNT, not per-VM â N chunks just multiply 429
      collisions. The `sfi_chunk_parallel_backfill_2026_04_22` plan's premise (independent per-chunk rate budgets) is
      invalid for a shared key; supersede it. Optionally tighten the per-instance pace 0.34sâ0.25s to use the full 4/s
...
## Autonomous batch (2026-06-20 ~00:10Z) â gross-now + Kalshi + residuals
...
deployed Cloud Run job â **source NOT in the workspace**, the foreign paper-determinism work) emits only a single
`gross_usd` that flips planned-ceilingâlive with no gross*\*\_now split. **Fix (UI-derive):** added "Gross exposure
...
(regression: tests/smoke/paper-trading.smoke.spec.ts). â SHIPPED unified-trading-system-ui@f4afdd83 (UI QG green:
tsc+ESLint+285 tests+build). **Kalshi:** the adapter was already built and uses **PUBLIC** read endpoints
(markets/trades â no auth/RSA-PSS; signing is trading-only), and the MTDS factory routes `kalshi â KalshiAdapter`. The
only gap was the prediction launcher hardcoding POLYMARKET. **Fix:** `launch-mtds-prediction-backfill-vm.sh` now takes
`--venue POLYMARKET|KALSHI` (deployment-service@0a7c3f8). **Launched** `mtds-prediction-kalshi-20260620-000833` â but it
hit a DEEPER gap ("No active venues", see below): KALSHI was hardcoded-disabled in `get_venues_for_asset_groups`. â
...
- [ ] [SCRIPT] P2. **paper_engine.py** (foreign paper-determinism Cloud Run job; source not yet on LDR) â emit
      ...
      15M/6x â 5.6M/2.2x between runs). UI currently derives gross-now from positions as the interim. Target: whoever
      owns paper_engine.py (batch-live-reconciliation / citadel paper-determinism).
- [ ] [SCRIPT] P3. deployment-service â `launch-mtds-prediction-backfill-vm.sh` singleton lock matches
      `^mtds-prediction-` so a KALSHI run is blocked by a concurrent POLYMARKET run (different APIs, no shared rate
      limit) â make the lock per-venue. `--force` is the current bypass.

### Residuals status (operator-gated / foreign â NOT agent-fixable)

- **cefi MTDS (801K failed)** â billing-blocked; enabling billing is operator-only. No code fix.
- **Extended Finance** â NOT a blocker for the data pipeline (corrected 2026-06-22): public `/info/*` market data
...
- **MTDS STEP 5.88b** â the smoke-matrix agent's `quality-gates.sh` wiring is foreign uncommitted WIP, blocked on the
  foreign dirty UTL tree (`honest_coverage_ratchet`/`run_writer`) being committed by its owner. Not mine to ship.

## Kalshi â deeper root cause found + fixed (2026-06-20 ~00:35Z)

The first Kalshi launch (mtds-prediction-kalshi-000833) COMPLETED exit-0 but logged "No active venues for date=X
asset_groups=['PREDICTION']" for all 91 dates â zero data. Root cause (deeper than the launcher):
`get_venues_for_asset_groups` in `market_tick_data_service/engine/orchestrator/__init__.py` hardcoded
PREDICTIONâ[POLYMARKET] with a stale "KALSHI disabled â requires API key + US jurisdiction" note, so `--venues KALSHI`
...
list â market-tick-data-service@ebf947b** (MTDS QG green). Combined with the launcher --venue param (0a7c3f8), Kalshi is
...
NOT fresh LDR git â so the get_venues fix (and the earlier gas parallelization mtds@7421693) reach a VM only after a
...
- [ ] [INFRA] P2. deployment-service â once the foreign dirty trees clear (MTDS scripts/quality-gates.sh + UTL
...
## Kalshi Q&A canonical parser â SHIPPED (2026-06-20, operator-requested)
...
- **unified-api-contracts@c3bf51d**: `KALSHI_TICKER_PREFIX_TO_GROUP` (72 rule entries, full KX*
  crypto/equity-index/commodity/FX/macro families); `classify_kalshi_to_canonical_group` upgraded override-only â 3-tier
  (exact override â longest-prefix â OTHER); +25 tests (63 total) incl. the **cross-venue arb invariant** (Kalshi KX*
...
- **instruments-service@b313b0e**: classifier docstring + 3 prediction tests updated. Shipped via the carve-out
  ...
  blocked the sentinel â that failure is from the SEPARATE Kalshi/Polymarket PERPS venue addition (KALSHI-PERP CeFi
  launch date), tracked in `prediction_venue_perps_and_live_clob_depth_2026_06_20.md`, owned by the perps build.

Now Kalshi prediction-Q&A markets bucket to canonical groups (was OTHER) AND share canonicals with Polymarket â
...
- [ ] [TEST] P3. unified-api-contracts â UEI-lifecycle contract-call ratchet baseline (27) for
      ...
      files (file now ~21, was 27) â NOT deleted, NOT a regression. Both UAC + IS QG pass overall (warn-tier cross-repo
...
> instruments-store canonical-form single-walk (CF-1â¦CF-12) + the IS-side audit-finding code remediation. Full detail
> lives in the archived sources under `archive/2026_06/` and `archive/issues/`.

### From `instruments_manifest_canonicalisation_2026_06_01` (archived â G1 canonicalisation ROOT)

- [ ] [DATA] P0. **C0 â ONE bundled single-walk per non-sports instruments bucket**: `category=`â`asset_group=` (CF-2) +
...
- [ ] [DATA] P1. **C-source RIDER (CF-4)** â re-consolidate the `source` column into the instruments `_index`
...
- [ ] [CODE] P1. **C-reasons (CF-5)** â instruments writers emit typed `EmptyConfirmedReason` (non-sports AGs);
      fetch-failure â `attempted_failed` not `empty_confirmed` (CF-11 swallow sweep). (MIGRATED FROM: same.)
- [ ] [DATA] P0. **E3** â confirm instruments writer drained; snapshot each `_index`. (MIGRATED FROM: same.)
- [ ] [DATA] P0. **E4** â dry-VM â timing â optimise â run (small: 30k/20k/493 rows; no fire-and-forget). (MIGRATED
      FROM: same.)
- [ ] [DATA] P0. **E5** â manifest rebuild per bucket: `ManifestWriter` stamps `source` + `pipeline_mode` +
      `available_at` + typed reasons â consolidator â v9; writer-fix CF-5/CF-11 so future writes are honest. (MIGRATED
      FROM: same.)
- [ ] [DATA] P0. **E6 + post-walk CF audit** â `cf_manifest_audit_2026_06_01.py` per instruments-store bucket â
      CF-1â¦CF-12 GREEN, 0 legacy-only cells vs canonical; flip CF-coverage in
      `instruments_master_audit_instructions.md`. â ï¸ IRREVERSIBLE: only after GREEN, hand C-GREEN to L6
      (`bucket_name_ssotâ¦`) â delete the legacy instruments-store buckets permanently. (MIGRATED FROM: same.)

### From `issues/instruments_service_audit_findings_2026_06_08` (archived â IS downloadâmanifest audit)

- [ ] [UTL] P2. **Confirm UTL `record_captured_from_counts` auto-stamps `default_source` for single-source cells** â
...
- [ ] [MTDS] P2. **`engine/orchestrator.py:4271` `_af_record_empty(reason="")`** â make `reason` a required typed
      `EmptyConfirmedReason` (latent `LegacyBlankErrorReasonError`). Repo: instruments-service. (MIGRATED FROM: same.)
- [ ] [MTDS] P2. **Narrow the broad excepts at `orchestrator.py:3794, 7821`** â `:3794` swallows all on a
...
- [ ] [MTDS] P2. **Residual bar-edge fallback-to-open** â `cefi/hyperliquid.py:257`, `cefi/ccxt_adapter.py:310-312`,
...
- [x] â [MTDS] P2. **De-duplicate the IS venue universe** â make the cefi/tradfi/prediction fetch path read UAC
      `VENUES_BY_ASSET_GROUP` instead of hardcoded mirrors. **SHIPPED by `instrument_universe_registry_consolidation`
      Phase 1 â `instruments-service@4da6fe8`** (verified live 2026-06-30): `_CEFI_VENUES`/`_TRADFI_VENUES` DELETED from
      ...
      KEPT** â defi is operator-decided EXEMPT from set-equality (registry-consolidation Decision D / A6), with a UAC
...
- [ ] [MTDS] P2. **IBKR systemic-failure hardening (LATENT)** â `tradfi/ibkr.py:337-348` per-symbol isolation is
      correct; harden the systemic case (`_ib is None`/all-fail â `[]` no raise) when/if IBKR becomes a live reference
      venue (not in `_TRADFI_VENUES` today). Repo: instruments-service. (MIGRATED FROM: same.)
- [ ] [INFRA] P2. **Prediction catalogue bucket mismatch** â
      `deployment-service/terraform/gcp/lifecycle_catalogue_scheduler.tf:40-44` targets
      `instruments-store-prediction-â¦` vs SSOT `instruments-store-PRED-â¦`; reconcile to the SSOT bucket. Repo:
      deployment-service. (MIGRATED FROM: same.)
- [ ] [CLAUDE-MD] P2. **Correct the over-broad "instruments-service owns all venue URLs via `InstrumentRecord`" line** â
...
- [ ] [AUDIT] P2. **Fix `instruments_master_audit_instructions.md` item (g)** â "`rg URDI` â 0 hits" is wrong;
...
- [ ] [MTDS] P3. **Split the `instruments-service` `engine/orchestrator.py` (8,192 lines, 9Ã the 900 cap)** into focused
      ...
      `engine/orchestrator.py` (4,219L) split tracked in M-2 â same filename, different repo; do not conflate.**
      (MIGRATED FROM: same.)
- [ ] [SCRIPT] P3. **Script-tier cloud-agnostic sweep** â ~60 scripts `from google.cloud import storage`/`boto3` â
      `get_storage_client()`; ~30 inline legacy bucket literals â `resolve_bucket_name`;
      `enumerate_expected_universe.py:1381` hardcoded `/tmp/` â `tempfile.gettempdir()`. Repo: instruments-service.
      (MIGRATED FROM: same.)
- [ ] [PLAN] P3. **Delete the orphaned static-snapshot catalogue path** (`reference_data/catalogue/catalogue_builder.py`
      `CatalogueBuilder` + `orchestrator.py refresh_catalogue`) â superseded by `build_instrument_catalogue.py`, no
