...
- [ ] [CREDENTIALS] P1. **BLOCKED-CREDENTIALS** — SoccerFootball.info RapidAPI plan tier is the binding throughput constraint for the sports historical backfill (429s at the current tier even with 0.34s pacing → ~handful matches/min; multi-year backfill is throttle-bound). Operator ask: upgrade the SFI RapidAPI subscription to a higher req/min tier (vendor=RapidAPI/soccer-football-info; need tier+price; unblocks full-speed sports historical backfill). The adapter + key already work (200s, honest-absence empties) — purely a throughput/tier ask, NOT a code defer.
- [ ] [INFRA] P3. deployment-service / unified-trading-pm — cosmetic `qg-common.sh:159` bug: `stat` output leaks into an arithmetic `(( ))` expression in the pip-audit deps-hash cache check → "syntax error in expression" + a redundant full pip-audit run (non-fatal). Fix the cache-hash comparison. Target repo: unified-trading-pm (qg-common.sh SSOT).
