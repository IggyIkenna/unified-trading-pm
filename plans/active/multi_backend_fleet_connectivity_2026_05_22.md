...
- [x] ✅ DEFERRED-BLOCKED [BLOCKED-OPERATOR] P3. **GCS-read for the JWT secret.** Central VM authenticates to GCP as an `authorized_user` (a
...
- [x] ✅ DEFERRED-BLOCKED [BLOCKED-COUPLED] P3. Wire `reload_secret()` to a poller — **no-op until GCS-read lands**: with the env-var source
      `os.environ` is fixed at process start, so a poll re-reads the same value. Ships together with the GCS-read item.
- [x] ✅ DEFERRED-NEEDS-GO-AHEAD [NEEDS-GO-AHEAD] P3. **RS256/ES256.** Not IAM-blocked, but a deliberate fleet-wide auth migration (generate
