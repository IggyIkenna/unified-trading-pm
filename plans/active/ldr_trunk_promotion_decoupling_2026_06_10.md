...
- **D1 — QG triggers need NO change.** `quality-gates-v2.yml.tmpl` already fires on `pull_request:[staging]` (the
  LDR→staging PR = "CI on LDR", with the `base_ref=staging` dep-resolution fallback) and `push:[staging]` (status writer
  / required-check parity). We do **NOT** add a `push: live-defi-rollout` trigger — a raw LDR-push QG has _no_
  `base_ref=staging` fallback, so it would re-introduce exactly the BLR failure (a dep that's on staging-but-not-LDR).
  We do **NOT** trim `push:[staging]` — a staging head must carry check runs (the `[skip ci]`-deadlock rule + hardening
  todo @4950). **"Both run?" → the LDR→staging PR is the gate; the staging-push run is the status confirmer; do not add
  a third on LDR.**
...
| @4950 flag staging head w/ ZERO check runs (P2)                   | **Constrains D1**                  | This is _why_ we keep `push:[staging]` QG. Recorded in D1.                                                                                                                                                                |
...
- [x] [SCRIPT] P0. **Drain cadence 6h → 30min.** `ldr-to-staging-promote.yml:26` cron `"17 */6 * * *"` →
      ...
      fire only from the default branch). — unified-trading-pm@<sha>
- [ ] [SCRIPT] P0. **quickmerge: land-on-LDR, skip per-unit staging PR for service repos.** After the LDR push, for a
      ...
      `scripts/quickmerge.sh` (PR_BASE block ~1337-1421). FAIL-SAFE (worst case: promotion waits ≤30min).
- [ ] [SCRIPT] P0. **quickmerge: decouple LDR commits from staging state.** STAGE 1.5 (staging-lock, guard @662) and
      STAGE 1.7 (dep-tier, guard @908): change guard `TO_STAGING=true` → `HOTFIX=true` so only a hotfix-to-staging
      respects the lock / dep-tier gate; normal LDR landings skip them (the drain re-gates dep-order). FAIL-SAFE.
- [ ] [SCRIPT] P1. **quickmerge: STAGE 1.6 dep-version gate → WARN for non-hotfix, BLOCK for hotfix.** Building against
      a dep that is version-behind-staging is legitimate on the LDR trunk; the drain + SIT catch real incompatibilities.
      Keep the hard BLOCK on the `--hotfix` path (a hotfix must reconcile with staging).
