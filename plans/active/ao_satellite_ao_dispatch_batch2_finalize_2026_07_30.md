...
- [x] ✅ [REVIEW] P0. **Re-verify every batch-2 done-claim against reality, not against its checkbox** — for each of the
      8 todos in `/plans/archive/2026_07/ao_satellite_ao_dispatch_batch2_2026_07_30.md`, re-run `git show --stat <sha>`
      for every cited commit and re-run the specific named test(s) directly rather than trusting the claim, and re-run
      each todo's own stated done-when check where it is a command (the na-eligibility-timer fire-completion read, the
      orch_token reporter-staleness read, the JWT-secret token-survives-restart + healthz check, the 4 orphan-commit
      dispositions, the wip-preserve ref disposition). **NOTE (plan_reconciler agt-c7578b, 2026-08-10)**: the
      na-eligibility-timer item and the wip-preserve-ref item were EXTRACTED 2026-08-09 to
      `ao_satellite_ao_dispatch_batch10_2026_08_09.md` (todos 1/2) and never completed inside batch2 itself — do NOT
      expect a done-when check to succeed against batch2 for those 2 specifically; check batch10's own evidence for them
      instead. **Done when**: all 8 verified, and any claim whose evidence does not hold up is re-opened as a new
...
- [x] ✅ [REVIEW] P0. **Archive every source doc that has reached zero open todos, and repoint any referrer.** —
      ...
      `ao_done_gate_no_carveout_for_red_gate_evidence_only_closure_2026_07_28` → `plans/archive/2026_07/issues/`,
      `branch_reset_to_origin_orphans_unpushed_worker_commits_2026_07_27` → `plans/archive/issues/`,
      `mtds_backfill_sequential_true_dispatch_order_violated_2026_07_29` → `plans/archive/issues/`,
      `na_eligibility_auditor_timer_not_yet_installed_2026_07_27` → `plans/archive/issues/`,
      `orchestrator_jwt_secret_not_pinned_causes_fleet_git_status_outage_2026_07_24` → `plans/archive/issues/`,
      `dispatch_sequential_gate_fix_2026_07_24` → `plans/archive/issues/`,
      `wip_preserve_refs_silently_unrecovered_2026_07_29` → `plans/archive/2026_08/`. **1 doc CANNOT be archived:**
      `git_status_reporter_stale_public_url_token_expiry_2026_07_24.md` — 1 open todo (`[INFRA] P3` ghost host rows
      `ip-172-31-0-185`, a genuine prune-vs-tombstone design call). **Referrer fix:** repointed 3 stale
      `plans/active/issues/wip_preserve...` → `plans/archive/2026_08/wip_preserve...` references in
      `orphaned_commit_recovery_has_no_dispatch_path_2026_07_30.md` (related, context_scope, body prose). One remaining
      grep match for `ao_done_gate_no_carveout` in `plan_reconciler_findings_ao_2026_08_10.md` is historical Progress
      Log documentation of a prior repointing (lines 196, 333-334), not a live referrer — modifying it would falsify the
...
- **2026-08-10 (slot 23, review)**: todo 4 done — verified all 8 batch-2 source docs: 7 already archived by prior
