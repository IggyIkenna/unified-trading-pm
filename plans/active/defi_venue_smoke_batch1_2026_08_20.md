...
- [x] ✅ [BACKEND] P1. Convert every failed or absent DeFi row into a tracked follow-up with venue, data type, source, and owner rather than treating absence as success; Gate: every non-passing row has a linked plan todo or an explicit declared-absence reason. Audit result: every non-passing row across this plan's three evidence layers already resolves to either a linked todo or an explicit declared-absence reason — (1) the MDPS-layer 182 failed / 595 skipped shard checks → declared-absence reason = the open blocker [mdps_defi_captured_days_stale_consolidated_index_despite_healthy_consolidator_2026_08_21.md](/plans/active/issues/mdps_defi_captured_days_stale_consolidated_index_despite_healthy_consolidator_2026_08_21.md) (linked from todo 1 above); (2) the Ethereum smoke-dump's 32 `no_evidence` + 1 `FAILED` rows → the two P2 todos below (timeout retries; LIDO crash fix) plus the per-row "architecturally proven only via shared CHAINLINK-ETHEREUM" declared-absence note in the 2026-08-21 results table; (3) the testnet-verdict groups (A-L) → group H already had a P2 todo, group B's unconfirmed venues already had a P3 todo. Two genuine gaps found this pass that were only prose ("named low-confidence items", not tracked todos) — both converted to real todos immediately below, closing the invisible-follow-up violation.
...
- [ ] [BACKEND] P2. Build the missing production caller for `get_execution_provider()` (groups C/D/E fork leg, 92 of 127 testnet-verdict venue-chain rows) — `execution_service/providers/factory.py`'s `get_execution_provider()` has exactly two non-test references repo-wide (its own definition + one unit test), so even though the Tenderly trio of GSM secrets is confirmed provisioned and the fork suite runs green for AAVE_V3/EigenLayer/Uniswap-V3-LP, no production code path in this repo actually invokes it — check whether a sibling batch/paper orchestrator repo already wires it (not confirmed either way this pass) before building a new caller here. Gate: either a real production call site is found/built, or the gap is recorded as an accepted permanent limitation with operator sign-off.
- [ ] [BACKEND] P3. Confirm whether `execution_service/governance/proposal_simulator.py`'s V3-keyed protocol enum (`AAVE_V3_PROTOCOL`/`COMPOUND_V3_PROTOCOL`/`SPARK_PROTOCOL`/`LIDO_PROTOCOL`) actually covers testnet-verdict group F's legacy bare-label rows (`AAVE-ETHEREUM`, `COMPOUND-ETHEREUM`, `governance_events`) — label-alignment was never checked, and UNISWAP-ETHEREUM's governance_events row has no simulator coverage under any label. Gate: either the legacy labels are confirmed to resolve through the V3-keyed enum (record how), or the mismatch is fixed, or the gap is recorded as an explicit declared-absence reason.
...
**2026-08-22 (slot 13, backend_engineer) — non-passing-row follow-up audit.** Verified the P1 "convert every failed
or absent row into a tracked follow-up" gate against all three evidence layers this plan carries. Found the gate
was already substantively met (MDPS-layer 182 failed/595 skipped shard checks trace to the linked
`mdps_defi_captured_days_stale_consolidated_index...` blocker; the Ethereum smoke-dump's 32 no_evidence + 1 FAILED
rows trace to existing P2 todos or the per-row CHAINLINK-shared declared-absence note; testnet-verdict group H
already had a P2 todo, group B's unconfirmed venues already had a P3 todo) — but the testnet-verdict table's own
"Named low-confidence items" paragraph carried two genuine remaining gaps only as prose, never as tracked todos:
the missing production caller for `get_execution_provider()` (92/127 Tenderly-gated rows) and the unconfirmed
group-F proposal-simulator legacy-label alignment. Both converted to real `- [ ]` todos above. No code changes
this pass — pure plan-hygiene closing an invisible-follow-up gap.
