...
- **na-eligibility-audit 2026-08-17 (cross-cutting tranche, dispatch agt-3931fd)** [body-hash:eef83641d4201ce4]: KEEP-NA, valid — 2 open todos, both genuine judgment/design items, not stale. Item 1 (VenueFeature/VenueCapability vocabulary overlap) is explicitly deferred by the doc's own done-when ('resolve... not before' VenueCapabilityV2's first real instance) — confirmed by the doc's own final Progress Log 'Correction' entry: 'deliberately parked pending a future event.' Item 2 (venue→chain SSOT, found same day, P1) requires a genuine merge-vs-keep-orthogonal design decision ('Decide on the same basis [as todo 1]... Do NOT assume merge') that blocks strategy_service_centralization_fixes_2026_08_16's 'fix the exemplar' todo. Note (finding, doesn't change verdict): the doc's own final Progress Log entry claims 'one todo remains' but 2 are actually open — the venue→chain P1 item isn't mentioned in that tally, a minor doc-narrative undercount.
