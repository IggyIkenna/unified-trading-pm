...
- **Every AO-dispatched plan needs a gated finalize plan (operator ruling 2026-07-24).** Alongside any
  ...
  `scripts/quality_gates/check_finalize_plan_coverage.py`. **Before authoring a companion, run the creation idempotency
  guard** — `python3 scripts/quality_gates/check_finalize_plan_coverage.py --guard-parent <plan-slug>`; exit 1 means the
  parent is ALREADY gated (a peer may have landed a differently-named finalize plan) — do NOT author a second; find and
  reconcile the existing one instead
