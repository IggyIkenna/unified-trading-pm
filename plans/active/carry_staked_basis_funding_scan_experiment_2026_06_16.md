...
summary: >-
  ...
  CarryStakedBasisRankAllocator, batch==live). Trimmed 2026-07-24 (line-cap remediation) to the core harness + journal;
  the cross-venue funding-reversion research and the strategy-service ensemble-productionization work forked to sibling
  plans (see `related:`).
status: active
nature: process
...
> **🟢 Plan split 2026-07-24 (line-cap remediation, operator-approved unlock).** This plan was trimmed to the core
> carry-scan harness + journal (was 1426 lines / over the 1000L hard cap). Two workstreams forked out to sibling plans,
> content moved verbatim, nothing dropped:
>
> - **Cross-venue funding-reversion research** (Pass-B reconciliation, the deployable reversion book, turnover/DD
>   control, capital-flow sim) → `cross_venue_funding_reversion_research_2026_07_24.md`.
> - **Ensemble orchestrator engine + strategy-service productionization** (the 4-strategy ensemble engine, the
>   `CARRY_FUNDING_DISPERSION` archetype fold, the paper-VM launcher) →
>   `carry_strategy_ensemble_productionization_2026_07_24.md`.
>
> `locked_by: live-defi-rollout` cleared on this trimmed plan per the operator's `[unlock-plan]` grant
...
- **na-eligibility-audit 2026-08-17 (cross-cutting tranche, dispatch agt-3931fd)** [body-hash:e0cf72fbd3c1b3f0]: KEEP-NA, valid — Exploratory operator-driven research journal (net-carry model, execution structures, share-class design, live/paper venue expansion) with continuous operator engagement; na-eligibility-audit 2026-08-02 already confirmed KEEP-NA valid on this exact class of content, and a fresh 2026-08-15 operator ruling via AskUserQuestion directly re-touched 3 open items (Drift-into-derivative_ticker rejected, 5-venue registry deferred, legacy-bucket ownership redirected). FLAGGED CONTRADICTION (not resolved, evidence not clean enough to force a verdict change): the same 2026-08-15 date carries two apparently conflicting notes on Drift — the bottom Progress Log's AskUserQuestion ruling says 'Drift canonical venue inclusion into derivative_ticker — rejected, Drift stays out... do not re-add in any capacity without a fresh explicit decision,' while an inline 'plan_reconciler Item E' note near the Drift todos treats the MTDS-ingestion todo and the STRATEGY-consumption todo as live, sequenced, non-duplicate work with no mention of rejection. Minor separate hygiene note: frontmatter still carries the assigned_vm:NA + execution_scope:orchestrator-agent mismatch that sibling 2026-07-24 fork plans already had corrected to local-only — functionally inert (NA alone yields an empty owning-VM set) but worth a follow-up fix. Ruling/citation: na-eligibility-audit 2026-08-02 (KEEP-NA valid, re-confirmed 2026-07-30); operator ruling 2026-08-15 (AskUserQuestion, 3 items ruled: Drift rejected / 5-venue registry deferred / legacy-bucket ownership redirected); operator ruling 2026-08-08 (credentialed-venue asks stay BLOCKED-CREDENTIALS, backfill-10-coins cost-cleared to launch)
...
- [ ] [DATA] P2. Production Drift funding in **MTDS** via the isolated-venv reader pattern (a Drift handler that shells
      ...
      isolation — driftpy stays out of MTDS flat deps. **Repo: market-tick-data-service.** **Dependency note (operator
      ruling, plan_reconciler cross-cutting Item E, 2026-08-15): the STRATEGY "Wire Drift via the creds/RPC path" todo
      below (currently ~line 646) CONSUMES this todo's output** — once this MTDS handler lands and canonizes Drift
      funding into `derivative_ticker`, the STRATEGY todo reads from that same canonical path rather than
      re-implementing a parallel on-chain reader. Kept as two separate open todos (not merged) — this one is the
      data-ingestion half, the STRATEGY one is the strategy-consumption half.
