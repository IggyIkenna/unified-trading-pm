...
- **context-scout 2026-08-15**: re-verified context_scope (6 entries), unchanged — all 6 (harness script, carry-venue
  ...
  still resolve.
- **na-eligibility-audit 2026-08-17** [body-hash:e0cf72fbd3c1b3f0]: KEEP-NA, valid -- Exploratory operator-driven research/analysis journal (harness is analysis-only; production path is strategy-service's CarryStakedBasisRankAllocator). Already re-confirmed KEEP-NA valid by na-eligibility-audit on 2026-07-30 and again 2026-08-02 (body byte-identical), and by an explicit operator ruling via AskUserQuestion on 2026-08-15 covering three of its open items directly. The 26 remaining open todos are a mix of strategy-design judgment (share_class axis, GAS/slippage/capital-ledger fidelity), cost- or credential-gated data asks (Aave re-run, Paradex/Backpack/Edgewink), and items explicitly sequenced behind other in-flight work (v9 migration completion, the MTDS Drift canonical handler, the dedicated bucket-naming SSOT remediation doc) — none stands alone as a clean bounded/deterministic AO task. No new stale item found: the one item that looks supersede-able (Aave backfill, line 203) carries its own 2026-08-10 plan_reconciler note explicitly instructing to leave it open (narrowed scope, tracked by a sibling todo, do not dispatch both independently) — not a checkbox to close.
- **context-scout 2026-08-17**: re-verified context_scope, no change needed (6 entries) -- today's na-eligibility-audit found no new stale item and confirmed the same open-item mix; all 6 entries (harness script, carry-venue codex SSOT, 2 forked sibling plans, perp-funding-semantics issue doc, wallet-hierarchy codex doc) still resolve and remain the accurate minimal set.
