...
- [ ] [DATA] P1. **Fill the HYPERLIQUID recent-tail gap via the HL batch lane.** ~2026-06-24 through now-2 days —
      non-Tardis venue, exempt from N=1 cap — launch the cefi HL batch launcher for the range per
      `issues/cefi_hl_aster_batch_data_gaps_2026_06_22.md`. Repos: deployment-service, market-tick-data-service. **Done
      when**: manifest rows show `captured` across 2026-06-24→now-2, no new `attempted_failed` (before/after counts).
      Source: `issues/cefi_residual_followups_after_honest_done_2026_07_17.md`. **2026-07-27 (slot-6)**: launched +
      verified healthy `cefi-hyperliquid-2026-20260727-123922`; baseline captured=5928/30787; still open (multi-hour).
