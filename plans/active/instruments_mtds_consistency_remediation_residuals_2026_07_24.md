...
- [ ] [SCRIPT] P1. **N1b prerequisite — build the missing CEFI IS lifecycle catalogue** (found 2026-08-09 during N1b's
      Step-4 verification, see note above).
      `instruments-service/scripts/build_instrument_catalogue.py --asset-group cefi --mode full` (cold-start — no prior
      catalogue exists to incrementally extend) may already be running in the background from the 2026-08-09 session;
      check for a completed/dangling run before re-launching (idempotent, monotonic-guard promotion — safe to re-run if
      the prior one didn't land). Output: `gs://instruments-store-cefi-prd-central-element-323112/prd/catalog.parquet`.
      Once it lands: (1) verify `read_instruments_catalog_bounds("cefi", <venue>, <instrument_id>)` returns non-None for
      a spot-checked known instrument, (2) re-run N1b's corrector dry-run and complete N1b per its own updated todo text
      above. This is also the SAME artifact `/codex/02-data/availability-manifest-and-data-status.md`'s
      `enumerate_expected_universe.py` v2/CeFi path implicitly assumes exists (confirm no OTHER consumer silently
      degrades the same way). — instruments-service
