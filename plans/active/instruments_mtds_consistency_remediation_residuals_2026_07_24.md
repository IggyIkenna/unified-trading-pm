...
- [ ] [SCRIPT] P2. **RULED 2026-08-09 (operator): APPROVED — N5r/N6r — DEFI rebuild-for-real-replace to land
      venue-dedup + VAULT-0-row + 496 chain-pollution on LIVE**. Was `[OPERATOR]`, sign-off required before AO dispatch
      (round5-cross-cutting-audit 2026-08-08) — wholesale LIVE index replace-not-merge, not object/prefix-scoped, so it
      does NOT qualify for the reversibility downgrade the tradfi-stragglers sibling item used; cite
      `/codex/02-data/gcs-and-manifest-delete-safety-protocol.md` §3a. The per-object rebuild (mtds@3f5cc6e/cf63cf6)
      normalizes venue + detects 0-row vaults + would clean the 496 `chain`-pollution rows (token-pairs
      ETH-USDC/1INCH-ETH in `chain`, all attempted_failed UNISWAP_V4 swaps_ohlcv), but reaching LIVE needs a WHOLESALE
      replace of the defi `_index` (the consolidator merge leaves stale un-normalized rows; the index-walk can't
      normalize venue without desyncing from object paths). Run the rebuild to produce the full v9 index + write it as
      the live `_index` (replace, not merge). NOT a double-count/data-loss (P2 grouping hygiene). **ALSO picks up the
      31,773 newly-migrated canonical UNISWAP_V4 `pool_id` cells (3.24M rows) the content-aware fan-out wrote
      2026-06-18** — the rebuild walks all canonical objects, so these get their `availability_index` rows on this same
      wholesale-replace run (no separate V4 manifest pass needed). — market-tick-data-service
...
- [x] ✅ [DATA] P3. **N8 — PRED index data_type label drift** (`prediction_canonical_question_group` vs GCS
      ...
      market-tick-data-service
- [ ] [DATA] P1. **RULED 2026-08-09 (operator): APPROVED — N1b — CEFI: reconcile the (now-verified ~1,550, NOT ~698k —
      ...
      **2026-08-09 Step-4 verification (slot 6) — dependency check RAN, found NOT-yet-ready + a stale premise; NOT
                  applied.** Two independent findings from a live re-audit, in order:

                  1. **The ~698k figure (F3-reframed, 2026-06-18) is STALE.** A fresh read of the live prd cefi MTDS
                     `_index/availability_index.parquet` (`market-data-tick-cefi-prd-central-element-323112`, 10,532,576 rows total,
                     807,871 `attempted_failed`) today shows only **1,550** rows carrying `UNCLASSIFIED_ADAPTER_ERROR` — the bulk of
                     `attempted_failed` is now typed venue-fetch errors (`UNCLASSIFIED:Tardis HTTP 403` 337,797;
                     `VENUE_FETCH_FAILED` 218,038; `Tardis HTTP 403` 150,703; several `schema contract violated for cefi/…`
                     classes; etc.) — none of which N1b's scope covers (only the specific blank/unclassified label is in scope).
                     No commit/todo/Progress-Log entry anywhere in this doc explains the ~698k→1,550 drop over the last ~2 months;
                     flagging as a process-hygiene gap (mirrors the "Fresh audit 2026-07-13" pattern earlier in this doc) rather than
                     re-deriving a root cause here — **operator FYI, not a data-safety concern** (the direction of drift is toward
                     fewer unclassified rows, i.e. improvement, not regression).
                  2. **Step 4 (the catalog cross-ref this reconcile needs) is genuinely NOT ready for CEFI** — the corrector script
                     (`instruments-service/scripts/reconcile_correct_legacy_blank_misflips_cefi_2026_05_13.py`, updated + shipped
                     this session — see below) reads `unified_trading_library.instruments_catalog_reader.read_instruments_catalog_bounds`,
                     which needs `gs://instruments-store-cefi-prd-central-element-323112/prd/catalog.parquet`
                     (the `build_instrument_catalogue.py` lifecycle roll-up). **That object does not exist in prod for cefi** — a
                     live dry-run confirmed `WARNING Instruments catalog not found ... catalog cross-ref disabled for
                     asset_group=cefi`, so 0 of the 1,550 candidates could be classified (all fall through to
                     `SOURCE_RETURNED_ZERO`/no-change). This is a DIFFERENT artifact from `enumerate_expected_universe.py`'s v2
                     writegate scheduler (confirmed separately deployed + running for cefi via
                     `deployment-service/terraform/gcp/expected_universe_v2_scheduler.tf`) — that scheduler's "CeFi: FULL (v2)"
                     status does NOT imply this lifecycle catalogue exists; they are two independent Step-4-adjacent artifacts and
                     only one is confirmed live.
                  3. **Fixed + shipped 2 genuine bugs in the corrector script regardless** (instruments-service@097e230b, QG-green,
                     19 unit tests incl. 3 new): (a) `ASSET_GROUP_BUCKETS` hardcoded `market-data-tick-cefi-{pid}` with NO env-tier
                     segment — the real bucket is `market-data-tick-cefi-prd-{pid}` — so every invocation 404'd since authoring
                     2026-05-13 (`last_executed: NEVER`); now resolves lazily via `resolve_bucket_name` (module-level resolution
                     would require env vars just to import, breaking the test suite). (b) candidate mask only matched the retired
                     `LegacyBlankErrorReasonError` label; widened to also match the current live `UNCLASSIFIED_ADAPTER_ERROR` label
                     (F3-reframed renamed it, mtds@aaeada9) so a re-run is idempotent either way.
                  4. **In flight (background, this session):** `build_instrument_catalogue.py --asset-group cefi --mode full`
                     (cold-start; no prior catalogue to incrementally extend) — bounded smoke-test (`--max-blobs 50 --dry-run`,
                     50/44,484 by_date blobs, 581 catalogue rows, 0 errors, ~6s) confirmed the tool works and is fast per-blob;
                     full run over all 44,484 by_date blobs is estimated ~45-90min at the smoke-tested per-blob rate. Once it lands,
                     re-run the corrector dry-run (`--asset-group cefi`, no flags) to get the REAL correction count against the
                     now-1,550-row candidate set, review, then `--apply-flips --confirm` with `MANIFEST_PER_VM_SHARDS=true
                     VM_NAME=<unique>` per the script's own usage docstring. **Todo NOT flipped done — the actual reconcile has
                     not run yet** (Step 4 was genuinely not ready when checked; flipping now would be false-progress, the exact
                     HARD RULE this workspace polices hardest). See the new todo immediately below for the catalogue-build tracking.
...
- [ ] [SCRIPT] P2. **Fix `reconcile_correct_legacy_blank_misflips_cefi_2026_05_13.py`'s `_download_manifest` to avoid a
      full unfiltered manifest load** (found 2026-08-09 during the N1b-prerequisite catalogue verification above — see
      note there for the measured RSS/kill evidence). The script needs only `capture_status`/`error_reason`-matching
      rows (~1,550 of 10,532,576, 0.01%) plus a handful of columns (`date`, `venue`, `data_type`, `instrument_id`,
      whatever `classify_blank_reason_row` reads) but currently does a bare `pd.read_parquet(local_path)` of the whole
      object. Fix via `pyarrow.dataset` column projection + predicate pushdown (or a DuckDB query) so the materialized
      DataFrame is candidate-scoped, not corpus-scoped — mirrors this doc's own single-walk/efficiency discipline.
      Preserve the existing `n_candidates / len(df)` percentage in the log line (get total row count from
      `ParquetFile.metadata.num_rows` without materializing all rows). Verify by re-running the dry-run on this same
      shared host and confirming it completes without a `resource-watchdog.sh` kill
      (`tail -f /var/log/resource-watchdog.log` clean for the run's duration). Once fixed, re-run the dry-run + hand off
      to N1b's own checkbox (operator-approved 2026-08-09) for the apply step. — instruments-service
