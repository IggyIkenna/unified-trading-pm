...
- [ ] [SCRIPT] P3. **DOWNGRADED 2026-08-09 from P2-blocker to P3 (N1b unblocked via a different fix — see above).**
      `reconcile_correct_legacy_blank_misflips_cefi_2026_05_13.py`'s `_download_manifest` does a bare `pd.read_parquet`
      of the whole 10.5M-row manifest for a 0.01%-selective candidate set. Real blocker was elsewhere
      (`unified-trading-library@a35819ee`); post-fix total runtime is 33.5s so this is no longer urgent, but still worth
      a `pyarrow.dataset` column-projection + predicate-pushdown pass on principle (single-walk discipline) — get total
      row count from `ParquetFile.metadata.num_rows` to preserve the existing `n_candidates/len(df)` log line without
      materializing all rows. — instruments-service
