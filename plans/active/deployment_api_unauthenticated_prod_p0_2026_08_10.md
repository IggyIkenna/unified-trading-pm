...
- **2026-08-12 (interactive session, slot 5, hardening — auth-contract CI gate shipped)**. Diagnosed live operator
  ...
  **Root-cause hardening**: the actual gap this P0 exposed was that NOTHING in the deploy pipeline checks the auth
  ...
  watches. Not yet shipped to origin (pending this session's commit) — deployment-api@`<pending>`. Did not attempt the
  Google OAuth console fix (genuinely operator-gated — OAuth client creation isn't exposed via `gcloud`, needs GCP
  Console access); relayed the exact manual steps to the operator instead.
