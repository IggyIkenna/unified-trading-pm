...
health_factor/LTV/RADIANT on-chain sources (features-service), PnL-surfaces collapse (W9/W10/W13), W9 account
balances as single strategy I/O, PnL attribution across every dimension (W13), MEV opportunity-detection
feature producers (BACKRUN/JIT_LIQUIDITY/LIQUIDATION_BUNDLE — explicitly needs a design decision, not a blind
build), `compute_handler` cron wiring + PnL-residual/Slack-alerting machinery (operator gave the full spec
verbatim in an earlier session — re-read the main plan's W9/W10/W13 section for it), wizard UI/schema
exposure for the 2 already-shipped overlay mechanisms, service-config-ownership typed schema remainder.
