...
| Phase 2.D — all fields        | **SHIPS NOW (full scope, 2026-05-12)**: (1) `match_end_time` = SFI progressive-stats freeze (deterministic, retroactive). (2) `announced_at` = API Football fixture object (retroactively available for any historical date — NOT a forward-poll problem). (3) `SFI_DATA_LAG_P95_SECONDS` = UAC constant measured once from (SFI freeze timestamp − scheduled_kickoff − nominal_duration) distribution p95. (4) `report_time` = `match_end_time + timedelta(seconds=SFI_DATA_LAG_P95_SECONDS)` (derived). (5) Historical postponed/cancelled fixtures → `empty_confirmed` with `EXPECTED_FIXTURE_POSTPONED` / `EXPECTED_FIXTURE_CANCELLED` (2 new `EmptyConfirmedReason` values). Live postponement state-machine deferred to live-pipeline plan. |
