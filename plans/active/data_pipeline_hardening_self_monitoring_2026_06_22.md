...
- [ ] [CODE] P0. **Cron-watches-cron (in-band closure).** Each fleet-monitor Cloud Run job (`uts-prod-dp-exit-code-monitor`, `-heartbeat-watcher`, `-meta-watchers`) writes a `vm-census/<name>-last-run.json` sentinel at end-of-sweep; extend `deployment_service.data_pipeline_monitors.meta_watchers.check_cron_fired` `cron_targets` with a FreshnessTarget per cron (budget = 2× cadence). Repo: deployment-service. So a stopped monitor cron fires `DP_WATCHER-002` (cron-did-not-fire) → page.
- [ ] [INFRA] P0. **Out-of-band dead-man's-switch (the top-of-chain watcher).** TARGET DECIDED + WIRED 2026-06-23: separate Slack channel + app **`monitoring-deadman`** (App ID A0BCEL33UGM), independent of the `#data-pipeline-alerts` relay. Creds in Secret Manager `MONITORING_DEADMAN_SLACK_*` (WEBHOOK/SIGNING_SECRET/CLIENT_*/APP_ID/VERIFICATION_TOKEN) — webhook delivery VERIFIED (test post 200, 2026-06-23). Build: a small INDEPENDENT `uts-prod-monitoring-deadman` Cloud Run job on its own Cloud Scheduler (separate minimal image; does NOT touch PubSub/alerting-service) that each tick reads every fleet-monitor + meta-watcher `vm-census/<name>-last-run.json` sentinel + queries `lifecycle-events-sub` oldest_unacked age, and on any staleness posts DIRECTLY to `MONITORING_DEADMAN_SLACK_WEBHOOK`. Terminal bedrock: ONE `google_monitoring_alert_policy` on that deadman job's own execution-absence → a `google_monitoring_notification_channel` (GCP-managed, survives our whole infra dying).
- [ ] [INFRA] P1. Set `retry_count > 0` (e.g. 2) on the fleet-monitor scheduler jobs in `data_pipeline_fleet_monitor_scheduler.tf` so a transient Cloud Run invocation failure does not silently drop a tick.
