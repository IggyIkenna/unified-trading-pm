...
- [ ] [VERIFY] P0. **Prove monitors + deadman EXECUTE exit 0** on the rebuilt image (deployment-api build eb626798 in flight → re-resolve 4 jobs → execute each → assert succeededCount=1/failedCount=0 + deadman posts/decides cleanly). DO NOT flip the "watch-the-watchers LIVE" items or close until this is green.
