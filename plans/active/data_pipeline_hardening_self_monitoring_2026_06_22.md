...
- [ ] [INFRA] P0. **Out-of-band GCP-native dead-man's-switch (the top-of-chain watcher).** Add `google_monitoring_alert_policy` in `deployment-service/terraform/gcp/`: (a) on `pubsub.googleapis.com/subscription/oldest_unacked_message_age` for `lifecycle-events-sub` > 30 min (catches alerting-service subscriber down / Slack 404 — events piling up undelivered); (b) on the meta-watcher's own `last-run.json` sentinel (uptime/absence). Route to a `google_monitoring_notification_channel` **outside the DP_* Slack path** so the same failure can't swallow its own death-alert. BLOCKED-OPERATOR-DECISION: where does the out-of-band page go? (operator email / PagerDuty / a separate Slack webhook).
