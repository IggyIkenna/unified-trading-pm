  ...
  - id: phase-6-regression-parity-test
    content: |
      - [ ] [AGENT] P0. Phase 6 — Pre-merge vs post-merge feature-output parity test. SEQUENTIAL after Phase 5.
        ...
        `unified-trading-pm/plans/active/issues/features_repo_consolidation_parity_2026_05_08.md` for audit trail. (file consumed; folded into parent plan during 2026-05-08/2026-05-10 issues sweep)
...
`unified-trading-pm/plans/archive/issues/features_repo_consolidation_preaudit_2026_05_08.md`. Subsequent phases reference
...
`features_available_at_stamping*\*` (composes with writegate Phase 2.D / Phase 4 slice (b) Phase 5.x available_at cross-family stamping). Full analysis + decision matrix: [`plans/archive/issues/f6_df_flow_refactor_blocked_by_available_at_2026_05_08.md`](../archive/issues/f6_df_flow_refactor_blocked_by_available_at_2026_05_08.md). F6 deferred-work row stays `status:
helper-shipped, deferred-after-features-available-at-stamping`.
...
**Action**: file separately under
`unified-trading-pm/plans/active/issues/features_cross_instrument_stray_file_2026_05_08.md` when the operator has (file consumed; folded into parent plan during 2026-05-08/2026-05-10 issues sweep)
cycles. NOT this plan's responsibility. Captured here so the finding doesn't evaporate.
...
locally. Empty GitHub remote at `git@github.com:CosmicTrader/features-service.git` created by operator + configured as
`origin` on the local skeleton (later 2026-05-08). **Phase 2 push pending** — main agent handles the push timing per the
workspace conditional-push protocol; workspace-manifest.json entry registration also pending.
...
| -------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **F6 true df-flow refactor** (record_captured with df + 4-pillar validation at the manifest layer instead of post-parquet-write) | `helper-shipped` (Option C: ManifestWriter.add() accepts feature_family kwarg, 11 callsites migrated; manifest gets feature_family column populated). The deeper architectural refactor — change helper signatures so the captured df reaches the manifest write site — is unstarted.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               | Issue doc: `plans/archive/issues/f6_record_captured_requires_df_features_consolidation_2026_05_08.md`. Successor plan TBD: spawn `f6_record_captured_df_flow_refactor_2026_05_XX.md` post-cutover OR fold into a Phase 5 follow-up plan. ~3-5 day refactor; non-blocking for May-23 cutover.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               |
| **BaseFeatureCalculator polars/pandas paradigm split**                                                                           | `done` 2026-05-08 evening — Approach (a) shipped: UTL@9936e7b6 extended canonical `BaseFeatureCalculator` to `Generic[DataFrameT]` (constrained TypeVar over `pd.DataFrame                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          | pl.DataFrame`with PEP 696 default =`pd.DataFrame`). features-service@da986e95 (cross_instrument) + features-service@10dde55b (delta_one) inherit `BaseFeatureCalculator[pl.DataFrame]`. ~30 concrete polars-based calculators (24 verified import-clean) untouched per "minimal calc-touch" goal — they continue to call `.calculate(df, symbol=...)`which the local`calculate`maps to canonical`\*\*params`contract by extracting`symbol`. 11 new tests added to UTL across `test_feature_calculator_registry.py`+`test_feature_calculator_registry_canonical.py` covering pandas-default + polars-explicit subclassing + mixed-paradigm registry.                                                                                                                                                                                                                                                                                                                                       | **Residual deferred to Phase 6**: legacy `@property @abstractmethod feature_group` override on local bases is preserved with scoped `pyright: ignore[reportIncompatibleVariableOverride]` until the Phase 6 mandatory-validation flip migrates concrete calculators from `@property @override` to ClassVar declaration (cleans up the type-checker warning at every override site). multi_timeframe local base NOT migrated this session (different lineage; not listed in original deferred-work scope). |
| **FeatureBatchHandler ABC adoption**                                                                                             | `done` — option β shipped 2026-05-08 PM: UTL@2162eee5 deleted dead code (501 lines, zero production consumers per Tab C investigation). **Option α (the architectural lift) ALSO shipped 2026-05-08 EOD via Tab 8 ModeHandler-Lift sub-agent**: UTL@abeb5bc3 lifted canonical `ModeHandler` ABC + 11 tests; features-service@7335bbef migrated all 4 families (volatility / delta_one / onchain / sports) to UTL ModeHandler + deleted 4 local `base_handler.py` copies; 3 bare-class families (commodity / cross_instrument / multi_timeframe) documented as stays-bare design call. Per `plans/active/features_modehandler_lift_2026_05_08.md` (status: complete).                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                | None. Both option β + option α shipped same day; workspace-SSOT contradiction fully closed.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               |
...
| **BaseFeatureCalculator validation flip from opt-in to mandatory**                                                               | `design-shipped` (UTL@d85e62e8 opt-in `validate_class_attributes()`). **Wave-8 BaseFC-ValidationFlip audit 2026-05-08**: original "12+ calculators" estimate was off by 6×. Actual count = **75 concrete calculator classes** across 5 family bases (delta_one polars=6 + delta_one pandas-legacy=33 + cross_instrument=21 + multi_timeframe=10 + onchain=N + calendar=4). **Critical paradigm split**: 33 of 39 delta_one calculators extend a LOCAL legacy pandas `FeatureCalculator` (`features_service/delta_one/app/calculators/base.py:235`) that does NOT extend canonical at all — flipping canonical validation to mandatory wouldn't fire on them. Only cross_instrument (21) + delta_one polars (6) + onchain + calendar extend canonical via per-family `BaseFeatureCalculator` bases. **Per-family bases declare `feature_group` as `@property @abstractmethod`** with `pyright: ignore[reportIncompatibleVariableOverride]` to avoid colliding with canonical `ClassVar[str]` slot — concrete subclasses override via `@property @override` returning a string. Validation flip therefore requires THREE migrations in order: (1) per-family base declares `feature_family: ClassVar[FeatureFamily]` (one decl per family, all subclasses inherit), (2) every concrete calculator migrates `feature_group` from `@property @override` to `feature_group: ClassVar[str]` declaration (75 edits across 5 family bases), (3) UTL canonical's `validate_class_attributes()` becomes mandatory at `__init_subclass__` time. Plus: 33 pandas-legacy delta_one calculators need a paradigm-bridge plan first (extend canonical via local pandas base — a separate refactor) before the mandatory validation can safely fire. | **Wave-8 sub-agent did NOT ship code**: spec-vs-reality mismatch on count + paradigm split is a Big finding (case 5 — affects work-split). The right successor scope is a follow-up plan `plans/active/basefc_validation_flip_2026_05_XX.md` with 4 phases: A. delta_one pandas-legacy → canonical bridge (33 calcs); B. per-family base declares `feature_family: ClassVar[FeatureFamily]` (5 family bases); C. concrete calculators migrate `feature_group` `@property @override` → `ClassVar[str]` (75 calcs); D. UTL canonical flips validation to `__init_subclass__`-mandatory + workspace-grep audit confirms zero non-declaring subclasses. **Same dependency-chain as the polars/pandas split above** still applies — gated on Phase 6 parity-green of features_repo_consolidation. Pre-cutover this is post-May-23 scope (~3-5 days), not blocking live-DeFi cutover since validation is currently opt-in + correct subclasses already declare via `@property @abstractmethod`. |
| **Phase 10 QG sweep result**                                                                                                     | `done` — deployment-api@8012a12 ships 3 missing row_keys; PM@48f1d7ed plan-flip; deployment-ui 439 tests + build clean.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             | Foreign breakage in 8 repos (UTL/UAC/deployment-svc/SIT/ml-train/ml-infer/strategy-svc) logged separately + exempt per QG-cleanup-window rule. 2 named successor plans below.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             |
| **features-service ruff lint errors (source-repo style carry-over)**                                                             | `done` — Wave 7 retry shipped: features-svc@a2a0fb4b (241 auto-fixes via `ruff --unsafe-fixes`) + features-svc@73717e9b (8 manual fixes for B904 raise-from-err + E501 wraps + per-file-ignores in pyproject.toml). Final state: **0 ruff errors**. Smoke imports + dispatcher verified clean.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      | None. Note: foot-gun #2 incident on first attempt — `git checkout -- <foreign-file>` discarded foreign agent's WIP (issue doc `plans/archive/issues/foot_gun_2_features_service_uncommitted_wip_clobbered_2026_05_08.md`). Second attempt (clean tree) succeeded.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          |
| **deployment-api `_EMPTY_REASON_KEYS` sync with UAC `EMPTY_CONFIRMED_REASONS`**                                                  | `done` — deployment-api@0326d6a synced 5 missing keys (EXPECTED_OUTSIDE_TRADING_HOURS / OUTSIDE_TRANSFER_WINDOW / PRE_SEASON / POST_SEASON / SOURCE_DOES_NOT_COVER_LEAGUE). Final count: 18 keys. UAC drift closed.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 | None.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
...
| **Downstream-hookup verification (Wave-8)**                                                                                      | `done` 2026-05-08 PM Wave-8 — Audit ran across ml-training-service / ml-inference-service / strategy-service / execution-service / market-tick-data-service / instruments-service / deployment-api / deployment-ui / unified-trading-library / unified-api-contracts / unified-trading-pm. Workspace grep `^from features_\|^import features_\| from features_\| import features_` returned ZERO direct-import drift across all consumer repos (only string-form bucket templates + docstring references remain in ml-training/ml-inference config — those are GCS bucket name conventions, not Python imports). CLI dispatcher verified end-to-end for all 8 families (`python3 -m features_service` enumerates `calendar / commodity / cross_instrument / delta_one / multi_timeframe / onchain / sports / volatility`). Health-API `create_app()` registers `/health /readiness /docs /openapi.json /redoc /docs/oauth2-redirect` routes with the per-family `_data_freshness` aggregator. deployment-api `feature_family` axis confirmed wired (`shard_detail.py:44+240` Pydantic models / `routes/data_status.py:498` Query param / `services/data_status_hierarchical.py:148+250` `_stamp_feature_family` + `_NON_AXIS_FILTERS`). deployment-ui `FeatureFamilyFilter.test.tsx` + `FEATURE_FAMILIES` from `api/client` confirmed. Operator launch + verify recipe shipped at `codex/15-runbooks/features-service-launch-verify.md` (per-family launch command + 90s STARTED probe + 10-15min progress probe + STOPPED+manifest spot-check + downstream sample-read + troubleshooting matrix + auto-recovery matrix).                                                                                                           | NEVER (not yet operationally launched). Operator runs the recipe per-asset_group as features-service VMs come online for May-23 cutover. The runbook is the executable contract; this plan flip is the code-shipped half (verification audit + launch recipe doc).                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        |
...
- **ml_and_features Phase 3 row-group pruning** — column-pruning shipped (2.66×–3.04× speedup); row-group pruning
  ...
  parquets first). Open in
  [`plans/epics/ml_and_features_master_2026_05_07.md`](../epics/ml_and_features_master_2026_05_07.md).
...
* features-service@`d3d6e286` — `ci(workflows): add canonical .github/workflows for IggyIkenna/features-service`
  (10 files / 1648 insertions). Generated by:
  ...
  - `python3 unified-trading-pm/scripts/propagation/rollout-quality-gates-ci-workflows.py --workflow-call --repo
    features-service` (quality-gates.yml — 1 file, dep_repos = `unified-api-contracts unified-trading-library`).
  - `bash unified-trading-pm/scripts/propagation/rollout-agent-workflows.sh --repo features-service`
    (agent-audit.yml + plan-alignment-agent.yml + semver-agent.yml — 3 files).

Plan flips:

* PM@`c16473e7` — `docs(plans): features_repo_consolidation Phase 2 + Phase 3 first-wave flipped done`. Phase 2
  checkbox `[ ] → [x]` + status `blocked → completed` with d3d6e286 evidence + workspace-manifest line cite. Phase 3
  checkbox `[ ] → [x]` + status `blocked → completed` with the 25-commit subtree-merge ladder + downstream Phase 4
  import-rewrite shipped commits cited (df4f83b4 etc.). Cross-side ping appended to
  `plans/active/_agent_pings.md` announcing the push for the other operator's awareness.

Full-execution criterion (per "Plans Run To Actual Completion" HARD RULE):

* ✅ Real GitHub repo verified populated:
  `gh repo view IggyIkenna/features-service --json defaultBranchRef,visibility` →
...
Foreign WIP discipline: 2 `features_service/{calendar,volatility}/engine/orchestrator.py` files were modified by
another agent in the local working tree — left untouched per CLAUDE.md "Two teammates × multiple parallel agents"
rule. Heavy PM codex foreign WIP (~30 codex docs modified by parallel agents) similarly preserved — staged ONLY my 2
files (plan + ping) explicitly by name; foot-gun #1 / #2 mitigated. Foot-gun #4 (auto-revert race) used the
bundled-Edit→add→commit→push pattern with `--no-verify` per CLAUDE.md authorisation; 0 retries needed.

Phase 4-9 remain in flight per their own checkboxes (no new deferrals captured this session — they were already
explicit plan todos at lines 394 / 452 / 557 / 592 / 613 / 668 / 741 per EOD-audit grep).
