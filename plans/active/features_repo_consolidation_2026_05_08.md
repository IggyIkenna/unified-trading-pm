...
| **FeatureBatchHandler ABC adoption** | `done` — option β shipped: UTL@2162eee5 deleted dead code (501 lines, zero production consumers per Tab C investigation). Successor plan for option α (lift local 4-family ModeHandler to UTL) filed at `plans/active/features_modehandler_lift_2026_05_08.md` (P2, post-cutover). | None. Workspace-SSOT contradiction closed; future ModeHandler-lift work captured in named successor plan per HARD RULE "Temporary state must have a named successor plan". |
