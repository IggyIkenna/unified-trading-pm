...
| **FeatureBatchHandler ABC adoption** | `helper-shipped` (UTL@7aba113c) — **2026-05-08 follow-up investigation: actually `zero-consumers` not 5/8 adopted; 8 families run 4 distinct unrelated shapes (3 bare `class BatchHandler:`, 4 `ModeHandler` subclasses, 1 `BaseModeHandler` subclass).** | **NEEDS-OPERATOR-TRIAGE** per [`plans/active/issues/feature_batch_handler_abc_zero_consumers_2026_05_08.md`](issues/feature_batch_handler_abc_zero_consumers_2026_05_08.md). Three real options: (α) narrow UTL ABC + lift `ModeHandler` to UTL (the 4-family overlap is the actual canonical shape); (β) delete `FeatureBatchHandler` from UTL (zero consumers = dead code); (γ) defer permanently with successor plan. Until operator routes: do NOT spawn another adoption sub-agent — same wall every time. ~2-3 day architectural call, not a 1-2 hour adoption refactor. |
...
| **features-service 345 ruff lint errors (source-repo style carry-over)** | `style-only` (no behaviour impact). Source repos had per-file-ignores in their old pyproject; those configs dropped during subtree-merge so rules now apply uniformly. | Successor plan: `plans/active/features_service_lint_cleanup_2026_05_XX.md` (TBD spawn). Mechanical cleanup: `ruff check --unsafe-fixes` for RUF005/SIM117/etc., manual review for C901/RUF012, OR restore per-package per-file-ignores in features-service `pyproject.toml`. ~1-2 hour spike. |
| **deployment-api `_EMPTY_REASON_KEYS` sync with UAC `EMPTY_CONFIRMED_REASONS`** | `dependency-drift` — 1 test fail in `test_empty_reason_breakdown` because UAC taxonomy expanded with new reasons (EXPECTED_OUTSIDE_TRADING_HOURS / EXPECTED_PRE_SEASON / EXPECTED_OUTSIDE_TRANSFER_WINDOW / EXPECTED_SOURCE_DOES_NOT_COVER_LEAGUE / EXPECTED_POST_SEASON) but deployment-api `_EMPTY_REASON_KEYS` not synced. | Successor plan: fold into next deployment-api maintenance commit (or spawn `deployment_api_empty_reason_keys_sync_2026_05_XX.md`). 5-line dict update + test refresh. ~10 min. |
