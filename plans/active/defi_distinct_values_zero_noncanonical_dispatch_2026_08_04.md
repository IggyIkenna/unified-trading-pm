...
| 4   | Migrate `dex_pools`/`dex_swaps`/`rate_indices` (~4M rows)                                                                                                                                                                                    | Interactive session 2026-08-05 (dex_pools only)                                              | **`dex_pools` DONE.** Turned out to be pure manifest retirement (content-verified redundant with an existing `dex_pool_state` twin), not a GCS rename — see `retire_dex_pools_legacy_captured_rows_2026_08_05.py`. Ran against prod: **453,985 rows retired** (capture_status captured→attempted_failed, GCS objects untouched, fully reversible), **29 rows excluded** (no canonical twin found, all dated 2025-01-17 — a real, narrow residual, NOT retired, tracked as a small follow-up). Round-trip verified. `dex_swaps`/`rate_indices` UNCHANGED — still not started, see the linked doc.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     | Check `plans/active/issues/defi_legacy_data_type_names_manifest_migration_scope_2026_08_04.md` for the dex_swaps/rate_indices remainder — those are genuinely bigger, unstarted items, not touched this session.                                                                                                  |
