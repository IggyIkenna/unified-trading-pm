...
| Phantom manifest audit (P1) — `instruments-service/scripts/reconcile_phantom_manifest_rows_all.py --asset-group {cefi,defi,sports,tradfi,prediction} --dry-run` | **DeFi ✅ RAN 2026-05-11** on `defi-phantom-recon-defi-20260511-192115` (GCE same-region, e2-standard-4; `launch-defi-phantom-recon-vm.sh defi --dry-run`; 88557 prefixes listed @~502/sec, completed 13:58 UTC, exit 0, VM self-deleted): **311602 real captures, 1298 "phantom captures" — ALL `venue=EIGENLAYER` / `data_type=rewards`, ALL FALSE-positive** (data exists on disk at `…/venue=EIGENLAYER/chain=ETHEREUM/instrument_type=staking/data_type=eigenlayer_rewards/rewards.parquet` — the audit probes the manifest row's `data_type=rewards` segment which doesn't match the disk's `data_type=eigenlayer_rewards/`). Root cause = shard-key drift in `eigenlayer_rewards_handler.py` (records `data_type="rewards"` but writes parquet under `data_type="eigenlayer_rewards"/`) → routed to `defi_master_2026_05_07.md` § Discoveries (P1, fix = align the `record_captured` data_type to the parquet path + one-time manifest migration + fix the stale handler docstring). **Real residual = 0 for DeFi** (no actual missing data). **Did NOT `--apply`** (flipping 1298 good rows would corrupt the manifest, 2026-05-04-class). **cefi/tradfi/prediction/sports — still pending** (cross-asset rescan launcher `launch-cross-asset-rescan-vm.sh` is the newer canonical tool but it's gated on the operator watchdog-VM relaunch to pick up the `cross-asset-rescan-` prefix + a tarball refresh; `launch-defi-phantom-recon-vm.sh` only covers DeFi). 2026-05-04 cefi baseline = 354 residual stands until re-run. | `plans/active/work_split_2026_05_11_harsh.md` § Slot 6 (P1); `plans/active/issues/qg_sweep_2026_05_11.md` § cross-refs + "Days 2-4 follow-up" (1); `defi_master_2026_05_07.md` § Discoveries (EIGENLAYER shard-key drift) |
