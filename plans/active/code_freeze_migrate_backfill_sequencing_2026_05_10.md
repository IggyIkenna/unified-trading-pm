...
| Phantom manifest audit (P1) — `instruments-service/scripts/reconcile_phantom_manifest_rows_all.py --asset-group {cefi,defi,sports,tradfi,prediction} --dry-run` | **DeFi ✅ + CeFi ✅ RAN 2026-05-11** (GCE same-region, e2-standard-4, via `launch-defi-phantom-recon-vm.sh <ag> --dry-run` — the launcher accepts any asset_group despite the `defi-` name). **DeFi** (`defi-phantom-recon-defi-20260511-192115`, 88557 prefixes @~502/sec, done 13:58 UTC, exit 0): **311602 real / 1298 "phantom" — ALL `venue=EIGENLAYER`/`data_type=rewards`, ALL FALSE-positive** (data exists at `…/venue=EIGENLAYER/chain=ETHEREUM/instrument_type=staking/data_type=eigenlayer_rewards/rewards.parquet`; audit probes the manifest's `data_type=rewards` ≠ disk's `data_type=eigenlayer_rewards/`). Root cause = shard-key drift in `eigenlayer_rewards_handler.py` → routed to `defi_master_2026_05_07.md` § Discoveries (P1; fix = align `record_captured` data_type to parquet path + one-time manifest migration + fix stale docstring). **DeFi real residual = 0**. **CeFi** (`defi-phantom-recon-cefi-20260511-193451`, 129220 prefixes @~370/sec, done 14:16 UTC, exit 0): **1290706 real / 2223 "phantom" = 0.17% phantom rate — UNDER the <0.5% `cefi_master` criterion**. Residual 2223 across drift-axis-suspicious clusters: blank `venue` 1453, DERIBIT 136 (mostly `options_chain`/`futures_chain` — bundled types), `venue=UNKNOWN` 111, Bitfinex `*F0` perp codes ~400. NOT obviously real-missing — classic manifest-vs-disk shape drift (blank/UNKNOWN venue, chain-bundle equivalence, venue-normalization). Per-cluster real-vs-false-positive triage pending (cefi owner) — annotated in `cefi_master_2026_05_07.md` § "Port phantom-audit" todo. **Did NOT `--apply`** either run (false-positive majority; flipping would corrupt the manifest, 2026-05-04-class). **tradfi/prediction/sports — still pending** (run `launch-defi-phantom-recon-vm.sh {tradfi\|prediction\|sports} --dry-run` sequentially — singleton-locked; OR the cross-asset rescan launcher `launch-cross-asset-rescan-vm.sh` does all-in-one but is gated on the operator watchdog-VM relaunch for the `cross-asset-rescan-` prefix + a tarball refresh). | `plans/active/work_split_2026_05_11_harsh.md` § Slot 6 (P1); `plans/active/issues/qg_sweep_2026_05_11.md` § cross-refs + "Days 2-4 follow-up" (1); `defi_master_2026_05_07.md` § Discoveries (EIGENLAYER shard-key drift); `cefi_master_2026_05_07.md` § "Port phantom-audit" todo (CeFi residual breakdown) |
