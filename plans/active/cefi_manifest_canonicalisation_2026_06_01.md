...
| ⑧   | IS-catalogue completeness (CF-14)    | 🔴 RED-tracked | Catalogue `prod/catalog.parquet` = 213,990 rows / **12 venue IS reference universe**; manifest captured present-set = **45 venues** → **29 captured venues MISSING from IS reference = 108,556 captured rows (8.3%)**, headline **KRAKEN-SPOT (75,714) + KRAKEN-FUTURES (31,582)** (genuine — KRAKEN-FUTURES on disk 2026-05-24), + PACIFICA-SOLANA/LIGHTER-ZKSYNC + ~650 `*F0`/`UNKNOWN` manifest-pollution rows. ROOT = **IS cefi reference universe gap** (`instrument_availability/by_date/day=2026-05-22/` lists only 12 venues, KRAKEN absent at SOURCE — not a rollup bug) ⇒ could-exist denominator excludes venues MTDS captures ⇒ falsely-high coverage. Gates honest coverage, **NOT the G4 data/manifest migration**. New todo below. WALKED both full datasets (sub-agent). |
...
- [ ] [DATA] P0. **⑧ — IS cefi REFERENCE-UNIVERSE gap: catalogue not ⊇ manifest present-set (CF-14, falsely-high
      coverage).** Real-prod (2026-06-08): IS `instruments-store-cefi-prd`
      `instrument_availability/by_date/day=2026-05-22/` lists only **12 venues** {ASTER, BINANCE-FUTURES/SPOT,
      BITFINEX-FUTURES, BYBIT, COINBASE-SPOT, DERIBIT, HYPERLIQUID, OKX-FUTURES/SPOT/SWAP, UPBIT}; the MTDS manifest
