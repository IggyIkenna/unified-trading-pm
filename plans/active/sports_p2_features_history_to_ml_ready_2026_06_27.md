...
> **🔶 2026-07-23 — SCOPE-OVERLAP NOTICE (plan-reconcile audit): this plan overlaps
> `sports_consolidated_closeout_2026_07_19.md` and was discovered as an orphan (never linked to it) during that audit.**
> Specific conflict: this plan's frontmatter `last_updated: 2026-06-27` is stale relative to its own Progress Log
> (entries continue through at least 2026-07-17); it still carries an open P0 `[VERIFY]` todo ("Folded-in scope
> 2026-07-15" section) re-confirming `FIXTURES_SCHEDULE`/`FIXTURES_OUTCOMES` column population post-entity-split — the
> IDENTICAL schema migration the closeout claims as its own canonical target (its Track C1 / Track F); and its Progress
> Log documents live production paths still keyed on a lower-case `data_type=odds_horizon_bucket` (a dead cohort per the
> closeout's canonical target) and a raw/manifest-style `league_id` (e.g. `SOCCER_RUSSIA_PREMIER_LEAGUE`) in the same
> league_id-namespace class as the closeout's own unresolved P0 league_id migration. **Do not resolve this conflict
> unilaterally from this doc** — check `sports_consolidated_closeout_2026_07_19.md`'s current Track sections (esp. Track
> C/F/V) for the latest state before acting on either doc.
...
- [x] ✅ [DATA] P0. **Compute features 2015→present** (year-chunked, skip-existing) for all three groups within their
      coverage windows. **Gate**: `sports_features/by_date/day=*/feature_group=*/features.parquet` exists for every
      in-coverage day with fixtures; features manifest `captured`; runs `exit_code=0`. — 10-VM `fss-backfill-vm-1..10`
      fleet (2015-01-01→2026-07-17) all exited rc=0 and self-deleted; 2 residual gap dates (2019-04-05, 2019-07-25,
      confirmed real fixtures via upstream check) gap-filled directly via
      `features_service.sports --operation compute --mode batch --asset-group SPORTS --skip-existing`, both exit=0.
      Final
      `check_pipeline_completeness.py --start-date 2015-01-01 --end-date 2026-07-17     --services features-sports-service`:
      **4216/4216 dates fully complete (100.0%), 0 missing, exit_code=0**. 1163 total ML features (993 derived across 22
      calculators + 142 odds + 28 xg).
...
### 2026-07-17 02:xxZ — data_engineering slot-15 (Todo 1 dispatch — BLOCK CONFIRMED LIFTED, launched 10-VM full-history compute)

Fresh-pulled all 24 slot repos clean. Dispatched to Todo 1. Before launching anything, independently re-verified the
freeze state live via the non-snap `gcloud` (`/home/ubuntu/google-cloud-sdk/bin/gcloud`, account
`ikenna@odum-research.com` — the default `github-actions-deploy` SA hits a workload-identity 404 outside real GHA
runners):

- `gcloud scheduler jobs describe uts-prod-manifest-consolidator-{features,market-data,instruments}-sports-cron --location=asia-northeast1 --project=central-element-323112`
  → **all three `ENABLED`**, `lastAttemptTime` ~02:12-02:13Z today — matches the API activity log's `prerequisite_set`
  event (`sports-cutover-phase6-consolidator-resumed=true`, `set_by=slot-phase6-restore`, ts `2026-07-17T02:07:55Z`,
  evidence "T6.1 gate passed... 4 clean ticks"). Cross-checked `sports_legacy_bucket_cutover_2026_07_16.md` T6.1
  Progress Log entry — MERGE COMPLETE, both instruments+MDT indices verified BY CONTENT (a real data-loss bug fired and
  was recovered in-band during that merge; see
  `issues/consolidator_content_write_marker_strip_silent_shard_reap_2026_07_17.md` — orthogonal to this task). The
  backlog `/api/backlog/<id>/blockers` endpoint independently confirms `"ready (no blockers)"`.
- **Conclusion: the freeze genuinely lifted overnight — this is NOT the regen-drop-prereq bug class, it's a real,
  evidence-verified un-gate.** T6.1b's plan checkbox is cosmetically still `- [ ]` (restore-agent didn't flip it) but
  the underlying gate (3 clean ticks, scheduler ENABLED) is done per the activity-log evidence.

Ran
`features-service/scripts/sports/check_pipeline_completeness.py --start-date 2015-01-01 --end-date 2026-07-16 --services features-sports-service`
fresh (needed `GCP_PROJECT_ID=central-element-323112 DEPLOYMENT_ENV=prod` env vars; undocumented in the script's own
usage block — worth a follow-up doc fix): **4019/4215 dates present (95.3%)**, 196 genuine gap dates (2018: 57, 2019:
61, 2020: 59, 2024: 6, 2026: 13) — NOT the 0/365 empty-bucket state from the 2026-06-27 Todo-2 VERIFY run; a prior
slot's 4-VM 2017-02-01→2026-07-16 launch (19:58Z 2026-07-16, `e6c545565`) mostly landed before crash-looping on the
then-live freeze. Checked live GCE (`gcloud compute instances list`) — 0 running VMs, confirming that fleet is fully
gone (self-deleted post-fail_fast or completed-and-cleaned).

**Launched fresh 10-VM SPOT parallel backfill**
(`deployment-service/scripts/vm/launch-features-sports-parallel-backfill-vm.sh --start 2015-01-01 --end 2026-07-17 --vms 10 --env prod`,
default skip-existing, no `--force`): `fss-backfill-vm-1..10`, ~421-day chunks each, all confirmed `RUNNING` at launch.
**Verified past the startup gate** (the exact failure mode that killed the prior fleet): vm-5's log shows
`sports batch startup gate: market-data consolidator healthy for sports` +
`instruments-store consolidator healthy for sports` — no staleness fail_fast. vm-1/2/3/4/6 are actively iterating dates
(mostly `SKIP <table> — manifest shows prior captured/empty`, confirming skip-existing is working — not a silent
no-op-everything, since the 196 gap dates will fall through to real compute as each VM's chunk reaches them).
vm-7/8/9/10 were still in normal tarball/uv bootstrap at check time (no failure signal). Checkbox stays `- [ ]` — gate
...
### 2026-07-17T12:0xZ — data_engineering slot-15 (Todo 1 dispatch — fss-backfill-vm-4 completed, 2 residual gaps gap-filled directly, gate PASSES 4216/4216, checkbox flipped)

**Todo 1 COMPLETE.** Continuation from this same slot's 02:xxZ and 07:5xZ entries above. `fss-backfill-vm-4` (the last
of the 10-VM fleet) finished around 11:52Z — confirmed via 2 consecutive clean zero-reads from
`gcloud compute instances list` (exit=0, empty output), not a single read, because this session hit a recurring
`github-actions-deploy` SA workload-identity token expiry (~2.5h cadence, two occurrences: ~07:43Z and ~10:13Z) that
produces a **false empty-fleet read** when unguarded (`gcloud ... 2>/dev/null | wc -l` silently returns 0 on the auth
error). **Operational lesson for future long infra-monitoring sessions**:
`gcloud config set account ikenna@odum-research.com` recovers it; never trust a single `running_vms=0` from this SA
class without a second confirmed-clean read.

Ran `check_pipeline_completeness.py --start-date 2015-01-01 --end-date 2026-07-17 --services features-sports-service`:
**4214/4216 (99.95%), 2 genuine gap dates** (2019-04-05, 2019-07-25 — "STALE" in this script's output is just a 48h
freshness label, not a correctness signal; only `gap_dates` drives the exit code). Verified both dates have real
upstream fixtures (`gsutil ls .../entity=fixtures/` — 8 and 44 fixtures respectively, not honest-absence), so gap-filled
them directly (no VM needed for 2 dates):
`features_service.sports --operation compute --mode batch --asset-group SPORTS --start-date <D> --end-date <D> --skip-existing`
— both exit=0, `derived_features`/`fixture_features` written, manifest updated.

**Re-ran the completeness check**: `4216/4216 dates fully complete (100.0%), 0 missing, exit_code=0`. Gate met. Flipped
Todo 1's checkbox with evidence. Todo 2 (ML-ready VERIFY) and Todo 3 (manifest-clean, completed concurrently by slot-4's
08:0xZ dispatch above) were also already `[x]` — all three original Todos now closed; only the "Folded-in scope
2026-07-15" VERIFY item (Q5/Q6 phase-timestamp columns, BLOCKED-UPSTREAM) remains open on this plan.

Follow-up work (both already tracked, not done here — out of this task's craft/scope):
`issues/sports_elo_calculator_tz_naive_season_boundary_silent_skip_2026_07_17.md` Todo 2 (gap-fill date-ranges computed
before the elo fix landed) and Todo 3 (audit for sibling naive-Timestamp-comparison bugs). Shipping this flip + `/done`.
