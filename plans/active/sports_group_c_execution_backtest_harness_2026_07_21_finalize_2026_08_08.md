...
- [ ] [REVIEW] P3. **Resolve the parent plan's one remaining judgment call — todo 5 (docs/BACKTESTS.md placement).**
      **PREMISE CORRECTED 2026-08-09 (plan_reconciler, adversarially verified)**: the parent plan's "currently DEAD"
      finding was wrong — `docs/BACKTESTS.md` lives in strategy-service (not execution-service), is current, and already
      lists `run_sports_arb_backtest.py`. It documents a DIFFERENT surface (strategy-service's Group-B portable scripts)
      than this todo's actual target (execution-service's Group-C `backtest_domains.py` CLI functions). Confirmed via
      `docs`/`specs` search: **no equivalent surface exists in execution-service for ANY of the 3 existing domain
      runners** (`run_cefi_backtest`/`run_tradfi_backtest`/`run_defi_backtest`) — so the "if yes, add alongside the
      sibling precedent" branch this todo originally offered is unreachable as worded; only the "leave it a
      manually-invoked one-off and record why" branch applies, UNLESS the executor decides this is the moment to create
      a first `execution-service/docs/BACKTEST_DOMAINS.md` for all 4 Group-C runners together. **Done when**: the
      placement decision is made and evidenced (either a real diff adding a new execution-service doc, or a stated
      reason to stay manually-invoked), and the parent plan's todo 5 is flipped `[x]` with that evidence. Repo:
      execution-service.
...
- **2026-08-08 (na-eligibility-audit, sports tranche, round7 RECLASSIFY sweep)**: Drafted alongside
  ...
  finalize sibling.
- **plan_reconciler 2026-08-09 (agt-c3a27f, slot 13, prediction tranche)**: corrected todo 2's premise in lockstep with
  the parent plan's todo 5 fix — the "if yes" precedent-match branch was unreachable as worded (no execution-service
  surface exists for any Group-C domain runner), so pointed the executor directly at the real decision (stay
  manually-invoked, or create a first execution-service doc for all 4 runners together).
