...
- [x] ✅ [DECISION] P2. **Retagged 2026-07-29 (corpus hygiene pass): resolved-by-reference — see
      ...
      into dispatchable todos here. **Addendum (2026-07-30, batch3_finalize todo-3 re-check) — this retag over-claimed
      for 2 of the 4**: batch5's "wider-corpus re-audit", cited above as covering all 4, actually only genuinely
      re-examined 2 of them (`sports_legacy_fixtures_path_migration_2026_07_24.md`'s census,
      `issues/sports_phantom_audits_reference_not_marketdata_2026_07_14.md`'s spot-check) — its own
      `data_completion_sports_2026_07_24.md` section discusses two DIFFERENT candidate items (a rate-limit calibration
      probe + an API-Football quota bump), not the Transfermarkt re-attempt / ODDS+PREDICTIONS blank-reason items this
      doc's own Deferred section named. Those 2 were genuinely still un-actioned as of 2026-07-30. Now actually
      resolved: converted from prose into 2 tracked `- [ ]` todos directly in `data_completion_sports_2026_07_24.md`
      (citing entries #5/#6), the legacy-fixtures census confirmed conflict-cleared per entry #7 (note added in its own
      doc), and the phantom-audit spot-check explicitly folded into
      `sports_satellite_ao_dispatch_batch7_2026_07_27.md`'s decision-16 todo per entry #8 (that todo's text amended +
      the phantom-audit doc's own todo marked "do not dispatch independently"). `unified-trading-pm` (this commit).
