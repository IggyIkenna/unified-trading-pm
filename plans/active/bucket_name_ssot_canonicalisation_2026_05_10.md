...
| Done-def #5 — QG STEP 5.69 (`f"gs://..."` ratchet)                                                                                      | `todo` ([ ])                                                                      | NO LONGER blocked-on-#3 (A6 deferred #3 to Phase 2.6; the baseline ratchet is Phase-1-shippable — ratchet point is "no NEW inline f-strings beyond today's count"; when the to-be-removed inline templates land in Phase 2.6 the baseline ratchets DOWN). Design in § Pre-audit manifest → "QG STEP 5.6X design"; STEP **5.69** (5.66 reserved multi-process-launcher; 5.67 taken banned-NaN-placeholder; 5.68 reserved by `available_at_lookahead_bias_completion_2026_05_08.md` — confirmed via `base-service.sh` grep; matches CLAUDE.md). Implementation owner = slot 4 (this session, in progress).                                                                                                                                             |
| Done-def #6 — plan-flip cite + grep audit table (zero drift)                                                                            | `blocked` ([ ]) — partial doable now; full zero-drift = Phase 2.6                 | A PARTIAL audit table (workspace-grep of `get_bucket_name`/`BUCKET_PREFIXES`/inline `gs://` f-strings → migrated-vs-remaining classification) is producible NOW; the FULL zero-drift verification is DEFERRED-AFTER #3 + #5 + the Phase-2.6 provisioning/migration (then verify drift ≤0.01% per bucket + zero readers still hit flat names). Slot 4 (this session) ships the partial table; full table = Phase-2.6 owner.                                                                                                                                                                                                                                                                                                                           |
