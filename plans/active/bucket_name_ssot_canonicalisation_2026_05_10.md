...
1. **Phase 2.6 cutover sequence** (the Phase-2.6 owner — slot 4 was assigned this window; Ikenna can re-assign): per
   ...
   - **2.6.4 / Done-def #3** — flip `get_bucket_name`/`BUCKET_PREFIXES`→`resolve_bucket_name` workspace-wide (~36+
     ...
     internal flat-shape bucket templates with `resolve_bucket_name(...)`, reconcile the L5.1↔L5.2 `ml-*` drift — recipe
     in § Pre-audit manifest "Layer 5") **+ the L2-tail `dependency_checker.py` probe-template migration** (the
   ...
   - **2.6.5** — verify writers writing to env-tiered buckets; archive (don't delete) flat buckets to
     ...
     > **2026-05-19 dry-run pre-verified (slot 2)**:
     > `deployment-service/scripts/archive-flat-buckets.sh --env prod --cloud both --dry-run` ran clean — 53 GCP + 53
     > AWS flat buckets enumerated; lifecycle JSON logic verified. macOS compat note: needs bash 4+ (use
     > `/opt/homebrew/bin/bash` on macOS; script requires `mapfile`). No issues found — ready to run live post
     > write-pause ack. **Write-resume verification one-liners (pre-staged)**: run these after L3/L5 push to confirm
     > env-tiered buckets accessible before resuming writes:
     >
     > ```bash
     > PID=central-element-323112
     > gcloud storage ls gs://instruments-store-cefi-prd-${PID}/
     > gcloud storage ls gs://instruments-store-defi-prd-${PID}/
     > gcloud storage ls gs://instruments-store-tradfi-prd-${PID}/
     > gcloud storage ls gs://instruments-store-sports-prd-${PID}/
     > gcloud storage ls gs://market-data-tick-cefi-prd-${PID}/
     > gcloud storage ls gs://market-data-tick-defi-prd-${PID}/
     > gcloud storage ls gs://market-data-tick-tradfi-prd-${PID}/
     > gcloud storage ls gs://market-data-tick-sports-prd-${PID}/
     > gcloud storage ls gs://features-calendar-prd-${PID}/
     > gcloud storage ls gs://ml-models-store-prd-${PID}/
     > gcloud storage ls gs://ml-predictions-store-prd-${PID}/
     > ```
     >
     > If any fail: STOP writes, ping operator.
...
## Deferred work after 2026-05-13 Slot 3 session

| Phase / item                                                                          | Status as of 2026-05-13 UTC                                                                                                                                                                                                                                                    | Successor / blocker                                                                               |
| ------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------- |
