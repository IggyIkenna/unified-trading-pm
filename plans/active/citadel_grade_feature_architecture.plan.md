...
| ------------------------------------------------------------------ | ------------- | ----------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| unified-feature-calculator-library (UFC) not T2-hardened           | `[PLAN_TODO]` | [phase2_library_tier_hardening.plan.md](phase2_library_tier_hardening.plan.md) § todo `t2-code-rewrite`                                         | New calculators (trendline.py, fibonacci.py, etc.) are added to UFC; UFC must pass D5 before they can be consumed downstream                                                                        |
| features-delta-one-service and ml-training-service not T4-hardened | `[PLAN_TODO]` | [phase3_service_hardening_integration.plan.md](phase3_service_hardening_integration.plan.md) § todos `t4c-features-layer` and `t4d-ml-pipeline` | The `feed-all-22-groups`, `multi-timeframe-stacking`, `cross-asset-features`, `regime-conditional-models` todos modify these services; services must be green at Phase 3 before these changes merge |
| Coinglass API key not obtained (liquidation-levels todo)           | `[EXTERNAL]`  | [api_keys_and_auth.plan.md](api_keys_and_auth.plan.md) — no current entry for Coinglass                                                         | liquidation_levels.py uses Coinglass heatmap API; verify Tardis coverage first; if not covered, must obtain Coinglass API key and add to SM before this todo can be implemented                     |
