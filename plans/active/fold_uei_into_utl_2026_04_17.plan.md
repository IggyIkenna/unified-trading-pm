---
name: fold-uei-into-utl
overview:
  Consolidate unified-events-interface into unified_trading_library.events (new sub-package name, aggregate of both
...
todos:
  - id: phase-1-aggregate-schemas
    ...
    status: done
    note: "Committed in 4da72fc0. Split into schemas.py + event_types.py."

  - id: phase-1-port-tests
    content: |
  ...
  - id: phase-1-wire-top-level-exports
    content: |
      - [x] [AGENT] P0. **Phase 1 / SEQUENTIAL** — Update `unified_trading_library/__init__.py` to re-export from new path. **DONE** (commit `4da72fc0`).
    ...
    status: done
    note:
      "Top-level re-export surface expanded with all new constants (AUTONOMOUS_RECOVERY_*, TRANSFER_LIFECYCLE_*,
      POSITION_DRIFT_*, BRIDGE_*, etc.). Plus 19 internal UTL source files rewritten."
  ...
  - id: phase-1-qg-utl
    content: |
      - [x] [SCRIPT] P0. **Phase 1 QG / SEQUENTIAL** — `cd unified-trading-library && bash scripts/quality-gates.sh`. **DONE** — passed in 61s on first clean run (17s on re-run post-split). UTL quickmerged as `4da72fc0` to `live-defi-rollout` (PR #222). Deviated from plan's "do not quickmerge yet" because (a) consumers pull UTL from git not local editable install, so UTL must be published before consumer QGs, and (b) the old `events_interface/` sub-package is retained in parallel, so no migration break window exists.
    status: todo
    note: ""
  ...
  - id: phase-2-migrate-mirror-consumers
    content: |
      - [ ] [SCRIPT] P0. **Phase 2 / PARALLEL** — Bulk rename `unified_trading_library.events_interface` → `unified_trading_library.events` across remaining 28 repos using UTL mirror path. QG each.
  ...
  - id: phase-3-delete-old-utl-mirror
    content: |
      - [ ] [AGENT] P0. **Phase 3 / SEQUENTIAL** — Delete old `unified_trading_library/events_interface/` sub-package (directory + __init__.py + schemas.py + sink.py + tests/events_interface/ if anything remained).
  ...
  - id: phase-4-update-qg-templates
    content: |
      - [ ] [HUMAN+AGENT] P0. **Phase 4 / PARALLEL with phase-4b** — Update QG templates that reference UEI.
  ...
  - id: phase-5-archive-uei-repo
    content: |
      - [ ] [HUMAN+AGENT] P0. **Phase 5 / SEQUENTIAL** — Archive UEI repo.
        - `mv unified-events-interface/ archive/unified-events-interface/` (workspace root).
        - Update [unified-trading-pm/workspace-manifest.json](unified-trading-pm/workspace-manifest.json) — remove all 9+ `unified-events-interface` entries (top-level repo map at line 90, cluster refs at lines 424/1320/1667/1870/1979/2130/2186, full repo def at line 1819).
        - Update workspace code-workspace files (`.cursor/workspace-configs/*.code-workspace`) to drop the UEI folder entry.
        - GitHub: archive the remote repo via `gh repo archive IggyIkenna/unified-events-interface` (HUMAN step — not run by agent).
    status: todo
    note: ""
  ...
  - id: phase-6-workspace-qg
    content: |
      - [ ] [SCRIPT] P0. **Phase 6 / FINAL** — Run `bash scripts/quality-gates.sh` in every repo listed in `repo_gates` above. Every repo must pass before the plan is archived. Each repo's QG is the C4 gate; quickmerge to staging/main is C5.
    status: todo
    note: ""
