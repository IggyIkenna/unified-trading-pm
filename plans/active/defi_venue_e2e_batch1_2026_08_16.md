...
- [ ] [BACKEND] P0. **Steps 1-5 per unit — declaration through features**, across defi's 200 (venue, data_type)
      ...
      the P1 item below.
- [ ] [BACKEND] P0. **Steps 6-8 per unit — strategy and execution**, across the same 200 rows. A position adapter
      resolves in batch, live AND paper; the venue is declared in the archetype/slot catalogues that can legitimately
      trade it; an execution adaptor handles every `InstructionActionV2` those archetypes emit. Re-run
      `generate_venue_work_list.py` first — rows with `archetype_consumers=NONE` cannot pass this step until the
      archetype-declaration backlog covers them (§ STRATEGY CONSUMABILITY, umbrella plan); record those as
      `BLOCKED-ON:archetype-declaration-backlog`, never silently skip. Done-when: same per-row verdict discipline.
- [ ] [BACKEND] P0. **Step 9 per unit — transfers**, across the same 200 rows. Every applicable `BusTransferType`
