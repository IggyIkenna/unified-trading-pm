...
- [ ] [DATA] P0. **Promote the shared primitive `canonicalize_raw_tradfi_id(raw, venue, instrument_type)` into UAC**
      (canonical_id_builder neighborhood) — re-derives type via `classify_databento_symbol` + builds via
      `build_instrument_id(margin_marker="LIN", quote_asset="USD")` with the 4 body-normalizations; returns a TYPED
      result (`OK`/`QUARANTINE_*`/`ALREADY_CANONICAL`/`NULL_OR_EMPTY`), never a silent fallback; **venue comes from the
      row column, NEVER the id string / never default-CME** (bare `BRN_MK0023!` must not mint `CME:...`). So
      IS+MTDS+migration+gate share ONE implementation and can't drift. (repos: unified-api-contracts)
