...
- [x] 23. ✅ [BACKEND] [UI] P2. **Task Token Usage role-group filter — planning / scheduled / cicd / conflict_resolver /
      ...
      `data_engineering` groups as "planning" per operator ruling (same todo-20 ruling cited above, this doc
      `deepseek_flash_ab_routing_test_2026_08_05.md` — matches its real backlog dispatch mechanism, not its role file's
      `scheduled` label). New `role_group` query param on `GET /api/backlog/usage/windows` + second filter row in
      `TaskUsageWindows.tsx`. New backend pytest + Playwright spec (`task-usage-role-group-filter.spec.ts`). Fixed a
      real pre-existing Playwright locator bug found along the way (`.panel, {hasText}` ambiguity vs the Accounts
      panel's own cross-reference hint text); filed a separate, deeper pre-existing e2e-fixture issue as a follow-up
      rather than fixing it here (see Progress Log). QG green (2499 backend / 225 frontend). Landed
...
| Todo 12 — decide whether review-agent findings become a structured event                                                                               | **RESOLVED 2026-08-08** (ao round-5 operator Q&A) — "Yes, build it"; spun off as new todo 12a `[BACKEND] P2`, not yet built                                                                                        | Todo 12a                                                          |
...
| Todo 25 — historical backfill for one-off completions lost while todo 24's bug 1 was live                                                              | **No longer operator-gated (2026-08-08)** — investigated live: `backfill_task_usage.py` has zero candidate path for one-offs; retagged `[OPERATOR]`→`[BACKEND]` with a concrete extension spec, now build+run work | Nothing — pick up directly (build the extension, then run)        |
