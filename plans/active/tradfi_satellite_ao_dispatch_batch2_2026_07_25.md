...
- [x] ✅ [TEST] P1. **NICE-TO-HAVE — fix `deployment-service/tests/unit/test_event_logging.py`'s `get_service_name()`
      ...
      `tradfi_multisource_backfill_2026_06_22.md`. — deployment-service@077a063: `get_service_name()` now reads
      `pyproject.toml`'s `project.name` (falling back to `Path.cwd().name` only if the file is absent); verified live by
      copying the checkout to a differently-named directory and confirming the skip still fires there while continuing
      to pass in the canonical checkout; `quality-gates.sh` green.
