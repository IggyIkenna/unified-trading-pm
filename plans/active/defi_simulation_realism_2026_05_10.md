...
- [ ] [AGENT] P0. **3C — Validation harness**. Replay 1 month of historical Aave V3 large supplies (>$10M); compare
  ...
  - [ ] **3C.1 — Event Collector** (RPC `eth_getLogs` batching). Query mainnet for Aave V3 Pool `Supply` events Sep 2025
  ...
  - [ ] **3C.2 — Rate Fetcher** (RPC contract state reads). For each event, RPC call Aave V3 Pool contract
  ...
  - [ ] **3C.3 — Validator** (harness logic). For each ≥50 event: call `LendingRateImpactCalculator.post_trade_rate`
