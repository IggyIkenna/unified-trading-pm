...
- [ ] [AGENT] P0. **AAVE Lending** (simplest — good starting point for demo)
...
- [ ] [AGENT] P0. **Basis Trade** (multi-venue, CeFi + DeFi)
...
- [ ] [AGENT] P0. **Recursive Staked Basis** (most complex, demo highlight)
...
- [ ] [AGENT] P1. **Staked Basis** (simpler than recursive)
- [ ] [AGENT] P1. **Ethena Benchmark** (simplest — deploy and hold)
- [ ] [AGENT] P1. **LP Market Making** (Uniswap V3 — ADD_LIQUIDITY, fee tracking, IL)
...
- [ ] [AGENT] P0. Audit unified-trading-system-ui for existing DeFi trading pages
...
- [ ] [AGENT] P0. Map frontend requirements from e2e workflows:
...
- [ ] [AGENT] P0. Add `expected_output` field to StrategyInstruction:
...
- [ ] [AGENT] P0. Compute `instant_pnl` in execution-service fill:
...
- [ ] [AGENT] P0. Add `instant_pnl` to FILLS_SCHEMA in execution-service

## Phase 2B: Trade History with Running Totals

- [ ] [AGENT] P0. Trade history endpoint in position-balance-monitor or API gateway:
  - Reads fills from GCS, computes running totals
  - Columns: timestamp, type, instrument, amount, price, gas, slippage, running_pnl
  - Filterable by strategy, date range, instruction type

## Phase 2C: Live Treasury Observation

- [ ] [AGENT] P0. Treasury balance endpoint:
  - Reads from custody provider (Copper API or on-chain RPC)
  - Returns per-token balance per chain
  - Includes treasury vs trading wallet split

## Phase 2D: Risk Dimensions

Per-strategy risk computation and display. Frontend first (mock), backend after.

- [ ] [AGENT] P0. Define risk dimension types in `lib/types/defi.ts`:
...
- [ ] [AGENT] P0. Delta neutrality composite:
...
- [ ] [AGENT] P0. Liquidation cost estimation:
...
- [ ] [AGENT] P1. Backend: risk-and-exposure-service computes from live positions
...
- [ ] [AGENT] P0. Create `unified_api_contracts/internal/domain/defi/rate_model.py`:
...
- [ ] [AGENT] P0. Export from `internal/domain/defi/__init__.py`
- [ ] [AGENT] P0. Add `DEFI_RATE_DEVIATION` to `DefiAlertType` enum in UAC

### 2E-2: Rate Impact Feature Calculator

- [ ] [AGENT] P0. Create `features_onchain_service/app/calculators/aave_rate_impact_calculator.py`:
...
- [ ] [AGENT] P0. Add `check_rate_deviation()` to `alerting_service/rules/defi_rules.py`:
...
- [ ] [AGENT] P0. Wire into event router (`_ALERT_TYPE_TO_EVENT` map)

### 2E-4: Strategy Uses Projected Rates

- [ ] [AGENT] P0. Update `AAVELendingStrategy.generate_signal()` in strategy-service:
...
- [ ] [AGENT] P0. Update recursive/staked strategies similarly:
...
- [ ] [AGENT] P0. In batch pipeline (pnl-attribution or batch_handler):
...
- [ ] [AGENT] P0. Add strategy IDs to DeFi types (AAVE_LENDING, BASIS_TRADE, etc.)
- [ ] [AGENT] P0. Align instruction types with backend OperationType enum
- [ ] [AGENT] P0. Add algo-per-instruction dropdown (instruction type → algo selector)
  - SWAP → SOR_DEX / SOR_TWAP
  - LEND → BENCHMARK_FILL
  - TRADE → DIRECT_MARKET
  - TRANSFER → DIRECT
  - FLASH_BORROW → FLASH_LOAN_MORPHO / FLASH_LOAN_AAVE
- [ ] [AGENT] P0. Align venue names to canonical IDs (UNISWAPV3-ETHEREUM not "Uniswap")
- [ ] [AGENT] P0. Align instrument IDs to canonical format
- [ ] [AGENT] P0. Add `max_slippage_bps`, `expected_output`, `benchmark_price` to instruction params
- [ ] [AGENT] P0. Add `instant_pnl` decomposition to order/fill types (gross, slippage, gas, fees, net)

### 3B: Risk Visualisation (in existing tabs)

- [ ] [AGENT] P0. **Positions tab**: net delta per underlying, per strategy
- [ ] [AGENT] P0. **Risk tab**: delta composite heatmap, liquidation cost %, risk dimension breakdown
- [ ] [AGENT] P0. **Overview tab**: total portfolio delta exposure KPI
- [ ] [AGENT] P1. Per-strategy drill-down: what it's neutral to, current deviation

### 3C: Trading Functionality

- [ ] [AGENT] P0. Instant P&L display per instruction (alpha decomposition)
- [ ] [AGENT] P0. Trade history table with running totals
- [ ] [AGENT] P0. Position dashboard (collateral, debt, HF, LTV, APY, delta)
- [ ] [AGENT] P0. Treasury balance per chain per token

### 3D: Rebalance Flow

- [ ] [AGENT] P0. Rebalance button on DeFi wallet summary widget (or positions tab):
...
- [ ] [AGENT] P0. Rebalance preview dialog:
...
- [ ] [AGENT] P0. Extend reconciliation to DeFi protocols:
...
- [ ] [AGENT] P1. Reconciliation resolution workflow:
  - Accept: mark system as correct, adjust exchange record
  - Reject: mark exchange as correct, adjust system record
