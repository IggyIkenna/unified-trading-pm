...
- **na-eligibility-audit 2026-08-17 (cross-cutting tranche, dispatch agt-3931fd)** [body-hash:623b4887e36857be]: KEEP-NA, valid — P0 provenance enforcement (split from M-1). All 12 open todos are either prod corpus backfills explicitly conditioned on cross-plan single-walk fold decisions (defi/cefi/sports manifest-canonicalisation C-source riders — 'fold in if open, else run direct') or P0 rollup/summary items restating that same underlying work, plus one genuine design decision (manifest_consolidator dedup-key change, fleet-wide cardinality impact, 'do NOT change unilaterally'). This is cross-plan coordination judgment, not worker-alone-determinable. Reaffirms 2 prior audit rounds (2026-07-30, 2026-08-08) with unchanged reasoning on a fresh full read; no item read as cleanly bounded/deterministic in isolation.
