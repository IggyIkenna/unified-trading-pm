...
| 4   | Residual ~216 ambiguous CeFi wire-keys (1,018 total − 802 DERIBIT now fixed = 216 across OKX-FUTURES/BYBIT/BITGET-FUTURES/OKX-SWAP/BINANCE-DELIVERY/BINANCE-FUTURES/KRAKEN-FUTURES) — the shipped dedup fix explicitly does NOT cover these (different wire-symbol date encodings the parser doesn't recognize, confirmed in the implement pass) | Not done           | Real work, unscoped — needs its own root-cause investigation per venue before a fix can be designed                                                                                                                             |
