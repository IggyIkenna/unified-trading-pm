...
- **CeFi OVERALL: NOT done.** Seven separate open tracks remain (Tracks 2–8, excluding resolved Track 1b) — corrected
  2026-07-25 to account for Track 7 (candle-namespace, folded 2026-07-23) and the former duplicate "Track 6" renumbered
  to Track 8. None blocks the id-migration; several are real data-correctness work; one (Track 2) is a decision that
  reframes "cefi done."
...
## Track 1b — 🟢 RESOLVED 2026-07-25: CeFi raw-tick capture outage (was HALTED 2026-07-21..24, OOM crash-loop) · P0

- [x] ✅ [INFRA] P0. **CeFi's batch-download Cloud Run Job (`uts-prod-market-tick-data-service-cefi-t1-recon`) was
      crash-looping on signal 9 (SIGKILL/OOM) within 10-40s of every execution since ≥2026-07-23, before any venue
      download began — 3 straight days of ZERO manifest writes.** Fixed by bumping the job's memory 8Gi→16Gi
      (`gcloud run jobs update ... --memory=16Gi`) and manually triggering a verification execution rather than waiting
      for the next cron. **Confirmed recovered**: the test execution ran the full 10m2s and succeeded (vs. dying in
      10-40s before), the service's own resource profiler logged `peak_rss=8646.5MB` (measured proof the old 8Gi limit
      was the proximate cause — it doesn't fit 8Gi, it does fit 16Gi), and real per-symbol Tardis shards (20K+ rows
      each) were uploaded to the canonical GCS path + registered in fresh per-VM manifest shards. Root-cause ATTRIBUTION
      (which code path drives the ~8.6GB peak — leading hypothesis: `market-tick-data-service@a6e974b6`'s HyperLiquid
      parse-once-per-day cache) and a permanent code-level bound remain open follow-ups, tracked in the issue doc, not
      blockers — capture itself is flowing again as of 2026-07-25T01:44Z. Full evidence + follow-up todos:
      `/plans/active/issues/cefi_batch_download_oom_crashloop_capture_halt_2026_07_24.md`. (repos:
...
- **Also here (P0 data-correctness bugs found in the audit, filed in their own docs):**
  - `issues/deribit_options_chain_af_g4_blocker_2026_07_03.md` — **⚠️ its "structurally-absent channel" premise was
    DEBUNKED by the "CORRECTION (operator 2026-07-18)" below (§ "Gate Tardis cefi on the vendor response"): the
    futures_chain 122,585 `attempted_failed` are NOT source-absence / a writer-gate defect — the failure + aggregation
    are ON OUR SIDE (per-symbol capture gap → bundle never built). This is NOT a writer-gate fix; it FILLS on the
    Track-2 coverage backfill. Do not propagate the debunked premise; do not treat this as an open Track-4 fix.**
...
- **Close-out criterion**: the impossible-combo bug fixed; denominator gaps resolved or accepted. (The
  `deribit_options_chain_af_g4_blocker` writer-gate premise is dropped per the 2026-07-18 correction above — its real
  fix is tracked under Track-2, not here.)
...
## Track 8 — Non-canonical ENUMERATION audit → the migration must align EVERY form to one SSOT (operator ask, 2026-07-18) · P0

> **Renumbered from a duplicate "Track 6" heading (P2 finding, corrected 2026-07-25)** — this section and the
> "Independent cefi data-correctness / hygiene items" section above were both numbered Track 6, creating ambiguity for
> cold readers and cross-doc citations (e.g. `data_pipeline_e2e_milestones_gate_2026_07_24.md` §2's "cefi Track 6"
> citation resolves to THIS section's terminal-checkpoint todo, not the hygiene-items one). Track 7 already exists below
> (candle-namespace, folded 2026-07-23), so this section becomes Track 8 to avoid a second collision.
...
- **Coverage gap (why the `--apply` must WAIT for these)**: Scripts 1–4 resolve ~380k bare-wire; the other **~1.48M**
  ...
  current `--apply` would relabel ~380k and leave ~1.48M non-canonical — "canonical" would be a lie. **Track 8 gates the
...
- [ ] [FEATURE] P1. **Re-add the "data status" enumeration to deployment-ui/api** — the distinct instrument_type /
      ...
      **INVESTIGATED 2026-07-18 (slot-3) — no single removal commit exists; the capability eroded across several
          ...
          literally lost the raw-value signal. (2) The NEWER hierarchical drilldown (`data_status_hierarchical.py`) picked
          up a same-day (2026-07-18 08:14, `deployment-api@512180b`) DISPLAY canonicalisation that MERGES
          ...
          venue/instrument_type/data_type values, but reads the per-instrument IDENTITY catalogue (`prod/catalog.parquet`)
          for cefi/defi/tradfi, NOT the raw manifest, and has NO `chain` axis at all — so it only partially covers the
          ask. **Restoration shipped as a NEW, dedicated, read-only endpoint** (the operator's own suggested shape) rather
          than un-doing 512180b's legitimate math fix or bolting onto the filter-dropdown endpoint: `GET
          /api/data-status/axis-value-census` (`deployment_api/routes/data_status/_axis_census.py`) reads
          ...
          mirrors `CatalogueExplorer`'s scope decision) flags raw `instrument_type` values that fold to the same canonical
          label via the existing `canonicalInstrumentTypeLabel` alias map (reuses P4-A's table; other axes list raw values
          unflagged — no registry exists to safely fold venue/chain without false-positiving two genuinely different
          venues together). **Shipped: deployment-ui@3fb6779** (full `[UI]` gate green — tsc/eslint/vitest 1007
          passed/build; `pw:L2 ✓` `tests/e2e/data-status-axis-value-census.spec.ts`). **deployment-api: code complete,
          tests green, full `quality-gates.sh` PASSED** (`.qg_last_passed_sha` written at HEAD `e765660`) — includes a
          real, unrelated pre-existing-bug fix found+fixed while chasing a false-positive test failure:
          ...
          the dirty-deps carve-out without its author's context). **Next step once those clear (no code change needed):**
          `cd deployment-api && bash scripts/quickmerge.sh "feat(data-status): restore raw manifest axis-value census —
          non-canonical-naming / duplication detector (Track-8)" --agent --files
          'deployment_api/routes/data_status/__init__.py deployment_api/routes/data_status/_axis_census.py
          tests/unit/test_route_data_status_axis_census.py deployment_api/services/data_status/manifest.py'` (working tree
          already has all 4 files + the green sentinel; re-verify sentinel still matches HEAD before re-running).
...
> lowercase** (row 1: `PERPETUAL`/`SPOT_PAIR`/`FUTURE`/`OPTION` → "lowercase", 7.58M rows) — **directly contradicting
> this same file's own "✅ CASING FREEZE LIFTED 2026-07-20 (operator ruling D1 — UPPERCASE column ratified)" note
> above** (`instruments-service@555ddf1c`), which already shipped these 7.58M rows UPPERCASE and ratified that as the
> target. The row count is unchanged and preserved below; **the direction is corrected — these rows are already
