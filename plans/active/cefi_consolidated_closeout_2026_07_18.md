...
## Track 1b — 🔴 LIVE PRODUCTION OUTAGE: CeFi raw-tick capture HALTED since ~2026-07-21, OOM crash-loop since ~2026-07-23 · P0

- [ ] [INFRA] P0. **CeFi's batch-download Cloud Run Job (`uts-prod-market-tick-data-service-cefi-t1-recon`, 4 CPU/8Gi)
      is crash-looping on signal 9 (SIGKILL/OOM) within 10-40s of every execution since at least 2026-07-23, before any
      venue download begins — confirmed via direct `gcloud logging read`. Manifest writes have been ZERO (not just
      failing — no attempt of any kind) for 3 straight days (07-22/23/24), collapsing from a steady ~1,000-1,200
      captured/day baseline; corpus-wide `attempted_at` max is ~23h stale. NOT documented in any currently-active cefi
      migration issue doc — this is a genuinely new, undiscovered incident, not a known/tracked one.** Investigate root
      cause (what changed — recent deploy, dependency bump, data-volume growth) and restore capture; a Cloud Run
      memory-limit bump may be a safe immediate mitigation while root cause is found, but confirm it isn't masking a
      real leak first. Full evidence:
