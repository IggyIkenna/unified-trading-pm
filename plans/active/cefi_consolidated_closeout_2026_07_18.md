...
| 4b  | OKX-FUTURES (70) + OKX-SWAP (5) margin_type wire-key collision reclassification — evidence suggests the shipped dedup docstring's "REAL ambiguity, stays excluded" classification is actually a stale pre-2026-07-09 mislabel artifact; BITGET-FUTURES's 18 is a separate-but-analogous already-fixed-bug artifact                                                                                                                                                                                                                            | Operator-owned                | Explicit sign-off required per `plans/active/issues/cefi_okx_margin_type_wire_key_ambiguity_reclassification_2026_07_22.md` — margin_type is a substantive economic field, a wrong call would silently merge two real distinct instruments; do not implement without it |
...
| 7   | DERIBIT combo PARTITION-MOVE (15,119 rows, the actual data migration — distinct from the WRITER classification fix already shipped at `2ddc6d4a`)                                                                                                                                                                                                                                                                                                                                                                                             | Operator-owned                | Explicit operator sign-off required per `deribit_combo_perpetual_partition_move_2026_07_21.md` §7 (widened scope, live-fleet sequencing, code-fix-first ordering) — do not start without it                                                                             |
