...
## Track 7 — Candle namespace bundle-collision residual (`processed_candles/`, todo 19 — SEPARATE from Track 1's

## raw-tick instrument-id migration: different bucket prefix, different script, different defect class) · P2
...
| --- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| 1   | ~~KRAKEN-SPOT dry-run v4 finish clean~~ — **DONE** (157,035 planned renames, 0 unhandled collisions). `--apply` (user-confirmed) hit 2 transient-network near-misses, hardened (`market-tick-data-service@b6e2ce1d`), attempt 3 in flight with the fix                                                                                                                                                                                                                                                                                        | Cannot be done yet                                       | Watching apply attempt 3 (`stamp=20260723T0612Z`) for the `SCRIPT 2 FILENAME RENAME SUMMARY (APPLIED)` marker with 0 unhandled collisions, or a further failure                                                                      |
| 2   | Fleet agent (`ae18c5ef1b16bc8e8`) continue its self-drive chain: error-recon (348 would_patch errors) → re-run `verify_cefi_canonical_4surface_2026_07_20.py` → LATE colliding-venue renames (serialized) → MID window → colon_wire (1,697) → loop-until-dry (2 clean passes)                                                                                                                                                                                                                                                                 | Cannot be done yet                                       | Gated on item 1 (KRAKEN-SPOT) completing — this is the next link in the same chain                                                                                                                                                   |
| 3   | Surface C: land the v2 manifest dedup apply for the still-duplicate rows (confirmed on both verifier probe instruments)                                                                                                                                                                                                                                                                                                                                                                                                                       | Not done                                                 | Needs the consolidator drain (`gcloud scheduler jobs pause uts-prod-manifest-consolidator-market-data-cefi-cron`) coordinated first — real work, pick up once items 1-2 give a fresh post-KRAKEN-SPOT baseline to apply against      |
| 4   | ~~Residual ~216 ambiguous CeFi wire-keys~~ — **120/216 SHIPPED**: `instruments-service@bf5322bb9` (BINANCE-DELIVERY 4/4, BINANCE-FUTURES 2/2, KRAKEN-FUTURES 2/2, OKX-FUTURES sub-pattern-A 76/146 — numeric-wire-date off-by-one shape) + `instruments-service@39e26bfe` (BYBIT FUTURE base-asset-parsing 36/36). **96 remain**: BITGET-FUTURES 18 + OKX-FUTURES sub-pattern-B 70 + OKX-SWAP 5 (margin_type mislabel questions — item 4b, operator-owned) + BYBIT's 3 PERPETUAL keys (correctly-excluded, not a bug — will never be "fixed") | Operator-owned (item 4b only)                            | Item 4b is gated on operator review; nothing else outstanding in item 4 itself                                                                                                                                                       |
| 4b  | ~~OKX-FUTURES (70) + OKX-SWAP (5) margin_type wire-key collision reclassification~~ — **RULED 2026-07-23**: operator said build the fix (treat as stale artifact, delete bad formats, migrate to canonical); BITGET-FUTURES's 18 folded into the same fix (same root-cause class, already high-confidence). See `cefi_okx_margin_type_wire_key_ambiguity_reclassification_2026_07_22.md`'s OPERATOR RULING section                                                                                                                            | Not done (dispatched)                                    | No longer operator-gated — implementation dispatched as a workflow; land it, no further sign-off needed                                                                                                                              |
| 5   | Build the missing catalogue-enumeration-gap measurement script (~422 figure has no re-measurable SSOT)                                                                                                                                                                                                                                                                                                                                                                                                                                        | Not done                                                 | No script exists at all yet — greenfield                                                                                                                                                                                             |
| 6   | LIGHTER-ZKSYNC market-index → symbol map (~11,283 objects, 93.5% bare numeric stems)                                                                                                                                                                                                                                                                                                                                                                                                                                                          | Not done                                                 | Greenfield, zero code confirmed                                                                                                                                                                                                      |
| 7   | DERIBIT combo PARTITION-MOVE (15,119 rows, the actual data migration — distinct from the WRITER classification fix already shipped at `2ddc6d4a`). **RULED 2026-07-23**: operator approved the P1 PREP work now (cross-check + widen/port the combo-shape guard into `tardis_cefi_shards.py`, code-only, dispatched as a workflow) — the actual `--apply` data move stays gated, separate future review required, this ruling does not cover it                                                                                               | Prep: not done (dispatched) / Move: still operator-owned | Prep work no longer gated, dispatched; the `--apply` partition-move itself still needs explicit operator sign-off per `deribit_combo_perpetual_partition_move_2026_07_21.md` §7 — do not start that without a SEPARATE future review |
