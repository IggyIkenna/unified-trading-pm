...
> **Machine-gated on `data_pipeline_check_mdps_features_2026_07_20.md`** (`depends_on` + `gate_on_depends: true`) — the
> dispatcher will not queue any todo below until that plan's todos are done (or on explicit operator direction to start
> reconciling early).
