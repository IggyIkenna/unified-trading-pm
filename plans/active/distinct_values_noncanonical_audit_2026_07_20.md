...
- [ ] [OPERATOR] P1. **DeFi honest-coverage denominator exclusion — operator ruling received 2026-07-22 ("make it both
      Definition #2 and Definition #1 to be live for safety" / OR-semantics), implementation attempted, BLOCKED by
      adversarial verify — still not shipped, still needs operator input.** 11 venues with claimed real capture
      (ANKR/FRAX/MAKER/STADER/STAKEWISE/SWELL/MANTLE/ACROSS/STARGATE/FLASHBOTS/ALCHEMY) are structurally excluded from
      the DeFi `completeness_pct` denominator because `DEFI_VENUE_PHASE` labels them `"pipeline"` and
      `VENUES_BY_ASSET_GROUP['defi']` filters to `phase=="live"` only. An additive (non-flipping) implementation was
      built per the operator's OR-ruling, but the adversarial verifier pulled the live prod GCS manifest and found the
      underlying data claim is false today: 5 of the 11 (FRAX/ALCHEMY/FLASHBOTS/ACROSS/STARGATE) have **zero** captured
      rows; 5 (ANKR/STADER/STAKEWISE/SWELL/MANTLE) have exactly **one** synthetic sample row each dated 2026-07-20 (the
      previously-cited 1,078/1,162/937/990/2,000-row historical figures are NOT present in the live manifest); MAKER has
      real data but is unregistered in the manifest (orphan capture). NOT-SAFE verdict — nothing shipped,
      `DEFI_VENUE_PHASE` untouched, `completeness_pct` unchanged (`n_expected=109`, `n_present=3`,
      `completeness_pct=2.75` before AND after). Full blocker + required next step (correct membership to the ≤6
      GCS-verifiable venues, or get an explicit operator re-confirmation given the false historical figures) in
      `plans/active/issues/defi_venue_phase_live_definition_contradiction_2026_07_22.md` § "BLOCKED 2026-07-22".
...
| 🟡 **DeFi honest-coverage denominator exclusion** (11 claimed-capture venues silently excluded from `completeness_pct`)                                               | **OPERATOR-OWNED — ruling received 2026-07-22 (OR-semantics), implementation attempted, BLOCKED by adversarial verify, NOT shipped.** Additive (non-flipping) implementation was built per the ruling; adversarial verifier pulled the live prod GCS manifest directly and found the underlying "real, months-long capture" claim is false for the venues today: 5 of 11 (FRAX/ALCHEMY/FLASHBOTS/ACROSS/STARGATE) have zero captured rows anywhere; 5 (ANKR/STADER/STAKEWISE/SWELL/MANTLE) have exactly one synthetic sample row each dated 2026-07-20, not the previously-cited 1,078/1,162/937/990/2,000-row historical figures; MAKER has real data but is unregistered in the manifest (separate orphan-capture bug). NOT-SAFE verdict — `DEFI_VENUE_PHASE` untouched, nothing shipped, `completeness_pct` unchanged both before and after (`n_expected=109`, `n_present=3`, `completeness_pct=2.75`). Full trace + required next step: `plans/active/issues/defi_venue_phase_live_definition_contradiction_2026_07_22.md` § "BLOCKED 2026-07-22".                                                                                                                                                                                                                                                                                                                                                                                                                                                                       | Enshrining a checkably-false data claim into a permanent SSOT constant, locked in by a membership-only regression test that can never catch the falsehood — would mislead whoever wires it into a real metric later. | Operator re-reads the issue doc's BLOCKED section and either (a) approves narrowing `DEFI_VENUE_MTDS_CAPTURED` to the ≤6 GCS-verifiable venues (single-sample-point, not months-long) and dropping the other 5 entirely, or (b) explicitly re-confirms the OR-ruling now that the 1,078/1,162/937/990/2,000-row figures are known not to exist in production. |
