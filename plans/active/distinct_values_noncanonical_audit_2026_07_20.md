...
| 🟡 **DeFi honest-coverage denominator exclusion** (11 real-capture venues silently excluded from `completeness_pct`)                                                  | **OPERATOR-OWNED — confirmed real bug, NOT fixed.** Code-traced + adversarially verified (`wf_7a8796e2-7a6`): `VENUES_BY_ASSET_GROUP['defi']` filters to `DEFI_VENUE_PHASE=="live"` only (`market_data_categories.py:395`), so ANKR/FRAX/MAKER/STADER/STAKEWISE/SWELL/MANTLE/ACROSS/STARGATE/FLASHBOTS/ALCHEMY's real captured data never enters the `completeness_pct` ratio (`check_enumeration_completeness.py:512`). Two fix paths exist, both change a production metric — needs an operator pick, not a guess. Full trace + both options: `plans/active/issues/defi_venue_phase_live_definition_contradiction_2026_07_22.md`.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          | Fixing either way changes a real number operators see; do not silently pick a side.       | Operator reads the issue doc, picks option (1) flip 11 venues to `"live"` + re-measure the `completeness_pct` delta, or (2) leave `"pipeline"` + fix the misleading 2026-05-07 comment + decide on a separate data-availability metric. |
