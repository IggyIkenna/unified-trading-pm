...
| ⏳ **RESTAKING availability-index re-stamp** (IS-side `_index/availability_index.parquet`, 36 rows)                                                                   | **SCRIPT READY + DRY-RUN VERIFIED, NOT APPLIED.** `instruments-service/scripts/restamp_restaking_lrt_availability_index_2026_07_22.py` — measured 36 rows (ETHERFI 16/RENZO 10/KELPDAO 5/PUFFER 5). This bucket IS one of the `*/1` `uts-prod-manifest-consolidator-instruments-defi-cron` targets (same class as the venue-as-chain fix's `market-data-cefi` target) — per the mandatory-rules note this session did not pause the cron itself.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             | BLOCKED-OPERATOR — needs an authorized paused-writer window (mirrors this plan's 2026-07-22 venue-as-chain precedent exactly, same credential path already proven). | Pause `uts-prod-manifest-consolidator-instruments-defi-cron` → `GCP_PROJECT_ID=central-element-323112 .venv/bin/python scripts/restamp_restaking_lrt_availability_index_2026_07_22.py --apply` → verify 36 rows flipped, 118,944 total unchanged, 0 dup row_keys → resume cron. File is only 3.3MB — should land on attempt 1. |
| 🔴 **instruments-service RESTAKING code ship** (4 adapters + 4 tests + 2 scripts + sports-golden resync, 12 files)                                                    | **BLOCKED — not shipped.** All files individually verified green; blocked by `instruments-service@a9be6ce9` (unrelated concurrent commit) introducing 4 new codex-compliance violations (ceiling 3) in `tests/unit/test_smoke_matrix.py` + `engine/orchestrator/writers.py::_write_venue()` (211L > 200L limit) — confirmed pre-existing to that commit via a clean full-gate run at the prior HEAD `f33c2ec0`. `quickmerge.sh` needs a whole-tree-passing sentinel regardless of `--files` scope.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           | Blocked on `a9be6ce9`'s owner fixing their own regression (or an operator-authorized bypass) — not this session's files.                                            | `cd instruments-service && bash scripts/quality-gates.sh --no-fix` (should go green once `a9be6ce9`'s 2 violations are fixed) → `bash scripts/quickmerge.sh "<msg>" --agent --files '<the 12 files listed in the 2026-07-22 Progress Log entry>'`.                                                                             |
