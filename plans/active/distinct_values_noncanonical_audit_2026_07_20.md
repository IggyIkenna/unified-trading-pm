...
| 🟡 **Paired manifest re-stamp** for the above (operator-approved "one pass")                                                                                          | **Option (b) chosen by operator + SHIPPED: `market-tick-data-service@568f1404`.** Root cause of the 25/25 exhaustion profiled precisely: `classify()` itself cost 321.5s of a 392.4s per-attempt cycle (82%) — full-corpus `composite_key()`/`set()`/`drop_duplicates` work whose true search space is only the ~1.5M rows sharing venue with the 4 affected venues, not the full ~10.5M-row corpus. Narrowed to the venue-in-TARGETS candidate subset (lossless — a collision requires a shared venue) + switched the one genuinely full-corpus check (`n_dup_existing`) to a vectorized hash. Synthetic benchmark measured ~3x cut on the narrowed operations; real per-attempt timing will be confirmed by the new fine-grained timing prints on the next live run. **Extended retry (25 attempts / 3h cap) with the optimized script launched 2026-07-21 ~18:15 UTC** — see Progress Log for the live task id + outcome once it completes. | n/a — fix shipped, retry in progress                                                      | See the 2026-07-21 Progress Log entries below for the profiling breakdown + the live retry's outcome once it lands. |
