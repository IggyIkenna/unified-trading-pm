---
doc_type: plan
title: Data-pipeline alerts batch remediation — closeout & historical narrative (2026-07-15 to 2026-07-16)
summary: >-
  Extracted historical narrative (ground truth, original todo list, Progress Log, and every operator-reconciliation /
  adversarial-verification round) from plans/active/data_pipeline_alerts_batch_remediation_2026_07_15.md, split out
  2026-07-24 for plan line-cap remediation. Holds the full closed record of the 2026-07-14/15 #data-pipeline-alerts
  incident wave — dedup fix, sports/defi consolidator livelock, cefi orphan-row resurrection saga, the EXPECTED_*
...
tags: [data-pipeline, alerting, manifest-consolidator, dedup, autonomous, incident, closeout]
related:
  [
...
source:
  [
    "Split from plans/active/data_pipeline_alerts_batch_remediation_2026_07_15.md during plan line-cap remediation,
    2026-07-24 — see plans/active/issues/plan_line_cap_remediation_2026_07_23.md, row 7 / bucket (c).",
  ]
assigned_role: infra
...
# Data-pipeline alerts batch remediation — closeout & historical narrative (2026-07-15 to 2026-07-16)

> **Extracted 2026-07-24** from `plans/active/data_pipeline_alerts_batch_remediation_2026_07_15.md` as part of the plan
> line-cap remediation (`plans/active/issues/plan_line_cap_remediation_2026_07_23.md`, bucket (c) — clean-partition
> split). This doc holds the FULL historical narrative for that plan's original ~60-alert incident wave: the ground
> truth established before the plan was written, the initial todo list as authored, every Progress Log entry, and every
> operator-reconciliation / adversarial-verification round through the 2026-07-16 close. **Everything below is
> closed/verified history — for the plan's current open work, read the parent plan.**
>
> 4 todos that were interleaved with this narrative were relocated: 2 are now flipped `[x]` with evidence in the
> parent's own Todos section (the cefi orphan-row delete/hardening, and the `EXPECTED_*`-prefixed misclassification fix
> — both were narratively closed later in this very history, just never had their checkbox flipped in the parent until
> this split); 2 remain genuinely open and are tracked solely in the parent (the 24h alert-observation item, and the
> tradfi mbp_10 doc-correction item). Each original position below carries a short relocation pointer instead of the
> checkbox, so this doc reads continuously without duplicating the parent's live Todos section.
...
> **Todo relocated 2026-07-24** (plan line-cap remediation split): the "[REVIEW] P2 — full 24h observation cycle /
> RESOLVED-bookend verification" item that originally sat here remains genuinely open — it is tracked verbatim, in full,
> in the parent plan's own Todos section: `plans/active/data_pipeline_alerts_batch_remediation_2026_07_15.md`.
...
> **Todo relocated 2026-07-24** (plan line-cap remediation split): the "[DATA] P1 — cefi blank-`data_type` phantom-audit
> hardening + 9,757-row orphan delete" item that originally sat here is now RESOLVED (see "Fourth round" below +
> `legacy_seed_captured_outranks_resurrection_risk_2026_07_15.md`'s closing section) — it is flipped `[x]` with full
> evidence in the parent plan's own Todos section: `plans/active/data_pipeline_alerts_batch_remediation_2026_07_15.md`.
...
> **Todo relocated 2026-07-24** (plan line-cap remediation split): the "[DOCS] P2 — tradfi mbp_10 issue-doc correction /
> detector suppression check" item that originally sat here remains genuinely open (never confirmed done — see the
> recurring "mbp_10 / corp_action historical rows" follow-up mentions throughout this doc) — it is tracked verbatim, in
> full, in the parent plan's own Todos section: `plans/active/data_pipeline_alerts_batch_remediation_2026_07_15.md`.
...
> **Todo relocated 2026-07-24** (plan line-cap remediation split): the "[DATA] P0 NEW FINDING — cross-cutting
> `EXPECTED_*`-prefixed / `attempted_failed` misclassification bug" item that originally sat here is now RESOLVED (see
> "Post-reconciliation progress" below + the "🔴 2026-07-15 (later) — independent re-verification pass" section's "NEW
> cross-cutting finding, fixed same session" entry +
> `tradfi_expected_reason_attempted_failed_misclassification_2026_07_15.md`) — it is flipped `[x]` with full evidence in
> the parent plan's own Todos section: `plans/active/data_pipeline_alerts_batch_remediation_2026_07_15.md`.
