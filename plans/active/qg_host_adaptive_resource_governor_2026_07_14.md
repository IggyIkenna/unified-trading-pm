...
### 2026-08-15 — TEN consecutive silent deaths of queued (pre-admission) `quality-gates.sh` runs across two repos —
condensed 2026-08-16 (was 155 lines of session-by-session narrative; see git history on this doc for the full trail)

Across slots 25/29, ~10 independent queued runs (mtds, then unified-api-contracts) vanished with **zero terminal
marker** while still inside an acquire loop's `queued Ns` wait — no exit code, no shell "Killed" line, nothing in
`dmesg`/`journalctl -k`. Positively **ruled out** by direct measurement: kernel OOM-killer, the per-repo cgroup cap
(`MemoryCurrent` well under `MemoryHigh`/`MemoryMax`), and this plan's own post-admission RAM watchdog
(`_qg_watchdog_start` only starts AFTER `ADMIT|SOLO_ADMIT` — every death was pre-admission). One long-running control
(run6, mtds) survived past 1170s with zero deaths and was traced to **genuine legitimate contention** — a 4-5-deep
pileup against mtds's own repo sub-cap of 1, each run taking ~20min, ordinary fleet parallelism, not a bug. The
original ~300s-death signature was never root-caused (a `slot-cron-ff-pull.sh`/`slot-git-status-report.sh` 300s-cadence
coincidence was flagged, not confirmed). Separately, the unified-api-contracts ship in this same window found a REAL,
confirmed bug: repo identity resolving to the literal string `"unknown"` in `_qg_governor_acquire_reservation()`,
defaulting to a worst-case 5500MB RAM request vs a measured real 1117MB — a ~5x overstatement that can fail
`WAIT_RAM_LIVE` under ordinary transient pressure, indistinguishable in the log from genuine contention.

**RESOLVED 2026-08-16 — operator ruling + fix, `unified-trading-pm@<PENDING-SHA>` (this session).** The operator was
asked to choose between this doc's "Option 1" (queue-timeout, converts a silent hang into a loud attributable failure)
and "Option 2" (a dedicated low-resource fast lane for small diffs) — **ruled Option 1, default 60 minutes.** Shipped:
(1) `_qg_governor_check_wait_timeout()` — every acquire loop (token, reservation, total-instance) now gives up after
`QG_GOVERNOR_MAX_WAIT_SECONDS` (default 3600s, chosen with headroom above every measured LEGITIMATE queue depth on
record, the worst being run6's ~1170s) of real queue time: writes a marker file (`timeout.<pid>`, same shape as the
watchdog's), fires a Slack CRITICAL alert, and `exit 75` — self-scoped, only ever terminates the WAITING process
itself, never a peer's. Does **not** root-cause the original ~300s deaths (still unconfirmed) — converts any future
recurrence, of that or any other cause, from a silent hang into an attributable, retryable failure, which is what
"Option 1" was scoped to do. (2) The confirmed `"unknown"`-repo bug fixed at its source: `_qg_governor_acquire_reservation`
and `_qg_watchdog_start` both now fall back to `_qg_repo_name()` (the robust git-remote-derived resolver the
total-instance gate already used successfully) instead of the literal string `"unknown"`. (3) **Manually verified
live**: the fix's own first real-world trigger was a genuinely-stuck `unified-api-contracts` run that had been queued
**9h+** on this exact bug — traced via `qg-host-governor.sh --status` + `lsof` on the real shared ledger slot files
(not guessed), confirmed it was purely a waiter holding nothing, terminated it with a manually-written marker
documenting the root cause, and relaunched clean under the fixed code. Portable by construction — no macOS-specific
calls (`date +%s`/`mkdir -p`/`hostname`, no `sysctl`/`vm_stat`), works identically on Linux hosts and glue-runner CI.
Tests: `tests/test-qg-governor-wait-time.sh` extended with 4 new blocks (no-op below threshold, marker file + exit 75
at threshold, `QG_GOVERNOR_WAIT_TIMEOUT_DISABLE` opt-out, and a real end-to-end flock-contention case proving
...
- [x] [INFRA] P1. ✅ **RESOLVED 2026-08-16** — see the condensed 2026-08-15/16 entry directly above (wait-timeout +
      `"unknown"`-repo fix, both shipped, tested, manually verified against a real 9h+ stuck run).
- [ ] [INFRA] P2. Root cause of the ORIGINAL ~300s mtds death signature (4/4 reproduced 2026-08-15, never explained) is
      still open — now low-priority: the wait-timeout above bounds the SYMPTOM (any future recurrence fails loudly
      within 60min instead of hanging silently), so this is a root-cause curiosity, not a blocker. Next lead if
      revisited: whether `slot-cron-ff-pull.sh`/`slot-git-status-report.sh`'s 300s cadence can reap a peer's
      backgrounded child.
