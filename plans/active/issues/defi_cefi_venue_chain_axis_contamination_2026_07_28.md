...
doc_type: issue
title: >-
  DeFi venues axis carries 15 non-DeFi tokens (9 chain names + 6 CeFi exchange names); defi+cefi chains axis carries a
...
summary: >-
  ...
  values, 15 of which are NOT DeFi-protocol-shaped: 9 are literal chain names (ARBITRUM, AURORA, AVALANCHE, BASE, BSC,
  ETHEREUM, LINEA, OPTIMISM, POLYGON) and 6 are CeFi exchange names (BINANCE, BITFINEX, BITGET, BYBIT, KRAKEN, OKX) —
  tokens that belong on a different axis (chain) or a different asset_group (cefi) entirely, not on the defi venue axis.
  Separately, `defi.chains` and `cefi.chains` both carry a non-canonical `FUTURES` value — cefi has NO chain axis at all
  per this same plan's own RESULT 3 finding (`UAC SHARD_AXIS_MATRIX[(MTDS,cefi)]` has no `chain` axis), and `FUTURES` is
  a tradfi instrument_type spelling, not a chain — suggesting a shared, not-yet-identified cross-axis or
  cross-asset-group write path. This doc catalogues the finding + the two most plausible root-cause classes (wrong-axis
  writer mis-stamp vs. cross-AG manifest-consolidator bleed, the latter matching the shape of the ALREADY-RESOLVED
...
- **1 already-known/tracked** (was 2 — `HYPERLIQUID` wrong, per this doc's 2026-08-04 correction below): `BLAZESTAKE`,
  covered by `/plans/archive/2026_07/defi_venue_phase_live_definition_contradiction_2026_07_22.md` (repointed).
...
- [x] ✅ [DIAG] P1. **ROOT-CAUSED 2026-07-30 — COMPLETED with the exact splitter location (see slot-15 Progress Log
      ...
      **DeFi** bucket. Comparison-checked: the identical `(day, pipeline_mode, asset_group, venue,     instrument_type)`
      ...
      `_KNOWN_DEFI_CHAINS` allowlist guard its sibling
      `market-tick-data-service/market_tick_data_service/scripts/rebuild_defi_manifest.py` already has.** Timestamp
      reconciliation: this entry's `day=2026-05-16..22` is the ORIGINAL CAPTURE date (from the GCS path partition); the
      concurrent trace's `written_at=2026-07-24T20:06:38` is the LATER MANIFEST-REGISTRATION timestamp (from
      `backfill_orphan_class_e.py` sweeping + registering these already-misplaced objects into the manifest, corrupting
      venue/chain in the process) — the two are consistent, not contradictory: same underlying 35 objects, two different
      lifecycle timestamps. Not independently re-verified that these are literally the SAME 35 rows (both traces used
      different query methods) — flagged for whoever executes the fix to spot-check before relying on it.
...
- **interactive session 2026-08-04 (autonomous, operator away 8h, `/autonomous`)** — operator re-raised this exact DEFI
  ...
  **Pattern B (35 rows, the 5 cefi-exchange-shaped venues) — FULLY ROOT-CAUSED, 3-hop chain across 2 repos, confirmed
  ...
  2. **`instruments-service/scripts/migration_orphan_sweep.py:253`**, `shard_key_from_segments()` — when an operator
     ...
     `market-tick-data-service/market_tick_data_service/scripts/rebuild_defi_manifest.py` does the identical split but
     GUARDS it with a `_KNOWN_DEFI_CHAINS` frozenset — `migration_orphan_sweep.py` is missing that guard. Run against
...
## Deferred after 2026-08-08

- **P1 corpus recompute**: gated on `cefi-fwd-20260808-123230` + GCS probe. See P1 todo ~line 300.
- **[OPERATOR] P0 VM deletion root cause**: open in VM issue doc.
