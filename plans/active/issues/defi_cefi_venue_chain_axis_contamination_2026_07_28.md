...
- **slot-9 2026-08-06 — BLOCKED-OPERATOR-DECISION (escalated to dashboard as BLK-af77b2bb; recorded here by the
  ...
  **Recommendation: A. can_continue: false** (the compute would be a no-op for the target venues). Operator/main: answer
  in the dashboard to route next steps.
...
| Raw-capture fix — follow-up `- [ ]` filed in `cefi_tardis_derivative_ticker_historical_gap_2026_08_04.md` (root-cause instrument-store 404 for BINANCE-FUTURES/BYBIT/OKX-SWAP/KRAKEN-FUTURES/BITGET-FUTURES/DERIBIT; re-run backfill 2026-05-23→08-02; verify resumed cron captures them) | Not done — needs a fix worker                                                                                                                                                                   | BLK-af77b2bb operator/main routing; no live writer currently captures these venues                                             |

**Recommended next item**: the raw-capture fix (Option A of BLK-af77b2bb) — it unblocks the corpus recompute; verify the
raw input landed at the reader-exact path (see Progress Log 2026-08-06) BEFORE re-running the compute.
