...
- [ ] [DATA] P2. **2026-08-08.** Root-cause IS data gap for post-06-04 dates + fix forward-cron gap — VM
      `cefi-fwd-20260808-110409` confirmed 0 records/day for 2026-06-05 and 06-06 (~29s/day, no Tardis downloads); IS
      (instruments-service) not returning symbols for FUTURES venues on these dates; forward cron also not capturing
      (BLK-6363d525 addendum). VM stopped; IS backfill needed.

- **slot-14 2026-08-08**: Probe confirmed 2026-06-05→2026-08-08 absent; `cefi-fwd-20260806-065837` terminated after
  12/75 days (archival premature). Backfill VM `cefi-fwd-20260808-110409` launched but confirmed 0 records for
  2026-06-05 + 2026-06-06 (~29s/day, IS data gap for post-06-04 dates). VM stopped. New blocker: IS must cover
  2026-06-05→2026-08-05 for FUTURES venues before MTDS re-run.

## Deferred work after 2026-08-08

- **P1 corpus recompute**: BLOCKED — root cause: IS data gap for post-06-04 FUTURES venues (slot-14: VM confirmed 0
  records for 2026-06-05/06-06, ~29s/day). Audit log (slot-17): `cefi-fwd-20260808-110409` deleted by
  `unified-trading- sa` at 11:14:58Z + 11:17:12Z (10-13 min after double-insert). GCS probe confirms 0 for
  BINANCE-FUTURES/BITGET-FUTURES across full window. Fix: IS backfill for post-06-04 FUTURES → new VM re-run. Issue:
  `/plans/active/issues/cefi_fwd_backfill_vm_deleted_by_sa_within_10min_2026_08_08.md`.
- **Forward-cron + IS fix**: not started; root cause IS gap; new P2 todo above.
