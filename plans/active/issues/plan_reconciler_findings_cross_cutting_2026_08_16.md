...
- **na-eligibility-audit 2026-08-17 (cross-cutting tranche, dispatch agt-3931fd)** [body-hash:f065a91866a8d429]: KEEP-NA, valid — Zero checkbox-tracked todos (grep-confirmed, matches the phase0 annotation), but the doc is explicitly NOT resolved: status: open, and it is a live, mid-flight /plan-reconcile run doc. Its own body carries substantial prose-tracked (not checkbox-tracked) remaining work: 5 of 10 planned hunt batches (batches 5-9, ~57 of 114 workable docs, ~50% by count) were never hunted this run, and a 'Confirmed findings this run did NOT apply' section lists 5 named, substantive unresolved items (a stalled P0 dispatch-gap doc, a nuanced still-evolving root-cause doc, an un-actioned guard-holding-confirmation, the 23+-day infra_ops_residual audit-stall pattern, a stale-prose lock instance). This is the corpus's own documented trap class ('prose-only remaining work with no checkbox at all') — genuine live investigative/judgment work, not archive-eligible and not reducible to bounded checkboxes.
