...
- [x] 3b. [DATA] P2. **Scope the cefi/prediction real-orphan `record_captured` backfill** (cefi's 935,714 + prediction's
      ...
      **Update 2026-07-22 (~21:30 UTC) — VM launcher SHIPPED + cefi validated + apply run launched.** Built
          ...
          **cefi `--dry-run` validated cleanly**: `backfill-orphan-e-cefi-20260722-212420` completed in ~4 minutes on
          ...
          convert_failed=0 verify_failed=0`, `would record_captured 105 cells`, `exit_code=0`. **Real `--apply` run
          launched for cefi** (`backfill-orphan-e-cefi-20260722-213220`) and **a `--dry-run` validation launched for
          prediction** (`backfill-orphan-e-prediction-20260722-213250`, 3,137,183 rows — 3.3x cefi's scale, worth
          confirming separately before its own `--apply`). Both running at write time; watchdog armed.
