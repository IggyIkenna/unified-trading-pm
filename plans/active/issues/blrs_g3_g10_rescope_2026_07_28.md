...
- [ ] [CODE] P3. Build the BLRS resolution-API gateway proxy — `GET /api/reporting/reconciliation/breaks`,
      ...
      (mock-mode + real-mode per the file's existing `mock_mode` split). Repo: unified-trading-api.
- [ ] [CODE] P3. Build the strategy-service/position reconciliation-API gateway proxy — the 6 routes
