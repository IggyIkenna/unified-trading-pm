...
- [x] ✅ [DOC] P1. **RULED 2026-08-09** — option (a) decided + implemented in
      ...
      the live `/api/backlog` — the parked task `-0d5981dddb99` is ABSENT (cleaned up when its source doc's `[INFRA] P1`
      flip was re-derived by PlanRegenLoop; no cooldown row remains, so the `/unpark` call correctly returned 404 "not
      auto-parked"). No unpark needed. The sibling "complete the deferred archival" `[INFRA]` todo is live-queued as
