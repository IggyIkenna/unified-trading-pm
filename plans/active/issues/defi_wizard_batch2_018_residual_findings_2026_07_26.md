...
- [ ] [REGISTRY] P3. Make `spot_venue` a selectable axis for `CARRY_STAKED_BASIS_DATED` too, mirroring the base
      archetype's `_STAKED_BASIS_ETH_SPOT_VENUES`/`_STAKED_BASIS_SOL_SPOT_VENUES` pattern (or explicitly document why
      the DATED variant is intentionally single-venue if there's a reason it wasn't just an oversight). Repo:
      strategy-service (`catalog_staked_basis.py::build_carry_staked_basis_dated`).
