...
- [ ] [INFRA] P2. Rollup-svc coverage write 403 (`storage.objects.create` denied for uts-prd-sa on
      `central-element-323112-data-status-rollups`): the healthy-revision redeploy (`-00395..-00398`) runs as
      `uts-prd-sa` while the prior working `-00388` ran as `unified-trading-sa`; uts-prd-sa has project-level
      `roles/storage.objectAdmin` but the write still 403s (no bucket binding, no deny rule — investigate the IAM
      nuance/conditional binding). Fix: redeploy with the correct SA or resolve the grant. Repo: deployment-api. Done
      when: the live coverage write succeeds.
