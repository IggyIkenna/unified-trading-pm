...
- 2026-07-28 ~23:08-23:26 UTC (cicd, slot-4, `agt-ddd14c`, `ldr_qg_failure` on `instruments-service`, PR #1007): another
  corroboration, protected-6 repo (stays self-hosted, resolve via retrigger). Run `30374297022`: `uv` cache-write race,
  `typecheck exit=124`, `lint-codex` `MAX_DURATION` miss — all substantive checks passed. NOT a code regression: local
  `quality-gates.sh --no-fix` at LDR HEAD `e722bdee` passed clean (238s). `uptime` 55→104 load avg (16 vCPU), swap
  14/15GB. PR #1008's `tests` job sat `in_progress` 51+ min zero progress (starves-under-saturation mode). Did NOT
  revert `self_hosted_runner_labels`; canceled the stuck job, triggered a fresh dispatch (`30407834160`), closed out
  without holding the slot. No repo-blocker; allowlist/VM untouched.
