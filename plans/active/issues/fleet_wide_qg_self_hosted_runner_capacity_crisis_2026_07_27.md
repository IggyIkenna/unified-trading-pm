...
> **2026-08-14 line-cap remediation**: every 2026-08-02 corroboration entry extracted verbatim to
> `/plans/archive/2026_08/fleet_wide_qg_self_hosted_runner_capacity_crisis_progress_log_history_2026_08_14.md` (doc was
> at 998/1000 lines). New entries append below this note going forward (2026-08-03 entries onward were already below the
> extraction point and are unaffected). **2026-08-03 ~10:53 UTC corroboration + automation-gap finding
> (instruments-service, slot-11, data_engineering craft, via
> `instrument_availability_hive_migration_unrecognized_shapes_and_content_mismatch_2026_08_03.md` todo 12)**: same
> signature, `quality-gates-v2` run `30800087100` (`workflow_dispatch`, started 09:07:46Z) — `content sentinel`
> completed 09:12:01Z but both QG-slice jobs (`tests`, `checks`) sat `queued` since 09:12:02Z, confirmed unchanged
> across 4 consecutive checks (todos 9-12 of that plan, spanning ~1h45m) against the repo's one registered `glue` runner
...
> of three signatures — `v2_present==False` (never fired at all), v2 concluded `action_required`, or a stale
> failed-check — none of which describes this run (v2 DID fire; the stall is GitHub's own job scheduler leaving 2 jobs
> `queued` against a runner that reports itself alive/busy, not a PR-merge-state condition at all).
> `glue-pool-starvation-monitor` (`scripts/cicd/glue_pool_starvation_monitor.py`) is scoped
> `--repo IggyIkenna/unified-trading-pm` by default (`find_starved_glue_jobs()`, its own docstring) and never queries
> `instruments-service`; even in-scope, its own starvation rule (lines 111-127) treats "queued behind a busy runner" as
> normal backlog, not starvation, so it would not fire on this pattern regardless of repo. No runbook covers "restart a
> live/busy single self-hosted runner to unstick a GitHub-side queued job"
> (`central-vm-relaunch-glue-runner-reinstall.md` only covers post-VM-relaunch reinstall). Added a Follow-up todo below
> for this gap rather than filing a duplicate P1 doc (same root incident/repo/runner this doc already tracks at P0). No
> code/workflow change made; slot did not intervene on the stuck jobs (cancelling a runner-claimed job to force a retry
> doesn't help a saturated single-runner pool, same established reasoning above).
...
- **2026-08-14 (`ci_satellite_ao_dispatch_batch13` todo, slot 20) — billing + load re-measurement for the 17+PM revert
  (`self_hosted_runner_public_repo_revert_2026_08_05.md` todo 20), real data both sides.**
  - **GitHub Actions billing (real, GitHub Enhanced Billing usage API, `github-billing-token` GSM secret, the
    already-proven mechanism `/codex/05-infrastructure/billing-cost-observability.md` documents)**: pulled the full
    August 2026 `usageItems` for all 18 target repos, `sku=Actions Linux` (GH-hosted compute minutes; excludes the
    separate always-near-zero `Actions storage` sku). Day-by-day aggregate `netAmount` across the 18 repos: 08-01
    $3.83 → 08-02 $11.69 → 08-03 $23.39 → 08-04 $26.56 → 08-05
    $13.00 (pre-revert-complete, mixed), then **08-06
    through 08-13: exactly $0.00 net every single day** (8
    consecutive full clean days) despite `grossAmount` climbing fleet-wide (7.4k → 16.3k GH-hosted minutes/day) — the
    public-repo-unmetered premise holds in the REAL billing data, not just in theory. 08-14 (today, partial/in-flight
    day) shows
    $8.74 net, concentrated in
    deployment-service/instruments-service/deployment-api/unified-api-contracts/unified-trading-library/
    market-data-processing-service/trading-agent-service (7 of 18 repos, $0
    elsewhere) — reads as the same billing-reconciliation lag the AWS CUR side of this same codex doc already documents
    for its own "provisional day" cutoff (the public-repo discount posts a day or two behind raw usage), not a
    regression; worth a follow-up spot-check next week to confirm 08-14 settles to $0 once reconciled.
  - **Self-hosted VM load average — TWO VMs in scope post-2026-08-05 split, only one directly measurable this session.**
    (a) The OLD shared host `i-0c9b283b31d6b5ca7` — this worker's own session runs ON this exact VM
    (`hostname`/`instance-id` confirmed) — live `uptime` = **6.38/6.34/6.78** (down from the 25-65+ range this doc's
    entries establish as the historical contention baseline, and from this doc's own 2026-08-05 post-split reading on
    the SAME host). Corroborated structurally, not just by the load number: `systemctl list-units --all | grep glue` now
    shows **zero** `github-glue-runner-*` units on this host (only the unrelated `glue-runner-crash-loop-watchdog`
    timer/service) — the 17+PM repos (plus the other 8 private repos, all migrated to the dedicated escalation VM per
    this doc's 2026-08-05 entry) are genuinely gone from here, not just idle. (b) The NEW dedicated
    `ci-escalation-runner-vm-1` (`i-042a6332509482556`) — where the actual glue-runner pools live now — could **not** be
    re-measured this session: `aws ssm start-session`/`send-command` against it both returned `AccessDeniedException`
    for this worker's AWS identity (`arn:aws:iam::427895769566:user/ikenna-worker`). This is the same non-self-fixable
    IAM-gap CLASS already documented for `ce:GetCostAndUsage` in
    `/codex/05-infrastructure/billing-cost-observability.md` (verified 2026-07-08) — `ikenna-worker` is not the
    self-service `uts-orchestrator-epic-role`/`unified-trading-sa` ambient identity this workspace's IAM self-service
    rule covers, so this is a genuine, pre-existing access gap, not something to self-grant here. The last real
    measurement on that VM remains this doc's own 2026-08-05 entry (load 29.25/29.36/30.65, disk-throughput- bound at
    89.5% `%util`) — flagging for whoever next has SSM reach on that specific instance to get a fresh number; the (a)
    side's clean zero-runner confirmation is nonetheless real, positive evidence the revert's capacity relief is
    holding.
  - **Verdict**: the billing half of todo 20 is now fully closed with real dated evidence (8/8 clean days at $0 net);
    the load-average half is partially closed (old host: measured + confirmed zero-runner; new escalation VM: blocked on
