...
- [x] [INFRA] P0. Re-enable `uts-prod-dp-exit-code-monitor-cron` — **DONE 2026-08-22 (slot 7, infra)**. Corrected a
      ...
      confirmed `state=ENABLED`, `schedule=*/5 * * * *`; (5) watched 4 fleet-size samples 3min apart over ~15
      minutes (spanning 3+ firings at the 5-min cadence): `fleet_size=3` on every sample, zero growth, and the 3
      running VMs (`mdps-defi-2025-*`, two `mdps-sports-bucket-*`) are all distinct cells — no duplicate-cell
      dispatch. The literal "26-31" range this todo's prior text named was specific to the 2026-08-15 incident-day
      state (a full backfill campaign actively running across every cell); today's much smaller live fleet (most
      2026-08-15-era campaigns have since completed) makes that exact number inapplicable, but the actual invariant
      it existed to protect — stable, non-climbing, no duplicate-cell relaunches — is directly confirmed. No
      re-pause needed.
...
- **2026-08-22 (slot 7, infra)**: closed the sole remaining open todo (cron re-enable). Corrected the todo's own
  ...
  `uts-prod-dp-exit-code-monitor-cron` (now `ENABLED`, `*/5 * * * *`). Watched 4 fleet-size samples over ~15 min
  (3+ firings): stable at 3, all distinct cells, zero duplicate-dispatch. All todos in this doc are now closed —
  archive-eligible.
