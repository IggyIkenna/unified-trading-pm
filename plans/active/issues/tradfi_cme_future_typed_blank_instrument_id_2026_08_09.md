...
- [ ] [DATA] P3. **[SCOPE INVALIDATED — needs operator re-triage]** Fix the root-cause `continuous_future` → `FUTURE`
      conflation in `canonicalize_manifest_instrument_type()` and re-run the rebuild script. The original 20,254-row
      "backfill per-contract instrument_ids" scope is WRONG — a 2026-08-10 slot-21 census found **473,374 rows** (23×
      original), 425K written TODAY (actively regenerated by the consolidator). These are NOT per-contract singles
      needing `instrument_id` derivation — they are **bundle-grain `continuous_future` rows** (ALL have populated
      `underlying` + null `instrument_id` + empty `quote_asset`/`margin_type`) that
      `canonicalize_manifest_instrument_type()` at
      `unified_trading_library/canonical/_manifest_instrument_type_canon.py:54` mis-maps to `FUTURE`. The fix is a
      **canonical-lib change, not a per-row backfill**: (1) remove `"continuous_future": InstrumentType.FUTURE` from
      `_MANIFEST_ITYPE_CANONICAL`, (2) add `"combo"` and `"continuous_future"` to `_BUNDLE_GRAIN_EXCLUDED` (the
      consolidator at `manifest_consolidator.py:2287` already classifies `continuous_future` as structurally-blank — the
      canonicalizer is the outlier), (3) update tests asserting `continuous_future` → `FUTURE`, (4) re-run
      `rebuild_tradfi_manifest.py` to regenerate the manifest with correct `instrument_type`. **This reverses an
      operator ruling** (the docstring at line 27-29 cites it) — needs operator sign-off before executing. Repos:
      unified-trading-library, market-tick-data-service. **Done when**: operator re-rules, canonical-lib fix ships,
      rebuild re-run, live manifest recount shows 0 `instrument_type=FUTURE` rows with populated `underlying` + null
      `instrument_id`.
