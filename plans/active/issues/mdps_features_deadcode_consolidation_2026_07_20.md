...
- [ ] 1. [SCRIPT] P2. S1-a — `launch-prediction-features-vm.sh` BROKEN (packages removed
      ...
      registry (no longer operator-gated — see round5 declassification below).
- [ ] 2. [SCRIPT] P2. S1-b — `launch-mdps-features-live.sh` non-runnable (no dispatcher branch;
      ...
      in `vm_prefix_registry.py:841-851` (5 rows). DELETE launcher + 5 rows OR finish the dispatcher branch (no longer
      operator-gated — see round5 declassification below).
