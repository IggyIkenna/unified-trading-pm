...
summary: >-
  ...
  clean-LDR-checkout tarball rebuild (`market-data-processing-service@2ef81aa8`) and relaunched `mdps-backfill-tradfi-*`
...
- [x] ✅ [DATA] P1. **DONE 2026-07-27 (slot-6, data_engineering)** — Root-cause finding (1): locate the MDPS caller that
      ...
      **Root cause identified**: `market_data_processing_service/app/core/batch_workers.py`'s `_handle_empty_tick_data`
                              ...
                              **Fix shipped**: (1) added a new closed-taxonomy `RecordFailedReason.NO_RAW_TICK_DATA_FOR_SHARD` value
                              ...
                              own typed calendar-aware `classify_sports_empty_reason` path) — `market-data-processing-service@e97effd`,
...
- [x] ✅ [DATA] P1. **DONE 2026-07-27 (slot-16, data_engineering) — `market-data-processing-service@e1303fa` + live
      ...
      **Root cause identified + fix shipped (slot-16, data_engineering, 2026-07-27) — `market-data-processing-service@e1303fa`.**
...
- [x] ✅ [DATA] P2. **DONE 2026-08-03 (slot 5, worker) — `market-data-processing-service@de8ea9f`.** Deeper root cause
      ...
      **MECHANISM CONFIRMED (2026-07-30, slot-7, independent static-code read against the live tree — mechanism (a),
                              ...
                              **FIX SHIPPED 2026-08-03 (slot 5, worker) — `market-data-processing-service@de8ea9f`.** Added
...
- [x] ✅ [SCRIPT] P2. **DONE 2026-08-03 (slot 8, worker).** Once finding (1) or (2) ships, re-run
      ...
      **Prereq confirmed shipped**: `market-data-processing-service@de8ea9f` (the "Deeper root cause" mechanism-(a) fix
...
- [ ] [DATA] P3. **SPLIT 2026-08-12 (/plan-reconcile), OPTION half — now UNBLOCKED, ready to dispatch (unlike the ETF
      ...
      crash is likely the SAME class of caller-scoping issue as the combo/futures_chain todo above, not a registration
      gap. Per this doc's own 2026-08-09 "UNBLOCKED (plan_reconciler)" note above, the P2 "Deeper root cause" dependency
      this OPTION half needed has since shipped and is verified live (`market-data-processing-service@de8ea9f`) — this
      half no longer needs the schema-shape decision the ETF half does, only a caller-scoping investigation (same class
      as the resolved combo/futures_chain todo). Repo: market-data-processing-service. Evidence: same run.log, 34
      `instrument_type='OPTION'` "No SchemaContract registered" lines.
...
- **2026-07-27 (slot-16, data_engineering) — finding (2) code shipped, live re-run pending.** Fix
  `market-data-processing-service@e1303fa` (LDR): streaming chain-bundle empty-timeframe now emits an honest manifest
...
- **2026-08-03 (slot 8, worker) — re-run-verify todo CLOSED, positive evidence obtained, 2 new findings tracked.**
  Confirmed the "Deeper root cause" mechanism-(a) fix (`market-data-processing-service@de8ea9f`, mdps
...
- **2026-08-09 (slot 8, data_pipeline_failure escalation agt-157000, DP-FETCH-009)** — cross-referencing a
  `DP_RUN_MOSTLY_EMPTY`/DP-FETCH-009 alert (asset_group=tradfi, data_type=ohlcv_15m, 2692 `attempted_failed` of 5459
  attempted, ratio 49.3%) that auto-filed no separate issue doc. Queried the live manifest directly
  (`gs://market-data-tick-tradfi-prd-central-element-323112/_index/availability_index.parquet`, column-pruned +
  filtered, bounded via `run-bounded-analysis.sh`): all 2692 rows are `error_reason=NO_RAW_TICK_DATA_FOR_SHARD` (venue
  breakdown NYSE 2128 / FX 264 / NASDAQ 112 / FRED 84 / KRX 72 / ICE 28 / CME 4). This is byte-for-byte the intended,
  correct output of finding (1)'s shipped fix above (`batch_workers._handle_empty_tick_data` routing tradfi through
  `record_failed_for_shard(NO_RAW_TICK_DATA_FOR_SHARD)` instead of a false `empty_confirmed`) applied to instrument-days
  where the underlying raw `ohlcv_1m`/`ohlcv_1s` tick data genuinely doesn't exist yet — the root gap (NASDAQ/NYSE raw
  tick coverage) is separately tracked and actively backfilling in `/plans/active/data_completion_tradfi_2026_07_15.md`.
  **No new root cause, no code fix needed** — this alert is expected to keep firing (correctly, as "not yet
  honest-terminal until the raw-tick backfill completes") until that sibling plan's NASDAQ/NYSE 1m/1s backfill lands and
  is manifest-verified. Leaving this note so a future recurrence of this exact DP-FETCH-009 shape resolves in one grep
  instead of a re-diagnosis.

- **2026-08-10 (slot 4, data_pipeline_failure escalation agt-157000 re-fired, DP-FETCH-009) — RE-CONFIRMED, no action.**
  Same escalation id re-dispatched a day later; boot context explicitly flagged it as a **STATIC BACKLOG** (no new
  `attempted_failed` activity in 1d) — consistent with yesterday's finding. Independently re-derived the same conclusion
  from the codex/plan trail before finding the 2026-08-09 note above (cross-checked
  `/codex/02-data/tradfi-databento-sourcing-ssot.md` + this doc's own fix history): the NASDAQ/NYSE
  `NO_RAW_TICK_DATA_FOR_SHARD` cells stay static because their prerequisite raw-tick backfill is itself blocked — TradFi
  Databento is account-suspended for non-payment
  (`/plans/active/issues/tradfi_databento_account_billing_suspended_2026_08_09.md`, `status: blocked`, P0,
  `BLOCKED-OPERATOR-DECISION`), and `data_completion_tradfi_2026_07_15.md` (lines 300/356) already cross-refs that same
  gate. No new fetches → no new `attempted_failed` cells → the "static" shape is exactly what's expected. No code fix
  applicable; this alert will self-clear once the operator restores Databento billing AND the NASDAQ/NYSE 1m/1s backfill
  resumes and lands. `/done` called without further changes.

- **2026-08-10 (slot 2, data_pipeline_failure escalation agt-157000 re-fired a 3rd time, DP-FETCH-009) — RE-CONFIRMED,
  no action, same root cause.** Independently downloaded and queried the live
  `gs://market-data-tick-tradfi-prd-central-element-323112/_index/availability_index.parquet` directly (bounded,
  targeted single-file read) rather than trusting the prior notes alone: `ohlcv_15m` total 98,836 rows (captured 44,899
  / empty_confirmed 23,688 / attempted_failed 21,236 / expected_unattempted 9,013); **100% of the 21,236
  `attempted_failed` rows carry `error_reason=NO_RAW_TICK_DATA_FOR_SHARD`** (venue breakdown: NYSE 10,668, FRED 3,335,
  CME 2,932, NASDAQ 1,436, CBOE 1,224, KRX 792, FX 505, ICE 344 — a superset of the alert's own 2,692-row batch
  snapshot, consistent with continued periodic re-derivation of the same static population, not growth). This is
  unchanged from the 2026-08-09/2026-08-10-slot-4 findings above: the correct honest classification while the
  NASDAQ/NYSE raw-tick backfill is itself gated on
  `/plans/active/issues/tradfi_databento_account_billing_suspended_2026_08_09.md` (still `status: blocked`, P0,
  `BLOCKED-OPERATOR-DECISION`, unresolved as of this dispatch). Three same-`escalation_id` (agt-157000) dispatches in
  ~30h (2 within ~3h15m of each other today) is consistent with `DP_RUN_MOSTLY_EMPTY`'s documented periodic
  re-remind-on-standing-condition behavior (`/codex/05-infrastructure/data-pipeline-alerts.md`), not a dedup defect —
  the underlying condition (billing suspended) genuinely has not changed between dispatches, so re-paging is correct
  until it resolves. No code fix applicable; will self-clear once Databento billing is restored and the NASDAQ/NYSE
  backfill lands. `/done` called without further changes.

- **2026-08-10 (slot 16, data_pipeline_failure escalation agt-157000 re-fired a 4th time, DP-FETCH-009) — CODE FIX
  SHIPPED, closes the redundant-dispatch loop.** Correction to the slot-2 entry immediately above: re-paging
  (Slack/`#data-pipeline-alerts` cooldown re-remind) and re-_dispatching a full `data_pipeline_failure` worker_ are two
  different mechanisms (`/codex/05-infrastructure/data-pipeline-alerts.md` § "Severity → routing" vs
  `deployment-service/deployment_service/data_pipeline_monitors/escalation_dedup.py`'s Option A dispatch dedup) — the
  former re-firing on a standing condition is correct and expected; the latter spawning a brand-new orchestrator-agent
  session 4 times in ~30h for an already-fully-diagnosed, purely operator-gated condition is exactly the failure mode
  `escalation_dedup.py` exists to prevent, and it WAS a genuine dedup defect, not "working as intended." Root-caused
  live: `check_dispatch_dedup` never found THIS doc as the covering issue for
  `(asset_group=tradfi, data_type=ohlcv_15m, registry_id=DP-FETCH-009)` because (1) `find_open_issue_for_tuple`'s
  free-text regex required the tuple written as `asset_group=X data_type=Y` (whitespace-only separator) but the prior
  2026-08-09 entry above wrote it comma-separated (`asset_group=tradfi, data_type=ohlcv_15m` — line 506), which the
  regex silently failed to match (5 of 8 corpus docs carrying this signature use the comma form), and (2) this doc's own
  `tags:` was missing the `dp-fetch-009` registry-id tag the matcher also requires. Both gaps meant
  `find_open_issue_for_tuple` returned `None` on every one of the 3 prior dispatches, so dedup silently fell through to
  a full respawn each time despite `is_static_backlog=True` already being stamped on the finding (which would have
  short-circuited straight to a skip had the doc been found). Fixed the regex (tolerate an optional comma) + added a
  regression test reproducing this exact doc's comma form, shipped `deployment-service@5796de52` (QG green, verified
  ancestor of `origin/live-defi-rollout`). Added the missing `dp-fetch-009` tag to this doc's frontmatter (same edit as
  this entry) so the next `(tradfi, ohlcv_15m, DP-FETCH-009)` re-fire now matches and dedup-skips the fast-spawn
  (informational Slack re-remind continues unaffected; only the wasteful full-worker-session spawn stops). Underlying
  diagnosis unchanged from the 3 entries above — still `BLOCKED-OPERATOR-DECISION` on
  `/plans/active/issues/tradfi_databento_account_billing_suspended_2026_08_09.md`, no data-pipeline code fix needed or
  attempted here.

- **2026-08-10 (slot 2, data_pipeline_failure escalation agt-157000 re-fired a 5th time, DP-FETCH-009) — RE-CONFIRMED,
  no action; plus one NEW observation: slot-16's dedup fix is not yet promoted/deployed, which is why this re-dispatch
  occurred despite it.** Fresh bounded re-query of the live
  `gs://market-data-tick-tradfi-prd-central-element-323112/_index/availability_index.parquet` (column-pruned read via
  `run-bounded-analysis.sh`, MTDS `.venv`, 7,735,500 total rows): `ohlcv_15m` total 205,335 (captured 134,599 /
  empty_confirmed 23,686 / attempted_failed 38,037 / expected_unattempted 9,013) — **100% of the 38,037
  `attempted_failed` rows carry `error_reason=NO_RAW_TICK_DATA_FOR_SHARD`** (venue breakdown: NYSE 15,780, FRED 8,966,
  CBOE 3,746, CME 2,932, NASDAQ 2,420, KRX 2,256, FX 1,029, ICE 908). vs the earlier slot-2 read today (98,836 total,
  21,236 af): captured +89,700 and attempted_failed +16,801 grew from continued backfill runs correctly re-deriving the
  same gated population (CME exactly flat at 2,932; empty_confirmed/expected_unattempted flat) — growth is honest
  classification of newly-attempted instrument-days, NOT a new failure mode. Verdict unchanged from all 4 prior entries:
  correct honest classification while the NASDAQ/NYSE raw-tick backfill is gated on
  `/plans/active/issues/tradfi_databento_account_billing_suspended_2026_08_09.md` (re-checked: still `status: blocked`,
  P0, `BLOCKED-OPERATOR-DECISION`, unresolved). **NEW OBSERVATION (why this 5th full-worker dispatch fired despite
  slot-16's `escalation_dedup.py` fix):** `deployment-service@5796de52` (the dispatch-dedup fix) is on
  `origin/live-defi-rollout` but **NOT on `origin/main`** — checked directly, deployment-service's LDR→main promotion is
  1,274 commits behind and no open `chore(promote)` PR exists — so the running data-pipeline monitor still executes
  pre-fix dedup code and fast-spawns on every static-backlog re-fire. Did NOT manually trigger promotion (per
  `ci-cd-flow.md`, ad-hoc `ldr-to-main-promote-fleet.yml` dispatches are banned); the redundant-spawn loop should
  self-close once deployment-service promotes/deploys or the billing gate resolves. No data-pipeline code fix
  applicable; `/done` without further changes.

- **2026-08-12 (/plan-reconcile) — CORRECTION: the Databento billing gate cited in all 5 entries above has been RESOLVED
  since 2026-08-10 13:05:30 UTC** (`unified-trading-pm@5ed8364ccb`: `metadata.list_datasets()` succeeded, 29 datasets,
