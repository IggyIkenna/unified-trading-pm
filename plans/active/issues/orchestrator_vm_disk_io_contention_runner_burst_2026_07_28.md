...
- [ ] [INFRA] P2. **Reconcile the m8i.4xlarge-vs-c7i.4xlarge discrepancy found 2026-07-30.** This doc's own
      `[x] [INFRA] P1` entry above (RAM right-sizing) describes a verified, operator-authorized resize
      `i-0c9b283b31d6b5ca7` `m8i.4xlarge` (64GB) → `c7i.4xlarge` (32GB) on 2026-07-28, with a live post-verify
      (`orchestrator.service` active, AutoSpawn respawned workers, 21 glue-runner units reconnected). Live instance
      metadata checked 2026-07-30 (`curl 169.254.169.254/latest/meta-data/instance-type` from this same host) shows
      `m8i.4xlarge`, and `free -h`/`/proc/meminfo` shows ~61.8GB total RAM — the ORIGINAL 64GB spec, not 32GB. Either
      the box was reverted/re-resized back at some later point (undocumented anywhere in this doc or found by this
      session's search), or the original resize's own verification was wrong. Matters because
      `orchestrator.service.d/memory-cap.conf`'s `MemoryHigh`/`MemoryMax` (currently ≈23G/26G, confirmed live via
      `systemctl show`) were computed as 87.5%/75% ratios of the 32GB box — if the box is genuinely back on 64GB, that
      cap artificially throttles the orchestrator service to well under half the box's real headroom instead of the
      ratio it was tuned for. Repo: unified-trading-pm (this doc) + agent-orchestrator (`bootstrap_vm.sh`'s
      not-yet-shipped self-correcting cap computation, already blocked on the unrelated
      `fleet_fastapi_upper_bound_stale_vs_utl_floor_bump_2026_07_28.md` SSOT contradiction per that P1 entry's own note
      — check whether that blocker has cleared before re-deriving the caps).
