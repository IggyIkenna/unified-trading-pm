...
- [x] ✅ [SCRIPT] P2. **2026-08-14: pre-fetch fixture gate landed — `_fetch_all_leagues` now skips the vendor discovery
      HTTP call entirely for a league+date with no catalogued api_football fixture**, instead of calling unconditionally
      for all 33 leagues every day. Reuses the same UAC catalog registry (`catalog_list_instruments`) `sentinels.py`'s
      v2 path already trusts — no new cross-service read. Fails open (attempts every league, unchanged behavior)
      whenever the catalog is empty for the WHOLE date (not just one league) — the T0-not-landed signal — so it cannot
      silently suppress genuine coverage on same-day/near-future fetches; only a demonstrably populated catalog makes
      one league's absence a reliable skip. Cuts wasted vendor calls (relevant now that credits are a live constraint)
      without touching the fixture-gating logic in `sentinels.py` — that logic was investigated separately and found NOT
      to be the cause of this doc's two largest gap windows (v2's catalog has real fixture data for nearly every day in
      both `2024-11-21..12-31` and `2026-03-06..03-27`; the gaps are downstream capture failures, not a sentinel-gating
      problem — see `mtds_odds_backfill_watchdog_kill_after_silent_hang_2026_08_08.md`).
      `market-tick-data-service@d7498e6e43`, QG green, landed on LDR.

## Progress Log

- **2026-08-12T01:09Z (slot 9, P2 VERIFY, bare check): fleet STILL EMPTY — 0 `mtds-backfill-odds-*` VMs, unchanged since
  slot 6's 21:56Z reading (~3h13m ago) — the longest observed stall gap in this doc's history (every prior stall
  recovered within 2-11min).** Confirmed via `gcloud compute instances list --filter="name~mtds-backfill-odds"` (0
  rows). No successor to `smallchunk23` has appeared. Since nothing has processed since the last census, re-running it
  now would reproduce slot 6's unchanged 277-missing-days result (1981/2258d present) — declined. No VM launched from
  this VERIFY todo (standing instruction: relaunch is `sports_all_vendor_honest_coverage_convergence_2026_08_07.md`'s
  scope, not this todo's). Flagging the stall duration itself as the genuinely new information this dispatch — it's no
  longer a routine multi-minute preemption gap, so whoever next picks up the P1 backfill todo or the tracker doc should
  treat this as needing an active relaunch, not just a passive re-check. Not flipping either checkbox (gaps clearly
  still remain). Skip `GATED` (est 180).
