...
- [x] ✅ [DOC] P2. Add `launch-cefi-hl-aster-historical-backfill.sh` to `vm-launcher-runbook.md` § 2 "MTDS Backfill VMs"
      ...
      unified-trading-pm@pending (see commit below); `vm-launcher-runbook.md` codex/ is out of `check_line_caps.sh`'s
      scope (plans/active + plans/epics only), so no line-cap gate applies to it.
