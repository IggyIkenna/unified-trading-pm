...
- **2026-08-21 (slot-8 worker, incidental discovery while shipping unrelated work)**: Fresh evidence this doc's "load-induced" framing does not fully explain the failure mode -- **the same 4 tests fail 4/4 times, deterministically, at `load average: 5.75` (not the 48-164 this doc's diagnosis section cites), both inside a full `quality-gates.sh` run and standalone**: `not ok` on "detects a live foreign process whose cwd is INSIDE the slot dir", "warning names the slot dir...", "falls back to plain-text output when jq is unavailable" (all `tests/test_session_start_collision_check.bats`), and "foreign_claude_pids still finds a live peer through the batched path" (`tests/test_slot_collision_detect_lsof_batching.bats`). This contradicts the todo-3 fix's own "Verified: 10/10 pass standalone" claim -- either a regression since 2026-08-17, or (more likely given the low load + perfect determinism) the `_spawn_fake_peer`/`foreign_claude_pids` detection technique has an execution-environment dependency this doc hasn't identified yet (e.g. how a Claude-Code-agent-driven Bash tool invocation spawns/backgrounds the fake-peer process vs. a plain interactive shell) -- not investigated further here, out of scope for the task in progress (a `[BACKEND]` P1 elsewhere in the corpus). **Cross-checked against live CI**: `quality-gates-v2` on `live-defi-rollout` is green (`gh run list`, most recent success at 2026-08-21T18:09:33Z) -- the trunk itself is healthy; this is a local/environment-specific gate artifact, not a real regression, confirming this doc's own "converts host/environment quirks into a fleet-wide block on unrelated work" concern (P2 todo above) is still live. Declaring a `qg_red` repo-blocker for the unrelated task this was discovered from rather than blocking on a fix here.
