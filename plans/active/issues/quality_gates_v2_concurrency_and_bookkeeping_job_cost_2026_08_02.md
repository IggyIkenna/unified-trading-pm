...
summary: >-
  ...
  appeared to contradict /codex/08-workflows/ci-cd-flow.md's still-live-branch-ref "PM Option-B standing LDR->main PR"
  description (codified 2026-06-09) — **VERIFIED 2026-08-02 (slot-15, this doc's `[SCRIPT] P2` todo): the workflow DOES
  create a `promote/unified-trading-pm/<sha12>` immutable per-SHA ref (same pattern as the fleet bot); migration pinned
  to `40386f0274` (2026-07-18), not the 07-27 "churn fix" `48800b7ad`; ci-cd-flow.md Option-B section + squash/`--merge`
  table row corrected with a dated banner.**
status: open
nature: issue
asset_group: [ci]
stage: [meta]
...
related: [
    ...
    /plans/archive/2026_07/ci_consolidated_closeout_2026_07_25.md, # ci-tranche closeout family link (added 2026-08-07, plan_reconciler agt-6eb8c5 — AG-closeout linkage REAL-orphan fix)
  ]
created: 2026-08-02
author: unknown
priority: P2
...
context_scope: [
    /codex/08-workflows/ci-cd-flow.md,
    /plans/active/github_actions_operator_gated_followups_2026_07_17.md,
    unified-trading-pm/.github/workflows/python-quality-gates-v2.yml, # PM-local copy deleted 2026-08-06 (b62a209dc); live source = unified-trading-ci/.github/workflows/python-quality-gates-v2.yml (note added 2026-08-07, plan_reconciler agt-6eb8c5)
...
> **🟢 REACTIVATED 2026-08-07 (plan_reconciler agt-6eb8c5, per the doc's own reactivation instruction below)** — the
> `DEFERRED-until-2026-08-05:` prefix was dropped once real post-fix PR churn had accumulated; the re-measure todo below
> is again dispatchable. Original park rationale (kept for history): the lone remaining todo was calendar-gated, not
> blocked/ambiguous — it needed a few days of real post-fix `push`/`pull_request` PR churn to produce a statistically
...
- [x] ✅ [INFRA] P2. **`quality-gates-v2`'s 3 fixed bookkeeping jobs — `content-gate`, the aggregation job named
      ...
      **Finding 1 — the naive "merge all 3" is topologically impossible, so it wasn't attempted.** `content-gate`
                                                                                                                                                                                  ...
                                                                                                                                                                                  **What shipped**: `record-qg-result`'s two steps (GCP auth + read/decide/persist) moved verbatim into
                                                                                                                                                                                  ...
                                                                                                                                                                                  unified-trading-pm@60f3c0dd2 (fold commit, reachable on origin/live-defi-rollout; the original
                                                                                                                  evidence line held a placeholder `@<shipped in this commit>` — sha recovered + filled in by
                                                                                                                  plan_reconciler agt-6eb8c5 2026-08-07) — `.github/workflows/python-quality-gates-v2.yml` (that PM-local
                                                                                                                  file was deleted 2026-08-06 by `b62a209dc`; the live source is now
                                                                                                                  unified-trading-ci/.github/workflows/python-quality-gates-v2.yml — see context_scope note).
