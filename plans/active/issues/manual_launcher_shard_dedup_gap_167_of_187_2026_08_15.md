...
- **na-eligibility-audit 2026-08-17 (cross-cutting tranche, dispatch agt-3931fd)** [body-hash:64f3ac4f71545856]: KEEP-NA, valid — 1 open item (the fix for the incident-implicated launcher family is already [x] DONE). The sole remaining todo (triage the other ~166 launchers without any dedup check) explicitly self-assesses as not a single bounded task in its own text: 'Genuinely audit-scope — size as its own plan, do not attempt as a single todo.' The doc's own 'Recommended decision' section states some of the 166 are non-idempotent/one-shot and need per-launcher judgment on collision semantics, not a mechanical sweep, and recommends a dedicated follow-up /ag-closeout-audit-style batch rather than direct dispatch from here. Ruling/citation: Doc's own text (Todos + Recommended decision sections): 'Genuinely audit-scope — size as its own plan, do not attempt as a single todo' — some of the 166 launchers need per-launcher judgment, not a single mechanical sweep.
