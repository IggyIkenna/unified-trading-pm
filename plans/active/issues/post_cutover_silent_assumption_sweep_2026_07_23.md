...
- [x] ⛔ [INFRA] P1. **RETIRED 2026-08-08 (operator ruling: formally retire Option B now).** (ruling SSOT:
      `github_actions_operator_gated_followups_2026_07_17.md` § "Cost ruling 2026-07-23" +
      `/codex/08-workflows/ci-cd-flow.md`'s Release tag reconciler section) ~~Implement conventional-commit bump
      computation + tag mint in `reconcile_release_tags.py`, reusing the proven rules. Gate behind an explicit `--mint`
      flag so the detector stays usable standalone.~~ Moot -- the per-repo `semver-agent` retarget
...
- [x] ⛔ [DOC] P1. **SUPERSEDED 2026-07-25, formally retired 2026-08-08 (operator ruling).** (ruling SSOT:
      `github_actions_operator_gated_followups_2026_07_17.md` § "Cost ruling 2026-07-23" +
      `/codex/08-workflows/ci-cd-flow.md`'s Release tag reconciler section) Option B (this whole sub-steps list) was
      never built; the per-repo `semver-agent` retarget shipped instead (see the ⛔ banner above this section + the
      Resolution checklist's F2 item). There is no reconciler-side message-only-vs-API-diff tradeoff to rule on since
      the reconciler itself does not exist and is not being built.
- [x] ⛔ [INFRA] P1. **RETIRED 2026-08-08 (operator ruling).** (ruling SSOT:
      `github_actions_operator_gated_followups_2026_07_17.md` § "Cost ruling 2026-07-23" +
      `/codex/08-workflows/ci-cd-flow.md`'s Release tag reconciler section) ~~Port the bump-rate circuit breaker to the
...
- [x] ⛔ [INFRA] P2. **RETIRED 2026-08-08 (operator ruling).** (ruling SSOT:
      `github_actions_operator_gated_followups_2026_07_17.md` § "Cost ruling 2026-07-23" +
      `/codex/08-workflows/ci-cd-flow.md`'s Release tag reconciler section) ~~Batch the manifest write to ONE commit per
      run (the whole point of B) — verify by confirming a single `chore(manifest):` commit after a multi-repo mint.~~
      Moot -- Option B (the whole point of which was this batching) was never built; the per-repo semver-agent retarget
...
- [x] ⛔ [INFRA] P2. **RETIRED 2026-08-08 (operator ruling).** (ruling SSOT:
      `github_actions_operator_gated_followups_2026_07_17.md` § "Cost ruling 2026-07-23" +
      `/codex/08-workflows/ci-cd-flow.md`'s Release tag reconciler section) ~~First supervised drain: run with
      `--dry-run`, eyeball the 22 proposed versions, then mint.~~ Moot -- there is no reconciler-side mint path to
...
- [x] ⛔ [INFRA] P1. **SUPERSEDED 2026-07-25, formally retired 2026-08-08 (operator ruling: retire it formally now —
      ruling SSOT: `github_actions_operator_gated_followups_2026_07_17.md` § "Cost ruling 2026-07-23" +
      `/codex/08-workflows/ci-cd-flow.md`'s Release tag reconciler section).** ~~F2 — restore version minting via OPTION
...
- [ ] [INFRA] P2. **Fix the unconditional `&& echo "...dispatched"` success reporting at the F3 orphan-dispatch sites**
      ...
      **`service-deployed → deployment-service` SLICE: DONE 2026-08-06** (operator-approved same-day fix, not just
      ...
      `sit-gate.yml` SLICES: also DONE, verified 2026-08-09 by plan_reconciler agt-c80749** —
      `ci_satellite_ao_dispatch_batch5_2026_08_02.md` todo 6 shipped this exact scope 2026-08-07 (confirmed `[x]` in
      that doc). Live-code-verified directly: `sit-gate.yml` now reports honest `::notice::`/`::warning::` for the
      best-effort `game-day-sit`/`synthetic-smokes` dispatches (no more unconditional `&& echo "...dispatched"` success
      claim); `cascade-qg-ordering.yml`'s fake `repository_dispatch` was replaced with a real `workflow_dispatch` of
      `quality-gates-v2.yml` (2026-08-03 fix, `sit_failure agt-34df9c`). **Only remaining open sub-scope: the 24-repo
      `semver-agent.yml` `schema-changed` dispatch** — deferred, contends with batch5's own todo 4 for the same file
      (batch5 D5-2). Leaving this todo `- [ ]` open (half-done) until that slice ships.
