...
- **na-eligibility-audit 2026-08-17** (ci tranche, autonomous, dispatch agt-b9cf62) [body-hash:4598fffa65ff513f]: KEEP-NA, valid — 5 open items, all individually evidenced. F1 kill-switch is explicitly time-gated behind a 2026-07-28 operator ruling (do not fix until execution-service handles live order flow). The '~4 weeks of missing tags' item already cites its own tracking vehicle (ci_satellite_ao_dispatch_batch1_2026_07_26.md's read-only census, archived) but has REAL remaining work beyond that census — 11 of 22 repos still stalled per the doc's own 2026-08-02 update, genuine partial completion, NOT a stale-citation-only case (correcting a hunter over-read this pass: the citation is already present in-doc, real work remains). F3's remaining 24-repo schema-changed slice has been a named 'extraction candidate' since 2026-08-01 but never extracted. F4 bundles a plausibly-bounded sub-part with an open-ended one. sit_validated_workspace_digest gap is a 7-times-reaffirmed design call.
