...
ship silently produces NOTHING (no commit, no LDR push, no staging PR) while exiting cleanly — the same dangerous
"ships NOTHING" class as the documented clean-tree early-exit, but triggered on a DIRTY tree whose only changes are new
files.
...
the strict-quickmerge ship path for exactly the case it should handle cleanly. Cross-cutting: `quickmerge.sh` is the
PM SSOT symlinked into every repo, so the fix is fleet-wide.
...
Fix line 1175 to account for untracked `--files`: gate on `git status --porcelain` being empty (consistent with line
508) rather than only `git diff origin/main`, OR when `--files` are explicitly supplied, stage them before the guard
(quickmerge already re-asserts `--files` scope on the commit-retry). Add a regression: a new-file-only
