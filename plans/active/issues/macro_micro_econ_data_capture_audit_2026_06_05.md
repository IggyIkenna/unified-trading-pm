      ...
      **IN PROGRESS 2026-07-29 (slot 8) — 2 real bugs found + fixed before any row could be captured, launch pending
                      ...
                      **IN PROGRESS 2026-07-30 (slot 6) — resumed here; confirmed both slot-8 bugs already shipped (under a
                              ...
                              4. **Verification completed 2026-07-30 (slot 6)**: `unified-api-contracts@6d87d95e` landed +
                                 ...
                                 **Full production backfill BLOCKED on a newly-discovered, SEPARATE bug (2026-07-30, slot
                                 6)**: `tradfi-bf-fred-full-20260730-020236` launched (SPOT, `1962-01-02..2026-07-29`,
                                 single VM) but hung indefinitely on the very first chunk — CPU flat ~0%, zero progress
                                 for 7+ min, no captured/skip/error line, no retry-warning ever fires. Reproduced 3/3
                                 (2 attempts at 1962-01-02, 1 at `--start-floor 1970-01-01` to rule out the exact floor
                                 date) — full diagnosis + evidence + ruled-out causes + recommended next steps in
                                 `fred_backfill_early_date_indefinite_stall_2026_07_30.md` (P1, new issue doc). This is
                                 DISTINCT from the calendar-exemption bug above (already fixed + verified, not the cause
                                 here — the 2024 smoke test with the SAME fixed code worked perfectly). All 3 hung VMs
                                 stopped (not left running). **Checkbox NOT flipped** — the fix + verification steps above
                                 are genuinely done, but this todo's actual deliverable ("Run the FRED backfill") remains
                                 blocked until the new stall bug is root-caused and fixed (needs live process inspection
                                 — py-spy/gdb against a fresh repro — beyond what a worker slot's GCS-log observation can
                                 do; see that issue doc's Recommended next steps).
