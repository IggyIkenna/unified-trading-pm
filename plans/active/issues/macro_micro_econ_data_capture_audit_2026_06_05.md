...
- [ ] [DATA] P1. **Run the FRED backfill** once the driver above exists — operator confirmed they already hold the
      ...
      **IN PROGRESS 2026-07-29 (slot 8) — 2 real bugs found + fixed before any row could be captured, launch pending
              ...
              **IN PROGRESS 2026-07-30 (slot 6) — resumed here; confirmed both slot-8 bugs already shipped (under a
                      ...
                      4. **Next steps (resume here)**: `unified-api-contracts@6d87d95e` is landed on `live-defi-rollout`
                         (pushed + verified). Next: confirm whether `market-tick-data-service`'s own
                         `_umi_fred.py`/dependent code needs a dep-bump to pick up the fixed `unified_api_contracts` content
                         (check how it's pinned — this repo's CI resolves siblings CONTENT-FIRST off LDR HEAD per
                         `ci-cd-flow.md`, so a fresh VM tarball rebuild should already pick up the fix without a version
                         bump, but verify rather than assume), THEN relaunch a FRESH `--year 2024` smoke VM via
