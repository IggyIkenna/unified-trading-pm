...
- [x] ✅ [TERRAFORM] P3. **DONE 2026-08-04 — deployment-service@4de0784.** Added `google_project_iam_member`
      (`roles/storage.objectViewer`) for `github-actions-deploy@central-element-323112.iam.gserviceaccount.com` scoped
      via IAM Condition `startsWith` on Group A's `-prd-`/`-test-` bucket prefixes (reuses the existing
      `group_a_bucket_prefixes` local, covering both tiers in one condition expression). Literal-email reference — no
      new `google_service_account` resource declared (github-actions-deploy predates this repo's IaC adoption and is
      externally managed). `tofu validate` clean; shipped via quickmerge.
