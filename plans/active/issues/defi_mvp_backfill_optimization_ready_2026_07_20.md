...
- [x] ✅ [DATA] P0. **DONE 2026-07-29 (slot-5).** Confirmed the auth-block framing was stale —
      `gcloud config set     account unified-trading-sa@central-element-323112.iam.gserviceaccount.com` (ambient
      self-service IAM identity, finding W) resolved `gcloud compute`/`gcloud run`/`gcloud storage` cleanly, no
      interactive login needed. **2-VM TheGraph canary — SATISFIED by existing production evidence, no new launch
      needed**: `mtds-dex-swaps-     backfill-1`/`-2` have been running concurrently against TheGraph for 6 days (since
      2026-07-23); full run.log scan found 0 genuine HTTP 429s and 0 `attempted_failed` shard corruption across
      92,317+43,913 log lines
      (`gsutil cat gs://deployment-scripts-central-element-323112/vm-logs/mtds-dex-swaps-backfill-{1,2}/run.log`) — the
      shared TheGraph key pool holds fine at N=2. **Pagination-fix re-backfill validation — SATISFIED**: launched
      `mtds-lending-indices-20260729-193529` (2026-06-01..06-30) via `launch-mtds-lending-indices-backfill-vm.sh`
      (confirmed the `mtds@6e2677b9` pagination fix is an ancestor of the published code tarball SHA before launching) —
      AAVE_V3 daily row counts repeatedly exceed the old 1000-item single-shot cap (up to 17,844 rows/ day on BASE,
      16,151 on ARBITRUM, 11,915 on ETHEREUM), proving `_paginate_history` correctly fetches past The Graph's cap with
      no silent truncation. VM was still mid-window (day 2/30) when this was flipped — SPOT +
      `VM_SHUTDOWN_ON_COMPLETION=true`, self-terminating, not fire-and-forget; follow-up verification todo filed.
      **Unplanned finding**: the same validation run surfaced a genuine, unrelated regression — COMPOUND_V3
