...
- [ ] [OPERATOR] P0. **2-VM TheGraph canary + pagination-fix re-backfill validation** — blocked on expired gcloud CLI
      auth (`gcloud compute`/`gcloud run`/`gcloud storage` all fail); needs an interactive `gcloud auth login` before
      the canary and `/data-pipeline-check-mtds` can run.
