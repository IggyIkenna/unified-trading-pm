...
- [ ] [DATA] P0. **Retagged 2026-07-29 (corpus hygiene pass): resolved-by-reference — see
      `plans/active/defi_consolidated_closeout_2026_07_18.md` (~line 651, 2026-07-28 gate-cleanup pass citing finding W
      — ambient self-service IAM identity, no different-identity credential requirement) and
      `plans/active/defi_dex_pool_symbol_fix_backfill_purge_2026_07_25.md`'s own Progress Log (successful
      `gcloud compute instances create`/`describe`/`delete` calls through 2026-07-28, directly contradicting this doc's
      "gcloud compute is blocked" framing). No longer operator-gated — dispatchable as a normal AO VM-launch todo.**
      2-VM TheGraph canary + pagination-fix re-backfill validation — blocked on expired gcloud CLI auth
      (`gcloud     compute`/`gcloud run`/`gcloud storage` all fail); needs an interactive `gcloud auth login` before the
      canary and `/data-pipeline-check-mtds` can run.
