...
- [ ] [OPERATOR] P1. **Grant `unified-trading-sa@central-element-323112.iam.gserviceaccount.com` Cloud Scheduler read
      access** (`roles/cloudscheduler.viewer` or equivalent `cloudscheduler.jobs.get`+`.list`) — confirmed missing
      2026-07-27 (slot-6): every `market-data-{cefi,defi,sports,tradfi}` consolidator scheduler-job-state lookup
      404s/`PERMISSION_DENIED`s under both the ADC runtime identity and the `github-actions-deploy` CLI identity, even
      though `manifest_consolidator_scheduler.tf` defines all 5 jobs. This silently blocks the fail-closed
      `assert_consolidator_paused` pre-check every prod-manifest remediation script (this one + the already-shipped
      `sports_manifest_remediation_safety.py`-based ones) needs before any real `--apply`. Without this grant, NO
      manifest remediation script using that safety contract can be run to completion, only dry-run.
