...
doc_type: issue
title:
  Half-time odds — SFI vs in-play — resolved history PART 1 of 2 (original investigation §1-§5 + cross-checks/loose ends
  + the lookahead-leak-fix and SFI h1_* capture-gap Progress Logs + the closing-line/CLV leak fix leg), extracted
  2026-07-25 for the line-cap split
summary:
  'Archive-bound extraction (2026-07-25, plan line-cap remediation pass, precedent
  `/plans/active/issues/plan_line_cap_remediation_2026_07_23.md` § FINAL RESOLUTION) of the fully-historical body of
  `/plans/active/issues/sports_halftime_odds_sfi_vs_inplay_2026_07_16.md`. **PART 1 of 2** (this doc was itself over the
  1000L active-tier hard cap as a single history file, so it is chunked — mirrors the
  `cefi_4surface_migration_execution_log_history_part1/2_2026_07_24.md` precedent). Part 1 carries: the original
  read-only investigation (SFI column inventory, the 746,928 in-play rows anatomy, the T-0/HT horizon analysis, the
  features-consumption audit, the OR-5b(c) B-REFINED verdict, cross-checks and loose ends), the first "Todos" list
  (opened by the lookahead-leak fix leg, DONE/PARTIAL items only), and the Progress Log entries for the lookahead- leak
  fix, the SFI `h1_*` capture-gap fix, and the closing-line/CLV leak fix + model quarantine. **Part 2**
  (`sports_halftime_odds_sfi_vs_inplay_history_part2_2026_07_25.md`) carries the T-0 recompute execution, the second
  "Todos" list (DONE items only), the collapse recompute execution, the ODDS_FEATURES recompute execution, the
  fixture-identity-collapse fix, and the phantom-block + pivot fixes. Verbatim, byte-for-byte, in original order across
  both parts. 9 of the parent doc''s original 14 checkboxes live across parts 1+2 (8 `[x]` done + 1 `[~]` partial, all
  superseded/completed by later work per the parent''s own RE-TRIAGE section) — the 5 genuinely still-open `[ ]`
  checkboxes stay in the parent, not duplicated here. Record-only; not intended for further action. The parent''s live
  open work + RE-TRIAGE section is at /plans/active/issues/sports_halftime_odds_sfi_vs_inplay_2026_07_16.md.'
status: resolved
nature: record
asset_group: [sports]
stage: [data]
...
related:
  [
    ...
    /plans/active/issues/sports_halftime_odds_sfi_vs_inplay_history_part2_2026_07_25.md,
...
source:
  [
    "Plan line-cap hygiene remediation, /plans/active/issues/plan_line_cap_remediation_2026_07_23.md § FINAL RESOLUTION
    — extract-to-archive-bound-history-child pattern",
  ]
---

> **Archive-bound history child.** For current status, the 5 open todos, and the RE-TRIAGE section, see the parent:
> `/plans/active/issues/sports_halftime_odds_sfi_vs_inplay_2026_07_16.md`. Everything below is extracted verbatim,
> byte-for-byte, from that doc's original body (2026-07-16 investigation through the 2026-07-17 fix legs).
...
> _(One item originally at this position in this list — a `[ ]` [CODE] P1 open todo, "`_apply_ht_odds_pit_gate`'s
> default-cutoff branch is unreachable in production" — is still genuinely open as of 2026-07-25. It was moved to the
> parent doc's "Open Todos" section on the 2026-07-25 line-cap split, not duplicated here. See
...
> **Continued in part 2**: `/plans/active/issues/sports_halftime_odds_sfi_vs_inplay_history_part2_2026_07_25.md` (the
> T-0 recompute execution, the second "Todos" list, the collapse recompute, the ODDS_FEATURES recompute, the
> fixture-identity-collapse fix, and the phantom-block + pivot fixes — in original order, continuing directly from
> here).
