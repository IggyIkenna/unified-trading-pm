...
- **na-eligibility-audit 2026-08-21** (defi tranche, wave 2): KEEP-NA, valid — conflict-checked before considering RECLASSIFY: `plans/active/issues/execution_delta_proxy_repricer_generalization_2026_08_18.md` (status: open) independently tracks the same HealthFactorMonitor/DeleverageExecutor trace AND the identical `_SCE_1H` suffix fix as its own open todos (lines ~638-641) — a genuine duplicate-tracking risk across two NA docs, not yet reconciled. Per the conflict-check rule ("if you find a genuine conflict, note it and stay NA rather than flip"), NOT extracting the enumerate-consumers/sibling-config-check todos this pass despite their bounded shape — the two docs need a coordinated citation-fix first so a future dispatch doesn't duplicate. The execute-migration todo (gated on enumeration) also touches live-trading strategy_id identity used in ledgers/manifests — money-path-adjacent, warrants care alongside the citation fix. Doc stays `assigned_vm: NA`. **Flagging for a follow-up pass**: reconcile this doc against `execution_delta_proxy_repricer_generalization_2026_08_18.md`'s overlapping todos before either is worked.
