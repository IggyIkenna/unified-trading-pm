...
- 2026-08-10 (slot-27, agt-3c44d1): **CONTINUATION** — slot-19's run (agt-8005f6) crashed mid-STEP-3 (8 hunters
  launched, results lost). This run picks up the same findings doc. STEP 1 (repo sync — PM behind 4 commits with 1
  grace-window dirty file from main, stashed+FF'd+restored, all 24 sibling repos FF-clean) + STEP 2 (grace set
  recomputed against current corpus) complete. Re-ran `run_hygiene_sweep.sh --ci`: 4 hard failures corpus-wide
  (prosewrap ratchet, NA ratchet, reference-paths ratchet, AG-closeout-linkage ratchet) — all outside sports tranche
  scope, same as slot-19's run observed. Discarded the regen side-effect.
- 2026-08-10 (slot-27): Corrected corpus inventory (using the skill file's `^[ \t]*[-*] \[ \]` pattern with
  `re.MULTILINE` — the initial scan's `line.strip().startswith()` missed indented/star-bullet variants per the skill's
  Phase-2 rules). **105 sports-tagged docs** (up from slot-19's 101 — 4 new docs since): 38 grace-protected (36%), 5
  locked, 62 actionable non-grace. **586 done checkboxes, 264 open across all sports docs.**
- 2026-08-10 (slot-27): INDEX.md regenerated with the parser fix now live — confirmed both previously-missing sports
  docs (`sports_fixtures_browser_single_catalogue_source_2026_07_24.md`,
  `sports_odds_bookmaker_coverage_enumeration_2026_06_20.md`) now appear. Shipped `unified-trading-pm@ff1bfd2f3e`.
- 2026-08-10 (slot-27): **Archive candidate analysis — 8 docs with open=0, unlocked, non-grace. ALL 8 BLOCKED:**
  1. `sports_consolidated_native_ao_extract_2026_07_25.md` (33 done, DEFERRED=1) — deferred K1/K2 delete (operator-gated
     Track V) needs migration; `_finalize.md` companion still has open REVIEW todo.
  2. `sports_dependency_check_manifest_vs_gcs_path_2026_07_08.md` (10 done) — `archive_exempt: true`; cross-referenced
  ...
  8. `mtds_pipeline_check_enumerate_shards_masks_cefi_sports_mvp_2026_08_06.md` (0 done, zero-checkbox) — all-prose
     issue doc; root fix tracked elsewhere. **27 near-complete docs (open=1)** — all gated on operator decisions,
     dependency completion, or companion-finalize reconciliation. None eligible for the narrow `[REVIEW]`/`[DOC]`
     fold-by-default carve-out.
- 2026-08-10 (slot-27): **Missed-flip scan** — 38 candidates with potential evidence patterns identified. Sampled the
  highest-confidence: all either name a SHA as context (not completion evidence), are genuine blockers, or are in
  finalize docs whose parent work isn't fully shipped. No missed flips with conclusive HARD evidence this run. The 2
  RECOVERY NOTES added by main today target grace-protected or locked docs → not actionable.
- 2026-08-10 (slot-27): **Cross-tranche archival check** — grepped other 9 tranches' closeout docs; the 3 cross-tranche
  archive candidates flagged by sibling runs are all GRACE-protected this run → noted, not actioned.
- 2026-08-10 (slot-27): **Run summary**: 0 flips, 1 hygiene fix (INDEX.md regen `ff1bfd2f3e`), 0 contradictions
