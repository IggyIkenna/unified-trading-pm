...
- **na-eligibility-audit 2026-08-17 (cross-cutting tranche, dispatch agt-3931fd)** [body-hash:934e3753d8971620]: KEEP-NA, valid — Both open todos are the same ongoing, unsolved native-memory-leak root-cause investigation (sports/odds_api MTDS backfill OOMs) spanning 15+ investigation sessions since 2026-07-22: tracemalloc profiling (flat, ruled out Python-heap leak), a shipped session-reuse fix (did not reduce RSS), a first memray --native capture (2026-08-14, too-small scope, inconclusive) directly motivating the second open todo (re-run at reproducing scale). Genuinely open-ended research, not worker-determinable-by-fiat; reaffirmed KEEP-NA across 6 prior audit passes (07-30 through 08-08) with zero reversal. Note: doc's asset_group was corrected 2026-08-13 from [cefi,meta] to [cross-cutting], consistent with this tranche assignment.
