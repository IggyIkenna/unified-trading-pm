...
- [x] [SCRIPT] P3. **DONE 2026-08-10 — `deployment-service@ca2ecd5706`.** Add a memory-aware pre-launch gate INSIDE
      ...
      session, operator ask). The 2026-08-09 draft of this todo (below, superseded) proposed extending
      `odds-api-concurrency-guard.sh`/`tardis-concurrency-guard.sh` — that is the WRONG location: those guards run OFF
      the backfill VM, at launch time, on whoever invokes the launcher script, and only count fleet VM instances via
      `gcloud compute instances list`; they have no visibility into any given VM's own memory (the VM doesn't exist yet
      when the guard runs). The actual per-subprocess launch decisions happen inside `mtds_chunk_loop.sh`'s bash loop
      (the `for LEAGUE in "${CHUNK_LEAGUES[@]}"` / chunk `while` loop generated by
      `deployment-service/scripts/vm/setup-data-pipeline-vm.sh`), which runs ON the VM itself and already captures each
      subprocess's exit code for the `CHUNK_FAILED` fail-loud logging (2026-07-26 addendum above) — that is the correct
      site to extend, reading the SAME VM's own live memory state before spawning the next subprocess. Distinct from the
      P1 item below: this is a mitigation (stop launching the next subprocess into memory that's already too tight) that
      stands regardless of whether P1's root cause turns out to be the dirty-page/writeback backlog or pyarrow-mmap
      hypothesis. **Resolved design (operator-confirmed 2026-08-10)**: - **Action on low headroom: skip and continue**
      (not pause-and-poll — risks reintroducing the exact silent-hang failure mode this doc is about if the degradation
      never releases; not terminate-and-relaunch — bigger scope, needs wrapper/launcher changes beyond this loop). Log a
      new
...
- **2026-08-10 (main, Claude Code session) — implemented + shipped the corrected P3 todo.
  `deployment-service@ca2ecd5706` (`quickmerge`, landed on `live-defi-rollout`, quality-gates.sh green).**
  `mtds_chunk_loop.sh`'s generator (`setup-data-pipeline-vm.sh`) now reads `/proc/meminfo` (via a new
  `MTDS_MEMINFO_PATH` test seam, defaults to the real path) immediately before spawning each subprocess in the
  `for LEAGUE` loop; below `VM_MEM_FLOOR_MB` (new `_meta` default 3000, same override convention as `VM_CHUNK_DAYS`) it
  logs `CHUNK_SKIPPED_LOW_MEMORY` and `continue`s instead of spawning, setting `HAD_FAILURE=1` so the checkpoint is
  suppressed like any other gap; every check (skip or not) also logs `Dirty:`/`Writeback:` alongside `MemAvailable` —
  the P1 diagnostic bundled per the resolved design above. 4 new pytest regression tests (`TestMtdsChunkLoopMemoryGate`
  in `tests/unit/test_vm_launcher_scripts.py`, extending the existing real-heredoc-extraction harness so they can't
  drift from the shipped script) cover: low-memory skip with no subprocess spawned, healthy-memory pass-through with the
  diagnostic fields logged, fail-open on an unreadable meminfo path, and per-league skip isolation (both leagues in a
  fanned-out chunk skip independently, matching the existing OOM shard-isolation behavior). Full `deployment-service`
  `quality-gates.sh` run green (3221+4 passed, 0 failed). Also fixed 7 pre-existing test failures this change caused in
  the existing `TestChunkLoopPartialPayloadLossGating`/`TestMtdsSportsPerLeagueFanOut` harness (needed `VM_MEM_FLOOR_MB`
  added to its generator-time variable list — an unbound-variable failure, not a logic bug in the gate itself).
  **Note**: the `cefi_coverage_chunk_loop.sh` generator (the separate `VM_TASK=cefi-coverage-backfill` branch) was NOT
  touched — the todo's scope named `mtds_chunk_loop.sh` specifically, which is the SAME generator/for-loop the original
...
- **na-eligibility-audit 2026-08-17** [body-hash:934e3753d8971620]: KEEP-NA, valid -- 2 open todos confirmed via grep, matches inventory. Both are the same unresolved native-memory OOM root-cause investigation thread (a P1 item from 2026-08-07 covering the sports odds_api backfill leak, and a sharper 2026-08-14-dated follow-on next-step created after a memray capture failed to reproduce the blowup at small scope). Extensive Progress Log history (2026-07-31 through 2026-08-14) shows multiple hypotheses tested and refuted (session-reuse fix shipped but did not reduce RSS; FixtureIdResolver cache ruled out; per-VM-shard-growth mechanism ruled out; a memray --native single-league/5-day capture came back import-time-dominated, not reproducing the failure) with the true root cause still genuinely unknown -- explicit iterative diagnostic/investigation work, not a bounded deterministic task. Neither item is stale or duplicative of anything else in this batch; the two checkboxes overlap in topic (umbrella investigation vs. its current concrete next step) but neither is 'done elsewhere' with hard evidence.
- **na-eligibility-audit 2026-08-17** [body-hash:15af5350e23d6458]: KEEP-NA, valid -- re-verified, 2 open todos confirmed via grep (matches inventory's open_todos=2). Both are the same unresolved native-memory OOM root-cause investigation thread the 2026-08-17 marker already covered (sports odds_api backfill leak, memray re-run at reproducing scale still pending; PROGRESS.json monitoring-bug residue). Extensive multi-week investigation history (2026-07-22 through 2026-08-17) with multiple hypotheses tested and refuted; true root cause still genuinely unknown -- explicit iterative diagnostic work, not a bounded deterministic task. Cross-cutting tranche audit.
