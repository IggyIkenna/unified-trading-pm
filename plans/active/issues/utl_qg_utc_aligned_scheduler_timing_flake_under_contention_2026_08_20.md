...
status: open
archive_exempt: true # BRIDGE: land active-path checkbox first for cross-repo server verification; archive in a separate follow-up commit.
nature: issue
...
- **Archive bridge:** `archive_exempt: true` is temporary so the active-path checkbox commit can land before the required separate archival `git mv`.
