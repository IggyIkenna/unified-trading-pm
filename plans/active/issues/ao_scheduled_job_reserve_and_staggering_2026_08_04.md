...
- [x] ✅ [DATA] P2. Verify the capacity QUEUE end-to-end on live traffic (agent-orchestrator@5087f30, deployed
      ...
      **VERIFIED LIVE 2026-08-09** against the live SQLite state (S3 DR backup
      ...
      agent-orchestrator)
- [x] ✅ [DATA] P3. Same-day timer refire after its `<job>:<tranche>:<day>` row already dispatched reported a misleading
      `status="queued"` (a no-op read as real pending work on the dashboard) — **SHIPPED agent-orchestrator@a38423e**
      (verified on `origin/live-defi-rollout`; slot-33 flip — the fix commit landed 2026-08-11 21:37 without its
      checkbox flip, Half-1-without-Half-2). `_queue_for_capacity()` (`server/plan_health.py:455`) now returns
      `f"already_{row.status}_today"` (e.g. `already_dispatched_today`) on `queue_scheduled_job()`'s no-op branch,
...
- [x] [DOC] P3. ✅ **DONE 2026-08-14.** Added a new subsection "Pane-guard-before-cap-branch ordering invariant
      (2026-08-06, `agent-orchestrator@9d26598`)" to `/codex/04-architecture/agent-orchestrator-worker-liveness.md`,
      after the existing "Spawn-heartbeat watchdog hardened" bullet that already references
      `_auth_failover.check_spawn_heartbeat_timeouts`. Verified the live function directly
      (`agent-orchestrator/server/worker_liveness/_auth_failover.py:54-181`, `9d26598` confirmed ancestor of
      `origin/live-defi-rollout`): `_diagnose_unbooted_pane` runs unconditionally once a slot passes the
...
- [x] ✅ [OPERATOR] P1. **RE-INSTALL ALL TIMER UNITS on the orchestrator VM — DONE 2026-08-12 (operator, via SSM,
      `sudo -u ubuntu -H` — no sudo on the installer itself, per the 2026-08-12 correction).** Ran all 10
      `scripts/install-*-timer.sh` (ag-closeout-auditor, cefi-mtds-smoke, cefi-reconciliation, ci-reconciler,
      context-scout, data-pipeline-alerts-reconciler, docs-reconcile, escalation-queue-reconciler,
      na-eligibility-auditor, plan-reconciler). All 10 confirmed `active (waiting)` with correctly-loaded unit files and
      fresh dispatch scripts under `/home/ubuntu/.local/bin/`, picking up every fix shipped after the stale 2026-08-06
      15:04 UTC install (agent-orchestrator@5f15d0a shard+Saturday, @4a77bfe shared-guard+`ui`+UTC-day-check). The live
      fleet now runs the current dispatch scripts. (repo: agent-orchestrator)
