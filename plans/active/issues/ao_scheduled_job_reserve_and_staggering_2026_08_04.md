...
- [x] ✅ [DATA] P3. Track residual `reaped-stale` after the fix: on 08-06, 3/20 auditor runs still ended reaped-stale
      ...
      /done failure. — **CLASSIFIED 2026-08-11 (slot 18, data_engineering): genuine mid-run deaths, not post-work /done
      failure.** Live AO API query (`/api/agents?include_finished=true&limit=200`): 73/200 (36%) `reaped-stale`, **0
      have `done_evidence`** (label-defect class extinct post @2f485e3). All 73 never called `/done`; 55% die <10min
      (spawn/early-session), median=8min, 19% >1h (max 3.5h); 73/73 have transcripts. Raw count climbing (3→12→11→25→ 20
      over 08-07→11) but volume grew ~8x (per `-020`'s 08-09 re-check). Instrumentation (@0c27963) is live; a worker
      with VM access can correlate transcripts + journalctl for per-death mechanism. (repo: agent-orchestrator)
