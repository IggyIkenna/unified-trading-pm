...
- [x] ✅ [BACKEND] P3. **CONFIRMED 2026-08-10 (slot 5, infra) + closed 2026-08-11 (slot 18, data_engineering).** SIGABRT
      recurred 6+× 2026-08-10T22:05-22:16Z (revisions 00515/00516/00517) during venue-year-coverage cefi requests.
      Faulthandler dump reached Cloud Logging (stdout blackout fixed by `e8ce86a`'s JSON formatter) and identified the
      `abort()` call site: `_live_coverage_venue_year.py:186` = `df.apply(_classify, axis=1)` in
      `_process_manifest_chunk` — pandas' per-row `.apply()` triggered the `abort()` under the memory pressure of an
      unfiltered full-manifest read. **Fix shipped**: `deployment-api@fb3df79` replaced `df.apply(_classify, axis=1)`
      with vectorized column-level operations (~100× faster, comment in source). Preceding fixes
      `deployment-api@049d8d5` (bound manifest reads by yearly date_window) and `deployment-api@3d72470` (stream
      manifest reads row-group-at-a-time) addressed the OOM/SIGKILL trigger path that exposed the SIGABRT during
      unfiltered cefi manifest loads. Root cause: pandas internals calling `abort()` on a per-row `.apply()` over a
      large row-group under memory contention — not a gunicorn or gVisor defect. See also
      `/plans/active/issues/venue_year_coverage_cefi_oom_deployment_api_2026_08_09.md` for the full OOM investigation.
      (repo: deployment-api)
...
> **2026-08-11 line-cap remediation (6th pass)**: the 4 2026-07-31 Progress Log entries (slots 8/6/14/7 — blackout
> bootstrap, e8ce86a rollout refutation, IAM investigation + diagnostic sink, cold-start RESOLVED) extracted verbatim to
> `/plans/archive/2026_07/deployment_api_sigabrt_crash_loop_progress_log_history_2026_07_31.md` § "6th-pass extraction".
> All fully resolved; refer to that file for detail.
...
- **2026-08-11 (slot 18, data_engineering)** — Dispatched `deployment_api_sigabrt_crash_loop-032` (the `[BACKEND] P3`
  "investigate WHY gunicorn WORKERs call abort()" todo). Investigation already complete from the 2026-08-10 recurrence
  (Progress Log entry above): faulthandler dump identified `_live_coverage_venue_year.py:186` =
  `df.apply(_classify, axis=1)` in `_process_manifest_chunk` — pandas' per-row `.apply()` calling `abort()` under memory
  pressure from an unfiltered full-manifest read. Fix shipped same-day: `deployment-api@fb3df79` vectorized `_classify`
  to column-level ops. Preceding fixes `deployment-api@049d8d5` (yearly date_window bound) and `deployment-api@3d72470`
  (row-group-at-a-time streaming) addressed the OOM/SIGKILL trigger path. Flipped checkbox with confirmed root cause +
  shipped SHA. No new code (fix already landed 2026-08-10).
