...
- **2026-08-01 (slot 11, backend_engineer, dispatch `tradfi_backfill_oom_remediation-001`)**: this doc's open
  `[TRADFI] P3` todo above ("Fix the pyarrow per-symbol-writer fan-out...") was dispatched as a single 1h-class backlog
  task. Raised a BLOCKED-question on whether to attempt the full rewrite inline; main ruled: do NOT attempt the full
  rewrite from this single dispatched task — author a properly-scoped, phased plan with gated, independently-shippable
  todos instead, and leave this backlog item alone pending that re-scope. Per that ruling, the P3 todo's own ambiguous
  "either/or" framing (batch symbols onto a shared writer vs. cap concurrently-open writers) is now tracked as a
  dedicated phased plan —
  ...
  (+ gated [`_finalize`](/plans/active/tradfi_pyarrow_writer_fanout_remediation_2026_08_01_finalize.md)) — which splits
  the work into a bounded, contract-preserving tuning todo (dispatchable now) and an explicitly-deferred
  structural-options design pass (the two candidate approaches both change `PartitionedTickWriter`'s one-file-per-symbol
  GCS output contract, which BigQuery external tables + the manifest shard atom depend on). The P3 checkbox above is
  intentionally left as-is (not flipped, not reworded) — the new plan's own finalize step reconciles it once its todos
  land. Cross-referenced in `tradfi_backfill_throughput_followups_2026_07_24.md`'s bundled OOM digest line.
