...
- [ ] [INFRA] P1. **Verify the CBOE/VX log-flood OOM fix holds on a real relaunch.** Relaunch
      `tradfi-bf-cboe-ohlcv-1m-vx-2026-*` (via `deployment-service/scripts/vm/launch-tradfi-bf-cboe-ohlcv-1m.sh` or let
      the standing wave-launcher pick it up) now that `market-tick-data-service@92037f45` is live; confirm via
      `gcloud compute instances describe`/deployment registry that this run does NOT OOM-kill again and
      `PROGRESS.json`'s `last_completed_date` advances monotonically to completion (or a genuine data-boundary, not a
      repeat crash). If it STILL OOMs, the log-flood was a contributing factor but not the sole cause — re-open
      investigation (candidate: the underlying per-row spread-drop processing itself, independent of logging, may still
      be doing wasteful work per unclassifiable row). Target repo: `deployment-service` (relaunch) +
      `market-tick-data-service` (if a further fix is needed). Codex:
