...
- **2026-01-02**: request issued 00:25:53Z but — unlike day 1, which explicitly logged
  ...
  pattern recurs, for future debuggability — not fixed this session, out of scope for a live VERIFY run).
- **VM left RUNNING at session-suspend time** (not killed) — this is a real, lease-respecting wait, not a crash or a
  hard 403 rejection; no reason to kill a legitimately-waiting VM. Whoever next reads this doc should check
  `opt-deribit-combo-2026`'s live state first (`gcloud compute instances describe`, then SSH `tail /tmp/vm-exec-*.log` —
  the GCS-teed copy lags real-time and this VM name has a known stale-leftover-log gotcha, verify via `gsutil stat`
  generation before trusting its content) before deciding whether to let it keep running, or kill + relaunch once the
  bitget-futures wave finishes and releases sole claim to the lease.
...
classification, tarball currency). The sole remaining obstacle is ordinary lease contention against a legitimate,
concurrently-running production wave — not a code defect, not one of this doc's tracked P0s. Did not reach a
non-zero-row confirmation this session; the `[VERIFY]` top-level checkbox stays open pending either this VM's own
eventual lease turn, or a relaunch once the bitget-futures wave completes.
