...
- [ ] [VERIFY] P1. Test DERIBIT-COMBO via its UAC-declared data types (`--data-types trades` and/or `book_snapshot_5`,
      per-instrument against real combo symbol IDs — Tardis's `api.tardis.dev/v1/exchanges/deribit` metadata lists
      68,847 combo-type symbols total, 64,754 BTC/ETH-based, 53,509 already available by 2026-01-02), NOT via
      `launch-targeted-options-chain-backfill.sh` (which is `options_chain`-only). If real non-zero rows land via
      `trades`/`book_snapshot_5`, that confirms the options_chain-specific todo wording is simply the wrong test and
      should be re-scoped/closed on that basis (operator decision — this changes the todo's own literal acceptance
      criterion, flagging per the big-finding/data-correctness triage rule rather than silently re-defining "done"
      myself). If `trades`/`book_snapshot_5` ALSO show 0 real rows on multiple real dates, that would be a much stronger
      signal of a genuine capture-path defect worth a dedicated investigation. (repo: market-tick-data-service,
      deployment-service)
