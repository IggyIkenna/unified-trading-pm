...
- **na-eligibility-audit 2026-08-17** [body-hash:d3aa891505ff9483]: KEEP-NA, valid -- PM BATS/tmux shared-host wedge issue -- 3 of 5 original fixes already shipped and verified (bounded timeout + dedicated-socket fixes at unified-trading-pm@3895be718f, plus an operator host sweep). The 2 remaining open todos are both explicitly phrased as open design questions: 'Make the claim/heartbeat behaviour under test injectable' (requires designing a mock/injection seam for tmux-server-dependent test fixtures -- no single mechanical approach specified) and 'Consider whether the QG host governor should account for load average / MemAvailable' (literally an open architecture question about the governor's resource-accounting design). Neither is a bounded, worker-determinable outcome.
