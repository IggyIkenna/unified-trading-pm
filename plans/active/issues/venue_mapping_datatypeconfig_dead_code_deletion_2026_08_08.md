...
- [ ] [SCRIPT] P3. Re-grep `unified-api-contracts`, `unified-trading-library`, and `market-tick-data-service` for any
      read of `DataTypeConfig` / `.instrument_data_types` beyond the passthrough re-exports and the one unit test named
      above (confirm the zero-call-site premise still holds at execution time). Repo: all three.
- [ ] [SCRIPT] P3. Delete `DataTypeConfig` from `unified-api-contracts/unified_api_contracts/registry/venue_mapping.py`
      ...
      unified-api-contracts, market-tick-data-service, unified-trading-library.
- [ ] [SCRIPT] P3. Delete `TestDataTypeConfig` from
      ...
      `DataTypeConfig`/`instrument_data_types` in any of the three repos.
