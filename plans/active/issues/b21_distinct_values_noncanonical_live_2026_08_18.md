...
- [x] ✅ [DATA] P2. Extend `SPORTS_ODDS_API_ACCEPTED_NONCANONICAL_BOOKMAKERS` / add a new registry export for the
...
- **2026-08-21 (slot 26, data_engineering) — bookmaker exception closeout:** confirmed the 12 remaining spellings against the UAC Odds API key/region maps; extended the non-canonical exception registry without promoting them to canonical sports venues, and retained panel rows while excluding them from the actionable drift count. Evidence: `unified-api-contracts@3767b5b7`, `deployment-api@34c0f373`; both repositories passed `quality-gates.sh --no-fix` (deployment-api: 5,414 passed, 11 skipped).
