...
"in `market-tick-data-service`" for all 5 finding-classes; two of them are now known NOT to be MTDS bugs:
...
`manifest_hygiene_red_2026_07_06.md` (same script, same cefi asset_group) for the full root-cause writeup. A worker
picking up this todo should NOT re-diagnose these two classes as an MTDS bug — `schema_version_not_v9`,
`oracle_expects_but_empty`, and `noncanonical_path_on_disk` are NOT addressed by that fix and remain open diagnosis
targets as originally written.

(was: the todo below presented as an undifferentiated 5-class MTDS diagnosis target with no cross-reference to the
since-shipped audit-script fix above.) Checkbox intentionally left unflipped below — this is a scope-narrowing
annotation, not a completion claim; 3 of the 5 classes are still open and undiagnosed.

## Todos

- [ ] [CODE] P1. Manifest hygiene RED — 1 AG(s) with findings (2026_06_29) — diagnose + fix the root cause
