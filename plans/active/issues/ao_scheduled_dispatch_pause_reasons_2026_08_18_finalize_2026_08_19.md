...
- [ ] [REVIEW] P2. Reconcile: confirm the `reason`/`paused_at` field landed with a cited commit, and that
      `GET /api/scheduled-dispatch/status` + the dashboard's pause UI actually surface both for the still-paused
      `ag_closeout`/`cefi_mtds_smoke` modes (a live check, not just a code read) — this is the doc's own stated
      acceptance bar for the fix.
- [ ] [DOC] P2. Once reconciled AND `ag_closeout`/`cefi_mtds_smoke` have themselves been resumed or the doc's
      unblock-when conditions otherwise resolve (whichever is later — do not archive while either mode is still
      genuinely paused with this doc as the only record of why), run the standard 6-step archival ritual on
      `ao_scheduled_dispatch_pause_reasons_2026_08_18.md`. If the two modes are STILL intentionally paused when the
