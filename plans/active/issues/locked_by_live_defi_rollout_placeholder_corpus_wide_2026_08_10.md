...
- **na-eligibility-audit 2026-08-17 (cross-cutting tranche, dispatch agt-3931fd)** [body-hash:433905a7777dbda9]: KEEP-NA, valid — IMPORTANT FINDING: the citing sibling (operator_ruling_record_plan_reconcile_session_2026_08_15.md Item 2) claims the corpus-wide clear ran via scripts/plans/clear_locked_by_placeholder_2026_08_12.py --apply with only '2 remaining docs' as of 2026-08-15 -- but a fresh corpus grep run this session (2026-08-17) finds 52 docs still carrying the literal placeholder, including at least 2 created 2026-08-16 (after both the script run and the ruling), confirming the root-cause doc-creation template was never found/fixed and the placeholder keeps recurring. The only substantive open item is genuinely still needed and is repeatedly, explicitly framed in this doc's own text as HARD-STOP/operator-only ('clearing locked_by is itself an unlock action... operator-only') -- stays NA despite being mechanically simple.
