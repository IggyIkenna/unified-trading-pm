...
summary:
  ...
  citation not in the recorded baseline set: plans/active/sports_satellite_ao_dispatch_batch2_2026_07_24.md:769 cites
  `unified-trading-pm@9022488a2` (PR #1492), and that SHA does not resolve anywhere in this repo's local git history
  (`git cat-file -e`, `git rev-list --all | grep`, both empty). This blocks `bash scripts/quality-gates.sh` (and
...
```
plans/active/sports_satellite_ao_dispatch_batch2_2026_07_24.md:769: [todo] unified-trading-pm@9022488a2
```
...
`plans/active/sports_satellite_ao_dispatch_batch2_2026_07_24.md`, and that doc's entry is dated
...
- [x] ✅ [SCRIPT] P2. Resolve PR #1492 via `gh pr view 1492 --repo IggyIkenna/unified-trading-pm` (or equivalent) to
      ...
      finding cited at `plans/active/sports_satellite_ao_dispatch_batch2_2026_07_24.md:769`. Correct the bad citation
      (repo `unified-trading-pm`, cited SHA `9022488a2` — note the space, not `@`, to avoid this todo itself matching
      the very citation regex it's about) to the real resolvable SHA, or annotate it unverifiable if PR #1492 doesn't
      exist. Re-run `python3 scripts/quality_gates/check_plan_commit_sha_evidence.py --workspace-root $WORKSPACE_ROOT`
      to confirm the corpus is back to 22 unresolvable citations (the pre-existing baseline, not a re-baselined 23).
