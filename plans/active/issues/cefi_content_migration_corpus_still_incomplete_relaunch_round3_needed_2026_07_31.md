...
- [ ] [DIAG] P1. **RULED 2026-08-06 (operator), option (b): check the sibling memory-freeze root-cause fix before
      spending more relaunch budget or bumping machine size.** `[DIAG]` tag (was `[OPERATOR]`) — check whether a related
      memory-freeze root-cause doc already closes this for shards 16/17/21/41 before escalating to `e2-standard-32` or
      waiting out the budget reset; only bump machine type if that check comes back negative. **Shards 16, 17, 21, 41
      are now over `RB-INFRA-RELAUNCH`'s `≤2/(vm-prefix,day)` budget** (queried live via
      `DeploymentsRegistry.list_recent_archive(days=1)` 2026-07-31T13:35Z: shard 16=2, shard 21=2 archived today — AT
      cap; shard 17=3, shard 41=3 archived today — OVER cap) and were correctly NOT relaunched this round. All four died
      again on their most recent (pre-round-3) attempt showing the SAME symptom — repeated
