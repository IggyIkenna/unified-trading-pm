...
> `/plans/archive/issues/mtds_live_smoke_vm_not_tardis_guarded_2026_07_28.md` both touch the Tardis-guard machinery.
> They now own **disjoint** files: **this doc** owns `tardis-concurrency-guard.sh`'s exempt logic (DONE —
...
- **context-scout 2026-08-06**: re-scouted; context_scope re-verified (4 entries), unchanged. All todos are now closed
  (0 open checkboxes) — a completion-candidate for a future archival pass, not this skill's scope.

## Follow-ups

- [x] ✅ [DATA] P3. **DONE 2026-08-08 (cefi_mtds_smoke_tester, agt-9e871f, slot 8)** — exercised during a genuine live
      MID-BACKFILL window. Scheduled
      `/data-pipeline-check-mtds --day 2026-08-07 --asset-group CEFI --legs force,skip     --mvp-only --require-captured --auto-day`
      run (driver VM `pipeline-e2e-check-mtds-20260808-022238-ce5cd3`) overlapped a genuinely live Tardis-consuming
      production VM the whole ~63min it ran: `cefi-deribit-2019-light-20260807-194407` (confirmed via
      `gcloud compute instances list`, running since 2026-08-07T19:45:41Z, still RUNNING after my driver VM
      self-deleted). Result confirms **both halves of P1's fix are still correct**: (a) every genuine Tardis-sourced
      venue attempted (BINANCE-\*, BYBIT, BITFINEX-\*, BITGET-\*, DERIBIT, OKX-\*, UPBIT — ~30 distinct venue/data_type
      cells, ~89 launch attempts) was correctly refused by `tardis_concurrency_guard` (root cause confirmed in source,
      `tardis-concurrency-guard.sh:94`/`:286`-`:297`, `TARDIS_MAX_CONCURRENT_VMS=1`; the "5 streams (default 4 — its own
      cap)" text visible in `run.log` is a truncated fragment of the guard's own usage/help dump, NOT a distinct second
      cap being breached — don't misread it as one); (b) the CAP-EXEMPT venue present in this run's matrix,
      **COINBASE-CDE**, was correctly UNAFFECTED — its one real cell (`trades`) passed cleanly, force AND skip both
      `EXIT_STATUS=0` (`mtds-backfill-cefi-pipelinecheck-20260808-030832-fedea6` /`-031122-fedea6`), launched without
      any guard refusal despite the concurrent Tardis VM. This closes the exact gap this doc's own P2 evidence flagged
      as unexercised ("Tardis venues still correctly refused... needs a genuine MID-BACKFILL window, which wasn't
      available" — now available and confirmed). No code change needed — P1's fix holds.

- [x] ✅ [DATA] P1. **NEW 2026-08-08** — separate, adjacent gap surfaced by the same run above: when every
      Tardis-sourced cell in a `pipeline_e2e_check.py --asset-group CEFI` run gets refused by the guard, the CHECKER's
      own retry (3 attempts, 5s apart — `unified_trading_library.pipeline_e2e_check`'s `launch_vm_and_wait` retry
      wrapper, NOT the guard itself) exhausts almost immediately relative to how long a real production Tardis backfill
      can hold the N=1 slot (this run's contending VM had already been running ~7h and was still running afterward) —
      the checker then aborts the ENTIRE process (`rc=3`) rather than recording that one cell
      `skipped/tardis_guard_busy` and continuing to the next cell. Practical effect: a daily-scheduled
      `cefi_mtds_smoke_tester` run that happens to overlap ANY real Tardis-consuming backfill anywhere in the fleet gets
      essentially zero cefi Tardis-venue coverage for that whole day, with no partial-progress report — worse than the
      CAP-EXEMPT bug this doc fixed, because it's not venue-scoped noise, it's a full-run abort. Suggested fix (not
      attempted here — real code change, outside a smoke-tester's one-shot scope): treat a guard refusal as a per-cell
      `skipped` outcome (consistent with how `--require-captured` already turns "unprovable" into a recorded skip rather
      than a hard failure) instead of letting `launch_vm_and_wait`'s exhausted-retries path propagate into a
      process-level exit. Evidence: full `run.log` for `pipeline-e2e-check-mtds-20260808-022238-ce5cd3` (178×
      guard-refusal retries logged, final line `[vm-exec] command exited rc=3` at 2026-08-08T03:25:51Z after 63min). —
      market-tick-data-service@41bf3869c; QG: 10140 passed 80.70% coverage; 2 new regression tests
      (TestTardisGuardBusySkip); _verify_batch_shard + _run_skip_leg both fixed.

> **2026-08-06 archive-candidate audit**: All 3 todos are [x] and P1 shipped (deployment-service@2d6b01a) with P2
> proving zero guard refusals for the 4 CAP-EXEMPT venues. But P2's own text explicitly says its 'Tardis venues still
> correctly refused' half 'stays open as a standing verification gap' — open verification work described in prose. The
> doc argues 'no new todo needed', but the todo's full done-when was never exercised; conservative bias -> NEEDS_TODO.

- **cefi_mtds_smoke_tester 2026-08-08** (day=2026-08-07, slot 8): flipped P3 (the exact gap the 2026-08-06 audit
  flagged) using real evidence from a genuine MID-BACKFILL collision this run hit organically — see P3 above for the
  full evidence. Added P4 as a new, still-open, distinct finding (checker aborts the whole run on sustained guard
  contention instead of per-cell-skipping). Doc is NOT fully archival-eligible yet (P4 open) — leaving archival to a
  future pass once P4 is resolved or explicitly deferred.
