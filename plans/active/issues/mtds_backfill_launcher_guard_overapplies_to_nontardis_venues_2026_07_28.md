...
- [ ] [DATA] P1. **NEW 2026-08-08** — separate, adjacent gap surfaced by the same run above: when every Tardis-sourced
      cell in a `pipeline_e2e_check.py --asset-group CEFI` run gets refused by the guard, the CHECKER's own retry (3
      attempts, 5s apart — `unified_trading_library.pipeline_e2e_check`'s `launch_vm_and_wait` retry wrapper, NOT the
      guard itself) exhausts almost immediately relative to how long a real production Tardis backfill can hold the N=1
      slot (this run's contending VM had already been running ~7h and was still running afterward) — the checker then
      aborts the ENTIRE process (`rc=3`) rather than recording that one cell `skipped/tardis_guard_busy` and continuing
      to the next cell. Practical effect: a daily-scheduled `cefi_mtds_smoke_tester` run that happens to overlap ANY
      real Tardis-consuming backfill anywhere in the fleet gets essentially zero cefi Tardis-venue coverage for that
      whole day, with no partial-progress report — worse than the CAP-EXEMPT bug this doc fixed, because it's not
      venue-scoped noise, it's a full-run abort. Suggested fix (not attempted here — real code change, outside a
      smoke-tester's one-shot scope): treat a guard refusal as a per-cell `skipped` outcome (consistent with how
      `--require-captured` already turns "unprovable" into a recorded skip rather than a hard failure) instead of
      letting `launch_vm_and_wait`'s exhausted-retries path propagate into a process-level exit. Evidence: full
      `run.log` for `pipeline-e2e-check-mtds-20260808-022238-ce5cd3` (178× guard-refusal retries logged, final line
      `[vm-exec] command exited rc=3` at 2026-08-08T03:25:51Z after 63min).
