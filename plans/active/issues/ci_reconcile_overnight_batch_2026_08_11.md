...
- [ ] [CODE] P3. **[BLOCKED-PERMISSIONS] Fix the 7 failing `github-glue-slot-refresh-*` systemd units** on host
      ...
      affect the live glue runners themselves, but is an unattended-failing component retrying every ~10 min. (repo:
      unified-trading-pm, host infra) **Root-caused + code fix shipped 2026-08-15 (slot-19)**:
      `unified-trading-pm@32da67cce1` — `gh repo clone` only authenticates its own one-shot invocation; it never wires
...
- **na-eligibility-audit 2026-08-17 (cross-cutting tranche, dispatch agt-3931fd)** [body-hash:e37fc89d1e2e07b9]: KEEP-NA, valid — 3 remaining Follow-ups. The 2 CODE-tagged AO-wall_type additions are explicitly self-characterized by the doc's own text as 'a design change, not a same-pass mechanical fix' needing 'a routing decision,' and were deliberately NOT actioned in-band, instead flagged per the findings-triage NOTIFY-OPERATOR big-finding rule as a cross-cutting AO-architecture gap -- the doc's own authors already made this judgment call in real time rather than treating it as mechanical. The 3rd (systemd units) has its code fix already shipped (unified-trading-pm@32da67cce1) but live application is genuinely blocked on a different session/identity having SSM access this session's identity was directly confirmed to lack (non-self-service, unlike the trading GCP/AWS SAs' documented self-service path). Ruling/citation: Doc's own text on Structural Findings A/B: 'adding a new wall_type needs its own escalation-worker prompt template + routing decision, a design change, not a same-pass mechanical fix' -- flagged per the findings-triage HARD RULE ('big finding... cross-cutting... NOTIFY OPERATOR') rather than attempted in-band.
