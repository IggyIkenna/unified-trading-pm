...
- [x] ✅ [DATA] P2. **DONE 2026-08-03 (slot-16) — old gap CLOSED for 3/4 protocols' `dex_pool_state`; a NEW recent-week
      gap opened; `dex_pool_swaps` patchy-improved but TRADER_JOE_V2 historical fill genuinely failed.** Re-ran the same
      GCS-object-existence spot-check method (144 combos: 4 protocols x 2 data_types x 18 dates spread 2026-03-01
      through 2026-08-03) against
...
- [ ] [DATA] P2. **New (2026-08-03).** Close the fresh live-edge `dex_pool_state` coverage gap: UNISWAP_V2, UNISWAP_V4,
      and VELODROME_V2 have zero captures for every date from 2026-07-27 through today — the currently-running
      `mtds-dex-pools-backfill` VM is bounded `--end 2026-07-24` and nothing else appears to keep `dex_pool_state`
      current going forward. Either launch a follow-on incremental backfill covering 2026-07-25→today (and repeat
      periodically until a permanent live/daily capture exists), or confirm/wire a live capture path. Repo:
      market-tick-data-service / deployment-service.
- [ ] [BACKEND] P2. **New (2026-08-03).** TRADER_JOE_V2 `dex_pool_state` coverage gap (roughly 2026-03/05 through today)
      never actually closed despite the relaunched `mtds-dex-pools-backfill` VM running for 9+ days — every other of the
      4 target protocols shows real recent captures, TRADER_JOE_V2 does not (17/18 sampled dates ABSENT). Diagnose why
      this one protocol isn't landing `dex_pool_state` rows on the currently-running VM (schema/adapter issue analogous
      to the swaps-side cascade bug, or a catalogue/pool-list gap) — this is a `dex_pools_handler.py` question, distinct
      from the already-tracked `dex_swaps_handler.py` cascade findings. Repo: market-tick-data-service.
- [ ] [BACKEND] P2. **New (2026-08-03).** Re-verify TRADER_JOE_V2/AVALANCHE's `dex_swaps_handler.py` cascade against
      CURRENT code (post `market-tick-data-service@74cd6cfd`, 2026-07-30): the `mtds-dex-swaps-historical` VM's real run
      (2026-07-27T22:25, pre-dating that fix) shows the cascade logging only the 1st schema attempt
      (`messari schema drift`) then silently giving up on every shard-cycle — never attempting `messari_from` (the
      variant the 2026-07-27 P1 todo's live-probe claimed works) or the other 3 fallbacks, and never logging a
      `Failed to collect swaps` warning. This is a different shape than the already-fixed bad-indexers class. If still
      reproducible on current code, this — not indexer health — may be the real reason TRADER_JOE_V2 `dex_pool_swaps`
      shows 0% for its entire pre-2026-08 historical range. Repo: market-tick-data-service.
- [ ] [DATA] P2. **New (2026-08-03).** Re-run the `mtds-dex-swaps-historical` backfill (trader_joe_v2 + velodrome_v2,
      2023-01-01→2024-10-06) once the two todos above land — the prior run of this exact VM completed with
      `EXIT_STATUS=0` but produced literally ZERO real rows for both target protocols
      (`{'trader_joe_v2_AVALANCHE': 0, 'velodrome_v2_OPTIMISM': 0}`), so the historical gap for this date range is still
      fully open despite the previous todo's health-check-based DONE marking. Repo: deployment-service.
