...
- **na-eligibility-audit 2026-08-17 (cross-cutting tranche, dispatch agt-3931fd)** [body-hash:08284afbc4b338eb]: KEEP-NA, valid — Live, actively multi-session-worked issue doc mixing explicit unresolved [OPERATOR] items (close-all ruling sign-off, OTC auto-book decision, RecursiveLoopOrchestrator architectural placement) with judgment-laden AGENT build items on live-capital audit-trail/reconciliation/kill-switch-adjacent machinery — the rubric explicitly warns against reclassifying 'live-dispatch-critical-path machinery' even when self-framed as bounded, and this doc's remaining work is exactly that class (order/position/audit/reconciliation on a live-capital path).
