...
- **2026-08-11 (slot 1): `assigned_vm` corrected `planning` → `NA`.** Every remaining open todo here is operator-gated
  (BLOCKED-OPERATOR-DECISION — premise invalidated, needs an operator ruling), so AO can see nothing to dispatch — the
  doc was an `assigned_vm: planning` plan the orchestrator never touches, which is exactly the condition
