...
- [ ] [BLOCKED-CREDENTIALS][INFRA] P2. **D118 ruling applied 2026-08-21 (issues_corpus_completion_dispatch_2026_08_21.md
      ...
      parent plan's mirrored item for the full rationale. **OPERATOR-DECIDED 2026-08-11: provision egress from
      `europe-west2` (London), not `europe-west4`.** Reversing the 2026-08-10 "permanently parked" disposition: asked
      again, the operator declined to re-approve `europe-west4` but asked for whichever Betfair-accepted region matches
      the credential's actual jurisdiction. Confirmed the adapter authenticates against
...
- 2026-08-09 (slot-8, backend_engineer): dispatched to todo 4 (live-verify). **Premise invalidated, same pattern as
  ...
  **Left the created `betfair-session-token` secret container in place** (empty, zero versions — harmless, and it's the
  ...
  - **2026-08-11** (slot-20, backend_engineer): dispatched to the parent plan's original P2 item. Retagged todo 4 above
    ...
    worker.
  - **2026-08-12 (slot-16, infra): egress PROVISIONED + VERIFIED — new blocker surfaced (credential rejection).** The
    europe-west2 egress proxy is live (`betfair-egress-proxy-20260811-211046`; `deployment-service@7a7e847e` shipped
    `launch-betfair-egress-proxy-vm.sh` + registered the prefix — tinyproxy on :8888, firewall restricted to
    13.113.200.22/32). Verified the London geolocation is real (route through 35.230.151.99 → ipinfo "country: GB, city:
    London") and the geo-block is GONE — no more 403 "Restricted", Betfair now processes the request. Re-ran
    `execution-service/scripts/refresh_betfair_session_token.py` through the proxy: it reaches Betfair but fails
    `INVALID_USERNAME_OR_PASSWORD` — a genuine credential rejection, not network (no storage artifact: raw secret bytes
    are clean, no trailing whitespace). Disambiguated app-key-vs-password: non-interactive `login()` needs `/certs` (not
    provisioned — interactive was chosen precisely to avoid certs), so the only reachable path is `login_interactive()`,
    which rejects the credentials. **Remaining blocker is operator-gated**: either (a) the operator-provided
    `betfair-password` (provisioned 2026-08-03) is wrong/rotated, or (b) `betfair-app-key` is a DELAYED (free) developer
    key — `loginInteractive` requires a LIVE (paid) key and Betfair can surface a credential-shaped error for delayed
    keys. `/blocked` filed for the operator decision. Todo 4 stays unchecked (its done-when needs the secret populated +
    live-verify); the egress half is done and recorded here.
  - **2026-08-12 (slot-16, infra, re-run after main's interim guidance): diagnosis REFINED to a specific Betfair account
    state.** Confirmed no secret rotation since my first run (`betfair-password` v1 2026-08-03, `betfair-app-key` v1
    2026-04-22, `betfair-username` v2 2026-08-03 — unchanged). Re-ran `refresh_betfair_session_token.py` through the
    London proxy: Betfair now returns `ACCOUNT_PENDING_PASSWORD_CHANGE` (not the earlier
    `INVALID_USERNAME_OR_PASSWORD`). This is a Betfair account flag, not a credential-shape error — the account holder
    must change the password via the Betfair web portal (betfair.com → login → change password), then update the GSM
    `betfair-password` secret to the new value, before `login_interactive()` will be accepted. It narrows the A/B/C
    options decisively: not a wrong app-key (different error), and the account is now in a mandatory-password-change
    state rather than a plain wrong-password state. **Operator/account-holder action only — no worker can complete
