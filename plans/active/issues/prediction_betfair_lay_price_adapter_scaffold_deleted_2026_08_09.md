...
- [ ] [BACKEND] P2. **OPERATOR-DECIDED 2026-08-11: provision egress from `europe-west2` (London), not `europe-west4`.**
      ...
      Exchange API) and this workspace's account is a direct UK Exchange account (`BETFAIR_EX_UK` primary venue,
      `capital-structure-and-regulatory.md`) — `europe-west2` is the only GCP EU region with UK IP geolocation, which is
      the actual mismatch that got both Tokyo and `europe-west4` (Netherlands) 403'd; `europe-west1` (Belgium, cheaper)
      carries the same jurisdiction-mismatch risk already vetoed once, so not used. **Next step (not done this
      session)**: provision an egress path from `europe-west2`, re-run `refresh_betfair_session_token.py` against it,
      confirm the `betfair-session-token` GSM secret populates, then live-verify per the original done-when below. Prior
      context preserved: the `betfair-session-token` secret was NOT actually live (execution-service@7e03bf7b shipped
      only the refresh mechanism, never ran it) and, once provisioned, Betfair's interactive-login endpoint hard-rejects
      this workspace's ONLY current network egress (the single orchestrator VM's Tokyo/`asia-northeast1` IP) as
      geo/traffic-restricted. Live-verify: using a working `betfair-session-token` GSM secret, call the restored
      adapter's `get_markets()`/`get_prices()` (or the new `download_batch()`) against a real sampled in-play Betfair
      market, confirm both `backs` and a populated `lay_price` persist through the normal MTDS capture write path (not a
