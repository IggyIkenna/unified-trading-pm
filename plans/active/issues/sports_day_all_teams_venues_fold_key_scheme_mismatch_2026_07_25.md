...
resolved_by: "slot-13 (data_engineering), sports_day_all_teams_venues_fold_key_scheme_mismatch-001, 2026-08-03"
locked_by:
context_scope:
  [
    /codex/02-data/sports-2020-06-data-floor.md,
...
**✅ SHIPPED 2026-08-03 (option a).** `unified-api-contracts@c85ecdaa` added `SportsPathLayout.FLAT_PER_SEASON`
(`sports_reference/{F}/season={S}/{F}.parquet`) + the `TEAMS_SEASON_SNAPSHOT` data_type (folder `teams`, shared with the
unaffected daily `TEAMS` data_type — the two never collide on disk since `FLAT_PER_SEASON` vs `PER_DAY_PER_LEAGUE`
produce disjoint path shapes) + `candidate_parquet_paths()` builder support, with a locked-down regression test suite
(`tests/unit/sports/test_gcs_paths_teams_season_snapshot.py`). `instruments-service@a1d50768` then folded the archived
30,069-row `day=all` snapshot into the new layout (one parquet per season, 2019-2025) — see the updated Disposition
below for the run evidence. TEAMS's canonical home is no longer undefined.
...
**2026-08-03 — item (2) SHIPPED, doc RESOLVED.** Both todos now done: the schema half (`unified-api-contracts@c85ecdaa`,
`SportsPathLayout.FLAT_PER_SEASON` + `TEAMS_SEASON_SNAPSHOT`) and the data-migration half
(`instruments-service@a1d50768`) — the archived 30,069-row `day=all` TEAMS snapshot is now live at
`sports_reference/teams/season={2019..2025}/teams.parquet`, verified row-count-in == row-count-out (30,069/30,069, 0
aborted) via a live `--dry-run` then `--apply` run against prod, and confirmed idempotent (a second `--apply` correctly
skipped all 7 already-written destinations). Manifest registration was deliberately skipped for these rows —
`ManifestWriter.record_captured`'s `row_key` contract requires a `date`, which this genuinely season-keyed data
structurally has none of; forcing a fake date would repeat the exact "invent a label to fit an existing shape"
anti-pattern this whole ruling exists to avoid, and matches the only other FLAT-layout precedent (VENUES, which the live
writer also never registers in the manifest). No remaining open items — every root finding (1, 2, 3) is now either
shipped (1) or previously resolved via the executed Option-A delete (2, 3 — see todo 1's Progress Log). Archiving.
...
- [x] [SCRIPT] P2. **🟢 AUTHORIZED 2026-07-25 (operator, in-session) — Option A.** Delete
...
- [x] ✅ [DATA] P2. **RULED 2026-07-28 — Option A: add a net-new season-keyed UAC FLAT layout for TEAMS.** (1) Added
      `SportsPathLayout.FLAT_PER_SEASON` (`sports_reference/{F}/season={S}/{F}.parquet`) + the `TEAMS_SEASON_SNAPSHOT`
      data_type + `candidate_parquet_paths()` builder support, with a dedicated regression test suite —
      `unified-api-contracts@c85ecdaa`. (2) Built + ran the one-time fold script
      (`instruments-service/scripts/fold_sports_day_all_teams_season_snapshot_2026_08_03.py`): reads the archived
      `_legacy_archive/by_date/day=all/entity=teams/teams.parquet` backup, splits by `season`, writes each season to the
      new canonical path (resolved via the UAC SSOT, never a hand-rolled string), with a per-season already-exists guard
      (never silently overwrites) — `instruments-service@a1d50768`. (3) Verified live against prod: `--dry-run` then
      `--apply`, all 7 seasons (2019-2025) written + read-back-verified, **30,069/30,069 rows accounted for** (row count
      in == row count out), 0 aborted; a second `--apply` run correctly skipped all 7 already-written destinations
      (idempotent). Manifest registration deliberately skipped (see Disposition below for the reasoning —
      `record_captured`'s `row_key` requires `date`, which this data structurally has none of). (4) Updated Finding 1 +
      Disposition above to record the layout as shipped. (repo: unified-api-contracts, instruments-service)

## Progress Log

- **context-scout 2026-08-03**: reviewed context_scope (4 entries), no change needed — still accurate.
- **2026-08-03T~18:50Z** (AO dispatch, slot 13, `data_engineering`) — Shipped the last open todo (2 of 2). UAC schema
  half: `SportsPathLayout.FLAT_PER_SEASON` + `TEAMS_SEASON_SNAPSHOT` data_type + builder support + tests
  (`unified-api-contracts@c85ecdaa`). Migration half: wrote + ran
  `fold_sports_day_all_teams_season_snapshot_2026_08_03.py` against prod — `--dry-run` confirmed 30,069 rows across 7
  seasons matching this doc's own Finding-section byte/row counts exactly, then `--apply` wrote + verified all 7 season
  parquets (`sports_reference/teams/season={2019..2025}/teams.parquet`), 30,069/30,069 rows accounted for, 0 aborted
  (`instruments-service@a1d50768`). Confirmed idempotent via a second `--apply` (all 7 skipped as already-present). Both
  source repos' `quality-gates.sh` full runs green; both SHAs verified ancestors of `origin/live-defi-rollout`. Doc
  `status` flipped to `resolved`; archiving per issue-doc-lifecycle (all todos done, unlocked).
