...
- [ ] [BACKEND] P3. As a cheaper interim alternative to the above, investigate making the per-VM flush debounce
      entries-threshold dominant over the interval-threshold once the existing shard exceeds some size (e.g. skip the 5s
      time-based trigger and require a real entry-count batch once the shard is large), so large shards get bigger,
      less-frequent batches instead of constant small ones. Note this trades durability under SIGKILL for throughput —
      needs an explicit call on whether that tradeoff is acceptable, not a default flip. (repo: unified-trading-library)
