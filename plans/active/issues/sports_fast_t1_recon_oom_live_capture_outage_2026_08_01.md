...
> **2026-08-06 (slot 13, data_engineering) — duplicate wide-window catch-up VM OOM'd; backfill substance DONE; residual
> af = P2 operator decision.** The wide-window catch-up VM `mtds-backfill-odds-catchup-20260806` (03-28→08-06,
> skip-enabled, launched 2026-08-06 ~13:12Z by a prior worker on this todo) was OOM-KILLED (exit 137, RSS 30.7/32GB) at
> 13:29:02Z while at `SKIP date=2026-05-06` — ~~40/132 days in, all SKIPs, never reaching the gap days; the chunk loop
> swallowed the failure (rc=0, VM self-deleted). A wide single-chunk window re-creates the documented large-chunk OOM
> class (`mtds_backfill_vm_memory_hang_large_chunk_2026_07_22.md`); the sanctioned targeted `--force` window (slot 12's
> `mtds-backfill-odds-gap-20260806`, 07-27→08-06, rss~~509MiB) is the correct shape — no further VM relaunch. Fresh
> bounded manifest read (2026-08-06, source=odds_api, 10 days) confirms coverage UNCHANGED from the post-`-gap`-VM
> table: af counts identical (86/26/488/112/165/468/1082/144/42/5/743) and dominated by guard-rejected
> `record_empty(SOURCE_RETURNED_ZERO) rejected: ... ALIVE on <bookmaker>` = in-coverage expected-but-empty. A `--force`
> re-fetch provably does NOT reduce af (slot 12's VM demonstrated it). P1's backfill substance (re-fetch the gap via
> Odds-API historical endpoint) is executed and at its captured ceiling; literal "full coverage" is structurally
> unreachable without the operator's P2 classification decision (BLOCKED-OPERATOR-DECISION below, options A/B/C, rec C
> then A). P1 stays open, GATED on P2.

- [ ] [DATA] P2. **AF-classification decision (BLOCKED-OPERATOR-DECISION, filed 2026-08-06 slot 13)**: the OOM-gap
      residual `attempted_failed` is ~99% IN-coverage expected-but-empty — `record_zero_rows(was_expected=True)` →
      `EmptyFromLiveInstrumentError` from the fetch path's guard-rejected `record_empty(SOURCE_RETURNED_ZERO)`: the
      (bookmaker, league) pair IS in the UAC observed-coverage map, a fixture existed, and Odds-API returned a
      legitimate zero-row (HTTP 200) response. Per the operator directive these are correctly classified as fetch
      failures (not honest absence), so the 2026-06-21 relabel (`relabel_sports_odds_no_coverage_2026_06_21.py`,
      out-of-coverage → empty_confirmed) converts only 0-2% of them — literal "full coverage" (0 af) for the gap days is
      structurally unreachable without a decision on whether in-coverage expected-but-empty shards are honest absence
      (relabel risk: masking a real gap) or tracked failures (current). Options: (A) accept residual af as the honest
      terminal state for the backfill; (B) relabel in-coverage expected-but-empty → empty_confirmed; (C) vendor-verify a
      sample of af shards against the Odds-API first. **Recommendation: (C) then (A).** (repo: market-tick-data-service,
