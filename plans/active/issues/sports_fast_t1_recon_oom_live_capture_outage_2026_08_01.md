...
- [ ] [DATA] P2. **RULED 2026-08-06: vendor-verify first (refined option C) — fix root cause if vendor CAN return data,
      ...
      option B.** AO-dispatchable — **the AF-classification decision this todo originally opened with is RULED (option
      C, not open)**; do not re-litigate. Background: the OOM-gap residual `attempted_failed` is ~99% IN-coverage
      expected-but-empty — `record_zero_rows(was_expected=True)` → `EmptyFromLiveInstrumentError` from the fetch path's
      guard-rejected `record_empty(SOURCE_RETURNED_ZERO)`: the (bookmaker, league) pair IS in the UAC observed-coverage
      map, a fixture existed, and Odds-API returned a legitimate zero-row (HTTP 200) response. Per the operator
      directive these are correctly classified as fetch failures (not honest absence), so the 2026-06-21 relabel
      (`relabel_sports_odds_no_coverage_2026_06_21.py`, out-of-coverage → empty_confirmed) converts only 0-2% of them.
      Execute: vendor-verify a sample of `attempted_failed` shards against the live Odds-API (option C); once verified,
      accept the genuinely-confirmed-empty residual as the honest terminal state (option A). (repo:
...
**2026-08-02 (slot 9)** — precondition re-checked, root cause of the block identified (still not met): traced to the
fleet-wide self-hosted-runner capacity crisis (`fleet_wide_qg_self_hosted_runner_capacity_crisis_2026_07_27.md`),
already owned by a different craft. Full entry (moved 2026-08-09, line-cap remediation, no content change):
`/plans/archive/2026_08/sports_fast_t1_recon_oom_live_capture_outage_history_2026_08_09.md`.
...
**2026-08-09 (plan_reconciler agt-196785) — checkpoint window has passed, unobserved.** Today is past the 2026-08-08
t1h-trigger window above with zero follow-up entry recorded. Attempted a live manifest re-check
(`read_availability_index_safe` on the sports market-data bucket) but hit missing service-context env requirements
(`GCP_PROJECT_ID`) not available outside the deployed service's own runtime — not completed this pass. **Next resumer**:
re-run the day=2026-08-08 ODDS_API/odds_horizon_bucket manifest check from a proper service context (or via
`gcloud storage cat` on the raw index parquet) to confirm whether `505c538`'s fix is now observably holding.
