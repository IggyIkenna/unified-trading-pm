...
- **data_engineering worker (slot 5) 2026-08-04 (v7 relaunch, then session died; resumed same day)**: relaunched the backfill script. **Chunk 1/7**: completed on retry 3 (EXIT_STATUS=0, 636,632 rows, 3 shard parts). **Chunk 2/7 (2021)**: unexpectedly massive backlog — 4 batches completed while session was dead (VMs `153915`→`154443`→`155826`→`160551`, each EXIT_STATUS=5 with 1M rows), totalling ~4M+ rows for 2021 alone. A 5th batch VM (`161419`, EXIT_STATUS pending) is still RUNNING as of session resume. The backfill script process died when the session was killed, but the VM it launched survived. **Chunks 3-7 not yet reached.** Chunk 2's size suggests sports instrument catalog grew significantly from 2020→2021 (2020 had only ~7 months + partial catalog). Convergence working correctly — each retry excludes already-written per-VM shards. If picked up cold: check for RUNNING VM; if none, relaunch `bash deployment-service/scripts/vm/launch-expected-universe-v2-historical-backfill-vm.sh sports` (idempotent).
