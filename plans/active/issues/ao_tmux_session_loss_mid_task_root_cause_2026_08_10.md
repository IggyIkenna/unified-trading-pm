...
## Mechanism CONFIRMED, three candidate triggers now RULED OUT (2026-08-13)

**Confirmed** (strace, reproduced repeatedly): death is always a SELF-directed `kill(<own_pid>, SIGTERM)` (si_pid ==
traced pid) then clean `exit_group(0)` — never external SIGKILL/crash/OOM/cgroup, why no core dump ever exists even
post-LimitCORE. Reaped children consistently appear AFTER the self-kill line — a shutdown CONSEQUENCE, not sessions
naturally reaching zero. **Three ruled out with solid evidence, not guesses**: (1) `exit-empty` — an ISOLATED controlled
A/B test (throwaway `-L` socket) proves the mechanism itself works exactly as documented (off=survives empty, on=exits
empty); but `agent-orchestrator@ef5ba1e2cc` is confirmed LIVE-DEPLOYED (checkout sha + grep both verify) and setting it
correctly on every new spawn (verified `off` on a brand-new server pid) — yet 31 deaths happened in the ~10h after
deploy, same cadence as before. (2) `destroy-unattached` — confirmed `off`. (3) external `tmux kill-server` — `auditctl`
watches every tmux execve system-wide (`tmux_exec_watch`); dozens of hits across multiple deaths, ALL routine
(`capture-pane`/`has-session`/`new-session`), zero `kill-server` ever. **Next**: widen the strace filter beyond
signal/exit to see what the server was doing (socket reads, etc.) in the seconds before the next self-kill — signal-only
tracing can't see the DECISION, only its execution. Cross-reference: the "n=3 utempter pty-registration burst" section
further below may be the same phenomenon, still unreconciled.
...
- 2026-08-13 11:35Z (fleet unfrozen, resolves the utempter-burst cross-reference): with production dispatch resumed and
  ...
  **zero `tmux_pty_creation_burst` rows in `activity_log` for its entire uptime**, spanning both fresh deaths. The
  earlier n=3 utempter-registration-burst signature did NOT occur for either of these. This is the data point the
  "Mechanism CONFIRMED" section's cross-reference note (further above) was waiting on: it now reads as evidence AGAINST
  the utempter burst being a necessary precursor/trigger of the self-kill — consistent with the peer's framing that the
  earlier bursts were likely a side-effect of specific self-kill shutdown cascades (the server forking pty-teardown
  children as it exits), not a general cause. Weakens, does not fully retire, the utempter-burst hypothesis. No new
  action from this — the live edge stays the peer's widened-strace-filter work (socket reads pre-self-kill /
  client-IPC), watched for by both `pty-burst-watchdog` and the `auditctl` execve watch. Also condensed the now-
  superseded death #1/#2 narrative above (66→17 lines) to stay under the 1000-line hard cap; no factual content lost,
  same evidence and conclusions kept.
