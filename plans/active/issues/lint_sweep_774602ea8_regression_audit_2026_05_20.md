...
> **🟢 RESOLVED 2026-05-20** — both regressed files restored at execution-service@195cf6829.
> Reference incident for the foundation-completion-gate mega-audit (codex/11-project-management/foundation-completion-gate-discipline.md).
...
1. `execution_service/sports_execution/adapters/exchanges/kalshi.py`
   - Added imports: `from unified_api_contracts import (... ErrorAction, classify_venue_error)`
     (root facade per CLAUDE.md UAC import rule, not the deep `canonical.crosscutting.errors` path used at a2b5eef46);
...
- 2 pre-existing failures in `tests/unit/test_mock_data_provider.py::TestGetWorkspaceRoot::{test_from_env_var,test_from_unified_env_var}` (workspace_root env-var resolution; unrelated to adapter changes — verified pre-existing by stashing the adapter changes and rerunning).
