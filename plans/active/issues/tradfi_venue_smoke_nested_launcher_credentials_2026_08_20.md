...
- [ ] [BACKEND] P0. Provision or repair the staging TradFi instrument catalogue with `venue` and `instrument_type` columns before rerunning failed MTDS chunks; the terminal nested VMs fail closed with `InstrumentCatalogUnavailableError` for `instruments-store-tradfi-stg-central-element-323112`, and wrapper `EXIT_STATUS=0` does not indicate a successful capture.
