...
context_scope:
  [
    ...
    agent-orchestrator/.github/workflows/update-dependency-version.yml,
  ]
resolved_by:
depends_on: []
---
...
- **context-scout 2026-08-16**: refreshed context_scope (5 entries, was 4) — added
  `agent-orchestrator/.github/workflows/update-dependency-version.yml` (one of the 6 repos this doc's own investigation
  already sampled; the workflow file is identical across the fleet). The sole open `[DEVOPS] P1` todo investigates
  THIS workflow's dormant cascade, not `digest-drift-sweep.yml` (already fixed 2026-07-26) — the prior context_scope
  only cited the latter, leaving the actual investigation target uncited.
