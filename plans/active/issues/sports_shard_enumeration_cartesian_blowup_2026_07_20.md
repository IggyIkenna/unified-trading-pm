...
| **Phase 5 / Part 3** — execute manifest remediation (3.1/3.2/3.3/3.4) — **3.0 prereq now MET; all 3 operator decisions RULED 2026-07-22, no reconciliation gap remaining**                                 | Not done (scoping closed, executable pending the human trigger) | 3.0's prerequisite (Phase 6d) is now satisfied — `deployment-api@6d20724` confirmed live via `gcloud run services describe` (image `deployment-api:7ab62ec`, revision `uts-shared-deployment-api-00251-f45`, content-verified). All 3 operator decisions RULED: (1) human-only triggers the write; (2) row-count scope = **1,066,231** (same population 3.1 already reclassified — no widening needed, see below); (3) 3.3 destination = `empty_confirmed` + one of 3.1's new codes. Note also 3.2 itself is independently marked NOT-TO-DO (§3.2, four grounds) regardless of scope. Full runbook + every open question: see the `- [ ]` todos below. |
