...
matched HEAD). **Still not yet confirmed LIVE in production** — the remaining path is: LDR→main promote PR
(`quality-gates-v2` green + dep-order + provenance gate, empirically ~15 min effective cadence) → the native
`deployment-api-main-deploy` Cloud Build trigger (`gcloud run deploy uts-shared-deployment-api`, ~5–10 min documented) →
only then is the new code running in the one shared Cloud Run instance that serves the real data-status UI/decisions.
This promote+deploy watch is in progress as of this edit; read-only verification once live:
...
| **Phase 4 / 4.4** — shipped `deployment-api@6d20724`, promote/deploy verification in progress (see §4.4)                                                                                                   | Shipped, deploy-verification in progress         | The prior UAC blocker cleared (that DEFI_VENUE_PHASE work finished + committed). Remaining: LDR→main-promote→Cloud-Build-deploy pipeline (~20-35 min documented) must complete and be read-only-verified before Phase 6d is genuinely live in the shared Cloud Run instance — not yet confirmed as of this edit.                                                                                                                                                                                                                                        |
