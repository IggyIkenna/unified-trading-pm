...
- [x] ✅ [SCRIPT] P2. **fastapi/starlette adoption — lift the 1.5b caps — DONE 2026-07-28 (slot-12).** Fix the
      ...
      **[2026-07-28 note, slot-6]**: the UTL `_IncludedRouter`/`.path` route-introspection fix landed today
                                      ...
                                      **[2026-07-28 note, slot-12]**: this todo is FULLY EXECUTED — flipping its checkbox above, since its OWN
                                          done-when criteria (fix + fleet bump + regen locks + fleet QG + drop ignores) are all literally satisfied
                                          and main gave interim hold-as-is/don't-revert guidance (`BLK-0d51ff66`). This does NOT ratify the
                                          SEPARATE `[OPERATOR]` todo in the P0 doc below (whether fastapi>=0.137 was the right fleet-wide direction
                                          to have chosen) — that decision stays open for the operator; only the engineering work this specific todo
                                          describes is what's marked done here. I authored the
