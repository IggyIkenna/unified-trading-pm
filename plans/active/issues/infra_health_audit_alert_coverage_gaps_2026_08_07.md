...
summary: >-
  ...
  **(C) AWS IAM/STS failures have zero AlertCode coverage** — grepped the full closed `AlertCode` set
  ...
  were genuinely unmet (finding 4 kept emitting a fresh PIPELINE_HEARTBEAT → `classify_vm_liveness` reads ALIVE; finding
  8's OOMs were IN-VM subprocess kills — the GCE instance never terminated nonzero). Two NEW coverage-gap classes
  surfaced (capture-complete-but-not-exited VMs; in-VM degraded-but-running backfills) — filed as follow-up todos below.
status: open
nature: issue
...
- [x] ✅ [DIAG] P3. For findings 4 and 8 (mtds-dex-swaps-backfill-2 idle VM; mtds-backfill-odds-401-retry
      self-recovering OOM): checked the live MissTracker state for DP-VM-003/DP-VM-001. BOTH are benign-by-design, NOT
      wiring bugs. Neither DP-VM-003 (`heartbeat_stall_watcher.py`) nor DP-VM-001 (`exit_code_fleet_monitor.py`) uses
      MissTracker at all (0 references each; MissTracker is meta-sweep-only — confirmed in `_miss_tracker.py`,
      `meta_watchers.py`, and the full `data_pipeline_monitors/` directory). Finding 4: `classify_vm_liveness` gates
      EVERY stall branch on heartbeat/sidecar staleness — the VM kept emitting fresh PIPELINE_HEARTBEAT every 60s +
      fresh sidecar after shard-complete, so it reads ALIVE by design (a fresh sidecar ⇒ never STALL, per the module
      docstring). Finding 8: OOMs were IN-VM subprocess OOM-kills — the GCE instance never terminated nonzero
      (`read_terminal_exit_code`=None), and DP_VM_EXIT_NONZERO fires only on GCE instance termination. Two NEW coverage
      gaps surfaced — see follow-up todos below. Repo: deployment-service (code-read only; no code changes shipped).
- [ ] [INFRA] P3. New coverage-gap class: **capture-complete-but-not-exited VMs**. A backfill VM that finishes all
      shards but never self-terminates keeps emitting PIPELINE_HEARTBEAT + fresh sidecar forever →
      `classify_vm_liveness` reads ALIVE by design (fresh sidecar ⇒ never STALL) → no DP_VM_STALL alert ever fires for
      genuinely idle/wasted VMs. Needs a per-VM captured-count FLAT detection path gated on the VM having completed its
      launch-budget workload (i.e. all shards DONE, not just flat between ticks). Repo: deployment-service.
- [ ] [INFRA] P3. New coverage-gap class: **in-VM degraded-but-running backfills**. A backfill VM that OOM-kills its
      worker subprocess but stays running (GCE instance exit_code never written, heartbeat sidecar stays fresh) is
      invisible to both DP_VM_STALL (sidecar fresh → ALIVE) and DP_VM_EXIT_NONZERO (instance never terminated). Needs a
      run.log `exit_code` grep path that detects subprocess failures on LIVE VMs, not just terminated ones — distinct
      from the existing `read_terminal_exit_code` which only fires post-termination. Repo: deployment-service.
...
- **2026-08-11 (item 4)**: DIAG complete. Root-caused both findings — BOTH benign-by-design, NOT wiring bugs: (a)
  DP-VM-003 never fires for sidecar-still-fresh VMs (the capture-complete-but-not-exited class — `classify_vm_liveness`
  reads ALIVE per design contract); (b) DP-VM-001 only fires on GCE instance termination, not in-VM subprocess OOM-kills
  (the degraded-but-running class). Verified via code-read: 0 MissTracker references in either watcher (MissTracker is
  meta-sweep-only). Two follow-up P3 coverage-gap todos filed for the new classes surfaced.
