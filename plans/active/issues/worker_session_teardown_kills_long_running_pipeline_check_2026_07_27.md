...
- [ ] [SCRIPT] P2. **NEW 2026-07-27 (slot-6)**: port the identical `_find_inflight_duplicate_vm` duplicate-in-flight
...
- [ ] [OPERATOR] P1. **NEW 2026-07-27 (slot-3)**: the `_find_inflight_duplicate_vm` fix (`features-service@6981b2b8`) is
      ...
      **CORRECTION 2026-07-27 (slot-3, later same session)**: the `-101851` "byte-identical-stalled at ~2.5h" claim
                  ...
                  showed the SAME VM's `run.log` line count climbing steadily and repeatedly (159,987 → 356,664 → 361,392 →
                  363,748 → ... → 437,578+ across dozens of properly-spaced checks over several more hours), with no further flat
                  reads. It never stopped computing; the earlier single flat-then-stalled read was misdiagnosed as a genuine stall
                  instead of a transient upload-cadence gap. Root cause of why it runs this slowly (not why it "stalled" — it
                  didn't) is separately, correctly diagnosed by another slot:
