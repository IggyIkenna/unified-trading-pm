...
- [x] ✅ [CODE] P2. **Extend `_enumerate_v2_sports` to emit `EXPECTED_PRE_SOURCE_COVERAGE_START` at (source, data_type,
      ...
      tests pass.
- [ ] [CODE] P2. **Extend cefi/defi/prediction v2 enumerators to emit venue-grain `EXPECTED_PRE_VENUE_LAUNCH` sentinel
      rows** when the catalog is empty for a `(venue, day)` in the pre-launch window; single sentinel row per (venue,
      data_type, day) matching v1's grain; add regression test using an empty catalog (repo: instruments-service).
