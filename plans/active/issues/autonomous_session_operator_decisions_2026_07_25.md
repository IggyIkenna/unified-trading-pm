...
## 11. `[unlock-plan]` to archive the resolved-but-locked MTDS/UAC baseline-regression doc (2026-07-26, cross-cutting)

[`/plans/active/issues/mtds_uac_adapter_contract_baseline_regression_2026_07_09.md`](/plans/active/issues/mtds_uac_adapter_contract_baseline_regression_2026_07_09.md)
carries `status: resolved` with **all 4 todos `[x]`** (all 4 originally-flagged files confirmed non-regressions;
baseline already regenerated by `unified-trading-pm@ba098a7cc`) — i.e. a terminal-status doc sitting in
`plans/active/issues/`, which `check_terminal_status_archived.py` tracks as debt. It also carries
`locked_by: live-defi-rollout`, and `cursor-configs/CLAUDE.md` is explicit: _"`locked_by:` blocks archival without
`[unlock-plan]` (ASK, never autonomous)"_. `cross_cutting_consolidated_closeout_2026_07_25.md` Track 15 already records
this as _"Archival … deliberately NOT done … flagged as a named follow-up"_ — raising it here so it is actually in front
of you rather than only noted inside the doc it blocks. This is the explicit-human-signal class: strong evidence does
not buy the authority.

A: **Approve** — reply `[unlock-plan] mtds_uac_adapter_contract_baseline_regression_2026_07_09`, and the next reconcile
pass runs the 6-step archival ritual (clear the lock, move to `plans/archive/issues/`, fix referrers corpus-wide).
[WORKER REC] — the doc is genuinely terminal; the lock looks vestigial (`live-defi-rollout` is the branch name, not a
person, the same pattern flagged as "an abandoned lock" on `empty_reprobe_disagreement_2026_06_22.md`). B: **Keep it
locked + active** — if the lock is deliberate and you want the resolved doc visible in `plans/active/` for now; the
terminal-status-in-active debt row stays as a known, accepted exception. Other: operator can type a custom answer
