...
| ~~Clean `/done {one_shot_complete:true}` for this dispatch~~      | **Effectively done 2026-08-06T13:59Z** — `GET /api/agents?kind=plan_reconciler&include_finished=true` confirms `agt-4fdce1` already `status:archived`, `exit_reason:"lifecycle-complete"` (not `reaped-stale`), `finished_at:13:29:00Z`; an earlier `/done` call this session DID succeed server-side, every 400 since is a correct-but-misleading retry-against-already-done response. See `data_pipeline_failure_one_shot_done_no_agentrow_2026_07_29.md` "Second addendum" for the full evidence chain. | —                                                                       |
...
- **A "no DB access" blocker is sometimes just an unchecked API.** After `/done` kept 400ing (post-`/compact`, via a
  self-armed hourly `ScheduleWakeup` recheck), `GET /api/agents?kind=plan_reconciler&include_finished=true` — a plain
  anonymous-local REST call, no different from any `/heartbeat` — turned out to answer the exact question 8 prior
  entries in `data_pipeline_failure_one_shot_done_no_agentrow_2026_07_29.md` had all marked "needs live DB access this
  worker slot does not have." It showed `agt-4fdce1` archived `exit_reason:"lifecycle-complete"` (an earlier `/done`
  call this session HAD actually succeeded server-side; every subsequent 400 was a correct rejection of a retry against
  an already-done row, not a fresh bug). Before writing off a diagnostic as "needs access I don't have," check whether
  the read-only list/status endpoint for that resource already exists — `/api/agents` was reachable the entire time.
