...
last_updated: "2026-08-21 (ruling D44 applied — see Progress Log)"
context_scope:
  [
    /codex/05-infrastructure/data-pipeline-alerts.md,
    /plans/archive/issues/cefi_tardis_write_schema_contract_column_mismatch_2026_07_27.md,
...
- [ ] [SERVICE] P3. Per D44 ruling (2026-08-21, autonomous-dispatch authority): change the calculators —
      `BookDepthCalculator`/`LiquidityWallCalculator` (features-service) should read the real flattened L5 columns
      MTDS actually writes (`asks[0..4].price/.amount`, `bids[0..4].price/.amount`) instead of the fictional
      `bids`/`asks`/`mid_price` shape — do NOT build a new `l2_book_checkpoints` writer. Repo: features-service.
      Done when: `CrossInstrumentRawDataLoader.load_book_snapshots` feeds these calculators the real columns and
      their existing tests pass against real wire-shaped fixtures.
- [ ] [SERVICE] P2. Per D44 ruling (2026-08-21, autonomous-dispatch authority): reformat the message — change
      `finalise_rows_and_path`'s schema-violation `ValueError` message to omit the colon before the violation count
      (e.g. `"schema contract violated for cefi/X/Y/Z -- 1 violation(s); first=..."`) so
      `_classify_tardis_error()`'s `raw.split(":", 1)[0]` truncation (`tardis_adapter.py:164`) no longer collapses
      distinct violations for the same (venue, instrument_type, data_type) into an identical `error_reason` bucket —
      no schema/manifest change needed. Repo: market-tick-data-service. Done when: a regression test confirms two
      distinct schema violations produce two distinct `error_reason` strings.
...
- **Progress Log entries for dispatches 22-30+ (2026-08-08 through 2026-08-15, the `data_pipeline_failure`
  escalation-worker's repeated static-backlog re-confirmations) extracted verbatim to
  [history extract](/plans/archive/issues/cefi_book_snapshot5_schema_contract_ts_event_levels_mismatch_history_2026_08_21.md)
  for line-cap compliance (2026-08-21) — nothing open there, every entry confirms the 3 root-cause fixes below still
  hold and the residual backlog is decaying, not regressing.**
...
**2026-08-21 — ruling D44 (book_snapshot5 contract fixes)**: ADOPTED-REC 2026-08-21 (autonomous-dispatch authority,
AUTONOMOUS_AGENT_RULES rule 2): Change the calculators and reformat the message — no writer produces the expected
shape, and the reformat needs no schema change. Source:
