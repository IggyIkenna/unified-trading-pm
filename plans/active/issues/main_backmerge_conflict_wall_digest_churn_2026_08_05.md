...
already commented out 2026-06-28 (WS-L, `plans/active/cicd_retire_staging_branch_2026_06_27.md`) once it was measured
billing ~2,000-2,900 GHA-minutes/month for zero value under `staging_dormant_mode`; its `repository_dispatch` listener
was ALSO stopped 2026-07-23 (`plans/active/issues/staging_workflow_shutdown_2026_07_23.md`) — the upstream dispatcher
(`ci-status-update.yml`) still fires the event, GitHub accepts it as a harmless 204 no-op. **Zero GHA minutes currently
spent on the staging leg for `ldr_main`-model repos.**
