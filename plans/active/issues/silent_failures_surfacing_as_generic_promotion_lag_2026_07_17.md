...
- [x] ✅ [DEVOPS] P0. **Re-do the `|| true` fix — the first attempt BROKE PROD and was rolled back (2026-07-17).** The
      ...
      candidate is kept at `/tmp/glue-runner-run.sh.broken-keepme` on the VM for post-mortem. — **REDO SHIPPED same-day
      2026-07-17 (`unified-trading-pm@41e2b47bb`, reachable on origin/live-defi-rollout)**: the current
      `scripts/self-hosted-runners/glue-runner-run.sh` carries the loud-failure idiom — "Deliberately NO
      `2>/dev/null || true` here … cost ~16h of a silently bleeding pool on 2026-07-17" + a `|| { …FATAL… exit 1 }`
      block — subsequently hardened by `e170e6ccf` (gcloud-config isolation) and `a4eb9a288` (loud stale-registration
      retry). **DEFERRED** (plan_reconciler agt-6eb8c5 2026-08-07): the `--selfcheck` mode +
      roll-one-unit-then-confirm-`Listening for Jobs` canary from this todo's own "Why my tests missed it" was never
      added — open follow-up.
...
- **context-scout 2026-08-03**: refreshed context_scope (6 entries — added
  `/plans/active/ci_satellite_ao_dispatch_batch1_2026_07_26.md`, where the runner-liveness-paging todo actually
  shipped).
