...
- **na-eligibility-audit 2026-08-17** (ci tranche, autonomous, dispatch agt-b9cf62) [body-hash:32dfaff5bd63e859]: KEEP-NA, valid (this run's first verdict — no prior na-eligibility-audit marker existed, doc filed 2026-08-16). 6 tracked todos: 2 OPERATOR_QUESTION (transcription confirm; a 6x-reconfirmed design escalation), 2 explicitly redirected to a different owning tranche/corpus-hygiene sweep per the concurrent-sharded-worker safety rule (not this tranche's to fix), 1 sequenced behind batch15 draining, and 1 (pre-scope workflow_template_runs_on_placeholder's P2 item) that reads as bounded/worker-executable standalone — held rather than extracted this pass because the doc's own Phase 3 explicitly recommends folding it into batch16 'once batch15 has meaningfully drained' (batch15 currently 23/25 open) rather than front-running the iterative-drain model; re-assess once batch15 drains.
