...
- [ ] [INFRA] P2. **Fix the manifest-consolidator alerting no-op found above**: `manifest_consolidator.py`'s CLI
      `main()` wires `setup_events(..., sink=MockEventSink())`, so `MANIFEST_CONSOLIDATION_FAILED` (severity=ERROR)
      never reaches any real consumer despite the SSOT doc (`codex/05-infrastructure/manifest-consolidator-ssot.md` §
      "Liveness + health contract") claiming it "routes to the alert sink." Before swapping the sink, first confirm
      whether `alerting-service` (or any other consumer) actually reads from a `GcsEventSink` output path for this event
      -- if not, wiring `GcsEventSink` alone doesn't close the loop, it just moves the no-op. Also confirm whether
      `uts-prod-consolidator-liveness-watchdog`'s per-bucket status/CONSOLIDATOR_DOWN emission has the same gap (its
      Cloud Logging output was silent throughout this incident despite `basicConfig(level=logging.INFO)` being set --
      worth checking its `log_event`/sink wiring too). Once a real consumer path is confirmed, the code fix itself is a
      1-line sink swap in `unified_trading_library/manifest_consolidator.py::main()`; landing it in production
      additionally requires an image rebuild (`BASE_IMAGE_DIGEST` bump per this doc's own "Image deploy-hygiene" note) +
      redeploy across all ~10 manifest-consolidator Cloud Run jobs, which is bigger scope than this todo alone -- track
      that redeploy as a follow-up once the code fix lands. (repo: unified-trading-library, deployment-service)
