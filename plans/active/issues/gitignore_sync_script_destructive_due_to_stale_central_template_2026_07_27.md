...
- [ ] [SCRIPT] P2. **Fix `sync-gitignore-cursorignore.py`'s `--dry-run` flag to actually gate all writes** (the base
      sync AND the chained `untrack-ignored-files.py --untrack` call), not just the separately-documented
      `--purge-history` path. A flag named `--dry-run` that mutates the filesystem is a footgun for the next person
      (human or agent) who reasonably trusts the name.
