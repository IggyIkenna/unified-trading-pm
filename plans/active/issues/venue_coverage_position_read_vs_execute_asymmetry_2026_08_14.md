...
- [x] ✅ [AGENT] P0. **CLOSED (na-eligibility-audit 2026-08-17)** — all four resolved per this item's own body below:
      ...
      there is no persistent "Jupiter position." Kamino — **SHIPPED 2026-08-16 — `strategy-service@70c3c05f5c`**: a real,
      working `KaminoPositionAdapter` built in the strategy-service worktree
      ...
      an unrelated capabilities-test fix) and confirmed unrelated via content grep. **Landed 2026-08-16
      (`strategy-service@70c3c05f5c`)** using this workspace's own already-documented mitigation for this exact
      `.env`-pollution artifact (move `.env` aside for the duration of the quickmerge run, restore immediately after —
      already used earlier this session for execution-service and strategy-service's other ships), not by waiting for
...
- [x] [AGENT] P1. ✅ **SHIPPED 2026-08-15 — `execution-service@37bfaeed0b`.** Added `borrow()`/`repay()` to `kamino.py`.
      ...
      since a wrong market silently misroutes a real loan) — **shipped 2026-08-16 as `unified-api-contracts@4821f0d92e`**
      (same commit also added the `"SOLANA"` mSOL/jitoSOL addresses cited in the position-adapter todo above).
...
- **na-eligibility-audit 2026-08-21** (cross-cutting tranche, batch 3/3): KEEP-NA, valid — 2 open items,
  re-affirming 3 prior na-eligibility-audit passes (2026-08-17 x3). Solblaze/Jito Restaking real write paths are a
