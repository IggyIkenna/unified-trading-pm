...
- [ ] [AGENT] P0. **Build DeFi position adapters for the carve-out path first** — Lido, Marinade, Kamino, Jupiter. These
      ...
      cleaned up in the same edit that closed out Kamino).** Lido — done (READ SIDE section below, `wstETH` via the
      generic reader). Marinade — done, `strategy-service@926be71046`: added `"SOLANA"` chain addresses (mSOL, jitoSOL)
      to UAC's `LST_TOKEN_ADDRESS_BY_CHAIN` and taught `factory.py`'s generic-first fallback to check both chains.
      Jupiter — **determined NOT to need a position adapter**: it's a one-time USDC→SOL swap router for the archetype
      (`carry-staked-basis.md`'s `spot_venue: JUPITER`), unlike Uniswap's own adapter (which reads real LP state) — after
      the swap the resulting SOL/LST sits in the wallet, covered by whichever venue's own adapter reads that token;
      there is no persistent "Jupiter position." Kamino — **code DONE, NOT YET SHIPPED (2026-08-15)**: a real, working
      `KaminoPositionAdapter` built in the strategy-service worktree
      (`strategy_service/position/position_interface/adapters/kamino.py`), calling the verified public
      `GET /kamino-market/{market}/users/{wallet}/obligations` endpoint (no credentials needed, confirmed live — the
      earlier "no verified per-wallet endpoint found" note was a research gap, not a genuine API absence). The
      endpoint's actual shape differs from Kamino's OpenAPI-documented `ObligationMetrics` (that shape belongs to the
      separate `/metrics/history` endpoint) — it returns raw on-chain obligation state instead, one array entry per
      obligation (confirmed live: a wallet CAN hold multiple obligations per market, all enumerated in one response, no
      per-obligation call needed). Adapter surfaces the pre-computed `refreshedStats` aggregate (collateral_usd/
      debt_usd/LTV — no fixed-point decoding needed) as `DeFiLendingPosition` + `CanonicalBalance`; per-reserve entries
      resolve the underlying mint via a live `GET /reserves/{reserve}/stats` call (same endpoint execution-service's
      `KaminoConnector` already uses) rather than fabricate a decimals/exchange-rate guess — honest-absence (entry
      skipped) on any reserve that fails to resolve. Wired into `factory.py` (`market_address` REQUIRED, not defaulted
      — same reasoning as `KaminoBorrowParams.market_address`) and `capabilities.py` (`live="deployed"`). 15 unit tests
      + a factory-routing test added, all green in isolation (`pytest tests/position/position_interface/unit/
      test_kamino_adapter.py tests/position/position_interface/unit/test_capabilities.py` via quality-gates.sh). **NOT
      committed**: two full `quality-gates.sh` runs (with/without `--skip-preflight`) both blocked on the SAME 4
      pre-existing, content-unrelated failures (`tests/unit/test_config_reloaders.py::TestStrategySafeFieldAllowList`,
      zero mention of kamino/position_interface) — root-caused to this laptop's local `.env` (`CLOUD_PROVIDER=gcp`,
      `TESTNET_MODE=mainnet`, `RUNTIME_MODE=live`, `GCS_REGION`/`GCS_LOCATION`/`bigquery_location`/
      `tardis_secret_name`/`databento_secret_name`/`features_onchain_bucket` — all present in `.env.example` as normal
      local-dev defaults) getting read by `StrategyDomainConfig` (a pydantic-settings model with `extra="forbid"`) via
      quickmerge's own "ENVIRONMENT AUTO-DETECT" `.env`-export step (`scripts/quickmerge.sh` ~line 2225) — a standing,
      machine-local incompatibility between a normal `.env` and this one test class, predating this session, not caused
      by this change. Confirmed pre-existing via 2 independent full-suite runs (identical 4 failures before AND after
      an unrelated capabilities-test fix) and confirmed unrelated via content grep. Not fixed here (editing another
      session's shared local `.env` or `StrategyDomainConfig`'s schema is out of this todo's scope and risks the
      concurrently-running sessions on this same host) — see the new P1 residue todo below. The adapter code itself is
      complete and ready to ship the moment that gate clears; nothing about the Kamino work itself is blocked.
- [ ] [AGENT] P1. **Land the Kamino adapter commit once the `.env`/`StrategyDomainConfig` gate clears** (blocker
      described in the P0 todo above) — the code in the strategy-service worktree is done and tested; this is purely a
      "run quickmerge again" step once `tests/unit/test_config_reloaders.py::TestStrategySafeFieldAllowList` stops
      failing on this host. Separately: consider whether `StrategyDomainConfig` should tolerate a superset `.env`
      (`extra="ignore"` instead of `extra="forbid"`, or have `TestStrategySafeFieldAllowList` construct via
      `_env_file=None`) — as written, ANY local `.env` matching `.env.example`'s own documented defaults breaks this
      test class, which likely blocks every quickmerge attempt against strategy-service from this machine, not just
      this one.
...
| Lido / Marinade / Kamino / Jupiter position adapters                                    | **Code 4 of 4 done, 1 unshipped** | Lido (generic reader) + Marinade (`strategy-service@926be71046`) done; Jupiter determined NOT to need one (swap-only venue, no persistent position); Kamino code DONE but not yet committed — see P0/P1 todos above for the `.env`/`StrategyDomainConfig` shipping blocker                                                       |
...
| Kamino bespoke lending-position adapter                                                 | **Code done, unshipped** | `KaminoPositionAdapter` built + tested (see P0/P1 todos above); blocked from committing by a pre-existing, unrelated local `.env`/`StrategyDomainConfig` test failure on this host, not a code problem                                                                                                                                                                                       |
