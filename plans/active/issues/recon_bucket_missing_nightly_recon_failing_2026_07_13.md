...
- **na-eligibility-audit 2026-08-17** [body-hash:78cf5eb532d4c60b]: KEEP-NA, valid -- Grep-verified 1 open checkbox (line 164), matching inventory_open_todos=1. This single todo bundles ~5 distinct deliverables across 4 repos (provision 2 missing Cloud Run Jobs, implement 2 different _SUCCESS-marker writers, add a date-fallback, un-pause 7 schedulers) -- three independent na-eligibility-audit passes (07-30, 08-03, 08-06) already examined this and explicitly declined to RECLASSIFY it in place, reasoning that splitting it correctly first requires becoming its own scoped plan (an ask-before-creating operator call), not a mechanical flip. That recommendation was acted on 2026-08-10 via a live operator escalation (BLK-8bb28da4) that remains open and unresolved as of today (confirmed by two fresh 2026-08-17 escalation-dispatch entries this same day). Re-litigating the split myself here would ignore both the standing multi-pass audit conclusion and the active, unresolved escalation asking the operator exactly this question.
