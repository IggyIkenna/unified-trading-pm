...
- [x] ✅ [DATA] P3. **Apply the same per-date full-catalogue `iterrows()` → cached-row-dict fix to the CeFi and DeFi
      ...
      stall. Repo: market-tick-data-service. — market-tick-data-service@5d428486 (2026-08-05, LDR; QG-green tree
      `mtds-qg-slot7-iterrows3.log` exit=0: 10007 passed / 0 failed + 6 passed, STEP 5.94 ratchet holds at 237).
      Regression tests shipped alongside: `test_per_date_calls_reuse_cached_records_not_reconverted` +
      `test_perp_bases_computed_once_across_per_date_calls` (mvp gate) + iterrows re-uses in margin/defi gates.
...
- **2026-08-05 (slot-7, SHIP)**: **P3 iterrows fix SHIPPED `market-tick-data-service@5d428486`** (LDR, 2026-08-05). Path
  ...
  blanket header, deleted after verifying 0/0/0 without it), aster WS-test gw0 worker crash (load-flake under 7-9
  concurrent QGs; passed on the fair-field gate), and rule11 DEFI pin drift 2856→2958 (see below). (2) Full QG green on
  a fair field (field=3, cap 4): `mtds-qg-slot7-iterrows3.log` exit=0, 10007 passed / 0 failed + 6 passed, STEP 5.94
  holds at 237. (3) Cross-slot dedup: slot-4 `d5882379` had ALREADY shipped the same header removal + the pin VALUE
  (2856→2958) — so I shipped only the 5-file bundle and a correcting comment `655c9320` (slot-4 attributed +102 to "2
  new protocols"; measured composition = 102 venues × 29 data_types, 29th = `oracle_prices`, verified wired). P3
  checkbox flipped with evidence in this doc. `/done` pending.
...
| ~~Ship P3 iterrows fix (5 MTDS files: 2 readers + 3 tests)~~                               | ✅ DONE `market-tick-data-service@5d428486` - iterrows->cached-row-dict fix in both readers + 3 regression tests, shipped on a QG-green tree (field=3, cap 4): `mtds-qg-slot7-iterrows3.log` exit=0, 10007 passed / 0 failed + 6 passed, STEP 5.94 ratchet holds at 237 | -          |
| ~~Flip P3 checkbox~~                                                                       | ✅ DONE - flipped with `market-tick-data-service@5d428486` + QG evidence (this turn's `docs(plans:)` commit)                                                                                                                                                            | -          |
| Correct rule11 DEFI pin-rise explanation                                                   | ✅ DONE `market-tick-data-service@655c9320` - slot-4 `d5882379` bumped the pin value attributing +102 to 2 new protocols; measured composition is 102 venues x 29 data_types, the 29th dimension being `oracle_prices` (verified wired)                                 | -          |
...
**Operational lesson (08-05, slot-7, cross-slot dedup)**: while this fix was staged, a concurrent slot-4 session
(`d5882379`, 15:44Z) independently shipped BOTH the same blanket-pyright-header removal in `lending_rewards_handler.py`
AND the rule11 DEFI pin value 2856→2958 — but attributed the +102 to "2 new protocols solend/marginfi". The measured
composition (102 venues × 29 data_types, 29th = `oracle_prices`) shows that explanation was imprecise; the pin value
itself was correct and I did NOT re-push it, only the correcting comment (`655c9320`). Two lessons: (1) on a shared LDR,
before re-shipping a "fix", `git pull` and check whether a concurrent slot already landed it (the header fix vanished
from my working tree on pull — that was the signal); (2) when two slots both attribute the same +102, MEASURE the
composition rather than trusting either attribution.

**Operational lessons (08-05 15:xxZ, slot-7) — added when the 14:37 re-gate came back RED after a green pytest**:

- **Method-size cap counts comments**: `MAX_METHOD_LINES=50` (`base-service.sh:262`) — my 16 comment-heavy lines added
  inside `CeFiCatalogReader.__init__` pushed it to 51L and FAILED the gate AFTER a fully-green 10001-pass pytest stage.
  Comment blocks inside a method count toward the cap; keep them tight (≤2 lines) and push the WHY into a docstring or
  the plan. Verify a method's line span (`grep -n '^    def '` + difference) before re-gating.
- **A "NEW" bare-call flag on a file you never touched can be a behind-LDR LAG artifact**: the gate flagged
  `venue_fetch.py:186` bare `read_availability_index(bucket)` as a new ratchet violation, but slot-12 had already
  projected it on LDR TODAY (baseline entry "REMOVED 2026-08-05 (slot-12): now projected"). My checkout was 6 commits
  behind. Diagnosis: `git rev-list --count HEAD..origin/live-defi-rollout` before re-gating a shared checkout; a
  foreign-file red may already be fixed upstream → `git pull --rebase --autostash`, then re-run the failing checker
  standalone
  (`.venv/bin/python unified-trading-pm/scripts/quality_gates/check_bare_read_availability_index.py --workspace-root <ws> --scope market-tick-data-service`)
  to confirm green before assuming a real baseline red.
- **pytest-green ≠ gate-green**: the 14:37 gate passed all 10001 tests then failed 2 static stages (`5.5` function-size,
  `5.106` bare-read) at the very end. A waiter watching only the pytest pid exits "green" mid-gate; the terminal verdict
  is the wrapper's exit-file content (`MTDS_QG_EXIT=1` was the truth here). Watch the WRAPPER pid + exit file, and read
  the exit file's value, not just its existence.
- **A single-worker xdist crash on a FOREIGN slow WS test under host overload is a load-flake, not a regression — but it
  still blocks shipping**: the 15:xx full gate failed 1 test (`test_aster_ws_connector.py … depth5_tick`, NOT in my
  6-file bundle, no shared imports) via `replacing crashed worker gw0` + `coverage: failed workers` (the only xdist
  worker died mid-test — likely CPU-starvation under 7-9 concurrent QGs on a 16-core host, cap 4). The same test PASSED
  in the 14:37 gate (185s) and hangs >480s standalone under the current load. Since `quality-gates.sh`-green is a HARD
