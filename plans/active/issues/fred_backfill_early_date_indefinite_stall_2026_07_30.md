...
| Ship P3 iterrows fix (5 MTDS files: 2 readers + 3 tests)                                   | Cannot ship yet — the 14:37 full re-gate came back RED on 2 STATIC checks (not pytest): (a) `CeFiCatalogReader.__init__` 51L over `MAX_METHOD_LINES=50` — MY regression (the 16 comment-heavy lines I added inside `__init__`); FIXED by condensing to 41L. (b) `venue_fetch.py:186` bare `read_availability_index(bucket)` — NOT my file; already fixed on LDR by slot-12 (projected, baseline REMOVED); my checkout was 6 behind LDR. RESOLVED by `git pull --rebase --autostash`. Fresh full re-gate running (bg task bp2fts4wk, log `/home/ubuntu/.cache/qg-tmp/mtds-qg-slot7-iterrows.log`). MTDS ahead=0, 5 files modified | Fresh re-gate green verdict, then quickmerge `--agent` the 5 files                           |
...
**Recommended next item**: on the fresh MTDS re-gate's green verdict (bg task `bp2fts4wk`, log
`/home/ubuntu/.cache/qg-tmp/mtds-qg-slot7-iterrows.log`, exit file
`/home/ubuntu/.cache/qg-tmp/mtds-qg-slot7-iterrows.exit`), run
