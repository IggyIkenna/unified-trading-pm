...
| ----------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------- |
| Ship UAC removal commit (AAVE `rewards`/`collect-rewards` + POLYMARKET/KALSHI `fills`)    | Edits applied 08-05 (operator-ruled UAC-side removal, `mtds_qg_red..._2026_08_05.md`); UAC QG running — cannot commit until green (green-tree rule) | UAC `quality-gates.sh` result (bg task bs54ax77c) — then quickmerge `--agent` the 2 UAC files |
| Ship P3 iterrows fix (5 MTDS files: 2 readers + 3 tests)                                  | Cannot ship yet — committed-ready, but MTDS `quality-gates.sh` is RED until the UAC removal lands (editable-install view)                           | UAC removal commit on LDR, then MTDS re-gate (expect green)                                   |
...
**Recommended next item**: on the UAC QG's green verdict, quickmerge the 2 UAC files, then ship the iterrows fix. After
the MTDS re-gate is green, run
`bash scripts/quickmerge.sh "..." --agent --files 'market_tick_data_service/engine/cefi_catalog_reader.py market_tick_data_service/engine/defi_catalog_reader.py tests/unit/engine/test_cefi_catalog_reader_mvp_gate.py tests/unit/engine/test_cefi_catalog_reader_margin_gate.py tests/unit/engine/test_defi_catalog_reader.py'`,
