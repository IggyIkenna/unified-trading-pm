...
- [ ] 11. [DATA] P1. Re-verify `instruments-service@ba87cc32` (or whatever later LDR SHA carries the sports writer fix)
      has reached `main` AND the `uts-prod-instruments-service-sports-fixtures` Cloud Run Job is running an image built
      after it (same two checks as todos 9-10). Once confirmed live: run a fresh dry-run + apply the sports
      ...
      todos 9-10's hold (do not force-promote, do not escalate for the shared-runner/v2-never-reported stuck-promote
      pattern), close this todo documenting the hold, and spawn the next pass as a new todo (repo: instruments-service +
