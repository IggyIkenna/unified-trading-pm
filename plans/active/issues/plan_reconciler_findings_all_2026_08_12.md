...
- **na-eligibility-audit 2026-08-17 (cross-cutting tranche, dispatch agt-3931fd)** [body-hash:fe18476dc8bf9bf0]: KEEP-NA, valid — Read end-to-end (747 lines). Sections 1-2 (done-but-unchecked candidates, zero-checkbox docs) are now fully [x] flipped/resolved. All 24 remaining open items live in Section 3, the compact P2/P3 backlog log from the 2026-08-12 full-corpus /plan-reconcile run. Each is a terse per-doc triage note (check whether a stale/cosmetic finding about ANOTHER doc still holds); most conclude 'confirmed real, left open' (judgment/investigative, not deterministic) or 'needs a closer read beyond this pass's budget'. The doc's own Section 3 header explicitly redirects remaining triage to the mechanism, not direct dispatch: 'A future /plan-reconcile pass (sharded or full) should triage these.' This doc's own scope is meta-review/triage, not the underlying engineering in the docs it references (which are independently NA/planning-classified elsewhere). Ruling/citation: Doc's own Section 3 intro: 'A future /plan-reconcile pass (sharded or full) should triage these' — redirects remaining work to the /plan-reconcile skill mechanism, not direct AO dispatch from this doc.
