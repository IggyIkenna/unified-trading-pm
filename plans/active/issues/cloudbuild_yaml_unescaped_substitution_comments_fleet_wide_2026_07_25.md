...
- [ ] [INFRA] P1. Fix `agent-orchestrator/cloudbuild.yaml` line 150 (bare `$VERSION` in comment → `$$VERSION`) + run the
      exhaustive re-scan above to confirm zero remaining. Ship via quickmerge. (repo: agent-orchestrator)
