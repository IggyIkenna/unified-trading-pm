...
- [ ] [DEVOPS] P3. Fix shellcheck findings in `unified-trading-ci/.github/workflows/semver-agent.yml` (7 findings —
      SC2015 ×4, SC2034, SC2129 ×2) so the `lint` actionlint job goes green.
- [ ] [DEVOPS] P3. Fix shellcheck findings in `unified-trading-ci/.github/workflows/update-dependency-version.yml` (4
      findings — SC2129 ×3, SC2015) so the `lint` actionlint job goes green.
- [ ] [DEVOPS] P3. Fix shellcheck findings in `unified-trading-ci/.github/workflows/request-major-bump.yml` (1 finding —
      SC2015) so the `lint` actionlint job goes green.
