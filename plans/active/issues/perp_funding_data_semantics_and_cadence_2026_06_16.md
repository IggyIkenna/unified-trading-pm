...
- [ ] [DATA] P2. Scale `reprocess_bulk_tardis_derivative_ticker_funding_timestamp_2026_07_28.py --mode apply --apply` to
      ...
      per the bounded-sample proof + real scope above. **GO recommendation** — the mechanism is proven (86/86 real
      objects round-tripped with zero errors; manifest-unchanged verified), per-shard classification is data-driven
      ...
      **IN PROGRESS 2026-07-28 (slot-4 session, real state independently verified — do not trust prior text reports
                                  ...
                                  **DEFERRED — not this task's scope, flagged for follow-up**: (1) at ~1-4 objects/sec single-threaded per the
                                  ...
                                  `plans/active/issues/cefi_migration_vm_launcher_no_sharding_and_spot_preemption_churn_2026_07_28.md` (`SHARD_DAYS`-
...
- [x] ✅ [OPERATOR] P2. **DECIDED 2026-07-28 (autonomous session, operator directive + independent confirmation): Option
      ...
      **Original investigation (2026-07-28) — from-scratch raw-column derivation, investigated and STOPPED:
...
- [ ] [DATA] P2. Backfill Aster perp funding into GCS — the handler + public endpoint work
      ...
      **CONFLICT — DO NOT DISPATCH AS WRITTEN (2026-07-30, conflict-check)**: the named launcher/handler
                  (`launch-mtds-perp-funding-backfill-vm.sh --perp-protocols aster` → `PerpFundingHandler`) was RETIRED 2026-07-08
                  (Aster funding now rides `derivative_ticker`, a different handler/launcher) — running this command as literally
                  written hits the unknown-protocol branch and writes false `attempted_failed` manifest rows. The current, correct
                  ground for this exact backfill is owned by
                  `plans/active/issues/aster_perp_funding_backfill_stale_launcher_and_genesis_conflict_2026_07_28.md`, which is
                  currently OPERATOR-BLOCKED (BLK-a94f446d) on the 3-way genesis-date ambiguity (2023-07-22 vs 2023-11-01 vs
                  2024-01-01 — this todo's own "2023-07-22" is one of the disputed candidates, not settled). Defer to that doc; do
                  not launch the retired command.
...
- [ ] [DATA] P2. Genesis is PER-(venue, data_type), not per-venue — encode it. Aster API availability (verified
      ...
      forward-only)**. **NOTE (2026-07-30, conflict-check)**: the funding-genesis "2023-07-22" figure this rollup
      restates is one of 3 disputed candidates per the sibling doc cited on the todo above — this rollup's remaining
      open scope (trades pre-genesis window, live book WS, margining check, below) is unaffected and stays CLEAR to
      dispatch; only the funding-genesis figure itself is unsettled. Canonize the Aster native API INTO the Tardis CEX
      benchmark schemas (klines→OHLCV, aggTrades→`trades`, premiumIndex+funding+OI→`derivative_ticker`,
      depth-WS→`book_snapshot_5`) so downstream can't tell it's not Tardis; record genesis per data_type with
      `captured`/`expected_unattempted` honest-absence for the forward-only ones. **Repo: market-tick-data-service +
      unified-api-contracts.**
