...
- [x] ✅ [DESIGN] P1. **COINBASE / DERIBIT-COMBO MVP_SCOPE membership — RESOLVED 2026-07-10 (operator decision #6: "keep
      both declared").** `DERIBIT-COMBO` added to `MVP_SCOPE["cefi"].venues` + `venue_data_types` override ({trades,
      book_snapshot_5}) + matching `VENUE_DATA_TYPE_CAPABILITIES["DERIBIT-COMBO"]` entry (`build_expected("cefi")` now
      yields `(DERIBIT-COMBO, options_chain, trades)`, previously 0). COINBASE cost-control already existed
      (`CeFiMvpRule` v11); bare `COINBASE` deliberately NOT added (retired per
      `coinbase_bare_name_migration_2026_07_06.md` in favor of `COINBASE-SPOT`). Shipped
