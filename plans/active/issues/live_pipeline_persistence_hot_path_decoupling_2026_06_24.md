...
- [ ] [VERIFY] P2. **Only remaining sub-item, corrected 2026-08-12 to match the 2026-08-10 banner above (was stale —
      the compaction-leg build this item originally described is DONE, see banner).** The warm→cold compaction leg
      described in the pre-2026-08-10 version of this todo now runs successfully (5 consecutive daily
      `live-event-log-compactor` executions confirmed, most recently 2026-08-09; `live-events/cold/` has real
      `cefi/`/`prediction/` data). Remaining scope is item (4) only: re-run the `paper(W)==batch-rerun(W)` determinism
      test now that the full three-tier pipeline actually executes end-to-end (not yet run as of 2026-08-10).
