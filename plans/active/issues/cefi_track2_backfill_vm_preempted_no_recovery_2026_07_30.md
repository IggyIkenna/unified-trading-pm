...
- [ ] [OPERATOR] P1. **Decision gate — the 5th VM died 2026-08-03; do NOT reflexively fire a 6th SPOT relaunch.** As of
      2026-08-04 the chain has died **5 times in 8 days** (3 GCP preemptions 07-28/07-30/07-31 + 2 `WORKER_STALLED`
      watchdog kills 08-01/08-03), each reaching only ~2-5% of the ~2769-day span before dying — i.e. it is effectively
      **non-completing** under chronic SPOT relaunch, exactly the Recommended-decision-B scenario this doc raised at
      filing. **Live downstream impact (new, main 2026-08-04)**: the unmet `cefi-track2-backfill-vm-terminated`
      prerequisite is currently gating **213 queued backlog tasks** (measured via `/api/state` idle-slot reports +
      `/api/backlog/.../blockers`) and is a primary contributor to the depressed fleet busy-count. Operator to rule
      between: **(a)** accept a fresh coverage-% at the genuinely-captured level and close the Track-2 chain (unblocks
      the 213 downstream tasks against honest partial coverage), or **(b)** authorize a fundamentally different
      execution approach (on-demand/non-SPOT VM, or a day-chunked multi-VM fleet with a larger completion budget) rather
      than a 6th SPOT relaunch that the measured 5× failure rate says will also die at low-single-digit-% progress. This
      is a scope/strategy call (operator-owned per this doc's "Recommended decision" §B), NOT a mechanical relaunch — a
      bare `[INFRA]` 6th relaunch should NOT be dispatched until this is ruled. Repo: unified-trading-pm (decision),
      then deployment-service (execution of whichever path). (main agt-1756f6, evidence in the 2026-08-04 Progress Log
      entry below.)
