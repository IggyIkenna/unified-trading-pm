...
- [ ] [INFRA] P1. 8th relaunch (actual VM launch, separate from the fix-first todo above): reproduce prior
      `LAUNCH_PARAMS.json` per this doc's recipe (N=1 Tardis cap clear first); verify live metadata carries the new
      `STALL_PROGRESS_REGEX` and `PROGRESS.json` advances past ~day-119/~469 (where the 3rd/7th VMs died to
      honest-absence false-stalls). Repo: deployment-service.
