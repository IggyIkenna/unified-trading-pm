...
- [ ] [INFRA] P1. 8th relaunch — FIRST fix `STALL_PROGRESS_REGEX` (defaults to literal `uploaded` in
      `launch-cefi-sharded-backfill.sh:485/807`), which never matches during a genuine honest-absence date stretch (no
      venue has instruments → nothing uploaded → timer never resets), so the watchdog kills a healthy worker regardless
      of `STALL_TIMEOUT_SEC`. Broaden to also match the per-date advance line (`Processed date=`), mirroring
      `launch-mdps-sharded-backfill.sh`'s `Processing|Skipping` precedent. Repo: deployment-service.
...
- 2026-08-09T01:5x (slot-25, review craft, dispatched on todo `-003`): Independent re-verify.
