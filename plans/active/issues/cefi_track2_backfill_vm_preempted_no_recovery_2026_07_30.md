...
- [ ] [INFRA] P1. **7th relaunch**: 6th VM (`cefi-queue-heavy-binancefutu-x17-20260806-163512`, ON_DEMAND/STANDARD) died
      via `WORKER_STALLED (no-progress-marker): no progress in 1841s (threshold=1800s)` at `date=2019-10-22` on
      `2026-08-06T17:09 UTC`, `exit_code=137`, `VM_SHUTDOWN_ON_COMPLETION=true` self-delete. Runtime ~34 min.
      `PROGRESS.json` last: `{"last_completed_date":"2019-10-21","monotonic":true,"updated":"2026-08-06T17:09:16Z"}` —
      scope `2019-01-01..2026-08-05` (~2769 days), ~295/2769 days (~10.7%). Stall is distinctive: `date=2019-10-22` was
      fully pre-flight skipped (DERIBIT only venue, already 100% covered) yet the watchdog fired 1841s after that skip
      (`do_wait` in kernel stack — waiting on a child process). This is the 4th WORKER_STALLED kill and the 7th VM
      failure overall. Before relaunching, investigate why the pipeline hangs between a fully-skipped date and the next
      date's processing — a blind relaunch hits the same stall. Repo: deployment-service.
...
- 2026-08-08T20:16Z (slot-24, review craft, dispatched on todo `-003`): Independent re-verify (8th dispatch today,
  ~27min after slot-10's last check): `gcloud compute instances list --filter="name~cefi-queue"` → empty; broader
  `name~cefi` shows 5 live `cefi-*` VMs (`cefi-fwd-…`, 2× `mdps-backfill-cefi-…`, `mdps-features-live-cefi-…`,
  `mtds-live-cefi-consolidated-…`), none `cefi-queue-*` — confirms the 6th VM remains dead since its
  `2026-08-06T17:09:47 UTC` `WORKER_STALLED` self-delete; no 7th relaunch exists.
  `cefi_track2_coverage_backfill_checkpoints_2026_07_25.md` confirmed still 3/5 done via direct grep (`-004`/`-005` both
  `[ ]`, unchanged). Gate ("genuinely completes, measured exit") remains unmet.

  **Refinement of slot-10's `-007`/slot-4 finding**: `GET /api/state` on slot 4 now shows
  `last_spawned_at 2026-08-08T19:49:35Z` / `last_ping 2026-08-08T19:55:26Z` (ping AFTER spawn this time, unlike
  slot-10's 18:25:35Z/18:19:58Z snapshot) — slot 4 DID come back alive and successfully booted since slot-10's check,
  but its `last_msg` shows a routine idle heartbeat ("285 task(s) blocked on ...sports_taxonomy...", unrelated to this
  issue), and `worker_alive`/`tmux_alive` are both `false` again now — its tmux session has since been reaped per the
  normal idle-session lifecycle (worker.md § "When idle": the watchdog reaps an idle session ~2min in, AutoSpawn
  respawns within ~60s once dispatchable work lands). So slot 4 was alive long enough to poll but was not offered `-007`
  before going idle and being reaped — consistent with slot-10's root cause (the `target_slot:4, affinity:high` binding
  gates `-007` to a slot that keeps cycling alive→idle→reaped without ever claiming it) rather than contradicting it.
  `GET /api/backlog` confirms `-007` still `status: queued, dispatched_to: None`. Not re-pinging main (already flagged
  ~30min ago via msg 4338, no new information to add — AutoSpawn should retry spawning slot 4 on its own per the
  server-owned liveness design; worth a fresh escalation only if `-007` is still unpicked-up after several more
  AutoSpawn cycles). Declining todo `-003` and skipping via `reason_code: "GATED"` per the established pattern.
