...
2. **UAC registry check** — confirmed AAVE_V3-ARBITRUM/POLYGON are declared fully `"live"` in `defi_venues.py` with real
   subgraph IDs, launch dates, and capability entries; EULER_V2 has real, _recently verified_ Goldsky subgraph IDs for
...
- [ ] [CODE] P0. **PARTIALLY RE-VERIFIED 2026-07-10 — not reproducible today via the exact function chain `/turbo`
      wraps; root cause still not identified, so checkbox stays open pending a live-HTTP cross-check.** Called
      `deployment_api.services.data_status_service.DataStatusService().get_manifest_status(service="market-tick-data-service",     start_date="2026-06-01", end_date="2026-06-21", asset_groups=["DEFI"], cloud="gcp")`
      directly against real prod GCS (this IS the function `/turbo`'s `_manifest_source` wraps, modulo the 5-min result
      cache) — all 8 originally "hidden" venues now show real, correct coverage: `AAVE_V3-ARBITRUM`
      dates_found=394/dates_expected=63 (100% completion within window), `AAVE_V3-POLYGON` 331/63, `SPARK-ETHEREUM` 100%
      (21/21), `MANTLE`/`STADER`/`STAKEWISE`/ `SWELL-ETHEREUM` all 100% (21/21), plus the 4 flagged-for-cross-check
      venues also now correct: `COMPOUND_V3-{ARBITRUM,BASE,ETHEREUM,OPTIMISM}` all showing real 76-82% completion,
      `FLUID-ETHEREUM` 100% (21/21). **`PUFFER-ETHEREUM` shows a DIFFERENT, smaller anomaly** — `dates_found=0` despite
      `capture_status_counts.captured=21` (an internal inconsistency between the two computations inside
      `_build_venue_breakdown`/`_apply_mtds_honest_coverage`, not "hidden/absent" — worth its own smaller follow-up, not
      the same symptom as the original finding). `HYPERLIQUID`/`ASTER` do not appear anywhere in the DEFI response at
      all (0 substring matches) — consistent with the tracker's existing operator-confirmed intentional CEFI-side-only
      scoping for these hybrid venues, not a new instance of this bug. **Not yet done**: a live HTTP `/turbo` call with
      the cache explicitly cleared (`POST /turbo/clear` first) to rule out this being a stale-cache artifact rather than
      a genuine data-path fix; and the ROOT CAUSE of the original 2026-07-07 finding was never identified, so it's
      unknown whether it was fixed by an intervening change (candidate: the 2026-07-07
      `_derive_instrument_type`→`_split_by_instrument_type` manifest-writer fix,
      `honest_coverage_shard_dimension_model_definitional_data_2026_07_07.md`) or was itself a transient/stale-index
      artifact that self-corrected via normal consolidator activity. Recommend: (1) the live-HTTP+cache-clear
      cross-check to close this for real, (2) if confirmed fixed, downgrade this todo to a VERIFY-closed rather than a
      CODE fix (no code change was made this session — read-only investigation only). ~~Original todo text: root-cause
      the deployment-api read/aggregation bug that produces `0/0` for `AAVE_V3-ARBITRUM`, `AAVE_V3-POLYGON`, and drops
      `SPARK` entirely, despite real captured rows existing under the exact canonical `(venue, chain)` key in the source
      manifest.~~
...
- [ ] [VERIFY] P0. Directly query the live turbo API
      (`GET /api/data-status/turbo?service=market-tick-data-service&asset_group=DEFI`) for `venue=HYPERLIQUID`,
      `venue=ASTER`, `venue=COMPOUND_V3`, and `venue=FLUID&chain=ETHEREUM` to confirm whether they're ALSO hidden —
      these are large (HYPERLIQUID: 3.77M rows, ASTER: 1.07M rows, COMPOUND_V3: 233K rows), so if hidden, this is a
      bigger deal than the original 3-venue finding. Raised bumping this to P0 because HYPERLIQUID/ASTER are the two
      highest-volume DEFI-side datasets found hidden anywhere so far.
- [ ] [VERIFY] P3. Spot-check one shard's actual parquet contents for the 5 tiny liveness-marker-shaped venues
      (MANTLE/PUFFER/STADER/STAKEWISE/SWELL-ETHEREUM, ~1 row/entry) before treating them as commercially meaningful —
      the bug is real either way, but the fix priority should reflect actual data volume, not just bug presence.
