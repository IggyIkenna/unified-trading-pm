...
- [ ] [OPERATOR] P3. **Decide whether to register the 35 leagues with no canonical UAC registry entry** (named in the
      gate-(b)/(c) section above — mostly reserve/U18-U20/regional-tier/friendlies leagues) as `LeagueDefinition`s,
      mirroring the curated-universe expansion pattern shipped elsewhere. Until ruled, their `day=2026-04-14`
      `fixtures_schedule` data has no canonical folder to land in and stays untouched. **Done when**: operator rules
      register-vs-leave-unmapped; if register, add the `LeagueDefinition`s and re-run the DATA P2 recovery for their raw
      folders; if leave unmapped, flip this doc's `status` to `resolved` noting the explicit non-registration decision.
...
with un-dispatchable open work (`regen_backlog_from_plan.py` only reads `plans/archive/2026_08/*.md`) invisible to every
active-corpus audit. `check_terminal_status_archived.py` does not catch this direction (it only flags a
_terminal_-status doc left in `plans/archive/2026_08/`, not a _non-terminal_ doc sitting in `plans/archive/`).
Corrected: added the missing `- [ ]` todo above (the § 2 "todos not prose" rule), moved this doc back to
`plans/archive/issues/`, and fixed the 8 corpus referrers that pointed at the archive path. No content judgment changed
— the doc's own diagnosis (35 leagues structurally unmapped, pending an operator decision) stands as-is.
