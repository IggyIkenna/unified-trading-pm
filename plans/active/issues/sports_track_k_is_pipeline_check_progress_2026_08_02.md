...
depends_on: []
locked_by:
resolved_by: "slot-16 (data_engineering), sports_track_k_is_pipeline_check_progress-010, 2026-08-02T22:04Z"
...
## Mid (2025-12-24) checkpoint -- COMPLETE (2026-08-02T21:14Z, slot 7, data_engineering)

Driver PID 283424 (launched 19:33:40Z, the same driver referenced throughout the Progress Log entries below as "slot-7's
driver") ran to completion: 21/21 legs attempted, report written to
`plans/audit/results/data_pipeline_e2e_check_is_2025_12_24.md` (+ sibling `.json`),
`total=21 passed=12 failed=9 ambiguous=0 skipped=0` -- **identical shape to the baseline (2025-12-20) checkpoint's 12/9
split**, consistent with the two already-documented false-negative/known-gap classes accounting for all 9 failures:

- **6x `skip_signal_not_found`** (API_FOOTBALL 539.8s, OPEN_METEO 216.6s, TRANSFERMARKT 174.8s, SOCCER_FOOTBALL_INFO
  172.7s, UNDERSTAT 216.8s, FOOTYSTATS 215.3s) -- matches the already-root-caused per-league checker false-negative
  class documented in the "Baseline checkpoint" section above (`process_preflight.py`'s
  `_has_sports_per_league_in_scope` branch structurally suppresses the coarse `SKIP date=...` line the checker greps
  for). **KNOWN, do not re-investigate.** Note: this checkpoint's OPEN_METEO skip failure reads plain
  `skip_signal_not_found` (216.6s), NOT the baseline's distinct `vm_run_not_successful:launcher_script_nonzero_rc=1`
  failure mode on the same venue/leg -- i.e. the baseline's OPEN_METEO issue (still open, its own todo below) did NOT
  reproduce here; this run's OPEN_METEO skip failure is the same per-league false-negative every other venue hit, not a
  second instance of the unexplained baseline anomaly.
- **3x BETFAIR (force/skip/live, ALL legs)**: `no_parquet_at:.../venue=BETFAIR/` +
  `manifest_status_invalid:no_matching_row` (449.2s / 463.9s / not separately timed live -- exit=0 but 0 parquet rows
  for all 3 legs). Matches BETFAIR's already-documented BLOCKED-CREDENTIALS/zero-PROD-rows state (same as baseline's
  identical 3-leg failure). **KNOWN, pre-existing gap, NOT a new finding -- do not re-investigate as part of this
  checkpoint's scope.**

All 6 non-BETFAIR venues' `force` and `live` legs PASSED cleanly (170-255s force legs, 170-233s live legs, each with the
standing `--mode batch` caveat note already documented for baseline). **Net: this checkpoint's data-collection is
complete and its 9 failures are fully accounted for by the two known classes above -- no new failure modes found, no
further triage needed for mid (2025-12-24).**
...
## Final (2025-12-18) checkpoint -- driver died, NO report produced (2026-08-02T~20:30Z, slot 15, data_engineering)

Re-verified live at dispatch of task `sports_consolidated_native_ao_extract-029` (the parent flip task): `ps -p 474834`
returns NOTHING -- the final-checkpoint driver documented above (launched by slot 12,
`--day 2025-12-18 --legs force,skip,live`) is **no longer running**, and no
`data_pipeline_e2e_check_is_2025_12_18.{md,json}` report exists anywhere under `plans/audit/results/` (confirmed via
`find`, both separator variants). `gcloud compute instances list --filter="name~instr-backfill-sports-pchk"` shows only
ONE VM now, belonging to the still-live MID checkpoint driver (slot-7, `0802193411-cab3` hash) -- the final checkpoint's
own VMs are gone too. Git log confirms no report was ever committed for this day. **This checkpoint made zero further
progress past the single API_FOOTBALL force-leg failure documented above, then its driver died silently** -- most likely
because it was launched as a harness-tracked `run_in_background` process scoped to slot-12's own Claude Code session,
and that session ended (rotated/reassigned/context-reset) before the 21-leg matrix finished, killing its child process
tree. **Process lesson for future pickers**: a multi-hour driver launched via `run_in_background` does not survive its
spawning session ending -- if you need a driver to survive a session boundary, it needs to be launched in a way that's
independent of this specific interactive session's lifetime (out of scope to solve here; flagging so the eventual
final-checkpoint relauncher knows to re-verify liveness with fresh `ps` rather than trusting this doc's earlier
"healthy" status, which was accurate only at write-time). **The final (2025-12-18) checkpoint needs a full relaunch from
scratch** -- its own P1 todo below is updated to reflect this.

## Final (2025-12-18) checkpoint -- COMPLETE (2026-08-02T22:04Z, slot 16, data_engineering)

Driver PID 921523 (relaunched after PID 474834 died, see sections above) ran to completion: 21/21 legs attempted, report
written to `plans/audit/results/data_pipeline_e2e_check_is_2025_12_18.md` (+ sibling `.json`),
`total=21 passed=12 failed=9 ambiguous=0 skipped=0` -- **identical shape to both prior checkpoints' 12/9 split**, both
known classes fully accounting for all 9 failures, no new failure modes:

- **6x `skip_signal_not_found`** (API_FOOTBALL, OPEN_METEO, TRANSFERMARKT, SOCCER_FOOTBALL_INFO, UNDERSTAT, FOOTYSTATS
  skip legs) -- known per-league checker false-negative, already root-caused above. Note: OPEN_METEO's skip failure here
  is the plain `skip_signal_not_found`, NOT the baseline's distinct `vm_run_not_successful` anomaly -- consistent with
  the mid checkpoint's finding that the baseline anomaly does not reproduce.
- **3x BETFAIR** (force/skip/live, ALL legs): `no_parquet_at:.../venue=BETFAIR/` +
  `manifest_status_invalid:no_matching_row` -- known BLOCKED-CREDENTIALS/zero-PROD-rows gap, matches both prior
  checkpoints exactly.

All 6 non-BETFAIR venues' `force` and `live` legs PASSED cleanly this run (171-227s force legs, 178-492s live legs) --
notably API_FOOTBALL's force leg PASSED here (unlike the killed prior driver's single completed leg, which hit the
manifest-consolidation-lag false-negative documented above); that false-negative appears to be a narrow race-window
condition, not deterministic per (venue,day) as originally hypothesized -- worth a caveat on the P2 checker-fix todo but
not worth re-opening investigation for this already-complete checkpoint.

**All 3 checkpoints (baseline 2025-12-20, mid 2025-12-24, final 2025-12-18) are now COMPLETE and committed**:

- `plans/audit/results/data_pipeline_e2e_check_is_2025_12_20.md` (12/21 passed)
- `plans/audit/results/data_pipeline_e2e_check_is_2025_12_24.md` (12/21 passed)
- `plans/audit/results/data_pipeline_e2e_check_is_2025_12_18.md` (12/21 passed)

Every failure across all 63 legs maps to one of the two known classes (per-league skip false-negative, BETFAIR
credentials gap) plus the one still-open OPEN_METEO baseline anomaly (its own todo, unaffected by this checkpoint).
Track K (IS)'s parent todo is now eligible to flip.
...
- [x] ✅ [DATA] P1. Run the mid (2025-12-24) checkpoint, same 7-venue force/skip/live matrix -- unified-trading-pm@(this
      ...
      `total=21 passed=12 failed=9` -- both known classes (6x per-league skip false-negative, 3x BETFAIR
      BLOCKED-CREDENTIALS), no new failure modes. See "Mid (2025-12-24) checkpoint -- COMPLETE" section above. (repo:
      instruments-service, skill-driven)
- [x] ✅ [DATA] P1. Run the final (2025-12-18) checkpoint, same 7-venue force/skip/live matrix --
      unified-trading-pm@(this commit). COMPLETE: 21/21 legs ran, report at
      `plans/audit/results/data_pipeline_e2e_check_is_2025_12_18.md`, `total=21 passed=12 failed=9` -- both known
      classes (6x per-league skip false-negative, 3x BETFAIR BLOCKED-CREDENTIALS), no new failure modes. See "Final
      (2025-12-18) checkpoint -- COMPLETE" section above. (repo: instruments-service, skill-driven)
...
- [x] ✅ [REVIEW] P2. Once all 3 checkpoints are done, flip `sports_consolidated_native_ao_extract-029` in
      `/plans/active/sports_consolidated_native_ao_extract_2026_07_25.md` citing all 3 report paths, and mark this doc
      `status: resolved` -- unified-trading-pm@(this commit). All 3 checkpoints (2025-12-20, 2025-12-24, 2025-12-18) now
      complete + committed, all 63 legs' failures accounted for by the two known classes + the one still-open OPEN_METEO
      baseline anomaly (its own separate todo). Parent todo flipped, this doc marked resolved. (repo:
      unified-trading-pm)

## Progress Log

- 2026-08-02T22:04Z (slot 16, data_engineering, task `sports_track_k_is_pipeline_check_progress-010`, the same task this
  slot has been monitoring since 20:24Z): driver PID 921523 ran to completion, 21/21 legs, report committed at
  `plans/audit/results/data_pipeline_e2e_check_is_2025_12_18.md` (+ `.json`), `total=21 passed=12 failed=9`. All 9
  failures map cleanly onto the two already-documented classes (6x per-league skip-signal false-negative, 3x BETFAIR
  BLOCKED-CREDENTIALS) -- no new failure modes. See "Final (2025-12-18) checkpoint -- COMPLETE" section above for the
  full breakdown. **All 3 checkpoints are now complete** (baseline+mid+final all committed, 12/21 passed each, identical
  failure shape) -- flipped this doc's final-checkpoint todo AND the parent-flip todo, flipped
  `sports_consolidated_native_ao_extract-029` in the parent plan citing all 3 report paths, marked this doc
  `status: resolved`. Two todos remain genuinely open and out of scope for this task: the P2 checker-fix todo (skip-leg
  per-league false-negative) and the OPEN_METEO baseline `vm_run_not_successful` investigation (never reproduced in mid
  or final checkpoints, still unexplained for baseline specifically) -- both left for future dispatch, not blocking the
  Track K (IS) parent flip.
- 2026-08-02T22:51Z (slot 9, data_engineering, task `sports_track_k_is_pipeline_check_progress-012` -- Nth re-dispatch
  of this exact task id, same collision as every prior dispatch documented below): re-verified ground truth per Resume
  Instructions step 1 before touching anything. `ps -o pid,etime,rss,cmd -p 921523` confirms slot-16's final-checkpoint
  driver (`--day 2025-12-18 --legs force,skip,live`) still ALIVE, elapsed 1:25:39, RSS ~187MB, healthy. No
  `data_pipeline_e2e_check_is_2025_12_18.{md,json}` report yet (full directory listing checked, not a hyphenated glob).
  `gcloud compute instances list --filter="name~instr-backfill-sports-pchk"` shows exactly 1 VM
  (`instr-backfill-sports-pchk-0802202440-s-1571-betfair`, run_ts `0802202440` matches slot-16's driver, now on
  BETFAIR's skip leg) -- no orphans, no duplicates. Did NOT launch a duplicate driver. Releasing via
  `/skip-current-task` -- no new information vs. prior checks beyond a fresher timestamp confirming progress; the
  collision-gate ask is already tracked at
  `/plans/active/issues/ao_backlog_no_collision_gate_long_running_driver_todos_2026_08_02.md`, not re-filing.
- 2026-08-02T21:25Z (slot 10, [REVIEW] task `sports_track_k_is_pipeline_check_progress-005`, dispatched to flip the
  parent `-029` checkbox -- same collision every prior dispatch of this task id has already documented): re-verified
  ground truth per Resume Instructions step 1 before touching anything. `ps -o pid,etime,rss,cmd -p 283424,921523`:
  283424 (mid, 2025-12-24) no longer running (expected -- its report already committed 21:14Z); 921523 (final,
  2025-12-18) still ALIVE, elapsed 1:00:31, RSS ~166MB, healthy.
  `ls plans/audit/results/ | grep data_pipeline_e2e_check_is` confirms only `2025_12_20` (baseline) and `2025_12_24`
  (mid) reports exist -- no `2025_12_18` report yet. Only 1 VM running
  (`instr-backfill-sports-pchk-0802202440-s-1571-understat`), consistent with the final driver's own skip-leg, now on
  UNDERSTAT -- no orphans/duplicates. **Precondition ("all 3 checkpoints done") is still false** -- final checkpoint not
  yet complete. Did NOT flip `sports_consolidated_native_ao_extract-029`. Not re-filing the collision/no-gate complaint
  -- already tracked at `/plans/active/issues/ao_backlog_no_collision_gate_long_running_driver_todos_2026_08_02.md` per
  slot-4's entry below. Releasing via `/skip-current-task` rather than fabricate completion.
- 2026-08-02T21:14Z (slot 7, data_engineering, task `sports_track_k_is_pipeline_check_progress-003`, the mid-checkpoint
  driver this doc's own Progress Log has been tracking as "slot-7 PID 283424" since 19:33Z): driver ran to completion,
  21/21 legs, report committed at `plans/audit/results/data_pipeline_e2e_check_is_2025_12_24.md` (+ `.json`),
  `total=21 passed=12 failed=9`. All 9 failures map cleanly onto the two already-documented classes (6x per-league
  skip-signal false-negative, 3x BETFAIR BLOCKED-CREDENTIALS) -- no new failure modes, no further investigation needed
  for this checkpoint. Flipped this todo's checkbox; see "Mid (2025-12-24) checkpoint -- COMPLETE" section above for the
  full breakdown. Final (2025-12-18) checkpoint's driver (slot-16, PID 921523) confirmed still alive (53:12 elapsed) at
  time of this edit -- not yet done, out of scope for this task. Did NOT touch the parent flip todo (still gated on
  final checkpoint completing).
- 2026-08-02T~21:35Z (slot 8, data_engineering, task `sports_track_k_is_pipeline_check_progress-012` -- Nth re-dispatch
  of this exact task id, same collision as every prior dispatch below): re-verified ground truth per Resume Instructions
  step 1 before touching anything. `ps -o pid,etime,rss,cmd -p 283424,921523` confirms BOTH remaining drivers still
  ALIVE and healthy: mid (2025-12-24, slot-7 PID 283424) at 1:30:04 elapsed; final (2025-12-18, slot-16 PID 921523) at
  39:16 elapsed. Cross-checked VM `run.log` chunk headers directly (not trusting VM-name leg-letters, per this doc's own
  caution): `...-0802202440-f-1571-transfermarkt` reads `--- Chunk 1/1: 2025-12-18 → 2025-12-18 ---` (confirms it's
  slot-16's final driver, now on TRANSFERMARKT); `...-0802193411-s-cab3-betfair` reads
  `--- Chunk 1/1: 2025-12-24 → 2025-12-24 ---` (confirms it's slot-7's mid driver, now on BETFAIR). No
  `data_pipeline_e2e_check_is_2025_12_{24,18}.{md,json}` report exists yet (full directory listing checked, not a
  hyphenated glob). No orphans, no duplicates, did NOT launch a duplicate driver. Releasing via `/skip-current-task` --
  same collision 9+ prior dispatches of this task id have already documented (slot-8 x2, slot-15 x2, slot-4 x1, slot-6
  x1, slot-12 x1, slot-10 x1). **Repeating the standing ask for main**: this backlog task keeps re-entering the queue
  every dispatch cycle against the same still-in-flight drivers with no prereqs/collision gate -- please gate it on the
  two drivers' completion (report file existing for both `2025_12_24` and `2025_12_18`) instead of blind re-dispatch.
- 2026-08-02T21:03Z (slot 4, data_engineering, task `sports_track_k_is_pipeline_check_progress-008` -- this slot's 3rd
  dispatch against this exact collision, Nth overall): re-verified ground truth per Resume Instructions step 1.
  `ps -p 283424,921523` confirms both drivers still alive: mid (2025-12-24, slot-7) at 1:34:41 elapsed, final
  (2025-12-18, slot-16) at 43:53 elapsed. No `data_pipeline_e2e_check_is_2025_12_{24,18}.{md,json}` report yet (checked
  the full `plans/audit/results/` listing, not a hyphenated glob). Did NOT launch a duplicate driver. **Filed
  `/plans/active/issues/ao_backlog_no_collision_gate_long_running_driver_todos_2026_08_02.md`** -- converting the 7+
  prior prose flags in this log ("needs a prereqs/collision gate", slot-8 x2/slot-15/slot-10/slot-6/slot-12) into an
  actual tracked issue doc with concrete `[BACKEND]` todos, per the Findings Closure HARD RULE (a flag left only in
  prose here was never closing anything). Considered applying a manual park directly (this session has live filesystem
  access to `agent-orchestrator/data/config/backlog.yaml`, since the orchestrator server runs on this same host) but
  declined -- a hard `prerequisites` gate that's never flipped back true would permanently stop this todo from being
  offered even once genuinely ready, which is worse than the current self-correcting-but-wasteful status quo; full
  reasoning in the new issue doc. Releasing via `/skip-current-task`.
- 2026-08-02T21:00Z (slot 15, data_engineering, task `sports_track_k_is_pipeline_check_progress-012` -- Nth re-dispatch
  of this exact task id): re-verified ground truth per Resume Instructions step 1 before touching anything. Both
  remaining drivers still alive and progressing, no collision: mid (2025-12-24, slot-7 PID 283424) elapsed 1:26:55;
  final (2025-12-18, slot-16 PID 921523) elapsed 36:08. No `data_pipeline_e2e_check_is_2025_12_{24,18}.{md,json}` report
  exists yet. `gcloud compute instances list --filter="name~instr-backfill-sports-pchk"` shows exactly 2 VMs
  (`...-0802193411-s-cab3-betfair` = slot-7's mid driver on BETFAIR; `...-0802202440-f-1571-transfermarkt` = slot-16's
  final driver on TRANSFERMARKT force leg) -- no orphans, no duplicates. Did NOT launch a duplicate driver. Releasing
  via `/skip-current-task` -- same collision 8+ prior dispatches of this task id have already documented; repeating the
  standing ask for main: gate this backlog task on the two drivers' completion (report file existing) instead of blind
  re-dispatch every cycle.
- 2026-08-02T~21:20Z (slot 4, data_engineering, task `sports_consolidated_native_ao_extract-029`, dispatched immediately
  after releasing `-012` above -- same flip precondition slot-15 already checked): re-verified both remaining
  checkpoints' drivers are still alive (`ps -p 283424,921523`: mid at 1h22m elapsed, final at 31min elapsed, neither
  exited) and neither has produced a report yet (`plans/audit/results/` still only has `2025_12_20` + unrelated dates).
  Precondition ("all 3 checkpoints done") is still false. **Did NOT flip.** Releasing via `/skip-current-task` rather
  than fabricate completion -- no new information vs. slot-15's last check, just a fresher timestamp confirming both
  drivers are still progressing, not stalled.
- 2026-08-02T~21:15Z (slot 4, data_engineering, task `sports_track_k_is_pipeline_check_progress-012` -- Nth re-dispatch
  of this exact task id, same collision already documented 6+ times below): re-verified ground truth per Resume
  Instructions step 1 before touching anything. `ps -p 921523` confirms slot-16's final-checkpoint driver still ALIVE
  (elapsed 30:00, RSS ~546MB, healthy). No `data_pipeline_e2e_check_is_2025_12_18.{md,json}` report yet (checked full
  directory listing: only `2025_12_20`/`2026_03_15`/`2026_07_05`/`2026_07_13` reports exist).
  `gcloud compute instances list --filter="name~instr-backfill-sports-pchk"` shows exactly 2 VMs, both confirmed
  legitimate by run_ts+hash match to the two known live drivers: `...-0802193411-f-cab3-betfair` (slot-7's mid,
  `2025-12-24`, driver PID 283424, still alive per `ps aux`) and `...-0802202440-s-1571-open-meteo` (slot-16's final,
  `2025-12-18`, driver PID 921523, on its skip leg). No orphans, no duplicates. Did NOT launch a duplicate driver.
  Releasing via `/skip-current-task`. **Repeating the flag prior slots (slot-8 x2, slot-15, slot-10, slot-6, slot-12)
  have already raised for main**: this task id keeps re-entering the queue every dispatch cycle against the SAME
  still-in-flight driver with no prereqs/collision gate -- it should be gated on the driver's own completion (report
  file existing) rather than re-dispatched blind.
- 2026-08-02T20:45Z (slot 6, data_engineering, task `sports_track_k_is_pipeline_check_progress-012` -- 2nd dispatch of
  this exact task id, ~6min after slot-12's identical check directly below): re-verified ground truth per Resume
  Instructions step 1. `ps -p 921523` confirms slot-16's final-checkpoint driver still ALIVE (elapsed 20:29, healthy,
  RSS ~535MB). No `data_pipeline_e2e_check_is_2025_12_18.{md,json}` report yet (checked full directory listing, not a
  hyphenated glob). `gcloud compute instances list --filter="name~instr-backfill-sports-pchk"` shows
  `instr-backfill-sports-pchk-0802202440-l-1571-api-football` RUNNING (slot-16's driver, run_ts matches, now on the live
  leg) and `...-0802193411-f-cab3-footystats` STOPPING (slot-7's mid-checkpoint driver, self-deleting on completion) --
  no orphans, no duplicates. Did NOT launch a duplicate driver. Releasing via `/skip-current-task`; same collision
  slot-12 already flagged, no new information to add.
- 2026-08-02T~20:40Z (slot 12, data_engineering, task `sports_track_k_is_pipeline_check_progress-012`, dispatched to run
  the final (2025-12-18) checkpoint -- collision with slot-16's already-relaunched driver from the entry directly
  below): per this doc's own Resume Instructions step 1, re-verified ground truth before touching anything.
  `ps -p 921523` confirms slot-16's final-checkpoint driver is ALIVE (~15:42 elapsed, healthy,
  `--day 2025-12-18 --legs force,skip,live`). No `data_pipeline_e2e_check_is_2025_12_18.{md,json}` report exists yet
  (checked the full `ls plans/audit/results/ | grep data_pipeline_e2e_check_is` listing, not a single hyphenated glob,
  per this doc's own file-naming-trap caution).
  `gcloud compute instances list --filter="name~instr-backfill-sports-pchk"` shows exactly 2 VMs, both confirmed
  legitimate via name/run_ts match: `instr-backfill-sports-pchk-0802202440-s-1571-api-football` (slot-16's
  final-checkpoint driver, run_ts `0802202440` matches its 20:24Z launch, currently on API_FOOTBALL's skip leg) and
  `instr-backfill-sports-pchk-0802193411-l-cab3-understat` (slot-7's mid-checkpoint driver, already documented above,
  now progressed to UNDERSTAT's live leg). No duplicates, no orphans. Did NOT launch a duplicate driver for this
  checkpoint -- releasing via `/skip-current-task` since both remaining checkpoints (mid + final) are already being
  actively worked by their existing owners (slot-7, slot-16) and neither has produced a report yet for me to act on.
- 2026-08-02T20:24Z (slot 16, data_engineering, task `sports_track_k_is_pipeline_check_progress-010`, dispatched to run
  the final (2025-12-18) checkpoint): per Resume Instructions step 1, checked for an existing report (none — correct
  underscored glob), running VMs (`gcloud compute instances list --filter="name~instr-backfill-sports-pchk"` — only
  slot-7's mid (2025-12-24) driver's VMs, confirmed via name's embedded `run_ts` matching slot-7's own launch time), and
  driver processes (`ps aux | grep pipeline_e2e_check` — slot-12's PID 474834 from the entry below was NO LONGER ALIVE,
  and no report file existed for `2025_12_18`). **Conclusion: slot-12's final-checkpoint driver died mid-run without
  producing a report** (likely killed when that session's Bash-tool-backgrounded process was torn down at session end —
  the harness parents a `run_in_background` process to the spawning session, so it does not outlive that session the way
  a `nohup`'d process would, per this doc's own established caution against `nohup`). No partial state (no JSON/MD stub,
  no leftover files) was left behind to resume from. This slot's `instruments-service` worktree also had no `.venv` yet
  (fresh slot clone) — ran `uv sync` to provision it before launching anything. Re-verified no collision immediately
  before launch, then launched a fresh final-checkpoint driver: PID 921523,
  `GCP_PROJECT_ID=central-element-323112 .venv/bin/python -u scripts/pipeline_e2e_check.py --asset-group SPORTS --day 2025-12-18 --legs force,skip,live --report-dir ../unified-trading-pm/plans/audit/results`,
  harness-tracked `run_in_background` (not `nohup`), from this slot's `instruments-service/` worktree. Will monitor to
  completion and file the report + any new findings once the 21-leg matrix finishes. Per this doc's established
  practice, the known false-negative classes (6x `skip_signal_not_found` per-league false-negative;
  `manifest_status_invalid:no_matching_row` on force legs for a `-test`-bucket-first-ever day) are EXPECTED on most/all
  of this checkpoint's legs and will not be re-investigated per-venue. **Note**: slot-15 independently found the same
  driver-death fact ~6 min earlier (task `sports_consolidated_native_ao_extract-029`, see the entry immediately below)
  but released without relaunching -- this entry's relaunch is the actual fix for that finding, not a duplicate.
- 2026-08-02T~20:27Z (slot 10, data_engineering, task `sports_track_k_is_pipeline_check_progress-008` -- 4th dispatch of
  this exact task id, same collision as the 3 prior dispatches below): re-verified ground truth per Resume instructions
  step 1 before touching anything. `ps aux` confirms slot-7's driver (PID 283424,
  `--day 2025-12-24 --legs force,skip,live`) still ALIVE, elapsed 54:34, healthy, no crash signature. No
  `data_pipeline_e2e_check_is_2025_12_24.{md,json}` report exists yet (checked both underscore and hyphen glob
  variants). Also noted: the final (2025-12-18) checkpoint's driver has been relaunched fresh by slot-16 (PID 921523,
  elapsed 03:46, healthy) since the prior driver (PID 474834) died silently. Did NOT launch a duplicate mid-checkpoint
  driver. Releasing via `/skip-current-task {"reason_code": "OTHER"}`. Flagging again for main: `-008` needs a
  `prereqs`/collision gate on the live driver so it stops re-entering the queue every cycle until slot-7 finishes and
  commits its report.
- 2026-08-02T~20:30Z (slot 15, data_engineering, task `sports_consolidated_native_ao_extract-029`, dispatched to flip
  the parent Track K checkbox): re-verified ground truth before touching anything, per this doc's own established
  practice. Mid (2025-12-24): slot-7's driver (PID 283424) still alive and progressing (confirmed via `ps aux` +
  matching VM run_ts+hash), no report yet -- consistent with the immediately-preceding entry below. Final (2025-12-18):
  **NEW finding** -- `ps -p 474834` returns nothing, the driver documented as "IN PROGRESS" in this doc's "Final
  checkpoint" section is dead, no report exists, no VMs remain for it. Only 1 of 21 legs for this checkpoint ever
  completed before the driver died silently (most likely: a `run_in_background` process doesn't outlive its spawning
  session). Updated the "Final checkpoint" section + its own P1 todo above to reflect this so the next picker relaunches
  from scratch instead of assuming it's still progressing. **Did NOT flip `sports_consolidated_native_ao_extract-029`**
  -- its precondition (all 3 checkpoints done) is further from true than the last check, not closer. Releasing via
  `/skip-current-task` rather than fabricating a completion claim; this parent flip task should not be re-dispatched
  until both remaining checkpoints actually finish and commit reports.
- 2026-08-02T~20:25Z (slot 15, data_engineering, task `sports_track_k_is_pipeline_check_progress-008` -- 3rd dispatch of
  this exact task id, same collision as the two prior dispatches below): re-verified ground truth per Resume
  instructions step 1 before touching anything. `ps aux` confirms slot-7's driver (PID 283424,
  `--day 2025-12-24 --legs force,skip,live`) is still alive (~14min CPU time accrued, up from the last check).
  `gcloud compute instances list --filter="name~instr-backfill-sports-pchk"` shows 2 VMs, both bearing the
  `0802193411-cab3` run_ts+hash already confirmed in this doc as slot-7's own legitimate mid-checkpoint driver, now
  progressing through SOCCER_FOOTBALL_INFO/TRANSFERMARKT venues -- further along than the 19:52Z check, i.e. genuinely
  advancing, not stalled. No `data_pipeline_e2e_check_is_2025_12_24.{md,json}` report exists yet (matrix incomplete).
  Did NOT launch a duplicate driver. Releasing via `/skip-current-task` -- collision, not gated; this checkpoint is
  still being worked by slot-7. Flagging for main: this task id has now been re-dispatched 3 times (slot-8 x2, slot-15
  x1) against the same still-in-flight driver -- the backlog entry for `-008` likely needs a `prereqs`/collision gate
  (or the driver needs to finish + commit its report) so it stops re-entering the queue every cycle.
