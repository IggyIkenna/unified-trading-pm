...
## Superseded (duplicate-frontmatter artifact — see Progress Log entry below)

The `73010b58e1` push accidentally concatenated the doc's original (pre-audit) frontmatter+todo block ahead of
the corrected one. The 5 todos below are that stale duplicate, converted to explicit SUPERSEDED entries (not
deleted) so the todo-count ratchet stays conserved across the correction — their real, current-content
equivalents are the todos in the `## Todos` section above.

- [x] [BACKEND] P1. SUPERSEDED-DUPLICATE — stale pre-audit copy of "Audit the current state of
      `strategy_service/position/storage/...` + `position/api/`"; superseded by the completed todo 1 above.
- [x] [BACKEND] P1. SUPERSEDED-DUPLICATE — stale pre-audit copy of "Scope the 'forced to read the same way'
      mechanism"; superseded by the central-reader-build todo above (the cross-service pattern is now confirmed,
      not open).
- [x] [BACKEND] P2. SUPERSEDED-DUPLICATE — stale pre-audit copy of "Design the venue-subscription reconciliation
      check"; superseded by the equivalent P2 todo above.
- [x] [BACKEND] P2. SUPERSEDED-DUPLICATE — stale pre-audit copy of "Design each service's independent
      trades-vs-position-monitor reconciliation"; superseded by the equivalent P2 todo above.
- [x] [REVIEW] P3. SUPERSEDED-DUPLICATE — stale pre-audit copy of "Once centralized, revisit the venue-eligibility
      resolver work"; superseded by the equivalent P3 todo above.
...
- **2026-08-21** — `safe-doc-push.sh`'s reconcile pass concatenated this doc's stale-origin content with the
  new content into one 324-line file with two frontmatter blocks (the isolation-mode fallback + autostash-chain
  interaction; landed as `73010b58e1`) — corrected in a follow-up push to the single clean version above. If
  you're reading a duplicated/broken version of this file, that push didn't land; re-pull.
