...
- [ ] [DESIGN] P2. **`RecursiveLoopOrchestrator` translation layer for `CARRY_RECURSIVE_BORROW_LENDING_ONLY` /
      ...
      **RULED 2026-08-09 (operator, interactive): all 3 numbers in the 2026-08-09 DRAFT PROPOSAL below are ADOPTED
      ...
      explicit translation layer, not a registry-case addition. **Next step**: a code-landscape mapping pass is scoping
      the actual translation-layer build (LTV-table wiring + `RecursiveLoopRequest`→`AtomicInstruction`/ `AtomicLeg`
      translation) into concrete, bounded implementation todos — not yet filed, in progress same session.
