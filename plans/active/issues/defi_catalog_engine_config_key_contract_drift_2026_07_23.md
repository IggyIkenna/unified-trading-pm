...
universe — separate finding, see the linked curtailment doc) and the 6 already-`_ENGINE_DRIVABLE_ARCHETYPES` beyond
...
- [ ] [DESIGN] P0. **Human design decision for 5 broken archetypes (66 rows)** — `RULES_DIRECTIONAL_CONTINUOUS`,
      ...
      `xfail(strict=True)`.
- [ ] [BACKEND] P2. **Scope + build the liquidation-feed live-injection integration** for `LIQUIDATION_CAPTURE` +
      `ARBITRAGE_MEV_LIQUIDATION_BUNDLE` — the per-archetype table above promised "new todo" (2026-07-23) but no
      checkbox ever materialized (doc's own 2026-08-06 audit: "2 untracked prose-only design items"); the live on-chain
      liquidation-opportunity feed must inject per-event identity params into a running engine instance (the
      RecursiveLoopOrchestrator translation-layer half is tracked in
      `defi_archetype_universe_no_curtailment_mechanism_2026_07_23.md`). Tracked 2026-08-06 plan_reconciler agt-24f4b0
      per "every follow-up is a `- [ ]` todo".
