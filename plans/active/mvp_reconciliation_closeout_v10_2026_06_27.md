...
| ----------------------------------------------------------------------------- | ----------- | ----------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------- | ----------------------------- |
| `instruments_mtds_subset_consistency_remediation_2026_06_17.md`               | 1223, 1234  | #1 sports = 94 leagues (`SportsMvpRule` 4-league EPL/LA_LIGA still stated as scope) | **FIX** (annotate v10) — CONFIRMED ACTIVE CONFLICT                                                                                                                                                                                                                                                                                                                                                                      |
| `cefi_manifest_canonicalisation_2026_06_01.md`                                | 176-273     | #2 `options_chain→{trades}` path shape                                              | **HISTORICAL-CONTEXT-OK** — describes GCS hive path axis (data_type=trades within options_chain shard), NOT a per-strike trades backfill instruction                                                                                                                                                                                                                                                                    |
| `tradfi_multisource_backfill_2026_06_22.md`                                   | 96-114, 123 | #6 ohlcv_1s in EU count + databento billing floor                                   | **HISTORICAL-CONTEXT-OK** — ohlcv_1s references describe (a) databento billing-level floor clips (not an MVP backfill scope choice) and (b) existing EU row counts that include 1s from already-captured CME/CBOE data. No instruction to backfill ohlcv_1s as MVP                                                                                                                                                      |
| `tradfi_sp500_ml_and_arb_backtest_readiness_2026_06_20.md`                    | 77-124      | #6 ohlcv_1s read-path dependency                                                    | **HISTORICAL-CONTEXT-OK** — features-service can't yet read ohlcv_1s from MTDS (features-read dependency gap), plan describes that gap; no instruction to backfill 1s as MVP                                                                                                                                                                                                                                            |
| `instruments_foundation_completeness_2026_06_24.md`                           | 1117-1440   | #3/#4 BINANCE-DELIVERY / LIGHTER / PACIFICA                                         | **HISTORICAL-CONTEXT-OK** — L1117 describes G1.1/G1.4 prod audit counts (including BINANCE-DELIVERY + LIGHTER + PACIFICA as captured facts); L1126 "PRESERVING all v9 incl. the new backfill" is a data-preservation note; v10 catalogue decision (BINANCE-DELIVERY non-MVP; LIGHTER/PACIFICA = CeFi) is correctly applied by `mvp_catalogue_finalization_v10_2026_06_27` — no instruction in this plan to extend scope |
| `sports_canonical_universe_and_apifootball_reference_expansion_2026_06_24.md` | 122         | #1 EPL/LA_LIGA as parallel-VM safety example                                        | **HISTORICAL-CONTEXT-OK** — explicitly an example of parallel-VM file isolation (EPL/ + LA_LIGA/ are different GCS prefixes), not a 2-league scope definition                                                                                                                                                                                                                                                           |
| `path_to_100pct_backfill_mtds_is_2026_06_17.md`                               | 233-238     | #4 BLOCKED-OPERATOR-DECISION on LIGHTER/PACIFICA/EXTENDED classification            | **RESOLVED-BY-V10** — v10 decision is LIGHTER/EXTENDED/PACIFICA = CeFi; annotate if the plan is still active (verify archival status)                                                                                                                                                                                                                                                                                   |
| `data_completion_to_100_all_ag_2026_06_21.md`                                 | 1926+       | ohlcv_1s CME+CBOE capturing                                                         | **HISTORICAL-CONTEXT-OK** — operator-authorized ohlcv_1s for CME (futures) + CBOE futures (pre-v10 decision, CAPTURING is live state not a new backfill MVP scope)                                                                                                                                                                                                                                                      |
| Kalshi scans                                                                  | —           | #5 Kalshi post-MVP/excluded                                                         | **CLEAR** — no hits on `kalshi.\*(post.mvp                                                                                                                                                                                                                                                                                                                                                                              | not.\*mvp | excluded)` in any active plan |
| options trades/book5 MVP                                                      | —           | #2 per-strike trades/book5 MVP references                                           | **CLEAR** — no conflicting hits (only the self-referential task description in this plan)                                                                                                                                                                                                                                                                                                                               |
