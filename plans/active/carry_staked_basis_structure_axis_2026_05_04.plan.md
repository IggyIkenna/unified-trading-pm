...
todos:
  ...
  - id: phase-4b-uac-solana-subgraph-ids
    content: |
      - [ ] [HUMAN+AGENT] P1. **UAC `SUBGRAPH_IDS["jito"|"marinade"]["SOLANA"]` follow-up.** MTDS `solana_lst_archival.py` Tier 2 lookup `get_subgraph_id("jito"|"marinade", "SOLANA")` is structurally complete but returns None today. When UAC's `registry/capability_declarations/_defi.py` (or wherever `SUBGRAPH_IDS` lives) gains entries for the Jito Solana subgraph (`jito-foundation/jito-mainnet`-style) + Marinade Solana subgraph, the MTDS Tier 2 historical path lights up automatically — no MTDS code change. Action: (a) audit existing Solana subgraphs at https://thegraph.com/explorer for `jito` + `marinade-finance` Solana mainnet entries, (b) add to UAC `SUBGRAPH_IDS` with chain="SOLANA", (c) re-run `mtds-lst-rates-` backfill VM over historical window — Tier 2 fan-out per day starts emitting historical rates.
    status: todo
