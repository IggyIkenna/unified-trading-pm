...
- [x] [DATA] P3. ✅ _(follow-up)_ **FORWARD-ONLY code SHIPPED + adversarially verified (NO re-capture — the workflow's
      ...
      **instruments-service** (`@2257a067`): Polymarket sources `question=market.question` (honest-None, not `or ""`);
      ...
      every NEW prediction capture now carries the real question.** **History backfill (operator ruling: in-place, from
      the 43.7% of legacy by_date objects that ALREADY carry `question` on disk — lossless, no re-capture, preserves
      `available_at`) — dispatched to a sub-agent with snapshot + independent-verify discipline.** Note: the slug
      fallback already labels ~99.996% of rows readably, so the backfill upgrades slug→human-question for the
      legacy-covered historical markets; forward markets already have it.
