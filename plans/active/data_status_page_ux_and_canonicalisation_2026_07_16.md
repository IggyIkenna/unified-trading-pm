...
- [ ] [DATA] P3. **DECIDED (operator 2026-07-18: FIX ALL NOW — demote the 847 phantom `captured` rows to honest state +
      root-cause the writer + register/triage OKX-SWAP). Executing this session — will flip on ship.** _(NEW —
      side-discovery 2026-07-17, found while diagnosing the cefi instrument_type backfill's residual)_ **4 cefi manifest
      rows claim a capture under BARE venue names that have no objects — the same phantom-pointer class as the P7 split
      rows (`PACIFICA`/`LIGHTER`), and a live venue-registry gap alongside it.** Found because the cefi captured-blank
      backfill correctly drained 13,046 → **4**, and those 4 would not resolve: `venue=COINBASE` (2 rows) + `venue=OKX`
      (2 rows), both `capture_status=captured` with real-looking `row_count`/`instrument_count` (69 and 77), both only
      on **2026-03-01 / 2026-03-02**. The backfill left them blank rather than guessing a type — the honest-absence
      guard working correctly, which is how they surfaced. **Verified on real GCS + the UAC registry:** - **No object
      exists** for any of the 4, under either the `pipeline_mode=…/asset_group=cefi/venue=…` layout OR the legacy
      `by_date/day=…/venue=…` layout. The real venue dirs that day are `COINBASE-SPOT`/`COINBASE-FUTURES` and
