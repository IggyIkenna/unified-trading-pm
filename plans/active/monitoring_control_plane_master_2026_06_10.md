...
- [x] ✅ [CODE] P0. **(G3) Manifest consolidator health (`CONSOLIDATOR_DOWN`) is homeless** — the consolidator watchdog
      ...
      **IN PROGRESS — slot 3 (operator 2026-06-12). NOT for other slots to pick up.** **(was: this line had no update
      ...
      PROGRESS", or reconciled with the operator on what remains distinct from the cockpit surface.)** **CLOSED
      2026-07-31 (`ci_satellite_ao_dispatch_batch2_2026_07_29.md` reconciliation todo)** — re-verified both docs
      directly: `unified_deployment_health_cockpit_2026_06_23.md` is genuinely `status: complete` and shipped
      `GET /api/health/consolidator` (index age, per-VM shard fallback, last successful run) wired live into the
      deployment-ui **Consolidators** tab (§ "O1 — Wire the consolidator drill-down", pw:L2 ✓ verified against the live
      API). `consolidator_throughput_backlog_monitor_2026_07_09.md` (`status: active`, 23/26 todos done) extends that
      same tab further with per-AG backlog + throughput. Between the two, the original ask here — a standing
      `CONSOLIDATOR_DOWN` surface with index-age + shard-count — is fully delivered; no residual gap distinct from what
      those two docs already own/track. Closing G3 against them rather than re-scoping — any further consolidator
      cockpit work belongs in `consolidator_throughput_backlog_monitor_2026_07_09.md`, not a resurrected G3.
...
| sit-repo full-workspace-sit report-back is LDR-only (inert until its main promotion)                                                                                                                                                                                                                                                                                                                                                                        | cicd_contract_hardening conflict notes               | ✅ RESOLVED 2026-07-31 — `system-integration-tests` main is content-caught-up with LDR (`git diff origin/main origin/live-defi-rollout --stat` empty; routine `chore(promote): LDR → main (Option-B direct)` PRs merging continuously, e.g. #326 2026-07-31T05:31Z). `cicd_contract_hardening_2026_06_01.md` is archived; the routine drain (not a manual promote) has kept this current — no action needed. |
