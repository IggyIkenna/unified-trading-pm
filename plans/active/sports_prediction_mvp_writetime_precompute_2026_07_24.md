...
- **na-eligibility-audit 2026-08-17 (cross-cutting tranche, dispatch agt-3931fd)** [body-hash:7de3005e9d576ce8]: KEEP-NA, valid — Sole open todo is an explicit, twice-reaffirmed (na-eligibility-audit 08-02, 08-07) scope-risk STOP, not an oversight: a MANIFEST_SCHEMA_VERSION 9→10 bump on UTL's shared AvailabilityRecord (written by every asset_group + every producer service) needing a FULL-FLEET redeploy plus unverified manifest-consolidator schema-evolution behavior ('loud-fails on stale index' per codex). The doc explicitly cites a directly parallel precedent within the same investigation (a CeFi manifest re-stamp) that 'needed a snapshot + guarded rollout + an operator-gated Cloud Scheduler pause and is still not fully landed' as the reason for not force-shipping despite having a fully-specified 5-step implementation design already written. Ruling/citation: In-doc parallel precedent: the CeFi manifest re-stamp needed 'a snapshot + guarded rollout + an operator-gated Cloud Scheduler pause and is still not fully landed' — same schema-evolution risk class as this todo's MANIFEST_SCHEMA_VERSION 9→10 bump; reaffirmed KEEP-NA by na-eligibility-audit 2026-08-02 and 2026-08-07.
