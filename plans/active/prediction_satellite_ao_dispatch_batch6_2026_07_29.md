...
- [ ] [CODE] P3. **New, opened 2026-07-31 by the Betfair back+lay todo above.** Provision a live Betfair session token
      so the real Exchange API (`listMarketBook`) can be called — needed for BOTH this satellite plan's
      ...
      the real account username + password (source: a local file the operator had already used for prior e2e-testing
      runs, `e2e-testing/scripts/sports/betfair_live_feed.py`'s `BETFAIR_USERNAME`/`BETFAIR_PASSWORD` env-var convention
      — no cert material was present, so path (a), not (b)). Live-verified in GSM (hash-compared, no raw value ever
      printed): created `betfair-password` (new secret, v1) and corrected `betfair-username` (existing secret was STALE
      — a different, shorter value than the operator's live account; added v2, confirmed matching via SHA-256). **Still
      open, retagged `[CODE]` (no longer `[OPERATOR]` — the credential blocker is cleared)**: implement the actual
      interactive-login refresh cron using `betfairlightweight`'s (already vendored in execution-service's `.venv`)
      session-login flow or the adapter's own `authenticate(username, password)` path, mint + rotate
      `betfair-session-token` on a schedule (~4-24h expiry), THEN do the wiring this todo's done-when already describes
      (`factory.py` `VENUE_REGISTRY`, `lay_price` column, live-verification re-run).
