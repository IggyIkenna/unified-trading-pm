...
- [ ] [BLOCKED-OPERATOR-DECISION][BACKEND] P2. **Two-sided Betfair odds — persist back+lay, not just one side.** Item
      `[5]` under the source doc's "Smaller open items (documented, not blocking paper)" — items `[1]`-`[4]` shipped
      ...
      **BLOCKED-OPERATOR-DECISION 2026-08-10 (slot-11, backend_engineer — third independent verification)**:
      ...
      session). Operator declined the europe-west4-egress option (jurisdiction/compliance risk to the real betting
      account). Todos 1-3 shipped and green (`market-tick-data-service@fc9e36cd`/`@85872cab`/`@766e776d`, all
      verified ancestors of origin; `tests/unit/test_betfair_adapter.py` present). This item's done-when ("back+lay
      both persist for a sampled Betfair market") is NOT met and cannot be met by a worker until the operator
      resolves egress or re-scopes/descopes the live-verify — do not re-dispatch. Tracking + 4 re-scoped todos:
