...
- [x] ✅ [CODE] P0. **DONE 2026-07-30 — `instruments-service@e0f7aaad`.** Fix the write-time `canonical_question_group`
      ...
      **Verified**: the fix + its `test_kalshi_composite_instrument_key_still_classifies_correctly` regression test were
      ...
      `tests/unit/test_prediction_canonical_group_shard.py -k kalshi` at HEAD — 3/3 pass. **Evidence citation corrected
      2026-08-09 (plan_reconciler)**: `e0f7aaad` itself only exists on the abandoned
      `wip-preserve/slot-5-instruments-service-diverged-20260805T111826Z` branch, not `origin/live-defi-rollout` — the
      identical content landed live under a different hash, `instruments-service@94f3ee11` (same commit message, almost
      certainly a cherry-pick/rebase rehash, mirroring this same doc's documented 2026-07-31 slot-14
      cherry-pick-recovery incident elsewhere). The work is genuinely shipped; only the cited SHA was wrong. **Source**:
...
- [ ] [SCRIPT] P1. **Kalshi execution credential reshape + live paper-order verify.** Todo 1: read the existing
      ...
      **Todo 1 DONE 2026-07-31**: both secrets provisioned + verified non-empty and byte-identical to source (evidence
                              in the source doc). **Todo 2 ACTIONABLE (non-live path only) — ruled 2026-08-06** (was:
                              BLOCKED-OPERATOR-DECISION 2026-07-31 — retagged same edit per CLAUDE.md's stale-tag rule). Original conflict this
                              todo's own text didn't resolve: this codebase's `OperationalMode.PAPER` never calls any real venue API (routes
                              everything through a simulated `PaperBettingAdapter` — `execution_service/adapters/sports_factory.py`'s
                              `_PAPER_VENUE_KEYS` includes `kalshi`), so "paper order" cannot mean that operational mode. `KalshiAdapter`'s
                              default `base_url` is `https://api.elections.kalshi.com` — Kalshi's LIVE production host, which literally
                              matches this todo's own "elections-subdomain host" instruction (there's a separate
                              `KALSHI_DEMO_BASE = "https://demo-api.kalshi.co"` the code supports but does not default to).

                              **RULED 2026-08-06 (operator, on the source issue doc)**: **NO — do not touch the live exchange.** Option B
                              below is CLOSED: the 2026-07-28 ruling's scope limit stands and placing a real order on
                              `api.elections.kalshi.com` remains unauthorized. Proceed via a non-live verification path — the operator's
                              redirect names two acceptable directions and leaves the choice between them to the executor: a sandbox/testnet
                              host if Kalshi offers one (Option A, `KALSHI_DEMO_BASE` — still needs confirming it accepts the provisioned
                              credentials), or verifying the order-submit/fill/ack/position-update code path via mocked responses instead of
                              a real venue call (Option C). See
                              `plans/active/issues/kalshi_execution_credential_secret_name_mismatch_2026_07_26.md` line ~136 for the ruling's
                              full text — verified 2026-08-09 (plan_reconciler agt-c3a27f) this mirror was still missing, a sync gap the
                              2026-08-07 na-eligibility-audit pass on that issue doc had already flagged but left for "whichever pass next
                              touches batch6."
                              ...
                              the same provisioned credentials before trying. [WORKER REC] — RULING-COMPATIBLE.
                              B: ~~Place a real order on the live host as literally instructed~~ — **RULED OUT 2026-08-06 (operator): do not
                              touch the live exchange.** Do not attempt.
                              C: Some other verification method (e.g. a dry-run / signature-only test that proves the credential wiring works
                              without submitting a live order) — RULING-COMPATIBLE (the ruling explicitly names "mocked responses instead of
                              a real venue call" as acceptable); still needs the executor to decide what constitutes sufficient evidence.

                              Todo 2 is now ACTIONABLE via A or C (no longer blocked on an operator go-ahead) — attempt one, capture evidence,
                              and flip this todo + the source doc's `[DATA] P1` item citing the SHA. `can_continue: true`; other backlog work
...
- **plan_reconciler 2026-08-09 (agt-c3a27f, slot 13, prediction tranche)**: todo 5's Question block still posed the
  pre-ruling "how should todo 2 be executed?" with Option B (live order) framed as awaiting an operator go-ahead —
  stale. The source issue doc's 2026-08-06 "NO — do not touch the live exchange" ruling was never mirrored in (a sync
  gap the 2026-08-07 na-eligibility-audit pass on that doc had already flagged but left unfixed). Rewrote todo 5 to
  close Option B, point execution at A/C (both ruling-compatible), retagged the stale
  `BLOCKED-OPERATOR-DECISION 2026-07-31` label to `ACTIONABLE (non-live path only)`, and cleaned up a large
  stray-whitespace corruption in the same block (unrelated continuation lines had ~300 leading spaces each — cosmetic,
  no content change). Independently triple-verified (2 hunters + adversarial refuter + confirmer) before applying; see
  `plans/active/issues/plan_reconciler_findings_prediction_2026_08_09.md`. Also: added the missing `sequential: false`
  frontmatter field (only one of the 5 batch generations lacking it) and corrected todo 1's evidence SHA (`e0f7aaad`
  only exists on an abandoned wip-preserve branch; the live equivalent is `94f3ee11`, same content rehashed — the work
