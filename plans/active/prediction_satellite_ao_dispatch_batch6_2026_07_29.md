...
created: "2026-07-29"
last_updated: "2026-08-07" # was 2026-07-30 (very stale -- Progress Log alone shows real edits through 2026-08-06) -- fixed by plan_reconciler agt-e7f024, which also retagged 1 non-recognized exclusion marker to [OPERATOR] and clarified 1 stale line-1 todo brief
...
- [ ] [SCRIPT] P1. **Kalshi execution credential reshape + live paper-order verify.** Todo 1 **[DONE 2026-07-31, see
      below]**: read the existing `kalshi-api-credentials` bundled JSON secret's fields and provision two new Secret
      Manager secrets (`kalshi-api-key-id`, `kalshi-private-key-pem`) from them, verified non-empty via
      `gcloud secrets versions access`. RULED 2026-07-28 — explicitly NOT wallet-key-class (a reshape of already-live
      credential data), now AO-executable IAM-self-service per RULES.md §5. Todo 2 (gated on todo 1, STILL OPEN — see
      below): place a real Kalshi paper order end-to-end through execution-service, capture logs/commit evidence.
      Different files (`execution_service/adapters/...` Kalshi credential wiring) from the EventTransport-bridge todo
      above — no overlap expected. **Source**:
      `plans/active/issues/kalshi_execution_credential_secret_name_mismatch_2026_07_26.md` (both todos). **Done when**:
      both secrets exist and verify non-empty, a real paper order completes end-to-end with captured evidence, and both
      source-doc todos are flipped citing the SHAs/evidence.
...
- [ ] [SCRIPT] P1. [OPERATOR] (retagged 2026-08-07, plan_reconciler agt-e7f024 — was "DEFERRED-BY-DESIGN", a marker
      `task_template.md` §3 does not recognize as a dispatch-exclusion token, so this todo was still live-dispatchable
      despite the doc's own text below saying "no worker can resolve alone" and "do not re-dispatch it as originally
      worded"; `[OPERATOR]` is the recognized tag matching that stated intent). **Phase 5 — canonical-groups backfill,
      ~24 remaining groups beyond the initial 9+7-CME.** Mirror the already-shipped 7 CME-linked groups' pattern:
      per-group UAC `PREDICTION_GROUPS` registry entry + instruments-service catalog backfill + MTDS backfill, with
      cluster-validation counts as the done-when. Needs unified-api-contracts + instruments-service +
      market-tick-data-service (structurally outside `prediction_phase_c_data_status_ui_2026_07_24.md`'s repos, which is
      deployment-api/deployment-ui only and cannot deliver this — confirmed by 3 independent prior audits). **Source**:
