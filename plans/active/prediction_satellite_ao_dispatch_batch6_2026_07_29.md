...
- **2026-08-12** (slot-4, infra): provisioned/verified the `europe-west2` egress (proxy VM
  `betfair-egress-proxy-20260811-211046`, `35.230.151.99`, confirmed RUNNING + functional). Re-ran
  `refresh_betfair_session_token.py` through it — the prior geo-block 403 is GONE (Betfair's real login now responds
  `INVALID_USERNAME_OR_PASSWORD`, a credential issue, not network). New `[OPERATOR]` BLOCKED-CREDENTIALS todo + full
  detail in the issue doc's Progress Log. This item stays unchecked (done-when needs a working token + live-verify).
