...
- [x] ✅ [PM] P1. **DONE 2026-07-28 (slot-11).** Filed
      ...
      value on the "more data" branch, so any day with >100 Flashbots relay payloads only ever captures its newest
      page (~100 rows) with no `attempted_failed`/partial signal — silent under-coverage, not an outage. Correct
      frontmatter (`doc_type: issue`, `asset_group: [defi]`, `repos: [market-tick-data-service]`) + a concrete
      `[BACKEND] P2` fix todo (confirm cursor semantics, decrement correctly, add multi-page unit test, re-verify with
      a live sample-day backfill). Cross-referenced from the source doc's deferred-work row: updated
      `plans/archive/issues/defi_five_never_captured_venues_fix_2026_07_22.md` line 266 ("File the mev_events
      >100-payload/day pagination gap") from "Not filed" to point at the new doc. Repo: unified-trading-pm. Source:
