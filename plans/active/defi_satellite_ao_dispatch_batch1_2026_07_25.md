...
- [ ] [CHORE] P1. **Retagged 2026-07-29 (corpus hygiene pass): confirmed NOT actually gated — the "BLOCKED-CREDENTIALS"
      phrase in this item's body only describes what the CORRECTED docstring text should say about `HeliusSolanaAdapter`
      (a separate, already-known gate on THAT adapter), it is not a blocker on making THIS docstring edit. Re-verified
      2026-07-29: `market_interface/adapters/onchain/__init__.py` still carries the stale docstring ("All adapters
      previously here (Glassnode, MEV) were blacklisted dead stubs and deleted during the UMI → MTDS merge
      (2026-04-11)"), and the corrected replacement text is fully drafted verbatim in the source issue doc. This todo is
      fully actionable now — not flipping to done since the actual docstring edit has not been made.** Replace the stale
      `onchain/__init__.py` module docstring with the corrected text already drafted and verified in the source issue
      doc §2.2 (documents `GlassnodeAdapter` as `PLANNED_VENUES`-parked and `HeliusSolanaAdapter` as
      `BLOCKED-CREDENTIALS`-gated, replacing a stale 2026-04 "all adapters deleted" claim) — first confirm via
      `git status`/mtime that no other session has live uncommitted WIP in this file/package. Repo:
...
- [x] ✅ [DATA] P1. Diagnose (and fix ONLY if a clear code bug) the implausible identical `instrument_count=14,630,914`
      ...
      **DONE 2026-07-28 (slot-13)**: no live code bug; see dated Update in the issue doc.
