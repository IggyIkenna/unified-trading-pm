...
| 30 | `_STRIKE_INCREMENT` | vol_trading/atm_straddle_resolver.py:57 | Per-underlying option strike-price grid (btc=1000, eth=100, spx=25) | 1 | **CHECKED 2026-08-21 (per the parent plan's `[OPERATOR]` todo prep-work instruction)**: grepped instruments-service in full for `strike_increment`/`strike_step`/`strike_interval`/`tick_size` under `reference_data/` — no static strike-GRID registry exists anywhere in that service. What DOES exist is a different mechanism: `reference_data/schemas.py`'s `CanonicalOptionsChain` (`strikes: list[Decimal]`), populated by `adapters/cefi/deribit_options_adapter.py` from Deribit's LIVE instrument list — i.e. instruments-service's answer to "what strikes exist for this underlying" is "fetch the real ones from the venue," not a formula-derived grid. This is NOT the same shape as `_STRIKE_INCREMENT` (a cheap static increment used to round an arbitrary spot price to the nearest plausible strike for ATM-straddle construction, not an enumeration of real listed strikes) — so there is still no confirmed SSOT this constant could migrate INTO as-is; adopting the live-chain approach instead would be a bigger design change (a network call inside the resolver) than a migration | AMBIGUOUS — confirmed no static SSOT exists in instruments-service (checked, not assumed); the only related mechanism there (live options-chain enumeration) is a different shape, not a drop-in replacement. Flag for operator: keep the static local grid (cheapest, matches current use) vs. invest in wiring a live-strikes lookup |
...
| 45 | `_STABLE_PREFERENCE` | target_universe/catalog_staked_basis.py:99 (pre-migration) | Stablecoin preference order for `start_token` selection (USDC, USDT, FDUSD) | 1 | No UAC stablecoin-preference-order registry found (UAC has peg-history/exit-route data, not preference ordering) | ✅ MIGRATED 2026-08-21 — see #69/#47/#70; new shared module `strategy_service/engine/strategies/v2/stablecoin_conventions.py::STABLE_PREFERENCE`, imported here under the original private name |
...
| 47 | `_PERP_MARGIN_STABLE_PREFERENCE` | target_universe/catalog_staked_basis.py:107 (pre-migration) | Stable preference order for stables-only perp venues (USDC, USDT) | 1 | Same probe as #45 | ✅ MIGRATED 2026-08-21 — exact duplicate of #70 (`carry_and_yield/staked_basis.py:261`) confirmed and consolidated; both now import `PERP_MARGIN_STABLE_PREFERENCE` from `stablecoin_conventions.py` under the original private name |
...
| 69 | `_STABLECOINS` | carry_and_yield/staked_basis.py:245 | Stablecoins the strategy can start in (USDC/USDT/FDUSD) | 1 | Same probe as #45 | ✅ MIGRATED 2026-08-21 — same cluster as #45/#47/#70 (identical value set across 3 files); now derived as `frozenset(_STABLE_PREFERENCE)` from the shared `stablecoin_conventions.py` import (a membership-set view of the same ordered tuple, not a second literal) |
| 70 | `_PERP_MARGIN_STABLE_PREFERENCE` (staked_basis.py) | carry_and_yield/staked_basis.py:261 (pre-migration) | Stable preference order, byte-identical to #47 | 1 | N/A | ✅ MIGRATED 2026-08-21 — exact duplicate of #47, both now import from `stablecoin_conventions.py` |
