...
- [ ] [DESIGN] P1. **C — DeFi venue-spelling canonicalisation in UAC + migration-write (operator 2026-06-14: "manifest should migrate to canonical via migration mappings").** Two registries disagree: `ALL_DEFI_VENUES` carries BOTH `AAVEV3-X` (no-underscore ghost) and `AAVE_V3-X` (canonical underscore); `VENUE_DATA_TYPE_CAPABILITIES` is underscore-only → no-underscore captured rows get `expected_dts=[]` (uncredited, slight found-understatement). `LEGACY_DEFI_VENUE_ALIASES` resolves `VELODROMEV2`/`TRADERJOEV2` but is MISSING `AAVEV3`/`UNISWAPV3`/`COMPOUNDV3`. Work: (a) **UAC** complete `LEGACY_DEFI_VENUE_ALIASES` (or make `normalize_defi_venue` handle no-underscore generically) so every captured form → single canonical underscore `PROTOCOL-CHAIN`; reconcile `ALL_DEFI_VENUES`/`VENUE_DATA_TYPE_CAPABILITIES` to one spelling; 3 orphan forms to resolve/exclude (`SUSHISWAP_V3-ARBITRUM` 2061 rows, `VAULT` 1113, `YEARNV3`→`YEARN_V3` 801). (b) **migration** apply the canonicalisation at manifest-rebuild write-time so the data is canonical (not just read-reconstructed) + update the card `_allowed_defi_venue_chain_pairs` whitelist to accept the canonical form. Re-project → operator re-eyeball the true (slightly >29%) number → THEN resume G4 with canonicalisation included. **Migration HELD until this lands.** Provenance: slot-4 2026-06-14 beta-eyeball session.
