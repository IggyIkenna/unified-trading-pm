...
- [ ] [VERIFY] P0. **Time-gated billing/capacity re-measurement sweep — 4 items in
  ...
  - **Done when**: all 4 measurements are recorded with real numbers (or flagged as an operator credential gap if the
    billing token is unavailable — do not estimate) in one dated Progress Log section of the source doc, honoring its
    own documented measurement traps (skipped jobs aren't billed; a throttled API call silently counts as 0;
    `billable: {}` with no `UBUNTU` key is the real zero, not `/timing.total_ms`).
