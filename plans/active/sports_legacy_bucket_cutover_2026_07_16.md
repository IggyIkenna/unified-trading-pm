...
- [x] ✅ [DATA] P0. **T4.1 — OBJECT-LAYER zero-unique proof — BOTH buckets now PASS (2026-07-17, slot-3).** MDT half
      closed: the OR-5b recovery landed the 32-day / 549,392-key gap into canonical (**524,486→525,823 pre-match into
      `data_type=trades`; 23,569 in-play into the quarantined `data_type=trades_inplay`/`inplay_ticks.parquet`,
      `pipeline_mode` stays `batch_odds_api`**), verified BY CONTENT (fresh independent re-read of all 32 gap days ⇒
      **every day `legacy_only==0`**; per-day `added==legacy_only`, 0 mismatches, 0 errors). The non-raw prefixes are
      dispositioned by PRESERVATION, not assumption: `processed/` (~90,947 derived `odds_horizon_bucket` objects —
      canonical covers all 1,813 legacy days + 120 more, but object-count differs on the recovered days), `scripts/`
      (3), and `_vm_staging/` (51) are server-side-copied to canonical
      `_legacy_migrated_{processed,scripts,vm_staging}/` before any delete; `_index/` is derived control-plane
      (availability index + snapshots, regenerated by the consolidator). **INSTRUMENTS half** was already
      delete-eligible + the bucket is DELETED (T5.4). _Original mechanism / prior state (retained for provenance)_:
      `instruments-store-sports` is now object-layer delete-eligible: all 2,078 formerly-unaccounted objects carry a
      written, measured disposition and the accounting closes at **968,927, delta +0** (`~/tmp-or9/or9_verdict.py`,
      re-runnable). OR-9 recovered **482 distinct legacy-only keys + 803 progressive rows into 131 canonical cells**,
      all re-verified by an INDEPENDENT fresh-read pass (131/131 ok, 0 failed) in a process that never touched the
      writer; **zero legacy objects were mutated or deleted** and the consolidated index is untouched (generation still
      `1784207377339311` from the T3.1 purge). **Still not a licence to delete**: T5.4 additionally requires the two
      `tofu apply`s (T1.3/T1.4) and T5.2's final writer re-check. _Superseded verdict (2026-07-16, retained for
      provenance)_: the pass first reported **FAIL** on 2,078 objects / "6,673 genuinely legacy-only keys"; OR-9's
      re-measure proved **~94% of that was not recoverable data at all** (5,897 payload-free keys) and recovered the 482
      that were. **Result**: 968,927 objects (= 969,321 − 398 v1_archive + 4 T0.2 snapshot backups), every one
      classified, and **2,078 UNACCOUNTED** — objects whose canonical twin holds strictly fewer rows in entities **OR-1
      never enumerated**. Key-level containment cleared 244 of them by measurement (`injuries` 151 → 0 legacy-only keys,
      the 2× row ratio is legacy-side duplication; `fixtures_outcomes` 93 → legacy has no fixture-id column ⇒
