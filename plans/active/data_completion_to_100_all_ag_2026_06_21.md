...
- **na-eligibility-audit 2026-08-21** (cross-cutting tranche): KEEP-NA, valid — reaffirms 2026-08-07's characterization (last full na-eligibility-audit pass; ~2 weeks stale, due a fresh per-item staleness sweep, but no evidence found this pass that the mix has changed): open items remain a genuine blend of bounded-but-unverified data work (EXTENDED-STARKNET backfill, IS-store/cross-data_type completeness steps, DeFi swallow-fixes, dex_swaps_handler.py baseline restore), items already tracked elsewhere (VM-launch gate-check, Step-4 credential asks — CREDENTIAL_BLOCKED), one DEPENDENCY_BLOCKED (features-service category=defi ban, gated on defi C-GREEN), and 2 genuinely operator-gated items (CF-2/CF-3 scope confirmation; the BYBIT delete needing literal human execution per delete-safety-protocol hard-stop #2). None individually clears the whole-doc RECLASSIFY bar; flagging this doc as a good staleness-recheck candidate for a future pass given the audit gap.
