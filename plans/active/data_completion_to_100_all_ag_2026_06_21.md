...
- [x] ✅ [CODE] P2. Add explicit `CAST(schema_version AS BIGINT)` / `CAST(instrument_count AS BIGINT)` to the final SELECT
      ...
      — needs its own reviewed change, not a same-session bolt-on. parent_epic: mtds_mdps_master. **DONE
      (staleness-recheck 2026-08-09)** — `unified-trading-library@3dda987b` ("fix: harden manifest consolidator merge
      against VARCHAR-numeric shard poisoning via TRY_CAST of typed columns", 2026-07-21) added
...
- [x] ✅ [DATA] P1. **CF-2/CF-3 legacy-vs-canonical cell-diff gaps — confirmed real, NOT fixed this pass (needs a
      ...
      **operator ruling 2026-08-08 (NA-corpus blocker digest round 5, id=49)**: yes, scope it and dispatch. Filed as a
      concrete follow-up plan: `defi_cf2_cf3_legacy_canonical_backfill_2026_08_08.md` (NOT present in the repo as of
      this edit — link removed pending re-filing; do not re-add the link until the file actually exists at that path)
