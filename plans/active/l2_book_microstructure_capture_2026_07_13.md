...
- [x] ✅ [SCRIPT] P2. **NEW (discovered 2026-07-13, slot-13):** Build the handler + CLI wiring to dispatch
      ...
      `record_captured(source="mtds_microstructure", pipeline_mode=PipelineMode.BATCH_MTDS_MICROSTRUCTURE)` with shard
      isolation (no `raise` in the per-venue/ per-instrument loop, `classify_venue_error`+`ADAPTER_FETCH_FAILED`).
