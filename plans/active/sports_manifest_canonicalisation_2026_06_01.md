...
- [x] ✅ [CODE] P2. Migrate the remaining sports `KNOWN_COVERAGE_GAPS` consumers off the frozen registry and DELETE it
      ...
      (see next todo).
- [ ] [CODE] P2. Ship the deployment-api companion commit for the `KNOWN_COVERAGE_GAPS` deletion (above) once
      repo-blocker `RB-8c93e9b1` / condition `repo-deployment-api-qg-green` resolves — fresh-pull, re-run
      `quality-gates.sh`,
      `quickmerge --agent --files "deployment_api/routes/data_status/_downloads.py     deployment_api/services/data_status/sports_helpers.py"`.
      Provenance: coverage-exclusions design 2026-07-17, discovered mid-migration 2026-07-17.
