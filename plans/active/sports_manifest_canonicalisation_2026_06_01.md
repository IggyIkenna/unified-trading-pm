...
> **🗄️ SUPERSEDED 2026-07-23 — FOLDED INTO THE CLOSEOUT. Do not pick up further work from this file.**
>
> This plan is superseded by
> [`plans/active/sports_consolidated_closeout_2026_07_19.md`](./sports_consolidated_closeout_2026_07_19.md), which is
> now the live SSOT for the sports vertical's manifest/data canonicalisation work. This plan predates and CONTRADICTS
> the closeout's canonical target: it still treats the bare `entity=fixtures/` path + a single `FIXTURES` data_type as
> the active canonical write target, but the closeout declares the `fixtures` entity SPLIT into
> `fixtures_schedule`/`fixtures_outcomes`, with the legacy bare `entity=fixtures/` path FROZEN since 2026-05-23. Any
> reference below to a single `FIXTURES`/`entity=fixtures/` canonical form, or to sports `data_type` casing, should be
> read as historical context only, not as current direction — verify against the closeout + current codex before acting
> on anything here.
>
> **Every still-open item from this plan has been extracted and folded into the closeout** (as of the archival pass —
> verify the closeout for current status), including:
>
> - The E8 legacy-bucket-delete gate (`market-data-tick-sports` + `instruments-store-sports`), blocked on CF-8
>   (`available_at`) captured-row backfill regression
>   (`plans/active/issues/sports_cf8_available_at_backfill_regression_2026_07_13.md`) and an operator-scheduled
>   maintenance window (`BLK-d9137d48`).
> - The IS L6 legacy-vs-canonical row-count regression caused by the `uts-prod-instruments-service-sports-fixtures`
>   direct-write race vs. the manifest consolidator
>   (`plans/active/issues/sports_is_index_fixtures_job_direct_write_328k_row_cut_2026_07_15.md`).
> - The legacy no-env `instruments-store-sports` bucket migration/decommission question, whose ownership already moved
>   to `plans/active/sports_legacy_bucket_cutover_2026_07_16.md`.
> - A mis-filed, defi-scoped `features-service` `category=defi` cleanup item, gated on the (now-successor)
>   `data_completion_to_100_all_ag_2026_06_21.md` defi C0 walk — not a sports item, cross-referenced here for
>   completeness only.
>
> This plan itself carries **no further actionable work** — treat it as historical evidence/provenance only (dry-run
> logs, CF-audit history, code-shipped SHAs). Do not re-dispatch any checkbox in this file; do not delete the legacy
> sports buckets based on anything in this file alone.
...
- [ ] [DATA] P0. E8 Verify: `cf_manifest_audit_2026_06_01.py` on both sports surfaces → CF-1…CF-12 GREEN (esp. 0 blanket
      ...
      GREEN: hand C-GREEN to L6 → **delete legacy `market-data-tick-sports` permanently**. **2026-07-17 (slot-4)**:
      dispatched again onto this checkbox; did NOT re-run the audit (zero new info per the note below). DB-verified
      `sports-cf8-maintenance-window-scheduled` is still `false`, and found the backlog parking gate meant to stop this
      exact redispatch had been silently orphaned by task-ID renumbering across plan edits (not re-run, not a condition
      problem) — re-applied it to the CURRENT task ID + verified it survives a real `/api/backlog/regen` tick. Full
      root-cause + fix in the issue doc's new "Root-caused + repaired the parking-gate churn" entry + a filed P1 for the
      structural (agent-orchestrator) fix. `sports-e3-e4-fleet-drain-complete` condition (2026-07-13) is now GREEN (see
      the DONE todo below) — that gate is CLOSED, do not cite it. **Current blocker (as of the 2026-07-14 re-audit
      above): CF-8 only, on both surfaces** — the aggregate `available_at` backfill succeeded (85-88% fill) but
      `capture_status='captured'` rows specifically are still only ~40-50% filled; see the new P0 todo in
      `sports_cf8_available_at_backfill_regression_2026_07_13.md` for the precise gap + root-cause candidates. **Do not
      re-dispatch this checkbox until that todo is resolved** (a repeat audit will reproduce the identical CAPTURED-row
      gap with zero new information, same as the old E3/E4-gate churn this note used to warn about).
...
- [x] ✅ [INFRA] P0. **Schedule + execute E3 fleet drain + E4 VM apply** for the sports v9 migration (main decision
      ...
      `deployment-service` VM-launcher access (`/codex/05-infrastructure/vm-launcher-runbook.md`), not a single-repo
      code change. On completion: `POST /api/prerequisites/sports-e3-e4-fleet-drain-complete {value:true}` to ungate the
...
- [x] ✅ [CODE] P2. Migrate the remaining sports `KNOWN_COVERAGE_GAPS` consumers off the frozen registry and DELETE it
      outright (no-shim rule). The plan's callsite list undercounted the real consumer surface — grep found
      `is_in_known_gap`/`get_known_coverage_gaps`/`KNOWN_COVERAGE_GAPS` live in 6 repos, not 2. Migrated + deleted in
      full (every call site was a dead no-op today since the registry was frozen empty, so removal is behavior-
      preserving): unified-api-contracts@ee6228b7 (the registry + accessors + `_is_sports_in_known_source_gap` gate
      deleted outright), market-tick-data-service@31d4a9c0 (`_step7_known_gap_gate` + its call site deleted),
      instruments-service@a4adf1a7 (weather/sfi/understat/footystats orchestrator pre-skip guards + backfill/reconcile
      scripts + test mocks), unified-trading-library@7b5bb35b (`_classify_sports` PAUSED_LEAGUE branch + its dedicated
      tests), features-service@e0eed9ee (honest_coverage_report.py filter). **deployment-api** companion commit is ready
      locally (`sports_helpers.py` + `_downloads.py`, QG-verified safe in isolation) but BLOCKED from shipping — its
      full `quality-gates.sh` is RED for an unrelated pre-existing reason (`c8f96e6` dropped the
      `features-onchain`/`cefi` bucket-naming key; verified byte-identical failure with my diff stashed). Declared
      repo-blocker `RB-8c93e9b1` (`repo-deployment-api-qg-green` condition); tracked in
      `plans/active/issues/deployment_service_onchain_cefi_bucket_naming_test_regression_2026_07_17.md` (added
      deployment-api to its scope + a P1 todo). Follow-up: ship deployment-api's commit once that condition flips green
      (see next todo).
- [x] ✅ [CODE] P2. Ship the deployment-api companion commit for the `KNOWN_COVERAGE_GAPS` deletion (above) once
      repo-blocker `RB-8c93e9b1` resolved. Resolution: the repo-blocker's OWN fix (`deployment-api@f2a3307`
      "fix(data-status): migrate off UAC symbols/keys deleted in today's asset-group parity sweep") independently made
      the exact same `_downloads.py` / `sports_helpers.py` edit this todo needed — my rebased commit became a true no-op
      and git correctly dropped it as empty. Verified: 0 remaining `is_in_known_gap`/`KNOWN_COVERAGE_GAPS` references,
      `bash scripts/quality-gates.sh` green (4634 passed, 16 skipped) at `deployment-api@f2a3307`, `HEAD` ==
      `origin/live-defi-rollout` (0 ahead, 0 dirty). All 6 repos now fully migrated. **Independent re-verification
      (data_engineering slot-5, 2026-07-17):** re-dispatched onto this same todo, hit the identical
      `deployment-api`/`features-service` regressions + repo-blocker independently (own diff converged to zero net
      change against the above SHAs once rebased — confirms the migration is genuinely complete, not just
      self-consistent). Net new: `unified-api-contracts@6f1851d4` re-exports `is_out_of_bounds` via the
      `unified_api_contracts.registry` facade (STEP 5.23 blocks the deep `canonical.coverage_exclusions` import a
      data_type-aware consumer needs) — hardens the facade-compliant migration path for any future consumer that needs
      the check (today's shipped consumers all took the simpler "drop, don't reimplement" route since the registry is
      empty).
- [x] ✅ [CODE] P1. Wire the `COVERAGE_EXCLUSIONS` gate into the instruments-service expected-universe enumerator
      (`scripts/enumerate_expected_universe.py`) so it does not seed `expected_unattempted` inside a declared range.
      Unblocked: the FOREIGN `[3.5/6] IMPORT PATTERNS` violation this item was blocked on (2026-07-17 11:59) was fixed
      upstream by slot-3 (`e7867424 fix(qg): exempt the necessary manifest_writer deep import`) — re-verified 0 import
      pattern violations across 624 files before landing. Implemented per the written-and-reverted diff: imported
      `is_out_of_bounds` from UAC `canonical.coverage_exclusions` and added a bounded check BEFORE the
      `dt_start_ts`/`cov_ts` source-coverage-start floors in the cefi per-`data_type` loop (keyed on `instr.venue`) and
      the sports per-league loop (keyed on `dt_source.get(dt)`) — both
      `yield ExpectedRow(...,     reason="EXPECTED_UPSTREAM_OUT_OF_BOUNDS")` + `continue` before the floor check runs.
      Inert today (registry is empty by design) — verified via a dedicated "no exclusion declared unaffected" test per
      asset group. 5 new unit tests added (2 wiring-precedence + 2 no-op-when-empty + 1 declared-range-fires), full v2
      enumerator suite 173/173 green, full `quality-gates.sh` green (116s). — instruments-service@302ebc02. **Deferred**
      (out of scope for this todo, tracked as item -009 below): extending the same check to the tradfi/defi/prediction
      enumerator loops, which have no coverage-start consumption point today.
- [x] ✅ [CODE] P2. Extend the `COVERAGE_EXCLUSIONS` gate (item -008 above) to the instruments-service expected-universe
      enumerator's tradfi/defi/prediction loops (`scripts/enumerate_expected_universe.py` `_enumerate_v2_tradfi` /
      `_enumerate_v2_defi` / `_enumerate_v2_prediction`) — these have no existing coverage-start consumption point to
      piggyback the check on today (unlike cefi's `dt_start_ts` / sports' `cov_ts` floors), so each needs its own
      bounded-check insertion point designed first. Inert until a range is declared (registry is empty), so no
      functional loss from the delay. Provenance: coverage-exclusions design 2026-07-17, scoped out of item -008
      2026-07-17 to keep that todo's diff matched to the pre-validated written-and-reverted diff. **Shipped
      2026-07-17**: each loop now checks `is_out_of_bounds(asset_group, venue, data_type, day)` BEFORE seeding
      `expected_unattempted` and yields `EXPECTED_UPSTREAM_OUT_OF_BOUNDS` instead when a declared range covers the cell
      (defi keyed on the canonical protocol venue, tradfi on `instr.venue` before the Databento rolling-floor clip,
      prediction on `instr.venue`) — mirrors the cefi/sports wiring exactly. 6 new unit tests (fires + inert-when-empty
      per asset group), full enumerator suite 185/185 green, `quality-gates.sh` green (136s). —
      instruments-service@29a0030f.
- [x] ✅ [CODE] P2. Adapter pre-fetch parity for bounded exclusions — the MTDS orchestrator's
      ...
      floor. Provenance: coverage-exclusions consumer study 2026-07-17. — Added a BOUNDED check alongside the floor
      (venue-level, wildcard-scoped, since the pre-skip runs before any data_type is selected): UAC's
      `registry.expected_coverage.is_bounded_excluded` (unified-api-contracts@715012b0) + MTDS's
      `is_venue_bounded_excluded` sibling pre-skip with its own `EXPECTED_UPSTREAM_OUT_OF_BOUNDS` honest-absence log
      line (market-tick-data-service@32200e6c). Registry ships empty so inert until a range is PROVEN; QG green both
      repos (40 UAC tests + 6017 MTDS tests).
- [x] ✅ [AUDIT] P2. Reconcile the THREE parallel coverage-floor registries that do not propagate to each other —
      **PREMISE LARGELY FALSIFIED; registry 2 IS genuinely unlinked but is a DIFFERENT CONCEPT; the one real sports
      ...
      data_engineering + this slot) and AGREE on the core; merged findings: 1. **Registries (1) and (3) are ONE
      registry, not two — c280e1ff reached the oracle BY CONSTRUCTION.** `coverage_starts.py` does not hardcode sports
      floors; it re-exports them (`SPORTS_SOURCE_COVERAGE_START = dict(_SPORTS_SOURCE_COVERAGE_START)` imported from
      `league_data.py`). Runtime identity proof: `SPORTS_SOURCE_COVERAGE_START == SOURCE_COVERAGE_START` → **True**.
      c280e1ff's real diff = 2 files (`league_data.py` + its test), confirmed by reading the diff. Nothing to sync.
      [both agents] 2. **c280e1ff DID reach the WRITE-guard.** ManifestWriter's guard is `is_pre_launch_date` →
      `get_source_coverage_start` → `DATA_TYPE_COVERAGE_START`/`SOURCE_COVERAGE_START` — i.e. `league_data.py`, the
      amended file itself. Composed against the real registry: `is_pre_launch_date('MATCHES', '2018-06-15')` = **False**
      (accepted) for every footystats data_type; **15/17** sports data_types accepted at 2018-06-15. This RECONCILES the
      31,301 re-emitted rows — they were writable _because_ the amendment reached the guard. 3. **c280e1ff DID reach the
      FETCH-gates.** The sports fetch pre-skip is NOT `venue_start_dates`: every IS sports fetcher
      (`orchestrator/{footystats,understat,sfi,weather}.py`) gates on `get_source_coverage_start(...)`, the amended
      registry. `clip_dates_to_source_coverage(src, 2018-01-01..2018-12-31)` now **fetches** the full window for
      api_football / footystats / transfermarkt / open_meteo / understat. 4. **c280e1ff DID reach the ENUMERATOR.**
      `enumerate_expected_universe.py::_yield_v2_sports_pre_source_coverage_rows` calls
      `get_source_coverage_start(source, dt)` → the amended floor. (The separate "enumerator never calls the oracle"
      finding is real but concerns the `expected_coverage()` **wrapper** — it reads the same amended SSOT.) 5.
      **Registry (2) never gated ANY amended sports source.** Measured: `get_venue_start_date()` returns **None** for
      ...
      c280e1ff did not amend. So the sports fetch/write path was never at risk from registry 2. 6. **Registry (2) IS
      genuinely unlinked from (1), and disagrees by YEARS outside sports** — but note the two encode DIFFERENT CONCEPTS:
      `coverage_starts` CeFi = **venue launch date**, `venue_start_dates` = **earliest manifest / archive data** ("Start
      dates = earliest manifest data, NOT exchange founding dates"). So DERIBIT 2016-06-13 vs 2019-03-30 is
      launch-vs-archive, both true, not a typo. It is still a REAL honesty question (the oracle expects a window the
      fetcher can never serve), so it is NOT dismissed: filed with 6 fix todos in
      ...
      (launch ≠ archive floor) rather than blind-syncing the two dicts.** [slot-9] **The one GENUINE sports defect —
      found by both, FIXED here**: `source_data_start_dates['ODDS_API']` and `SOURCE_COVERAGE_START['odds_api']` are the
      SAME FACT (odds-api vendor floor) hand-typed twice, gating DIFFERENT surfaces (MTDS FETCH pre-skip vs oracle / IS
      guards / writer WRITE-guard). They agreed (both 2020-06-06) but nothing pinned them — slot-9 called it "unenforced
      coincidence"; the drift is silent in the dangerous direction (lower the sports SSOT on new evidence → the
      hand-typed fetch floor keeps pre-skipping the dates the oracle now expects → backfill no-ops while coverage calls
      the days missing). Fixed **derive-not-duplicate**: the fetch side now reads the sports SSOT
      (`_build_source_data_start_dates`). Behaviour-preserving — derived dict byte-identical to the old literal
      (`{'ODDS_API': '2020-06-06'}`), so no adapter attempts a range it previously skipped and no denominator moves.
      Falsifier `TestOddsApiFloorDerivesFromSportsSsot::test_amending_the_sports_ssot_moves_the_fetch_preskip` amends
      ONLY the SSOT and requires the pre-skip to follow; **mutation-tested** — re-hardcoding the floor FAILS it while
      the plain equality assertion still PASSES (so equality alone would have been a false guard). This closes the issue
      doc's ODDS_API/falsifier todo. Evidence: no import cycle (both orders probed); UAC QG green (**sentinel==HEAD
      `d8060c93`**, not merely exit-0); consumers green against this tree — MTDS 100 passed, deployment-api 325 passed,
      UAC venue_mapping + expected_coverage 102 passed. **Residual (legitimate, NOT stale)**: only 2 (source, data_type)
      remain clipped at 2018-06-15 — `SFI_PROGRESSIVE_STATS` @ 2020-01-01 (probed: SFI returns empty for every match
      before 2020-01-01; earliest real object measured EXACTLY 2020-01-01) and `ODDS_HORIZON_BUCKET` @ 2020-06-06
      (odds_api vendor 401 floor). Both are evidenced floors, not amendment misses. Provenance: coverage-exclusions
      consumer study 2026-07-17; slot-9 registry reconciliation audit; verified by runtime proof 2026-07-17.
- [x] ✅ [CODE] P2. The ManifestWriter pre-launch guard DROPS below-floor writes SILENTLY (`logger.debug` + bare
      `return`, no row, no exception): UTL `manifest_writer/_writer_ingest.py:232-244` + `_writer_record.py:681-693` via
      ...
      is not currently dropping any row we have evidence we should be keeping. Priority P2 stands. **Fixed 2026-07-17**:
      both guard sites now emit the already-registered-but-unused `DP_FAILED_BEFORE_GENESIS` DP-COVERAGE alert
      (best-effort, via a new shared `_emit_prelaunch_write_dropped` helper in `_rows.py`) instead of only
      `logger.debug` — the drop itself is unchanged (a genuinely pre-launch date still must not become a fictitious
      manifest row), but a wrong floor is now visible on `#data-pipeline-alerts` instead of vanishing without a trace. 4
      new unit tests in `test_manifest_writer_prelaunch_guard.py` (both call sites emit + drop; best-effort emit does
      not mask the drop when the event sink isn't initialised; non-pre-launch writes emit nothing). QG green (380s). —
      unified-trading-library@a849839d.
- [x] ✅ [UI] P2. deployment-ui typed-reason taxonomy has drifted from deployment-api AND **its parity test cannot
      detect it**. `TypedReasonBadges.tsx` `EMPTY_REASON_KEYS` is a stale 13-member list vs deployment-api's 38-member
      ...
      2026-07-17. — Backfilled all 28 missing keys (current backend truth is 42 members, re-measured from the
      `EmptyConfirmedReason` enum in UAC `canonical/crosscutting/_honest_coverage_empty_reasons.py`, not the plan's
      stale 38-count) + their `EMPTY_REASON_META` entries (label/description/color, descriptions sourced from the enum's
      own docstrings) into `TypedReasonBadges.tsx`. Re-labeled the misleading test honestly — it is now a documented
      MANUALLY-SYNCED snapshot comparison (set-equality, not exact-order) with a comment explaining it cannot do real
      cross-repo verification (deployment-ui CI checks out this repo alone) and pointing at the SIT follow-up below. tsc
      clean, eslint clean, vitest 15/15 (+ dependent FailurePillarStack suite green). `pw:L2` full-suite run was 355/367
      (12 pre-existing failures, unrelated to this change — verified byte-identical on a clean tree with this diff
      stashed out; already tracked in `plans/active/issues/deployment_ui_l2_smoke_gate_red_2026_07_17.md`, filed the
      same day by another agent with a full per-spec triage — NOT claiming a false `pw:L2 ✓`). deployment-ui@03122f2.
- [x] ✅ [SCRIPT] P3. Add a genuine cross-repo parity test in `system-integration-tests` asserting deployment-ui's
      `EMPTY_REASON_KEYS` (`deployment-ui/src/components/TypedReasonBadges.tsx`) is a superset-or-equal of
      deployment-api's `EMPTY_REASON_KEYS` (`deployment-api/deployment_api/services/data_status/coverage_metrics.py`).
      This is the ONLY place such a check can run with both repos actually checked out (deployment-ui's own CI has no
      sibling access to deployment-api) — the same-repo manual-sync test added in the item above only catches accidental
      self-drift, not deployment-api adding a new reason without a UI follow-up. Provenance: coverage-exclusions
      denominator study 2026-07-17 (follow-up from the `TypedReasonBadges` taxonomy-drift fix). — Added
      `tests/architecture/test_typed_reason_key_parity.py`: AST-parses deployment-api's `coverage_metrics.py` (never
      imports `deployment_api.services.*` directly — its `__init__.py` eagerly resolves cloud bucket settings on import,
      which would make a pure taxonomy test depend on GCP credentials) and regex-extracts deployment-ui's `.tsx` array,
      asserting UI ⊇ API. Verified: currently exact parity, 42/42 keys both sides. QG green (44s).
      system-integration-tests@0ac9eca.

## Deferred work — migrated to:

**`data_completion_to_100_all_ag_2026_06_21.md` (M-1)** — the `**DEFERRED**` todo's named successor
(`defi_manifest_canonicalisation_2026_06_01.md` § C0/C-GREEN) was itself SUPERSEDED/FOLDED 2026-07-13 (operator ruling
"Approve all + unlock", MTDS/MDPS 2-survivor consolidation) into `data_completion_to_100_all_ag_2026_06_21.md` — every
open todo from that plan, including the defi C0 path+bucket canonicalisation walk this deferral is gated on, was
migrated verbatim there. As of the 2026-07-13 session that plan reports defi is "NOT full C-GREEN" (real CF-2/CF-3
partition-path gaps, ~703 dates, still open as a P1 todo) — so the gate this sports-track item is waiting on has not yet
cleared; `data_completion_to_100_all_ag_2026_06_21.md` is the current live successor to track for C-GREEN.
