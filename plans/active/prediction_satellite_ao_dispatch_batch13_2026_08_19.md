...
- [ ] [DATA] P0. **Post-walk `(date,venue,data_type)` comparison for prediction manifest.** Source: `data_completion_prediction_2026_07_15.md` item 3 (verbatim: "re-run the `(date,venue,data_type)` comparison → legacy-only CELLS = 0; canonical `_index` all v9; `pipeline_mode` non-null; `source` populated on every cell (HARD — zero blank; the API source per venue)"). Read-only — no GCS mutation, no code change; reuse `rebuild_prediction_manifest.py`'s classify/comparison logic (or a dedicated read-only comparison invocation of it) against the live prediction `_index`.
