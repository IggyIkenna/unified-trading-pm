...
- [x] ✅ [OPERATOR] P0. Solana-drift backfill performance stall — decide intervention path: Consolidated sig index
  ...
  - [x] ✅ [SCRIPT] P0. (Was: **BLOCKED-OPERATOR-DECISION** — Backfill the DRIFT perp_funding cells, blocked on the
    ...
    - [ ] [INFRA] P1. Launch DRIFT `perp_funding`/`perp_trades` Velocity backfill
          ...
          expected universe) — do NOT read it as genuinely complete; it will drop once P1.3 lands. **Action needed**:
          either (a) extend `launch-mtds-solana-drift-backfill-vm.sh` to accept a comma-separated `--markets` list (the
          underlying `backfill_drift_v2_historical.py` already supports `--markets A,B,C` — only the launcher hardcodes
          a single `--market`) and launch once across all markets/full history, or (b) fan out one VM per market (the
          same per-shard pattern G1's other launches use). Pull the authoritative DRIFT market list from the
          instruments-service catalogue (the same source `enumerate_expected_universe.py` used to derive the 51,301
          count) — do not hand-type a market list. SPOT default, e2-highmem-8 (manifest-index-OOM caveat, see
          `manifest_index_read_oom_canonical_cache_2026_06_24.md`). **Gate:** re-run
          `measure_honest_coverage.py --asset-group defi` after launch; DRIFT `perp_funding` `attempted_failed=0` +
          `expected_unattempted=0` closes item 4 of `-003` above. Repos: `deployment-service`,
