...
- [x] ✅ [DATA] P2. **DONE 2026-08-04 (slot-10, data_engineering) — root cause already fixed + verified by an earlier
      independent dispatch; this investigation confirms it and closes the loose thread.** The guard was **correctly**
      blocking — the bug was in the CALLER, not the guard. Fresh scoped manifest reads
      ...
      code this dispatch).
- [ ] [SCRIPT] P3. Clean up the 198 residual phantom `attempted_failed` `lending_indices` rows for MORPHO/COMPOUND_V3
...
- [x] ✅ [DATA] P2. Investigate `dex_pool_state`'s 19 `build_instrument_id` errors — identified 33 errors (grown
      ...
      hard-check (UAC `canonical_id_builder.py:851`). Filed
      `issues/dex_pool_state_build_instrument_id_colon_in_symbol_2026_08_04.md` with per-row isolation fix
      recommendation. — unified-trading-pm@<SHA>
- [x] ✅ [SCRIPT] P3. Retry the 2 `lst_rates` `429`-rate-limited cells (trivial). Repo: `market-tick-data-service` —
      market-tick-data-service@33440748 (2026-08-05, slot-13). Both cells (LIDO/ETHEREUM 2022-09-05, 2022-11-22) retried
      and verified captured. One-off retry script at scripts/retry_lst_rates_429_cells_2026_08_05.py.
