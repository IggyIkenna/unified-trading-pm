...
- [x] ✅ [DOCS] P2. **DONE 2026-07-31 — `unified-trading-pm@<see plan-flip commit>`.** Both checkpoint checkboxes in
      ...
      three and do NOT re-open work there.** Sub-item (b) (identifying the three bucket-SSOT audit issue docs) is
      genuinely this todo's own and is unaffected. Original text: (a) re-verify current deletion status of the 3 items
      in the "Track to completion the deletions OWNED BY OTHER PLANS" checkpoint —
...
- [ ] [DESIGN] P2. **Land colocated feature pipeline in-memory DAG handoff + parquet consolidation + basedpyright
      strictness restore.** Source: colocated_feature_pipeline_in_memory_handoff_2026_06_21.md (items 1.4/1.3b/1.7e;
      1.5b excluded, still gated). Repo: features-service. Done when: `quality-gates.sh` green with a measured I/O
      reduction for items 1-2, and basedpyright `reportUnknown*` at `error` (or a downward-only ratchet) with the masked
      errors resolved, zero new suppressions for item 3. **PARTIAL 2026-08-01 (slot 7): (1) SHIPPED**
      `features-service@b457ee43` — composite_sr/flow_interaction now get the same-run liquidity_walls/
      liquidation_clusters frame in-memory via the registry's `depends_on` order — fixes a real live-prod bug found
      mid-task (composite_sr was ALWAYS null; never wired). 47 tests + full QG green. **(3) config half already done**
      pre-existing (`e8c8693d`, 2026-05-26), not `"none"` as assumed; live count is 862 `reportUnknown*`/`reportAny`
      errors (drifted from ~574). Burn-down NOT attempted (590 files — needs a dedicated session, not a rushed pass).
      **(2) NOT attempted** — delta_one's per-instrument parquet writer/reader is a live prod path; safe consolidation
      needs a real migration design, not a same-session rewrite. Stays open; remaining scope is (2) + (3)'s burn-down.
