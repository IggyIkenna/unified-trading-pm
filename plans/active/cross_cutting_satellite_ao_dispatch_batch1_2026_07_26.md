...
- [x] ✅ [DOCS] P2. **DONE 2026-07-31 — `unified-trading-pm@<see plan-flip commit>`.** Both checkpoint checkboxes in
      ...
      search `plans/archive/issues/` for GCS hardcoded-bucket-literal / bucket-naming audit docs dated around the
...
- [ ] [SCRIPT] P1. **Cross-cutting data-completion prep residuals — `data_completion_to_100_all_ag_2026_06_21.md` Step
      ...
      ask docs (one per vendor, under `plans/archive/issues/`) for Helius/Alchemy, Glassnode/Kaiko, Tardis (historical
...
- [x] [DATA] P1. ✅ Reconcile the CURRENT (2026-07-25 refresh, 45-total) non-canonical distinct-value set to an owning
      ...
      plan if one owns it, or file a new `plans/archive/issues/<slug>_<date>.md` if none does. Source:
...
- [x] ✅ [SCHEMA] P0. **DONE 2026-07-26 (slot-7) — `unified-api-contracts@1407b7f`.** Landed
      ...
      hits for `CompletenessProbe`/`probe_registry`/`use-defi-oracle` across `plans/archive/2026_08/`, including the
      named landing plan `defi_pipeline_e2e_and_coverage_validation_2026_06_20.md`). Implement the §9 P0 schema step
