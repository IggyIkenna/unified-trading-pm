...
created: "2026-07-26"
last_updated: "2026-08-03" # line-cap remediation split -- extracted 07-26/07-28 Progress Log history to the archive doc above; context_scope backfilled
...
> **🟡 STALE 2026-08-07 (plan-reconcile agt-c6e8c7)**: this doc is `status: active` and dispatched (21/22 todos [x]) —
> the authoring-time draft banner below is retained for provenance only. **Status: draft.** Per CLAUDE.md's
> plan-destination rule and the ag-closeout-audit skill's autonomous-mode guidance, a skill-drafted AO batch is never
> auto-shipped to `active` — flip this frontmatter's `status` to `active` only after operator review. This doc + its
> sibling `cross_cutting_satellite_ao_dispatch_batch1b_2026_07_26.md` should be approved/flipped together (both cover
> the same conflict-checked pass, split only for the line cap). All 16 todos below are same-priority-independent and
> touch distinct files/docs, except the `smoke_matrix.py` pair (todo citing
...
- [x] ✅ [DATA] P2. **Close 5 small bounded residuals from
      ...
      already `[x]` with operator ruling 2026-07-28 confirming reclassify disposition (bare-venue subset distinct from
      the broader blank-data_type fix). **(3) CME EC\* verification**: ALREADY RESOLVED prior — source doc checkbox
      already `[x]` via na-eligibility-audit 2026-08-01; plan-of-record ARCHIVED status:complete/TRULY-DONE (214k rows,
      9 EC roots, 100% coverage). **(4) F7 DeFi 19 pre-genesis cells**: NOT SPURIOUS — UAC
      `defi_venue_capabilities.py:218` registers `ALCHEMY-ETHEREUM` gas_fees at 2020-01-01, exactly matching earliest
      MTDS data. 141 rows in 2020-01-01..19: gas_fees captured (legitimate), lst_rates/dex_pool_state empty_confirmed
      (honest pre-launch). No earlier genesis needed. **(5) N8 PRED label drift**: INTENTIONAL dual-data_type design —
      UAC `data_type_capability.py:1018-1040` documents POLYMARKET writes both `trades` (raw per-market, GCS
      `data_type=trades/`) AND `prediction_canonical_question_group` (CQG aggregate, REST-derived). `prediction_trades`
      retired 2026-04-19. Not drift. All 5 source-doc checkboxes now `[x]` with cited evidence.
