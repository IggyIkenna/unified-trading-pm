...
22. **Trading guardrails** — circuit breakers configured per archetype; kill switches wired (codex
    ...
    `DEFI_AAVE_UTILIZATION_SPIKE` / `DEFI_FUNDING_RATE_FLIP` / `DEFI_FEATURE_STALE` / `DEFI_WEETH_DEPEG`
    **🟢 PULLED FORWARD May-23 (operator direction 2026-05-13)** — 4 DeFi-specific codes
    (`DEFI_AAVE_UTILIZATION_SPIKE` / `DEFI_FUNDING_RATE_FLIP` / `DEFI_FEATURE_STALE` / `DEFI_WEETH_DEPEG`) added to
    AlertCode enum + rule wiring + features-onchain emission sites pre-cutover; tracked in owning plan
