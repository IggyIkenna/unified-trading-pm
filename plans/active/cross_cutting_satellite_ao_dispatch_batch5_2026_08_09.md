...
- [ ] [SCRIPT] P0. Phase A — run the features-onchain staked-basis slice end-to-end: a `--dry-run` smoke followed by an
      `IS_TEST_RUN=true` run of `lst_yields`/`lst_native_rates`/`perp_funding_rates`/`health_factor` (asset_group DEFI,
      window 2026-04-07 through 2026-04-09) writing to the `features-onchain-defi-test` bucket, then a read-back
      ...
      every APY field is plausible (non-negative, non-absurd) for all 4 feature families.
