...
todos:
  ...
  - id: p1-mock-features-to-gcs
    content: |
      - [ ] [AGENT] P0. Write mock features to GCS instead of in-memory only. When ml-training-service runs with --use-mock-data:
  ...
  - id: p1-mock-swing-targets-distribution
    ...
    status: todo
    note: "Mock generator has correct distribution. Pipeline ordering strips swing columns before filter."

  - id: p1-shap-artifacts-to-gcs
    content: |
      - [ ] [AGENT] P0. Write SHAP artifacts to GCS instead of local /tmp/. Currently ShapExplainer saves to /tmp/models/shap_explanations/ which is ephemeral on Cloud Run/VMs. Move to:
    ...
    status: todo
    note: "SHAP plots are generated successfully (verified in E2E). Just need GCS write path."
  ...
  - id: p2-training-config-from-gcs
    content: |
      - [ ] [AGENT] P1. Move train_test_split_date from hardcoded config to GCS grid config. Currently hardcoded as "2023-01-01" in ml_training_service/config.py line 216. Should be:
  ...
  - id: p2-min-test-samples-guard
    content: |
      - [ ] [AGENT] P1. Add minimum test samples guard to walk-forward splitter. If a fold produces fewer than MIN_TEST_SAMPLES (default 100) test rows, skip the fold with a warning instead of silently proceeding with empty features.
    ...
    status: todo
    note: "Currently proceeds with test=0 and fails on 'Features DataFrame is empty'."

  # ── Phase 3: ML API Endpoints (unified-trading-api) ──
  - id: p3-training-job-endpoints
    content: |
      - [ ] [AGENT] P0. Add training job management endpoints to unified-trading-api/routes/ml.py:
    ...
    status: todo
    note:
      "UI hooks useCreateUnifiedTrainingRun, useUnifiedTrainingRunDetail, useCancelUnifiedTrainingRun, useTrainingQueue
      all need these."

  - id: p3-run-analysis-endpoints
    content: |
      - [ ] [AGENT] P0. Add run analysis endpoints:
    ...
    status: todo
    note: "UI hooks useRunAnalysisBundle, useMLRunComparison. SHAP data exists (saved during training)."

  - id: p3-model-registry-endpoints
    content: |
      - [ ] [AGENT] P1. Add model registry CRUD endpoints:
    ...
    status: todo
    note: "GCS data exists from our E2E training run. API just reads and serves it."

  - id: p3-grid-config-endpoints
    content: |
      - [ ] [AGENT] P1. Add grid config CRUD endpoints:
    ...
    status: todo
    note: "Grid configs are the experimental configs that drive thermal ML runs."

  - id: p3-pipeline-status-endpoints
    content: |
      - [ ] [AGENT] P1. Add pipeline status/monitoring/governance endpoints:
        ...
        Files: unified_trading_api/routes/ml.py
    status: todo
    note: "These are dashboard endpoints. Can start with mock + basic GCS reads, enhance over time."

  # ── Phase 4: ML Inference Integration ──
  - id: p4-inference-reads-trained-model
    content: |
      - [ ] [AGENT] P0. Verify ml-inference-service reads from ml-models-store and produces predictions. Check:
    ...
    status: todo
    note: "We trained a model and saved it to GCS. Inference should be able to load it."

  - id: p4-inference-mock-alignment
    content: |
      - [ ] [AGENT] P1. Ensure inference mock mode aligns with training mock mode. If training wrote mock features to GCS mock/ prefix, inference in mock mode should:
  ...
  - id: p5-tier-mode-alignment
    content: |
      - [ ] [AGENT] P0. Document and verify tier mode alignment across frontend → API → backend for ML. The 5 mode axes (VITE_MOCK_API, VITE_SKIP_AUTH, CLOUD_MOCK_MODE, DISABLE_AUTH, MOCK_STATE_MODE) must produce consistent behavior:
    ...
    status: todo
    note: "The tier structure exists but ML endpoints aren't wired into it yet."

  - id: p5-ui-ml-pages-data-flow
    content: |
      - [ ] [AGENT] P1. Verify ML UI pages render correctly with both mock and real data. Start tier 1 and check:
    ...
    status: todo
    note: "UI pages exist. Need to verify they work with the new API endpoints."

  # ── Phase 6: GCS Path Consistency ──
  - id: p6-hive-path-consistency
    content: |
      - [ ] [AGENT] P1. Ensure all GCS paths use = (not -) for hive partition keys across ALL services. We migrated features-delta-one-cefi from day- to day= (647 files). Check:
    ...
    status: todo
    note: "We migrated 647 files in features-delta-one-cefi. Other buckets may need same treatment."
  ...
  - id: p7-e2e-mock-pipeline
    ...
    status: todo
    note: "The full loop: UI → API → training → model → inference → predictions → UI"
