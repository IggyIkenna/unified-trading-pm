...
- **na-eligibility-audit 2026-08-17 (cross-cutting tranche, dispatch agt-3931fd)** [body-hash:4df90236bc3a353d]: KEEP-NA, valid — 1 open item — every W1/W3/W4 engineering todo is [x] DONE with extensive live-verified evidence; W6 (codex refresh) has no todo here, deferred to the gated finalize companion doc. The sole open item (W2, the 20 archetype rows NOT declared in the feature-group scaffold: 8 Medium-confidence ambiguous, 12 Low-confidence genuine registry gaps) is explicit follow-on review requiring strategy-domain judgment — this SAME doc's own W2 finding states plainly that fabricating such declarations would be a CLAIM≤MEASUREMENT violation ('a wrong entry here would silently mislead a contract-step-17 BACKTESTABLE check... only archetypes traced to real dispatch code are declared'), and the parallel already-executed W2 item required explicit operator review + approval before any declaration shipped. Ruling/citation: This same doc's own 'Two findings that changed the dispatch prompt's original scope' section (W2): declaring undeclared archetypes without real dispatch-code tracing would be a CLAIM≤MEASUREMENT violation; the already-shipped 35-row batch required explicit operator review+approval (2026-08-16) before declaration.
