...
| ------------------------------ | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| unified-trading-library        | registry.py, config_interface/paths/**init**.py, config_interface/**init**.py, **init**.py, domain_client/clients/market_data.py, tests/config_interface/unit/test_paths_registry_smoke.py   | ✅ DONE, QG-green                        | registry template + `pipeline_mode=`; new `build_canonical_candle_path` + `candle_read_prefixes` (dual-read); MarketCandleDomainClient passes pipeline_mode                                                                                |
| unified-trading-api            | services/batch_candles.py, tests/unit/test_batch_candles.py                                                                                                                                  | ✅ DONE, QG-green (411 tests)            | charts reader dual-read via candle_read_prefixes                                                                                                                                                                                           |
| features-service               | volatility/core/feature_writer.py, tests/volatility/unit/test_feature_writer.py                                                                                                              | ✅ DONE, QG-green                        | P2 volatility prefix fix (volatility/by_date/)                                                                                                                                                                                             |
| features-service               | delta_one/app/core/data_loader.py, delta_one/app/core/dependency_checker.py, volatility/core/data_loader.py, volatility/io/loader.py, tests/delta_one/unit/test_data_loader.py               | ⏳ agent **a24495981a78caecd** IN-FLIGHT | delta_one+volatility readers dual-read — VERIFY the agent's return + QG before landing                                                                                                                                                     |
| market-data-processing-service | app/core/candle_write_mixin.py, canonical_writer.py, canonical_writer_shaping.py, data_sink.py, output_path_helpers.py, io/writer.py (+ codex `per-asset-group-bucket-layouts.md:166` amend) | ⏳ agent **ae6bc63584d3b7c8a** IN-FLIGHT | writer single-derivation (add instrument_type + source data_type + pipeline_mode + empty-stem fix) + manifest records SOURCE + get_processed_path pipeline_mode KeyError fix + codex amend — VERIFY the agent's return + QG before landing |
...
1. Read agent outputs: `.../subagents/agent-ae6bc63584d3b7c8a.jsonl` + `agent-a24495981a78caecd.jsonl` (or re-run the
   two agents if their working-tree output is incomplete). Also do the ONE gap they don't cover: MDPS
   `build_continuous_engine.py:52` per-contract candle READER input needs dual-read (it's MDPS — do it after the writer
   agent, same repo, no concurrent agent).
