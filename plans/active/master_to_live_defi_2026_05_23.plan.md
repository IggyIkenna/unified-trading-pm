...
| `check_shard_freshness` ignores `capture_status`        | Medium     | `attempted_failed` rows treated as fresh; orchestrators skip retry. Forces DELETE-and-refetch hack in sports plans. | Architectural fix: extend `check_shard_freshness` to treat `ATTEMPTED_FAILED` as stale (default-on, opt-out via `retry_failed: bool = True` param). UTL change pending; once shipped the DELETE workaround in `sports_fixtures_truthset_recovery` becomes optional, `phantom_recon_and_failure_triage` Phase 1 flip-to-attempted_failed approach works as designed. |
