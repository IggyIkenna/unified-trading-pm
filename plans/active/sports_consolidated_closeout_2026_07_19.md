...
- [ ] [DATA] P0. **Step (3) — data migration: revert the ~260,298 GCS objects + ~373,296 manifest rows K1/K2 moved to
      ...
      **Done when**: a corpus-wide `data_type`/`instrument_type` census for sports (the deployment-ui Axis Value Census
      ...
      assertion todo below passes on a fresh live read. **Adjacent finding (2026-07-27, slot-8, discovered while running
      `sports_closeout_exchange_fixed_odds_fork_2026_07_25.md` todo 5):** live-sampled 6 venues
      (`BETFAIR_EX_UK`/`BETFAIR_EX_EU`/`SMARKETS`/`MATCHBOOK`/`BETFAIR_SB_UK`/`BETMGM`) and confirmed this exact
      lowercase-original/uppercase-copy DUPLICATION is real and present for all 6 (K2's copy-only migration never
      deleted the lowercase originals it copied from) — e.g.
      `.../venue=BETMGM/league_id=ALLSVENSKAN/instrument_type=odds/data_type=trades/ticks.parquet` (lowercase, 15,756
      bytes) coexists with the now-moved `.../instrument_type=ODDS/data_type=TRADES/...` (uppercase, same 15,756 bytes,
      byte-identical). **For these 6 venues specifically, this Step (3) revert-to-lowercase will find NOTHING to revert
      under `instrument_type=ODDS`** — todo 5 above already moved every manifest-tracked uppercase shard for them to
      `instrument_type=exchange_odds`/`fixed_odds` (lowercase), so the `ODDS`/`TRADES` axis is now empty for these 6.
      The pre-existing LOWERCASE `odds`/`trades` duplicates for these 6 venues were deliberately NOT touched by todo 5
      (out of its scope, unregistered in the manifest) and are now genuinely STALE — content-identical duplicates of
      data that has since been correctly forked into `exchange_odds`/`fixed_odds`. Whoever runs this Step (3) (or a
      follow-up census) should treat these 6 venues' lowercase `odds` objects as delete-candidates in their own right
      (verify content-identical to what now lives at the fork target, then fold into the existing 5-part-proof-gated
      delete this plan already tracks at line ~698), not attempt to "revert" them since there is no uppercase twin left
      to revert FROM for these 6.
