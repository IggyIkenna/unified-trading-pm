...
- [ ] [VERIFY] P1. **NEW 2026-07-27 — close out the SPORT bookmaker-name-in-instrument_type fix's own stated
      verification gap.** The parts[]-index fix (`market-data-processing-service@51502c3` +
      `instruments-service@f46e553e`, todo below) shipped its own caveat: "Not independently verified: the literal
      done-when (Distinct Values panel on fresh LIVE writes) needs a post-fix write to land." A fresh read-only census
      this session (`read_availability_index` on `market-data-tick-sports-prd-central-element-323112`,
      columns=[instrument_type]) found only 8 `SPORT` rows and zero of the other 15 bookmaker-name values the original
      finding measured (PADDYPOWER/ PINNACLE/betmgm/etc.) — consistent with the fix holding, but this was a spot-check,
      not the doc's own stated done-when. **Done when**: a dated Distinct Values panel read on the sports asset_group
      shows 0 bookmaker-name `instrument_type` values (the residual 8 `SPORT` rows, if still present, are pre-fix
      historical rows, not new writes — confirm via `written_at` before treating them as a live regression).
- [ ] [DIAG] P3. **NEW 2026-07-27 — PERPETUAL (1 row) / football (9 rows) in the sports `instrument_type` axis could not
      be reproduced.** The operator's original Axis Value Census screenshot showed both; a fresh read-only query this
      session (same `read_availability_index(bucket, columns=["instrument_type"])` reader the axis-census endpoint
      itself uses, no new GCS walk) against `market-data-tick-sports-prd-central-element-323112` found ZERO rows for
      either value — the live distribution had also shifted dramatically from the screenshot in the same window (total
      rows 465,223 vs. an implied 660K+ from the screenshot's top values; `ODDS` 275,136 / `odds` 20,785 vs. the
      screenshot's `odds` 563,754 / `ODDS` 88,339), consistent with substantial concurrent manifest-rewrite activity
      from other sessions during this exact window (`mtds@e9d9dec0` wrong-source wipe + `mtds@f9f012cb` phantom
      `soccer_*` league_id prune are both cited elsewhere in this doc as recent sports-manifest writes). Most likely
      explanation: stale/already-resolved by that concurrent activity, or a deployment-api server-side cache artifact at
      screenshot time — not confirmed either way. **Done when**: a FRESH read (not this session's, which is now itself
      aging) reproduces or definitively fails to reproduce either value; if reproduced, trace
      `written_at`/`service_name` on the specific row(s) before deciding if it's live or historical.
