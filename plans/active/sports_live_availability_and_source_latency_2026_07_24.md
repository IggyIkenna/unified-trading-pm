...
| **LIVE_ODDS / odds_horizon_bucket** (odds_api)                   | FORWARD→intra-play / continuous | Moves continuously; bucketed 8 horizons T−24h/−12h/−6h/−4h/−2h/−1h/−10m/T−0; live poll **60s** interval.                                                                             | **NOT running** (stale claim — 2026-08-02 `gcloud compute instances list` confirms zero `mtds-live-sports-*` instances). WS connector `odds_api_ws.py` (60s poll) exists coded, unlaunched.                                          | **UPDATE 2026-08-02 — quota gap REOPENED, worse than before:** the 2026-07-29 `odds-api-key` (5,000,000-credits/month) is now EXHAUSTED (`x-requests-remaining: -772`, live-verified), 4 days after provisioning, with no live VM running to explain the burn — root cause unknown, see `/plans/active/issues/odds_api_key_quota_exhausted_4_days_after_provisioning_2026_08_02.md`. api_football second-source wiring was STRUCK 2026-08-02 (`BLK-b969f5f0`) — conflicts with the 2026-06-24 wipe ruling, see the P2 todo below. |
