...
| **LIVE_ODDS / odds_horizon_bucket** (odds_api)                   | FORWARD→intra-play / continuous | Moves continuously; bucketed 8 horizons T−24h/−12h/−6h/−4h/−2h/−1h/−10m/T−0; live poll **60s** interval.                                                                             | **LIVE — coded + RUNNING.** WS connector `odds_api_ws.py` (60s poll) + running VM `mtds-live-sports-odds-api-trades`. Tier-3 `odds_t24h/t6h/t1h` MTDS snapshots also fire.                                                           | **UPDATE 2026-07-29 — quota gap effectively closed:** was `GAP (quota, not code)`, ~43k credits/mo burned at 60s × markets vs. the planned Starter tier's 50k/mo. Operator rotated `odds-api-key` to a new 5,000,000-credits/month subscription (live-verified), ~116x current burn rate — quota is no longer a binding constraint. Second-source wiring (api_football `/odds` in-play, for redundancy/breadth rather than quota relief) remains open, see the P2 todo below.         |
...
- [ ] [DATA] P2. **RULED 2026-07-28 (operator direct answer): "Picked a paid sports-odds API quota tier now — this
      ...
      cost). No longer gated on an operator decision — the direction (proceed) is ruled. **UPDATE 2026-07-29**: the
      ...
      that `mtds-live-sports-odds-api-trades` has resumed/is consuming it) — and the api_football `/odds` in-play
      second source is still unwired. **Live ODDS quota decision + cheap second source** (market-tick-data-service +
      deployment-service) — size The Odds API Starter (~$10/mo)
      ...
      market-tick-data-service (connector) + deployment-service (VM cadence). **Done when**: the api_football `/odds`
      in-play second source is wired as a fallback/supplement, and the live sports-odds ingestion is confirmed resumed
      (a fresh poll cycle succeeding against the live key in production, not just a direct-API-call verification).
