...
| Operator-requested broader audit pass, part 1: full-corpus `/plan-reconcile`                                                                 | **DONE 2026-07-25 (hunt+verify)** — 74 hunters, 290 raw candidates, 246 confirmed (41 refuted) after adversarial verification. Severity: 1 P0, 35 P1, 119 P2, 91 P3. The sole P0 (`sports_satellite_ao_dispatch_batch2_2026_07_24.md`'s league_id-casing todo extending an already-reverted K1/K2 migration) handled separately as its own issue doc + BLOCKED-OPERATOR item (see decision queue below). 234 of the remaining 245 are auto-fixable classes (dangling-ref/structural/self-consistency/stale-drift/mechanical-flag-adjudication/done-but-unchecked/codex-alignment/ao-dispatch-readiness/contradiction/other); **APPLY phase IN PROGRESS** — background Workflow task `wnk9sc47x`, run `wf_ada40e95-0bf`, 16 parallel agents each fixing ~14-15 findings across ~10-12 files, bin-packed for balance. 11 findings (archival-candidate + near-complete) held out for direct handling (lighter-touch, no risky auto-archival). Script persisted at `/home/ubuntu/.claude/projects/-home-ubuntu-unified-trading-system-repos-unified-trading-pm/bbf81a1c-a8d9-4bff-857f-6b9f1b0f0229/workflows/scripts/plan-reconcile-apply-fixes-wf_ada40e95-0bf.js`, resumable via `Workflow({scriptPath, resumeFromRunId: "wf_ada40e95-0bf"})` if the launching session is gone                                                                                                                                                                                                                                                                                                                                                                                                                                       | Nothing — waiting on completion notification (no polling); once it lands, review each batch's NEEDS-OPERATOR-JUDGMENT list and fold into the decision queue below                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        |
| Operator-requested broader audit pass, part 2: full-corpus `/docs-reconcile`                                                                 | **Done 2026-07-24 (later, separate session)** — Phase 0 (5 deterministic checks, all green) + Phase 1 (`authoritative_for` collisions: 0 real collisions across 1025 topics after re-deriving the check with real YAML parsing instead of hand-rolled regex, which had a `\s*`-eats-newline bug; summary-quality: 0 genuine gaps, the 35 empty-`summary:` READMEs are structurally excluded from the L0 index so don't degrade retrieval; doctrine-consistency: clean; broken-link ratchet triage: 61 fixed total — 7 stale baseline entries dropped, 54 provably-moved refs repointed with 2 false-positive basename matches correctly caught and excluded before applying; self-consistency/structural hunters: 6 parallel batches over the 111 codex docs touched in the last 24h found 19 confirmed defects — 1 leaked tool-call XML artifact, 4 unclosed-markup-spanning-a-blank-line breaks, 4 systemic `manifest_dep_count` mismatches (checked deterministically across all 55 audit yaml files, exactly 4 real), 10 self-contradicting numbers/claims (each independently re-verified against the doc's own other content or a cited external source before fixing, not just trusted from the hunter's quote) — plus 2 follow-up investigations (a stale ~37%-vs-corrected-~80-90% GH-cron-throttle figure, a malformed pipe-table caused by an unescaped literal pipe character inside a code span) and 1 finding filed as an issue doc instead of guess-fixed (`production_readiness_checklist_file_missing_2026_07_24.md` — a cited backing file doesn't exist anywhere in the repo, 5 docs across 2 repos disagree on the item count, no way to determine ground truth without an engineer's decision) | `pm@7535801d3` (7 stale entries), `pm@07e24de0b` (54 repoints), `pm@8c364ce71` (XML leak + 4 yaml), `pm@0350c3262` (15 self-consistency/structural fixes + issue doc)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    |
| Operator-requested broader audit pass, part 3: `/data-pipeline-reconciliation` x5 (one per AG)                                               | **IN PROGRESS** — launched in parallel with part 1 rather than waiting (independent corpus, no dependency): background Workflow task `wtb0lnq1m`, run `wf_db7e7fab-127`, 5 parallel agents (tradfi/defi/cefi/prediction/sports), raw-tick layer only (candles layer out of scope for this dispatch). Phase-0 pre-verified before dispatch to remove the most common first-run failure points: venv `market-tick-data-service/.venv` (only one that imports both UTL+UAC), `GCP_PROJECT_ID=central-element-323112` required env, `resolve_bucket_name` is all-keyword-only, prediction uses a dedicated flat `kind='market-data-tick-prediction'` (NOT `asset_group='prediction'` on the `market-data` kind, which raises `BucketNamingError`), all 5 prod buckets confirmed reachable. Script persisted at `/home/ubuntu/.claude/projects/-home-ubuntu-unified-trading-system-repos-market-tick-data-service/bbf81a1c-a8d9-4bff-857f-6b9f1b0f0229/workflows/scripts/data-pipeline-reconciliation-x5-wf_db7e7fab-127.js`, resumable via `Workflow({scriptPath, resumeFromRunId: "wf_db7e7fab-127"})` if the launching session is gone. Each agent writes `plans/audit/results/data_pipeline_reconciliation_<ag>_2026_07_24.md` + JSON and commits/pushes its own report                                                                                                                                                                                                                                                                                                                                                                                                                                              | Nothing — waiting on completion notification (no polling); once all 5 land, relay each report + triage any filed issue docs                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              |
