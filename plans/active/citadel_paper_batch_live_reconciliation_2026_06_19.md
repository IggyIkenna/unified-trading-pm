...
- [x] ✅ [CODE] P2.11.14. **Wire the BTC-level trend-following (CTA) leg — SHIPPED 2026-06-22: UAC@61ac3ad2
      ...
      pre-flight) — recover with `git -C .tabs/1/unified-api-contracts stash pop`(stash msg "TSMOM_BTC_CTA archetype +
      WS-mapping fix — blocked on UAC version-lag"); strategy-service edits remain UNCOMMITTED in its clone.** The UAC
      ...
      **0.40.0** (the manifest-update workflow hasn't synced the bump — the documented VERSION_SPLIT promotion-lag, here
      ...
      (`enums.py archetype_leg_spec_seeds.py tests/unit/test_archetype_leg_spec.py`) + a separate `fix(tests):` commit
      for the WS mappings → then quickmerge strategy-service (it depends on the UAC enum, so promote UAC first). The
      agent's first pass left it unshipped + had ONE hallucinated WS-test edit (invented connectors) which was dropped;
      the real WS mappings were re-added.
...
- [x] ✅ [UI] P2.11.17. **Mirror the `TSMOM_BTC_CTA` archetype into unified-trading-system-ui — SHIPPED + VERIFIED
      ...
      `public/ capability-verdict-matrix.json` + 6 test files). tsc clean, 286 Vitest pass, `quality-gates.sh` exit 0.
      The playwright SMOKE gate (`tests/smoke/`) self-starts `PORT=3100 pnpm dev:mock` (120s boot) — the earlier
...
- **na-eligibility-audit 2026-08-17 (cross-cutting tranche, dispatch agt-3931fd)** [body-hash:b71a12f0d634c013]: KEEP-NA, valid — P2.7.3 is a CLAUDE.md-enumerated permanent human-only hard-stop (live wallet/custody approval), reconfirmed 2026-07-28 and unaffected by the fully-proven paper<->batch determinism spine; P2.11.15/P2.11.18(b) are ML-research cs-model retrain judgment work, explicitly a near-verbatim duplicate of crypto_alpha_research_2026_07_24.md's own open [RESEARCH] P2 todo (conflict-check-held-back, not extracted) — reaffirmed KEEP-NA by na-eligibility-audit 2026-08-02 and 2026-08-08. Ruling/citation: Reviewed 2026-07-28 (operator gate-cleanup pass), reconfirmed: live wallet/custody approval (Copper/CEFFU) is named in CLAUDE.md as a permanent, human-only hard-stop alongside force-push-main and 1.0.0 graduation — 'Not retagged, not unlocked.'
