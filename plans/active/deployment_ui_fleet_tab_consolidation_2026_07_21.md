...
> **🟡 Kept `draft` deliberately (operator 2026-07-21)** — dispatch held pending AO changes settling. Also authored in
> the tabs-3 worktree per operator instruction (keep new work off the root repo where other agents are active).
>
...
> (filters + shared-primitive extraction). To avoid a same-file collision (multi-agent safety), this plan `depends_on`
> that one and should be **activated/executed after** it lands. `depends_on` without `gate_on_depends` does NOT
> machine-hold dispatch (verified from regen code 2026-07-20) — so the ordering is enforced by **activation order**, not
> the backlog. Do not flip this `active` until the date-range plan's `Deployments.tsx` work is merged.

## Context — Fleet-tab audit findings (2026-07-21, read-only)

Fleet (`/fleet` → `CockpitFleet` → `FleetTab`, `Cockpit.tsx:505-641`) is ONE page stacking five sections:

| #   | Section                                                              | file                  | Verdict                                                            |
| --- | -------------------------------------------------------------------- | --------------------- | ------------------------------------------------------------------ |
| a   | Reconciliation cards (Accounted / Unknown / Expected-missing)        | `Cockpit.tsx:505-605` | REMOVE (redundant; keep only the "expected-missing" idea if cheap) |
...
- **Deployments already SEES idle spend** — `DISK`/`STATIC_IP` orphan rows (kind filter "disk (orphaned)" / "static IP
  (orphaned)", `Deployments.tsx:1223-1224`), per-row `LeakedBadge` "N leaked ·
  ~$X/mo", estate-wide
  `StrandedCostBadge`, a `stopped` status filter, and Resources sortable by leaked $/mo. **But it
  lacks** FleetOrphans' (1) reap/delete ACTIONS (`VmControls` is pause/resume/stop/restart only, `VmControls.tsx:22`;
  FleetOrphans has dry-run bulk `reapOrphans` + per-instance delete-with-boot-disk, `FleetOrphans.tsx:82-121,278-342`),
  (2) reap-verdict/grace classification (`FleetOrphans.tsx:28-34`), (3) idle-spend rollup cards (Stopped VMs · Reapable
  · Idle disk $/mo · Reclaimable $/mo, `:162-197`) + stopped-age, and (4) it defaults `status=running` so orphans are
  hidden until you switch the filter.
- **Idle-disk cost is an ESTIMATE on BOTH surfaces** — FleetOrphans `monthly_disk_usd` is "asia-northeast1 list rate ×
  GB" (`client.ts:4394`), not real billing; Deployments' leaked-$/mo is the same class of estimate. Real-billing idle
  spend (WS-1-style) is a possible later enhancement, NOT in scope here — preserve the estimate for parity.
- **FleetOrphans is self-contained** — depends only on `/api/fleet/orphans`, `/api/fleet/reap`,
  ...
  `DeploymentItem` (`deploymentApi.ts:740-807`) — merging them needs either Deployments consuming `/api/fleet/orphans`
  or new inventory fields (backend todo below).
- **`/vm-deployments` is a SEPARATE canonical tab** (`App.tsx:174`, `NavMenu.tsx:111-113`), not legacy — Fleet only
  embeds a compact copy. Operator decision: consolidate it into Deployments and retire it. Its unique content vs
  Deployments: the archive/history table (Outcome, Duration, Rows Captured, Completed, archive log links) + `rows_error`
  (Errors) + throughput (`rows_in`/`events_emitted`). The archive **log links overlap WS-4** (the run.log viewer) — do
  NOT build a second log renderer; defer log display to WS-4's viewer.
...
- **KEEP-set enhancement (operator 2026-07-21)** — FleetGit shows the derived `reporter_stale` "reporter dead" boolean
  (`FleetGit.tsx:58,70`) but NOT the actual snapshot time. The `reported_at` timestamp is already stored server-side
  (`SlotRow.git_status_reported_at`, posted as `reported_at`) AND already on the wire type (`client.ts:4231`) — it's
  simply not rendered. Surfacing it is a pure UI add (see todo). Context: each host's reporter POSTs to ITS OWN local
  orchestrator; a laptop running a local standalone AO instance shows only its own slots (the "no `ORCHESTRATOR_VM_ID` ⇒
  standalone/isolated" rule, `vms.py:220`), so the snapshot time also tells the operator which local instance's clock
  produced the row.
...
- [ ] [BACKEND] P0. **Orphan/idle data on the Deployments surface** — make the reap-verdict, grace, `stopped_age_hours`,
      ...
      estimate.
- [ ] [UI] P1. **Idle-spend rollup cards on Deployments** — port the four FleetOrphans cards (Stopped VMs · Reapable ·
      Idle disk $/mo · Reclaimable $/mo, `FleetOrphans.tsx:162-197`) as a header/section on the Deployments tab.
...
- [ ] [UI] P1. **Retire /vm-deployments** — remove the route (`App.tsx:174`) + nav entry (`NavMenu.tsx:111-113`) once
      its content is folded in; add a redirect `/vm-deployments → /deployments`.
...
- [ ] [UI] P1. **Remove the VM-census embed + reconciliation cards** from `FleetTab` (`Cockpit.tsx:505-605,609`) — after
      the merges above land. Optionally preserve the "expected-missing" (registered-but-not-running) count as a small
      Deployments summary if cheap; otherwise drop.
- [ ] [UI] P1. **Fleet = FleetGit only** — `FleetTab` renders only `FleetGitContent`; update the page title + the
      `NavMenu.tsx:148-156` description ("Census · orphans · git · infra" → "git health · dirty repos"). Verify FleetGit
...
- [ ] [UI] P1. **Show the snapshot timestamp per slot in FleetGit** — render the `reported_at` field (already on the
      wire type, `client.ts:4231`; already stored server-side as `git_status_reported_at`) next to each slot in
      `FleetGit.tsx`, so the operator can see WHEN the status snapshot was taken, not just the derived `reporter_stale`
      "reporter dead" boolean (`FleetGit.tsx:58,70`). Show both an absolute time and a relative age (e.g. "3m ago") so
      freshness reads at a glance and pairs with the existing stale badge. Pure UI — NO backend change (the timestamp
      already reaches the frontend).
