...
- [x] ✅ [DATA] P0. **⑧ — IS cefi REFERENCE-UNIVERSE gap: catalogue not ⊇ manifest present-set (CF-14, falsely-high
      ...
      PACIFICA-SOLANA was removed from scope entirely by a later operator ruling (see
      `plans/archive/issues/solana_perp_dex_cull_drift_pacifica_2026_07_16.md`, "operator kill ruling 2026-07-16"; code:
      `instruments_service/engine/orchestrator/defi.py` comment "all Solana perp DEXes dropped except Jupiter, not
      integrated") so its CLOB-enumeration sub-part is now moot. **(4) ~650 UNKNOWN/blank-venue pollution rows —
      RESOLVED**: read `market-data-tick-cefi-prd`'s `_index/availability_index.parquet` directly (8,764,263 rows) — 0
      blank-venue rows, 0 `UNKNOWN`-venue rows, 0 `*F0`-suffixed instrument_ids today (was ~650). Corroborating
      evidence: the first-ever complete `cf_manifest_audit.py` rollup
...
- [x] ✅ [OPERATOR] P0. ~~E8 ⚠️ IRREVERSIBLE — only after E7 GREEN: hand C-GREEN to `bucket_name_ssot…` L6 → **delete
      legacy `market-data-tick-cefi` permanently** (single source of truth; legacy data is gone). **(MIGRATED FROM:
      `cefi_manifest_canonicalisation_2026_06_01.md`, 2026-07-13 per MTDS consolidation ruling.)**~~ **DONE-BY-OPERATOR
      (corrected 2026-08-09, plan_reconciler — was mistagged `[DATA]` and left open):** the identical action is already
      confirmed done in `cefi_e4_e8_orphan_sweep_gapfill_rebuild_execution_2026_07_28.md`'s Phase F —
      `market-data-tick-cefi-central-element-323112` returns 404 on `gcloud storage buckets describe`, Cloud Audit Logs
      confirm `storage.buckets.delete` by the operator at 2026-07-14T11:02:29Z (~2 weeks before this plan's E7/E8 todos
      were even authored). Retagged `[OPERATOR]` to match — this was never worker-dispatchable in the first place
      (whole-bucket destroy, no §3a reversibility carve-out) and flipping now prevents a future dispatch against a
...
- [x] ✅ [DATA] P1. **VERIFY MDPS candle-manifest faithfulness — DONE 2026-07-27 (slot-14).** Verdict: **YES, MDPS is
      ...
      vocabulary — a 2026-07-21 operator ruling (see
      `plans/archive/issues/mdps_generic_classifier_processed_regression_2026_07_21.md`; code:
      ...
      **Cross-write reconciliation**: not re-measured this session (the old 782/616 figures are themselves now suspect
      ...
      emission-policy gate (`should_publish_row=False`, the "heartbeat-only" path) intentionally uploads GCS bytes while
      skipping `record_captured` — a second, BY-DESIGN source of files-without-manifest-rows, plus a broad
