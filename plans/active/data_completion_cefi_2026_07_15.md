...
last_updated: 2026-08-07 # line-cap remediation split -- extracted 06-21/07-27/07-28 corroborating-audit Progress Log history to the archive doc above; context_scope backfilled; E8 flip + E4-measured + 3 repoints (plan_reconciler 2026-08-07)
locked_by:
locked_since:
supersedes:
superseded_by:
...
- [x] ✅ [DECISION] P0. **Retagged 2026-07-29 (corpus hygiene pass): resolved-by-reference — see
      ...
      **RETAGGED [DATA]→[OPERATOR] 2026-07-28 (slot-9)**: same reclassification as the two sibling todos above — the
      ...
      root L-flat orphans** now their `pipeline_mode=` forms exist. **(Measured actual 2026-08-03: 287,074 objects
      deleted, 0 errors — the ~1.2M estimate was contaminated by a separate candle destination-shape bug; see
      `cefi_e4_e8_orphan_sweep_gapfill_rebuild_execution` Phase B + market-tick-data-service@fa991f12.)** PRE-DELETE
      GUARANTEE (mandatory): first run `migrate_cefi_flat_to_v9_canonical --apply` over the FULL range once (idempotent
      — copies any orphan still lacking a sibling, skips the rest) so EVERY orphan provably has a migrated dest; THEN
      delete (count via Monitoring live-object, NOT naive recursive `ls`; per-object isolation; idempotent). This IS the
      E7 orphan-sweep. (b) `--also-legacy` 5,233-cell legacy→canonical gap-fill (additive; VM-scale — the 1.9M legacy
...
- [x] ✅ [DATA] P0. E8 ⚠️ IRREVERSIBLE — only after E7 GREEN: hand C-GREEN to `bucket_name_ssot…` L6 → **delete legacy
      ...
      2026-07-14 (flipped 2026-08-07, plan_reconciler, evidence: `cefi_e4_e8_orphan_sweep_gapfill_rebuild_execution`
      Phase F :227-231 — bucket `market-data-tick-cefi-central-element-323112` 404 + audit-log `storage.buckets.delete`
      2026-07-14T11:02:29Z; see `issues/cefi_legacy_bucket_deleted_before_l3_gate_2026_07_28.md`). NOT a claim the
      E7-GREEN gate was honored before the delete; E7 (:495-498) stays OPEN — successor Phase D/E owns it.**
...
- [x] ✅ [DATA] P1. **Both (a) and (b) CLOSED 2026-07-29 (slot-12) — via the already-completed sibling campaign,
      ...
      `plans/archive/2026_08/mdps_candle_manifest_near_total_coverage_gap_2026_07_27.md` (corpus-wide orphan-sweep +
...
- [x] ✅ [DATA] P1. **cefi `instruments-store` `_index` v8→v9 single-walk — VERIFIED GREEN 2026-07-29 (slot-4,
      ...
      `plans/archive/issues/cefi_instruments_store_blank_data_type_residual_2026_07_29.md`. CF-2-paths / CF-3-partition
