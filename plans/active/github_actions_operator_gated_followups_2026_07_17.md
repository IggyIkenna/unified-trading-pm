...
- [x] ✅ [INFRA] P0. **RESOLVED 2026-08-08 — rewritten to reflect the stall-alarm reality; the original "DELETE
      ...
      **hard-refused for minting** and instead checked for the real invariant it exists to protect ("`main` must not
      ...
      `/codex/08-workflows/ci-cd-flow.md:1413` § _"Release tag reconciler — a STALL DETECTOR, not the minter (corrected
...
| **D3** | **The 3 dead workflows** — operator wants to review first (2026-07-17)              | **`reconcile-release-tags` RESOLVED 2026-08-08** — repurposed into the fleet's release-stall alarm (see the retired-todo entry above), not deleted; its part of D3 is closed. **CORRECTED 2026-08-21 (ag-closeout-audit, citing `plan_reconciler_findings_ci_2026_08_19.md`)**: the 2026-08-12 correction below is now itself stale — `digest-drift-sweep`'s primary cascade is **no longer dormant**. `plan_reconciler_findings_ci_2026_08_19.md`'s Batch E F1 independently re-ran `gh run list --workflow=update-dependency-version.yml` and found it firing multiple times/day across all 3 of the doc's own sample repos (execution-service, agent-orchestrator, ml-service), all `conclusion: success` — likely resumed as a side effect of `post_cutover_silent_assumption_sweep_2026_07_23.md`'s F2 semver-agent retarget fix (exact resuming commit not traced). The source SSOT, `issues/digest_drift_sweep_silent_noop_github_token_scope_2026_07_16.md`, is now 0-open/all-done and was **archived** the same run. **STEP 2d's blocking citation is therefore stale — recommendation 1 is resolved and no longer holds it back.** STEP 2d itself stays open: its actual deliverable (a cheap recurring "did work" ≠ 0 assertion across all movers) was never built. ~~CORRECTED 2026-08-12 (/plan-reconcile)~~: `digest-drift-sweep` is 3-of-4 FIXED (2026-07-26, per its own SSOT's banner) — the token bug + silent-failure hardening + dispatch cap all shipped. Only recommendation 1 (investigate the dormant `update-dependency-version.yml` primary cascade) is still open, and STEP 2d is still held on that one remaining item. |
...
> **Progress-Log history (2026-08-16)**: the "Hard-won context the next session should inherit rather than
> rediscover" (2026-07-17/22 post-migration system-check narrative + operational lessons) and "Cost ruling 2026-07-23"
> (semver-agent revert decision) sections were extracted verbatim to
> [`/plans/archive/2026_08/github_actions_operator_gated_followups_hard_won_context_and_cost_ruling_history_2026_08_16.md`](/plans/archive/2026_08/github_actions_operator_gated_followups_hard_won_context_and_cost_ruling_history_2026_08_16.md)
> to bring this doc back under the 1000-line hard cap (same pattern as the 2026-08-03 extraction below). Nothing was
> summarized or altered — read the linked doc for the full narrative.

## Deferred work after 2026-07-23

| #   | Item                                                                   | State / why deferred                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             | Blocked on               |
| --- | ---------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ------------------------ |
| 1   | **Trading kill-switch is a no-op** (`halt-order-flow` has no listener) | **STALE TAG — CORRECTED 2026-08-16 (plan_reconciler).** Was `OPERATOR-OWNED`; the SSOT (`post_cutover_silent_assumption_sweep_2026_07_23.md` F1) retagged this away from `[OPERATOR]` on 2026-07-28 — **TIME-GATED, not an authority decision**: correctly deferred because execution-service does not yet run live order flow, so there's nothing to implement/verify a halt against. Verified first-hand: execution-service has only a `dependency-update` listener; 204 reads as success; the code's own comment predicts a 404 that never comes.                                                                                       | Execution-service going live |
