...
- [ ] ⛔ [INFRA] P0. **SUPERSEDED 2026-07-26 — DO NOT EXECUTE THIS DELETION.** ~~DELETE `reconcile-release-tags`~~ The
      ...
      (banner at top).
- [ ] [REVIEW] P0. **OPERATOR DECISION on cassette-drift-check — CORRECTED 2026-08-06, do NOT close as false
      positives.** **(a) is WRONG as written — verified live against GitHub, do not act on the original framing.** This
      item originally claimed the open `[Cassette Drift]` issues were "self-refuting: 'Total cassettes checked: 0 /
      Drifted: 0 / No report file found'". That description matches the PRE-2026-07-17 failure mode (the detector
      calling a deleted script and swallowing the error). **Live check 2026-08-06**
      (`gh issue list --repo     IggyIkenna/unified-api-contracts --search '"[Cassette Drift]" in:title' --state open`):
      only **18** are currently open (not 52 — some were evidently already closed), and **every single one** of the 18
      (#641 2026-07-18 through #862 2026-08-06) shows a REAL report: `Total cassettes checked: 177-179`, `Drifted: 2`,
      with the **identical two errors every day for 19 consecutive days**: `pools.yaml` —
      `BalancerPoolsResponse.poolGetPools` missing a required field, and `ticker.yaml` — `DeribitTickerResponse.result`
      type mismatch (list vs dict). **This is a real, persistent, never-fixed schema drift bug, not a false-positive
      artifact** — closing these as "self-refuting" would silently bury 19 days of correctly-firing evidence of an
      actual unfixed bug. **Operator ruling needed on the REAL question**: (i) fix the two schema mismatches (Balancer
      pools response model, Deribit ticker response model) so the check goes green, then close all 18 as resolved-by-fix
      citing the commit; or (ii) if the drift is itself an artifact of FINDING #4's matching-lottery problem
      (cassette→model matching by filename stem, not venue-scoped), fix the matching first, re-run, and only then
      decide; or (iii) consolidate now regardless of root cause — keep the newest (#862) as the one live tracking issue,
      close the other 17 as duplicates-of-#862 (not false-positives), and let the fix land against the single survivor.
      **(b) unchanged from the original finding**: per FINDING #4 the detector's cassette→model matching is a lottery
      (filename stem only, venue discarded, substring match over 2172 models; 15 of 28 sampled were generic stems on an
      earlier report) — worth checking whether that's _why_ Balancer/Deribit specifically keep drifting (wrong model
      matched, not a real schema change) before assuming the fix is "adjust the model." **Ikenna owns the cassette-count
      verification (operator 2026-07-17) — do not duplicate it.** SSOT:
