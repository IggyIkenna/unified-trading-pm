...
related:
  [
    ...
    /plans/archive/2026_08/issues/digest_drift_sweep_silent_noop_github_token_scope_2026_07_16.md,
...
- [x] ✅ [REVIEW] P0. **RESOLVED 2026-08-08 — consolidated 20→1, root cause diagnosed, fix specified (not yet
      shipped).** **operator ruling 2026-08-08** (see SSOT below,
      `/plans/archive/issues/cassette_drift_check_calls_deleted_script_and_swallows_it_2026_07_17.md` banner):
      consolidate the 20 open issues down to 1, archive the rest, fix the 2 real schema mismatches (Balancer pools /
      Deribit ticker), then retire the last remaining issue once the fix lands. **Executed live via
      `gh issue close`/`gh issue comment` against `IggyIkenna/unified-api-contracts`**: closed 19 duplicates (#641,
      #660, #672, #683, #698, #713, #720, #729, #736, #758, #778, #791, #814, #825, #827, #843, #850, #862, #870) as
      dup-of-#880 (not false-positives — each is annotated as a correct daily detection of the same real condition);
      kept **#880** (2026-08-08, newest) as the single live tracking issue, verified via
...
- [ ] [VERIFY] P0. Re-measure a representative QG run's billed job-minutes + the docs-PR / identical-tree skip rates
      before/after (ledger + run counts). (tracked in `ci_satellite_ao_dispatch_batch4_2026_07_31.md`, status: active —
      todo 9 sub-item 2 of the 4-item billing sweep; open/dispatchable now, not yet completed. **Corrected 2026-08-07
      (na-eligibility-audit)**: previously said "status: draft, not yet dispatched" — batch4 has been `status: active`
      since creation; do not re-extract, batch4 todo 9 already owns this item)
...
- [ ] [VERIFY] P0. Two weeks after rollout, re-pull the billing ledger and compare to the Phase-0 baseline; record
      ...
      at the 2-week mark, not just the fleet aggregate. **Blocker-currency note (na-eligibility-audit 2026-08-03)**: the
      calendar wait is now over (rollout landed 2026-07-17; 2-week mark ~2026-07-31; today is 2026-08-03) — the re-pull
      can be run now. Work itself (pull ledger, compute deltas, compare to Phase-0 baseline) is still not done. (tracked
      in `ci_satellite_ao_dispatch_batch4_2026_07_31.md`, status: active — todo 9 sub-item 1 of the 4-item billing
      sweep; open/dispatchable now, not yet completed. **Corrected 2026-08-07 (na-eligibility-audit)**: previously said
      "status: draft, not yet dispatched" — batch4 has been `status: active` since creation)
...
| 3   | **QG sentinel is environment-blind**                                   | **STALE — MOSTLY RESOLVED, updated 2026-08-09 by plan_reconciler agt-c80749.** Per `plans/active/issues/qg_sentinel_environment_blind_2026_07_23.md`: sentinel-binding fix SHIPPED `unified-trading-pm@4545df4c6`; deployment-api/strategy-service CONFIRMED CLEAN 2026-07-31 (no fix needed). Only the MTDS-specific 2 test cases remain genuinely unresolved (that doc's `/na-eligibility-audit` 2026-08-06: KEEP-NA valid, MTDS sub-item is the sole open scope).                                                                                                                                                                             | Operator picks fix split (MTDS sub-item only) |
| 4   | `staging_versions` dep-gate fix                                        | **Blocker (Item 2) resolved 2026-07-25 — re-scope before acting, do not simply unblock-and-run as originally framed** (noted 2026-08-09 by plan_reconciler agt-c80749). Its ORIGINAL premise was inverted by finding F2 (`staging_versions` tracks the real git tag, `versions` does not) — Item 2 shipping doesn't automatically make this row's original framing correct; whoever picks this up should re-derive what the dep-gate fix should actually do now that F2 is understood, not execute the stale pre-F2 description verbatim.                                                                                                        | Nobody (was: Item 2, now resolved)            |
| 5   | Codex staging re-entry procedure + stale branch-model sections         | **STALE — RESOLVED, noted 2026-08-09 by plan_reconciler agt-c80749.** `codex/08-workflows/ci-cd-flow.md` now carries an explicit "#### Staging re-entry procedure (when a repo is routed back through staging)" section (line 169) with a full per-workflow re-entry-trigger table, and consistently frames staging as DORMANT/reversible throughout (lines 6/34/60/122/127+) — not the stale "staging as canonical" content this row's original L75-109/etc citations pointed at (codex has grown/reorganized substantially since 2026-07-17; those line numbers no longer resolve to the described content).                                   | Resolved, needs no owner                      |
...
- [ ] [VERIFY] P2. One week after the first repo's flip lands, re-pull the Enhanced-Billing usage report (method above)
      ...
      Gate ("one week after the first repo's flip") elapsed 2026-07-27/28. (tracked in
      `ci_satellite_ao_dispatch_batch4_2026_07_31.md`, status: active — todo 9 sub-item 3 of the 4-item billing sweep;
      open/dispatchable now, not yet completed. **Corrected 2026-08-07 (na-eligibility-audit)**: previously said
      "status: draft, not yet dispatched" — batch4 has been `status: active` since creation)
- [ ] [VERIFY] P2. Once ≥5 repos are flipped, re-pull the FULL fleet total (not just PM, and not just the flipped repos
      ...
      ~$1,000/mo → ~$300-400/mo" target was about, and the one that has never yet moved. Gate ("≥5 repos flipped")
      elapsed 2026-07-27/28 — all 24 non-PM repos flipped. (tracked in `ci_satellite_ao_dispatch_batch4_2026_07_31.md`,
      status: active — todo 9 sub-item 4 of the 4-item billing sweep; open/dispatchable now, not yet completed.
      **Corrected 2026-08-07 (na-eligibility-audit)**: previously said "status: draft, not yet dispatched" — batch4 has
      been `status: active` since creation)
...
(zero code changes needed) — 178 MOVE / 109 KEEP across 24 repos, all resolved cleanly (corrected 2026-08-09 by
plan_reconciler agt-c80749 — re-summed the table's own two KEEP columns directly: 52 + 57 = 109, not the 108 originally
stated):
...
- **context-scout 2026-08-07**: refreshed context_scope (5 -> 6 entries) — added
  ...
  found, not false positives — a live unresolved P0). Other 5 entries re-verified and still resolve. **⛔ SUPERSEDED the
  very next day (noted 2026-08-09 by plan_reconciler agt-c80749)**: the 2026-08-08 investigation above (see the
  `RESOLVED 2026-08-08 — consolidated 20→1` entry earlier in this doc) found the "18 real open drift issues" were NOT
  live API drift after all — a checker bug (missing GraphQL-envelope unwrap) plus one mis-recorded cassette, both fixed
  on `#880`. This context-scout entry accurately reflected the state as of 2026-08-07 but was never flagged as
  superseded once the deeper root-cause investigation reversed it the following day; leaving as historical record with
  this pointer rather than deleting it.
