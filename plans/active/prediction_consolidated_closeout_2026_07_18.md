...
| ----------------------------------------- | ---------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| CQG cluster atom in the live manifest     | **present at `captured`: 17,352 rows** — CORRECTED 2026-07-18 (A0 live read) | A0 measured 80,068 CQG bundle rows (captured 17,352 / empty_confirmed 60,286 / expected_unattempted 2,421 / attempted_failed 9), 81 distinct canonical CQG values — this SUPERSEDES the folded issue's "ZERO at captured" claim; the phantom wipe is fixed or intermittent. **Re-check** `issues/prediction_phantom_reconciler_wipes_bundle_atom_2026_07_10.md` status before assuming a wipe (Phase-B item downgraded to verify-not-fix). **RE-OPENED 2026-08-17, DIFFERENT mechanism — see
`manifest_hygiene_red_all_2026_08_17.md`**: a separate, still-open finding shows this SAME atom DIVERGENT_EMPTY on
419/970 days (43%, including today), root cause unidentified after 4 diagnosis rounds — not the same phantom-wipe
issue (that one is archived resolved) |
...
| MTDS prediction `-test-` bucket isolation | **FIXED 2026-07-18** (see Conclusion below)                                                 | `_test_bucket("prediction")` has no `-test-` sibling and falls back to the PROD `market-data-tick-prediction` bucket (`market-tick-data-service/scripts/pipeline_e2e_check.py:434-450`) — a prediction force/skip leg would write to PROD, breaking the test-bucket-only invariant cefi/tradfi enjoy                                                                                                                                      |
| Instrument-id canonical shape             | **CORRECTED 2026-08-12 (/plan-reconcile): largely done, not "partial"**      | `prediction_canonical_identity_migration_2026_07_08.md` (this row's own source doc) is now ARCHIVED at `plans/archive/2026_07/` with 7/8 checkboxes `[x]` (1 remaining), including the migration-gate confirmation "`migrate_prediction_to_pred_prd_v9.py` completed... complete live SSOT" — the "4/8 done" figure is stale                                                                                                              |
...
| Football fixture ↔ Kalshi market          | **CORRECTED 2026-08-12 (/plan-reconcile): resolved, not "NONE"**             | `prediction_phase_e_football_arb_live_2026_07_24.md` (`status: active`) reports Kalshi soccer team-name resolution SHIPPED (`instruments-service@ec8633ac` parser + `unified-api-contracts@e7ed754e`) — **~0% → 82.6%→~100% match on 92 live Kalshi fixtures** via the same `af_fixture_id` chain used for Polymarket; "no team registry, honestly absent" no longer holds                                                                |
...
- **Manifest / CQG / phantom**:
  ...
  - [`plans/active/issues/manifest_hygiene_red_all_2026_08_17.md`](/plans/active/issues/manifest_hygiene_red_all_2026_08_17.md)
    — **added 2026-08-19 (plan_reconciler)**: still-open, DIFFERENT mechanism than the archived phantom-wipe above —
    the SAME `(POLYMARKET, prediction_canonical_question_group)` atom is DIVERGENT_EMPTY on 419/970 days (43%, incl.
    today), real writer for a large share of rows still unidentified after 4 diagnosis rounds (slot-9/14/6/19/22). 3
    open `[DATA]`/`[SCRIPT]` P1/P2 todos (IS trace, 2× VM launch), 2 open P2 (git-history check, process trace), 1
    `[x]` P3 (docstring fix).
