...
- [x] ✅ [DATA] P0. Post-walk: fresh `_index` read — `schema_version=9` for 100% of rows (data-state), `pipeline_mode=`
      ...
      `total=5,553,198 rows · schema_version=9=5,553,198 (100%) · blank pipeline_mode=0 · blank source=0`, independently
      verified. This is the C-GREEN signal `bucket_name_ssot…` Phase 6/7 waits on for the legacy
...
- [ ] [DATA] P0. **R1 RUNBOOK — the tradfi `migrate_tradfi_to_v9_canonical --apply` MUST include `--also-legacy`** to
      ...
      permanently deleted (ADC `bucket.exists() == False`). Full evidence + final decision (accept the loss, resolved
      2026-07-26): `/plans/archive/issues/tradfi_legacy_bucket_deleted_without_also_legacy_migration_2026_07_26.md`.
...
- [ ] [DATA] P2. **ohlcv_15m/24h conversion — 429 FIXED but NOT done; 4-part diagnosis (corrected 2026-06-22, I had
      ...
      service tarball from a clean LDR checkout (`market-data-processing-service@2ef81aa8`) and relaunched
      ...
      **CITATION CORRECTION (na-eligibility-audit 2026-08-06, tradfi tranche)**: the batch5 tracker above is since
      ...
      the new issue doc. (3) ❌ ~64k of the 1m corpus is OLD migrated data with malformed
...
- **context-scout 2026-08-15**: re-verified context_scope, no change needed (6 entries) -- the Databento
...
- [x] ✅ [SCRIPT] P3. **EXTRACTED 2026-08-17 (na-eligibility-audit, tradfi tranche, dispatch agt-d99b5c) →
  ...
  **na-eligibility-audit 2026-08-17**: item 15 RECLASSIFIED, extracted to
