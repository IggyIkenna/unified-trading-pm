...
**Disposition**: todo 4 stays open. This is NOT a code bug and NOT something to keep retrying interactively — a 23M+-row
manifest's per-VM-shard fallback merge is exactly the heavy-compute-on-shared-host case the workspace's own
`vm-launcher-runbook.md` heavy-I/O rule exists for. **Next attempt should use `scripts/dev/run-bounded-analysis.sh`
(mem-capped) or a dedicated VM, never a bare interactive `python3 -c` on the shared host** — and should investigate
_why_ defi's consolidated blob is chronically >120s stale (heavy concurrent per-VM writing? a consolidator cadence gap
specific to defi?) since that staleness is what triggers the expensive path every time, not just for this one read.
tradfi/prediction were never reached. No code changed, no manifest touched.
