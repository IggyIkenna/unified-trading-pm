...
- [ ] [SCRIPT] P2. **Kalshi live WS may share the oversized-subscribe risk — verify/chunk (DISCOVERED 2026-06-23).** `kalshi_ws.py::_run_ws_session` (line ~218) also subscribes ALL `market_tickers` (~2000) in one frame. Kalshi's WS limit is unverified (no error logged yet on the kalshi-trades VM, but its log was sparse). If Kalshi captures with 2000 single-frame tickers, leave it; if it shows a connect/subscribe stall like Polymarket's, apply the same ≤N-per-frame chunking to `kalshi_ws.py` (+ `kalshi_clob_ws.py`). Verify against the live kalshi VM before changing (don't regress a working path). Repo: market-tick-data-service. Provenance: continuous-flow session 2026-06-23.
