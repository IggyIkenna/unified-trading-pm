...
- [ ] [SCRIPT] P0. **Prediction LIVE captures 0 — IS instrument-availability universe is STALE (max day=2026-05-22) with NO current-day `clob_token_ids` (DISCOVERED 2026-06-23; the remaining live blocker after the code fix mtds@aed9fb2).** The `_is_universe` code now correctly returns `[]` (no 0x pollution), but that means prediction live is honestly-empty until a FRESH-TODAY token-id universe exists at `instruments-store-<?>/instrument_availability/by_date/canonical_question_group=*/day=<TODAY>/venue=POLYMARKET/instruments.parquet` WITH `clob_token_ids` populated. Today there is NONE (all cqg groups max at day=2026-05-22; the 2026-05-22 parquet lacks the `clob_token_ids` column). The `expected-universe-v2-prediction` Cloud Run job only seeds `_index` (`prod/catalog.parquet`), NOT the availability parquet; `lifecycle-catalogue-regen-prediction` (`build_instrument_catalogue.py --asset-group prediction`) is PAUSED. **Focused IS-session fix**: make the IS prediction write/consolidation path refresh `by_date/.../venue=POLYMARKET/instruments.parquet` for the CURRENT day with populated `clob_token_ids` (the gamma-active set per IS@482b50f today-routing) — likely re-enable + repair the catalogue-regen job OR a one-shot live-universe writer (plan option a/b above) — AND reconcile the env-short `instruments-store-pred-prd-` (catalog) vs env-less `instruments-store-prediction-` (availability) bucket the live runner reads (defi-class env-bucket gotcha). Then relaunch the 4 prediction-live VMs on the mtds@aed9fb2 tarball + verify ≥1 `live_polymarket_clob`/`live_kalshi` CAPTURED row. Repo: instruments-service (+ deployment-service relaunch). Provenance: continuous-flow session 2026-06-23.
