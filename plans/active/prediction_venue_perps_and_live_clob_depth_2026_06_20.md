...
- [ ] [SCRIPT] P2. **Kalshi PREDICTION live CLOB depth → `book_snapshot_5`** (completes Phase-3 both-venues live depth; Polymarket done via item 69 @ mtds 26297e4). Kalshi `/trade-api/v2/markets/{ticker}/orderbook` (REST) + ws `orderbook_delta` channel are PUBLIC (verified-pattern: the kalshi-perp connector already does orderbook_delta) → maintain book → top-5 → emit `book_snapshot_5` for venue KALSHI / asset_group prediction; register `(prediction, book_snapshot_5)` source already includes `kalshi` (line 299). Mirror `kalshi_perp_ws.py` + the new `polymarket_clob_ws.py`. Repo: market-tick-data-service. — provenance: item 38 'prediction Q&A markets too' + operator both-venues directive 2026-06-22.
