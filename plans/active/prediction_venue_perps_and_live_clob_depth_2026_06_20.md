...
- [ ] [SCRIPT] P1. **Prediction BATCH recent-window (05-23→06-22) pre-flight FALSE-POSITIVE skip — the gap never fills (MEASURED 2026-06-23 continuity pass)**: `mtds-prediction-polymarket-20260623-131059` ran `--mode batch --venues POLYMARKET --start 2026-05-23 --end 2026-06-22 --data-types trades` and **pre-flight-skipped EVERY date** ("Pre-flight: venue=POLYMARKET date=… — all requested data_types fully covered (atoms ⊆ captured), skipping" → "0 venues ok, 0 total records") — yet `raw_tick_data/by_date/` has **ZERO Polymarket parquets for the whole 05-23→06-22 window** (partitions jump 05-22 → 06-23-live). So the pre-flight reads the manifest `_index` as `captured`/covered while the actual parquet objects are ABSENT — a manifest-vs-data divergence in the prediction batch pre-flight (`engine/orchestrator/preflight.py` "atoms ⊆ captured" check) or the `expected_unattempted` accounting. Net: the 32-day prediction batch hole is NOT data-availability (Polymarket trades ARE API-available for that window) — it's a backfill-MECHANISM bug, same write/consolidation-path class as the prediction-batch items above. **Fix**: make the prediction pre-flight verify the parquet object EXISTS (not just a manifest row) before skipping, OR re-derive the date as uncovered when no `raw_tick_data` object is present; then re-run the recent-window Polymarket batch. Repo: market-tick-data-service (engine/orchestrator/preflight.py). Provenance: continuity pass 2026-06-23 (`data_completion_to_100_all_ag_2026_06_21.md` line-232 note).
