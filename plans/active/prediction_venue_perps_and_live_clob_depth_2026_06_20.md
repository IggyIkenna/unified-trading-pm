...
- [ ] [SCRIPT] P1. **Kalshi live RESOLVES universe (2000) + connects clean (NO ws error, NO unknown-instrument) but captures 0 (all `empty_confirmed`) — likely the SAME parser-shape class as Polymarket; needs the kalshi WS message-shape probe (MEASURED 2026-06-23).** On the db7de3c fresh-tarball relaunch, `prediction-live-kalshi-{trades,book_snapshot_5}-20260623-1303*` both `resolved 2000 instruments` and update per-VM shards ~200 entries/window with NO `WS connection error`, NO `unknown instrument` skip, NO `INVALID` — i.e. the connector is healthy but every window is `empty_confirmed` (captured=0, caprows=0), exactly the symptom Polymarket showed BEFORE its parser fix (mtds@db7de3c). Polymarket was fixed by matching the REAL CLOB `market` channel message shapes (dict-levels + multi-asset `price_changes`); the Kalshi `orderbook_delta`/`trade` WS message shape was NOT re-probed this session. **Next:** probe the live Kalshi WS (`api.elections.kalshi.com` / the kalshi clob ws endpoint) to capture the real `orderbook_snapshot`/`orderbook_delta`/`trade` message shapes, compare to `kalshi_ws.py`/`kalshi_clob_ws.py` parsers, and fix any field/shape mismatch (mirror the Polymarket `_parse_book_msg` fix). Confirm whether the 2000 single-frame subscribe also needs chunking (no stall observed, so likely NOT the blocker — the parser is). Then relaunch + verify captured>0. NOTE: Kalshi markets are lower-frequency than Polymarket so SOME empty windows are honest, but 100% empty across 2000 instruments for 10+ min is a parser bug, not honest absence. Repo: market-tick-data-service (live/connectors/kalshi_ws.py + kalshi_clob_ws.py + live/_is_universe.py). Provenance: continuous-flow session 2026-06-23 — escalated P2→P1 (it blocks the prediction LIVE Kalshi leg; Polymarket leg now CAPTURES 50k+ rows).
