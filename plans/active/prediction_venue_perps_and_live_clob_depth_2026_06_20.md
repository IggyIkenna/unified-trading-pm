...
- [ ] [SCRIPT] P1. **Polymarket BATCH book_snapshot_5 — launcher PARAMETRIZED + adapter book path BUILT (2026-06-23); pending ship+launch+verify**: (1) **launcher DONE** (deployment-service@040e2fc): `launch-mtds-prediction-backfill-vm.sh` now accepts `--data-types trades|book_snapshot_5|trades,book_snapshot_5` (default trades) → `VM_DATA_TYPES` (was hardcoded `trades`). (2) **adapter book path BUILT (NOT yet shipped)**: `download_batch` in `polymarket_adapter.py` previously IGNORED `data_types` and ALWAYS fetched trades — discovered + fixed: added `get_books_batch` (semaphore-bounded concurrent `clob.polymarket.com/book?token_id=` fetches, CF-11 failure routing), `_build_book_snapshot_5_rows` (canonical top-5 `bid_px_1..5`/`bid_sz_1..5`/`ask_px_1..5`/`ask_sz_1..5` + `instrument_id=POLYMARKET:PREDICTION_MARKET:{token_id}`, `data_type=book_snapshot_5`), `_load_token_ids_from_gcs` (resolves per-date token_ids from IS `clob_token_ids`; empty→honest empty, NEVER condition_id fallthrough — mirrors mtds@aed9fb2), `_fetch_books_for_date` writer integration, lifecycle-gated, 2 regression tests; basedpyright clean. **REMAINING: ship the mtds adapter change (QG+quickmerge) + fresh tarball + launch `--data-types book_snapshot_5` recent-window backfill + verify captured book_snapshot_5 rows.** Repo: deployment-service (launcher, DONE) + market-tick-data-service (adapter, built/unshipped). Provenance: autonomous catalogue/backfill session 2026-06-23. **NICE-TO-HAVE** — does NOT block live capture (live book_snapshot_5 already captures via the WS connector once token_ids flow).
