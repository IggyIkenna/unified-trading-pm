...
- [x] ✅ DEFERRED-OPERATOR-DECISION [SCRIPT] P1. NEW UAC `availability_rule.py` — `AvailabilityRule` Protocol + per-source implementations lifted from
      `availability_stamping.py` (odds_api / tick_timestamp / bar_close / etc.).
- [x] ✅ DEFERRED-OPERATOR-DECISION [SCRIPT] P1. Row base class in UAC requires `available_at: datetime` field; pydantic validator invokes row's
      source's `AvailabilityRule.stamp(row)` automatically. Silent-wrong-rule becomes type-level unrepresentable.
- [x] ✅ DEFERRED-OPERATOR-DECISION [SCRIPT] P1. MIGRATE per-source row classes to inherit from base; `stamp_available_at_*` opt-in helpers become
      unnecessary.
- [x] ✅ DEFERRED-OPERATOR-DECISION [SCRIPT] P1. DELETE — `availability_stamping.py` collapses from 330 lines to ~50 lines (per-source rule
      implementations only).
- [x] ✅ DEFERRED-OPERATOR-DECISION [SCRIPT] P1. REDUCE — update workspace SSOT pointers in CLAUDE.md + codex for `available_at` rules to point to UAC
      `AvailabilityRule` as canonical.
...
- [x] ✅ DEFERRED-OPERATOR-DECISION [SCRIPT] P2. `quality-gates.sh` STEP 5.67 — `record_captured` must be preceded by stamping. AST-walk every
      `record_captured(` callsite; assert stamping helper precedes on same code path. Mirror STEP 5.64. Once Phase A
      ships (auto-stamping via validator), this check becomes vacuous — remove.
