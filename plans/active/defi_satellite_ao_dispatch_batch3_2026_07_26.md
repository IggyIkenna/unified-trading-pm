...
- [ ] [DATA] P1. BLOCKED-ON:defi_delta_one_funding_oi_hyperliquid_missing_open_interest_2026_07_31 D1 DeFi features
      ...
      **BLOCKED 2026-07-26 (slot-8) — real bug found + fixed (unblocked the preflight check), but the actual
              ...
              **2026-08-02 (slot-6, data_engineering craft, via `delta_one_lookback_...-002`) — returns leg confirmed
              essentially complete; closed the one real gap; funding_oi still correctly untouched (direction RULED, backend
              fix unlanded).** Checked prod GCS directly instead of relaunching: `features-defi-prd-.../delta_one/by_date/`
              has 1337 real `returns` day-partitions, `2022-11-25..2026-07-23` contiguous (all 3 prior VMs
              `094100`/`110727`/`132937` succeeded). Only gap: `2022-11-01..2022-11-24` (24 days, confirmed real, not
              honest-absence). Launched `features-delta-one-defi-20260802-235804` (`returns`, that window, `TIMEFRAME=15m`)
              to close it — confirmed `RUNNING`. `funding_oi`: OI-availability RULED = B today (see
              `defi_delta_one_funding_oi_hyperliquid_missing_open_interest_2026_07_31.md`) but its `[BACKEND] P2` join isn't
              shipped — left untouched (not my craft). **Not flipping** — done-when needs both legs. Parallel update in
