                                                          ...
                                                          **Onchain leg — real, previously-undiscovered blocker found + fixed + LIVE-CONFIRMED
                                                          ...
                                                          range relaunch unless `--force` is passed).
                                                              ...
                                                              **2026-07-31 (slot-11, data_engineering craft) — returns leg
                                                              ...
                                                              succeeded). Launched the FULL production window for `returns`
                                                              (`--start-date 2022-11-01 --end-date 2026-07-22`, the real
                                                              coverage window per the live manifest cited above,
                                                              `FEATURE_GROUP=returns`). First attempt
                                                              (`features-delta-one-defi-20260731-104418`) was SPOT-preempted
                                                              78s after insert — before writing any run.log, zero real work
                                                              lost, safe to relaunch from scratch per the preemption-recovery
                                                              rule. Relaunched as
                                                              `features-delta-one-defi-20260731-104738` — confirmed producing
                                                              real `Wrote .../Completed N/51` output within the required
                                                              verify-window (see this entry's own progress-check note below
                                                              if present). **At ~2.25 min/real-day observed on the narrower
                                                              verification VM, the full ~1,364-day production window is a
                                                              multi-hour (likely 1+ day) run — not completable within one
                                                              dispatch session.** SPOT + idempotent (`record_captured`
                                                              per-day) + `VM_SHUTDOWN_ON_COMPLETION=true`, so it is safe to
                                                              leave running unattended. **A future dispatch should verify**
                                                              `features-delta-one-defi-20260731-104738` reached
                                                              `DEPLOYMENT_COMPLETED exit_code=0` (VM absence from
                                                              `gcloud compute instances list` + a matching completion entry
                                                              in `gs://deployment-scripts-central-element-323112/deployments/archive/2026-07-31/`)
                                                              before treating the `returns` leg as fully done — if it shows
                                                              FAILED or was preempted again mid-run, a plain relaunch of the
                                                              same command is safe (already-written days won't be
                                                              recomputed absent `--force`). `funding_oi` remains correctly
                                                              untouched — still blocked on
                                                              `defi_delta_one_funding_oi_hyperliquid_missing_open_interest_2026_07_31.md`'s
                                                              `[OPERATOR]` P2 (HYPERLIQUID structurally missing
                                                              `open_interest`), unaffected by any of this session's work. Not
                                                              flipping this checkbox — the done-when needs BOTH legs
                                                              (`funding_oi` + `returns`) over the full window, and neither
                                                              is fully confirmed complete yet.
