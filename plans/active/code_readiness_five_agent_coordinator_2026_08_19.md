...
- **na-eligibility-audit 2026-08-21** (cross-cutting tranche, first audit pass on this doc): KEEP-NA, valid — coordinator for 5 operator-slot-launched tranche plans (explicit 'Launch prompts' mechanism: paste into a chosen `.tabs/N` slot and run `/autonomous` — a human-driven per-slot dispatch, structurally distinct from AO-backlog auto-dispatch, hence deliberately NA not planning). Of 4 open todos: 2 are recurring maintenance asks (re-run allocation script weekly, track artefact marker counts), 1 is explicitly `[OPERATOR]`-tagged (resolve the NA-corpus ratchet breach this effort itself caused), 1 is gated on all 5 tranches reaching zero open todos (currently 2,604 open across the corpus) before archival. No item clears the whole-doc RECLASSIFY bar.
