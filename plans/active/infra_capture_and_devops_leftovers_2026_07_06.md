...
- [ ] [DATA] P2. **NO LONGER OPERATOR-DECISION-GATED, checked 2026-08-02 (slot 2, finalize reconciliation) — retagged;
      ...
      tracked is fully cleared. — **⚠️ POINTER IS NOW DEAD, corrected 2026-08-08 (false-done audit). THIS DOC IS THE
      LIVE TRACKER AGAIN — do not flip on the "sibling owns it" reasoning above.**
      `sports_live_availability_and_source_latency_2026_07_24.md` was ARCHIVED to `/plans/archive/2026_08/` with 0 open
      todos, so the pointer this item defers to no longer tracks anything. Critically, that doc's own `[DATA] P2` was
      flipped `[x]` on the OPERATOR-DECISION half only, while its own text records the execution half as undone: "**Not
      yet confirmed**: the live connector actually resuming production polling ... and the api_football `/odds` in-play
      second source is **still unwired**", and its Net entry states "this todo's 'Done when' bar is not met this session
      — both remaining halves (api_football wiring, confirmed production resume) are genuinely blocked. Checkbox left
      `[ ]`." The backlog row `infra_capture_and_devops_leftovers-001` was `done` (`done_sha=c3c65402e`) against this
      unflipped checkbox; the false-done audit's initial read was FLIP, **overturned on verification** — flipping would
      have buried real work whose only tracker is now archived. Row REOPENED instead. Remaining scope, restated here so
      it is tracked somewhere live: (1) wire api_football `/odds` in-play as the free second source (no connector found
      in market-tick-data-service/instruments-service), (2) confirm `mtds-live-sports-odds-api-trades` has actually
      resumed production polling — a 2026-08-02 live check found ZERO `mtds-live-sports-*` instances running. (repo:
      market-tick-data-service, deployment-service)
- [ ] [INFRA] P1. **RULED 2026-08-06 (operator): AUTHORIZED — proceed with the disposable-IP probe.** The
      reputational/ToS risk-tolerance call this todo was gated on is answered: go ahead. AO-dispatchable now — the probe
      design is already stated as ready in this todo's own text; no further sign-off needed. **rate-limit probe VM.**
      Needs a disposable-IP VM (operator-gated). Gate: probe design ready; awaits the operator's disposable-IP sanction.
