...
2. **Phase 3.2** — per-venue-adapter reconnect→STALE-emission verification (each WS adapter's reconnect path emits a
   STALE marker per the `MDPSStreamingAggregator` Cat B'/C decision). Needs the 3.5 adapters first.
