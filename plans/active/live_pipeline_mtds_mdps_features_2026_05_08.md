...
todos:
  ...
  - id: phase-3-mtds-streaming-rollout
    ...
    note: "2026-05-11 harsh-live-pipeline-impl-tab — 3.1 (runner orchestration + CLI surface) + 3.3 (pool-size config) + 3.4 (InstrumentCacheRefreshConsumer) SHIPPED at mtds@97b2224. STILL OPEN: 3.2 (per-venue-adapter reconnect-STALE verification — needs the adapters from 3.5); 3.5 (per-venue WSFeedConnector implementations: defi→cefi spot/perp→cefi options/futures→tradfi→sports→prediction). **3.5a/b: 5 WSFeedConnectors landed 2026-05-16 (slot-3) — MTDS@c0b2c6e DriftSolanaWSFeedConnector (DRIFT-SOLANA), MTDS@3c8e074 HyperliquidWSFeedConnector (HYPERLIQUID), MTDS@b5c6223 BinanceFuturesWSFeedConnector (BINANCE-FUTURES), MTDS@1d822ba BybitFuturesWSFeedConnector (BYBIT-FUTURES) + OKXFuturesWSFeedConnector (OKX-FUTURES). 50 unit tests pass; basedpyright clean. All 5 verified producing live trades. Side-mapping codified per venue (HL A=sell/B=buy, Binance m=True=sell, Bybit/OKX canonical buy/sell). Pattern reusable for Deribit/Phoenix.** Remaining 3.5: PHOENIX (via Solana RPC) + Deribit perp + cefi options + tradfi + sports + prediction. ShardManifestRecorder ManifestWriter wiring rides with 3.5 completion. NOTE: per ikenna_orchestrator ledger commit 1e01433c the operator reassigned slot-5's Phase 3/5/6/15 to Ikenna slot 7."
