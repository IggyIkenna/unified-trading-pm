...
todos:
  ...
  - id: phase-3-mtds-streaming-rollout
    ...
    note: "2026-05-11 harsh-live-pipeline-impl-tab — 3.1 (runner orchestration + CLI surface) + 3.3 (pool-size config) + 3.4 (InstrumentCacheRefreshConsumer) SHIPPED at mtds@97b2224. STILL OPEN: 3.2 (per-venue-adapter reconnect-STALE verification — needs the adapters from 3.5); 3.5 (per-venue WSFeedConnector implementations: defi→cefi spot/perp→cefi options/futures→tradfi→sports→prediction). **3.5a DeFi: 2 connectors landed 2026-05-16 (slot-3) — MTDS@c0b2c6e DriftSolanaWSFeedConnector (DRIFT-SOLANA via wss://dlob.drift.trade/ws trades), MTDS@3c8e074 HyperliquidWSFeedConnector (HYPERLIQUID via wss://api.hyperliquid.xyz/ws trades, side A=sell B=buy mapping). 21 unit tests pass; basedpyright clean. HL WS verified producing live SOL trades at $86.74. Pattern reusable for PHOENIX + cefi venues.** Remaining 3.5: PHOENIX (via Solana RPC) + cefi spot/perp (Kraken WS scaffold already at execution-service@266d369f1; Binance/Bybit/OKX/Deribit TBD) + cefi options/futures + tradfi + sports + prediction. ShardManifestRecorder ManifestWriter wiring rides with 3.5 completion. NOTE: per ikenna_orchestrator ledger commit 1e01433c the operator reassigned slot-5's Phase 3/5/6/15 to Ikenna slot 7."
