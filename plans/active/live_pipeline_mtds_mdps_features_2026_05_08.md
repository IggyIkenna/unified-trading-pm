...
| Phase 13.4 — Watchdog VM relaunch (operational)                 | `done` (operational 2026-05-11 14:18 UTC) | Tarball refresh via `create-code-tarballs.sh` (CORE — UAC/UTL/MTDS/deployment-service all timestamped 2026-05-11T13:16Z). Old watchdog `vm-zombie-watchdog-20260510-194210` deleted; new `vm-zombie-watchdog-20260511-141810` RUNNING in `asia-northeast1-c`. Picks up the 14 new `VM_PREFIX_TO_BUCKET` entries from Phase 13 (5 mtds-live-{ag} + 5 mdps-features-live-{ag} + features-xc- + replay-). Event-stream verification pending per `No fire-and-forget VM launches` rule. |
