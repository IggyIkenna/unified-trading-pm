...
todos:
  ...
  - id: phase-3-mtds-streaming-rollout
    ...
    note: "2026-05-11 harsh-live-pipeline-impl-tab — 3.1 (runner orchestration + CLI surface) + 3.3 (pool-size config) + 3.4 (InstrumentCacheRefreshConsumer) SHIPPED at mtds@97b2224. STILL OPEN: 3.2 (per-venue-adapter reconnect-STALE verification — needs the adapters from 3.5); 3.5 (per-venue WSFeedConnector implementations: defi→cefi spot/perp→cefi options/futures→tradfi→sports→prediction). **3.5a/b: 12 WSFeedConnectors landed 2026-05-16 (slot-3) — DRIFT-SOLANA, HYPERLIQUID, BINANCE-FUTURES, BYBIT-FUTURES, OKX-FUTURES, DERIBIT, ASTER, BINANCE-SPOT, BYBIT-SPOT, COINBASE-SPOT, OKX-SPOT, KRAKEN-SPOT. Latest: MTDS@5935298. 82 unit tests pass; basedpyright clean. All 12 verified producing live trades. ALL 7 CUTOVER CEFI PERP VENUES + DRIFT + 5 CEFI SPOT VENUES (incl Kraken). Side-mapping codified per venue. Kraken execution-service adapter at execution-service@266d369f1 (separate trade-execution surface from MTDS WSFeedConnector for ingestion).** Remaining 3.5: PHOENIX (via Solana RPC) + KRAKEN-FUTURES MTDS connector + tradfi + sports + prediction. ShardManifestRecorder ManifestWriter wiring rides with 3.5 completion. NOTE: per ikenna_orchestrator ledger commit 1e01433c the operator reassigned slot-5's Phase 3/5/6/15 to Ikenna slot 7."
