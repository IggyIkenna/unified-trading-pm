...
todos:
  ...
  - id: phase-3-mtds-streaming-rollout
    content: |
      - [ ] [AGENT] P0. Phase 3 — MTDS websocket streaming rollout per asset_group. SEQUENTIAL after Phase 1 + 2.
        ...
        clean. **Still open**: 3.2 per-venue-adapter reconnect-STALE verification, 3.5 per-venue WS-adapter wire-in
    ...
    note: "2026-05-11 harsh-live-pipeline-impl-tab — 3.1 (runner orchestration + CLI surface) + 3.3 (pool-size config) + 3.4 (InstrumentCacheRefreshConsumer) SHIPPED at mtds@97b2224 (market_tick_data_service/live/websocket_runner.py + cli/handlers/websocket_streaming_handler.py + cli/main.py operation registration + config/service_config.py + tests/unit/test_websocket_runner.py 21 tests; ruff+basedpyright clean; full QG green except 2 pre-existing failures on HEAD's last 3 commits — test_league_partitioning + test_partitioned_writer_cefi_available_at, both from the `enforce_available_at` / cefi→tradfi available_at stamping commits 48254d2/a512edf/c186ecb, NOT mine). STILL OPEN: 3.2 (per-venue-adapter reconnect-STALE verification — needs the adapters from 3.5); 3.5 (per-venue WSFeedConnector implementations: defi→cefi spot/perp→cefi options/futures→tradfi→sports→prediction — WS_FEED_CONNECTOR_FACTORIES registry is empty, handler raises NotImplementedError on unregistered venue, that's the per-venue rollout's job); ShardManifestRecorder ManifestWriter wiring (per-asset_group v5 shard-atom row keys differ — rides with 3.5; runner currently constructed with manifest_recorder=None — boundary events + GCS tick parquets still written, only manifest rows deferred); per-asset_group smoke launches (Phase 13 launchers + Phase 15 smoke). NOTE: per ikenna_orchestrator ledger commit 1e01433c the operator reassigned slot-5's Phase 3/5/6/15 to Ikenna slot 7 — but operator gave Harsh slot 5 a direct 'continue with phase 3' instruction 2026-05-11; main to reconcile slot-5-vs-slot-7 ownership for the 3.5 rollout. successor: this plan Phase 3.5 + Phase 13 + Phase 15."
  ...
  - id: phase-14-codex-ssot-updates
    ...
    status: helper-shipped
    note: "2026-05-11 ikenna-live-pipeline-tab — Phase 14 items 1 + 4-8 of 8 SHIPPED. Item 1: live-pipeline-architecture.md extended (PM@earlier). Items 4-8 SHIPPED 2026-05-11 (PM@<this commit>): availability-manifest-and-data-status.md (live-pipeline 4-state taxonomy examples + pipeline_mode reference); batch-live-architecture.md (§10 UTC midnight alignment + cascade rule); alerting-batch-live.md (live-pipeline alert tier table + 3 circuit-breaker actions); runtime-tiers-and-deployment.md (live-pipeline VM topology); 00-SSOT-INDEX.md (3 new doc registrations). DEFERRED-AFTER-Phase-5/6/7/10-implementation: items 2 (replay-subsystem.md empirical benchmarks) + 3 (instrument-lifecycle-cache-delta-hot-reload.md per-service callback tables) — both depend on the corresponding implementation phase landing actual benchmarks / wired callbacks per the Post-Plan-Phase Codex Audit HARD RULE."
...
- **features-service@`225cc13b`** — Phase 5 + Phase 6 per-service wire-in. Shared factories
  (`features_service/common/live_runner.py` + `live_cross_cutting.py`) wrapping the UTL@`35425c70` primitives +
  6 asset-scoped + 2 cross-cutting thin family wrappers + 23 unit tests, all green.
- **mtds@`ab17cc3`** — Phase 3.5 ShardManifestRecorder + connector-registry helper. Closes the cross-venue
  wiring blocker (`live/manifest_recorder.py` `MTDSShardManifestRecorder` + `live/connector_registry.py`
  `register_ws_feed_connector()` + 10 unit tests). Per-venue real WS adapter implementations remain operator-
  directed per-venue trading-engineering work (Phase 3.5 sequencing: defi → cefi spot/perp → cefi options/futures
  → tradfi → sports → prediction).
- **PM@`<this commit>`** — plan flips Phase 5 + Phase 6 → `done`; Phase 15 → `helper-shipped` with the 7-day
  live smoke handoff to subsequent slots / operator-directed venue sequencing per Plans-Run-To-Actual-Completion
  HARD RULE; Phase 3 scoreboard entry expanded with the new wiring evidence; LEDGER refresh slot 7 entry.

EOD-audit (per CLAUDE.md "End-of-cycle audit clause"): every deferral cited above lives in this plan body
scoreboard rows above OR is owner-tracked in the named successor plan (per-venue real adapters → operator-directed
next-cycle sequencing; 7-day live smoke → Phase 3.5 rollouts + Phase 13 launchers; watermark-buffer integration →
UTL CrossCuttingFeaturesRunner follow-up).
