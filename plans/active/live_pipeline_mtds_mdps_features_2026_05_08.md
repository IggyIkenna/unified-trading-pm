...
todos:
  ...
  - id: phase-3-mtds-streaming-rollout
    ...
    note: "2026-05-11 harsh-live-pipeline-impl-tab — 3.1 (runner orchestration + CLI surface) + 3.3 (pool-size config) + 3.4 (InstrumentCacheRefreshConsumer) SHIPPED at mtds@97b2224. STILL OPEN: 3.2 (per-venue-adapter reconnect-STALE verification — needs the adapters from 3.5); 3.5 (per-venue WSFeedConnector implementations: defi→cefi spot/perp→cefi options/futures→tradfi→sports→prediction). **3.5a/b: 11 WSFeedConnectors landed 2026-05-16 (slot-3) — DRIFT-SOLANA (MTDS@c0b2c6e), HYPERLIQUID (MTDS@3c8e074), BINANCE-FUTURES (MTDS@b5c6223), BYBIT-FUTURES + OKX-FUTURES (MTDS@1d822ba), DERIBIT (MTDS@46988cd), ASTER (MTDS@be68cd2), BINANCE-SPOT + BYBIT-SPOT + COINBASE-SPOT (MTDS@1c2acb8), OKX-SPOT (MTDS@0165b80). 79 unit tests pass; basedpyright clean. All 11 verified producing live trades. ALL 7 CUTOVER CEFI PERP VENUES + DRIFT + 4 CEFI SPOT VENUES NOW HAVE LIVE WS SUPPORT. Side-mapping codified per venue. Kraken WS scaffold separately at execution-service@266d369f1.** Remaining 3.5: PHOENIX (via Solana RPC) + tradfi + sports + prediction. ShardManifestRecorder ManifestWriter wiring rides with 3.5 completion. NOTE: per ikenna_orchestrator ledger commit 1e01433c the operator reassigned slot-5's Phase 3/5/6/15 to Ikenna slot 7."
