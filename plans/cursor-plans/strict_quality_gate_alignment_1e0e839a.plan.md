...
overview: Reconcile PM and Codex Quality Gate SSOT, establish repo-type-specific templates, harden the audit prompt to cover all cursor rules and quickmerge behavior, and roll out quality gate infrastructure to every repo in the workspace (including scaffolded and incomplete repos).
todos:
  - id: reconcile-ssot
  ...
  - id: rollout-all-repos
    content: Roll out quality-gates.sh and QUALITY_GATE_BYPASS_AUDIT.md to every repo
    status: pending
...
| Updated cursor rules (coverage 70%)    | unified-trading-pm/cursor-rules/quality-gates/*.mdc, testing/*.mdc                      |
