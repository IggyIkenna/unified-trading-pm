...
todos:
  ...
  - id: liquidation-levels
    content: "Add liquidation_levels.py calculator using Coinglass heatmap API: long/short liquidation density at 1/3/5% distance bands, liq_gravity_ratio (directional magnet), next_liq_cluster_distance_pct, oi_leverage_estimate. Check Tardis coverage first"
    status: pending
  - id: level-confluence-score
    content: "Add level_confluence_score meta-feature aggregating: round_number + fib_level + volume_poc + demand_zone + sr_flip + weekly_anchor + liq_cluster signals into single weighted scalar per bar"
    status: pending
