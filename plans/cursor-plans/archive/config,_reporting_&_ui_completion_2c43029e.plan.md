...
**Updated:** 2026-02-27 — Incorporates full answers on auth (Google OAuth replacing Okta), service/UI separation, client data from mr_report, credentials-registry.yaml creation, reportAny=error mandate, client-reporting-api as new standalone repo, ML deployment UI correct scoping.
...
| alerting-service | warning | **error** |
...
**File**: `execution-service/configs/credentials-registry.yaml`
...
| execution-service api/manual_instruction_api.py | ✅ Yes — UI-facing manual trade | Keep |
| position-balance-monitor-service api/main.py | ✅ Yes — UI-facing position data | Keep |
| risk-and-exposure-service api/main.py | ❌ No — internal pre-trade checks, not user-facing | Remove FastAPI, pre-trade checks called as library import in execution-service |
| alerting-service | ✅ Yes — external SSE/webhook delivery | Keep + build out |
...
| ml-training-service | ❌ No — batch CLI only | Correct, add optional api/ for ml-training-ui |
...
Config versioning exists in ConfigStore (`schema-v{N}/config-v{timestamp}`) and GCS (`V{version}/{strategy_base}/`) but there are no UIC canonical Pydantic schemas for any config type. `execution-results-api/api/routes/config.py` has `GenerateConfigRequest` but returns 501 Not Implemented — grid generation wired to execution-service which has no UIC schema either.
    ...
    → BacktestGridConfig.param_grid (look up cell_id)
        → StrategyConfig.signal_params (copy param_grid)
            → ConfigStore.write(strategy_id, StrategyConfig, bump config_version)
                → execution-service reads StrategyConfig on startup/hot-reload
```
...
- Update `execution-service/execution_service/api/manual_instruction_api.py` — consume `StrategyConfig`
...
```python
# client_reporting_api/core/fee_calculator.py
...
### client-reporting-api: NEW standalone repo

**Repo**: `IggyIkenna/client-reporting-api`

Setup follows `new-repo-setup.mdc` exactly:
```bash
gh repo create IggyIkenna/client-reporting-api --private --clone
gh api /repos/IggyIkenna/client-reporting-api/collaborators/CosmicTrader -f permission='push'
gh api /repos/IggyIkenna/client-reporting-api/collaborators/datado -f permission='push'
```
...
ANALYST     → execution-analytics-ui (read), logs-dashboard-ui, batch-audit-ui, ml-training-ui
...
| `execution-analytics-ui` | Shell (Okta stub) | Replace → Google OAuth ANALYST | Build all pages |
| `trading-analytics-ui` | Shell (Okta stub + VITE_SKIP_AUTH) | Replace → Google OAuth TRADER | Build all pages |
| `ml-training-ui` | Shell (Okta conditional) | Replace → Google OAuth ANALYST | Build as ML Analytics UI |
...
/invoices           → ClientInvoice table (client-reporting-api GET /invoices/{id}/pending)
/reports            → MonthlyReport browser (client-reporting-api GET /reports/{id})
...
| alerting-service | `GET /stream/alerts` | AlertEvent (for live-health-monitor-ui) |
...
API gaps in `execution-service/execution_service/api/manual_instruction_api.py`:
...
Backend needed: onboarding-service (or extend an existing service). Given onboarding is ADMIN-only, extend `client-reporting-api` to add `/onboarding/*` routes rather than creating a fourth new service.

New routes in client-reporting-api:
```
...
| Repo | Type | Purpose |
|------|------|---------|
| `client-reporting-api` | Python FastAPI service | Report generation, fee engine, invoice management |
...
8. `client-reporting-api-new-repo`
