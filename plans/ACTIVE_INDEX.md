...
222).** This file's claims no longer hold: `plans/active/` is a real directory on disk (not a symlink), `.cursor/plans`
does not exist, and `plans/active/` holds ~140 plan files, not 4. `plans/active/INDEX.md` is the canonical, currently
maintained index (self-described as such, last updated 2026-07-12) and is the doc `PLAN_FORMAT.md`'s own SSOT references
cite. Treat this file as a historical Day-1–2 relic only.

**Note (was: unqualified, no staleness banner):** `active/` is a symlink to `.cursor/plans`. This index lists the 4
plans currently in progress (2 per person).
