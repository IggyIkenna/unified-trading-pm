...
related: []
created: '2026-04-29'
overview: Single artefact (instrument-catalogue.json + shard-dynamics.json + instrument-catalogue.md) joining static shard-dynamics SSOT (bucket → partition layout → schema → coverage-start → retention/cutoff → live/batch capability) with live availability-manifest aggregation (capture_status counts → coverage %) per (asset_group × data_type × venue × instrument_type) tuple. Published nightly to GCS catalogue prefix; consumed by a new UI matrix widget that cross-links existing data-status drilldown.
type: mixed
