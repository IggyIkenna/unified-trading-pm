...
todos:
  ...
  - id: a3-p0-ml-overview
    content: |
      - [x] [AGENT] P0. Verify `/services/research/ml/overview` has real content. Should show: model registry summary, training status, recent experiments, feature drift. Wire to `GET /ml/models`, `GET /ml/experiments`, `GET /ml/training-status` API endpoints.
    status: done
  ...
  - id: a3-p0-ml-training
    content: |
      - [x] [AGENT] P1. Verify `/services/research/ml/training` has: trigger training button, training queue, active jobs with progress. Wire to `GET /ml/training-jobs` and `POST /ml/training-jobs` APIs. In mock mode, POST should add a job to MockStateStore with status "queued".
    status: done
  - id: a3-p0-ml-features
    content: |
      - [x] [AGENT] P1. Verify `/services/research/ml/features` has: feature list with importance scores, correlation matrix, drift monitoring. Wire to `GET /ml/features` API.
    status: done
  - id: a3-p0-ml-validation
    content: |
      - [x] [AGENT] P1. Verify `/services/research/ml/validation` has: out-of-sample test results, signal quality metrics, walk-forward analysis. Wire to `GET /ml/validation-results` API.
    status: done
  - id: a3-p0-ml-registry
    content: |
      - [x] [AGENT] P1. Verify `/services/research/ml/registry` has: model version list with promote/deprecate actions, model comparison, A/B test setup. Wire to `GET /ml/models` and `POST /ml/models/{id}/promote` APIs.
    status: done
  - id: a3-p0-ml-remaining
    content: |
      - [x] [AGENT] P2. Verify remaining ML sub-tabs have content: `/services/research/ml/monitoring` (live model performance), `/services/research/ml/deploy` (deployment status), `/services/research/ml/governance` (model governance, approval workflows), `/services/research/ml/config` (ML pipeline configuration).
    status: done
  - id: a3-p1-strategy-backtests
    content: |
      - [x] [AGENT] P0. Verify `/services/research/strategy/backtests` has: backtest runs table with status, sharpe, drawdown, total return, trades count. Wire to `GET /execution/backtests` API. Add "New Backtest" button that opens a configuration modal/drawer.
    status: done
  - id: a3-p1-strategy-compare
    content: |
      - [x] [AGENT] P1. Verify `/services/research/strategy/compare` has: side-by-side comparison of 2+ backtests, equity curves overlay, risk metrics comparison. Wire to API.
    status: done
  - id: a3-p1-strategy-results
    content: |
      - [x] [AGENT] P1. Verify `/services/research/strategy/results` has: detailed backtest results with trade log, equity curve, drawdown chart, monthly returns heatmap.
    status: done
  - id: a3-p1-strategy-heatmap
    content: |
      - [x] [AGENT] P1. Verify `/services/research/strategy/heatmap` has: parameter sweep heatmap showing sharpe/return across parameter combinations.
    status: done
  - id: a3-p2-absorb-strategy-wizard
    content: |
      - [x] [AGENT] P1. Extract the multi-step strategy creation wizard from `strategy-ui/src/components/wizard/` (WizardShell, BasicConfigStep, StrategySelectionStep, InstitutionalShareClassStep, ReviewStep) and adapt it as a modal/drawer in the main UI. Trigger from "New Strategy" button on the backtests page. Adapt to use unified-trading-api endpoints and the main UI's component library (shadcn/ui, not whatever strategy-ui uses). Key features to preserve: CSV parameter upload (via papaparse), multi-step flow with validation.
    status: done
  - id: a3-p3-absorb-ml-training
    content: |
      - [x] [AGENT] P1. Review `ml-training-ui/src/` for any experiment tracking UI patterns not already in the main UI's ML sub-tabs. Key things to look for: training curve visualization (loss/metrics over epochs), hyperparameter display, model artifact download links, experiment comparison views. Absorb any missing patterns into the corresponding ML sub-tab pages.
    status: done
  ...
  - id: a3-p7-export
    content: |
      - [x] [AGENT] P1. Add split "Export" button (CSV + Excel) to: ML Experiments, Backtest Results, Feature list tables. Use `exportTableToCsv()` and `exportTableToXlsx()` from `lib/utils/export.ts` (created by Agent 2). Button: `[Export ▾]` → "CSV" / "Excel".
  ...
  - id: a3-p7-dynamic-imports
    content: |
      - [x] [AGENT] P1. Use Next.js `dynamic(() => import(...), { ssr: false })` for heavy chart components in research pages: equity curve charts, training loss curves, parameter sweep heatmaps, feature correlation matrices. These use browser-only APIs and should not be server-rendered.
    status: done
  - id: a3-p7-adopt-datatable
    content: |
      - [x] [AGENT] P0. Replace shadcn `<Table>` with `DataTable` from `components/ui/data-table.tsx` (Agent 1) for ALL research tables: ML Experiments, Backtest Results, Feature list, Strategy Candidates. Tables must handle 50+ strategies from expanded seed data.
