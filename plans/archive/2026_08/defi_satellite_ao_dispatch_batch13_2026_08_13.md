...
- [x] [CODE] P2. ✅ Cross-check instrument_type=spot_pair (9,802 rows) against defi-canonical-naming-ssot.md's locked
      ...
      EIGENLAYER's 3,816 `spot_pair` cells were NOT explained by its registry declaration (`_RESTAKING` = `spot_asset`
      only) — filed as
      `/plans/archive/2026_08/issues/defi_eigenlayer_spot_pair_unexplained_expected_cells_2026_08_14.md`, now
      root-caused (stale pre-taxonomy-fix expected-universe seed) and purged (instruments-service@552c576857). All 3
      venues' cells are 0% captured, which is also why the original "9,802 rows" figure (dated 2026-08-07) no longer
      matches any live rollup — the underlying population has moved substantially across the many DeFi canonicalization
      fixes that landed 2026-08-08..08-13; not independently reproduced, not material to the drift-vs-legitimate
      verdict.
...
- [x] [CODE] P2. ✅ PARTIAL — precondition verified + relaunch attempted, VM infra failures blocked completion. Verified
      live the DeFi manifest consolidator has genuinely caught up (real `read_availability_index` for
      `(defi, dex_swaps, captured)` WITHOUT `--allow-stale-fallback` succeeded: 3,454,808 rows/9 venues, matching the
      source doc's ~3.46M-row expectation). Attempted the relaunch twice: attempt 1
      (`backfill-defi-legacy-datatype-fold-20260815-001819`) never wrote a heartbeat/run.log and was zombie-reaped;
      attempt 2 (`...-004142`) started cleanly but the fold script was OOM-killed (rc=137, mem_pct=90.9%) under the
      launcher's oversubscribed `--workers 24` (2x the script's own tested default of 12) — root-caused + fixed,
      `deployment-service@7480588f57`. Per RB-INFRA-RELAUNCH's ≤2/(vm-prefix,day) bound, did not attempt a 3rd launch
      this session. Full evidence + a follow-up relaunch todo (now unblocked by the worker-count fix) filed at
      `plans/active/issues/defi_legacy_fold_relaunch_vm_infra_flakiness_and_oom_2026_08_15.md`. unified-trading-pm (this
      commit). Source: `plans/active/issues/defi_manifest_allow_stale_fallback_incomplete_for_long_pause_2026_08_07.md`
