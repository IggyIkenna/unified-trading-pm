...
resolved_by:
  "interactive session, ao_fleet_cache_tokens_and_task_count_2026_08_05 todo 1, agent-orchestrator@956e675, 2026-08-05"
locked_by:
...
> **🟢 ARCHIVED 2026-08-05 — RESOLVED.** `/done` failed for every worker fleet-wide from 2026-08-05T11:02:03Z to
> 12:59:55Z (~2h); fixed live same-session (`ALTER TABLE` via SSM) + hardened (`_record_done_task_usage()` isolated into
> its own `session_scope()`, `agent-orchestrator@7a7dd8d`). The systemic gap (the live fix was never actually checked
> into code, so any OTHER environment where `task_usage` predates `backfilled` still had the bug) is now also closed:
> `ao_fleet_cache_tokens_and_task_count_2026_08_05.md` added the retroactive `_TASK_USAGE_MIGRATION_COLUMNS` entry plus
> a static completeness test (`tests/test_migration_completeness.py`) that fails CI if a future column ships without a
> matching entry — `agent-orchestrator@956e675`. Codex-alignment: the underlying `_add_missing_columns()` convention is
> now documented at `/codex/04-architecture/agent-orchestrator-overview.md` § "Schema changes to an EXISTING table". All
> 4 todos done, nothing deferred. Moved to `/plans/archive/2026_08/task_usage_schema_drift_done_outage_2026_08_05.md`;
...
- [x] ✅ [INFRA] P1. **Guard `_record_done_task_usage()` so a usage-write failure can never roll back the rest of
      ...
      within 8s).
- [x] ✅ [INFRA] P2. **Systemic gap, not just this one field**: this codebase has no schema-migration mechanism beyond
      ...
      documented and cross-referenced from wherever ORM changes are made. — Resolved differently than any of (a)/(b)/(c)
      literally, but closes the same practical risk:
      `/plans/archive/2026_08/ao_fleet_cache_tokens_and_task_count_2026_08_05.md` found `_add_missing_columns()` +
      per-table dicts (`_SLOTS_MIGRATION_COLUMNS` etc.) already implement (a)'s pattern, just hand-maintained rather
      than auto-diffed against the ORM model — `backfilled` was simply never added to a dict entry, the exact
      forgettable step. Fix: `agent-orchestrator@956e675` adds the retroactive `_TASK_USAGE_MIGRATION_COLUMNS` entry
      PLUS a new static test
      (`tests/test_migration_completeness.py::     test_every_task_usage_row_column_is_baseline_or_migrated`) that fails
      CI the moment any FUTURE `TaskUsageRow` column ships without a matching dict entry — a de facto option (d): keep
      the existing hand-maintained mechanism, but make "forgot to update the dict" a caught-at-CI defect instead of a
      silent latent production bug. A true automatic PRAGMA-diff migrator (the literal (a)) was considered and
      explicitly deferred by the operator (2026-08-05) as more novel/riskier code touching the live startup path, for no
      incremental safety benefit here.
