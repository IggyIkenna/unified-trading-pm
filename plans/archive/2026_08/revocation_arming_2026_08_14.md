...
> **📦 ARCHIVED 2026-08-16.** The mechanism is fully armed and has fired live in production since 2026-08-15
...
### 2026-08-16 (later) — remaining follow-ups extracted for AO dispatch, scoped `/na-eligibility-audit` run

The 3 remaining open items (p95 measurement, `consolidator_bucket_resolver` production wiring, `release()`
scheduler-resume) were reassessed against the AO-eligibility bounded/deterministic-outcome bar — none needed genuine
operator judgment, they'd just defaulted to `NA` along with the rest of this plan. Conflict-checked against the full
active corpus first (all 4 surfaces per `ao-dispatch-batch-naming-and-conflict-check.md` § 3): found and cross-cited
two genuinely related but non-duplicate issue docs
(`dp_revocation_release_never_resolves_identity_2026_08_15.md` — a DIFFERENT release-path bug, already fixed, that
makes this plan's `release()` finding newly-observable rather than a duplicate of it;
`dp_exit_code_monitor_sweep_times_out_every_run_2026_08_14.md` — the still-open root cause `DP-VM-013` gives a
...
remaining non-superseded open todos; it stays `active` (not archived) until the batch + finalize close it out.
