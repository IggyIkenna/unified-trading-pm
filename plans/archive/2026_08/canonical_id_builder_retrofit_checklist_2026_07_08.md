...
> repo-blocker RB-04f4f852 (escalation agt-3dc7e9), 2026-08-06. No content was rewritten. **What already shipped
> (2026-07-08, this session)** — read before starting any todo below, it's the infrastructure every todo here builds on:
>
...
- [x] [DATA] P2. **Fix the real `/`-delimiter bug in Betfair's sports adapter** — `betfair.py:279`
      ...
      `archive/2026_08/issues/plan_reconciliation_operator_decisions_2026_07_11.md` §A2): `/` is Betfair's documented
      native id convention — canonical ids for Betfair KEEP the `/` delimiter; downstream consumers must treat it as
      venue-native, not normalise. Zero production Betfair rows exist today (dormant path); if Betfair activates,
      builders adopt this convention as-is.
...
- [x] ✅ [DATA] P3. **Cross-reference, don't duplicate, the TradFi combo-leg fix** — DONE 2026-08-05 (slot-7,
      `data_engineering`), `unified-trading-pm@8e39617f9`. The TradFi combo-leg plan
      ...
      links back to this checklist plan (`unified-trading-pm@8e39617f9`). No code changes needed — the `build_leg()`
