...
related:
  [
    /plans/archive/2026_08/ao_satellite_ao_dispatch_batch17_2026_08_10.md,
...
> **Machine-gated on `/plans/archive/2026_08/ao_satellite_ao_dispatch_batch17_2026_08_10.md`** (`depends_on` +
...
- [x] ✅ [INFRA] P0. **DONE 2026-08-16 (plan_reconciler, ao tranche)** — ran the 6-step archival ritual on
      `ao_satellite_ao_dispatch_batch17_2026_08_10.md`: sole todo verified `[x]` with hard evidence, no Deferred
      content, banner + `status: resolved` added, `git mv`'d to `plans/archive/2026_08/`. Corpus-wide referrers fixed:
      `plans/active/INDEX.md` + `plans/epics/agent_operating_framework_master.md` (regenerated via
      `populate_epic_bodies_2026_05_21.py --apply` + `regenerate_active_plan_inventory.py`). This finalize plan's own
      `related:`/`depends_on:` repointed in the same commit this doc itself archives in.
...
- **2026-08-16 (plan_reconciler, ao tranche)**: todo 4 done — ran the archival ritual on batch17. This finalize doc
