...
status: resolved # (was: active) 2026-08-05 -- all 19 todos done, doc archived per the 6-step ritual
nature: process
asset_group: [defi]
stage: [data]
...
locked_since:
supersedes:
superseded_by: mvp_backfill_defi_onchain_v10_2026_06_27_finalize_2026_07_27
...
> **✅ ARCHIVED 2026-08-05 — all 19 todos done, gated closeout complete.** Reconciled via
> `mvp_backfill_defi_onchain_v10_2026_06_27_finalize_2026_07_27.md` (slot-2 review, 2026-08-05). Two cited SHAs
> (`instruments-service@0fe364ff`, `unified-trading-library@d6c8f2b3`) exist only on wip-preserve refs, not LDR — both
> have documented explanations in this plan's own narrative (OOM'd reconciler, diverged slot). No residual work remains;
> all 6 MVP data_types backfilled. Codex-alignment check: no new contracts established — this plan was an execution
> vehicle, not a rule-setter. No CLAUDE.md/codex pointer updates needed.
>
...
- [x] ✅ [SCRIPT] P1. **GATE MET 2026-07-28 (slot-10) — re-measured 0 residual, checkbox flipped.** Direct scoped
      ...
      read with post-hoc pandas filtering — both agree) against the SAME
      `gs://market-data-tick-defi-prd-central-element-323112/_index/availability_index.parquet`: **captured=11,440,
      empty_confirmed=442, attempted_failed=0** (was captured=11,277, empty_confirmed=442, attempted_failed=831 per
      slot-7's 2026-07-28 measurement below). Both the 830 aiodns ghost rows and the 1 unrelated
      `PYTH_HERMES_HISTORICAL_HTTP_520` residual are gone. Full trail (2 failed VM attempts of my own hitting a genuine
      ...
      re-run never touched, because they were recorded at day-granularity while the fixed writer succeeds at
      per-instrument granularity — every one of those dates already has 14 legitimately `captured` per-instrument rows
...
- [x] ✅ [DATA] P1. **DONE 2026-07-27 (slot-2).** Investigated `dex_pool_swaps` subgraph GraphQL-error/schema-drift
      failures (733 rows, live through 2026-07-26) for uniswap_v3/OPTIMISM (316), curve/OPTIMISM (312),
      trader_joe_v2/AVALANCHE (73), pancakeswap_v3/BSC (13) + smaller counts for aerodrome_v3/BASE, uniswap_v4/ETHEREUM,
      uniswap_v3/POLYGON, pancakeswap_v3/ETHEREUM, velodrome_v2/OPTIMISM — live-probed every named subgraph deployment
      directly against the TheGraph gateway. Three distinct root causes, full evidence + follow-ups in
      `issues/defi_dex_pool_swaps_733_row_indexer_health_findings_2026_07_27.md`: (1) curve/OPTIMISM is the
      already-root-caused permanently-dead "no allocations" subgraph
      (`issues/defi_curve_optimism_subgraph_no_allocations_2026_07_15.md`) — its runtime-detection fix landed
      concurrently this session via slot-11, `market-tick-data-service@dddd1b21`; (2) uniswap_v3/OPTIMISM +
      pancakeswap_v3/BSC return a reproducible (3/3) gateway "bad indexers" error — a NEW, distinct-from-deindexed
      finding, follow-up filed; (3) trader_joe_v2/AVALANCHE + the 5 smaller-count pairs are all currently live-healthy
      (real swap data returned) — retry-fixable, no code bug, backfill-relaunch follow-up filed. Repo:
      market-tick-data-service.
- [x] ✅ [DATA] P2. **DONE 2026-08-04 (slot-10, data_engineering); batch-6 verified 2026-08-05 (slot-7,
      data_engineering, batch-6 todo 22)** — root cause already fixed + verified by an earlier independent dispatch;
      this investigation confirms it and closes the loose thread.** The guard was **correctly** blocking — the bug was
      in the CALLER, not the guard. Fresh scoped manifest reads
      (`gs://market-data-tick-defi-prd-central-element-323112/_index/availability_index.parquet`, column-pruned,
      `run-bounded-analysis.sh`-wrapped, no full-manifest materialization) show the 6 rows this todo names (dated
      2026-07-26) are the first instances of a pattern that grew to **198** identical `attempted_failed` rows
      (MORPHO×66, COMPOUND_V3×132; venues=MORPHO{ETHEREUM,BASE}, COMPOUND_V3{ETHEREUM,ARBITRUM,BASE,OPTIMISM}) between
      2026-07-27 and 2026-07-29T21:05:57Z — all sharing the identical `error_reason`
      (`record_empty(reason=SOURCE_RETURNED_ZERO) requires FetchEvidence proving a clean ...`). **Root cause** (already
      diagnosed + fixed in
      `plans/archive/issues/defi_compound_v3_lending_indices_zero_capture_regression_2026_07_29.md`, resolved
      2026-07-30): the 2026-07-26 catalogue-residual wiring (`market-tick-data-service@eae703b0`) diffed two DIFFERENT
      id spaces — `market_count_map()` keys captured markets by raw on-chain ADDRESS, but the IS catalogue's
      `instrument_id` for `instrument_type="lending"` is the canonical `VENUE-CHAIN:TYPE:SYMBOL` glued form — so
      `record_catalogue_residual_empty_typed()`'s residual set always evaluated to the FULL catalogue and wrongly tried
      to claim `SOURCE_RETURNED_ZERO` even on days with real captured data; the honest-absence gate correctly rejected
      the unsubstantiated claim (working as designed), routing the shard through `record_shard_failure` despite the
      write having already succeeded. **Fix**: `market-tick-data-service@d36e2498` removed the structurally-invalid
      `record_catalogue_residual()` call from `lending_indices_handler.py` (confirmed absent at current HEAD, live-read
      2026-08-04 — `record_market_captures()` alone reconciles the address-keyed EU cells, see the docstring note at
      `lending_indices_handler.py:811-825`); landed 2026-07-29T21:35:01Z, **after** the last observed guard-rejection
      (21:05:57Z) — confirmed **zero** new occurrences since (verified against today's manifest, 2026-08-04). Also fixed
      the identical bug in `risk_params_handler.py` (`market-tick-data-service@674fdd6e`); `lst_rates_handler.py` /
      `evm_defi_collectors.py` confirmed NOT exposed (different, already-matching id-space) — all per that doc's own
      Progress Log. **New finding this session** (not covered by the fix above — filed as a follow-up P3 todo below):
      all 198 guard-rejected cells now ALSO have a `captured` row for the same `(venue, chain, date)` (a later, post-fix
      re-attempt succeeded) — i.e. these are stale phantom `attempted_failed` residue, same class as
      `plans/archive/issues/pyth_oracle_prices_stale_ghost_failure_rows_2026_07_28.md`'s PYTH ghost rows, just not yet
      covered by that doc's PYTH-scoped reconciler flag. Repo: `market-tick-data-service` (fix already shipped, no new
      code this dispatch).
- [x] ✅ [SCRIPT] P3. Clean up the 198 residual phantom `attempted_failed` `lending_indices` rows for MORPHO/COMPOUND_V3
      (found 2026-08-04, see the P2 finding above) — each of these cells already has a superseding `captured` row for
      the same `(venue, chain, date)`, so this is manifest-hygiene debt, not a live data gap, but it inflates the G2
      gate's `attempted_failed` count. Mirror the `--report-pyth-oracle-prices-ghost-failures` precedent
      (`instruments-service@0fe364ff`, `reconcile_phantom_manifest_rows_all.py`) — add an analogous lending_indices-
      scoped mask (stale `attempted_failed` row superseded by a same-cell `captured` row), dry-run first, `--apply` on a
      dedicated VM (the PYTH precedent's in-session full-manifest read OOM'd at 64GB — do NOT attempt this on the shared
      host; only 198 target rows out of ~27M total, so a properly scoped/filtered read should stay small, but the
      reconciler's existing merge path reads the whole manifest regardless of target-row count per
      `issues/reconcile_phantom_manifest_rows_all_defi_memory_footprint_2026_07_28.md`). Repo: `instruments-service`.
      **Shipped:** `instruments-service@9c24f27b` — added `--report-lending-indices-ghost-failures` flag with
      `_lending_indices_ghost_failure_mask` predicate (MORPHO/COMPOUND_V3, same-cell superseded), dry-run report, and
      `--apply` delete path (staleness-guarded, captured-invariant-checked). Dry-run + `--apply` execution should be
      done on a dedicated VM per the memory-bounding rules (the reconciler's full-manifest read path is unchanged).
- [x] ✅ [DATA] P2. Investigate `dex_pool_state`'s 19 `build_instrument_id` errors — identified 33 errors (grown
      from 19) across 4 venue+chain pairs: SUSHISWAP/ARBITRUM (10), TRADER_JOE_V2/AVALANCHE (5), UNISWAP_V4/ETHEREUM
      (17), ORCA/SOLANA (1). Root cause: pool symbols containing `:` trigger `build_instrument_id`'s colon-delimiter
      hard-check (UAC `canonical_id_builder.py:851`). **Batch-6 verified 2026-08-05 (slot-7, data_engineering, batch-6
      todo 22).** Filed `issues/dex_pool_state_build_instrument_id_colon_in_symbol_2026_08_04.md` with per-row isolation
      fix recommendation. — unified-trading-pm@&lt;SHA&gt;
- [x] ✅ [SCRIPT] P3. Retry the 2 `lst_rates` `429`-rate-limited cells (trivial). Repo: `market-tick-data-service` —
      market-tick-data-service@33440748 (2026-08-05, slot-13). **Batch-6 todo 23.** Both cells (LIDO/ETHEREUM
      2022-09-05, 2022-11-22) retried and verified captured. One-off retry script at
      scripts/retry_lst_rates_429_cells_2026_08_05.py.
- [x] ✅ [SCRIPT] P3. **Wire up the disabled `shard_exists_prefix` skip-if-captured hook across DeFi handlers** (found
      2026-07-27, slot-10, while scoping the PYTH oracle_prices re-run above). **Batch-6 todo 23.** Every DeFi handler
      ...
      `unified-trading-library`, `market-tick-data-service`. ✅ [SCRIPT] P3 — utl@d6c8f2b3 + mtds@b55cf9ad. Implemented
      as a framework-level `pre_process_skip(payload) → bool` hook on `UnifiedServiceHandler` (not a
      `shard_exists_prefix` re-enable); adapters call it before acquiring the semaphore. `OraclePricesHandler` wires a
      `ManifestFreshnessCache` skip for all Chainlink+Pyth shards. Adjacent fixes: aiohttp 3.14.1→3.14.3
      (CVE-2026-59881/-69243/-69244); prek darwin markers aligned with UAC (2026-08-03).
...
> keep this plan under the line-cap. **Note (2026-07-27, slot-10):** the named operational-log target file has since
> been archived + split into 6 parts
> (`plans/archive/mvp_backfill_defi_onchain_v10_operational_log_part{2..6}_2026_07_24.md`); those are frozen archive
> artifacts, not a live append target, so this one entry lands here instead (still well under the line-cap) — if this
> recurs, retarget future entries at whichever doc actually stays open.

**2026-07-27 (slot-10) — G2 follow-up todo in progress: PYTH `oracle_prices` re-run launched, NOT yet complete.** Per
the P1 todo ("Re-run PYTH `oracle_prices` for the 2023-10-01→2026-07-22 date range now that
`market-tick-data-service@533514c2` is shipped"): confirmed the aiodns-fallback fix is present at HEAD
(`git merge-base --is-ancestor 533514c2 HEAD`), confirmed via a fresh Explore-agent read of
`oracle_prices_handler.py`/`_adapter.py` that this handler has no skip-if-captured logic (`shard_exists_prefix` is a
disabled dead stub, matching every other DeFi handler) — so `--force`/`--force-window` are no-ops here and a re-run
unconditionally re-fetches + re-writes the whole range (safe/idempotent, just not incremental) — and launched
`bash scripts/vm/launch-mtds-pyth-archive-backfill-vm.sh 2023-10-01 2026-07-22` (deployment-service). No conflicting
`mtds-pyth-archive-*` VM was running (singleton check clear). **VM: `mtds-pyth-archive-20260727-144533`**, SPOT,
`asia-northeast1-c`, launched 2026-07-27T14:45Z. Tarball freshness: `market-tick-data-service` fresh (the repo carrying
the actual fix); `unified-trading-library`/`deployment-service` tarballs were stale at launch (warned, not blocking —
neither carries the aiodns fix, so not launch-blocking for this specific re-run).

Verified genuinely progressing (not fire-and-forget): T+5min 1,371 log lines; T+~1h45m 20,104 lines, date cursor at
~2024-03-22 of the 2023-10-01→2026-07-22 range (~172/1,025 days), real Chainlink + Pyth rows landing in GCS
(`oracle_prices` parquet writes confirmed in `run.log`). Extrapolated full-range ETA ~9-10h from launch (~2026-07-27
23:30Z-00:30Z). **Checkbox intentionally NOT flipped** — re-run is in flight, not complete; flipping now would be the
same smoke-test-green false-completion this plan's G2 gate already burned time on once. A background poller
(tool-tracked, checks VM status + `run.log` line-count growth every 5min, fires on terminal VM status or a 15min flat
stall) was armed to catch completion/stall without babysitting.

**For whoever picks this up next** (resumed session, or AO re-dispatch of this same todo): do NOT re-launch — check
`gcloud compute instances describe mtds-pyth-archive-20260727-144533 --zone=asia-northeast1-c --format='value(status)'`
first (singleton lock in the launcher would refuse a duplicate anyway, but check before assuming stale). If TERMINATED
and shut down cleanly, verify via `measure_honest_coverage.py --asset-group defi` (or a direct manifest read scoped to
`data_type=oracle_prices, venue=PYTH`) that the 1,026 legacy `attempted_failed` rows converted to
`captured`/`empty_confirmed`, THEN flip this checkbox citing the measured before/after counts — not just "VM completed".
If still RUNNING, this is expected for hours; resume the same poll pattern rather than re-launching.

**2026-07-28 (slot-7) — resumed-session monitoring checkpoint, still in flight, no action taken.** Picked up this
already-in-progress todo (dispatcher `dispatch_reason: resume`); did NOT re-launch. Verified
`mtds-pyth-archive-20260727-144533` still `RUNNING` and `run.log` still growing steadily (metadata-only
`gcloud storage objects describe` checks — cheap, no content download) across the full session: ~4.87MB at 17:05Z
climbing to ~22.8MB by 00:22Z, i.e. still actively writing oracle_prices rows ~9h37m after the 14:45Z launch, consistent
with the ~9-10h ETA already recorded above. Two prior background watchdog processes (scratchpad-only, ephemeral, never
promoted — same pattern as this file already documents) were reported killed with exit 143 mid-session but both later
proved still alive on direct `ps` inspection; treat a background-watchdog "killed" notification as unconfirmed until
cross-checked directly against the VM/log, not as proof of death. **Checkbox still correctly NOT flipped** — no terminal
state observed yet. **For whoever resumes next**: same instructions as the paragraph above still apply verbatim (check
VM status first, never re-launch, verify manifest counts before flipping). No code changes, no commits this session —
pure monitoring continuation.

**2026-07-28 (slot-7) — VM completed, gate verification done, checkbox correctly NOT flipped, follow-up filed.**
`mtds-pyth-archive-20260727-144533` finished at 2026-07-28T00:25Z (`exit_code=0`, `DEPLOYMENT_COMPLETED`, self-deleted
per `VM_SHUTDOWN_ON_COMPLETION=true` — explains why a subsequent `gcloud compute instances describe` returned "not
found" rather than `TERMINATED`; do not treat "not found" as an error when the launcher's own self-shutdown behavior is
the explanation, confirm via the run.log tail first). Ran the plan's own prescribed gate verification:
`measure_honest_coverage.py --asset-group defi` plus a direct scoped manifest read
(`venue=PYTH, data_type=oracle_prices`, 5 columns, filtered — not a whole-corpus walk). Result: captured=11,277,
empty_confirmed=442, **attempted_failed=831** — NOT the expected 0. Diagnosed root cause (see the todo annotation
above + the new issue doc): 830 of 831 are stale day-level ghost failure rows from before the fix, never touched by this
...
stated gate regardless of root cause. The re-run itself is genuinely complete and the live data gap is genuinely closed;
what remains is a distinct, newly-scoped manifest-cleanup task now tracked separately. No code changes this session
(diagnostic + doc work only); PM plan + new issue doc committed and pushed.

**2026-07-28 (slot-10) — checkbox flipped, gate MET, PLUS a genuine infra-capacity finding.** Picked up the P1 todo
above. Found the reconciler extension already shipped (`instruments-service@0fe364ff`, slot-16) but not yet run to
completion by anyone confirmed. My own two attempts to run `--report-pyth-oracle-prices-ghost-failures` both failed on
...
bump default VM/Cloud-Run provisioning vs. add a column-pruned read path, or both). Flipped this checkbox + all 3 todos
in the PYTH ghost-failure issue doc, citing the measured before (831)/after (0) counts per this plan's own stated gate.
No code changes to instruments-service/market-tick-data-service this session; shipped only the deployment-service
launcher extensions (already-referenced SHAs above).

- **context-scout 2026-08-01**: populated/refreshed context_scope (4 entries).
- **context-scout 2026-08-03**: refreshed context_scope (5 entries) -- added the UTL skip-if-captured adapter loop + the
  DeFi handler file the open P2/P3 investigate/wire-up todos target.
