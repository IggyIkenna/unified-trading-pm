...
- [x] ✅ [DATA] P0. **UNBLOCKED 2026-08-09** — S&P options are explicit in-scope work per the MVP-of-MVP ruling
      ...
      **`underlying in (SP500, ES)`** — **CORRECTED 2026-08-09 (see 2026-08-09T~08:38Z Progress Log entry below)**: the
      ...
      `instruments_tradfi_g1_g5_gate_execution_2026_07_24.md`. — **deployment-service@6b1057cc/be6d4669/77a95833 +
      MTDS@29b4310e**. 5 VM launches (2 SPOT-preempted, switched to standard). Manifest (2026-08-10T13:13Z,
      `underlying in (SP500,ES)`): 2025 284/284 (100% ≥ 90% ✅), 2026 149/286 (52.1% < 95% ⚠️). 2025 gap CLOSED. P1
      chain='' bug found+fixed (`MTDS@29b4310e`,
      `/plans/active/issues/     tradfi_es_opt_manifest_chain_field_empty_blocking_coverage_2026_08_10.md`). 2026 gap
      tracked in issue doc.

- [x] ✅ [DATA] P3. **Investigate the 2 anomalous Sundays in the CME instrument-definitions manifest.** `2024-06-02` and
      ...
      (Deferred work after 2026-07-26, row 2). — **Root cause identified (2026-08-10, instruments-service code audit)**.
      `sessions.py:_resolve_trading_status()` treats CME Sundays as trading days
      (`is_trading = venue in {"CME","ICE"}     and not is_holiday` → True for non-holiday Sundays), so the orchestrator
      expects URDI to return records. For 363 weekends, URDI returns non-zero records (CME Sunday-evening session
      instruments exist at Databento). For these 2 specific Sundays, Databento returned zero instrument definitions — a
      vendor-side data gap, not a code defect. The `process_zero_records._zero_records_non_sports()` TradFi
      non-trading-day path correctly stamps `empty_confirmed` when `is_non_trading_day()` returns True, but this path is
      unreachable for non-holiday CME Sundays because `_resolve_trading_status()` says "trading day." Fix (future): add
      a resilience path in `process_zero_records.py` for "trading day per calendar but URDI returned zero" — stamp
      `empty_confirmed/EXPECTED_VENDOR_GAP` instead of raising RuntimeError. Not fixed here — P3, bounded scope,
      cosmetic impact (2 out of ~2,368 dates).
...
### 2026-08-10T11:46Z — 2025 self-deleted T+13 min, 2026 still running (standard, T+18 min+)

**Status: IN FLIGHT — todo #2 still `[ ]`.** 2025 VM (`tradfi-bf-es-opt-light-2025-20260810-113247`) self-deleted at
04:46:01 PDT (T+13m11s, `operation-1786362224647`, `delete` type). Serial console inaccessible post-deletion — root
cause unconfirmed (either successful early completion or pipeline error; will determine from manifest query after both
VMs finish). 2026 VM (`tradfi-bf-es-opt-light-2026-20260810-113302`) still RUNNING at T+18 min, past both the SPOT
preemption window (<5 min this zone) and the zombie-watchdog risk window (15-23 min). Pipeline launched correctly at
11:36Z (`mtds_chunk_loop.sh`, python pid 4914, `VM_MODE=full`, PIPELINE_HEARTBEAT active). Standard provisioning immune
to preemption — will run to completion. Monitor `bxax5rkte` polling every 5 min until terminal state. Year-shard OHLCV
backfill takes 30-90 min per launcher docs.

- **NEXT ACTION**: Wait for 2026 VM to self-delete (monitor `bxax5rkte`). Once both VMs terminated: download fresh
  manifest, run corrected query (`underlying in (SP500, ES)`), check 2025≥90% + 2026≥95% targets, flip todo #2 + update
  closeout plan MVP-cell row, commit+push, `/done`.

### 2026-08-10T11:46-12:17Z — 2025 confirmed complete, 2026 actively downloading

**2025 VM success confirmed** (manifest query 12:00Z): 284/284 rows captured (100%), 0 unattempted. The self-deletion at
T+13 min was a clean completion — the VM wrote the full 2025 year in ~10 min of pipeline runtime.

**2026 VM actively processing** (SSH confirmed 12:12Z):
`python -m market_tick_data_service --operation download --start-date 2026-01-15 --end-date 2026-01-21` (first of ~29
weekly chunks), 60% CPU, 4GB RSS. Pipeline spent ~31 min in chunk discovery before starting first download at 12:07Z.
Databento options chain historical data is slow (11 instrument IDs × 7-day chunk). Completed 2026 manifest should show
~260+ captured rows (currently 149/286 at 52.1%). Manifest consolidation lags behind real-time writes — the manifest may
not reflect the latest data until the consolidator runs after the pipeline finishes.

**Coverage summary** (manifest snapshot 12:17Z):

| Year      | Captured/Total | Pct   | Status                      |
| --------- | -------------- | ----- | --------------------------- |
| 2020-2024 | All 100%       | —     | Complete                    |
| 2025      | 284/284        | 100%  | ✅ Target met (≥90%)        |
| 2026      | 149/286        | 52.1% | 🔄 VM writing, manifest lag |

Monitor `b7vmtgqy7` waiting for VM termination (120s polling). Once VM self-deletes: fresh manifest → verify 2026≥95% →
flip todo #2 → update closeout MVP-cell row → commit+push → `/done`.

### 2026-08-10T12:36Z — 2026 VM self-deleted, manifest registration bug found (P1)

**Status: BLOCKED — todo #2 still `[ ]`.** 2026 VM self-deleted at 12:34:59 UTC (T+62 min, `operation-1786365162145`,
`delete` type). Pipeline processed 16 dates (2026-01-02 through 2026-01-26), downloaded ~300K records from Databento,
wrote parquet files to GCS successfully — but **manifest registration failed for every date**:

```
WARNING Manifest write failed (non-blocking): MalformedRowKeyError:
shard-atom field 'chain' was explicitly passed as empty.
Fix: either remove 'chain' from row_key (non-per-chain shard) or populate it.
```

Root cause: `manifest_recorder.py:181` passes `chain=chain or ""` and `manifest_writer_normalising.py:142` normalises to
`chain=""`. ES_OPT (S&P index options) has no meaningful chain value — the field should be OMITTED for non-per-chain
shards, not passed as empty string. The data EXISTS in GCS
(`gs://market-data-tick-tradfi-prd-central-element-323112/raw_tick_data/by_date/day=2026-01-{02..26}/`) but the manifest
can't see it.

**Coverage (manifest snapshot 12:36Z, 177.6 MiB)**:

| Year      | Captured/Total | Pct   | Source                                                                     |
| --------- | -------------- | ----- | -------------------------------------------------------------------------- |
| 2020-2024 | All 100%       | —     | CME campaign incidental                                                    |
| 2025      | 284/284        | 100%  | CME campaign incidental (today's VM hit same chain bug)                    |
| 2026      | 149/286        | 52.1% | CME campaign incidental; today's VM wrote ~137 more dates, none registered |

2026 target (≥95%) NOT met via manifest — blocked on the `chain=''` bug.

**Issue filed**: `/plans/active/issues/tradfi_es_opt_manifest_chain_field_empty_blocking_coverage_2026_08_10.md` (P1).
Fix is straightforward: omit `chain` from row_key when empty rather than passing `""`. Once fixed, re-running the
manifest consolidator (or re-launching the backfill) should register the existing GCS data — no re-download needed.

- **NEXT ACTION**: Fix the `chain=''` bug (remove `chain` from row_key when empty, or populate with valid value), then
  either: (a) re-run the manifest consolidator to pick up the already-written GCS data, or (b) re-launch the 2026
  backfill (idempotent, will re-download but the data is small). Once manifest shows 2026≥95%: flip todo #2, update
  closeout MVP-cell row, commit+push, `/done`.

### 2026-08-10T12:53Z — chain='' bug FIXED + shipped, re-launching ES_OPT 2026

**Status: IN FLIGHT — todo #2 still `[ ]`.** Root cause found and fixed: `manifest_finalize.py:254` unconditionally
included `"chain": manifest_chain` in `base_row_key`, where `_resolve_manifest_venue_chain()` returns `""` for all
non-DeFi venues (including CME). The empty string then tripped `hard_schema_enforcement` in `_coerce_row_key()` →
`MalformedRowKeyError`. Fix: conditionally include `"chain"` only when `manifest_chain` is non-empty (3 lines,
`29b4310ea`). QG green, quickmerge landed on LDR.

**Re-launching 2026 backfill** to register the existing GCS data under the fixed manifest writer. Re-download is
idempotent and fast (data already cached at Databento). 2025 already shows 100% in manifest (via CME campaign) — only
2026 needs the re-launch.
