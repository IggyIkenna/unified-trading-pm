...
- [x] ✅ [SCRIPT] P1. **Combined residual close-out — all 4 sub-items landed (2026-07-27, slot-11), read-only live
      ...
      specific todo). Ran `reconcile_phantom_manifest_rows_all.py --asset-group prediction --unphantom-only --dry-run`
      live against the 51: re-validated 51/51 — **0 unphantomed** ("Still phantom: 51 ... Unphantomed: 0 ... Manifest is
      clean"), i.e. all 51 genuinely have no parquet at ANY UAC candidate path (including the
