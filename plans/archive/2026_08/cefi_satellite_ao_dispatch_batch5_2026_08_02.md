...
status: complete # (was: active) 2026-08-05 archival sweep: all 5 todos [x], no locked_by
nature: process
asset_group: [cefi]
stage: [data]
...
related:
  [
    ...
    /plans/archive/issues/mdps_backfill_cefi_trades_gap_fill_completion_2026_07_28.md,
...
> **Adjacent work, confirmed NON-colliding (2026-08-02), now RESOLVED (2026-08-03).**
> `/plans/archive/issues/mtds_live_smoke_vm_name_exceeds_gcp_limit_2026_08_01.md` (`assigned_vm: planning`) concerned
> the same `mtds-live-smoke` launch path — its single todo claimed
> `market-tick-data-service/scripts/pipeline_e2e_check.py` only (bounding the generated VM name to GCP's 63-char limit)
> and shipped at `market-tick-data-service@bccb95ea`. It did **not** claim `launch-mtds-live.sh` and never touched the
> skill's `SKILL.md`, so it never collided with todo 1 or todo 2 here — noted historically in case a future reader
> wonders why a same-path adjacent doc wasn't folded in.

## Todos

- [x] ✅ **NOT-A-BUG (2026-08-02) — do not implement as literally written; premise refuted by code trace, operator-ruled
      Option A (BLK-5aa3ce78).** [INFRA] P1. ~~Wire the Tardis N=1 concurrency guard into `launch-mtds-live.sh` and
      ...
      **Resolution (2026-08-02, slot 15):** before implementing, traced whether MTDS's live-mode capture path actually
                                                                                                                          ...
                                                                                                                          `/plans/archive/issues/mtds_live_smoke_vm_not_tardis_guarded_2026_07_28.md` (P1 + P2 of 3). **Done when** —
...
- [x] ✅ [DOC] P3. **Add the live-leg Tardis guard-gap note to the `data-pipeline-check-mtds` skill's Phase-2 section.**
      ...
      with an active Tardis backfill/sharded VM for the single shared IP, and recommend deferring live-leg checks for
      Tardis-sourced venues while a real Tardis-consuming VM is confirmed running, until the launcher-guard todo above
      ships. **Scope-fenced: Phase 2 ONLY** — the same file's `§ 3 (Tardis cap)` section is owned by
      `/plans/archive/issues/mtds_backfill_launcher_guard_overapplies_to_nontardis_venues_2026_07_28.md`, whose
      ...
      **Corrected path (verified 2026-08-02)**: the skill lives at
      ...
      `/plans/archive/issues/mtds_live_smoke_vm_not_tardis_guarded_2026_07_28.md` (P3 of 3). **Done when**: the skill's
...
- [x] ✅ [SCRIPT] P3. **Run the additive MDPS manifest reconciliation for the cefi trades gap-fill campaign, on a VM.**
      — deployment-service@35e91fa, unified-trading-library@a01b3f13. Completed 2026-08-05 via VM
      `mdps-cefi-manifest-merge-20260805-085102` (e2-highmem-8, asia-northeast1-c). Exit code 0. Results: 9,720,496 →
      9,779,363 manifest rows (+58,867). Discovered 163,661 shards on GCS, 109,190 already present, 54,471 newly
      registered. r20251225 confirmed: 1,880 rows for 2025-12-25 registered (including the 7 originally unflushed). Full
      log: gs://deployment-scripts-central-element-323112/vm-logs/mdps-cefi-manifest-merge-20260805-085102/run.log
      ...
      `/plans/archive/issues/mdps_backfill_cefi_trades_gap_fill_completion_2026_07_28.md`. **Done when**: the merge runs
...
- [x] ✅ [DATA] P3. **Root-cause the KRAKEN-FUTURES / HYPERLIQUID PERPETUAL catalogue-completeness gap — ID-FORM
      MISMATCH, not a catalogue completeness gap (2026-08-05, slot 5).** Both venues were investigated via bounded
...
`/plans/archive/2026_08/cefi_satellite_ao_dispatch_batch5_2026_08_02_finalize.md` (`depends_on` +
`gate_on_depends: true`), mirroring the batch1 through batch4 finalize pattern.
...
- **context-scout 2026-08-03**: re-confirmed context_scope (4 entries) unchanged — already matches this doc's own "Codex
  SSOTs" citations; dispatch-batch coordinator doc, so no source-code paths added per the skip-source carve-out.
