...
- [x] ✅ [CODE] P2. **Verify the shipped vendor-catalog gate (`tardis_vendor_catalog.py`,
      ...
      `GET https://api.tardis.dev/v1/exchanges/bybit`, real response, 2026-08-09): `availableSymbols[]` entries carry
      ONLY `id`/`type`/`availableSince`/`availableTo` — **never** `dataTypes` (checked the union of keys across all
      1,812 entries) — while `datasets.symbols[]` entries carry all four fields
