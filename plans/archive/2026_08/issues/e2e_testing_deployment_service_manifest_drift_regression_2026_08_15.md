...
locked_by:
locked_since:
resolved_by: "see Progress Log below — landed via the scripts/manifest/fix-internal-dependency-alignment.py TIER_EXEMPT_REPOS addition in this same archival commit"
context_scope:
  [
...
- [x] ✅ [OPERATOR] P1. **RULED 2026-08-16: option (a)** — exempt e2e-testing from the tier-DAG check, mirroring
      deployment-service's own existing exemption. Black-box integration tests importing service internals is the
      repo's whole purpose, not an accidental violation; this doesn't weaken the check for real production code, only
      for this repo's documented testing role. **Implemented and verified**: added `"e2e-testing"` to
      `TIER_EXEMPT_REPOS` in `scripts/manifest/fix-internal-dependency-alignment.py`, alongside `deployment-service`
      (both fall to tier 0 via the same non-numeric/non-service arch_tier fallback — `integration` and `devops`
      respectively — which is why `0 <= 0` tripped the violation). Verified directly: `is_tier_violation('e2e-testing',
      'deployment-service', tier_map)` now returns `False`; a negative-control check
      (`is_tier_violation('strategy-service', 'execution-service', ...)`) still returns `True`, confirming the
      exemption is scoped to exactly this repo, not a blanket loosening. This makes the fix durable — the manifest
      entry that was previously manually re-added (fragile, would regress on the next `fix-internal-dependency-
      alignment.py --apply` run) is now the CORRECT, checker-sanctioned state, not a workaround sitting outside the
      tool's own logic.
