...
> **🟢 ARCHIVED 2026-08-10 — RESOLVED** (status: resolved, 0 open todos, unlocked). All 4 todos completed +
> live-verified: (1) CODE P1: row-group/column-pushdown structural fix → reprobe completes without OOM
> (e2e-testing@64d369a); (2) INFRA P1: mechanical ceiling bump 16→32Gi shipped (deployment-service, confirmed exhausted
> as a standalone fix); (3) CODE P1: columns= restriction + row_filter shipped (e2e-testing@507717c); (4) INFRA P3:
> sibling manifest-hygiene gap confirmed OOM-failing live + fixed with columns= restriction (e2e-testing@601f8beecd).
> Referrers updated. Sibling date-column bug separately resolved
...
- [x] ✅ [CODE] P1 _(escalated from P2 2026-08-09 — the memory-ceiling path is now confirmed exhausted, not
      ...
      **0.56GB peak RSS**, returning exactly the 3,897 `SOURCE_RETURNED_ZERO` rows out of 81.6M (measured via a direct
      ...
      e2e-testing.
- [x] ✅ [INFRA] P3. Check whether `uts-prod-dp-manifest-hygiene-changed` and `uts-prod-dp-manifest-hygiene-full` have
      the same unrestricted-columns gap in their own manifest-read call sites — **CONFIRMED + FIXED (2026-08-10, slot 23
      review agent).** Both call sites in `manifest_hygiene_daily.py` had the EXACT same unrestricted-columns
      antipattern: `_check_v9` L138 passed `columns=None`, `_check_path_canonicality` L167 passed no columns arg at all.
      Both Cloud Run jobs confirmed OOM-failing live: `uts-prod-dp-manifest-hygiene-changed` daily OOM since at least
      2026-08-06 (5 consecutive), `uts-prod-dp-manifest-hygiene-full` weekly OOM since at least 2026-07-12 (5
      consecutive). Fixed by adding `columns=["schema_version"]` to `_check_v9` and `columns=list(_PATH_PROBE_COLUMNS)`
      to `_check_path_canonicality`, mirroring the proven `daily_digest.py` + `reprobe_new_empty_confirmed.py` pattern.
      Regression test `test_hygiene_requests_column_restricted_reads` added (spy-asserts both calls pass restricted
      column sets). All 76 unit tests green, QG green, shipped e2e-testing@601f8beecd. Repo: e2e-testing.
...
- **2026-08-09T~20:00Z-20:40Z (slot 15, data_engineering, task
  ...
  7. **Sibling date-column bug (referenced above) is now fixed + archived** —
     `/plans/archive/2026_08/issues/reprobe_date_column_resolution_prefers_blank_available_at_2026_08_09.md`,
     e2e-testing@9c75040.
- **2026-08-10T23:07-23:15Z (slot 23, review agent, task `-92413d80279c`): checked the INFRA P3 — both jobs have the gap
  AND are OOM-failing live. Fixed.**
  1. **Confirmed the unrestricted-columns gap in `manifest_hygiene_daily.py`**: `_check_v9` L138 explicitly passes
     `columns=None`; `_check_path_canonicality` L167 passes no `columns=` arg at all — both read the full 41-column
     manifest index. The `_dp_common.read_manifest_index()` already supports `columns=` + `row_filter` (shipped for
     reprobe + digest), but hygiene's two call sites were never updated. Column needs: `_check_v9` only reads
     `schema_version` (confirmed via `schema_version_readiness()` source); `_check_path_canonicality` only reads path
     columns from `_PATH_PROBE_COLUMNS = ("gcs_path", "path", "object_path")`.
  2. **Both Cloud Run jobs confirmed OOM-failing live** (gcloud run jobs executions list):
     - `uts-prod-dp-manifest-hygiene-changed` (daily, `--mode changed`): "The configured memory limit was reached" on
       EVERY run 2026-08-06, 07, 08, 09, 10 (5 consecutive days).
     - `uts-prod-dp-manifest-hygiene-full` (weekly, `--mode full`): same OOM on EVERY run 2026-07-12, 07-19, 07-26,
       08-02, 08-09 (5 consecutive weekly executions). Both provisioned at 4CPU/16Gi (same ceiling reprobe had
       pre-bump). These jobs have been silently failing for WEEKS — the OOM manifests as exit code 0, and the
       DP_NOT_V9 + DP_NONCANONICAL_PATH_ON_DISK alerts have been silent because the job never completes.
  3. **Fixed**: `_check_v9` → `columns=["schema_version"]`; `_check_path_canonicality` →
     `columns=list(_PATH_PROBE_COLUMNS)`. Regression test added (spy-asserts both calls pass restricted column sets).
     All 76 unit tests green, QG green (90s), shipped `e2e-testing@601f8beecd` (verified ancestor-of-origin). Unlike
     reprobe, these functions iterate over ALL rows (aggregate checks), so `row_filter` isn't applicable — but column
     restriction alone (41→1 for v9, 41→3 for path-canonicality) + DuckDB's memory_limit should keep peak RSS within
     16Gi. The image rebuild uses the same cloudbuild-e2e-audit.yaml config reprobe used; once the new image is live and
     tomorrow's daily run completes without OOM, this issue doc's last remaining todo is fully closed.
