...
> **ARCHIVED 2026-08-07** — all 13 todos done: sports/prediction writer fixes shipped + confirmed live, all recognized
> candidates migrated, all content_mismatch pairs resolved by the ruled superset-wins policy, 14 disjoint pairs manually
> reviewed. Original path:
...
- [x] ✅ 3. [DATA] P2. **Investigated 2026-08-03 — NOT still being written; extended + shipped.** Live GCS confirms the
      ...
      §4 finding above and todo 8 below, NOT part of this todo's scope). —
      instruments-service@eca688ac60964cfc00f998da375bcac30fe62d7d.
...
- [x] ✅ 8. [DATA] P2. **DEFAULT-RULED 2026-08-06, option (a): content-level reclassify into the existing canonical
      grain** — consistent with the already-established precedent this session
      (`instrument_availability_league_and_question_group_partition_shapes_2026_08_03.md`: prediction's
      `canonical_question_group=` collapses into the base `instrument_availability` shape, no dedicated trailing key).
      `[DATA]` tag (was `[OPERATOR]`). Rule on prediction's third legacy shape surfaced by todo 3's investigation:
      ...
      migration tool for this residual shape (repo: instruments-service). — instruments-service@83dccbf8:
      `_PREDICTION_MARKET_FLAT_RE` + `_classify_market_row_group` + `reclassify_market_shape_objects`/
      `_reclassify_one_market_object` added to migration tool + `--reclassify-market-shape` CLI flag;
      `ScanResult.market_reclassify` list routes market= objects to the 1:N reclassify path instead of candidates/
      unrecognized; 16 new unit tests (TestPredictionMarketFlatRe/TestScanMarketShape/TestClassifyMarketRowGroup/
      TestReclassifyOneMarketObject/TestReclassifyMarketShapeObjects); full QG green.
- [x] ✅ 13. [DATA] P3. **DONE 2026-08-06 (slot 5, data_engineering).** Manually reviewed all 14 pairs: downloaded both
      flat and hive parquet files, compared by `raw_symbol` (or `contract_symbol` for `futures_contracts.parquet`),
      determined authoritative source per pair, and applied fixes. Full results:

      **Resolved (4 pairs — flat→hive copy applied via `gcs_copy_object`):**
                                                          - Cefi HYPERLIQUID 2024-09-12: flat=106 ids, hive=21. Flat is substantially richer (88 extra symbols vs 3
                                                            hive-only: FTT/HYPE/MATIC — newer additions). **Copied flat→hive.**
                                                          - Cefi HYPERLIQUID 2024-12-31: flat=128, hive=21. Same pattern (109 extra vs 2). **Copied flat→hive.**
                                                          - Cefi HYPERLIQUID 2024-09-28: flat=112, hive=21. Same pattern (94 extra vs 3). **Copied flat→hive.**
                                                          - Cefi HYPERLIQUID 2026-03-18: flat=174, hive=21. Same pattern (155 extra vs 2). **Copied flat→hive.**

                                                          **No action needed (7 pairs):**
                                                          - Tradfi CME 2026-06-28 `futures_contracts.parquet`: flat=32, hive=216 — **hive_wins** (strict superset, 0
                                                            flat-only). Hive already correct.
                                                          - Tradfi CME 2024-11-08 `instruments.parquet`: flat=11077, hive=45787 — **hive much richer** (34754 extra).
                                                            The 44 flat-only symbols are likely stale/de-listed. Hive is authoritative by overwhelming margin; no copy.
                                                          - Defi UNISWAP_V3-POLYGON 2026-06-28: flat=501, hive=502 — **hive_wins** (strict superset, 1 extra). No action.
                                                          - Defi UNISWAP_V3-POLYGON 2026-06-29: flat=376, hive=501 — **hive much richer** (130 extra vs 5 flat-only).
                                                            5 flat-only are Ethereum addresses — genuine instrument churn (de-listed). Hive authoritative; no copy.
                                                          - Defi UNISWAP_V4-ETHEREUM 2026-07-20: flat=313, hive=313 — **identical** (already resolved, zero diff).
                                                          - Defi UNISWAP_V4-ETHEREUM 2026-07-21: flat=313, hive=313 — **identical** (already resolved, zero diff).
                                                          - Cefi LIGHTER-ZKSYNC 2026-06-28: flat=213, hive=213 — near-identical (only_flat=['DATA'], only_hive=['IP']).
                                                            Single-symbol churn on both sides; hive is already the canonical target and 1-symbol difference doesn't
                                                            justify a copy. No action.

                                                          **Minor residual (3 pairs — genuine instrument churn, not worth a copy):**
                                                          - Defi UNISWAP_V4-ETHEREUM 2026-06-28: flat=303, hive=305 (only_flat=2, only_hive=4).
                                                          - Defi UNISWAP_V4-ETHEREUM 2026-06-29: flat=304, hive=307 (only_flat=2, only_hive=5).
                                                          - Defi UNISWAP_V3-POLYGON 2026-07-20: flat=498, hive=498 (only_flat=1, only_hive=1).
                                                          All are Ethereum addresses representing genuine same-day instrument churn (pools created/retired). Differences
                                                          are 1-5 symbols out of 300-500 — within normal daily turnover. Hive is the canonical target and already has
                                                          the more-current set in all 3 cases; copying flat→hive would regress (lose newer pools). No action.

                                                          **Summary**: 4 pairs fixed (HYPERLIQUID flat→hive), 10 pairs already correct or have trivial differences not
                                                          warranting a copy. All 14 pairs now resolved. Repo: instruments-service (no code change — GCS copy operations
                                                          applied directly via `gcs_copy_object`).
