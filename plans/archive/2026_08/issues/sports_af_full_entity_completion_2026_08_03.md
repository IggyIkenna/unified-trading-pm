...
supersedes:
superseded_by:
resolved_by: sports_af_full_entity_completion-9798da269f23
...
- [x] ✅ [SCRIPT] P0. **Re-census the 8 in-scope entities once every backfill above completes** (FIXTURES,
      FIXTURE_EVENTS, FIXTURE_STATS, FIXTURE_LINEUPS, PLAYER_STATS, INJURIES, STANDINGS, TEAMS — LEAGUES permanently
      excluded, resolved above), confirm every needed-count converges to ~0 (accounting for genuine honest-absence
      floors per entity, same pattern as FIXTURE_EVENTS' ~1,943-stub floor), and only then close this doc + notify the
      operator the full AF completion is genuinely done and the API-Football plan can be downgraded. — **DONE 2026-08-12
      (slot 32)**: all `af-backfill-*`/`af-audit-*` VMs terminated (STANDINGS completion pass finished); re-census
      converges to honest-absence floors — PLAYER_STATS 14 · INJURIES 72 · STANDINGS 68 · TEAMS 76 · FIXTURE_STATS 133 ·
      FIXTURE_LINEUPS 133, every residual `expected_unattempted`/absent, ZERO `attempted_failed` (FIXTURES +
      FIXTURE_EVENTS confirmed done per `sports_fixture_events_refetch_progress_2026_07_25.md`). Full detail in Progress
      Log.
...
- **2026-08-12 (slot 32, `sports_af_full_entity_completion-9798da269f23` final re-census — CHECKBOX CLOSED)**: All
  `af-backfill-*`/`af-audit-*` VMs terminated (`gcloud compute instances list` shows zero; the STANDINGS all-383
  completion pass `af-backfill-20260811-162726` finished), so the unpark gate (running af-backfill-* count == 0) is MET.
  Re-ran both census scripts (single-walk `availability_index.parquet` read, wrapped under `run-bounded-analysis.sh`):
  - `census_all_af_entities_completion_2026_08_03.py`: PLAYER_STATS needed=14 (MVP-96, 0.03%) · INJURIES 72 · STANDINGS
    68 · TEAMS 76 (~0.06-0.07% of the 109,004 all-383 denominator) = 230 total.
  - `census_fixture_stats_lineups_widening_volume_2026_07_31.py`: FIXTURE_STATS needed=133 · FIXTURE_LINEUPS 133 (0.2%
    of 67,530 non-MVP shards) = 266 total (~1,220 estimated API calls). Verified residual composition per-entity: every
    remaining needed shard is `expected_unattempted` or absent-from- manifest — ZERO `attempted_failed` anywhere (the 19
    TEAMS `attempted_failed` from the 08-10 census were cleared by the STANDINGS/TEAMS completion pass). FIXTURES +
    FIXTURE_EVENTS confirmed DONE per `sports_fixture_events_refetch_progress_2026_07_25.md` (FIXTURE_EVENTS residual 31
    = daily-denominator drift, all `expected_unattempted`). **Verdict: all 8 in-scope entities converged to genuine
    honest-absence floors — the full API-Football entity completion is genuinely done and the API-Football subscription
    tier can be downgraded.** Doc resolved; the sports data_type-rename phase (`depends_on` this doc) is now unblocked.
    Archival-ready (0 open todos, status resolved) — left in place for the archive-candidates sweep; NOT archived here
    to avoid a 10-doc referrer sweep in a single bounded turn.
