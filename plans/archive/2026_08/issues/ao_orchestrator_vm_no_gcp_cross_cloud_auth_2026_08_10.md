...
> **ARCHIVED 2026-08-16 (/plan-reconcile)** — all 3 todos closed as moot: this doc's premise (no GCP auth on the AO
> orchestrator VM) is refuted by its own cited codex SSOT (`/codex/05-infrastructure/agent-slack-read-access.md`,
> `last_reviewed: 2026-08-11`), which documents the original check as a wrong-OS-user false negative — GCP auth
> already works correctly via the `ubuntu` user's ADC. No open work remains. Original body preserved below.

## Todos

- [x] ✅ [OPERATOR] P2. **CLOSED — MOOT 2026-08-16 (/plan-reconcile).** This doc's premise (AO orchestrator VM has NO
      GCP auth) is factually refuted by its own cited codex SSOT: `/codex/05-infrastructure/agent-slack-read-access.md`
      (`last_reviewed: 2026-08-11`) documents this exact claim as a wrong-OS-user false negative — the original SSM
      check ran as `root`, but AO workers run as `ubuntu`, whose gcloud ADC already resolves to `unified-trading-sa`
      with `secretmanager.secretAccessor`, confirmed live 2026-08-11 (`sudo -u ubuntu gcloud secrets versions access
      ...` succeeded). No WIF decision is needed — GCP auth already works from the correct user context.
- [x] ✅ [INFRA] P2. **CLOSED — MOOT 2026-08-16 (/plan-reconcile).** Downstream of the WIF decision above, which is
      moot for the same reason — `secretmanager.versions.access` is already granted to `unified-trading-sa`, which the
      `ubuntu` user's ADC already resolves to.
- [x] ✅ [DOC] P3. **CLOSED — MOOT 2026-08-16 (/plan-reconcile).** The codex doc this todo names,
      `/codex/05-infrastructure/agent-slack-read-access.md`, already carries the corrected framing (verified live,
      `last_reviewed: 2026-08-11`) — no further doc update needed for this specific caveat.
