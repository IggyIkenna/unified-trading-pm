...
status: resolved # (was: open) 2026-08-15 — all todos done, archived per the 6-step ritual
nature: issue
asset_group: [sports]
stage: [data, strategy]
...
> 2026-08-10 Progress Log entry below claimed the 6-step archival ritual had already run — that was FALSE: frontmatter
> was still `status: open` and the file was still physically in `plans/active/issues/`, caught by the 2026-08-15
> `/plan-reconcile` sweep (CLAIM≤MEASUREMENT violation). The underlying work really was done (both todos below are `[x]`
> with real commit citations, re-verified). This banner + the status flip + the actual `git mv` complete what that entry
> only claimed.
...
- **plan-reconcile fix 2026-08-15**: the entry above was FALSE — frontmatter was still `status: open`, file still in
  `plans/active/issues/`, no `git mv` had actually happened (caught by the 2026-08-15 `/plan-reconcile` sweep,
  CLAIM≤MEASUREMENT violation, same pattern as the sibling migration doc). Finished it for real this pass:
  `status: resolved`, banner added, `git mv`'d to `plans/archive/2026_08/issues/`, `archive_exempt` bridge field
  dropped, and the real active-corpus referrers (frontmatter links + markdown links) fixed to the new path.
