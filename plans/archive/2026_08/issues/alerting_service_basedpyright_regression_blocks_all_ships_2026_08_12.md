...
resolved_by: alerting-service@afbbdd2df9
locked_by:
locked_since:
supersedes:
superseded_by:
...
> **🟢 ARCHIVED 2026-08-12 — RESOLVED** (status: resolved, 3/3 todos `[x]`, unlocked). Real basedpyright debt fixed
> (alerting-service@afbbdd2df9, 0 errors, cap ratcheted to 0). The isolated-worktree false-positive finding this doc
> also surfaced was closed as accepted-with-workaround (`--no-isolated`), not root-caused, after 7 independent
> elimination attempts across two sessions — see "NEW finding" below if it recurs. Archived by infra worker (slot 1).

# RESOLVED: alerting-service basedpyright debt fixed (afbbdd2df9); isolated-worktree re-gate false-positive closed accepted-with-workaround
...
## NEW finding — isolated-worktree basedpyright re-gate produces false-positive type-identity errors (closed, accepted-with-workaround)
...
- Replicating the gate's SECOND install step — `uv pip install -e ../unified-api-contracts --python .venv/bin/python`
  (the pip-compatible-interface reinstall `base-service.sh` runs after `uv sync --frozen`, specifically to pick up
  override-dependencies `uv sync` alone misses) — then re-running basedpyright: 0 errors.
- Matching the gate's exact invocation shape — backgrounded (`&`), stdout redirected to a file, not a TTY/pipe
  (`_qg_run_basedpyright_attempt`'s pattern): 0 errors.
- Setting `CI=true`: 0 errors.
...
the failure is not explained by divergent source content, a stale editable-install pointer, cache directory reuse, the
gate's second override-dependencies reinstall step, or its background/file-redirect invocation shape — every one of
these was checked and ruled out. Root cause is genuinely unknown after 7 independent elimination attempts across two
sessions. Shipped this fix via `--no-isolated` instead (verified safe: full non-isolated `quality-gates.sh --no-fix`
passed cleanly beforehand, and this session was the sole active worker on alerting-service, so the shared-checkout race
`--isolated` exists to guard against didn't apply here).

- [x] ✅ [INFRA] P3. **Root-cause the isolated-worktree basedpyright false-positive.** DESCOPED 2026-08-12 — closed as
      accepted-with-workaround, not root-caused. 7 elimination attempts across two sessions (see above) all failed to
      reproduce it outside the gate's own live invocation; every plausible mechanism tested (cache reuse, stale editable
      pointer, the override-dependencies reinstall step, invocation shape, `CI` env, local venv staleness) came back
      clean. Diminishing returns on further blind elimination without instrumenting the gate script itself mid-failure
      (which needs it to be actively failing to attach to, not reproducible on demand). **Practical impact is zero**:
      `--no-isolated` is a proven, sanctioned workaround (this doc's own fix shipped through it), and isolation was
      already opt-in-only for non-PM repos before this was found — nothing regresses. Re-open if it recurs with fresh
      evidence, ideally captured live (the gate script's own `PYRIGHT_OUT`/exit code/venv `site-packages` dist-info
      listing at the moment of failure, not a post-hoc reproduction attempt). Repo: unified-trading-pm.
