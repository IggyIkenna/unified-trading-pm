...
resolved_by:
  "unified-trading-pm@bd1d97a8b (runbook doc entry + cross-reference note) + deployment-service@5f598d9 (auto-route
  HL/ASTER/LIGHTER-ZKSYNC/EXTENDED-STARKNET to --operation collect-onchain-perp-batch when VM_DATA_TYPES is set,
  verified via a real --test-run smoke launch showing trades-only fetch, no over-fetch)."
locked_by:
