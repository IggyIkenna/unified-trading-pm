...
| --- | ----------------------------------------------------------------------- | --------: | ----------------: | ----------------- | :----: | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 1   | `bucket_estate_consolidation_to_sub100_2026_07_13.md`                   |      2129 |      21 (15 done) | —                 | **c**  | Fork 6 open mop-up todos → `bucket_estate_consolidation_closeout_2026_07_24.md`; archive parent (history intact)                                                                                                                       |
...
| 13  | `/plans/archive/2026_07/github_actions_ci_cost_reduction_2026_07_15.md` |      2094 |                42 | —                 | **c**  | 3-way split: completed migration (archive-bound) / operator-gated followups / new same-day Phase-6 staging-shutdown                                                                                                                    |
...
| ----------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | -------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `bucket_estate_consolidation_to_sub100_2026_07_13.md`                   | `bucket_estate_consolidation_closeout_2026_07_24.md` (6 todos: recon-bucket closeout, cross-plan deletion checkpoint, ml legacy variants, `_KIND_ALIASES` removal, asset-group-parity drift, 3 audit issue-doc closures)             | clean-partition                                                                                                                                          |
...
| `/plans/archive/2026_07/github_actions_ci_cost_reduction_2026_07_15.md` | `github_actions_self_hosted_runner_migration_2026_07_15.md` (0 todos, archive-bound), `github_actions_operator_gated_followups_2026_07_17.md` (8 todos), `github_actions_staging_machinery_shutdown_2026_07_23.md` (3 todos)         | clean-partition                                                                                                                                          |
...
**Execution status (2026-07-24, updated): 21 of 30 DONE in the original batch, +5 more fixed in a wave-2 pass, +3 more
(`defi_consolidated_closeout_2026_07_18.md` @ f6ac7f5f0, `prediction_consolidated_closeout_2026_07_18.md` @ 573d2b568,
`distinct_values_noncanonical_audit_2026_07_20.md` — fork landed, see below) landed by concurrent sessions/this session
— 7 of the original 30 remain HARD-flagged (plus `data_completion_to_100_all_ag_2026_06_21.md` mid-transformation by a
live concurrent session as of this update, not yet safely committable — see the table below), see the table below.**
...
**`distinct_values_noncanonical_audit_2026_07_20.md`'s rescoped fork landed** (found already-complete, uncommitted, on
...
| ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `citadel_paper_batch_live_reconciliation_2026_06_19.md` still ~1143L (cap 1000-2000 depending on exemption) — **correction 2026-07-24: its `## Progress Log` (line 1138) is NOT the extraction target, it's already just a 5-line pointer to the archived history file.** The real remaining bulk is `## Phase 11 — Autonomous rolling-book gaps` (lines 668-1133, ~465L) — a MIX of 26 done + 7 open todos interleaved, not pure history, so it needs a careful trim (condense each done item's verbose evidence to a 1-2 line summary, or extract fully-closed sub-sections while keeping the 7 open todos + their context in the parent), not a wholesale move       | Not done                      | Nobody — do the Phase-11 interleaved trim described above; verify conservation (7 open + 26 done = 33 todos preserved somewhere) before committing                  |
| `data_pipeline_hardening_self_monitoring_2026_06_22.md` still ~1856L (cap 1000, no longer umbrella-exempt — todos dropped under 100) — **confirmed 2026-07-24: the extraction target is real** — lines 1310-1856 (~546L) hold ~10 dated `## Progress Log — <topic> SHIPPED/DONE/FIXED` sections, all but one line pure closed-out history (5 done todos there). **The one exception**: line ~1804's `### REAL OUTAGE surfaced by the fixed watcher (P0 — needs recovery)` contains 1 genuinely OPEN `- [ ]` P0 todo inside that range — it must stay in the parent (or be relocated to a live section), not get swept into the archive-bound history file with the rest | Not done                      | Nobody — extract lines 1310-1856 minus the one open P0 item noted above to an archive-bound history child, same pattern as the existing one                         |
...
| `data_completion_to_100_all_ag_2026_06_21.md` — a concurrent live session is mid-transformation on this file RIGHT NOW (uncommitted, working tree references a not-yet-created `data_completion_to_100_all_ag_history_2026_07_24.md`)                                                                                                                                                                                                                                                                                                                                                                                                                                   | In progress (another session) | That session — do not commit/touch until either the history file exists and the frontmatter reference resolves, or the session finishes and pushes it itself        |
| `mtds_venue_backfill_and_ops_hardening_residuals_2026_07_24.md` still 1238L, unchanged — but its history child `mtds_venue_backfill_and_ops_hardening_residuals_history_2026_07_24.md` (archive-bound, 515L, 0 todos) now exists and is committed/archived, same as the citadel/data_pipeline_hardening pattern above                                                                                                                                                                                                                                                                                                                                                   | Not done                      | Nobody — that one slice is landed; the rest of its 2026-06-18-to-2026-07-13 fully-completed historical Progress Log still needs the same treatment                  |
...
| `artifact_pipeline_observability_2026_07_17.md` — **NEW, not part of the original 30**, found HARD-flagged (1045L) by re-running `check_line_caps.sh` on 2026-07-24; crossed the 1000L cap via unrelated concurrent-session edits after this remediation started                                                                                                                                                                                                                                                                                                                                                                                                        | Not triaged                   | Nobody — out of this doc's original scope, but it's real and current; give it the same bucket-classification pass as the original 30 before splitting it            |

**Recommended next item**: dispatch a fresh fix-workflow (or work through by hand) against the 8 real "Not done"/"Not
triaged" rows above (the `data_completion_to_100_all_ag_2026_06_21.md` row is a status note, not a dispatchable task —
check it first, don't duplicate the other session's in-flight work). Given TWO independent workflow runs targeting this
exact tail have now been silently lost mid-run with no completion record (`wf_22001490-e9b`, then `wf_87a8f203-aa1`) —
both times with real, uncommitted, unverified partial progress sitting in the working tree that had to be discovered by
direct measurement — the next session should NOT assume a launched workflow will be observed to completion in the same
session. Recovery drill each time: `wc -l` every target file vs `git show HEAD:<path> | wc -l`,
`grep -cE '^- \[[ xX]\]'` every target file + every claimed child/history file, confirm the removed-todo lines from the
parent's diff are each present in a child/history file via `grep -F` (a snippet of each removed line), THEN commit —
never trust the returned summary text alone.
