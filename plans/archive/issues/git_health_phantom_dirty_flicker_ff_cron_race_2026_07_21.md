...
resolved_by:
  "agent-orchestrator@2530316 (dirty_consecutive_ticks>=2 gate), agent-orchestrator@529b0dc (git-status keyed by
  (host,slot_id), likely root cause of the fleet-wide phantom), unified-trading-pm@d2b588688 (dirty_files re-derived
  from the sample array, single source of truth), unified-trading-pm@dd172d6b7 + @5cc0ea829 (FF-pull confirm-gate).
  VERIFICATION 2026-07-23 (live fleet measurement): phantom does not reproduce; every non-clean row matched real
  worktree state. Item 6's dirty_files=2172 row independently verified REAL (not phantom) 2026-07-24 via the per-slot
  debug endpoint."
locked_by:
depends_on: []
---
