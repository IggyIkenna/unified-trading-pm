...
> `plans/archive/issues/features_gas_fees_calculator_stale_legacy_venue_read_2026_07_30.md` P3) cannot ship under the
...
- **2026-08-05 (data_engineering slot-12) PUSH-BLOCKED NOTE**: this issue doc could NOT be pushed via quickmerge at
  ...
  `plans/archive/2026_08/issues/dex_pool_state_build_instrument_id_colon_in_symbol_2026_08_04.md` (mtime 12:54) dirtied
  the tree; the full gate then failed on the PRE-EXISTING `agent-rules-size-cap` check — `cursor-configs/CLAUDE.md` is
...
- **2026-08-05 (data_engineering slot-12) RE-GATE #4 — ONE NEW RED: rule11 shard-count sentinel, caused by a NEW UAC
  ...
  `plans/archive/2026_08/defi_distinct_values_zero_noncanonical_ dispatch_2026_08_04.md` row 8 explicitly dispatches
  "Register `perp_mark_price` + backfill" (status: Dispatched, in progress) mirroring the already-shipped
  `perp_daily_ctx` pattern; (3) the corpus is preserved in the shared bucket + has 22,374 manifest rows
