...
> **🗄️ ARCHIVED 2026-07-30** — `status: resolved`, `resolved_by: market-tick-data-service@46e85d94`. All 5 todos done:
> re-audited the true scope (8 venues / 12 files, not the originally-filed 6), fixed the ITYPE token + symbol-shape
> canonicalization for every venue (incl. 2 defects the original filing didn't anticipate — Kraken's `/` separator,
> Upbit's reversed base/quote order), added regression tests, and confirmed no downstream reader hard-codes the old
> shape. One deferred measurement (historical per-venue non-canonical row counts) migrated to a real todo in
> `plans/active/data_pipeline_reconciliation_skill_2026_07_20.md` per the archival ritual's step 1. Codex alignment:
> `/codex/02-data/shard-granularity-cefi.md` updated with the 4 new venue branches + the `derive_spot_pair_symbol()`
