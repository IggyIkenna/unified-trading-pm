...
- [x] ✅ [DOC] P3. **Backfill `author` on the 438 existing issue docs that lack it.** Only 6 of 444 issue docs carry
      ...
      `plans/archive/2026_08/issues/*.md` to populate `author` from each doc's git-log author or the Progress Log's most
      recent entry, defaulting to `unknown` where neither source exists. done-when: ≥90% of issue docs carry `author`,
      QG green (no new violations — elective fields don't gate). Repo: unified-trading-pm. —
