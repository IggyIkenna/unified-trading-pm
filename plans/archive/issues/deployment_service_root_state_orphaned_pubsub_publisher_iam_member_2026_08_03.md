...
- **context-scout 2026-08-03**: refreshed context_scope (3 entries, unchanged) — verified all 3 pre-existing entries
  ...
  while it was still `plans/archive/issues/`; landed here after the doc's own concurrent archival to `status: resolved`
  — context_scope is no longer operationally load-bearing now that the doc is out of context-scout's in-scope status
  set, kept only as historical record.
