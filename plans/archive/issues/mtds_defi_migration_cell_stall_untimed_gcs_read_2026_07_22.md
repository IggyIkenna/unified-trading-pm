...
## Addendum 2026-07-24 (tick 3) — the 9 cells migrated clean (errors=0); manifest still shows the stale glued ids

**Code**: `--stall-timeout-seconds` CLI/env override shipped (`market-tick-data-service@5218837e`) with a regression
test. Separately, the actual unblocking fix was leaf-write parallelization: `2ba4226e`+`57c05125`+`826cd6c1` (sequential
per-instrument writes -> shared/chunked parallel pool). Migration retry chain (`defi-pi-range` launcher, reconstructed
from GCS `run.log`/`LAUNCH_PARAMS.json`): `leafparallel` (2025-12-23, `cells=1 errors=0`) + `lpar3`(STALL, pre-fix
tarball) + `lpar5` (2025-12-24..31, `cells=3 errors=5`, cleared 12-27/29/30) + `lpar6` (STALL, still-stale tarball) +
`lpar7` (2025-12-24..31, `cells=5 errors=0`, cleared the remaining 24/25/26/28/31). Cumulative: all 9 target ORCA cells
migrated, 0 residual errors, confirmed 2026-07-24 10:20:53 UTC. A scoped manifest rebuild followed
(`canonical-migration-defi-rebuild-20260724-113048-rbverify`, exit 0, 2025-12-23..31).

**Sequencing check (this pass)**:
`gcloud compute instances list --filter="name~canonical-migration-defi" --project=central-element-323112` + an AWS
ap-northeast-1 sweep showed no live competing VM. No new VM launch was needed — the retry chain above had already run in
prior turns this session; discovered via GCS logs, not re-executed.

**Fresh `verify_defi_glued_ids_2026_07_24.py` run (this pass)**: **`GLUED-ID ROWS FOUND: 21`** — UNCHANGED. Same 9 ORCA
`dex_pool_state` rows (`orca_SOLANA_2025-12-23`..`2025-12-31`) + same 12 `liquidations` rows (dated 2026-07-22,
pre-`f2e3ad41`, already a known separate gap).

**Root cause of the still-present 9 ORCA rows (code-read, not inferred) — NOT a migration regression, a different,
previously-undocumented manifest-retraction gap**: `rebuild_defi_manifest.py::scan_and_rebuild()` skips every
`_`-prefixed leaf (`_migrated_*`) so it never RE-EMITS a new row under the old glued id — confirmed working. But its
CF-11 dedup only supersedes a prior NON-captured row sharing the exact
`(date, venue, data_type, instrument_type, instrument_id, chain)` key; the old glued row
(`instrument_id=orca_SOLANA_2025-12-23`, `capture_status=captured`) has a DIFFERENT `instrument_id` than the new
per-instrument rows, so it's never a supersession candidate. Nothing in the pipeline — migration, scoped rebuild, or
`delete_migrated_defi_markers_2026_07_23.py` (GCS-objects-only per its own docstring, never touches the manifest) —
retracts the pre-migration row once its source is renamed away; it persists alongside the new correct rows. Not tested:
whether a FULL (non-scoped) rebuild truncates-and-regenerates instead of upserting (this scoped 9-day run was
additive-only, confirmed via its own `ManifestWriter ... N new` log lines).

**Consequence**: `delete_migrated_defi_markers_2026_07_23.py --apply` would NOT clear these 9 rows (GCS-only). A
distinct manifest-row-level retraction (not yet built) is needed before ORCA's glued-id count reaches 0 — flagged as a
next step, not executed here (out of this task's verify+document scope).

**Doc-update note**: `plans/active/defi_consolidated_closeout_2026_07_18.md`'s "21 glued-id rows" todo could NOT be
updated this pass — that file is 1546 lines, over `check_line_caps.sh`'s 1000L HARD cap (already tracked,
`plan_line_cap_remediation_2026_07_23.md` row 11), and prek hard-blocks any staged commit touching it. This addendum is
the durable record until that plan is split/trimmed. `defi_track01_per_instrument_and_canon_id_2026_07_24.md` (656L,
under cap) WAS updated directly.
