...
> `2000`→`5000`, QG green. This doc was left `status: resolved` in `plans/archive/issues/` instead of being archived;
> archiving now per the issue-doc-lifecycle rule (a `status: resolved` doc must not sit in `plans/archive/issues/`) to
