...
> **🟢 RESOLVED 2026-07-28** — Option A shipped: `_invalidate_index_cache` (`_state.py`) now evicts every OTHER bucket's
> `_CANONICAL_CACHE` entry on each write-triggered call, capping the cache to the single active bucket instead of
> pinning one multi-GB DataFrame per bucket touched for process lifetime. Shipped `unified-trading-library@0db19a72`
> with regression tests (`test_invalidate_index_cache_evicts_other_buckets_canonical_cache`,
> `test_invalidate_index_cache_noop_on_single_bucket` in `tests/unit/test_manifest_read_index_slim.py`);
> `quality-gates.sh` green. Option B (streaming/iterative per-VM fan-in merge reduce) remains a separate, larger,
> still-open follow-on if the fleet needs it — not silently implied done by this fix. The duplicate `[UTL] P2` todo
> tracking this same fix in `cross_cutting_satellite_ao_dispatch_batch1b_2026_07_26.md` is flipped in the same edit.
...
- [x] ✅ [INFRA] P2. **Implement a durable fix for the un-evicted `_CANONICAL_CACHE`** — per "Recommended decision"
      Option A (cap the cache to the current bucket, evict on bucket-change) or Option B (stream/iteratively reduce the
      per-VM fan-in merge); only Option C (config mitigation) has shipped so far. — **DONE 2026-07-28**:
      `unified-trading-library@0db19a72` — Option A shipped, see banner above.
