...
depends_on: []
resolved_by:
  "Acked-into-plan: the seeder design/build (BLK-7c950d06, ruled Option A 2026-07-26) is tracked in
  defi_expected_unattempted_seeder_design_2026_07_26.md. Every follow-up todo in this doc is closed:
  agent-orchestrator@c9f805c (M3 /done gate exception), market-tick-data-service@7d90c119 +
  unified-api-contracts@2ceae9e9 (Morpho ARBITRUM wiring), plus the read-only reconciliation todos (no code change
  needed). C8's stale done-criteria corrected in data_completion_defi_2026_07_15.md and
  defi_consolidated_closeout_aggregated_sources_2026_07_24.md."
locked_by:
---
...
- **[DATA] P2. CANCELLED — SUPERSEDED 2026-07-26 (BLK-7c950d06 + BLK-3221d4b3, operator-ruled Option A).** Original ask:
  design + build a DeFi `expected_unattempted` seeder (mirrors `sentinels.py`'s `record_expected_unattempted` pattern)
  keyed off UAC's `DEFI_VENUE_DATA_TYPE_CAPABILITIES` + `DEFI_VENUE_PHASE`. **RULED 2026-07-26 (BLK-7c950d06: Option
  A)**: this is the correct direction, tracked now as its own human plan
  `defi_expected_unattempted_seeder_design_2026_07_26.md` (assigned_vm: NA per BLK-3221d4b3 — the
  capability-vs-collectibility reconciliation, see FLUID finding #5, is an open-ended per-venue judgment call, not
  AO-dispatchable until an operator resolves it). Retagged off BLOCKED-OPERATOR-DECISION 2026-07-28 per the 2026-07-28
  stale-gate audit — the decision itself is resolved; this line stays non-dispatchable and superseded-by-tracking, the
  actual work happens in that plan, not here. (repo: market-tick-data-service)
...
- [x] ✅ [DATA] P3. **DONE 2026-07-26 (slot-2) — claim is STALE, live-verified.** Queried Morpho Blue's live GraphQL API
      ...
      supply >$1,000 — unambiguous: the top market (`USDC`/`K`) alone carries
      **~$3.0B supplied /
      ~$3.0B borrowed**, not a rounding artifact. - **OPTIMISM**: 6 total, 5 major-asset, 3 with
      supply >$1,000
      (top: `WETH`/`wstETH` at
      ~$117k) — modest but real. - **POLYGON**: 202 total, 134 major-asset, but
      only 2 with supply >$1,000, and
      both are single-sided "idle" markets (`collateralAsset: null`, i.e. not a genuine lending pair) — still
...
- [x] ✅ [DATA] P2. **DONE 2026-07-26 (slot 4)** — **NEW (found 2026-07-26 while confirming the todo above)** — Wire
      ...
      The Graph, but the comment must stop asserting a now-false claim). Do NOT add OPTIMISM/POLYGON in the same change
      — OPTIMISM has only ~$117k
      max single-market liquidity and POLYGON's
      only >$1k entries are single-sided idle
      markets (no real collateral-paired liquidity yet); re-check both again in a future pass rather than wiring in what
      may still be noise. Repo: market-tick-data-service, unified-api-contracts. Done when: `_CHAIN_ID_BY_CHAIN`
      includes ARBITRUM, a live smoke-fetch confirms real ARBITRUM Morpho instruments get discovered, the stale comment
      is corrected, and `quality-gates.sh` is green in both repos. **Evidence**: `_CHAIN_ID_BY_CHAIN` now
      ...
      ARBITRUM has many single-sided "idle" markets (`collateralAsset: null`), unguarded in the existing code; filtered
      these out in `_parse_markets_response` before conversion (not genuine lending pairs anyway). Live smoke-fetch
      post-fix: **50 real ARBITRUM instruments discovered**, including `K-USDC` at `total_supply=2999125223832587` raw
      units (÷1e6 USDC decimals ≈ **$2.999B**,
...
- [x] ✅ [DOCS] P3. **DONE 2026-07-26 (slot 9)** — Corrected `data_completion_defi_2026_07_15.md` item C8: dropped the
      ...
      and the FRAX category-mismatch framing, with a pointer to this doc's full re-diagnosis. A `plans/archive/2026_08/`
      grep for the same stale framing turned up one more instance —
...
- 2026-07-26 (slot 2): Closed the P3 MORPHO-ARBITRUM/OPTIMISM/POLYGON confirmation todo. Live-queried
  ...
  ~~$3.0B `USDC`/`K`
  market (74 total major-asset markets, 16 with >$1k liquidity); OPTIMISM has modest real liquidity
  (~~$117k max);
  POLYGON's >$1k entries are single-sided idle markets only, not genuine paired liquidity. Mechanism
