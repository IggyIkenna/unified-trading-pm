...
resolved_by: "AO issue-doc sweep 2026-08-06 — remaining [REVIEW] P3 item descoped (self-admittedly non-actionable-standalone); every other todo independently re-verified live in code."
locked_by:
context_scope:
  [
    /plans/active/issues/fleet_wide_qg_self_hosted_runner_capacity_crisis_2026_07_27.md,
...
> **🗄️ ARCHIVED 2026-08-06** — fully resolved. Swap/iowait dashboard observability shipped
> (`agent-orchestrator@4742ce2`), resource-history sampling installed + verified live (`@231125b`/`@d9a23da`), instance
> resize + memory-cap fixes applied and verified, `features-service` dirty-worktree quarantine root-caused and fixed
> (`@f2b6d73`). The sole remaining `[REVIEW] P3` item was self-admittedly non-actionable-standalone and descoped in the
> same edit. No open todos, no Deferred items. Surfaced by the 2026-08-06 AO issue-doc sweep.
...
- [x] [REVIEW] P3. **🚫 DESCOPED 2026-08-06** — self-admittedly not worth a standalone action ("not worth a dedicated
      edit on its own given the line cap") and the crisis doc it references remains available for anyone to fold this
      into opportunistically; not tracked as a blocking item. Original text: Consider whether the crisis doc's "What I
      found" section is worth a one-line correction the next time
