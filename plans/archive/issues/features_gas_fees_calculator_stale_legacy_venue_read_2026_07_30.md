...
summary: >-
  ...
  (`plans/archive/2026_08/issues/defi_gas_fees_historical_venue_path_migration_2026_07_28.md`): a grep+READ Part-4
  ("does anything still read the legacy path") turned up
...
related:
  [
    plans/archive/2026_08/issues/defi_gas_fees_historical_venue_path_migration_2026_07_28.md,
...
- **2026-07-30 (data_engineering slot-7)**: found via delete-safety Part 4 grep+READ sweep (sub-agent search across 4
  ...
  proof recorded in `plans/archive/2026_08/issues/defi_gas_fees_historical_venue_path_migration_2026_07_28.md`.
...
- **2026-08-05 (data_engineering slot-12)**: P3 started. Code complete + verified (gas_fee_handler.py: removed
  ...
  fully documented in `plans/archive/issues/mtds_qg_red_combined_coverage_shortfall_2026_08_05.md`: (1) MTDS has 2
...
- **2026-08-05 (data_engineering slot-12) RB GREEN — UAC removal shipped, MTDS re-gate in progress**: the two
  ...
  `plans/archive/2026_08/issues/mtds_qg_red_uac_capability_declaration_drift_2026_08_05.md`). Blocker RB-04b8981e
  unblock criterion (MTDS QG green on LDR baseline) is being re-checked: full `bash scripts/quality-gates.sh` WITH the
  gas_fee diff running (started ~14:04Z, log `/tmp/qg_green_ship.log`) — the editable-install view now includes
  `5f441e0d`, so the 2 failures should clear. **NEXT STEP (the only remaining work)**: when that QG is green, ship the 3
  code files via
...
- **2026-08-05 (data_engineering slot-12) RE-GATE CAME BACK RED — P3 STILL BLOCKED, next step updated**: the MTDS
  ...
  **9989 passed / 1 failed**, coverage 80.63% (PASSES). The `[lending]` failure cleared
  ...
  `plans/archive/issues/mtds_qg_red_combined_coverage_shortfall_2026_08_05.md` Progress Log (RE-GATE RESULT entry,
...
- **2026-08-05 (data_engineering slot-12) PRE-COMPACT HANDOFF — P3 STILL BLOCKED on 2 UPSTREAM RATCHET REDS, owner =
  ...
  `plans/archive/issues/mtds_qg_red_combined_coverage_shortfall_2026_08_05.md` (RE-GATE #5-#8 entry, committed
...
- **2026-08-05 (data_engineering slot-12) RE-GATE #9 (AUTHORITATIVE) — P3 STILL BLOCKED; the `cec16b74` "fixed en route"
  ...
  `plans/archive/issues/mtds_qg_red_combined_coverage_shortfall_2026_08_05.md` Progress Log.
