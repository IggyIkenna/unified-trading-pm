...
superseded_by:
resolved_by:
  Corpus-wide grep + classification (2026-07-26) found NO VIOLATION anywhere — every coverage-percent site satisfies the
  symmetric-inclusion invariant. `/codex/02-data/honest-coverage-model.md` updated 2026-07-28 with a new "A third
  sanctioned pattern -- attempt_coverage_pct" subsection documenting the 3rd (previously undocumented) formula pattern
  and its 5 live call sites.
source: /plans/active/data_pipeline_e2e_milestones_gate_2026_07_24.md §10
depends_on: []
---

> **🟢 RESOLVED 2026-07-28** — NO VIOLATION found corpus-wide; codex SSOT updated with the 3rd sanctioned formula
> pattern. Archived per `/codex/11-project-management/issue-doc-lifecycle.md`.
...
- [x] ✅ [DOC] P3. `/codex/02-data/honest-coverage-model.md` § "Coverage formula" names only `reachable_coverage`/
      ...
      (audited 2026-07-26). — **DONE 2026-07-28.** Added a new "A third sanctioned pattern — `attempt_coverage_pct`"
      subsection to `/codex/02-data/honest-coverage-model.md` § "Coverage formula" (right after the symmetric-inclusion
      invariant paragraph), documenting the formula + all 5 live call sites named in this todo's own finding 3
      (`coverage_metrics.py`, `manifest.py`, `rollup_cache.py`, `data_status_mock.py`, `mtds.py`, `coverage_drift.py`).
