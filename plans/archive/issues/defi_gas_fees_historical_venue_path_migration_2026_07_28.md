...
> **ARCHIVED (2026-07-30) — complete.** Migration done: 12,424 legacy `gas_fees` manifest rows copied to canonical
> `venue=ALCHEMY` twins (`market-tick-data-service@8016c7e4`), the one live legacy-scheme reader fixed
> (`features-service@48f77f2a`), 5-part delete-safety proof staged. The actual prod-bucket delete stays
> `[OPERATOR]`/human-only per this doc's own ruling — that step was never this doc's scope to execute.
