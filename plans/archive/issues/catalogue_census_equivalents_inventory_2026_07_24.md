...
context_scope:
  [
    ...
    features-service/features_service/delta_one/schemas/feature_builder_registry.py,
...
> archive-on-resolve rule. All 3 todos [x]: written inventory DONE (2026-07-26), features-catalogue drift census shipped
> (features-service@79b83202), UAC-registry drift census shipped (unified-api-contracts@057017e8, 6 cross-registry
> checks, 336 findings). Every gap found by the inventory was converted to a tracked shipped todo. No prose-only open
> work or deferred follow-up. Moved by the 2026-08-06 AO issue-doc archive sweep.
...
- [x] ✅ [REVIEW] P2. Written inventory (no code changes): for each of the strategy catalogue (strategy-service
      ...
      **Baseline** (`deployment-api`): `routes/data_status/_axis_census.py::get_axis_value_census` — raw distinct-value +
              ...
              — fixed 2026-08-06 (/plan-reconcile ao): this whole block (Baseline paragraph + numbered findings 1-4) had
              runaway leading whitespace (1174-1177 spaces/line) causing markdown code-block rendering; re-indented to the
              normal 6-space continuation indent, text preserved exactly.

- [x] ✅ [DATA] P3. Build a features-catalogue drift census: enumerate each family's actually-registered
      ...
      expected-feature manifest, mirroring `_axis_census.py`'s shape — features-service@79b83202. Source: this doc's
      finding 2 (audited 2026-07-26).
- [x] ✅ [DATA] P3. Build a UAC-registry drift census covering `VENUE_TO_ADAPTER_KEY`, `MVP_SCOPE`,
      capability-declaration registries, and per-asset-group venue-manifest tables: flag a registered-but-unadapted
      venue, an adapter key with no matching capability declaration, or an MVP-scope entry with no live producer —
      unified-api-contracts@057017e8. `unified_api_contracts/registry_census.py`: 6 cross-registry checks
      (NO_ADAPTER_YET venues, adapter↔capability, MVP↔adapter, venue-manifest↔adapter, VENUES_BY_ASSET_GROUP↔adapter,
      PROTOCOL_TO_ADAPTER_KEY integrity). CLI with --json flag, exits non-zero on errors for CI/gate integration.
      Verified: run produces 336 findings (0 errors, 113 warnings, 223 info) against current registries. Source: this
      doc's finding 4 (audited 2026-07-26).

## Progress Log

- **context-scout 2026-08-01**: populated/refreshed context_scope (4 entries).
- **context-scout 2026-08-03**: refreshed context_scope (6 entries, unchanged) — verified all still accurate and
  resolve; the two open P3 todos (features-catalogue drift census, UAC-registry drift census) each map directly to an
  existing entry (`feature_builder_registry.py`, `unified_api_contracts/registry`) plus the `_axis_census.py`/
  `_distinct_values.py` pattern-to-mirror pair, so no change was needed.
