...
locked_by:
resolved_by: >-
  plan_reconciler 2026-08-02 — all 3 todos [x] with concrete evidence (unified-trading-pm@3a5b294ef for the
  regression-check todo, cross-verified against the sibling
  ag_closeout_linkage_gate_blind_to_four_tranches_2026_07_30.md which independently corroborates the same fix landing in
  the same commit). The ~48-doc delta this issue tracked is fully classified (Group A: 15 already-covered; Group B: 41
  freshly classified with a named tranche + reason each).
source: >-
...
- **plan_reconciler 2026-08-02** (whole-corpus pass): verified all 3 todos genuinely done with HARD evidence (shas
  cross-checked against the sibling `ag_closeout_linkage_gate_blind_to_four_tranches_2026_07_30.md`, which independently
  corroborates the same commit). Flipped `status` `open` -> `resolved`, archived. Fixed 4 corpus referrer occurrences
  across 3 files to the new path.
