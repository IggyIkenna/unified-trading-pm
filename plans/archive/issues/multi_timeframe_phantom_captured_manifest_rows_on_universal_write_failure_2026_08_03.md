...
> archive-on-resolve rule. All 5 todos [x] with shipped fixes: per-group tracking + per-group manifest scoring
> (features-service@7eca96ac), root-cause fixes @31bef7c3, calculator-set design ruling @94b64472, and record_failed
> manifest path @b0be030b verified against the real -test bucket (row reads back attempted_failed) — phantom-captured
> bug confirmed fixed, no prose-only open work. Moved by the 2026-08-06 AO issue-doc archive sweep.
