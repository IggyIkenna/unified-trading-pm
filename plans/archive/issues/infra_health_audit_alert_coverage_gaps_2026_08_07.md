...
doc_type: issue
archive_banner: |
  Archived 2026-08-11 — all 5 todos complete. Findings 4 & 8 confirmed benign
  threshold gap (no wiring bug). Resolved by infra_health_audit_alert_coverage_gaps-005
...
summary: >-
  ...
  **(C) AWS IAM/STS failures have zero AlertCode coverage** — grepped the full closed `AlertCode` set
  ...
  DIFFERENT: a conceptually-matching rule DOES exist (DP-VM-003 stall / DP-VM-001 OOM) but apparently didn't cross its
  miss-tracker threshold before self-recovery/investigation — not filed as a coverage gap here, just noted (todo 4
  below) since root-causing "why didn't the existing rule fire" needs the live miss-tracker state, not a Slack/code
  read.
status: resolved
nature: issue
...
supersedes:
depends_on: []
resolved_by: "2026-08-11 — all 5 todos complete; findings 4 & 8 confirmed benign threshold gap (no wiring bug)"
...
| 4   | `mtds-dex-swaps-backfill-2` idle/hung VM (finished ~07:50:33Z today)                                | **no**                                | Zero Slack hits for the VM name. DP-VM-003 (`DP_VM_STALL`, no-heartbeat detector) conceptually applies but apparently didn't cross its miss-tracker threshold — see note below, not filed as a gap here.                                                                                                                                       |
...
| 8   | `mtds-backfill-odds-401-retry` self-recovering OOM (every 7-9min today)                             | **no**                                | Zero Slack hits for the VM name. Same DP-VM-003/001 miss-tracker-threshold caveat as #4.                                                                                                                                                                                                                                                       |
