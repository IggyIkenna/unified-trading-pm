...
> archive-on-resolve rule. All 5 todos [x]: scope bug fixed (instruments-service@7409c5b1, _FOOTBALL_DATA_TYPES wired
> in), operator ruling applied + re-baselined (11,403/755), §U population reconciled as disjoint-by-construction
> (instruments-service@153063e4), corpus hygiene done (unified-trading-pm@dab1db5ec), [OPERATOR] mechanical-park todo
> closed RESOLVED-MOOT. No purge was executed (correctly, per STOP-and-report instruction). No prose-only open work or
> deferred follow-up. Moved by the 2026-08-06 AO issue-doc archive sweep.
...
- [x] ✅ [REVIEW] P3. **UNPARKED 2026-08-03 — items 1+2 above both landed, this is now actionable** —
      `unified-trading-pm@dab1db5ec` (was ⏸ PARKED 2026-08-02, main's BLOCKED-answer Option A, behind items 1+2). Corpus
      hygiene — update the closeout's G1 sub-item. After the two todos above land, update
