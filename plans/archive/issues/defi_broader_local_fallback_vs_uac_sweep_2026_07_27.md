...
> archive-on-resolve rule. Both todos [x]: the sweep of all 4 DeFi repos found 0 genuine UAC shadow drifts (all 42 sites
> clean — token decimals have no UAC SSOT, chain genesis byte-identical, factory addresses proper UAC cascade) and the
> drift-check todo's precondition was unmet, so 'Issue is fully addressed.'. Moved by the 2026-08-06 AO issue-doc
> archive sweep.
