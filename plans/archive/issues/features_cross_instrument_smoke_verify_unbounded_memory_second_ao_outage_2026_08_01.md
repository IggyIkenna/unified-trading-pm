...
- **na-eligibility-audit 2026-08-01** (autonomous, tranche `ao`, dispatch agt-8e95ca, slot 2): RECLASSIFY
  ...
  `plans/archive/2026_08/issues/features_e2e_smoke_matrix_writes_to_prod_bucket_2026_08_01.md` (active
  `assigned_vm: planning`) explicitly parks its own P2 re-verification todo on THIS doc's `[INFRA] P2` fix landing ("Do
  NOT flip this checkbox or unpark before then") — not a duplicate claim, but a genuine blocking prerequisite for
  already-dispatched AO work, which argues FOR reclassifying promptly rather than against it. No competing claim found
  on the `[DATA] P2`/`[INFRA] P2`/`[DATA] P3` todos themselves.
