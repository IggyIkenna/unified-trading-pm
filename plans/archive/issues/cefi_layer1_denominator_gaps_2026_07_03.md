...
resolved_by: "all own todos done; residual OKX-SPOT/DERIBIT-COMBO threads tracked elsewhere -- see 2026-07-25 banner"
locked_by:
...
> **🟢 RESOLVED 2026-07-25** — all of this doc's own todos are done; the two genuinely-still-open threads a deeper
> cross-doc investigation confirmed are tracked (with full context) in OTHER active docs, not lost by archiving this
> one: (1) the OKX-SPOT Option-A-vs-B design decision lives in
> `plans/active/issues/instruments_service_cefi_qg_red_on_ldr_head_2026_07_08.md`; (2) the DERIBIT-COMBO real-row
> capture is gated on the already-tracked Tardis concurrent-IP-lock P0
> (`plans/active/issues/tardis_concurrent_ip_lockout_2026_07_12.md`). v1-enumerator retirement and the DERIBIT-COMBO
> catalogue backfill are fully shipped. Separately surfaced (not this doc's scope, reported to the operator instead):
> `plans/archive/issues/bybit_spot_manifest_stray_captures_2026_07_07.md` was flipped `resolved` on 2026-07-14 on
> checkbox-only evidence, but a 2026-07-10 live-manifest read (`instruments_remaining_work_audit_2026_07_10.md` item 7)
> found the actual `--apply` relabel/delete was never run against production (135,444 anomalous BYBIT-SPOT rows
> unchanged) — that doc's status likely needs correcting independent of this archival.
