...
> **🟢 RESOLVED 2026-07-28** — `read_availability_index` (and its slim-path sibling `_read_availability_index_slim`,
> both branches) now resolve `get_storage_client()` OUTSIDE the broad
> `except (FileNotFoundError, OSError, ValueError, pd.errors.ParserError)`, so a config `ValueError` from
> `get_project_id()` (missing `GCP_PROJECT_ID`/`AWS_ACCOUNT_ID`) raises loudly instead of being folded into "index
> empty". Shipped `unified-trading-library@0db19a72`, `quality-gates.sh` green. Done-when unit tests added:
> `test_full_read_raises_on_missing_gcp_project_id`, `test_slim_read_raises_on_missing_gcp_project_id`,
> `test_slim_read_with_filters_raises_on_missing_gcp_project_id` (config ValueError raises) plus
> `test_full_read_missing_bucket_still_returns_empty_not_raise`,
> `test_slim_read_missing_bucket_still_returns_empty_not_raise` (existing 404/corrupt-parquet-return-empty contract
> unchanged) in `tests/unit/test_manifest_read_index_slim.py`.
...
- [x] ✅ [CODE] P3. In `unified_trading_library/manifest_writer/_read_index.py`'s `read_availability_index` (and its
      ...
      **Done when**: a unit test asserts that calling `read_availability_index` with `GCP_PROJECT_ID`/`AWS_ACCOUNT_ID`
      ...
      pass. (repo: unified-trading-library) — **DONE 2026-07-28**: `unified-trading-library@0db19a72` — see banner above
      for the exact done-when unit tests added; all pre-existing tests unchanged/green.
