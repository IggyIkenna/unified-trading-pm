...
- **na-eligibility-audit 2026-07-30 (prediction tranche)**: **RECLASSIFY — `assigned_vm: NA → planning`, conflict-check
  ...
  **Finalize-plan coverage**: not required — `doc_type: issue` under `plans/archive/2026_08/issues/` is structurally
  exempt (`check_finalize_plan_coverage.py` globs `plans/archive/2026_08/*.md` only).
