...
adopted it as a stopgap (grep for `LC_RUNTIME_SA=` in `plans/archive/2026_08/issues/` once this is closed).
...
- [ ] [DOCS] P3. Grep `plans/archive/2026_08/issues/` for `LC_RUNTIME_SA=` and drop the workaround from any
      runbook/monitoring instructions that adopted it as a stopgap, now that the underlying grant is durable.
