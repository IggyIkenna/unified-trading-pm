...
> archive-on-resolve rule. All 4 todos resolved (Baker Hughes adapter fix, calendar dead-code deletion, sentiment
> operator ruling applied, CL `weather_delta` factor-coverage fix). Moved by a `cicd` escalation (agt-5c37f6) triaging
> the `check_archive_candidates` hard gate failure. No content was rewritten.
