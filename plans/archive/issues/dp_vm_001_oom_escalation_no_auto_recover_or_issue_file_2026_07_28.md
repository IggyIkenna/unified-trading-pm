...
locked_by:
locked_since:
resolved_by: "slot 6, review, 2026-07-28 (all 4 todos landed — 3 backend investigate/wire/verify + 1 CLAUDE.md pointer)"
---
...
- [x] ✅ [REVIEW] P1. Once the above lands, add one line to CLAUDE.md's "Launching VMs / infra?" domain-index pointing
      to `data-pipeline-alerts.md` as the SSOT for VM/monitoring-tool escalation design, so future agents building new
      ...
      **DONE.** Added `**VM/monitoring-tool escalation design** (auto-recover-before-page, file-issue-worthy classes) →
                  `…/data-pipeline-alerts.md`.` to the "Launching VMs / infra?" bullet's SSOTs list in
                  `cursor-configs/CLAUDE.md`. CLAUDE.md was already at 40,941 B against the QG-enforced 40,960 B (40 KiB) hard cap
                  (`scripts/quality_gates/check_agent_rules_size_cap.py`), leaving only 19 B of headroom — condensed 3 nearby
                  phrases in the SAME bullet (registry-miss wording, the manual-preemption-check tail, the Tardis "full numbers in
                  the SSOT" aside) with no loss of the underlying rule, then added the new pointer. Verified:
                  `.venv/bin/python scripts/quality_gates/check_agent_rules_size_cap.py` → `cursor-configs/CLAUDE.md: 40,949 B /
                  cap 40,960 B` — `✅ all agent-rules files within hard cap` (post-prettier size). — unified-trading-pm (this
                  commit)
