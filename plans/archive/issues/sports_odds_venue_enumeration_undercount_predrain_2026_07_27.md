...
summary: >-
  ...
  incomplete list.
status: resolved # was: open — archived 2026-08-08, sole todo SUPERSEDED by operator ruling retiring the instrument_type split
nature: issue
asset_group: [sports]
stage: [data]
...
assigned_vm: NA
resolved_by:
  "resolve-round5-sports 2026-08-08 — operator ruling retired the EXCHANGE_ODDS/FIXED_ODDS split; owned by
  sports_taxonomy_p1_capture_and_contracts_2026_08_08.md + sports_taxonomy_p2_migration_2026_08_08.md"
context_scope:
  [
    /plans/archive/2026_08/sports_closeout_exchange_fixed_odds_fork_2026_07_25.md,
...
> **🗄️ ARCHIVED 2026-08-08** — sole todo SUPERSEDED by operator ruling (see banner below): the EXCHANGE_ODDS/FIXED_ODDS
> split is retired, not extended to the 19 unmapped venues as originally asked. Owned by
> `/plans/active/sports_taxonomy_p1_capture_and_contracts_2026_08_08.md` (contract) and
> `/plans/active/sports_taxonomy_p2_migration_2026_08_08.md` (purge).

> **✅ OPERATOR RULING 2026-08-08 — this doc's sole open todo is resolved MECHANICALLY; no per-venue classification.**
> The ask was "classify all 19 previously-unmapped bookmaker venues as EXCHANGE_ODDS or FIXED_ODDS". The operator ruled
> instead that **exchange-vs-sportsbook is a property of the VENUE, not the instrument** — UAC `SportsVenueType` already
> encodes it — so the `exchange_odds`/`fixed_odds` **instrument_type split is RETIRED and derived at read time**, and
> the stranded values are purged from both manifest rows and GCS objects. No venue needs hand-classifying. **Count
> correction**: this doc says 19 unmapped venues; the live prod manifest measured 2026-08-08 says **21** venues still
> carry only the pre-fork `odds` token (~290k shards), and the fork is only 16% complete (60,095 of 375,257) and split
> WITHIN venues — BETFAIR_EX_UK holds 9,204 `exchange_odds` AND 3,405 pre-fork `odds`. Re-measure at run time; the
> manifest is authoritative. Owned by `/plans/active/sports_taxonomy_p1_capture_and_contracts_2026_08_08.md` (contract)
> and `/plans/active/sports_taxonomy_p2_migration_2026_08_08.md` (purge).
...
- [x] ✅ SUPERSEDED 2026-08-08 (see OPERATOR RULING banner above) — the operator ruled a different disposition than this
      todo asked for: exchange-vs-sportsbook is a VENUE property (`SportsVenueType` already encodes it), so no per-venue
      classification is needed at all; the `exchange_odds`/`fixed_odds` instrument_type split is retired and derived at
      read time instead, with stranded values purged. Owned by
      `/plans/active/sports_taxonomy_p1_capture_and_contracts_2026_08_08.md` (contract) and
      `/plans/active/sports_taxonomy_p2_migration_2026_08_08.md` (purge), both active/unimplemented as of this edit.
      ...
      same `[OPERATOR]`-adjacent mapping-ruling pattern the fork plan's own todo 1 already used for the first 9
      venues.~~
...
- **resolve-round5-sports 2026-08-08**: RESOLVED — closed the sole open `[DATA] P0` todo as SUPERSEDED, citing the
  OPERATOR RULING banner added above earlier this session by a concurrent agent: the operator picked a different
  disposition (retire the split, no per-venue classification) than what this todo asked for. Verified the two owning
  taxonomy plans (`sports_taxonomy_p1_capture_and_contracts_2026_08_08.md`,
  `sports_taxonomy_p2_migration_2026_08_08.md`) genuinely exist and are `status: active`. Closes round-5 sports item 3.
