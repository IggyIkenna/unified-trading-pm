...
summary: >-
  ...
  step has no owning plan anywhere in `plans/archive/2026_08/`.
status: resolved
nature: issue
...
resolved_by: strategy-service@9a7de7f8, unified-trading-system-ui@35137c88
locked_by:
depends_on: []
---

> **✅ ARCHIVED 2026-07-25** — `status: resolved`, `resolved_by` cites `strategy-service@9a7de7f8` +
> `unified-trading-system-ui@35137c88`, 0 open todos, unlocked. Moved to `plans/archive/issues/` per the
> issue-doc-lifecycle archival ritual.
...
1. **`sports_ml_may_23_2026.epic.md`** — "Strategy backtest" and "Execution backtest (Sports L0 TOB matcher)". The
   ...
   Sports TOB per `plans/archive/2026_07/master_to_live_defi_2026_05_23.md:812`) but no plan runs it end-to-end for
   sports.
...
2. **`sports_e2e_validation_2026_03_27.plan.md`** — `spread_calculator` in FSS, a strategy-service arb backtest, and an
   ...
   anywhere in `plans/archive/2026_08/`.
...
Cross-checked all of `plans/archive/2026_08/*.md` and `plans/archive/issues/*.md` by keyword (`spread_calculator`, "live
mode", "execution backtest", "strategy backtest") — no active plan or issue owns any of this. `sports_master.md`'s own
routing tables account for the ML-training half (`predictions_ml_walk_forward_and_arb_2026_06_20.md`) and the
data/coverage half (`sports_consolidated_closeout_2026_07_19.md`, `sports_master_closeout_2026_07_21.md` (archived)),
but the strategy+execution+live-mode half was never given a routing entry.

# Why it matters

Per `plans/archive/2026_07/master_to_live_defi_2026_05_23.md`'s asset-group readiness ladder, Sports and Prediction are
...
**Verdict: RESOLVED (fully investigated + split into tracked follow-ups; this doc's own job is done).** All 7 todos were
already checked `[x]` with dispositions ranging from "not orphaned, functionality shipped under different names" to
"genuine gap, spun into its own scoped plan/issue" — re-verified every spun-off artifact this doc's todos claim to have
created actually exists on disk in `unified-trading-pm`:

- `plans/active/sports_group_c_execution_backtest_harness_2026_07_21.md` — present, active.
- `plans/active/sports_arb_decay_window_and_alpha_gate_design_2026_07_21.md` — present, active.
- `plans/active/sports_predictions_live_mode_activation_readiness_2026_07_21.md` — present, active.
- `plans/active/issues/sports_odds_feature_naming_four_way_mismatch_2026_07_21.md` — present, active.
- `plans/active/issues/sports_arb_dutching_engine_not_wired_to_factory_2026_07_21.md` — present, but already moved to
  `plans/archive/issues/` with `status: resolved` (a separate, already-closed loop — not a gap).

Also spot-checked the two code-verification claims cited in the todos: `strategy-service@9a7de7f8` (fixture +
`run_sports_arb_backtest.py` script) and `unified-trading-system-ui@35137c88` (SPORTS_ML/PREDICTION widget test cases) —
both commits exist in their respective repos' history.

This doc was itself a triage/investigation doc whose deliverable was correctly routing 4 real gaps into 4 separately-
scoped, still-active plans/issues rather than trying to fix everything inline — that routing is intact and none of the
downstream docs have gone missing or silently stalled. Flipped `status: open` → `resolved` and filled `resolved_by:`
accordingly; the substantive follow-up work now lives (and should be tracked) in the 4 downstream docs above, not here.
No contradiction with any other doc found.
