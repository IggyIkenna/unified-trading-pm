...
> archive-on-resolve rule. Single `[x]` todo shipped: `--json` now includes the full `candidates` array
> (unified-trading-pm@05ba63832) with the done-when (existing keys unchanged, QG green) met — no remaining prose open
> item. Moved by the 2026-08-06 AO issue-doc archive sweep.
