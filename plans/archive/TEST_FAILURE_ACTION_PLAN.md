...
UFCL only exports `FeatureCalculator`. **Fix:** Add aliases in UFCL `__init__.py`:

```python
BaseFeatureCalculator = FeatureCalculator  # backward compat alias
```

Or create `BaseFeatureService` and `FeatureCalculatorRegistry` as proper classes. **Effort:** 1h **Priority:** HIGH —
unblocks 2 services entirely
...
tests try to use real GCS client; cloud service tests mock wrong paths after UCS->UTS rename. **Fix:** Update mock
targets from `unified_cloud_services.*` to `unified_trading_services.*` or mock at the correct abstraction level.
...
- UMI syntax error in databento_batch_jobs.py → FIXED (test_split_libraries.py) **Fix:** Add missing exports to
  respective libraries, or remove tests for unimplemented features. **Effort:** 1-2h **Priority:** MEDIUM
...
- `test_storage.py` (1): Cache filename parse **Fix:** Individual fixes per test — mostly mock path updates and API
  alignment. **Effort:** 3h **Priority:** MEDIUM
...
- **ml-inference-service** has a cross-repo dependency: `orchestrator.py` imports
  `from ml_training_service.models import ModelVariantConfig`. This should be refactored to use a shared types library
  (unified-api-contracts or unified-domain-contracts).
