...
todos:
  - id: config-injection-codex
    content:
      ...
      new domain, service wiring pattern, UI usage guide. (Migrated from config_dynamic_injection.md p4-codex.) RESOLVED
      2026-03-08: File already exists at unified-trading-codex/08-workflows/config-injection.md (358 lines), covers all
      required sections: architecture ASCII, domain schemas (Instrument/Strategy/Client/Venue), adding a new domain,
      service wiring pattern, UI usage guide, anti-patterns, quality gate."
    status: completed
