...
todos:
  - id: gi-update-canonical
    content: |
      Update unified-trading-pm/.gitignore and .cursorignore with the only canonical change:
    ...
    status: pending
  - id: gi-create-sync-script
    content: |
      Create unified-trading-pm/scripts/sync_gitignore.sh.
      Behaviour:
      ...
      Safety rules baked in:
        ...
        - Prints a warning and skips if repo-specific section contains a pattern that
          conflicts with a new canonical rule — do not auto-resolve conflicts.
  ...
  - id: gi-dry-run-audit
    content: |
      Run: bash unified-trading-pm/scripts/sync_gitignore.sh --dry-run
    ...
    status: pending
  - id: gi-upgrade-old-style
    content: |
      Upgrade the 4 old-style repos to the canonical format. For each:
    ...
    status: pending
  - id: gi-create-missing
    content: |
      Create .gitignore and .cursorignore from the canonical template for:
        deployment-ui
        strategy-validation-service
      Use scripts/templates/ as source. Repo-specific section is left empty.
    status: pending
  - id: gi-audit-test-fixture-exceptions
    content: |
      Audit all repos that commit test fixture data files (CSV, feather, parquet) and
      ...
      For each MISSING case, add the negation to the repo-specific section of that
  ...
  - id: gi-apply-sync
    content: |
      Run: bash unified-trading-pm/scripts/sync_gitignore.sh --apply
    ...
    status: pending
  - id: gi-untrack-ignored-files
    content: |
      For every repo that had its .gitignore updated (all 55 git repos), find and remove
      ...
      IMPORTANT safety rules:
        - --cached flag only, never bare git rm — files stay on disk.
    ...
    status: pending
  - id: gi-commit-each-repo
    content: |
      For each modified repo, stage and commit in a single commit:
