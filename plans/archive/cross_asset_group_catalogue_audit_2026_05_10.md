...
- [x] ✅ [DOC] P1. Write ICE US softs venue entry to UAC capability declarations — **RESOLVED-UPSTREAM
      2026-05-17 (slot-8)**: the linked issue `ice_us_softs_dataset_disambiguation_2026_05_14.md` already
      RESOLVED 2026-05-14 by harsh-main during OOM recovery; canonical IFUS.IMPACT bindings shipped at
      `tradfi_symbology.py:103-115` + `TRADFI_DATA_BINDINGS:408-413` (CT/CC/KC/SB/OJ/DX). The "capability
      declaration" half is implicit via DATABENTO's existing `operations.market` enumeration — IFUS.IMPACT
      is a Databento dataset, not a separate source. No new capability_declarations file needed; physical-
      commodity softs are 100% Databento-sourced today (cross-vendor expansion is post-cutover scope).
