...
depends_on: [instruments-service-batch-validation]
todos:
- {id: p1-market-category-enum, content: '- [x] [AGENT] P0. Create/export MarketCategory enum in UIC
    ...
    ', status: done, note: Added to UIC (not UAC) per user direction. Commit c571e96.}
- {id: p1-timeframe-registry, content: '- [x] [AGENT] P0. Create canonical timeframe registry in UAC
    ...
    ', status: done, note: Added to UIC (not UAC). Commit c571e96 + a0f521e (30s/30m/1d fix).}
- {id: p1-instrument-shortcuts, content: '- [x] [AGENT] P1. Add instrument shortcuts to UAC reference data
    ...
    ', status: done, note: Kept as CORRECT-LOCAL CLI shortcuts — not shared reference data.}
- {id: p1-defi-api-urls, content: '- [x] [AGENT] P1. Centralize DeFi API URLs in UAC external/ facades
    ...
    ', status: done, note: Added to unified-features-interface (not UAC). Commit 71b551a.}
- {id: p1-commodity-mappings, content: '- [x] [AGENT] P1. Centralize commodity ticker/CFTC mappings in UAC
    ...
    ', status: done, note: Confirmed CORRECT-LOCAL — UAC lacks these specific mappings.}
- {id: p1-feature-group-lookback, content: '- [x] [AGENT] P1. Centralize FEATURE_GROUP_LOOKBACK in UAC or UTL
    ...
    ', status: done, note: Consolidated to service-local constants.py (CORRECT-LOCAL). Commit 75a279a.}
- {id: p1-qg, content: '- [x] [AGENT] P0. UAC quality gates pass after all Phase 1 additions

    ', status: done, note: UIC + features-interface QG passed. 7 new QG checks added (5.30-5.36).}
- {id: p2-features-onchain, content: '- [x] [AGENT] P0. Fix features-onchain-service (9 violations)
    ...
    ', status: done, note: Highest violation count — 9 issues}
- {id: p2-features-delta-one, content: '- [x] [AGENT] P0. Fix features-delta-one-service (8 violations)
    ...
    ', status: done, note: 'Second highest — 8 issues, heavy internal duplication'}
- {id: p2-ml-training, content: '- [x] [AGENT] P0. Fix ml-training-service (7 violations)
    ...
    ', status: done, note: ''}
- {id: p2-ml-inference, content: '- [x] [AGENT] P0. Fix ml-inference-service (7 violations)
    ...
    ', status: done, note: ''}
- {id: p2-features-volatility, content: '- [x] [AGENT] P1. Fix features-volatility-service (5 violations)
    ...
    ', status: done, note: ''}
- {id: p2-strategy, content: '- [x] [AGENT] P1. Fix strategy-service (4 violations)
    ...
    ', status: done, note: ''}
- {id: p2-execution, content: '- [x] [AGENT] P0. Fix execution-service (2 violations)
    ...
    ', status: done, note: CRITICAL — venue registry is 200+ lines}
- {id: p2-risk-exposure, content: "- [x] [AGENT] P0. Fix risk-and-exposure-service (1 critical violation)\n1. Delete local BetSide enum — import from unified_api_contracts\n2. CRITICAL: UAC uses lowercase (\"back\"/\"lay\"), service uses uppercase (\"BACK\"/\"LAY\")\n   — verify all consumers handle the case change\n3. Fix uncached UnifiedCloudConfig() in api/main.py\n", status: done, note: CRITICAL — enum value mismatch is data integrity risk}
- {id: p2-deployment, content: '- [x] [AGENT] P1. Fix deployment-service (2 violations)
    ...
    ', status: done, note: ''}
- {id: p2-features-cross-instrument, content: '- [x] [AGENT] P1. Fix features-cross-instrument-service (2 violations)
    ...
    ', status: done, note: ''}
- {id: p2-features-commodity, content: '- [x] [AGENT] P1. Fix features-commodity-service (2 violations)
    ...
    ', status: done, note: ''}
- {id: p2-mdps, content: '- [x] [AGENT] P1. Fix market-data-processing-service (1 violation)

    1. Replace local _asset_group_MAP with import from UAC INSTRUMENT_TYPE_FOLDER_MAP

    ', status: done, note: ''}
- {id: p2-position-balance, content: '- [x] [AGENT] P2. Fix position-balance-monitor-service (1 violation)

    1. Move sports commission rates to UAC or centralized reference

    ', status: done, note: ''}
- {id: p2-alerting, content: '- [x] [AGENT] P2. Fix alerting-service (1 violation)

    1. Fix type:ignore[assignment] in router.py:86 — add proper Literal validation

    ', status: done, note: ''}
- {id: p3-qg-all, content: '- [x] [AGENT] P0. Run quality gates on all 14 modified services

    bash scripts/quality-gates.sh per repo. All must pass.

    ', status: done, note: SEQUENTIAL — after all Phase 2 complete}
- {id: p3-workspace-grep, content: '- [x] [AGENT] P0. Workspace-wide verification grep
