...
overview:
  Merged plan for achieving all unit tests passing across T0–T3 and services. Combines per-repo execution workflow with
  root-cause analysis and fix options for ~98 T4 failures and ~18 collection errors.
isProject: false
todos:
  - id: phase1-quick-wins
    content:
      "Phase 1 — High-impact quick wins (~3h, unblocks 43 tests): RC-1: DONE — UFCL already exports
      BaseFeatureCalculator, BaseFeatureService, FeatureCalculatorRegistry from service_base. features-calendar lint
      fixed (RUF002, RUF060). RC-2: Implement DependencyChecker in ml-inference-service + execution-service OR move to
  ...
  - id: phase2-execution-core
    content:
      ...
      forward-looking volume, historical fallback) — 11 fixes. RC-5: Review swap alpha engine vs test expectations for
  ...
  - id: phase4-remaining
    content:
      ...
      unified_order_interface module, VENUE_CATEGORY_MAP to unified_config_interface, generate_strategy_id to
      unified_trading_services. CRITICAL: ml-inference importing from ml_training_service.models — move
