...
**Action:** Add chain-aware validation in both strategy-service (instruction generation) and execution-service (instruction validator). Reject STAKE/UNSTAKE/FLASH\_\*/ATOMIC for BTC-chain instruments. Document in unified-domain-client instruction schema and both service specs.
...
1. Add to api-contracts: `api_contracts/tardis/raw_schemas.py` – TARDIS\_\*\_SCHEMA, TARDIS_SCHEMA_MAP, get_tardis_schema(), get_tardis_required_columns()
