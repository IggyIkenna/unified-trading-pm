...
---
 ...
 0A: Matching Engine — L0 Matcher + Sports Routing ---
  ...
  - id: p2e-features-cross-instrument-impl
    ...
    blocked_by: p0d-move-topology-reader
    note:
      "Completed 2026-03-18. CLI main.py wired to BatchHandler/LiveHandler. BatchHandler reads delta-one parquet from
      GCS input bucket, dispatches to CrossInstrumentFeatureService.compute_features() per group, writes results to GCS
      output bucket. LiveHandler uses topology_reader (get_messaging_protocol/get_storage_protocol). 11 calculators
      already implemented with 360+ tests."
  ...
  # === Phase 4 (DeFi alignment) removed — elysium-defi-system deleted from system ===
  # Handler stubs and reconnection logic items were completed before repo deletion.
  # QG gate for unified-defi-execution-interface still applies.
