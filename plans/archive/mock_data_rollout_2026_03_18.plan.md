...
---
 ...
 Layer 1: Data Ingestion (SEQUENTIAL — instruments first) ---
  ...
  - id: p3-L1-instruments
    content: |
      - [x] [AGENT] P0. instruments-service: Refactored scripts/seed_mock_data.py to use InstrumentGenerator from UIC (unified_internal_contracts.testing.instrument_generator). Generates instruments with real venue rules: Deribit last-Friday expiries, CME quarterly month codes, $500 BTC strike intervals, wrapped tokens (wETH/wstETH) for Aave, deterministic 42-char hex pool addresses, real available_from dates. Generates across a 30-day date range to capture mid-month futures/options expirations, deduplicates by instrument_key (latest wins). Output: instrument_availability Parquet at DependencyChecker path template (by_date/venue partitioning). Added validate_mock_instruments.py self-test: 12,485 checks across 24 venues and 14 instrument types at 100% pass rate. SEQUENTIAL — must complete before Layer 2.
    status: done
...
## Implementation Notes (Updated 2026-03-20)

### InstrumentGenerator Approach

Instead of hardcoded instrument catalogues, instruments-service now uses `InstrumentGenerator` from
`unified-internal-contracts/testing/instrument_generator.py`. This generates instruments with real venue rules:

- **CeFi Spot**: 7 pairs across Binance, Coinbase, Bybit, OKX, Upbit with real ccxt symbols
- **CeFi Perpetuals**: 6 contracts across Binance-Futures, Deribit, Hyperliquid, OKX, Bybit, Aster
- **CeFi Futures**: Deribit BTC quarterly (last Friday expiry), CME ES (HMUZ month codes)
- **Options Chain**: Full BTC options at $500 strike intervals, weekly+monthly+quarterly expiries, CALL+PUT
- **TradFi**: AAPL (EQUITY), QQQ/GLD (ETF), VIX (INDEX), ES futures (CME)
- **DeFi**: Aave V3 aTokens/debtTokens (wrapped tokens: WETH, USDC, USDT), Compound V3, Uniswap V2/V3/V4 pools,
  Lido/EtherFi LSTs, Morpho vaults -- all with deterministic 42-char hex pool addresses
- **Sports/Prediction**: Polymarket binary, Betfair exchange odds, Pinnacle fixed odds

The seed script generates across a 30-day range to capture mid-month futures/options expirations, deduplicating by
instrument_key (latest occurrence wins). Single-day generation produces ~1,063 instruments; 30-day range produces
~1,200+ depending on the expiry calendar around the target date.

### Mock Prefix Architecture

Mock and real data coexist in the same bucket, isolated by prefix:

- Real data: `gs://{bucket}/{path}`
- Mock data: `gs://{bucket}/mock/{path}`
- Prefix determined by UCI `get_data_path_prefix()` which returns `"mock/"` when `DATA_MODE=mock`
- DependencyChecker already handles this via `get_data_path_prefix` -- no service code change needed

### data_mode Migration

Complete across 22 repos. All services now use `config.is_mock_mode()` instead of `config.cloud_mock_mode`. The
`cloud_mock_mode` property emits a DeprecationWarning on access but remains functional as a bridge.

### Cloud Storage Writer

UTL `SeedWriter` (Protocol) with `LocalWriter` and `CloudWriter` implementations. All 21 seed scripts upgraded to use
`get_seed_writer(env, service_name)`. Local mode writes to `.local-dev-cache/mock-seed/{service}/`. Cloud mode writes to
GCS/S3 via UCI `get_storage_client()` at the `mock/` prefix.
