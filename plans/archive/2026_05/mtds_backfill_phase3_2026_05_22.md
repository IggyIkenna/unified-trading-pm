...
- MTDS-3.2.A-V CeFi full verify (BLOCKED-IN-FLIGHT: multiple VMs from FullFleetRelaunch batch still RUNNING) →
  `plans/epics/mtds_mdps_master.md` P2 block — verify once all 95 VMs from batch 021423 terminate (**MIGRATED FROM:**
  mtds_backfill_phase3_2026_05_22)
- MTDS-3.2.C-GapFill-V dex-swaps pending (mtds-dex-swaps-backfill VM-IN-FLIGHT with BugFix3 retry) →
  `plans/epics/mtds_mdps_master.md` P2 block — verify once VM completes (**MIGRATED FROM:**
  mtds_backfill_phase3_2026_05_22)
- MTDS-3.2.B TradFi Databento top-up (BLOCKED-CREDENTIALS: Databento auth_account_locked — 6 datasets returning 403) →
  `plans/epics/mtds_mdps_master.md` P2 block — operator credential action required (reactivate Databento account at
  app.databento.com) (**MIGRATED FROM:** mtds_backfill_phase3_2026_05_22)
