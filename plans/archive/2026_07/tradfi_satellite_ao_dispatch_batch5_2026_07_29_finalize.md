...
> **🟢 ARCHIVED 2026-08-05.** All 3 todos done. Todo 1 (reconcile 15 source docs) complete 2026-08-05 (slot 3); todo 2
> (re-check batch5 Deferred sections) complete 2026-08-05 (slot 13); todo 3 (archive batch5 via 6-step ritual) complete
> 2026-08-05 (slot 16, this archival). The plan it gatekeeps
> (`/plans/archive/2026_07/tradfi_satellite_ao_dispatch_batch5_2026_07_29.md`) is archived alongside it in the same
> commit.
>
...
> draft" and contradicted the frontmatter). Upstream
> `/plans/archive/2026_07/tradfi_satellite_ao_dispatch_batch5_2026_07_29.md` is likewise `status: active` (approved +
> dispatched 2026-07-30, `5a6bbefc3`). Being `active` does NOT mean this plan's todos are dispatchable yet — see the
> machine gate below, which is the only thing holding them.
>
...
- [x] ✅ [REVIEW] P1. **Re-check batch5's own Deferred sections now that time has passed.** — `unified-trading-pm@<sha>`
      (slot 13, 2026-08-05). **First, two corrections landed 2026-08-02 that this re-check must respect rather than
      re-derive** (see batch5's Deferred — conflict-gated section): (i) the legacy-twin bucket delete's premise was
      FALSE — twin coverage measures **0%, not 100%** (dry-run 2026-07-30: 900 class-B twins loaded → 0 deletable, 900
      blocked, all "canonical twin NOT captured in manifest"), and batch1's dry-run HAS landed, so the old "bring it to
      the operator for a go/no-go" recommendation is spent — **do not re-raise it as an operator ask**; the gate is
      technical, not a decision. (ii) **Investigate the unexplained 995 → 900 shrink in the legacy-twin candidate set**
      — the delete's candidate population is cited as 995 class-B rows throughout
      `/plans/active/tradfi_legacy_twin_bucket_deletes_signoff_2026_07_24.md` (from the 2026-07-10 full orphan sweep)
      but the 2026-07-30 dry-run loaded 900 from the same report URI, 95 fewer, with no explanation recorded anywhere in
      the corpus. Determine which of these is true and cite the evidence: the report was regenerated by a later sweep;
...
- [x] ✅ [DOC] P1. **Archive `tradfi_satellite_ao_dispatch_batch5_2026_07_29.md`** via the standard 6-step ritual (per
...
- **2026-08-05 (slot 3, infra→review, task `tradfi_satellite_ao_dispatch_batch5_2026_07_29_finalize-001`) — Todo 1
  ...
  **Two docs flipped to resolved** (docs 5, 13). Two docs had `locked_by` set and were correctly left untouched (docs 7,
  ...
  - **2026-08-05 (slot 13, infra→review, task `tradfi_satellite_ao_dispatch_batch5_2026_07_29_finalize-004`) — Todo 2
    (re-check batch5's Deferred sections) complete.** Each Deferred/observation item re-checked against current state
    (2026-08-05). No item was re-asked to the operator.

    **Two correction items (landed 2026-08-02, re-verified):**
    - (i) Legacy-twin coverage confirmed 0% (not 100%) — the 2026-07-30 dry-run result stands: 900 loaded → 0 deletable,
      900 blocked. The old operator ask is spent — do NOT re-raise.
    - (ii) 995→900 shrink: **UNRESOLVED → filed issue doc**
      `issues/tradfi_legacy_twin_candidate_set_995_to_900_unexplained_shrink_2026_08_05.md` (3 tracked todos: fresh
      orphan sweep, correct stale 995 figure, document root cause). The report was regenerated (GCS mtime
      2026-07-30T04:08:14Z, after the 2026-07-10 sweep that produced 995). `cleanup_legacy_twins.py`'s
      `load_legacy_twins()` does NOT filter beyond `obj_class == "B_legacy_duplicate"` — the shrink is in the report
      data, not in the loader. The 4 candidate explanations (a-d) are preserved in the issue doc. Cannot determine
      definitively without a fresh orphan sweep.

    **Conflict-gated items (2):**
    1. **FX-yahoo-drain** (`/plans/archive/2026_08/tradfi_multisource_backfill_2026_06_22.md`): **GATE CLEARED.** The
       historical ICE/KRX/FX `ohlcv_24h` provenance re-stamp shipped + verified 2026-07-30
       (`restamp_ice_krx_fx_ohlcv24h_databento_provenance_2026_07_30.py --apply`: 1,141 rows re-stamped, 0 remaining
       mis-stamped; script deleted post-run per lifecycle marker). The FX `SPOT_PAIR` write-path fix also shipped
       2026-07-26. → **Ready for batch6 extraction** (the 2 other small open items can fold into whichever batch runs
       the FX backfill).
    2. **Legacy-twin bucket DELETE** (`tradfi_legacy_twin_bucket_deletes_signoff_2026_07_24.md`): **Gate NOT cleared**
       (technical — coverage=0%, not a decision). The 995→900 issue doc tracks resolution.

    **Too-large-or-risky items (4):**
    1. `tradfi_manifest_content_recovery_completion_2026_07_24.md`: 29/34 done (5 open). → Still too-large.
    2. `data_completion_tradfi_2026_07_15.md`: 22/39 open (17 open). → Still too-large.
    3. `issues/tradfi_canonical_path_migration_design_2026_07_19.md`: `locked_by: live-defi-rollout`, 1 done. → Still
       too-large.
    4. `tradfi_sp500_ml_and_arb_backtest_readiness_2026_06_20.md`: **MDPS blocker NOT cleared.** Batch5 todo 2
       re-measure improved hit rate from ~19% (454/2398) to ~20.8% (499/2398) — real improvement but ~79.2% still
       empty_confirmed. → **Flag as weaker batch6 candidate** (blocker partially improved, not cleared; new information
       produced).

    **Operator-gated items (~6):** All unchanged since batch5 was authored — no operator rulings have landed on any:
    `tradfi_chain_bundle_sampler_root_mismatch_2026_07_23.md` (still `P1-OPERATOR-DECISION`),
    `tradfi_unreachable_databento_data_types_mbp10_ohlcv_coarse_calendar_2026_07_15.md` (1 `[DESIGN] P2` open),
    `instruments_tradfi_g1_g5_gate_execution_2026_07_24.md` (still active, ES_OPT launch + ICE pending),
    `canonical_id_p1_tradfi_combo_leg_canonicalization_2026_07_08.md` (still operator-gated),
    `tradfi_autonomous_session_operator_decisions_2026_07_25.md` (2 open PM items).
    `tradfi_eu_not_draining_source_axis_drift_2026_06_24.md` — already RESOLVED + archived (as batch5 noted). NOT
    re-asked.

    **Process observations (2):**
    1. ~~batch4-finalize undispatched~~: **RESOLVED** (archived at
       `/plans/archive/2026_07/tradfi_satellite_ao_dispatch_batch4_2026_07_26_finalize.md`).
    2. `tradfi_registry_coverage_and_ao_readiness_2026_07_25.md`: **STILL UNCHANGED** — `status: draft`,
       `assigned_vm: NA`, 1/15 done. Gated on `manifest_content_recovery` (29/34 done) and
       `backfill_throughput_followups` (24/30 done). Both close to done — expected gating, not a gap.
