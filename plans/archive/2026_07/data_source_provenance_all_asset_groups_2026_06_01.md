...
| Prediction  | `polymarket_clob`/`polymarket_gamma_api`/… (single per venue)                                                         | 🟡 code-wired; historical `_index` source `""` until the walk runs                        | **🟡 CODE-WIRED** (operator 2026-06-03: source column is the consistent choice, not N/A) — UAC `SOURCE_PRIORITY[(prediction,*)]` already carries the pairs + the writer auto-stamps the sole external source via `default_source` (single-source → no `MissingSourceError`), so every NEW/live write stamps `source`; the historical `_index` stamp rides the `prediction_manifest_canonicalisation` C-source walk. _Venue ≠ source still holds_: cross-venue dispersion (Polymarket vs Kalshi) stays a feature-layer concern, NOT a source merge |
