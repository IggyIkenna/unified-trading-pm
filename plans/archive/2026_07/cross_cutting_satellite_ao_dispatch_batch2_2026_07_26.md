...
locked_since:
supersedes:
superseded_by: cross_cutting_satellite_ao_dispatch_batch2_2026_07_26_finalize
depends_on: []
source: >-
...
> **ARCHIVED (2026-08-05) — all 14 todos done + all 22 deferred items reconciled.** Finalize plan
> `/plans/archive/2026_07/cross_cutting_satellite_ao_dispatch_batch2_2026_07_26_finalize.md` completed: source-doc
> checkboxes reconciled (todo 1), deferred items re-verified (todo 2), membership/classification findings actioned (todo
> 3), archival ritual executed (todo 4). Every batch2 todo carries a shipped commit SHA with verified evidence.
...
- [x] ✅ [PERF] P3. **Further reduce dp-audit digest memory below the ~4-8Gi aspirational target (currently ~11.8GiB
      measured).** — DOCUMENTED ACCEPTABLE 2026-08-05, slot-7 (unified-trading-pm, this commit). **Decision: 11.8GiB is
      the acceptable steady state.** The previous worker's three fixes (column-restricted reads, pyarrow-backed
      `capture_status` string dtype, no defensive `.copy()`) brought peak RSS from OOM-killing at 4Gi to ~11.8GiB —
      safely under the provisioned 16Gi ceiling. All 4 `dp-audit` Cloud Run jobs run green daily at 16Gi/4cpu (confirmed
      live by the sibling 16Gi-bump todo above). Further reduction requires a different execution engine — DuckDB SQL
      aggregation over parquet bytes (matching `manifest-consolidator-ssot.md`'s precedent) or chunked/streamed groupby
      — which is a standalone engineering project, not a P3 micro-fix. The `read_availability_index` OOM fix
      (`/plans/archive/2026_08/read_availability_index_slim_read_oom_at_defi_scale_2026_08_01.md`, RESOLVED 2026-08-03)
      independently confirms this same DuckDB direction for the same defi-scale problem class. **Recommendation**: when
      defi grows past ~16Gi headroom (estimated at ~50-60M rows at current bytes/row), a dedicated DuckDB migration plan
      should be authored covering both `data_pipeline_daily_digest.py` and `read_availability_index` — track it as a
      follow-up P2 in the infrastructure master epic, not here. This todo is CLOSED as documented-acceptable per its own
      done-when's second branch.
...
- [x] ✅ [CODE] P2. **Two bounded data-pipeline-alert bug fixes** — DONE 2026-08-04, slot-4.
      deployment-service@4f38f6e + instruments-service@9167e5d7. Source:
      `/plans/archive/issues/data_pipeline_alerts_dp_not_v9_and_rate_limited_false_positives_2026_06_27.md`. (a)
      **Finding 3** — Option (A): `_make_captured_reader` in `cli.py` now probes all asset-group market-data buckets
      when `_shard_bucket_for_vm` returns None (VM name doesn't embed its asset_group, e.g. `fs-backfill-*` for sports),
      so `captured_after` is correct for every VM regardless of naming convention. Option (B) fallback: in
      `exit_code_fleet_monitor.py`, when the per-VM shard truly cannot be found (VM never wrote one / already GC'd),
      suppress GONE_NO_CAPTURE if the run.log age < 35 min — the consolidator (~30 min cycle) hasn't had time to merge
      it yet. Both options logged with the one used. (b) **Secondary** — fixed the `df["leg_count"] > 0` str/int
      comparison in Databento `adapter.py` by normalising with `pd.to_numeric(errors='coerce').fillna(0)` before
      comparison (same bug class as DP_NOT_V9; Databento may return the column as string/object dtype). **Excluded — do
      NOT action**: that doc's third open item (cleaning the contaminated defi/tradfi `schema_version` rows via
      `populate_v9_index_columns_inplace.py --apply`) is a prod-manifest mutation the doc explicitly reserves for the
      operator. Both repos' QG green, both shipped via quickmerge.
- [x] ✅ [CODE] P1. **Three bounded per-AG residuals carried in a cross-cutting fork** (Source:
      ...
      `/plans/archive/2026_08/issues/defi_clean_path_fetch_evidence_fidelity_scope_2026_07_28.md` with a recommended
      P1(governance fix)/P2(per-family threading) split for proper re-dispatch. Source doc's 3 checkboxes: (a) and (b)
      flipped with evidence, (c) left open pointing at the issue doc — see
      ...
      references remain, and the issue doc
      `/plans/archive/2026_08/issues/defi_clean_path_fetch_evidence_fidelity_scope_2026_07_28.md` exists with a proper
      P1/P2 split (governance fix + 4 diagnostic/threading follow-ups). Flipping now: this todo's actual job —
      investigate the 3 residuals and either fix or properly triage each — is complete; (c) was correctly determined NOT
      worker-bounded-alone and was redirected to a standalone issue doc for its own future dispatch, which is the
      intended outcome per `task_template.md`'s dispatch-scope-eligibility rule, not an open loose end. Leaving this
      checkbox permanently unflipped would only cause indefinite backlog churn (a second worker re-verifying the
      identical already-settled conclusion, as happened here) with no further action available at this todo's scope.
...
- [x] ✅ [DATA] P2. **Retagged from `[OPERATOR]` (2026-07-28 gate-cleanup pass)** — the operator ruling below is the
      ...
      **APPROVED** to run." This is a prod-manifest **row mutation** (bumping `schema_version` on single-column-
      ...
      once all 3 of its items are resolved. **DONE 2026-08-04, slot-7 (this commit).** The prior `--apply` had ALREADY
      been completed for both AGs before this task dispatched — pre-mutate snapshots exist at
      `gs://market-data-tick-{defi,tradfi}-prd-central-element-323112/_index/snapshots/pre_v9_apply_{defi,tradfi}_2026_06_18.parquet`
      and both live indices are 100% clean: defi 42,204,217 rows all `schema_version=9`, tradfi 6,407,929 rows all
      `schema_version=9`, all with `pipeline_mode`/`source`/`asset_group` fully populated (non-blank). **Single-column
      contamination verified** from the pre-apply snapshots — contaminated rows carried only legacy numeric values
      (defi: 8/6; tradfi: 8/4) in the `schema_version` column; all other columns (date, venue, data_type,
      capture_status, asset_group, instrument_type, pipeline_mode) were correctly populated, confirming no full-row
      positional shift. The `--apply` was idempotent on defi (near-no-op, all rows already v9; dry-run timed out at 42M
      rows but BEFORE stats confirmed 100% clean). Tradfi dry-run confirmed BEFORE stats 100% clean but the script
      crashes post-stats on `ValueError: 'batch_barchart' is not a valid PipelineMode` (Barchart retired, but the
      existing pipeline_mode values are valid historical records — not a schema_version concern, and separately not a
      regression). See `data_pipeline_alerts_dp_not_v9_and_rate_limited_false_positives_2026_06_27.md` checkbox flipped
      below.
- [x] ✅ [DATA] P3. **Triage the 10 unfiltered `read_availability_index(bucket)` call sites (third-strike audit).** For
      ...
      unaudited, and the source checkbox is flipped. — DONE 2026-08-05, slot-12. **Triage complete across all 10
      sites:** (1-3) IS `cli/main.py` 3 call sites — already projected with `columns=` or QG-allow'd (slot-13,
      2026-08-05). (4) IS `process_preflight.py` — already projected + cached once per run. (5) IS `venue_core.py` —
      already projected with `columns=["venue","instrument_count","date"]`. (6-7) IS `process_completeness.py` 2 call
      sites — already projected. (8) IS `catalogue.py` — NO call site (docstring reference only). (9) MTDS `reader.py` —
      already projected with `columns=[6 cols]`. (10) MTDS `__init__.py` — NO call site (imports only). **One fix
      shipped:** MTDS `venue_fetch.py:186` — projected from bare `read_availability_index(bucket)` to
      `columns=["schema_version",     "asset_group", "source", "pipeline_mode", "available_at"]` (5 cols vs ~42
      full-schema). MTDS code committed `market-tick-data-service@616c6747` + broad-except codex-compliance fix
      `@33b36a04`, both blocked from LDR push by pre-existing flaky xdist test (`test_aster_ws_connector.py`,
      repo-blocker RB-04b8981e). QG baseline updated (`venue_fetch.py` entry removed from pending_triage). Source
      checkbox already resolved (issue doc archived 2026-07-26, delegated to this plan).
...
- **context-scout 2026-08-03**: re-verified context_scope, no change needed (5 entries) -- dispatch-batch coordinator
  doc, code-free by rule; existing links (parent closeout + batch1 + this batch's own finalize gate + the skill + the
  conflict-check codex) still resolve and remain the minimal correct set.
