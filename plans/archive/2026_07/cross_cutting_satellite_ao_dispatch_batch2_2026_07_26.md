...
summary: >-
  ...
  Run jobs (still 4Gi, not the claimed 16Gi) and one doc was found existing in `plans/archive/issues/` AND
...
- **`infra_capture_and_devops_leftovers_finalize_2026_07_25.md`** (0 open) — DONE but deliberately NOT archived;
  removing it from `plans/archive/2026_08/` regresses `scripts/quality_gates/check_finalize_plan_coverage.py` from
  baseline 1 to 2 and hard-fails PM `quality-gates.sh` for every agent. Its own 🟡 banner documents this.
