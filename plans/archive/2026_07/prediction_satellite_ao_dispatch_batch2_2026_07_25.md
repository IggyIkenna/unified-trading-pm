...
> **🟢 ARCHIVED 2026-07-30.** All 6 todos + the wrapper + the draft→active flip todo are done. Reconciliation into the 5
> distinct source docs was independently verified by the companion finalize plan's todo 1 (all 6 commit citations
> confirmed present + ancestors of `live-defi-rollout`); `prediction_consolidated_closeout_2026_07_18.md`'s stale claims
> discovered during this batch's re-triage were corrected by that finalize plan's todo 2. This archival (finalize
> todo 3) re-verified the Deferred section below: nothing has newly cleared beyond what's already reflected —
> `predictions_other_bucket_and_ui_drilldown_2026_06_20.md`'s Phase-5 backfill item is already tracked in
> `prediction_satellite_ao_dispatch_batch6_2026_07_29.md`; its `[VERIFY][UI]` re-walk item stays genuinely blocked on
> `[BLOCKED-PLAYWRIGHT]` (unrelated to this batch's own sentinel-fan-out shipping); the arb-bridge doc's ruling is
> already captured (finalize todo 2 + `prediction_satellite_ao_dispatch_batch6_2026_07_29.md`'s dispatched todo); the
> `prediction_universe_capture_dead...` secondary conflicts stay Phase-D's P0, unchanged;
> `data_completion_prediction_ 2026_07_15.md`'s Phase-B stays too-large/operator-gated, unchanged (independently
> re-confirmed by batch6). No new durable contract from this batch — codex-alignment check: nothing to update (per this
> doc's own "Codex SSOTs" section, every todo executed an already-decided spec). Archived via
> `/plans/archive/2026_07/prediction_satellite_ao_dispatch_batch2_finalize_2026_07_25.md` (archived alongside this doc,
> same commit).
>
...
- [x] ✅ [SCRIPT] P1. **DONE 2026-07-27 (slot-16)** — instruments-service@70cf5c24. Fresh live single-walk read of
      ...
      **both sub-items already resolved, 0 rows to purge/backfill**: (a) `data_type` has exactly 5 distinct values
      corpus-wide, zero blanks — the 17 blank-`data_type` rows are gone (superseded by a subsequent full rebuild). (b)
      100% (711,544/711,544) of `pipeline_mode=batch_polymarket_clob` rows carry `source=polymarket_clob`, zero blanks —
      confirms the 2026-07-19 dry-run's residual count of 2 has since closed to 0. No writes needed; the
      snapshot-then-write purge machinery was never exercised since both predicates measured 0. Full findings + the
      diagnostic script cite recorded in
      `plans/active/issues/prediction_phantom_reconciler_wipes_bundle_atom_2026_07_10.md`'s "Update 2026-07-27
      (slot-16)" section (same commit). **Combined residual-row diagnosis for
      ...
      or scoped invocation of the existing canonicalizer). **Downgraded from `[OPERATOR]` 2026-07-27**
...
- [x] ✅ [DATA] P2. **PARTIALLY RESOLVED 2026-07-28 — the clearest example (the arb-bridge transport-seam decision) is
      now RULED, no longer blocked.** This wrapper todo previously bundled several "Deferred — still genuinely blocked"
      items under one operator-decision umbrella. Re-triaged 2026-07-28:
      `issues/prediction_arb_live_execution_bridge_     2026_07_20.md`'s "single remaining LIVE blocker" is
      architecture, not a fresh judgment call — the workspace already has a standing codex-mandated pattern for exactly
      this cross-tier shape (`/codex/02-data/live-data-     persistence-and-event-log.md` § "Live = batch (event-log
      spine)": every live=batch producer/consumer pair uses the UTL `EventTransport` facade). Ruled + retagged directly
      in that issue doc — its own `[OPERATOR]` todo is now `[BACKEND]`, with the EventTransport-facade build spelled
      out. **The other items this wrapper referenced are a DIFFERENT flavor and stay genuinely deferred, not part of
      this ruling** — `data_completion_prediction_2026_07_15     .md`'s cross-doc Phase-B-naming ambiguity and
      `issues/prediction_polymarket_legacy_dual_write_trees_metadata_     loss_2026_07_24.md`'s
      `prediction_trades`-axis-disposition contradiction are cross-doc fact/ownership reconciliation questions, not
      architecture-design calls — out of scope for this specific ruling pass (not touched here; still tracked in the
      Deferred section below). Done when: confirmed the arb-bridge retag landed in its own doc (it has — see that doc's
      Todos section) and this bullet's own checkbox is flipped, which it now is.
- [x] ✅ [OPERATOR] P1. **DONE 2026-07-30 (autonomous session)** — flipped
      `prediction_satellite_ao_dispatch_batch2_finalize_2026_07_25.md`'s frontmatter `status` from `draft` to `active`;
      verified all 6 todos in this plan are done (only this flip todo itself was open) before flipping. Flip
      `prediction_satellite_ao_dispatch_batch2_finalize_2026_07_25.md`'s frontmatter `status` from `draft` to `active` —
      its `depends_on:[prediction_satellite_ao_dispatch_batch2_2026_07_25]` + `gate_on_depends: true` condition is
      satisfied (all 6 todos above are done) but nothing has triggered the draft→active flip, so its reconciliation
      (flip checkboxes across the named source docs) and archival todos have never dispatched.
...
- **`/plans/archive/issues/prediction_lifecycle_prefetch_gate_and_resolution_day_catalogue_2026_07_14.md`** — ADDED
  2026-07-25 plan-reconcile (was silently missing from this Deferred section despite being one of batch1's original 12
  deferred docs). DUPLICATE-OF-BATCH1: its sole AO-eligible open item (line 115, a `[VERIFY] P2` re-measure of
  prediction attempted/captured trajectory) is substantively identical to batch1 todo 4, which writes its before/after
  counts into this exact same doc's Progress Log. No new todo needed here.
...
- **`issues/prediction_arb_live_execution_bridge_2026_07_20.md`** — **RESOLVED 2026-07-28, no longer deferred.** Its
  "single remaining LIVE blocker" is now RULED (UTL `EventTransport` facade, per the standing live=batch event-log codex
  SSOT) and retagged `[BACKEND]` directly in that doc — see this plan's own wrapper todo above. Separately,
