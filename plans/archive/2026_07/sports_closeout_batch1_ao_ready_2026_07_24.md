...
> **✅ ARCHIVED 2026-07-24 — COMPLETE.** All 21 todos shipped (20 hand-picked + todo 1's mid-execution split into a
> CODE + a DATA todo; both map to the same parent Track C1 checkbox). Parent plan
> (`sports_consolidated_closeout_2026_07_19.md`) reconciled — all 20 corresponding checkboxes flipped with
> independently-verified evidence (`sports_closeout_batch1_finalize_2026_07_24.md` todo 1,
> `unified-trading-pm@20915879b`). Every issue doc a todo spun off re-verified accurate: 2 genuinely still open
> (`fixtures_manifest_duplicate_collision_residual_2026_07_24.md`,
> `sports_fixtures_schedule_noncanonical_raw_league_id_folders_2026_07_24.md` — both independently tracked, not
> re-opened here) and the rest resolved. No codex doc required a status update (the "Distinct Values canonical
> vocabulary" target this batch worked toward is tracked live in the manifest, not asserted as a static codex claim that
> needed correcting). Moved to `plans/archive/2026_07/`.
...
- [x] [DATA] P0. ✅ Backfill the legacy `data_type="FIXTURES"` sports manifest rows to `FIXTURES_SCHEDULE` (read-only
      prod census 2026-07-24, bucket `instruments-store-sports-prd-central-element-323112`). **PRE-FLIGHT DONE
      2026-07-24 (worker)**: found + fixed 2 live bugs the original resolution assumed were clean — a 9th missed call
      site (`sports_fixture_status_refresh.py`) was CONTINUOUSLY re-creating legacy rows, and
      ...
      pattern) — NOT a GCS read/re-derive, NOT `migrate_fixtures_split.py`. **CONTENTION** confirmed
      (`uts-prod-manifest-consolidator-instruments-sports-cron` has its own `*/1 * * * *` schedule + a documented
      livelock/slow-run history on this bucket specifically) — paused via the coordinated, CAS-protected
      `deployment_service.data_pipeline_monitors.scheduler_maintenance` `pause_for_maintenance()`/
      `resume_after_maintenance()` primitive (operator-authorized, 2026-07-24), not a raw `gcloud` pause. **First
      `--apply` attempt was silently OOM-killed** mid-`classify()` (no write — confirmed via re-census, unchanged
      counts) — root cause: a hand-rolled Python string composite key + `set()` over the full 5.5M-row corpus; rewritten
      to pandas' native vectorized `duplicated()`/`merge()` (`instruments-service@e92efc78`), re-verified identical
      result in <1min. **SHIPPED + VERIFIED (2026-07-24)**: `instruments-service@e92efc78` (script), applied against
      `instruments-store-sports-prd-central-element-323112` (generation `1784932654620334` → `1784932868619431`,
      snapshot
      ...
      **282,231 of 337,464 rows restamped** to `FIXTURES_SCHEDULE`; **55,233 could NOT be safely restamped** — their
      post-restamp dedup key already collides with an existing (already-migrated) `FIXTURES_SCHEDULE` row, a genuine
      dual-write duplicate from the leak above. Post-write verified: 0 duplicate keys, `FIXTURES_OUTCOMES` (57,039)
      untouched, `FIXTURES_SCHEDULE` grew by exactly 282,231, remaining legacy `FIXTURES` == 55,233 — census
      re-confirmed via `deployment-api/scripts/census_manifest_data_type_2026_07_24.py`. Cron resumed + confirmed
      ENABLED. **RESCOPED Done-when (operator-authorized 2026-07-24, mirrors todo 1's own RESCOPE pattern)**: the
      original "census returns zero FIXTURES rows" bar is NOT met (55,233 remain, provably exact duplicates, not a
      mechanical-restamp problem — resolving them needs a DELETE-safety design decision) — split into a new issue +
      todo: `/plans/archive/issues/fixtures_manifest_duplicate_collision_residual_2026_07_24.md`. This todo's own scope
      (the mechanically-safe 1:1 restamp) is fully done. **`SCHEDULE_DEFINING_DATA_TYPES` stays ADDITIVE** (does NOT
      narrow back to `frozenset({FIXTURES_SCHEDULE})` yet — the 55,233 residual rows still need it) until that new
      issue's todo closes. Full analysis: `/plans/active/issues/fixtures_manifest_legacy_backfill_2026_07_24.md`.
...
- [x] [CODE] P1. ✅ Build a sports pipeline-check for the instruments-service → market-tick-data-service →
      ...
      it. — **features-service@7ea10aaa** (corrected citation — `4639106a` never reached origin; quickmerge hit repeated
      host-contention timeouts + a strict-quickmerge trailer conflict from two separate manual commits, resolved by
      squashing to one commit before the final green quickmerge run).
...
- [x] [DIAG] P2. ✅ Wire `is_promotion_relegation` (currently hardcoded `False` in `features-service`'s
      ...
      reference remains. — features-service@34b53186: `_compute_league_batch` derives the real flag from standings
      home/away league position vs. measured team count (bottom-3 = battle); new `_promo_relegation.py` leaf module
      merges it into `season_context`'s input before that calculator runs, so `points_at_stake`'s multiplier reflects it
      too. Unknown team-count is honest NaN. 33 new/updated tests; full `quality-gates.sh` green (17815 passed).
...
- [x] [DOC] P2. ✅ Verify `sports-data-source-coverage-matrix.md`'s body isn't stale-under-banner (check every claim
      ...
      `sports_master.md` had 7 broken paths (not 5) — files archived from `archive/2026_08/` to dated `archive/` subdirs
      without the referrer being updated; all fixed + verified resolving. Also found + fixed a pre-existing, unrelated
      broken link in `/plans/archive/2026_07/active_plan_inventory_dashboard_2026_07_24.md` (confirmed pre-existing on a
      clean tree before fixing) — someone else regenerated it concurrently with the identical fix, no separate commit
      needed from me.
- [x] [CLEANUP] P3. ✅ All 3 sub-parts resolved (full writeup + evidence:
      `plans/active/issues/sports_closeout_batch1_task018_partial_progress_2026_07_24.md`). (1) The 2018-2020
      `markets`/`outcomes`/`settlements`/`arbitrage_opportunity` GCS scaffolding — **CORRECTED 2026-07-24 (slot-7)**:
      the "verified-absent" ruling on `BLK-7aa96c0a` below only checked GCS objects and was half right — 26,352 dead
      `capture_status=empty_confirmed` MANIFEST rows for these exact 4 data_types (2018-01-01..2020-06-05) were still
      present in `instruments-store-sports-prd-central-element-323112/_index/availability_index.parquet`. Snapshotted +
      purged via `instruments-service@019cbae0`; post-purge census confirms 0 remain. See the issue doc's "CORRECTION"
      section for the full record. (2) `SPORTS_INSTRUMENTS.md`'s stale "lineups player-id strip" claim corrected —
      `instruments-service@97fbea22`. (3) Non-ASCII junk-symbol guard added — `unified-api-contracts@a6346f95`.
...
- [x] ✅ [DOC] P3. File an issue doc for the QG structural finding: at least two `quality-gates.sh` steps
      ...
      issues in the shared MAIN clone. File under `plans/archive/issues/` with `asset_group: [meta]` (workspace-infra,
...
- **2026-07-24 (slot-8, complete)** — Todo "Wire `is_promotion_relegation`" — see the flipped checkbox above for the
  full implementation summary. Shipped as `features-service@34b53186` after ~18 `quality-gates.sh` attempts were killed
  by severe multi-slot host contention on this shared box before completing (never a code defect — the full pytest suite
  itself passed cleanly every single time it was reached, 5 separate times). Root-caused to two distinct issues, both
  now fixed/documented: (a) transient host-wide OOM/CPU contention (load avg spiked 30+ on an 8-core/30GB shared host) —
  no code fix possible, just retried during lower-load windows; (b) `PYRIGHT_TIMEOUT` defaults to 120s in
  `base-service.sh`, too low for this repo's TYPE CHECK step even under moderate load — filed as a todo on
  `qg_host_adaptive_resource_governor_2026_07_14.md` (PM@b3ed4272d) and worked around here with `PYRIGHT_TIMEOUT=600`.
  One aside: the first `quickmerge --agent` attempt was rejected by the `check_strict_quickmerge` pre-push hook because
  my 2 separate commits only got the `Quickmerge:` trailer amended onto HEAD, leaving the OLDER commit flagged as
  bypassed — fixed by squashing both into one commit (`git reset --soft` + recommit, no `-i` needed) before the final
  successful quickmerge.
