...
> **🟢 ARCHIVED 2026-07-30.** All 33 todos resolved: 32 shipped/reconciled + 1 (BYBIT futures_chain extended
> duplicate-verification, line 355) MIGRATED — never completed under this plan — to its own named Source doc,
> `issues/bybit_futures_chain_write_shape_2026_07_13.md`, as a fresh open todo (see that doc for the live tracking; per
> `/codex/12-agent-workflow/plan-completion-and-archival-discipline.md`'s "split it, or fold the remnant" guidance for a
> near-complete plan with one genuine straggler). Closed out via the companion
> `cefi_satellite_ao_dispatch_batch1_finalize_2026_07_25.md` (source-doc reconciliation + too-large-doc batch2 re-check
>
> - LATE-renames re-verify + this archival). Moved to
>   `/plans/archive/2026_07/cefi_satellite_ao_dispatch_batch1_2026_07_25.md`; corpus referrers updated. No new durable
>   contract from this batch — codex-alignment check: nothing to update (every todo executed an already-decided spec,
>   per this plan's own conflict-check discipline).
>
...
- [x] ✅ [DATA] P1. **Extend MDPS candle-building to the 4 on-chain-perp CeFi venues + backfill.** Point MDPS's candle
      ...
      (`/plans/archive/issues/aster_raw_capture_manifest_registration_gap_2026_07_26.md` — its manifest doesn't yet
      reflect its real captured range, so a manifest-scoped backfill for it would be wrong). HYPERLIQUID's `trades`
      candles for a recent day (2026-07-19, BTC + ETH) were successfully backfilled end-to-end
      (`mdps-backfill-cefi-20260726-181434`, `e2-highmem-8`, exit_code=0, 2/2 succeeded, 15,230 candles, 7.9 min) with
      VERIFIED real, non-zero `volume` (BTC 24h: `volume=28140.06`, real OHLC ~$64.3-65.0k) — criterion 1 is met in
      substance (the field is `volume`, not literally `quote_volume` — see below). - **A real, reusable operational
      recipe now exists and is documented** for this class of backfill: a SINGLE real day for one high-volume venue does
      NOT fit in the default `e2-standard-8` (32GB) regardless of `--instrument-ids` scoping (proven via 3 separate
      crash reproductions — see Progress Log); the fix that actually worked was `MACHINE_TYPE=e2-highmem-8` (64GB).
      LIGHTER-ZKSYNC/EXTENDED-STARKNET were not individually re-verified with the same live GCS-read + ADV-reader check
      this session (time-bounded), but are covered by the still-running full-range VM below and use the identical,
      now-proven code path. - **Criterion 1's literal "`quote_volume`" wording cannot be satisfied by ANY venue, ever,
      currently** — a NEW finding this session: NO MDPS candle file (checked HYPERLIQUID `trades`, an established
      BITGET-FUTURES `trades`, and an established BITFINEX-FUTURES `derivative_ticker`, spanning 3 venues) has ever
      carried a column literally named `quote_volume` — only `volume`. This is universal and pre-existing, not
      introduced by this backfill. - **Criterion 2 (ADV reader non-`NO_DATA`) is BLOCKED on that same finding** —
      live-tested `RollingAdvReader.compute_rolling_adv()` against the real, verified HYPERLIQUID BTC candle above:
      returns `status=no_data`, `days_observed=0`, DESPITE real non-zero volume data existing. Filed as its own P1
      cross-repo issue: `/plans/archive/issues/rolling_adv_reader_quote_volume_column_never_exists_2026_07_26.md` — this
      bug predates this session (ships with the ADV reader itself, `features-service@8608ea5d`) and affects EVERY CeFi
      venue, not just these 4; it is a features-service↔MDPS schema-alignment fix outside this todo's own repo scope. -
      **Criterion 3 (manifest-verified backfill covers the full range)** — the universal MDPS candle-manifest-emission
      bug (`/plans/archive/issues/mdps_cefi_candle_manifest_never_emitted_2026_07_26.md`, found by a concurrent slot on
...
- [x] [SCRIPT] P1. **Purge orphaned CeFi on-chain-perp reference-data blobs left under the DEFI bucket.** For
      ...
      **[OPERATOR] tag added retroactively** (missing at authoring time — this is a real prod-bucket delete, a
      plan-authoring gap per CLAUDE.md's "every AO todo with a GCS delete needs [OPERATOR]+delete-safety-cite" rule).
      **Prep work done, delete NOT executed** (2026-07-26, worker slot-12): confirmed real orphan count via a
      prefix-scoped listing under `instrument_availability/by_date/` (not a whole-corpus walk) — EXTENDED-STARKNET=1256,
      PACIFICA-SOLANA=749, LIGHTER-ZKSYNC=1376, **total 3381 objects**, on `instruments-store-defi-prd-{PID}` (a `-prd-`
      PRODUCTION bucket); confirmed no live writer (venues excluded from `_build_defi_venues()`) and no live reader of
      this DEFI-bucket-scoped path for these venues (their live data now reads from the cefi bucket/manifest
      post-reclassification — grepped all repos, only cefi-context references remain). Wrote + shipped
      `instruments-service@4d6c2109` — snapshot-then-delete script
      `scripts/purge_cefi_perp_defi_blob_contamination_phase3_2026_07_26.py` (dry-run by default; `--apply` snapshots
      each object to `_purge_snapshots/cefi_perp_defi_blob_contamination_phase3_2026_07_26/` before deleting, then
      verifies 0 remain); dry-run output matches the investigation above (3381 total); `quality-gates.sh` green. **Did
      NOT run `--apply`** as of 2026-07-26. **2026-07-27 update — no longer `[OPERATOR]`-gated: reversibility-verified**
      (finding T, `task_template.md`): this is an object/prefix-scoped delete (3 named venue prefixes, never the bucket)
      — `gcs_bucket_soft_delete_retention_seconds(...)` returned `604800` (7 days) fresh-checked 2026-07-27 for
      `instruments-store-defi-prd-central-element-323112` per
      ...
      `/plans/archive/issues/defi_lending_protocol_capabilities_instrument_types_stale_atoken_debttoken_2026_07_27.md` —
...
- [x] → **MIGRATED 2026-07-30 to `issues/bybit_futures_chain_write_shape_2026_07_13.md` (new open todo there). NOT done
      — never shipped under this plan.** [DATA] P1. **Extend BYBIT futures_chain shape-2 duplicate verification to the
      full audited scope.** Extend the archived migration plan's 5-day sample to every day the existing Phase-1
      scope-audit output (`_index/audit/bybit_futures_chain_shape_scope_2026_07_13.parquet`,
      `market-tick-data-service@5e367479`) classified `bare_flat_only`/`bundled_flat_only`/`mixed` — row-level diff each
      bare_flat/bundled_flat object against its hive/canonical counterpart using the same columns Phase 1 Todo 2 used,
      and write a per-day duplicate-verdict audit parquet. Read-only verification only — does NOT delete anything (the
      actual cleanup stays BLOCKED-OPERATOR-DECISION). Repo: market-tick-data-service. **Conflict-check note**: the one
      flagged conflict is master closeout's Track 7 verification of 6 of 8 specific (day, venue) cells for raw-tick
      PRESENCE ahead of a candle backfill — a different specific days/purpose (presence-confirmation for 8 named days vs
      duplicate-status for ~500+ days ahead of a Phase-4 delete decision), and both sides are read-only audits with no
      mutation, so there is no regression risk from running both. **Done when**: a new audit parquet gives a per-day
...
- [x] ✅ [DATA] P1. **DONE 2026-07-27 (slot-14) — market-tick-data-service@4e5d0a24.** Proved + executed the cefi
      chain-tail v6 canonicalisation cutover, real GCS `-test-` bucket, no mocking: **(a) PASS** — wrote a real DERIBIT
      BTC options-chain chunk (two settlement variants) via `PartitionedTickWriter.write_chunk`; both objects landed at
      the v6-canonical path (confirmed `gcs_describe_object`); `CanonicalParquetReader.read_shard` round-tripped both
      rows back; `_assert_canonical_chain_path` raised on a synthetic v5 path. **Found + fixed 2 real, previously-
      unknown reader bugs while proving this** (both invisible to the existing unit tests because they mock `list_blobs`
      with plain strings, not the real provider's return type): (1) `reader.py::_derivative_underlying_tails` only built
      the v6 probe tail for `asset_group == "tradfi"` — a v6 cefi write was unreadable; fixed via a new
      `_cefi_chain_underlying_blobs` that live-lists the chain's own `underlying={id}/` subtree (bounded, single-shard —
      handles MULTIPLE v6 settlement variants sharing one underlying, which tradfi's single-guessed-tail approach
      cannot). (2) the real GCP/AWS `list_blobs` yields `BlobMetadata` objects (`.name`), not bare strings — 3 call
      sites called `.endswith()` directly on them; added `_blob_path_name()`. Also widened `_read_candidate_blobs`'s
      dispatch so cefi-chain multi-blob reads UNION (concatenate) instead of taking only the first — first-existing
      would have silently dropped one settlement variant, reproducing the exact v5 collision this fix eliminates. **(b)
      enumeration = 0** — queried the real availability manifest for
      `instrument_type in (options_chain, futures_chain)`: 307 rows, ALL
      `capture_status ∈ {attempted_failed,     empty_confirmed}`, ZERO `captured` — no v5 (or any) cefi chain object
      exists anywhere in the corpus; migration is a confirmed no-op. **(c) DONE** —
      `/codex/02-data/canonical-cutover-register.md` §6c/§7 updated, citing `market-tick-data-service@04222eb0` (W1),
      `unified-api-contracts@9a92cf4f` (structural guard), and this session's reader-fix sha; cefi chain-tail cell now
      reads "2026-07-22 — EXECUTED, 0-object migration". Unit tests: `tests/unit/test_reader.py` (+16 new tests:
      `TestBlobPathName`, `TestCefiChainUnderlyingBlobs`, `TestBlobPathsDerivativeCefiListing`,
      `TestReadCandidateBlobsCefiChainUnion`), full `quality-gates.sh` green. Full write-up + Progress Log:
      `issues/cefi_chain_tail_v6_canonicalisation_2026_07_21.md` (todos 5-8). Source:
      `issues/cefi_chain_tail_v6_canonicalisation_2026_07_21.md`.
- [x] ✅ [DATA] P1. **DONE 2026-07-27 (slot-5) — market-tick-data-service@94b4aff5.** Widened
      `_update_cluster_and_chain_counts` (`partitioned_writer.py`) to promote
      `_cluster_counts`/`_chain_available_at_max` to the v6 5-tuple `(itype, dt, underlying, quote_asset, margin_type)`
      when a chain bundle carries a settlement quote/margin, mirroring `_row_counts`/`_get_writer`. **Found + fixed a
      real, previously-latent collision one level up while wiring this through**: `venue_fetch.py`'s
      `_record_venue_shard_counts` and `_state.py`'s `chain_cluster_counts`/`chain_bundle_available_at` were ALSO only
      keyed on a 4-tuple (venue + the old 3-tuple) — since tradfi options/futures chains ALWAYS resolve a quote/margin
      (`_tradfi_chain_partition_dims` is underlying-independent), two tradfi settlement variants sharing an underlying
      were already colliding at this layer even before this todo (and the unwidened writer-level fix alone would have
      made this consumer CRASH on every tradfi chain write — unpacking a 5-tuple key as 3). Widened both to a 6-tuple
      (+quote_asset, margin_type) via a new shared `_widen_chain_key` helper, plus `manifest_finalize.py`'s
      `_write_bundle_shard_row` cluster/envelope lookups to match. New regression test
      `test_cluster_counts_same_underlying_different_margin_never_collides` (mirrors
      `test_cefi_chain_same_underlying_different_margin_never_collides`); updated 3 existing tradfi
      `test_partitioned_writer_cluster_counts.py` tests to the new 5-tuple key (quote=USD/margin=linear) + 3
      `test_cme_options_chain_bundle_finalize.py` tests' key-unpacking (3-tuple → variadic). All 6978 unit tests green,
      full `quality-gates.sh` green (sentinel-verified against HEAD). Repo: market-tick-data-service. Source:
      `issues/cefi_chain_tail_v6_canonicalisation_2026_07_21.md`.
- [x] ✅ [DATA] P1. **Corpus-wide scan for the missing-candle-SchemaContract failure class.** Enumerate every CEFI (and,
      ...
      — ✅ DONE 2026-07-27 (slot-11): written findings in
      `issues/cefi_future_instrument_type_no_candle_schema_contract_2026_07_21.md` todo 2 (real GCS sampling, 4 days).
      CEFI: NOT DERIBIT-specific (OKX-FUTURES also emits standalone `future`), but todo 1's venue-agnostic fix already
      covers it — no further CEFI action needed. DeFi: 2 unregistered types found (`spot_asset`, `lending`) but MDPS
      source never references either — a different, lower-priority pipeline-coverage gap, not this failure class.
      Prediction: not fully verified (no dedicated bucket found). No code shipped (pure research/audit todo). Source:
      `issues/cefi_future_instrument_type_no_candle_schema_contract_2026_07_21.md`.
- [x] ✅ [DATA] P1. **ALREADY DONE 2026-07-27 (slot-15) — discovered on pickup by slot-13; checkbox flip only, no new
      work.** The source issue doc's own todo 3 (`issues/cefi_high_attempted_failed_batch_cluster_2026_07_23.md`, now
      `[x]`) + a cross-linked `## 8a` in `issues/deribit_combo_perpetual_partition_move_2026_07_21.md` already trace
      this: 4,812 `book_snapshot_5`+`trades` rows, 100% `venue=DERIBIT`/`instrument_type=FUTURE`, 786 distinct symbols,
      **786/786 = 100% combo-shaped via the real `is_deribit_combo_symbol_shape()` classifier — hypothesis CONFIRMED**.
      Already fixed by `market-tick-data-service@2ddc6d4a` (2026-07-22, guard-widen predating every failing row's
      `attempted_at`); zero recurrence since. The COINBASE-FUTURES/BITFINEX-FUTURES/OKX-FUTURES venues in this todo's
      title were a mis-attribution in the source doc's original prose, corrected on re-trace (unrelated causes, not this
      defect). Residual 786-symbol backlog is an ordinary idempotent re-attempt, not a code fix. No code shipped here
      (fix already shipped under `2ddc6d4a`). Verified both docs at fresh-pulled HEAD, no drift. Source:
      `issues/cefi_high_attempted_failed_batch_cluster_2026_07_23.md`,
      `issues/deribit_combo_perpetual_partition_move_2026_07_21.md` § 8a.
...
- [x] [INFRA] P1. **Wire the already-built `DeploymentsRegistry.reap_stale()` into the exit-code fleet-monitor cron.**
      ...
      test in `test_data_pipeline_monitors_cli.py` proves a gone-VM `archive/2026_08/` entry gets archived after one
...
- [x] [BACKEND] P1. **Add `DP_VM_GONE_NO_CAPTURE` to alerting-service's recurring-alert cooldown map.**
      ...
      `origin/live-defi-rollout`.
- [x] ✅ [DATA] P1. **Probe Tardis exchange-info coverage for PACIFICA-SOLANA, investigation only.** Query
      ...
      probe evidence. Source: `issues/cefi_onchain_perp_batch_venue_allowlist_gap_2026_07_12.md`. — ✅ DONE 2026-07-27
      (slot-11): **DEFINITIVE NO** (`pacifica` HTTP 400, unrecognized exchange; full evidence + finding in source doc).
- [x] ✅ [DATA] P1. **Fill the HYPERLIQUID recent-tail manifest gap via the HL batch lane.** Launched
      `cefi-hyperliquid-2026-20260727-150820` (SPOT, HL, 2026-06-24→2026-07-25) via the HL batch lane — verified
      genuinely healthy (not fire-and-forget): `run.log` shows real manifest-shard writes advancing through day 4/32 at
      T+1h54m (~29min/day). Consolidated `_index`: captured=7017/empty_confirmed=9504/expected_unattempted=15376/
      attempted_failed=3 of 31,900 cells, 0 new failures. **Not 100% yet** — next check-in confirms
      `expected_unattempted`→0. Source: `issues/cefi_residual_followups_after_honest_done_2026_07_17.md`.
- [x] ✅ [SCRIPT] P1. **DONE 2026-07-27 (slot-2), root cause=box size.** Re-run the HL phantom-manifest re-census on a
      bigger VM (Repo: instruments-service; source: `issues/cefi_residual_followups_after_honest_done_2026_07_17.md`).
      Extended `launch-defi-phantom-recon-vm.sh` (`deployment-service@369c1e3`: `MACHINE_TYPE`/`VENUES` + SPOT); ran
      `defi-phantom-recon-cefi-20260727-173241` (`e2-highmem-4`, HL-scoped, `--apply`): `Real captures: 104153`,
      `Phantom captures: 0`. Re-verified via fresh `_index/phantom_audit_latest.json` (`phantom_count: 0`) — live
      manifest, not exit code.
...
- [x] ✅ [DATA] P1. **DONE 2026-07-27 (slot-2, `data_engineering`) — Re-partitioned the pre-~2026-02 LIGHTER-ZKSYNC
      `ohlcv_1m` tail out of `batch_tardis`.** Conflict-check: Track 1's Script 2 rename HAD already run over this
      window, but on a DIFFERENT axis (filename stem, not the `pipeline_mode=` segment) — orthogonal/commutative,
      confirmed safe. Ran
      `restamp_lighter_ohlcv_batch_tardis_to_lighter_api_2026_07_18.py --start-date 2025-07-01     --end-date 2026-02-05`:
      dry-run **1,030 objects** (matches ~1,050 estimate, stems already canonical form). `--apply`: **1,030 moved, 0
      conflicts**, 1,030 manifest rows written. Consolidator `--force` OOM'd on the 2GB `/tmp` tmpfs (fixed via `TMPDIR`
      redirect to the 157G disk); live prod cron merged the shard first anyway. **Re-verified** via fresh
      `availability_index`: LIGHTER-ZKSYNC `ohlcv_1m` = 1,505 rows, 100% `batch_lighter_api`/`captured`, ZERO
      `batch_tardis` corpus-wide. LIGHTER `derivative_ticker`/`trades`/ `book_snapshot_5` untouched (script scopes
      `data_type=ohlcv_1m` only). Repo: market-tick-data-service (script run, no code change). Source:
      `issues/cefi_onchain_venues_mislabeled_batch_tardis_2026_07_20.md`.
- [x] ✅ [DATA] P1. **DONE 2026-07-27 (slot-13, data_engineering).** Built + ran read-only
      `market-tick-data-service/scripts/characterize_extended_starknet_tardis_vs_extended_divergence_2026_07_27.py`
      against prod: 5 days (01-15/02-15/04-10/04-15/05-01) × 11 overlapping `derivative_ticker` instruments = 50 shards.
      **Row counts + `funding_rate` values match exactly** every shard (delta=0, 24/day) — both lanes see the same real
      market truth. The ONE genuine value divergence is `underlying`: `batch_tardis` stores the bare symbol (`AAVE`),
      `batch_extended` stores `SYMBOL-USD` (`AAVE-USD`) — disagrees 24/24 rows every shard, a representation difference,
      not conflicting data. Column-set diff every shard: `batch_tardis` has `chain`; `batch_extended` has
      `open/high/low/close/volume` (always null for this data_type). Manifest cross-check: 04-10/04-15/05-01 carry one
      `batch_tardis`/`expected_unattempted`/`source=tardis` row per key (NOT `captured` — the real object is
      unmanifested); 01-15/02-15 show no manifest row at all. `ohlcv_1m`: **zero overlap confirmed on all 5 days**
      (`batch_extended` had 0 `ohlcv_1m` objects every sampled day), extending the source doc's finding with 5 measured
      days. Zero GCS/manifest writes; no authoritative copy named (operator decision, per scope). Repo:
      market-tick-data-service@4346f587. Source: `issues/cefi_onchain_venues_mislabeled_batch_tardis_2026_07_20.md`.
- [x] [BACKEND] P1. **Register `volatility_index` in cefi's data-type enumeration.** Add `"volatility_index"` to
      ...
      `issues/cefi_shard_enumeration_blindspots_and_canonical_fetch_dependency_2026_07_18.md`. — ✅ DONE 2026-07-26
      (slot 3): `unified-api-contracts@dfecc787` — `"volatility_index"` added to `DATA_TYPES_BY_ASSET_GROUP["cefi"]`
      with an explanatory comment mirroring the OKX-FUTURES/OKX-SWAP precedent's style; added
      `test_cefi_has_volatility_index_data_type`. Confirmed via grep: `volatility_index` was already a registered
      `DataTypeCapability` (`data_type_capability.py`) and `SOURCE_PRIORITY[("cefi","volatility_index")]` entry
      (`_source_priority_data.py`) — only this ONE enumeration list was missing it, no parallel hardcoded cefi data-type
      list needed a matching edit. Full repo `quality-gates.sh` green. Shipped via quickmerge, confirmed on
      `origin/live-defi-rollout`.
...
- [x] ✅ [PM] P1. **DONE 2026-07-27 (slot 5)** — Verified + archived
      `mtds_mdps_order_book_imbalance_duplicated_2026_07_07.md`. Grepped UAC for `order_flow_imbalance`: zero live
      entries remain (only retirement comments), matching `unified-api-contracts@49314f51` as the shipping commit
      closing the P1 UAC-retirement checkbox; closed P2 numeric-agreement as MOOT (zero production rows ever captured to
      compare). Doc itself was already archived + `status: resolved` with both checkboxes `[x]` — only this plan
      checkbox + 5 stale corpus referrers (old `archive/issues/` path) hadn't been fixed:
      `instruments_completion_tracker_2026_07_06.md`, `cefi_consolidated_closeout_aggregated_sources_2026_07_24.md`,
      `cefi_satellite_ao_dispatch_batch1_finalize_2026_07_25.md`, `ag_closeout_audit_rollout_2026_07_25.md`,
      `issues/instruments_remaining_work_audit_2026_07_10.md` — all now point at `/plans/archive/issues/...`. Left the
      archived historical record `plan_reconciliation_operator_decisions_history_part1_2026_07_25.md` untouched (frozen
      point-in-time narrative). Evidence: `unified-trading-pm@<pending-sha>`.
- [x] ✅ [SCRIPT] P1. **DONE (2026-07-27, slot-5).** **Verify rotate-exchange-keys' venue registry + invocation path (2
      sub-items merged — both append to the same issue doc's evidence trail):** (a) Verify every venue secret name
      referenced in `deployment-service/functions/rotate-exchange-keys/main.py`'s venue list against live GCP Secret
      Manager (`central-element-323112`) — for all ~29 listed entries (including the 5 never-verified: coinbase, kraken,
      ...
      verdict for the invocation path, with the specific gcloud evidence, is appended to the same evidence trail. — (a)
      34/34 entries live-verified, 0 unverified; (b) verdict DEAD/UNWIRED, confirmed by both live gcloud (no
      function/scheduler/Run service anywhere) and Terraform (`enable_secret_rotation` defaults `false`). Full tables +
      gcloud evidence: `/plans/archive/issues/rotate_exchange_keys_stale_venue_registry_2026_07_23.md` Progress Log
      2026-07-27 entry. Source: `/plans/archive/issues/rotate_exchange_keys_stale_venue_registry_2026_07_23.md`.
- [x] ✅ [SCRIPT] P1. **Build a dry-run-only reclass script for cefi Tardis-400 impossible-combination
      `attempted_failed` rows.** Mirror the repo's established `reclass_*.py` pattern (dry-run by default, `--apply`
      flag present but NOT invoked in this todo's scope, snapshot-before-write, before/after row counts) — identify cefi
      manifest rows attributable to Tardis HTTP-400 `code=300` (invalid-symbol) / `code=140` (date-not-available), the
      ...
      `issues/tardis_impossible_combinations_recorded_as_attempted_failed_2026_07_17.md`. — **DONE**:
      `market-tick-data-service@c36d35d1`, QG green. Dry-run found 5,572 rows (was 24,410 on 2026-07-18 — see issue
      doc's Progress Log). `--apply` never invoked.

## Progress Log

- **2026-07-27 (slot-13, `cefi_satellite_ao_dispatch_batch1-015`):** `expiry_date` trace todo already done same-day by
  slot-15 in the source issue doc (`cefi_high_attempted_failed_batch_cluster_2026_07_23.md` todo 3 + the § 8a addition
  to `deribit_combo_perpetual_partition_move_2026_07_21.md`) — hypothesis CONFIRMED, fix already shipped (`@2ddc6d4a`).
  Verified at fresh-pulled HEAD, flipped checkbox citing that evidence. No code shipped (none needed).
- **2026-07-26 (slot 6) — todo -001 scoping.** Dispatched the "Extend MDPS candle-building to the 4 on-chain-perp CeFi
  ...
  - **ASTER carved OUT of this pass** — its manifest shows 486,890 `expected_unattempted` / 300 `attempted_failed` /
    ...
    banner. Filed as a P0 big-finding issue doc:
    `/plans/archive/issues/aster_raw_capture_manifest_registration_gap_2026_07_26.md` (`unified-trading-pm@580d1cdf7`).
    A manifest-scoped backfill range for ASTER would be wrong right now (it would think almost nothing exists), so ASTER
    is deferred to that issue doc's own remediation, not re-attempted here blind.
...
- **2026-07-30 (slot-11, `data_engineering`, `cefi_satellite_ao_dispatch_batch1_finalize_2026_07_25.md` todo 4 —
  archival).** The finalize plan's own todo-1 reconciliation pass (2026-07-30, slot-9) flagged that line 355's BYBIT
  futures_chain extended duplicate-verification todo was still genuinely `- [ ]`, the only one of this plan's 33 not
  done, creating a gate-vs-reality mismatch (this finalize plan is machine-gated `depends_on`+`gate_on_depends: true` on
  all 33 being done, yet the dispatcher queued the archival todo anyway). Resolved per
  `/codex/12-agent-workflow/plan-completion-and-archival-discipline.md`'s near-complete-plan handling ("split it, or
  fold the remnant"): migrated the still-open item to its own named Source doc
  (`issues/bybit_futures_chain_write_shape_2026_07_13.md`) as a fresh open todo there — marked `[x] → MIGRATED` here
  (not done, never shipped under this plan) rather than falsely claiming completion. No operator ruling needed: the fold
  target was unambiguous (the item's own cited Source doc, already active/unlocked, single candidate — not a multi-epic
  consolidation choice). Archiving this plan now via the standard 6-step ritual; corpus referrers (17 files using the
  literal `plans/archive/2026_08/...` path) repointed to the new archive location in the same commit.
...
| ------------------------------------------------------------------------------------- | ----------------------- | --------------------------------------------------------------------------------------------- |
| ASTER raw-capture manifest-registration gap                                           | Operator/AO-owned       | `/plans/archive/issues/aster_raw_capture_manifest_registration_gap_2026_07_26.md`             |
...
| ADV reader `quote_volume` column never exists (P1, cross-repo)                        | Operator/AO-owned       | `/plans/archive/issues/rolling_adv_reader_quote_volume_column_never_exists_2026_07_26.md`     |
