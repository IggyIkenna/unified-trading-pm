...
tags: [ci-cd, github-actions, cost, self-hosted-runner, workflows, spend-reduction, archive]
related:
  [
...
7. **One P0 is open and is NOT blocked on the gate** — the **quickmerge `--agent` sentinel race** (its own STAGE-0.4
   rebase invalidates the sentinel STAGE 3 then checks, so on a busy LDR it can never self-validate; workaround = chain
   `quality-gates.sh --no-fix && quickmerge.sh` in ONE shell to close the window). **[2026-07-24 split note: this todo
   now lives in `plans/active/github_actions_operator_gated_followups_2026_07_17.md`, still open — it is preserved here
   as historical context, not a live item in this archived doc.]**
