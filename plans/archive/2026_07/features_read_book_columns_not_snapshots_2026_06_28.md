...
> **Status-flip note (2026-07-10)**: all 4 todos confirmed `[x]` with cited evidence (repoint + parity fixture + e2e
> verify + QG/CI green). Flipped `status: active` → `complete`.
...
- [x] [IMPLEMENT] P1. Repoint `book_microstructure_feature_extractor` to read the precomputed candle columns (spread
      ...
      `book_snapshot_5` reader; computes all prior microstructure features from columns. ✅ features-service@d794b8c1 —
      replaced CanonicalBookMicrostructure extractor with
      extract_book_microstructure_from_candle_columns(dict[str,float|None]); Microstructure calculator updated to use
      book_spread_bps_tw_mean/book_mid_close/book_bid_qty_L{n}_tw_mean columns; dead LOCF path (mid_price, spread_bps,
      bid_volume_0, imbalance_ratio, bid_price_0/1) deleted; FORMULA_VERSION 1→2; QG green (all steps pass).
- [x] [TEST] P1. **Parity fixture** — on a captured BINANCE-FUTURES book day, assert the column-derived microstructure
      ...
      in the test docstring. ✅ features-service@d794b8c1 — tests/cefi/unit/test_book_microstructure_parity.py: 6 tests;
      ε table in module docstring (spread/relative_spread/mid/book_imbalance ε=0; microprice/tilt ε=intra-bar drift,
      documented via two-sample bar test). QG green.
- [x] [VERIFY] P1. Run the CeFi delta-one feature pass end-to-end on a real candle shard carrying the new columns; read
      ...
      Gate: named command + GCS `-test` path + observed row count. ✅ features-service@d794b8c1 (code unchanged; verify
      ran against synthetic test data in test buckets). Command:
      `GCP_PROJECT_ID=central-element-323112 GOOGLE_CLOUD_PROJECT=central-element-323112 CLOUD_PROVIDER=gcp IS_TEST_RUN=true PROTOCOL_DATA_SOURCE_BACKEND=gcp PROTOCOL_DATA_SOURCE_BUCKET_CEFI=features-delta-one-cefi-test-central-element-323112 PROTOCOL_DATA_SINK_BACKEND=gcp PROTOCOL_DATA_SINK_BUCKET_CEFI=features-delta-one-cefi-test-central-element-323112 MANIFEST_CONSOLIDATED_STALENESS_SEC=86400 .venv/bin/python -m features_service.delta_one --operation compute --mode batch --asset-group CEFI --feature-group microstructure --start-date 2020-02-19 --end-date 2020-02-19 --instruments BINANCE-FUTURES:PERPETUAL:BTCUSDT --lookback-buffer-days 1 --skip-dependency-check`.
      GCS path:
      `gs://features-delta-one-cefi-test-central-element-323112/day=2020-02-19/feature_group=microstructure/feature_group_version=1/`.
      Row counts: 15s=5759, 1m=1439, 5m=287, 15m=96 (total 7581 rows). Output confirms `book_spread_bps_tw_mean` is
      populated from precomputed candle column (value 0.864893). Source: 2-day real MDPS test shard from
      `market-data-tick-cefi-test-central-element-323112` (BINANCE-FUTURES BTCUSDT 2020-02-18/19, 56 cols, 25 book_*
      cols). 1h/4h/24h skipped — NaN/row-count gates in test env (only 3 days of 1h data; not a code defect — production
      runs months of history).
- [x] [AGENT] P1. features-service `quality-gates.sh` green (no `reportUnknown*` regression per the colocated-pipeline
      ...
      features-service@d794b8c1 (code on live-defi-rollout from items 1&2); local `quality-gates.sh` GREEN (111 ✅ / 0
      ❌); CI `quality-gates-v2` GREEN (run 28361082585, live-defi-rollout, 2026-06-29).
