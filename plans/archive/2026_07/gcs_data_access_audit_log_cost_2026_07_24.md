...
locked_by:
locked_since:
resolved_by: operator (ikenna@odum-research.com, 2026-07-25)
---
...
- [x] [DEVOPS] P2. **Remove the `storage.googleapis.com` / `DATA_WRITE` entry from the project's IAM `auditConfigs`** —
      DONE 2026-07-25, operator-run via personal ADC (`ikenna@odum-research.com`, confirmed both
      `getIamPolicy`+`setIamPolicy` via `testIamPermissions` before writing). Fresh `getIamPolicy` immediately before
      writing (etag `BwZXcJYHOpc=`, 107 bindings, the one `auditConfigs` entry matching this doc's description exactly);
      edited only `auditConfigs` → `[]`, diff-verified `bindings`/`etag` untouched; applied via `projects:setIamPolicy`
      REST (not the `gcloud` CLI — see the updateMask gotcha below). **Real gotcha hit**: the API's default `updateMask`
      is only `bindings,etag` — a plain `gcloud projects set-iam-policy` (or an unmasked REST call) would have silently
      NO-OPPED on the `auditConfigs` removal, leaving the policy unchanged with no error. Had to pass an explicit
      `updateMask: "bindings,etag,auditConfigs"` to make the write actually take. HTTP 200; response confirmed
      `auditConfigs: null`, `bindings` still 107, new etag `BwZXdmXibOc=`. Independent step-4 re-verification (separate
      `gcloud projects get-iam-policy --format='json(auditConfigs)'` call, not just reading the write response): `null`.
      `bindings` count re-confirmed 107. No other part of the policy touched. **Worth a codex note** (not yet filed as
      its own doc): any future `setIamPolicy` write that touches a field OTHER than `bindings`/`etag` must pass an
      explicit `updateMask` naming that field, or the API will silently accept the call and do nothing to it.
...
- **2026-07-25** — Operator ran the handed-off P2 recipe personally via ADC on their own machine (a live, non-expired
  `ikenna@odum-research.com` token this session never had access to). Both `getIamPolicy`+`setIamPolicy` confirmed via
  `testIamPermissions` before writing; fresh read (not reusing the stale prep from this doc's earlier quote) → edit →
  explicit-`updateMask` write → independent step-4 re-verification, all as specified. **All todos in this doc now
  closed** — see the P2 todo above for the full evidence trail and the `updateMask` gotcha worth carrying into any
  future project-IAM-policy write.
