...
created: 2026-07-06
last_updated: "2026-07-24" # all 13 todos closed (11 done in place, 2 forked out verbatim to a new plan + a sibling plan) — resolved + archived via the plan-hygiene line-cap remediation
...
> **🟢 2026-07-24 — RESOLVED + ARCHIVED.** All 11 of this plan's own todos are `[x]` done. The 2 remaining open items
> were forked out verbatim (not dropped) during the operator-approved plan-hygiene line-cap remediation
> (`plans/active/issues/plan_line_cap_remediation_2026_07_23.md` row 30): task 11 (legacy-twin bucket deletes,
> BLOCKED-OPERATOR-DECISION) → `/plans/active/tradfi_legacy_twin_bucket_deletes_signoff_2026_07_24.md`; the
> Folded-in-scope "Certify tradfi Layer-1" todo (BLOCKED-PLAN2) → directly into
> `/plans/active/tradfi_consolidated_closeout_2026_07_18.md`'s own Phase C todo list. This plan carries no further open
> work.
...
- [x] ✅ [DATA] P0. **🎯 GATE MET 2026-07-16 — the literal gate ("100% schema_version=9") is now genuinely true, closed
      ...
      **UPDATE 2026-07-12T09:05Z (slot-3) — the row-loss blocker is now RESOLVED; one gate metric changed materially,
                                                                                                                                                      ...
                                                                                                                                                      **UPDATE 2026-07-21 (this session) — GATE MET, checkbox flipped.** Task 10's fleet-drain + v4-tail re-stamp
                                                                                                                                                      closed 2026-07-16 (header banner). Re-ran the full CF audit fresh against the CURRENT tradfi manifest
                                                                                                                                                      (`cf_manifest_audit_2026_06_01.py market-data-tick-tradfi-prd-central-element-323112`, via the non-snap
                                                                                                                                                      `/home/ubuntu/google-cloud-sdk/bin/gcloud` on `PATH` — the snap install is still broken in this slot, same
                                                                                                                                                      workaround every prior session used). **Fresh read: 6,233,410 rows.** CF-1 GREEN (`v9=6,233,410/6,233,410`,
                                                                                                                                                      100% — the 13,971-row v4 tail is genuinely gone). CF-3 GREEN (`pipeline_mode` populated
                                                                                                                                                      6,233,410/6,233,410, 100%, all source-aware form). CF-4 GREEN (`source` blank=0/6,233,410). CF-5/CF-6/CF-9/
                                                                                                                                                      CF-13/CF-2-paths/CF-3-partition all GREEN; CF-7 clean (sane `data_type`/`venue` distributions); CF-10/CF-14
                                                                                                                                                      SKIP (documented cross-tool / cross-bucket scope, unchanged). **Only CF-8 (`available_at` non-null
                                                                                                                                                      1,863,562/6,233,410) and Era-B (chain `data_type` rows=112,847) remain RED-on-literal-check** — verified
                                                                                                                                                      against `tradfi_manifest_canonicalisation_2026_06_01.md` that these are the SAME documented, adjudicated
                                                                                                                                                      non-issues every prior session since 2026-07-08 found (CF-8: `available_at` is a per-row PARQUET-layer
                                                                                                                                                      field, not a manifest-schema field at all — the tool's literal check conflates the two, reclassified there
                                                                                                                                                      as an E4 parquet-layer verify, not a manifest gap; Era-B: the same long-standing non-zero chain-row class,
                                                                                                                                                      unchanged in kind across every session's reading). This is exactly the outcome this todo's own text
                                                                                                                                                      predicted ("it should now come back net-GREEN with only CF-8/Era-B RED-on-literal-check ... at which point
                                                                                                                                                      flip `- [x]`"). No repo code commit (read-only audit re-verification, same precedent as every prior session
                                                                                                                                                      in this history); the PM plan-doc flip ships via the `docs(plans):` carve-out.
...
- [x] ✅ [DATA] P0. **🎯 GATE MET 2026-07-21 — fresh E7 CF-1..CF-14 re-run comes back net-GREEN, only CF-8/Era-B
      RED-on-literal-check remain (the same pre-existing adjudicated non-issues every prior session found). Checkbox
      flipped.** History below. UNBLOCKED 2026-07-21 — task 10's fleet-drain + v4-tail re-stamp CLOSED 2026-07-16 (see
      task 10's own entry + the header banner: corpus now 100% `schema_version=9`, blank `pipeline_mode=0`, blank
      `source=0`, `5,553,198/5,553,198` rows). This clears the CF-1/CF-3/CF-4 v4-tail RED this todo was blocked on.
      **Was: BLOCKED- PREREQUISITES (2026-07-13, slot-5).** Do NOT blind-flip the checkbox — a fresh E7 CF-1..CF-14
      re-run is still the closing action for this DATA todo; it should now come back net-GREEN with only CF-8/Era-B
      RED-on-literal-check (both pre-existing adjudicated non-issues per the history below), at which point flip `- [x]`
      with the audit evidence. **E7 verify** — `cf_manifest_audit_2026_06_01.py market-data-tick-tradfi-prd-…` →
      CF-1…CF-12 all GREEN. Gate: audit passes clean; evidence recorded in the Progress Log. **STATUS 2026-07-08 (slot-7
      sonnet/high):** ran the full audit inline (the shipped `cf_manifest_audit_2026_06_01.py` uses subprocess
      `gcloud storage cp/ls`, broken in this slot per snap-confine — replicated via UTL storage client, same workaround
      as the 2026-07-07 session). Result: **not all-GREEN, checkbox stays unflipped.** See Progress Log for the full
      per-CF breakdown. Two genuine REDs remain (CF-1 schema tail = task 10's scope; CF-3 `pipeline_mode` blank = the
      new CF-3 todo filed today); CF-4/CF-7 are now GREEN; CF-8 and Era-B are RED on this tool's literal check but both
      are pre-existing, already-adjudicated non-issues per `tradfi_manifest_canonicalisation_2026_06_01.md` (linked
      below), not new gaps.
...
> **2026-07-24 — task 11 (legacy-twin bucket deletes, BLOCKED-OPERATOR-DECISION) forked out verbatim** to
> `/plans/active/tradfi_legacy_twin_bucket_deletes_signoff_2026_07_24.md` during the plan-hygiene line-cap remediation
> (`plans/active/issues/plan_line_cap_remediation_2026_07_23.md` row 30). Read that plan for the full todo text + the
> dry-run evidence pointer (task 2's orphan-sweep report, referenced not duplicated).
...
> **2026-07-24 — this section's sole todo (Certify tradfi Layer-1, BLOCKED-PLAN2) relocated verbatim into
> `/plans/active/tradfi_consolidated_closeout_2026_07_18.md`'s own Phase C todo list**, during the plan-hygiene line-cap
> remediation (`plans/active/issues/plan_line_cap_remediation_2026_07_23.md` row 30) — that plan is where the "FINAL
> STEP" catalogue rebuild/promote it is gated on is tracked. Not duplicated here.
