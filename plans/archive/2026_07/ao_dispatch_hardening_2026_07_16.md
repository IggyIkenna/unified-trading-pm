...
source:
  ...
  - ../archive/issues/ao_dispatch_residuals_2026_07_15.md (R1-R7 index; R1/R2/R5/R6 code-confirmed)
...
- [x] [REVIEW] P2. ✅ **DONE 2026-07-17 — Phase 3's runtime gate passed, so archival unblocked. 4 docs archived; the
      gate ("no residual left without a home") is MET, but only after re-homing R7 — it did NOT have a home.** Archived
      to `plans/archive/issues/`, each with an `ACKED-INTO-CODE`/`ACKED` banner + populated `resolved_by`:
      `ao_dispatch_residuals` (the R1–R7 index), `dispatcher_role_eligibility_gap_review_slots` (R6),
      `ao_fleet_stall_opus_spawn_and_skip_thrash` (R2/R3/R4), `ao_operator_message_silent_drop_no_reply_ack`. - **R7 had
      NO home and this todo's own text was wrong about it.** The line above says "R3/R4/R7 dispositions are recorded
      explicitly … so nothing goes dark" — but recording a disposition in a doc that is itself being archived is not a
      home. The R1–R7 index was the **only** live doc carrying R7, and the index's own suggested owner
      (`ao_dispatch_correctness_regen_reconcile_2026_07_07`) is **also archived**. Archiving it as written would have
      reproduced exactly what
      ...
      records in its own banner: archived with live `- [ ]` bugs and no `superseded_by`, orphaning them until a plan two
      days later rediscovered them. R7 is re-homed to
      [`regen_positional_task_ids_not_content_stable_2026_07_17`](issues/regen_positional_task_ids_not_content_stable_2026_07_17.md)
      with `superseded_by` wired from the index. - **R7's wording was imprecise and is corrected in the successor, not
      carried forward.** Re-verified at `agent-orchestrator@3f265cc`: ids ARE positional (`_make_task_id`
      `regen_backlog_from_plan.py:1024`, `next_index = max(used)+1` `:1223-1229`, over **backlog.yaml** not the DB —
      `:1130`/`:1135`), but a brief-keyed reconcile (`:1217`) and a cross-plan brief skip (`:1271`) mean a re-read `[x]`
      todo landing on its OWN id hash-MATCHES and is correctly left `done`. The real failure needs a stale read **AND**
      a position shift, and its worst consequence is not the re-dispatch — it is the **sibling row's destroyed
      `done_sha`**. The plan's original ruling ("R7 needs an external race and explains none of the operator's
      symptoms") is therefore **confirmed by code, not merely asserted** — the race is
      `ao_service_clone_frozen_by_untracked_checkpoint_2026_07_16`. - **`ao_skip_blind_spawn_budget_phantom_churn`
      deliberately NOT archived** — 1 genuinely open todo (durable park) whose own text says "this doc stays OPEN for
      it". It is the CODE feature (auto-park at ≥N skips + operator visibility); Phase 3's open P1 is the OPERATIONAL
      sports instance. Different work, so it is not dual-tracked and archiving would orphan a real residual. - **The 2
      `[~]` in `ao_operator_message_silent_drop` were stale, not open** — one superseded by design, one
      (`needs_operator_count` badge) shipped at `@fa73b5d` a day earlier. That doc's own `resolved_by` already recorded
      the sha while the checkbox still read "Remaining": half-1-without-half-2, inside the very doc set this todo
      audits. Both flipped, and the badge **re-verified at HEAD** (`layout.tsx:2469-2474`/`:2538`/`:2588`,
      `types.ts:447`) rather than trusted. - **Dead links swept**: 4 archived paths repointed in
      `ao_dispatch_hardening`, `instruments_completion_tracker`, `mvp_backfill_defi_v10_002_dispatch_thrash`; each
      verified to RESOLVE on disk. `plan_reconciliation_operator_decisions_2026_07_11` deliberately left alone — it
      QUOTES `file:line` as they stood at audit time (it already cites long-archived paths); rewriting a historical
      record to keep links green would falsify it. ~~Close out the source issue docs once Phase 3's runtime gate passes
      — archive~~ `ao_skip_blind_spawn_budget_phantom_churn` (R1), `dispatcher_role_eligibility_gap_review_slots` (R6),
...
| 8   | ~~**uv-cache dual-path reconcile**~~ **— CLOSED 2026-07-17** | `ao_host_disk_pressure_2026_07_16` (Phase 2 P2, now `[x]`)                        | **This row's reasoning was DISPROVEN by measurement — do not cite it.** "Both caches sit on the same filesystem" was true of the two known caches and irrelevant to the **third** (`/home/hk/.cache/uv`), which is cross-filesystem from the venvs it links into AND on the partition that is actually full (`/` 84% vs `/active` 53%). `/active/uv-cache` was also not "the live source" — it died 2026-07-08. Closed by correcting the record; 2 operator-owned actions now tracked in that plan's own `## Deferred work after 2026-07-17`.                                                                                                                                                                  |
...
| 2   | **Unauditable `done` rows** (`brief_hash` NULL) — **56 of 66 @ 2026-07-17T13:19Z** (was 58 of 64; the count MOVES — re-measure, don't cite) | **Not done.** Unknown ≠ clean; `gate_on_depends` trusts them. Backfill `brief_hash` or rule the tail out of scope with a recorded WHY. **Now also has a second consequence**: these rows are exempt from the id-reuse guard too, so they are un-GUARDED as well as un-AUDITABLE — see [`regen_positional_task_ids_not_content_stable_2026_07_17`](issues/regen_positional_task_ids_not_content_stable_2026_07_17.md), which owns the guard half and carries the live query. | nobody — pick it up   |
...
for the unhashed tail and is the last place a false `done` can still hide — and it got **cheaper** on 2026-07-17: the
live probe showed every unhashed row is a `done` row (no in-flight NULLs remain), which retires half the reasoning that
made the exemption look permanent.
