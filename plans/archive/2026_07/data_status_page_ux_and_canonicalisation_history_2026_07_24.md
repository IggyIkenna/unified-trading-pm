...
summary: >-
  Archive-bound extraction from /plans/active/data_status_page_ux_and_canonicalisation_2026_07_16.md, split out purely
  to bring that plan back under its enforced 1000-line non-umbrella size cap. Content is verbatim and historical only.
  Pass 1 (first section below): two fully-closed, dated Progress Log entries -- the 2026-07-17 P2/P3 remaining-work
  session journal (13 deprioritized follow-ups worked, session-end state, the "5 of 13 premises were wrong" lesson
  table), and the 2026-07-17 Playwright re-verification session that found + fixed a month-old mock-api.ts catch-all
  routing bug shadowing every /api/data-status/* endpoint. Pass 2 (second section below, same date): the FULL remaining
  Progress Log, the stale "Deferred / remaining work" tracking table, and every already-shipped (`- [x]`) checkbox's
  full evidence body across P1-P10 (61 checkboxes -- commit SHAs, real-infra migration evidence, rollback snapshots,
  unit-test names). Zero open todos here -- this file is a record, not a work queue; the one still-open checkbox from
  the parent plan (P3's `extra='forbid'` side-discovery) was deliberately NOT moved and stays live in the parent.
status: complete
nature: record
