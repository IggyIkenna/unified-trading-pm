...
doc_type: plan
title:
  Data-status page UX & canonicalisation — extracted history (Progress Log + all P1-P10 completed checkbox evidence)
summary: >-
  Archive-bound extraction from /plans/archive/2026_08/data_status_page_ux_and_canonicalisation_2026_07_16.md, split out
  purely to bring that plan back under its enforced 1000-line non-umbrella size cap. Content is verbatim and historical
  only. Pass 1 (first section below): two fully-closed, dated Progress Log entries -- the 2026-07-17 P2/P3
  remaining-work session journal (13 deprioritized follow-ups worked, session-end state, the "5 of 13 premises were
  wrong" lesson table), and the 2026-07-17 Playwright re-verification session that found + fixed a month-old mock-api.ts
  catch-all routing bug shadowing every /api/data-status/* endpoint. Pass 2 (second section below, same date): the FULL
  remaining Progress Log, the stale "Deferred / remaining work" tracking table, and every already-shipped (`- [x]`)
  checkbox's full evidence body across P1-P10 (61 checkboxes -- commit SHAs, real-infra migration evidence, rollback
  snapshots, unit-test names). Zero open todos here -- this file is a record, not a work queue; the one still-open
  checkbox from the parent plan (P3's `extra='forbid'` side-discovery) was deliberately NOT moved and stays live in the
  parent.
status: complete
nature: record
...
depends_on:
source:
  extracted 2026-07-24 from /plans/archive/2026_08/data_status_page_ux_and_canonicalisation_2026_07_16.md to bring that
  plan under its 2000-line umbrella cap
---
...
> this file. See `/plans/archive/2026_08/data_status_page_ux_and_canonicalisation_2026_07_16.md` for the live plan.
...
## Extracted 2026-07-24 (second pass) — completed P1-P10 checkbox evidence + full Progress Log + stale Deferred-work table

> Second extraction pass on the same date, needed because the parent plan (1971L) was still over its enforced
> non-umbrella 1000L hard cap after the first pass above (161L). Judgment call: the plan's remaining content was
> genuinely dense multi-paragraph active-work narrative (forensic root-cause writeups, adversarial-review banners,
> real-infra migration evidence with rollback snapshots) — not coordinator/index-shaped — so per `check_line_caps.sh`'s
> own umbrella-exemption doc comment, `umbrella: true` was NOT the right fix. Instead, every ALREADY-SHIPPED (`- [x]`)
> checkbox's full evidence body was moved verbatim below, per P-point, leaving only each point's Design Guide +
> Acceptance criteria + the one still-OPEN checkbox in the live parent. Every checkbox line moved here is UNCHANGED text
> — nothing summarized, nothing paraphrased, nothing deleted.

### Progress Log (full, verbatim — all dated session-narrative entries)

## Progress Log

> **History extracted 2026-07-24**: the 2026-07-17 "P2/P3 remaining-work session" + "Playwright re-verification"
> Progress Log entries were moved verbatim to
> `/plans/archive/2026_07/data_status_page_ux_and_canonicalisation_history_2026_07_24.md` to bring this plan under its
> 2000-line cap.
...
### Deferred / remaining work (verbatim — the 2026-07-17-dated tracking table + its narrative sub-entries; superseded by the per-section checkbox status in the live plan and by the current-as-of-2026-07-18 "Deferred work after 2026-07-18" table at the live plan's end)
...
| 5   | ~~Distinct `expiry` column in `CATALOG_COLUMNS`~~             | P2 clean-LT   | P2  | ✅ **DONE 2026-07-17** — forward-only column @f0223d6b; historical backfill = archival-only, not run (see todo). |
...
| 7   | ~~Real `question`/`title` column~~                            | P3 follow-up  | P3  | ✅ **FORWARD DONE 2026-07-17** — @c1de078a+@2257a067+@2f52991e+@9e1a6647 (NO re-capture); backfill in flight.    |
| 8   | ~~Root-cause remaining DeFi legacy `instrument_type`~~        | P4-A (A)      | P2  | ✅ **DONE 2026-07-17** — 0 legacy VALUES remained; real fix was 78,489 captured-BLANK rows @585bb845.            |
...
- [x] [DATA] P3. ✅ _(new, low-priority)_ Enriched `deployment-ui/src/lib/mock-api.ts` with the instruments-service
      data-status fixtures. **Re-verified all three gaps were REAL first** (the dispatch warned some might merely have
      been unreachable behind the `0c817d2` catch-all routing bug): (a) `shard_axes` had ONLY
      `market-tick-data-service.prediction` — no `instruments-service` key at all; (b) `/coverage-summary` was hardcoded
      to a single PREDICTION entry **regardless of `?service=`**; (c) no TEAMS/STANDINGS/VENUES fixtures existed. All
      three genuinely absent, so all three were added. — deployment-ui@25262b8 + Evidence: new
      `src/lib/mock-api.is-data-status.test.ts` **6 specs green** + full `quality-gates.sh` **green (143s:
      tsc/eslint/1005 tests/build)**. **(a)** shard/breakdown/primary axes copied **verbatim from the UAC SSOT**
      (`registry/data_status_axis_matrix.py`) rather than invented — this matters because
      `isHierarchicalDrilldownRedundant` (P5) decides purely on whether an IS asset_group's axes ⊆ `{venue, chain}`, and
      `BreakdownsAccordion` (P4-A) only mounts when `breakdown_axes` is non-empty; a hand-waved shape would make mock
      mode agree with a bug. A spec pins the P5 predicate directly (cefi/tradfi/defi ⊆ {venue,chain} = true;
      sports/prediction = false). **(b)** `/coverage-summary` is now **service-aware**, returning all 5 IS asset groups
      with real-ish shapes drawn from this session's own measurements; MTDS still gets its original PREDICTION payload
      (pinned by a spec so the pre-existing consumer can't regress). CEFI's `instrument_type` breakdown deliberately
      mixes canonical (SPOT_PAIR/PERPETUAL/FUTURE) **+ legacy-lowercase (`spot`, `perpetual`) + the blank `__legacy__`
      sentinel** so P4-A's alias map and the axis-aware "(unlabeled)" vs "(legacy — pre-job_id)" rendering finally have
      real inputs. Only DEFI carries `extras.chains` — a spec asserts cefi/tradfi have **none**, which is precisely the
      P7 bug. **(c)** TEAMS added as **`per_league_trigger_date` with a populated `leagues` map** (matching P8's
      reclassification @fb0eec8 — this fixture is what makes that check possible at all; it was the reason the P8
      browser check came back INCONCLUSIVE), modelling P8's honest-absence case too (LA_LIGA has expected trigger dates
      but 0 captured = legitimately empty, not a gap); STANDINGS per-league; VENUES `global_season` with no league map
      so the "Global reference entity" affordance renders. **Two findings while doing it:** (1) **a real bug in my own
      first cut, caught by testing through the router**: I read `?service=` off `path`, but `handleRoute` **strips the
      query string** from `path` (`.replace(/\?.*$/, "")`), so the branch never fired — fixed to read the raw `url` like
      the other query-aware handlers. (2) The specs deliberately drive the **REAL mock router**
      (`installDeploymentMockHandlers` → `window.fetch`) instead of importing fixtures, because that is exactly the
      blind spot that let the month-old catch-all shadowing bug survive: every component test `vi.mock`s its client
      function and never touches the dispatcher. These 6 specs also pin the `0c817d2` routing fix against regression.
      _(Ship note: the full suite showed 2 failures — `FixturesBrowser` (a concurrent agent's in-flight team/date-filter
      work) + `StrategyBacktests` — both **contention flakes, not mine**, proven three ways: neither spec references
      anything in my diff; both pass in isolation **with AND without** my change; host load average was **238-248 on a
      10-core box** (this plan already logged this exact flake class at load ~82-90). Re-ran the full gate once load
      allowed → green, sentinel written, then shipped.)_
...
### P1 — Honest Coverage: remaining hardening — completed checkbox evidence
...
### P2 — New Listings + Upcoming Expiries — completed checkbox evidence
...
- [x] [DATA] P2. ✅ _(clean long-term — FORWARD-ONLY column shipped; historical backfill deliberately NOT run, see
      below)_ Added a distinct `expiry` column to `CATALOG_COLUMNS` — **instruments-service@f0223d6b**. It emits the
      already-parsed venue-truth `_InstrumentAggregate.expiry` verbatim (the builder ALREADY computed it, then collapsed
      it into the overloaded `available_to` at the `elif agg.expiry is not None` branch — this just surfaces it), so
      `expiry IS NULL` honestly means "no venue-declared expiry" and the `available_to`+type-filter path in
      `list_upcoming_expiries` stays correct as the floor. 4 unit tests incl. the load-bearing case (a delisted SPOT
      pair: `available_to`=delisting date but `expiry`=NULL). **Forward-only: populates for every instrument on its next
      rollup.** **The historical backfill (operator ruled D3=A "targeted backfill") was investigated in depth and NOT
      run — there is no cheap honest mechanism, and it is archival-only value.** Measured (real prod GCS 2026-07-17): -
      A **full rollup** would re-walk from source and REVERT this session's direct-patch migrations (SPOT_ASSET
      population, address columns, split-venue canon) → the monotonic `CATALOGUE_SHRINK_BLOCKED` guard (same class that
      blocked defi). Unsafe. - An **in-place symbol-match** (`expiry = available_to` where a date parseable from the
      instrument_id confirms it) is honest but **poor coverage**: tradfi **2.8%** (its `CBOE:FUTURE:VX/F1`-style futures
      encode the contract month as a CODE, not a date — the real expiry is in `available_to` but unconfirmable from the
      symbol), cefi 66.4%. - An **`available_to`-blanket** (`expiry = available_to` for all FUTURE/OPTION/COMBO) is
      dense but **fabricates**: cefi source-expiry is only 25.6%, so ~74% of cefi expiring rows have `available_to` = a
      last-observed date, NOT an expiry — stamping it would invent expiries (honest-absence violation). - The only
      HONEST full-coverage path is a **corpus-scale source read** (re-read every instrument's `expiry` from its by_date
      snapshots — ~the same ~375k-blob, multi-hour walk the sports rollup just did), for a column whose ONLY consumer
      (`list_upcoming_expiries`) reads FORWARD instruments the rollup already covers. Nobody queries expiry on
      already-expired rows. **Recommendation: forward-only is the right stopping point** (operator flagged in chat). The
      corpus-scale D3 run remains available if archival completeness is wanted — it is a tracked decision, not lost.
...
### P3 — Prediction markets catalogue browser — completed checkbox evidence (the still-OPEN extra='forbid' todo stays in the live plan, not moved here)
...
- [x] [DATA] P3. ✅ _(follow-up)_ **FORWARD-ONLY code SHIPPED + adversarially verified (NO re-capture — the workflow's
      FATAL finding stands). In-place history backfill IN FLIGHT.** Done as a forward-only additive chain, NOT the
      re-capture the original todo implied (which would have destroyed Kalshi history + injected look-ahead bias — see
      the 🔴 redesign banner on the P3 note above): - **UAC** `InstrumentRecord.question: str | None` + aligned
      `INSTRUMENTS_PARQUET_SCHEMA` entry — additive, non-breaking (`unified-api-contracts@c1de078a`). This also FIXES a
      latent bug: both adapters were passing the title as a `symbol=` kwarg `InstrumentRecord` never declared, so
      pydantic's `extra='ignore'` SILENTLY DROPPED it every capture — that is why the label fell back to the slug.
      basedpyright already flagged `symbol=` as `reportCallIssue` at HEAD; both `symbol=` lines removed. -
      **instruments-service** (`@2257a067`): Polymarket sources `question=market.question` (honest-None, not `or ""`);
      Kalshi composes `f"{title} — {yes_sub_title}"` (100% unique vs 43.3% for title alone) — plus a follow-up fix
      (`@9e1a6647`) making Kalshi `question=None` when a market has NO human title (was falling back to the ticker — an
      honest-absence violation the adversarial verifier caught) so it is symmetric with Polymarket, raw_symbol the
      floor. `question` added to `CATALOG_COLUMNS`, emitted on the PREDICTION rollup path only (other AGs → None via
      reindex). - **deployment-api** (`@2f52991e`): `question` is the new TOP rung of `prediction_catalogue._label()`
      (`question → raw_symbol → base_asset[:50] → event_title → instrument_id`) + added to the schema-aware
      `_READ_COLUMNS` (degrades gracefully pre-backfill). Both code units adversarially verified PASS (forward-only
      chain real; honest-absence floor intact; tests assert the field survives + labels correctly). **Forward-only:
      every NEW prediction capture now carries the real question.** **History backfill — ✅ DONE + INDEPENDENTLY
      VERIFIED 2026-07-17** (operator ruling: in-place, lossless, no re-capture). Script + 22 tests shipped
      `instruments-service@ee4c33856`; applied to live `prod/catalog.parquet` (prediction bucket) by re-reading the
      8,882 legacy Shape-B raw-Gamma `by_date` objects that carry `question`/`condition_id` on disk (the catalogue rows
      themselves come from the IS-normalised/Shape-A objects, which never carried it). **Dispatcher independently
      re-read live GCS** (not the agent's log): catalog now **2,673,230 rows unchanged · 25 cols (was 24) · `question`
      at index 13 right after `base_asset` · 1,423,574 non-null (53.25%) == legacy-matched rows (no fabrication)**; the
      24-col pre-backfill rollback snapshot exists at `catalog.20260717T172057Z.questionbackfill.pred.bak.parquet`
      (2,673,230 rows / 24 cols / no question). Kalshi rows stay honest-null (legacy Gamma objects are Polymarket-only —
      never fabricated). The slug fallback already labelled ~99.996% readably; this upgrades slug→human-question for the
      legacy-covered historical POLYMARKET markets (711,787 markets, both data_types filled).
...
### P4 — Instrument Coverage Summary: canonical labels (A) + SPOT_ASSET population (B) — completed checkbox evidence
...
- [x] [UI] P1. ✅ _(A)_ Display-only canonical alias map `canonicalInstrumentTypeLabel`
      ...
      `[UI]` + pw:L2.
- [x] [DATA] P2. ✅ _(A)_ Root-cause the legacy values — grep the instruments-service catalogue/manifest writer for
      where `instrument_type` is stamped; ensure new rows emit `InstrumentType.value` (uppercase); author a one-off
      legacy-row canonicalization migration (pattern `scripts/canonicalize_*_2026_*.py`). NOTE: `instrument_type` is a
      SHARD axis for MTDS/MDPS/features — the migration must preserve shard-atom identity across those services, not
      just IS.
...
- [x] [DATA] P3. ✅ **CLOSED as NOT-A-DEFECT (operator ruling D1=A, 2026-07-17).** `odds` is a live UAC contract key +
      GCS hive partition with zero consumers; a manifest-only rename would be pure downside. Documentation-only note
      shipped to `unified-api-contracts@96790dfd` (`_instrument_enums.py`, above `EXCHANGE_ODDS`/`FIXED_ODDS`), stating
      the sports contract registry intentionally does not key on `InstrumentType` and that those two enum members ARE
      live in the derivation/adapter layer (verified: `betfair.py:287` constructs `EXCHANGE_ODDS`;
      `_derive_instrument_id.py:85` maps `("sports","odds")→EXCHANGE_ODDS`) — so nobody "fixes" the manifest to match
      the enum. `SPORT` (16 rows) + the 1,758 foreign-asset_group prediction rows in the sports index remain as their
      own issue docs (D2 deferred; the routing bug is separate). Full evidence + the rejected options retained below.
- [x] [DATA] P3. ✅ **DECIDED (operator 2026-07-18: D1 = FORK) → tracked in
      `sports_odds_exchange_fixed_fork_2026_07_18.md`.** Fork `odds` into EXCHANGE_ODDS/FIXED_ODDS for real (UAC
      contract fork + GCS migration, manifest LAST), venue→class by mechanism + operator confirms the edges (bare
      BETFAIR/ODDS_API/PINNACLE). D2 (`SPORT` 16 rows) skipped here — another agent owns it. Original finding retained
      below. _(NEW — side-discovery 2026-07-17, measured while root-causing the (A) legacy-values todo above)_ **Sports
      is the last asset group carrying non-canonical `instrument_type` values.** Live sports
...
- [x] [DATA] P3. ✅ **RESOLVED 2026-07-18 — the 847 phantom rows are already reconciled (verified 0 remain).** Ran the
      sanctioned `reconcile_phantom_manifest_rows_all.py --dry-run` scoped to the finding's venues on REAL infra: **cefi
      `--venues COINBASE,OKX` → 0 phantoms ("Manifest is clean")**; **defi `--venues CURVE,LIDO` → 0 phantoms (17,639
      captured rows in scope, all have backing objects, "Manifest is clean")**. The cefi 4 + defi 843 phantom residuals
      (incl. the physically-impossible CURVE-on-OPTIMISM-2020 row) were drained by the active cefi/defi consolidation
      reconciliations between the 2026-07-17 finding and now. **OKX-SWAP registry gap → owned by
      `cefi_consolidated_closeout_2026_07_18.md`** (line 309 remaps bare-OKX → OKX-SWAP/-SPOT/-FUTURES; line 470 has
      `OKX-SWAP:PERPETUAL:…@INV` as a canonical target) — NOT re-registered here to avoid conflicting with that active
      migration. **Writer root-cause** (stamps `captured` without a successful write — the recurring class) is tracked
      by the manifest_writer issue docs (`manifest_writer_record_captured_available_at_never_persisted_2026_07_13.md` et
      al.). _(NEW — side-discovery 2026-07-17, found while diagnosing the cefi instrument_type backfill's residual)_ **4
      cefi manifest rows claim a capture under BARE venue names that have no objects — the same phantom-pointer class as
      the P7 split rows (`PACIFICA`/`LIGHTER`), and a live venue-registry gap alongside it.** Found because the cefi
      captured-blank backfill correctly drained 13,046 → **4**, and those 4 would not resolve: `venue=COINBASE` (2
      rows) + `venue=OKX` (2 rows), both `capture_status=captured` with real-looking `row_count`/`instrument_count` (69
      and 77), both only on **2026-03-01 / 2026-03-02**. The backfill left them blank rather than guessing a type — the
      honest-absence guard working correctly, which is how they surfaced. **Verified on real GCS + the UAC registry:** -
      **No object exists** for any of the 4, under either the `pipeline_mode=…/asset_group=cefi/venue=…` layout OR the
      legacy `by_date/day=…/venue=…` layout. The real venue dirs that day are `COINBASE-SPOT`/`COINBASE-FUTURES` and
      `OKX-SPOT`/`OKX-FUTURES`/`OKX-SWAP` — **there is no bare `COINBASE` or bare `OKX` venue on disk at all.** -
      `VENUE_TO_ASSET_GROUP.get("COINBASE")` → **`None`** (not a registered venue), while `.get("OKX")` → `'cefi'`
      (registered, but with no objects on those days). So the rows are phantom either way. - **Separate, possibly larger
      finding in the same check:** `VENUE_TO_ASSET_GROUP.get("OKX-SWAP")` → **`None`**, yet `venue=OKX-SWAP/` **has a
      live GCS directory with real data**. Per CLAUDE.md's launcher rule, an unregistered name is "silently invisible in
      deployment-ui/cockpit/Slack until someone goes looking" — not a loud failure. Worth checking whether OKX-SWAP's
      captures are being silently dropped from coverage. Fix direction: same shape as
      `canonicalize_cefi_split_venue_chain_2026_07_17.py` — resolve each phantom row to its real venue if an object
      exists there, else demote it from `captured` (it never captured anything at that path). Tiny blast radius (4
      rows), but it is a FALSE `captured` claim, which is exactly what the manifest must never carry. The OKX-SWAP
      registry gap should be triaged separately and may not be tiny.
...
- [x] [DATA] P3. ✅ **DECIDED (operator 2026-07-18) → owned by the canonical
      `defi_consolidated_closeout_2026_07_18.md`.** The DeFi POOL identity is already canonicalised there (operator
      ruling): `canonical_instrument_id` = `VENUE-CHAIN:POOL:TOKEN0-TOKEN1[-FEE]` (chain-unique) with
      `instrument_id = pool_address` as the machine key. The 6 cross-chain collisions resolve via
      `canonical_instrument_id`; consumers must key on it, not the bare address. (My standalone
      `defi_pool_id_chain_uniqueness_2026_07_18.md` was SUPERSEDED by the closeout — it proposed a conflicting
      `pool::CHAIN::addr` machine id.) Original finding retained below. _(NEW — side-discovery 2026-07-17, found while
      verifying the LENDING drain)_ **DeFi POOL `instrument_id` is not unique across chains.** The live defi catalogue
      carries **6 duplicated `instrument_id`s (12 rows)** where the SAME pool contract address is deployed on TWO chains
      and both rows key on `instrument_id == pool_address.lower()`: `0x004c167d…` = CURVE on **AVALANCHE + OPTIMISM**;
      `0x01abc00e…`, `0x03cd191f…`, `0x06df3b2b…`, `0xc6a5032d…`, `0xfeadd389…` = BALANCER on **ETHEREUM + POLYGON**
      (deterministic EVM deployment puts the same address on multiple chains). Each pair has a DIFFERENT
      `available_from`, so they are genuinely distinct instruments colliding on one id. Pre-existing and **out of scope
      for the LENDING drain (left untouched — verified still exactly 6 before and after)**. Blast radius is small (6 of
      10,883) but the CLASS is an identity-uniqueness violation: any consumer keying on `instrument_id` alone silently
      picks one chain's pool. Fix direction: include `chain` in the DeFi POOL identity (or dedupe key) — needs a
      shard-atom check across MTDS/MDPS/features before changing, same as any identity migration.
...
### P6 — Instrument catalogue explorer — completed checkbox evidence (phase 1)
...
### P7 — Data Coverage breakdown: CeFi chain-axis drift — completed checkbox evidence
...
### P8 — Sports league-drilldown consistency + TEAMS data-correctness — completed checkbox evidence
...
### P9 — Operator review round 2 — Q1-Q4 completed checkbox evidence
...
### P9 — cross-agent verification (2026-07-16 pm, other agents' P3/P6 work) — completed checkbox evidence
...
### P10 — Sports fixtures browser: filter by date / league / team — completed checkbox evidence
...
### P10-B — single-file fixtures source + full-history catalogue — completed checkbox evidence

- [x] ✅ **[DATA] P2 — DONE.** Roll-up carries the fixture scheduling fields it was discarding. `CATALOG_COLUMNS` +=
      `kickoff_utc`/`status`/`home_team_name`/`away_team_name`/`venue_name`/`round`; latest-snapshot-wins (status
      evolves NS→FT); `_sports_attr_str` collapses None/NaN/"None"→"" (never a fabricated placeholder). No new walk —
      the `entity=fixtures` snapshots already carried all of it. — instruments-service@684a1b2b + Evidence: QG green
      (710s); +5 tests; **live prod/catalog.parquet verified** — kickoff/status/team names **17,058/17,064 (100%)**,
      venue_name 96.2% (source-None honest blanks), status real (`FT`=15,500 `NS`=1,070 `PEN`=320 `AET`=112 `CANC`=30),
      **zero leakage** onto team/player grains.
- [x] ✅ **[INFRA] P1 — DONE.** `--since` (sports-only) — the full-history escape hatch that **did not exist**: the FTP
      window start was hardcoded, `--mode full` doesn't help (sports bypasses the generic engine), and the frozen tail
