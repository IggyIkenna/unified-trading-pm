...
> **ARCHIVED (2026-08-05) — all 4 todos done.** Todo 1: source-doc checkboxes reconciled (2026-07-30). Todo 2: 22
> deferred items re-verified with dated verdicts (2026-08-05). Todo 3: two membership/classification findings actioned —
> UNION rule recorded in closeout Progress Log + Track-23 correction in sports batch6 (2026-08-05,
> unified-trading-pm@512a1c983). Todo 4: 6-step archival ritual executed — batch2 archived, 16 referrers updated,
> finalize self-archived (2026-08-05). Both docs now in `/plans/archive/2026_07/`.
...
- [x] ✅ [REVIEW] P1. **Reconcile every named source doc's checkboxes.** Batch 2's 14 todos cite ~22 distinct source
      ...
      `plans/archive/2026_08/{data_pipeline_self_healing_completion_residual_2026_07_24,     data_pipeline_ag_residual_backfill_decisions_2026_07_24}`.
...
- [x] ✅ [DOC] P1. **Archive `cross_cutting_satellite_ao_dispatch_batch2_2026_07_26.md`** via the standard 6-step ritual
      — unified-trading-pm@<sha>. (1) Deferred items verified routed by todo 2. (2) Archive banner added,
      status→resolved, superseded_by→finalize. (3) Codex check: no new durable contract — UTL writer-side canonical-path
      assert already documented in `/codex/02-data/availability-manifest-and-data-status.md`. (4) 16 active referrers
      updated to archive path. (5) locked_by empty on both. (6) Both docs in `plans/archive/2026_07/`.
