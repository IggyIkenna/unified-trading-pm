...
- [ ] [DATA] P0. **v1_archive ROW-coverage gate — CORRECTED 2026-07-25, was false-progress.** Originally ticked ✅ on
      "GATE SCRIPT SHIPPED" evidence (`verify_v1_archive_row_coverage_2026_06_27.py`, market-tick-data-service@18ca0e23)
      — i.e. on shipping a script, never on a verified run of it. Un-ticked per
      `sports_legacy_cutover_closeout_tasks_2026_07_24.md` T6.8 (the false-progress class the commit+flip rule targets).
      **However, the underlying row-coverage question was in fact resolved — just by a DIFFERENT, later mechanism**: a
      more rigorous per-object CONTENT-based supersession proof (`audit_sports_v1_archive_supersession_2026_07_16.py`,
      market-tick-data-service@18d6fb70) verified all 398 `sports_reference_v1_archive/by_date/` objects
      content-superseded (join on `source_fixture_id`==`af_fixture_id`, sample content fields matched), the operator
      approved the E8 delete, and the objects were ACTUALLY DELETED 2026-07-16T09:37Z (T2.1 in
      `sports_legacy_bucket_cutover_history_2026_07_24.md`). So this gate is genuinely CLOSED — via the 2026-07-16
      content-audit + real deletion, not via the originally-cited (never-run)
      `verify_v1_archive_row_coverage_2026_06_27.py`, which has since been deleted as doubly-broken (T6.8: never run +
      its subject no longer exists, so leaving it around was a trap that could re-issue a false COVERED verdict against
      nonexistent data). **Also correcting the standing claim below**: v1_archive is column-superseded by **v2 fixtures
      ALONE**, not the union of (understat_xg + v2 fixtures + v2 fixture_stats) — the columns that supposedly required
      the union are 100% empty.
