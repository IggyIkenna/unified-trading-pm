...
related:
  [
    ...
    /plans/archive/2026_07/tradfi_satellite_ao_dispatch_batch2_finalize_2026_07_25.md,
  ]
superseded_by:
created: "2026-07-25"
...
> **🟢 ARCHIVED 2026-08-06.** All 12 todos done across slots 2/5/6/9/13/14/15 (2026-07-26 through 2026-08-06). Deferred
> items resolved by slot-13 review — 6/8 conflict-gated docs archived, 2 remaining tracked in successor plans
> (`data_completion_tradfi_2026_07_15.md`, `instruments_tradfi_g1_g5_gate_execution_2026_07_24.md`), operator-gated item
> implemented (`deployment-service@c847395e`). Companion finalize plan
> (`tradfi_satellite_ao_dispatch_batch2_finalize_2026_07_25.md`) archived alongside.

# TradFi satellite AO batch 2 — re-triage clearance

> **Status: archived.** Per CLAUDE.md's plan-destination rule and the ag-closeout-audit skill's autonomous-mode
> guidance, a skill-drafted AO batch is never auto-shipped to `active` — flip this frontmatter's `status` to `active`
> only after operator review. All 11 todos below are same-priority; same-source-doc collisions were resolved by
> combining candidates into ONE todo per source doc (3 combined groups: data_completion_tradfi,
...
- [x] ✅ [SCRIPT] P1. **Correct the live tradfi `availability_index` manifest for the combo/chain objects quarantined by
      ...
      `correct_tradfi_recovery_quarantine_manifest_2026_07_27.py`), CAS pass run against prod 2026-07-27T21:00:54Z
      (generation `1785185981702358` → `1785186066760506`), consolidator cron
      (`uts-prod-manifest-consolidator-market-data-tradfi-cron`) paused before the pass and resumed after, verified
      `ENABLED`. **14,742 rows** corrected to `attempted_failed` (not the plan's ~97,828 estimate — of the 98,256
      candidate TSV rows from run `20260720-120911`, only 14,742 manifest keys actually existed AND were still
      `captured`; the rest either had no manifest row at all or were already a different honest status). A post-write
      spot-check caught a real 1-row near-miss: the first apply's filter ("not already `attempted_failed`") incorrectly
      flipped one genuinely `expected_unattempted` row (CME futures_chain ohlcv_1m underlying=MET, day=2026-03-10) —
      manually reverted via a verified CAS write (generation `1785186128010123` → `1785186215984802`) within minutes,
      then fixed in code (filter tightened to `capture_status == 'captured'` specifically) + regression test added,
      shipped as `market-tick-data-service@e6e6ce2d`. **No real conflict** — confirmed zero overlap with
      `tradfi_consolidated_closeout_2026_07_18.md`'s remediation items. Follow-up (428 content-recovered rows never
      registered under their real root, + the run's own `_quarantine/` physical mirror found stale/reused — both
      discovered mid-task) tracked in `issues/tradfi_recovery_quarantine_registration_gap_2026_07_27.md`. Source:
      `issues/cme_combo_underlying_extraction_garbage_2026_07_19.md`.

- [x] ✅ [AUDIT] P1. **DONE 2026-07-27 (slot-15, data_engineering)** — Swept `market-tick-data-service` for the
      remaining `run_in_executor(None, ...)` network-blocking call sites:
      `grep -rn "run_in_executor(None"     market_tick_data_service/` (excluding tests) confirms exactly 3 sites, no
      others missed — `databento_base_client.py:499` (warmup, already classified) plus the 2 previously-unclassified
      `live/connectors/databento_tradfi_ws.py` sites (`:482` `_do_subscribe`, `:515` `_do_start`). Both classified as
      setup-time-only, already `asyncio.wait_for`-bounded, same latent-not-live-defect disposition as the accepted
      warmup pattern — no code change needed. Per-site verdict + rationale written inline in
      `archive/issues/databento_default_executor_dns_starvation_risk_2026_07_17.md`'s [AUDIT] P2 todo + a new Progress
      Log entry; the doc's [CODE] P1 checkbox and `status`/`resolved_by` frontmatter left untouched as instructed. Repo:
      unified-trading-pm (doc-only). Source:
      `archive/issues/databento_default_executor_dns_starvation_risk_2026_07_17.md`.

- [x] ✅ [TRADFI] P1. **DONE 2026-07-27 (slot-14, data_engineering)** — Profiled market-tick-data-service's real
      per-date OHLCV fetch/decode/write path with `memray --native` against a live-credentialed repro
      (`--asset-group     tradfi --venues NYSE --data-types ohlcv_1m --source databento --force --start-date 2025-06-25 --end-date     2025-06-25`,
      writes routed to the `-test-` bucket, no prod writes). Measured peak RSS **2.899 GB** for 208,627 rows / 18.1 MB
      output (516 per-symbol writers) — a ~164× memory-to-output amplification, the same phenomenon as the fleet's
      documented ~15GB-vs-1.3MB gap at smaller scale. Top allocators (`memray stats`, both by size and by count) are
      `pyarrow/parquet/core.py write_table` (2.518GB / 6.16M allocations) and `pyarrow/pandas_compat.py convert_column`
      (1.128GB / 821K allocations) — i.e. **confirmed the "un-released pyarrow frame" half of the hypothesis, NOT eager
      DBN decode buffering** (decode functions don't appear in the top-5 allocators at all). Root cause:
      `PartitionedTickWriter` opens one `StreamingParquetWriter` per (instrument_type, data_type, symbol) for
      non-derivative types — each pyarrow `write_table()` call carries fixed per-call C++ overhead that doesn't scale
      down for symbols writing only 1-5 rows/day, so the fixed cost × hundreds of concurrently-open per-symbol writers
      dominates transient RSS. Dated addendum with the full allocator table + root-cause mapping + follow-up scope note
      added to `issues/tradfi_backfill_oom_remediation_2026_06_24.md`'s `[TRADFI] P2` todo, flipped to `[x]` in the same
      commit. Diagnostic only, no code fix or machine-type revert made per this todo's done-when. Source:
      `issues/tradfi_backfill_oom_remediation_2026_06_24.md`.

- [x] ✅ [DATA] P1. **DONE 2026-07-27 (slot-9, backend_engineer)** — Codified the "reclassification races the
      consolidator too" near-miss into the manifest-consolidator SSOT. Added a new subsection ("Pause-first applies to
      ANY canonical read-modify-write, not only row removal (2026-07-15 near-miss)") directly after the existing
      "Surgical ROW REMOVAL from the canonical" 6-step recipe in
      `/codex/05-infrastructure/manifest-consolidator-ssot.md`, stating that the PAUSE-first requirement (step 1)
      applies to ANY direct canonical read-modify-write — not only row-removal, but also in-place `capture_status`
      reclassification (floor-clip / rule-based reclass scripts) — and cross-referencing
      `plans/archive/issues/tradfi_eu_not_draining_source_axis_drift_2026_06_24.md`'s 2026-07-15 Progress Log entry as
      the near-miss precedent (race-free by observed timing luck, not by an enforced pause). Doc-only change. Repo:
      unified-trading-pm. Source: `archive/issues/tradfi_eu_not_draining_source_axis_drift_2026_06_24.md` (resolved +
      archived 2026-07-30).

- [x] ✅ **DONE 2026-07-28 (slot-6).** Root-caused: **NOT a Databento silent-zero-row bug.** Exhaustive live-log +
      GCS-events grep for `DATABENTO_EMPTY_BUT_VALID` (3 real production `run.log`s — CME ES 2019/2020/2026 backfills
      — + the full structured-events store for one run's hour) found ZERO hits: the SDK genuinely never returns an empty
      response for this population. Direct `availability_index.parquet` cross-reference instead proved a **stale
      bundle-grain shard-atom mismatch**: legacy CME/CBOE `futures_chain`/`options_chain` expected rows keyed by the
      retired `instrument_id=<parent>.FUT`/`.OPT` grain (pre-`instruments-service@f6d479f8`, 2026-06-22) are being
      reclassified `attempted_failed(WithinBoundsTradfiSourceZero)` by CF-11's honest-absence rebuild
      (`_rebuild_tradfi_cf11.py::_handle_srz_tradfi_row`, no captured-shard check) even though the SAME date's real data
      is already `captured` under the post-fix key (`instrument_id=''`, `underlying=<canonical>`) — 99-100% false-
      positive match rate for ES/MES/NQ/MNQ, 59.3% overall across 69,475 checkable CME rows (lower bound; commodity-
      group naming drift undercounts it), same mechanism confirmed for CBOE `VX.FUT` (2,489 rows). NASDAQ/NYSE's share
      of the original alert is a DIFFERENT, already-closed root cause (below-vendor-discovery-floor cleanup, this same
      session) — confirmed 0 as of 2026-07-28. Full evidence + remediation todos in the follow-up issue doc:
      `issues/tradfi_within_bounds_source_zero_shard_atom_mismatch_2026_07_28.md`. No fix implemented (investigate-only
      per this todo's Done-when); the migration/purge + CF-11 hardening are tracked as new P0/P1 todos in that doc.
      Source: `issues/tradfi_ohlcv_attempted_failed_cluster_2026_07_23.md`.

- [x] ✅ [DATA] P1. **DONE 2026-07-28 (slot-2).** All 4 steps complete: (1) re-measurement + year/venue breakdown was
      already recorded in `issues/tradfi_todo_cells_below_vendor_discovery_floor_2026_07_20.md` todo 1 (slot-9,
      2026-07-27) — 182,407 cells, boundary-verified (0 off-by-one, 103 on-floor cells correctly excluded); (2) the
      enumerator fix was already shipped `instruments-service@31cf3952` (slot-5, 2026-07-27) — `_enumerate_v2_tradfi`
      resolves `VenueMapping().get_instrument_discovery_start()` live at runtime; (3) ran the corrective `--apply` this
      session: paused `uts-prod-manifest-consolidator-market-data-tradfi-cron`, hit + fixed a genuine production bug
      blocking the safety pre-check (scheduler job-name reconstruction used the bucket short-form `uts-prd-` instead of
      Terraform's real `uts-prod-` prefix — `unified-trading-library@080a84a0`, deployment-service twin tracked
      separately in `plans/archive/issues/deployment_service_scheduler_job_name_reconstruction_bug_2026_07_27.md` (FIXED
      2026-07-28, `deployment-service@841f464`)), then `--apply` succeeded: 182,407 cells flipped, verified
      420,438→238,031 todo (exact delta), 0 below-floor cells remaining on re-check, consolidator resumed/confirmed
      ENABLED; (4) added the regression-guard test `instruments-service@5104befc` —
      `test_tradfi_v2_discovery_floor_invariant_holds_independently_per_venue`, sweeping REAL UAC floors for all 6
      tradfi venues (NASDAQ/NYSE/CME/CBOE/KRX/FX). `quality-gates.sh` green (both instruments-service and
      unified-trading-library, full runs). All corresponding todos flipped in
      `issues/tradfi_todo_cells_below_vendor_discovery_floor_2026_07_20.md` (now `status: resolved`) and in
      `tradfi_backfill_throughput_followups_2026_07_24.md` (see below). **Conflict-check (2026-07-25 plan-reconcile):
      this todo's Done-when ALSO flips a checkbox in `tradfi_backfill_throughput_followups_2026_07_24.md` — the SAME doc
      the P2 todo above (3-item combined pass) writes to. Run this todo FIRST (higher priority), then the P2 todo, never
      concurrently.** **Make TradFi's `todo` denominator honest below the vendor discovery floor (4-step ordered
      sequence, one worker, execute in order)** — ALSO closes `tradfi_backfill_throughput_followups_2026_07_24.md`'s own
      P2 candidate on the SAME 182,407-cell cohort (that doc's phrasing pointed at `sentinels.py`, but a live grep
...
- [x] ✅ [TEST] P1. **NICE-TO-HAVE — fix `deployment-service/tests/unit/test_event_logging.py`'s `get_service_name()`
      ...
      `/plans/archive/2026_08/tradfi_multisource_backfill_2026_06_22.md`. — deployment-service@077a063:
      `get_service_name()` now reads `pyproject.toml`'s `project.name` (falling back to `Path.cwd().name` only if the
      file is absent); verified live by copying the checkout to a differently-named directory and confirming the skip
      still fires there while continuing to pass in the canonical checkout; `quality-gates.sh` green.

- [x] ✅ [REVIEW] P2. **This is a review/investigation task, not a single decision — re-tag for dispatch rather than
      fabricate an answer.** `[REVIEW]` tag (was `[OPERATOR]`) — work through the named conflict-gated and
      too-large-for-batch items directly per the doc's own criteria; only escalate back to the operator for whichever
      specific sub-items turn out to be genuine judgment calls after investigation. **Resolve the still-conflict-gated /
      operator-gated / too-large candidates below** — the "Deferred — still genuinely conflict-gated" section (8
      candidates across 5 docs), "Deferred — operator-gated" section (the
      `tradfi_mvp_mode_unreachable_dead_gate_2026_07_08.md` design call — **RULED 2026-07-29 + IMPLEMENTED 2026-07-30
      (autonomous session, `deployment-service@c847395e`, see the issue doc for full detail); this portion of the
      bundled todo is fully resolved, the other two sections below are not**), and "Still excluded —
      too-large-or-risky-for-a- batch" section (`tradfi_manifest_content_recovery_completion_2026_07_24.md`) list
      AO-eligible/human-only work that remains genuinely unresolved and was never converted into a dispatchable todo
      here.

      **Evidence (slot-13 review, 2026-08-06):** Re-checked every deferred item against live doc state + git-log
                      sweep from 2026-07-25 to 2026-08-06. The pipeline advanced through batches 5 (archived, 18/22 done), 6
                      (active, 4 dispatched), and 7 (active, created today) since batch2, and the closeout doc
                      (`tradfi_consolidated_closeout_2026_07_18.md`) forked its residual work to children including
                      `tradfi_registry_coverage_and_ao_readiness_2026_07_25.md` (14 unchecked). Findings per category:

                      **(a) 8 conflict-gated candidates (5 docs):**
                      - `issues/tradfi_fx_provenance_and_manifest_id_defects_2026_07_24.md` — **ARCHIVED 2026-08-06, 0 unchecked.** Both candidates resolved. ✅
                      - `issues/tradfi_ohlcv_attempted_failed_cluster_2026_07_23.md` — **ARCHIVED 2026-07-30, 0 unchecked.** Purge/reclassify executed (`market-tick-data-service@0cd76b93`). ✅
                      - `tradfi_multisource_backfill_2026_06_22.md` — **ARCHIVED 2026-08, 0 unchecked (14/14 done).** FX yahoo backfill completed. ✅
                      - `data_completion_tradfi_2026_07_15.md` — **ACTIVE, 22 checked / 17 unchecked.** The ⑫ FOLLOW phantom-dry-run was flipped per operator ruling (2026-07-29). Gate-b chain work tracked in closeout child `tradfi_registry_coverage_and_ao_readiness_2026_07_25.md`. Conflict still real but properly tracked.
                      - `instruments_tradfi_g1_g5_gate_execution_2026_07_24.md` — **ACTIVE, 31 checked / 7 unchecked.** ES CME ohlcv investigated (ZERO capture, filed `tradfi_es_cme_ohlcv_zero_capture_2026_07_30.md`). ES_OPT deferred (live backfill held the singleton lock). Work tracked in successor docs.

                      **(b) Operator-gated:**
                      - `tradfi_mvp_mode_unreachable_dead_gate_2026_07_08.md` — **ARCHIVED, 0 unchecked.** Ruled 2026-07-29, implemented `deployment-service@c847395e` 2026-07-30. ✅

                      **(c) Too-large-for-batch:**
                      - `tradfi_manifest_content_recovery_completion_2026_07_24.md` — **ACTIVE, 29 checked / 5 unchecked.** Made substantial progress (29/34 done). Batch6 re-confirmed as still too-large-or-risky for batch extraction — unchanged assessment. Remaining 5 items need their own dedicated pass (not a batch-N slot).

                      **Disposition:** 6 of 8 conflict-gated candidates resolved by archival of their source docs. The 2 remaining (data_completion_tradfi, instruments_tradfi_g1_g5) are properly tracked in closeout children and later batches. Operator-gated item resolved. Too-large doc still needs dedicated triage. **No new dispatchable todos created from this investigation — all residual work is already tracked in successor plans.** The batch2→finalize→archive chain can proceed.
...
- `/plans/archive/2026_08/tradfi_multisource_backfill_2026_06_22.md` — run the FX yahoo backfill to completion
  (conflicts/sequences against `issues/tradfi_fx_provenance_and_manifest_id_defects_2026_07_24.md`'s still-open
  source-mislabeling fix — running the backfill while the mislabeling root cause is unfixed risks writing MORE
  mis-stamped rows; genuinely a sequencing question for the operator, not resolvable by evidence alone).
...
the same undecided question rather than resolving it. This was genuinely operator-gated per the ag-closeout-audit
skill's non-batchable taxonomy — **RULED 2026-07-29: wire via forward-poll opt-in flag, see the issue doc** (the issue
doc now carries a concrete [CODE] P1 implementation todo; a fresh batch could pick that up as an AO-eligible candidate
on the next triage pass).
...
- **context-scout 2026-08-03**: re-verified context_scope (8 entries, all resolving) — dispatch-batch coordinator,
  legitimately code-free; no changes needed.
