...
status:
  complete # (was: active) 2026-07-28 archival sweep: verified zero open `- [ ]` todos; see "Deferred work after
  # 2026-07-20" (already present) for the forked-out / handed-off follow-on work
nature: process
...
> **🗄️ ARCHIVED 2026-07-28 (plan-hygiene sweep)** — verified zero open todos; see "## Deferred work after 2026-07-20"
> below (already present in this doc) for the forked-out/handed-off follow-on work (already tracked in other active
> docs, not lost). Per `/codex/12-agent-workflow/plan-completion-and-archival-discipline.md`.
...
## Ground truth (REFRESHED 2026-07-25 — see Progress Log entry below for method + per-cell source + deltas)

Counts (non-canonical / total distinct). cefi/tradfi/prediction/sports read LIVE off today's consolidated manifest
(`_axis_census.py` + `_distinct_values.py`'s own canonical/grain/accepted-exception badging, reused verbatim); defi read
off the newest COMPLETE (5/5 asset_group) nightly honest-coverage rollup, `source_date 2026-07-24`, because defi's own
manifest consolidator was CURRENTLY STALE at refresh time (see Progress Log) and today's (2026-07-25) rollup turned out
to be a scoped cefi-only re-run — see the new issue doc cross-linked in `related:` for that hazard.

| asset_group | venues | instrument_types | data_types | chains |
| ----------- | ------ | ---------------- | ---------- | ------ |
| defi        | 14/72  | 0/11             | 6/23       | 3/16   |
| cefi        | 1/27   | 3/8              | 1/9        | 0/0    |
| tradfi      | 1/8    | 1/7              | 0/10       | 0/0    |
| prediction  | 0/2    | 2/3              | 1/5        | 0/1    |
| sports      | 7/14   | 3/3              | 2/10       | 0/0    |

**Delta vs 2026-07-20 baseline** (Δ non-canonical count / Δ total distinct count, this cell only):

| asset_group | venues    | instrument_types | data_types | chains  |
| ----------- | --------- | ---------------- | ---------- | ------- |
| defi        | −62 / −4  | −11 / −6         | −4 / −13   | +1 / −7 |
| cefi        | −1 / +3   | −1 / 0           | +1 / +5    | −4 / −4 |
| tradfi      | 0 / 0     | −8 / −9          | −2 / −2    | 0 / 0   |
| prediction  | 0 / 0     | +2 / +2          | +1 / +1    | 0 / 0   |
| sports      | −22 / −23 | −12 / −12        | −5 / −3    | −3 / −3 |

Total non-canonical across all 5 AGs × 4 axes: **175 (2026-07-20) → 115 (2026-07-22 detector-fix milestone, Progress Log
RESULT 1) → 45 (2026-07-25, this refresh)** — the cumulative effect of every fix this plan + its sibling AG-closeout
plans shipped in between, not just this plan's own todos (the sports instrument_types 15→3 collapse in particular tracks
the `sports_consolidated_closeout_2026_07_19.md` Track C MDPS `parts[1]`-adjacent work, not a fix landed here).
...
- [x] [DATA] P1. ✅ UAC SSOT additions (category 2) that are unambiguous and NOT contested by an in-flight migration —
      ...
      **PARTIALLY RESOLVED 2026-07-22** — every unambiguous cat-2 addition identified has an executed disposition: the
      ...
      scope; D6's remainder is tracked on its issue doc, not here. **Checkbox flipped 2026-07-25** (/plan-reconcile
      apply_batch_13) — matches the identical "nothing remains here" disposition the category-3 todo below already
      carries `[x]` for; verified `uac@bb42d8ee`, `uac@91b6f094`, `uac@9908520b` are all real commits and ancestors of
      `origin/live-defi-rollout` in `unified-api-contracts`, and
      `defi_swaps_ohlcv_candle_data_types_axis_gap_2026_07_22.md` exists as the D6 fork target.
...
- [x] [DATA] P1. ✅ Reconcile every drift cluster (category 1) to its owning in-flight plan; any cluster owned by NO
      ...
      which is unrecoverable — worked off live data instead, per the dispatched todo's own instruction) classified all
      81 currently non-canonical values (defi 24, cefi 2, tradfi 8, prediction 2, sports 45): 51 already resolve to a
      live owning plan/issue/operator ruling, 30 net-new values (almost entirely a sports MDPS market-token cluster)
      were newly attributed via 3 fresh issue docs. Full per-value classification table + evidence in
      `cross_cutting_satellite_ao_dispatch_batch1_2026_07_26.md`'s 2026-07-28 Progress Log entry (not duplicated here
      per the "plan references, does not duplicate" rule) — new issue docs:
      `plans/active/issues/sports_instrument_type_market_token_ssot_gap_2026_07_28.md`,
      `plans/active/issues/tradfi_distinct_values_net_new_clusters_2026_07_28.md`,
      `plans/active/issues/defi_cefi_venue_chain_axis_contamination_2026_07_28.md` (the last flagged P1/big-finding —
      cross-AG contamination, not yet root-caused). Supersedes the **PARTIALLY RESOLVED 2026-07-22** note below (kept
      for historical record) — this is now the exhaustive current-state reconciliation, not a spot-check. The 22
      original cat-1 clusters' individual owning-plan citations from the deleted JSON were not re-verified one-by-one
      against that specific list (impractical, per the 2026-07-22 entry's own assessment) — instead every VALUE
      currently live on the panel was independently classified against TODAY's owning plans, which supersedes the
      narrower "spot-check the 22" ask. **PARTIALLY RESOLVED 2026-07-22** — the 22 cat1 findings' owning-plan
      attribution was performed AS PART OF the classification fan-out itself (todo #1 above), scoped against this doc's
      `related:` frontmatter (the 5 AG consolidated-closeout + migration-catalogue plans); the one cat1-adjacent cluster
      that surfaced through later cat3 investigation (MDPS `parts[1]` parse, covering both the sports
      bookmaker-in-instrument_type AND `H2H`/`MATCH_ODDS`/`SPREADS`-as-chain clusters) is confirmed explicitly annotated
      on its owning plan (`sports_consolidated_closeout_2026_07_19.md` Track C F1/F2, cross-linked both directions —
      checked directly, present). **Still open / not independently re-verifiable** — the raw 175-finding per-cluster
      JSON (which would let a reader individually confirm all 22 cat1 owning-plan citations, not just the one
      spot-checked above) was later deleted from the scratchpad during the 2026-07-21 pre-compact sweep as
...
- [x] [REVIEW] P1. **2026-07-28 retag** (was `[OPERATOR]` BLOCKED-OPERATOR-DECISION — already resolved, tag left stale)
      — verified PURGE worklist (category 4): exact (asset_group, axis, value, row_count, GCS/catalogue locations,
      why-junk) for operator one-tap approval. NO blind deletion. **RESOLVED 2026-07-22** — the worklist WAS produced
      and adversarially verified per-item; its answer is that the worklist is EMPTY (every candidate walked back to
      quarantine/re-stamp/keep, several against standing operator rulings — see "PURGE worklist — EMPTY" section below
      for the full verdict table). Producing a verified worklist that turns out to contain zero executable purges is a
      complete, correct answer to this todo's ask, not a non-answer — there is nothing left requiring operator one-tap
      approval.
...
- [x] [DATA] P1. ✅ **Re-run the distinct-value census for all 5 asset_groups against the current nightly rollup +
      manifest, refresh the "Ground truth" table above** (per
      `/plans/active/data_pipeline_e2e_milestones_gate_2026_07_24.md` §2 — the table above is still stamped
      `source_date 2026-07-20`, pre-dating the D1b/D2/D5 detector fixes and the 15-defi-venue capture work already
      shipped and recorded in the Progress Log below, so the 175-finding baseline is now stale relative to the fixes
      already landed). Definition-of-done: a fresh non-canonical count per (asset_group, axis) cell replacing the table
...
- [x] ⚠️ [DATA] P1. **`perp_daily_ctx` / `perp_funding` manifest-invisibility — INVESTIGATED 2026-07-22,
      ...
      <details><summary>Original framing (2026-07-22 side investigation, superseded by the above)</summary> structurally
      invisible to this audit's detector, found by a 2026-07-22 side investigation, not by the panel. Neither is a
      canonical `data_type` (no member in `DATA_TYPES_BY_ASSET_GROUP['defi']`), and neither shows up in this doc's
      non-canonical inventory at all — both are written via raw `gcsfs` calls with **zero manifest writes**
      ...
      `plans/archive/issues/mtds_plan_reconciliation_2026_06_29.md:384` "MD5 `perp_daily_ctx` manifest-invisible —
      ...
      distinct-values panel, since the panel only ever sees what's in the manifest. </details>
...
- [x] ✅ [DATA] P1. **MDPS `canonical_writer_shaping.py::_type_token_from_canonical_id`** — highest-yield single bug in
      the corpus. It assumes a 3-segment cefi `VENUE:TYPE:SYMBOL` id and takes `parts[1]`; sports ids are 8-segment
      ...
      **DONE 2026-07-28 (confirmed, not re-annotated — already present).** Re-checked
      `plans/active/sports_consolidated_closeout_2026_07_19.md` directly: Track C already carries the full annotation
      (lines ~214-235 diagnose `parts[0]`/`parts[1]` venue/instrument_type swap via `_type_token_from_canonical_id`;
      lines ~459-468 restate the fix scope + cite `canonical_writer_shaping.py:257-266` / `canonical_writer.py:252`;
      line ~497 references "the original F1/F2 note"). This todo's ask (annotate the owning plan, do not fork a
      duplicate) was already satisfied — no new edit needed there, nothing forked here.
...
- [x] [DATA] P1. For each of ANKR, FRAX, MAKER, STADER, STAKEWISE, SWELL, MANTLE, ACROSS, STARGATE, FLASHBOTS, ALCHEMY,
      ...
      **DONE 2026-07-22.** Real sample-day backfill against all 15: 14/15 already had working, production-proven capture (verified with real on-chain/API calls, no code changes needed for 12 of them); ACROSS + STARGATE needed and got real fixes (`market-tick-data-service@a32dd58c`/`@4c21c7f6` — dead subgraph replaced with real on-chain Swap-log queries); FLASHBOTS' pipeline_mode was wrong (subgraph→onchain_rpc, `mtds@6bf6012a`); MORPHOVAULTS had a wrong on-chain vault address resolving to a different vault entirely (`mtds@6bf6012a`, verified correct via `convertToAssets` at block 25573787); MAKER's capture moved handlers (vault_share_price→ lst_rates) without the capability registry following (`uac@328a5cea`). All 15 were **already** in `ALL_DEFI_VENUES` (the full registry) — the literal "add" instruction against that registry was a no-op. **CORRECTION (same day, follow-up investigation) — this was NOT the whole picture.** `ALL_DEFI_VENUES` membership is NOT what the honest-coverage denominator actually reads. `VENUES_BY_ASSET_GROUP['defi']` (the list `expected_universe.py`/`check_enumeration_completeness.py` actually iterate to build the `completeness_pct` denominator) is a PHASE-FILTERED subset — `unified-api-contracts/.../market_data_categories.py:395` keeps only `DEFI_VENUE_PHASE=="live"` entries. 11 of these 15 venues (ANKR/FRAX/MAKER/STADER/STAKEWISE/SWELL/MANTLE/ACROSS/STARGATE/FLASHBOTS/ALCHEMY) are `phase=="pipeline"`, so despite being real, working, verified captures shipped today, they are STILL structurally excluded from `completeness_pct` for `defi` — confirmed via a full code trace (Step A→B→C: `market_data_categories.py:395` → `expected_universe.py:287` → `check_enumeration_completeness.py:512`), not a guess. The "measure the before/after `completeness_pct` delta" instruction in this todo's own text was therefore never actually actionable — there IS no delta yet, because the venues never entered the denominator in the first place. See `plans/active/issues/defi_venue_phase_live_definition_contradiction_2026_07_22.md` (upgraded P2→P1, `nature: issue`, 2026-07-22) for the full trace + the operator decision needed before this can actually be fixed. **This todo stays flipped `[x]` because the CAPTURE work (the actual ask) is done and verified — the denominator-visibility gap is real but is now separately tracked, not silently rolled into "done" here.** JUPITER: router-only, swap volume already flows through directly-captured pools (Raydium/Orca/Meteora/Phoenix) — kept as-is per the survey's "may be architecturally redundant" read, not force-built. ~~**One real finding NOT resolved, filed separately**: `DEFI_VENUE_PHASE` still labels 11 of these (ANKR/FRAX/MAKER/STADER/STAKEWISE/SWELL/MANTLE/ACROSS/STARGATE/FLASHBOTS/ALCHEMY) `"pipeline"` despite verified real MTDS capture, because the registry carries two contradictory definitions of `"live"` (2026-05-07 data-availability vs 2026-06-29 IS-producibility invariant) — a genuine SSOT contradiction, not something to silently resolve by picking one side.~~ (superseded by the CORRECTION paragraph above — this is confirmed a real coverage-math exclusion, not just a documentation contradiction.) See `plans/active/issues/defi_venue_phase_live_definition_contradiction_2026_07_22.md` for the full finding + the design decision needed before either flipping the phase or correcting the misleading comment.
...
- [x] [DATA] P1. **2026-07-28 retag** (was `[OPERATOR]` — the ruling was already received + partially executed, tag left
      stale) — **DeFi honest-coverage denominator exclusion — operator ruling received 2026-07-22 ("make it both
...
**Method.** Reused the panel's own shipped code, not a reimplementation, per this workspace's
`/codex/02-data/reconciliation-census-and-compute-tiers.md` census SSOT:

- **cefi / tradfi / prediction / sports** — read LIVE off today's consolidated manifest: deployment-api's
  `get_axis_value_census(service="market-tick-data-service", asset_group=...)` (`_axis_census.py`) for the raw
  `{value, count}` per axis, badged canonical/non-canonical by calling `_distinct_values.py`'s own
  `_canonical_set`/`_comparison_set`/`_is_accepted_exception`/`_is_blank` — the exact functions the shipped
  `/distinct-values/{asset_group}` endpoint uses, applied to a fresher (manifest-direct, not nightly-rollup) source.
  This is the G1 "manifest-side census reused verbatim" mechanism the codex SSOT documents.
- **defi** — `get_axis_value_census` for defi raised `ManifestConsolidatorStaleError` (consolidated blob ~75min old vs
  the 120s staleness threshold, per-VM shards present but not yet merged) — a live, real-time signal that the DeFi
  manifest consolidator was behind at refresh time, not forced past via `MANIFEST_ALLOW_STALE_FALLBACK` (that escape
  hatch exists for a deliberate recovery decision, not to route around casually inside an audit script — it can OOM on a
  large bucket per its own error text). Fell back to `_distinct_values.py`'s own mechanism (`enumerate_distinct_values`
  against the honest-coverage `coverage.json`), reading the newest COMPLETE (5/5-asset_group) rollup directly —
  `source_date 2026-07-24` — rather than the endpoint's own default "pick the newest reachable date" behavior, because
...
reduction since the 2026-07-22 milestone, on top of the 34% the detector fix alone bought. The improvement is NOT
attributable to this plan's own shipped work alone: sports `instrument_types` collapsing from 15 distinct
(bookmaker-in-itype contamination — `PADDYPOWER`/`betmgm`/`coral`/... ) down to 3 (`ODDS`/`odds`/`SPORT`, a plain casing
drift) lines up with the MDPS `canonical_writer_shaping.py::_type_token_from_canonical_id` `parts[1]` bug this plan's
own todo explicitly declined to fork and instead annotated onto `sports_consolidated_closeout_2026_07_19.md` Track C
F1/F2 — the live data now shows that fix's effect, even though this plan never touched the writer itself.

**New values not in the 2026-07-20 baseline, not classified here (out of this todo's bounded scope — a fresh
classification pass, if wanted, is separate follow-on work, not silently rolled into this refresh):** defi `chains`
gained `HYPERLIQUID` (now 3: `HYPERLIQUID`/`KALSHI_PERP`/`POLYMARKET_PERP`); defi `venues` non-canonical set is now
chain-shaped names (`ARBITRUM`/`AURORA`/`AVALANCHE`/`BASE`/`BSC`/`ETHEREUM`/`LINEA`/`OPTIMISM`/`POLYGON`) plus
`GMX`/`HYPERLIQUID`/`BLAZESTAKE`/`KALSHI_PERP`/`POLYMARKET_PERP` — visibly different in KIND from the 2026-07-20 list
(which was pure DeFi protocol names); cefi `data_types` gained `volatility_index`; prediction `instrument_types` gained
`prediction`/`prediction_market` and `data_types` gained `prediction_trades`. None of these were investigated further
here — this todo's definition-of-done was the fresh count + delta, not a re-classification (that was the 47-agent
workflow's job on 2026-07-20 and is out of scope for a bounded re-run).

**Not re-attempted**: the category-1 owning-plan citation re-verification (still spot-checked-only, per the todo above —
the raw 175-finding JSON needed to close that out was deleted in the 2026-07-21 pre-compact sweep and re-running the
full 47-agent workflow is impractical, exactly as the 2026-07-22 entry already concluded).

### 2026-07-28 — tradfi live-evidence run (`tradfi_consolidated_native_ao_extract_2026_07_25.md` REVIEW item)

**Method.** Called both shipped endpoints directly (in-process, `deployment-api` `.venv`, no reimplementation) against
today's live state, `GET_PROJECT_ID=central-element-323112`:

- `get_axis_value_census(service="market-tick-data-service", asset_group="tradfi")` — manifest-direct, raw
  `{value, count}`, no badging. `row_count=5,876,397`.
- `get_distinct_values("tradfi")` — nightly honest-coverage rollup, badged `is_canonical`, accepted-exceptions applied.
  `source_date=2026-07-28`, `generated_at=2026-07-28T00:36:37Z`.

**Verdict on the 3 named 2026-07-18 dupes — NOT 0, but explained, no new action needed on this todo:**

| Cluster                     | Manifest-direct (today)                                                                                  | Rollup-badged endpoint (today)           |
| --------------------------- | -------------------------------------------------------------------------------------------------------- | ---------------------------------------- |
| `FUTURE`/`future`/`FUTURES` | `FUTURE` 410,418 · `future` 4,307 · `FUTURES` 16 — lowercase + plural drift **still present, real rows** | Only `FUTURES` (16) badged non-canonical |
| `EQUITY`/`equity`           | `EQUITY` 2,939,411 · `equity` 28,914 — lowercase drift **still present, real rows**                      | Neither badged — both read canonical     |
| `BARCHART` (venue)          | 9,119 rows, unchanged                                                                                    | Badged non-canonical (expected)          |

`BARCHART` matches the doc's own 2026-07-20 operator ruling exactly ("quarantine-with-tracking," 100% `empty_confirmed`,
re-confirmed unchanged at 9,119 rows) — no new action. The `FUTURE`/`future`/`FUTURES` and `EQUITY`/`equity` case-drift
is real and **not zero**, but it is exactly the cat-1 "drift owned by an in-flight plan" class this doc already tracks
(`master_data_canonicalisation_migration_catalogue_2026_06_07.md`'s tradfi instrument_type casing track, line ~847) —
also still-present today: `ETF`/`etf` (323,388/5,372), `COMBO`/`combo` (1,314,608/23,428), `INDEX`/`index` (3,265/790),
`SPOT_PAIR`/`spot_pair` (48,720/316). This matches the parent audit's own accepted-exceptions register (no new register
entry needed — case-drift owned by an in-flight migration is not a "purge" or "detector gap" candidate, it is tracked,
in-progress work).

**New finding, not previously tracked — filed here, not forked (small, same-doc scope):**

- [x] ✅ [DATA] P2. The nightly-honest-coverage-rollup-backed `/distinct-values/tradfi` endpoint **silently
      under-reports** the tradfi `instrument_types` case-drift — it shows only
      `FUTURES`/`UD`/`UNKNOWN`/`continuous_future` (4) non-canonical, missing the
      `equity`/`future`/`etf`/`combo`/`index`/`spot_pair` lowercase rows entirely (tens of thousands of real captured
      rows each, confirmed via the manifest-direct axis-census above) — the rollup's `by_venue_instrument_type`
      enumeration appears to be collapsing or dropping some raw spellings before this endpoint ever sees them (not
      investigated further here — root cause is in `instruments-service/scripts/measure_honest_coverage.py`, out of this
      REVIEW todo's read-only scope). This is the SAME detector-completeness bug class as this doc's own headline
      finding (a comparison/enumeration mismatch hiding real drift), just in the rollup producer rather than the badging
      comparator. Two NEW clusters also surfaced, neither previously tracked on this doc: `chains` axis now carries
      `ESM0`/`ESM0_MIGRATED_20260418T131054Z` (7 rows each, manifest-direct) — futures-contract root symbols
      miscategorized as blockchain "chains", a wrong-axis writer mis-stamp (cat-3) needing its writer traced; `venues`
      axis now carries `YAHOO_FINANCE` (6 rows) alongside the existing `BARCHART` — plausibly the 2026-07-19
      Yahoo-as-daily-source addition never got a `VENUES_BY_ASSET_GROUP['tradfi']` registration. Root-cause + fix both
      deferred — file as a fresh, precisely-scoped follow-on if picked up; not fixed inline (read-only REVIEW task, no
      code touched). — **DONE 2026-07-28.** The follow-on this todo asked for was filed:
      `plans/active/issues/tradfi_distinct_values_net_new_clusters_2026_07_28.md` "formalises the YAHOO_FINANCE/ESM0
      findings deferred in the entry above, plus a new `UD` instrument_type finding" (per this doc's own 2026-07-28
      Progress Log entry). Confirmed directly: that doc's body carries both clusters by name (`YAHOO_FINANCE`
      venue-registration gap, `ESM0`/`ESM0_MIGRATED_20260418T131054Z` chain-axis mis-stamp) with their own dated
      `[DATA] P2`/`[DATA] P3` follow-up todos. This todo's ask is satisfied — nothing further to file here.

**Conclusion for the dispatching todo**: live-evidence run recorded, both endpoints called, the 3 named 2026-07-18 dupes
confirmed real (not zero) but already explained by the doc's own classification (case-drift → in-flight migration;
`BARCHART` → operator-ruled quarantine) — matches the todo's own "or explicitly explained" acceptable outcome. No purge,
no register edit. One new small finding filed above (rollup under-reporting + 2 new wrong-axis clusters), not chased
further per this task's read-only scope.

### 2026-07-28 — line-191 owning-plan reconciliation DONE (full re-classification of every current non-canonical value)

Worked via `cross_cutting_satellite_ao_dispatch_batch1_2026_07_26.md`'s line-163 dispatched todo. Re-ran
`GET /distinct-values/{asset_group}` in-process (deployment-api, no reimplementation) for all 5 asset_groups — current
total **81 non-canonical values** (defi 24, cefi 2, tradfi 8, prediction 2, sports 45), up from the 2026-07-25
refresh's 45. Classified every value: **51 already resolve to a live owning plan/issue/operator ruling** (including this
doc's own YAHOO_FINANCE/ESM0 findings from the entry directly above, now formally filed rather than left deferred); **30
net-new** (almost entirely a sports MDPS market-token cluster — real, deliberately-produced output from the
market-generalization writer branch, never registered against a canonical set) attributed via 3 fresh issue docs. Full
per-value classification table lives on the dispatching batch doc's own Progress Log (not duplicated here, per the "plan
references, does not duplicate" rule) — `plans/active/cross_cutting_satellite_ao_dispatch_batch1_2026_07_26.md`'s
2026-07-28 entry. New issue docs:

- `plans/active/issues/sports_instrument_type_market_token_ssot_gap_2026_07_28.md` (30 values, cat-2 SSOT gap)
- `plans/active/issues/tradfi_distinct_values_net_new_clusters_2026_07_28.md` (formalises the YAHOO_FINANCE/ESM0
  findings deferred in the entry above, plus a new `UD` instrument_type finding)
- `plans/active/issues/defi_cefi_venue_chain_axis_contamination_2026_07_28.md` (P1/big-finding — defi.venues carrying 9
  chain-names + 5 cefi-exchange-names, defi+cefi.chains both carrying `FUTURES`, cross-AG shape not yet root-caused)

This closes the line-191 todo above and the "Still genuinely open on THIS plan" note in the "Deferred work after
2026-07-20" section (the category-1 owning-plan citation re-verification) — every currently-live non-canonical value now
has an explicit citation, superseding the narrower "spot-check the 22 deleted-JSON clusters" framing.
