...
> **🟢 ARCHIVED 2026-07-26.** All 3 todos done: source-doc reconciliation, the 11 Deferred-item re-check, and this
> archival (parent moved to `/plans/archive/2026_07/prediction_satellite_ao_dispatch_batch3_2026_07_26.md`, corpus
> referrers updated). No new durable contract from this batch — codex-alignment check: nothing to update.
>
...
- [x] ✅ [REVIEW] P1. **Re-check the 7 operator-gated + 1 time-gated + 1 human-only Deferred items, plus the 2
      ...
      **Already-covered-elsewhere (2 notes) — (c):**
                                  ...
                                  **The other 9 (a/b) — one gate CLEARED, extracted into batch4; 8 confirmed still open:**
                                  ...
                                  - `/plans/archive/issues/prediction_lifecycle_prefetch_gate_and_resolution_day_catalogue_2026_07_14.md` — (b) still open,
...
- [x] ✅ [DOC] P1. **DONE 2026-07-26 (worker, slot 6).** Archived
      `prediction_satellite_ao_dispatch_batch3_2026_07_26.md` via the 6-step ritual: batch3's own 2 remaining open todos
      (both genuinely operator-gated, not silently dropped) migrated forward — todo 1's paper-order-flow half already
      had its own tracked issue doc (`kalshi_execution_credential_secret_name_mismatch_2026_07_26.md`); todo 2's
      `[OPERATOR]` residual close-out migrated as a new `- [ ]` todo into its own source doc
      (`prediction_phantom_reconciler_wipes_bundle_atom_2026_07_10.md`, flipped `assigned_vm: planning` so it stays
      dispatchable). Archive banners added to both this doc and the parent. Codex-alignment check: no new durable
      contract from this batch, nothing to update. Corpus referrers fixed (5 files:
      `prediction_satellite_ao_dispatch_batch4_2026_07_26.md`, `kalshi_execution_credential_secret_name_mismatch...`,
      `kalshi_live_capture_regression_and_drift_2026_07_13.md`, `prediction_phantom_reconciler_wipes_bundle_atom...` —
      `gate_on_depends_wiring_gap_defi_dex_pool_finalize_2026_07_25.md`'s mentions are historical incident-narrative,
      correctly left as-is). `locked_by` was already empty on both docs. Both moved to `plans/archive/2026_07/` in the
      same commit. **Done when**: the plan is moved to `plans/archive/2026_07/`, every corpus referrer resolves to the
      new path, and this finalize doc itself gets archived alongside it in the same commit.
