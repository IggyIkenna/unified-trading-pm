...
depends_on: []
resolved_by:
  unified-trading-pm@ac6f77c3d (P2-D), @39d663e92 (P2-B), @0409fa053 (P2-C) -- all operator-ruled parked items closed
locked_by:
context_scope:
  [
...
## Tracked follow-ups (2026-08-07 — the parked items above are real open work, not just decision-prose)

Caught by `check_archive_candidates.sh`: this doc's only checkbox was the migrated-deferral todo below, so a
0-open/1-done mechanical count misread it as fully done and archival-eligible. It is not — P2-B, P2-C, and P2-D above
are still live, unanswered operator decisions. Per the todos-not-prose HARD RULE
...
- [x] ✅ [OPERATOR] P2. **DONE 2026-08-09 (stale-check re-verify) — resolved 2026-07-31, never flipped here.**
      ...
      `unified-trading-pm@39d663e92bbf75c37998bb1221dd501035df98a7` ("docs(plans): unlock + archive 7 fully-done locked
      docs (operator [unlock-plan] ruling 2026-07-30)"), now at
      ...
      (`/plans/archive/2026_07/issues/plan_reconcile_autonomous_sweep_2026_07_30.md`, this doc's own § P2-B).
...
- [x] ✅ [OPERATOR] P3. **DONE 2026-08-09 (stale-check re-verify) — resolved 2026-08-07, never flipped here.** Operator
      ...
      2026-08-07 (operator ruling) — unrecovered loss, accepted, no further investigation. Both the source directory and
      the stash-backup bundle are confirmed genuinely absent; the operator declined recovery." Shipped
...
- [x] ✅ [OPS] P3. **DONE 2026-08-07 — done-when met, see the table below for full evidence.** Audit every other fleet
      host for the two stale-tmp cleanup crons and register them where missing. Provenance:
      `/plans/archive/issues/qg_hardcoded_tmp_paths_false_failures_on_full_tmpfs_2026_07_26.md`'s `[OPS] P2` todo closed
      with the prose sentence "Auditing/remediating other fleet hosts the same way … is tracked separately" — a corpus
      grep this run found **no other doc tracking it**, so the claim was false and the deferral was about to evaporate
      with that doc's archival (a peer archived it mid-run). Filed here per the todos-not-prose HARD RULE
...
- **na-eligibility-audit 2026-08-03** (infra tranche, incremental run, dispatch agt-a41abf): **KEEP-NA, valid —
  unchanged.** In scope only because a context_scope frontmatter backfill (batch 4/5) touched the file; `git show`
  confirms the only other diff in that commit was a pure leading-whitespace reflow inside an existing markdown table (no
  cell content changed). `grep -cE '^- \[ \]'` = **1**, matching this verdict. The 2026-07-30 conflict-check result and
  HELD-at-NA reasoning are not re-litigated here (this skill's own prior dated ruling, per the "never re-litigate an
  established ruling" rule). Standing cheap-overrule offer to the operator unchanged. No action needed.
- **context-scout 2026-08-03**: refreshed context_scope (4 entries, unchanged — reviewed against current doc content and
  still accurate).
