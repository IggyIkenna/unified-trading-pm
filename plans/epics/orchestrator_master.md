...
consolidation. Detailed design moved to codex SSOTs; implementation phases continue as the assigned active plans
listed below.
...
| 4b | Deprecate refresh chain (`swap_claude_account.sh` removed; `oauth_refresh.refresh()` disabled; `.credentials.<id>.json` removed) | 🟡 PENDING | `agent_orchestrator_per_spawn_account_isolation_2026_05_20` |
| 4c | Dashboard repurpose (`OAuthBadge` shows 1-year expiry; `notify_oauth_token_expiring` → 30-day + 7-day cadence) | 🟡 PENDING | `agent_orchestrator_per_spawn_account_isolation_2026_05_20` |
| 5 | Account roster expansion (operator runs `setup-token` per distinct subscription; verify-distinct-subscription test; GCS distribution) | 🟡 PENDING (operator action) | `agent_orchestrator_per_spawn_account_isolation_2026_05_20` |
...
| 8 | Persistence (state-backup cron 6h; KMS-encrypted creds; restore script) | 🟡 PENDING | `agent_reliability_mitigations_2026_05_20` |
...
| 10 | Review agent role (`agents/review.md` boot prompt; commit-poll logic; auto-merge happy-path) | 🟡 PENDING | `agent_orchestrator_workers_on_vms_2026_05_19` |
...
| 12 | Cleanup + docs (archive superseded plans; codex SSOTs landed; CLAUDE.md System-First Architecture pointer) | 🟡 IN-FLIGHT (codex SSOTs landed 2026-05-21; archives + CLAUDE.md pointer pending) | `d0_orchestrator_migration_2026_05_20` |
