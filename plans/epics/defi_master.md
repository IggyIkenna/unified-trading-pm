...
summary:
  L0 asset_group umbrella for DeFi — the May-23 live cutover (7-day continuous run on real wallet), on-chain data
  pipeline (oracle prices, chain expansion, DEX-perp adapters, subgraph/adapter fixes), custody (Copper + Cloud-KMS +
  Fireblocks), governance-param refresh, on-chain-derivable-values + UAC date-drift fixes, and coverage validation; work
  dispatched via child active plans.
status: active
nature: process
asset_group: [defi]
stage: [meta]
...
- [`/codex/02-data/availability-manifest-and-data-status.md`](/codex/02-data/availability-manifest-and-data-status.md) —
  manifest v9 schema (corrected 2026-08-09, was stale "v5") + DeFi `chain` first-class shard axis +
  `EXPECTED_PRE_GENESIS_CHAIN` / `EXPECTED_PRE_VENUE_LAUNCH` reasons + DeFi protocol-launch-date pre-skip semantics
...
- **2 DeFi perp DEXs live**: Hyperliquid + Aster. Plus historical-replay backfill for Lighter / Extended / Pacifica
  (originally scoped under CeFi venue expansion but ~~they are DeFi by asset_group~~ (was: "they are DeFi by
...
| Tail chains 25% coverage (Aurora / Celo / Fantom / Mantle / Metis / Moonbeam)   | **RESOLVED per this doc's own DIAGNOSIS COMPLETE 2026-05-19 entry below (line ~638) — not a May-23 gate; noted 2026-08-09**                                                                                                                                                                 | `consolidated_defi_data_pipeline`                                                  | Each tail chain shows ≥4/4 protocols captured for at least 1 sample day post-protocol-launch; rest stamped `EXPECTED_PRE_VENUE_LAUNCH`                                                                                                                                                                                                                                                                                                                                                  |
...
| Lighter / Extended / Pacifica historical-replay backfill                        | Lighter/Pacifica backfill VMs reached final state (GCS-verified 2026-06-16) but only from 2025-05-01+/2025-07-01+ — SHORT of this row's stated gate below by ~9mo/~1mo, unreconciled (2026-07-12 correction, finding #47, §A2 B-queue ruling; was: "scoped; ABI parsing per chain pending") | `dex_historical_replay_*` + `defi_pipeline_e2e_and_coverage_validation_2026_06_20` | Lighter + Pacifica OHLCV non-empty 2024-08-01+ / 2025-06-01+ respectively; Extended pending Phase 0 empirical research before any VM launch — **NOT MET as originally stated**: `defi_pipeline_e2e_and_coverage_validation_2026_06_20`'s own P0 checked ✅ 2026-06-16 against actual GCS data only from 2025-05-01+ (Lighter) / 2025-07-01+ (Pacifica), not this cell's 2024-08-01/2025-06-01 bar; the child plan's checkbox flip was never reconciled against this row's stricter gate |
...
| ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| Governance parameters refresh (Aave/Compound/Morpho event listener → time-versioned `governance_params` parquet → APR-calc + sizing migration + new UAC `GOVERNANCE_PARAMS_CHANGED`)                                                | **EXTRACTED (net-new)**                                                                                                         | [`defi_governance_params_refresh_2026_06_20`](/plans/archive/2026_06/defi_governance_params_refresh_2026_06_20.md) (repointed 2026-08-09, `status: complete`)                                                                                                                                                                                    |
...
| `mtds-s4-10` rescan-all-manifests + LIGHTER `perp_funding` (A5) + DeFi per-chain/per-venue launch dates + DEX per-chain launch-date + on-chain features + DeFi single-walk / G1 backfill / coverage rollup                          | **OWNED ELSEWHERE — do not duplicate**                                                                                          | **CORRECTED 2026-08-09**: `defi_manifest_canonicalisation_2026_06_01` is `status: superseded` (archived) — real owner is [`data_completion_to_100_all_ag_2026_06_21`](../active/data_completion_to_100_all_ag_2026_06_21.md) (active, 11 open todos)                                                                                             |
| `source=` provenance / row-level source column for DeFi                                                                                                                                                                             | **OWNED ELSEWHERE — do not duplicate**                                                                                          | **CORRECTED 2026-08-09**: `data_source_provenance_all_asset_groups_2026_06_01` is `status: superseded` (archived) — same owner, [`data_completion_to_100_all_ag_2026_06_21`](../active/data_completion_to_100_all_ag_2026_06_21.md)                                                                                                              |
...
- [ ] [HUMAN+AGENT] P0. mtds-s4-10-rescan-all-manifests: Re-scan ALL availability indexes after migrations. **Cross-plan
      ...
      `§ Manifest concurrency principle`. [AUDIT 2026-05-07: BLOCKED-ON manifest_migration_SUPERSEDED_2026_05_21:Stage
      3]
...
> [`defi_mtds_subgraph_and_adapter_fixes_2026_06_20`](../archive/2026_06/defi_mtds_subgraph_and_adapter_fixes_2026_06_20.md);
> the Lighter/Pacifica final-state VERIFY →
> [`defi_pipeline_e2e_and_coverage_validation_2026_06_20`](../active/defi_pipeline_e2e_and_coverage_validation_2026_06_20.md).
...
- [x] **[DEFERRED-POST-CUTOVER]** [AGENT] P2. **Lighter symbol-coverage scale-up** — currently
      `_LIGHTER_BACKFILL_TOP_SYMBOLS = (BTC, ETH, SOL, HYPE, TON)`; expand to top-30 (Lighter has 170 perps including
      NVDA, USDCAD, BRENTOIL, XAU, XAG, SNDK exotics). Rate-limit budget already validated — 12 RPS handles top-30
      comfortably. Unlocks cross-asset stat-arb / FX-perp arb against CeFi FX. [AUDIT 2026-05-07: FRESH — HANDOVER Item
      D; deferred pending strategy demand signal]
...
- `instruments-service@1a90185` — Bug 3: `get_protocol_floor_date()` consults UAC `PROTOCOL_LAUNCH_DATES` SSOT first;
  AAVE V3 ETHEREUM floor corrected from 2022-03-14 (legacy, mis-aligned with the L2 cohort) to 2023-01-27 (UAC-verified:
  AAVE V3 ETH subgraph earliest `reserveParamsHistoryItems` event is 2023-01-27 08:00:11 UTC; live
  `unified-api-contracts/registry/chain_env.py:175-189`). (was: "corrected from 2023-01-27 (legacy) to 2022-03-14" —
  direction was backwards; this line disagreed with this epic's own L1278 "AAVE_V3 ETHEREUM date was 49 days wrong
  (corrected 2023-01-27)" and with the live UAC SSOT. [SYNCED 2026-07-14, findings 39+44])
...
> **STALE-DRIFT ANNOTATION 2026-07-11 (was: "re-classified to DeFi")**: this section header's premise is superseded —
> LIGHTER-ZKSYNC / EXTENDED-STARKNET / PACIFICA-SOLANA are **CeFi, NOT DeFi** (v10 decision #4, UAC
> `unified_api_contracts.canonical.crosscutting.mvp_scope`: "classified cefi everywhere — reconciled vs the
...
- [x] [AGENT] P0 **DESCOPED per POD stack choice + operator ruling 2026-07-12 (plan-reconciliation finding 376, see
      `/plans/archive/issues/plan_reconciliation_operator_decisions_2026_07_11.md:117`, citation added 2026-08-09) —
...
- [x] [SCRIPT] P0. Ran `instruments-service/scripts/reconcile_phantom_manifest_rows_all.py --asset-group defi --dry-run`
      locally, scoped `--venues AAVE_V3` (sufficient for triage; full DEFI scan deferred to GCE VM after the prober
      landed below to avoid 18× slowdown × 313k row × 7 prefix template explosion). Initial run reported 29782
      false-positive phantoms — the entire AAVE_V3 dataset; would have destroyed all manifest state had `--apply` run.
...
- [x] [SCRIPT] P0. **Priority #5 — Lending-indices LINEA/BSC routing config.** Distinct workstream from priority #1
      ...
      **Recent-days catch-up `2026-05-07..2026-05-11`** (~5-10min scoped run) —
      `launch-mtds-lending-indices-backfill-vm.sh 2026-05-07 2026-05-11` (event-verify per "No fire-and-forget VM
      launches"; the daemon then re-consolidates). The full-history VM `mtds-lending-indices-20260511-181115` was
...
- [x] **[DEFERRED]** [SCRIPT] P2. **Future consolidator-poll-list gap — features-\* / execution-store /
      ...
      `market-data-tick-defi-{pid}`, but those MTDS handlers actually write the data to the dedicated per-`data_type`
      ...
      progress probe — `market-data-tick-defi-{pid}` is a valid (some handlers also write a catalogue index there) but
      imprecise target. Cosmetic watchdog-progress-check imprecision, NOT a consolidator gap (the consolidator poll-list
      is now correct vs the authoritative source = `get_write_bucket_name()` callsites + `_BUCKET_CATEGORY_OVERRIDES`).
      Fix when touching the watchdog dict next: point those 5 prefixes at their dedicated buckets. Operator relaunch of
      the watchdog VM required to pick it up (per CLAUDE.md VM-Naming-Convention rule).
...
- [x] ✅ **[FIXED — deployment-service@a2b3c92]** [SCRIPT] P1. **`create-code-tarballs.sh` has a stale repo list +
      non-graceful skip** — its `DEFI_REPOS`/`EXTRA_REPOS` list references `features-service (onchain family)`
...
- [x] ✅ [SCRIPT] P1. **Wire `ManifestFreshnessCache` into `lending_indices_handler` + sibling MTDS DeFi backfill
      ...
      (`liquidation_events`/perp_funding/solana_lst_archival) + MTDS@63ae34d (dex/liquidations handlers)
      ...
      different check from "manifest already has this day captured." So a full-history re-run re-does years of
...
derive expected universe). 5-phase remediation. ~~Operator decision required on asset_group classification.~~ (was:
"Operator decision required on asset_group classification.") **RESOLVED 2026-07-11** — see Phase 4 annotation below.
...
- [ ] [HUMAN] P1. **Phase 4 — asset_group classification decision (operator).** CLOB-on-chain venues (Lighter / Pacifica
      ...
      **Decision needed before Phase 5 below ships.** **[STALE-DRIFT ANNOTATION 2026-07-11]**: ~~decision pending~~ —
      RESOLVED. Neither option (a) nor (b) — the operator/UAC-SSOT ruling is **these venues stay CeFi** (LIGHTER-ZKSYNC
...
> [`defi_mtds_subgraph_and_adapter_fixes_2026_06_20`](../archive/2026_06/defi_mtds_subgraph_and_adapter_fixes_2026_06_20.md).
> The verification-recipe automation is ✅ shipped (mtds@507774e). Do NOT dispatch from here.
...
### [`mvp_backfill_defi_onchain_v10_2026_06_27`](/plans/archive/2026_08/mvp_backfill_defi_onchain_v10_2026_06_27.md)

**status**: ✅ ARCHIVED 2026-08-05 (was: active, corrected 2026-08-09) · **estimate**: 4.8 cal AI-days (class: infra)
**title**: MVP backfill — DeFi all on-chain data_types (SPOT-only, per-protocol genesis, reconcile-then-fill) — all 19
todos done

### [`mvp_backfill_defi_onchain_v10_operational_log_2026_07_24`](/plans/archive/mvp_backfill_defi_onchain_v10_operational_log_2026_07_24.md)

**status**: ✅ ARCHIVED (was: active, corrected 2026-08-09) · **estimate**: 4.8 cal AI-days (class: infra) **title**:
MVP backfill — DeFi all on-chain data_types — operational log (extracted from mvp_backfill_defi_onchain_v10)
...
| Plan                                                                                                                                                        | Role                                                                                                                | Status                                                                                                                                                               |
| ----------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| [`defi_catalogue_chain_primitives_2026_05_10.md`](/plans/archive/2026_05/defi_catalogue_chain_primitives_2026_05_10.md)                                     | Chain primitive registry + UAC capability declarations per-chain — feeds lending-indices and oracle prices coverage | ✅ Archived (was: Active — file frontmatter is `status: complete`; corrected 2026-08-09)                                                                             |
...
| [`defi_archetypes_canonicalisation_and_venue_matrix_2026_05_07.md`](/plans/archive/2026_05/defi_archetypes_canonicalisation_and_venue_matrix_2026_05_07.md) | Archetype naming canonicalisation + full venue × archetype matrix — Stream A/B naming and config-grid               | ✅ Archived (was: Active — file frontmatter is `status: complete`; corrected 2026-08-09)                                                                             |
| [`defi_recursive_borrow_archetypes_2026_05_10.md`](/plans/archive/2026_05/defi_recursive_borrow_archetypes_2026_05_10.md)                                   | CARRY_RECURSIVE_BORROW family (lending-only + perp-hedged) — backtested, code-complete by May-23                    | Archived dir, frontmatter still `status: active` (corrected 2026-08-09 — top-of-doc banner already redirects to the post-cutover successor plan, see line ~79 above) |
...
| [`hedge_ratio_snapshot_persistence_2026_05_13.md`](/plans/archive/hedge_ratio_snapshot_persistence_2026_05_13.md)                                           | Hedge-ratio snapshot persistence for DeFi perp shorts — feeds carry_staked_basis live position sizing               | ✅ Archived (was: Active — file frontmatter is `status: complete`; corrected 2026-08-09)                                                                             |
...
| [`dex_perp_and_venue_data_expansion_2026_05_12.md`](/plans/archive/2026_05/dex_perp_and_venue_data_expansion_2026_05_12.md)                                 | DEX perp and venue data expansion — Lighter / Pacifica / Extended forward-poll + historical replay completion       | ✅ Archived (was: Active — file frontmatter is `status: complete`; corrected 2026-08-09)                                                                             |
...
> **History extracted 2026-07-24 (epic line-cap compliance)**: the dated session logs "DONE-2026-05-08-tab1",
> "DONE-2026-05-08 — Tab 1 main", "DONE-2026-05-12 — Harsh tab 3", and the 2026-05-12 slot-5 cross-plan annotation moved
> verbatim to [`defi_master_history_2026_07_24.md`](/plans/archive/2026_07/defi_master_history_2026_07_24.md) (zero
> checkbox todos in that range — pure narrative/evidence, nothing open lived there).
