...
summary:
  ...
  strategy-consumption chain — the feature_group→required_inputs registry, no-lookahead/available_at gating,
  feature-DATA consolidation (pre-joined wide parquet), features-repo consolidation, and the regime-clustering structure
  allocator; batch feature compute (live compute lives in the live-pipeline plan).
status: active
nature: process
...
- [x] [AGENT] P1. **UTL `assert_no_lookahead_for_feature_group(feature_group, inputs_df, target_ts)` helper**. **SHIPPED
      UTL@4354276c**: reads UAC `FEATURE_REQUIRED_INPUTS[fg]`, computes max_horizon, raises `LookaheadBiasError` if any
      input row has `available_at > target_ts - horizon`. 9 unit tests.
