...
related_plans:
  ...
  - ../active/cefi_ml_directional_continuous_live_2026_06_20.md
  - ../active/cefi_satellite_ao_dispatch_batch4_2026_07_31_finalize.md
  - ../active/cefi_satellite_ao_dispatch_batch6_2026_08_02_finalize.md
  - ../active/cefi_satellite_ao_dispatch_batch7_2026_08_03_finalize.md
  ...
  - ../active/cefi_track7_candle_namespace_residual_2026_07_25.md
  - ../active/cefi_track7_candle_namespace_residual_finalize_2026_07_25.md
...
| --------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 4 CeFi perp venues live (Bybit / Deribit / Binance / OKX) | INSTRUMENTS LIVE; tick-data ~60%                                                                                                                                                                                                                                                                                                                                                                                                      | `cefi_tradfi_tick_data_backfill`                                                                                                                                                                                                                     | Per-venue per-`data_type` per-day shard ≥99% captured for 2024-01-01 → today; `perp_funding` populated continuously; forward-poll launcher emits `STARTED`+`PROCESSING_*` events for each venue          |
| DERIBIT options + futures bundles backfilled to genesis   | **[Corrected 2026-07-14, findings 28/34]** (was: "2024 done; 2025/2026 in flight") — NEVER genuinely captured pre-2026-06-12 (verification found 0/2,590 days real, 136/138 "captured" rows PHANTOM_CAPTURED_NO_OBJECT); full relaunch 2026-06-24 (33 VMs); `DERIBIT-COMBO/options_chain/trades` genuine gap still OPEN as of 2026-07-13 close-out (Layer-1 91.78%, 6 missing tuples)                                                 | `cefi_tradfi_tick_data_backfill` (2025/2026 VMs running 2026-05-06+); verified + relaunched per `cefi_deribit_binance_futures_bundle_verification_2026_06_20.md:73-82`; residual tracked in `mvp_backfill_cefi_tick_v10_2026_06_27.md:947-1030,2057` | All `options_chain` / `futures_chain` bundles record_captured with `expected_root_clusters` validation passing per-day (cluster coverage gate per CLAUDE.md "Cluster validation MANDATORY")              |
...
| Bitfinex / Bitget / Kraken Tardis venues                  | **[Corrected 2026-07-14, finding 211]** (was: "NOT STARTED" — self-contradicted this same doc's § "AI-day estimate" line ["mostly shipped per UAC@7cb9068/405cbf5"] and § "Consolidated todos" DONE items) — CODE + LAUNCH SHIPPED (2026-05-07 audit: UAC registry / MTDS adapter / launcher all verified DONE with commit SHAs, 24 VMs running); source plan `cefi_venue_universe_expansion_2026_05_01` is itself `status: complete` | `cefi_venue_universe_expansion` (archived, complete)                                                                                                                                                                                                 | All 3 venues × spot+futures × 2020-2026 backfilled; per-VM 4-pillar validation green (row count > 0, NaN ratio in tolerance, schema match, cluster coverage where applicable); zero silent-zero captures |
...
_14 active plans declare `parent_epic: cefi_master` in their frontmatter. Workers pick up in priority order (P0 first).
...
### [`cefi_track7_candle_namespace_residual_2026_07_25`](../active/cefi_track7_candle_namespace_residual_2026_07_25.md)

**status**: active · **estimate**: 0.25 cal AI-days (class: infra) **title**: CeFi Track-7 candle-namespace residual —
gated delete of the 149 stale bundle-collision objects
...
### [`cefi_satellite_ao_dispatch_batch4_2026_07_31_finalize`](../active/cefi_satellite_ao_dispatch_batch4_2026_07_31_finalize.md)

**status**: active · **estimate**: 0.28 cal AI-days (class: infra) **title**: CeFi satellite AO batch 4 — finalize
(reconcile source docs + re-check deferrals + archive)

### [`cefi_satellite_ao_dispatch_batch6_2026_08_02_finalize`](../active/cefi_satellite_ao_dispatch_batch6_2026_08_02_finalize.md)

**status**: active · **estimate**: 0.28 cal AI-days (class: infra) **title**: CeFi satellite AO batch 6 — finalize
(reconcile source docs + re-check deferrals + archive)

### [`cefi_satellite_ao_dispatch_batch7_2026_08_03_finalize`](../active/cefi_satellite_ao_dispatch_batch7_2026_08_03_finalize.md)

**status**: active · **estimate**: 0.24 cal AI-days (class: infra) **title**: CeFi satellite AO batch 7 — finalize
(reconcile source docs + re-check deferrals + archive)

### [`cefi_track7_candle_namespace_residual_finalize_2026_07_25`](../active/cefi_track7_candle_namespace_residual_finalize_2026_07_25.md)
