...
related:
  [
    ../archive/2026_08/predictions_other_bucket_and_ui_drilldown_2026_06_20.md,
...
- [x] [CODE] P0. Implement (or verify shipped) `arb_calculator` in FSS: cross-bookmaker arb %, eligible pairs, duration.
      ...
      `feat(sports/arb): implement arb_calculator — cross-bookmaker arb %, eligible pairs, duration`; verified reachable
...
- **Temporary state — SEEDED** (was: UAC `PREDICTION_GROUPS = {}` empty registry until taxonomy seeded — CLAUDE.md
  ...
  `OTHER` special-case) at `unified-api-contracts@306923a` per the now-archived P0 child plan
  `/plans/archive/2026_08/predictions_other_bucket_and_ui_drilldown_2026_06_20.md` — no longer the empty `{}`
  placeholder; the CLAUDE.md "Temporary state" rule no longer applies here.
...
**status**: resolved (archived 2026-08-16) · **estimate**: 0.16 cal AI-days (class: infra) **title**: Prediction
satellite AO batch 4 — finalize (reconcile sibling source docs + resolve deferrals + archive)
