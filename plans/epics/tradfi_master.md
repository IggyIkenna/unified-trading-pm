...
tokens as of 2026-07-21 — CODEX-GATED follow-up needed, same spirit as the resolved `tradfi-databento-sourcing-ssot.md`
VIX note above**): S&P 500 (CME ES + ES.OPT; **no SPY** — operator clarification 2026-05-13, "ES futures has more
trading hours") + BTC/ETH ETFs (NASDAQ IBIT, NASDAQ ETHA) + crypto futures (CME MBT, CME MET) + VIX FUTURE (VX via
Databento `XCBF.PITCH`; cash index deleted 2026-06-23 — **no CBOE BTC options on IBIT**: `option_underliers={ES}` ONLY,
operator ruling "no CME option for BTC and ETH", `/codex/02-data/mvp-scope-canonical.md:88`) + GLD/USO/UNG
cross-instrument carry. **2026-07-21 +409 additions** (`uac@afa2dd64`→`22e6a534`): treasury INDEX curve (CBOE
US3M/US2Y/US5Y/US10Y/US30Y) + KRW FX (`FX:SPOT_PAIR:KRW-USD`) + BTC/ETH FUTURES (already listed above as CME MBT/MET,
now joined by BTC/ETH roots). Served catalogue rebuild for the +409 expansion is pending post-backfill (catalogue still
reflects the old mvp count until rebuild+promote). Tier A archetypes touching TradFi: ml-continuous (ES) +
...
> window) are routed to [`master_to_live_defi_2026_05_23`](/plans/archive/2026_07/master_to_live_defi_2026_05_23.md)
> Group F (item 17 backtest fidelity + cutover gating); the TradFi-data + ES-feature execution slice is extracted to
> [`tradfi_sp500_ml_and_arb_backtest_readiness_2026_06_20`](/plans/active/tradfi_sp500_ml_and_arb_backtest_readiness_2026_06_20.md).
