...
related_plans:
  ...
  - ../active/ui_satellite_ao_dispatch_batch4_2026_08_17.md
  - ../active/ui_satellite_ao_dispatch_batch4_finalize_2026_08_17.md
last_updated: 2026-08-19 # (was: 2026-08-15 -- plan_reconciler epic-scoped run 2026-08-19: batch4_2026_08_17 + its
  # finalize (created 2026-08-17, 1 day after the last refresh) were never folded into related_plans/body; the
  # archived batch4_2026_08_13 pair (superseded 2026-08-17) removed. Cross-verified via
  # `populate_epic_bodies_2026_05_21.py --dry-run` (P0=2/P1=5/P2=3/P3=2, matching this doc's 12 children's own
  # frontmatter `priority:` fields exactly) -- fleet-wide --apply NOT run (would touch ~25 other epic files, 2
  # already dirty with foreign uncommitted WIP on this shared checkout); this file edited by hand to match.
...
Live HTML ledger: https://claude.ai/code/artifact/270829b4-8d73-470e-b7f3-988b5d5b4d61 (generated 2026-08-19,
`/plan-reconcile deployment_and_user_management_master`)
...
the plan-list body had drifted. **2026-08-15 refresh** (`/plan-reconcile ui`, Phase 3 epic-consistency check):
`populate_epic_bodies_2026_05_21.py --apply` re-run (this epic's diff isolated from the fleet-wide run and applied
alone) — `ui_satellite_ao_dispatch_batch4_2026_08_13` + its `_finalize` sibling (created after the 2026-07-30 refresh)
had never been folded into the body or `related_plans:`, and `batch1_finalize` was correctly present but the P2 block
still listed the now-archived `batch2`/`batch2_finalize` pair; regenerated to the current 12 genuinely-active plans.
**2026-08-19 refresh** (`/plan-reconcile deployment_and_user_management_master`, epic-scoped): same drift class one
cycle later — `ui_satellite_ao_dispatch_batch4_2026_08_17` + its `_finalize` (created 2026-08-17, the day after the
2026-08-15 refresh) had never been folded in either; the P2 block still listed the now-archived
`batch4_2026_08_13`/`_finalize` pair (superseded 2026-08-17). Regenerated by hand to the current 12 genuinely-active
plans (fleet-wide `populate_epic_bodies_2026_05_21.py --apply` not run — see `related_plans:` frontmatter comment
for why).
...
### [`ui_satellite_ao_dispatch_batch4_2026_08_17`](../active/ui_satellite_ao_dispatch_batch4_2026_08_17.md)

**status**: active · **estimate**: 0.4 cal AI-days (class: infra) **title**: UI satellite AO batch 4 — denominator-freshness
trust annotation (na-eligibility-audit RECLASSIFY-per-todo-split of data_status_tab_and_downloads_remediation)

### [`ui_satellite_ao_dispatch_batch4_finalize_2026_08_17`](../active/ui_satellite_ao_dispatch_batch4_finalize_2026_08_17.md)

**status**: active · **estimate**: 0.16 cal AI-days (class: infra) **title**: UI satellite AO batch 4 — finalize
(upgrade source-doc citation + archive)

> **2026-08-19 refresh** (`/plan-reconcile deployment_and_user_management_master`, epic-scoped): the P2 tier's
> `ui_satellite_ao_dispatch_batch4_2026_08_13`/`_finalize` pair (archived 2026-08-17) is removed here — both are
> superseded by the current `batch4_2026_08_17` pair above, which `parent_epic:` frontmatter confirms as this
> epic's real 11th/12th active child (was missing from this body + `related_plans:` since creation, same drift
> class as the 2026-07-30/08-15 refreshes above fixed for other stale entries).
