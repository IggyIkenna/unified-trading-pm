...
### CF-22 … CF-27 — cycle-gap hardening (added 2026-06-19; the gaps the 2026-06 data-migration cycle's audits MISSED)

> **Why these exist.** The 2026-06 data-migration cycle surfaced six correctness gaps that the THEN-current epic audits
> **passed over** — each because the existing check stopped at a PROXY (dedup-clean, schema-constant, "data exists",
> per-candidate delete gate) instead of the real invariant (columns POPULATED, the 4th state PRESENT, a comprehensive
> RUN performed, 100%-twin coverage, fetcher-true provenance, coverage-vs-universe ratio). These CF rows turn each
> missed gap into an explicit, re-runnable check so a future audit CANNOT green on the proxy. Read DATA-STATE, never a
> constant (the manifest-v8 lesson). Owning per-service audit files cross-ref these rows; this SSOT is their home.

| ID    | Invariant (canonical target)                                                                                                                                                                                                                                                                                                                                                             | Audit method (data-state — the trap to avoid)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                | Applies to                                          |
| ----- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------- |
| CF-22 | **v9 is COLUMN-POPULATION, not just `schema_version` + dedup.** A `_index` is canonical v9 ONLY when `schema_version==9` % is high AND the v9-added columns `pipeline_mode` / `source` / `asset_group` are POPULATED (% non-blank), for **BOTH** the instruments-store AND the market-data-tick (and MDPS candle) buckets, PER asset_group.                                              | Per `(bucket_kind ∈ {instruments-store, market-data-tick, processed_candles}, asset_group)` read FOUR distributions from the prod `_index`: (1) `schema_version==9` %, (2) `pipeline_mode` non-blank % AND source-aware (`{mode}_{source}`, not coarse `batch`/blank), (3) `source` non-blank %, (4) `asset_group` non-blank %. **TRAP (the "sports already clean" miss): dedup/blank-status-clean ≠ v9.** GREEN = all four ≥99% on BOTH bucket kinds, every AG. Extends/strengthens CF-1+CF-2+CF-3+CF-4+CF-13 jointly.                                                      | instruments · mtds · mdps (all AGs)                 |
| CF-23 | **`expected_unattempted` 4th state is PRESENT in the `_index` — 0 rows fleet-wide = FAIL.** It was absent fleet-wide → every coverage denominator silently under-counted (no owed cells). The materialised-by-writer rule (CF-6) is moot if the rows never appear.                                                                                                                       | Read the `capture_status` value-set per `(asset_group, bucket_kind)` from the prod `_index`; assert `expected_unattempted` appears with **count > 0** wherever the IS catalogue lists could-exist cells not yet backfilled. **TRAP: a `_index` with only {captured, empty_confirmed, attempted_failed} is RED, not "3-state clean".** GREEN = `expected_unattempted` row-count > 0 on every AG that has owed cells; 0 rows on an AG with genuinely-owed cells = FAIL. Data-state counterpart of CF-6.                                                                        | mtds · mdps · instruments · features (denominators) |
| CF-24 | **Shard 4-pillar validation was RUN comprehensively — evidence required, not a manifest proxy.** The shard atom must pass all four pillars (row-count>0-or-`record_empty` · NaN-ratio<threshold · schema-matches-contract · cluster-coverage≥expected); a green manifest row does NOT prove the parquet passed them.                                                                     | Require a cited, dated cross-AG 4-pillar RUN artifact (the validation report / CLI output, per asset_group × data_type) — not just "manifest says captured". Spot-open ≥1 parquet per AG and confirm the four pillars actually evaluated. **TRAP: "manifest divergence==0" is a proxy, not the 4-pillar run.** GREEN = a comprehensive 4-pillar run exists, is dated within the audit window, covers every AG, and reports 0 pillar failures (or the failures are tracked todos).                                                                                            | infrastructure (4-pillar SSOT) · mtds · mdps        |
| CF-25 | **100% canonical-twin coverage BEFORE any delete — the delete-safety gate is corpus-wide, not per-candidate.** Every legacy object must have a `gcs_describe`-verified canonical twin (or be proven superseded-and-covered) across BOTH bucket types per AG, BEFORE a single legacy delete runs. CF-21 gates each candidate; CF-25 gates the whole campaign.                             | Before any `--apply` delete: per `(asset_group, bucket_kind ∈ {instruments-store, market-data-tick})` enumerate legacy-shape objects and assert EVERY one has a `gcs_describe_object`-verified canonical twin in the manifest (`captured` + crc32c-identity per CF-21) OR is on the proven-superseded-covered list. **TRAP: deleting after a sampled/per-candidate pass leaves un-twinned legacy data unrecoverable.** GREEN = twin-coverage == 100% on both bucket kinds, every AG, with the report cited, before delete.                                                   | mtds · mdps · instruments (migration cleanup)       |
| CF-26 | **Provenance is write-stamped by the FETCHING ADAPTER, reflecting the ACTUAL fetcher — NOT `SOURCE_PRIORITY[0]`.** `SOURCE_PRIORITY` is READ-time resolution only. A writer that stamps `source`/`pipeline_mode` from `SOURCE_PRIORITY[(ag,dt)][0]` lies when a non-priority vendor actually fetched the cell (the VX-15m → `batch_massive` bug: Massive does not cover VIX cash index). | For every writer callsite, trace the `source=` / `pipeline_mode=` value to its origin and confirm it is the ADAPTER's own identity (the vendor that ran the fetch), never indexed out of `SOURCE_PRIORITY`. Data-state: pick a cell whose actual fetcher ≠ priority[0] (VX 15m: yahoo/barchart, not massive) and assert the stamped `source` matches the real fetcher. **TRAP: `source` present + in-closed-set still passes CF-4 while being the WRONG vendor.** GREEN = 0 writers index `SOURCE_PRIORITY` for stamping; sampled cross-vendor cells stamp the true fetcher. | mtds · mdps · instruments · tradfi (VX precedent)   |
| CF-27 | **Backfill coverage vs the TARGET UNIVERSE across the WHOLE timeframe — flag large gaps, not just "data exists".** Per AG compute `coverage = captured / (captured + empty_confirmed + attempted_failed + expected_unattempted)` over the full intended date range and flag low ratios; "some data is present" is not coverage.                                                          | Per `(asset_group, data_type, venue)` compute the 4-state coverage ratio across the full target timeframe (could-exist universe from the IS catalogue, CF-14/CF-16) and produce a ranked low-coverage table. **TRAP: an audit that confirms "rows exist for venue X" without the ratio misses a 5%-covered cell.** GREEN = coverage ratio computed per AG over the whole timeframe, low-coverage cells (< an agreed floor) flagged as backfill todos, none silently passed as "data exists".                                                                                 | mtds · mdps · features · instruments                |

> **CF-22 … CF-27 (added 2026-06-19)** harden the six gaps the 2026-06 data-migration cycle's audits missed (each passed
> on a partial check). They are written in steady-state form ("assert the v9 corpus holds X") so a future regression is
