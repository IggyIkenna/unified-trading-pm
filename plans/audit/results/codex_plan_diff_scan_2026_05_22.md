...
summary:
  Differential scan of all 717 codex docs × 44 active plans × 26 epics for broken codex refs, dead SUPERSEDED
  forward-links, and stale time-markers — found 5 real BROKEN-REF codex paths + 12 P0 post-May-23 markers (May-23 =
  audit day); Phase-4 after-counts show all resolved inline (broken-ref 0, 6 codex stubs written, 12/12 post-May-23
  markers fixed, archetype count 53→57 corrected).
status: pass
nature: record
...
audited_scope:
  ...
  and stale time-markers (post-May-23 / post-cutover / future / TODO / not-yet-implemented). NOT covered — a per-item
  actionability filter on plan open-item counts (mechanical checkbox tallies only).
date: "2026-05-22"
...
| `codex/09-strategy/architecture-v2/archetypes/<placeholder>.md` (e.g. `xyz-foo`)         | FALSE-POSITIVE   | `plans/active/issues/strategy_archetype_logic_audit_2026_05_20.md`                                                             | Template placeholder in issues doc, not a real path. No action needed. De-fanged here so it no longer regex-matches as a real cross-doc reference (check_reference_paths.py's --only precommit gate flags any /codex/... or /plans/...-shaped token found anywhere in a staged file's body, including inside a documentation table describing a known false-positive). |
