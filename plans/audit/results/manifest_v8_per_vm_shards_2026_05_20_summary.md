...
Master availability_index has 0% v8 rows workspace-wide (A4 v1). If per-VM shards are also v<8, the consolidator preserves the source version (correct behavior). If per-VM shards ARE at v8 but master isn't, the consolidator should be regenerating master from per-VM shards — gap.
