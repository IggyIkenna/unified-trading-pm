...
> Generated by the slot running `QG_SLICE=lint-codex bash scripts/quality-gates.sh --no-fix` in EVERY service/library/PM
> repo on 2026-06-10 (post grep-P→rg-pcre2 parity fix, so local counts == CI counts). This is the remediation matrix for
> the ratchet-to-≤5 plan: each repo's opaque budget number converted into the concrete failing check-classes +
> representative file/line offenders. Raw logs: slot-local `/tmp/census/<repo>.log` (re-runnable any time with the
> command above).
...
**Headline:** 5 repos are over the ≤5 ceiling (deployment-api 24, execution-service 22, MTDS 15, strategy 10, MDPS 7). 3
repos can ratchet DOWN immediately with zero code change (deployment-service 8→1-headroom, ibkr 4→2, ml 5→3 after its 3
classes clear). UAC needs its lxml advisory cleared (bump to ≥6.1.0) + budget re-pinned.
...
- ❌ STEP 5.71: NEW record_captured() callsite(s) without paired publish_with_policy() / publish_with_manifest_lookup().
  Wire the emission-policy helper or add '# QG-allow: emission-policy-not-applicable' on each input-data
  record_captured() line:
- ❌ Recheck: .venv/bin/python unified-trading-pm/scripts/quality_gates/check_emission_policy_paired_callsites.py
  --workspace-root /home/ubuntu/unified-trading-system-repos --scope execution-service
...
- ❌ os.getenv()/os.environ — use UnifiedCloudConfig for config, get_secret_client() for secrets
- ❌ asyncio.run() in loop: market_data_processing_service/cli/handlers/live_aggregator_handler.py — use
  asyncio.gather()
- ❌ Imports inside functions — move to top (AST-detected)
...
- ❌ STEP 5.64: Service has preflight-guard patterns but no emit_preflight_skip / PREFLIGHT_SKIPPED — silent skips will
  be invisible in the event stream. Files with preflight patterns:
...
- ❌ Fix: import emit_preflight_skip from unified_trading_library + PreflightSkipReason from
  unified_api_contracts.internal; emit at every preflight return-True / dependency-raise / skip-date branch. SSOT:
  features-onchain-service/cli/handlers/batch_handler.py:\_preflight_guard (commit reference 2026-05-07).
...
- ❌ STEP 5.94: NEW try/except-ImportError fallback-import shim(s) above the per-repo baseline. Import directly +
  declare the dep in pyproject, or add '# noqa: fallback-import' with a one-line reason (no-empty-fallbacks.mdc):
...
- ❌ VERSION_SPLIT — 14 repo(s) with a version split: agent-orchestrator, batch-live-reconciliation-service,
  deployment-api, deployment-service, e2e-testing, execution-service, fund-administration-service, ibkr-gateway-infra,
  instruments-service, market-data-processing-service, ml-service, strategy-service, unified-trading-library,
  unified-trading-pm
...
1. **Mechanical bulk classes first** (imports-in-fn: 127 dep-api / 122 exec / 74 MTDS / 8 strategy — AST tool lists
   every site; one sweep per repo).
...
4. **deep-imports** — Phase 2 facade re-export clears deployment-api / exec / MTDS / MDPS / strategy in one UAC change +
   per-repo callsite flips.
5. **file-size / fn-size** — Phase 1 splits (registry 18k, orchestrator 8k, data_status 6.6k, seed 5.1k, server 4.5k,
   MTDS orchestrator 4.2k + tail).
