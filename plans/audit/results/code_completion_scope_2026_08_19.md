...
| 55 failing tests in `config_interface`/`cloud_interface` | GCS config loaders, `TimeSeriesConfigStore` persistence, region-default constants — root cause not yet found; leading suspect (stale `.venv` vs `uv.lock`) unconfirmed | `/plans/active/issues/unified_trading_library_config_interface_mass_test_failure_2026_08_15.md` | S-M | Currently-red test suite in a shared library every service depends on | N |
