...
def summarize(results: dict[str, dict]) -> str:
    ...
    lines.append("")
    lines.append("| Path | Label | Total wall (s) | Mean wall/instr (s) | Mean peak RSS (MB) | "
    ...
    for i, inst in enumerate(files):
        cells = " | ".join(
            f"{results[pid]['cumulative_rss_after_each_mb'][i]:.0f}"
            for pid in PATHS
        )
    ...
    lines.append(f"- **A pure-polars-lazy** vs **C current-mdps-mixed**: "
                 f"{a_total/c_total:.2f}× wall, {a_ret-c_ret:+.0f} MB retention delta")
    lines.append(f"- **B pandas-pyarrow** vs **C current-mdps-mixed**: "
                 f"{b_total/c_total:.2f}× wall, {b_ret-c_ret:+.0f} MB retention delta")
    lines.append(f"- **D polars-eager** vs **C current-mdps-mixed**: "
                 f"{d_total/c_total:.2f}× wall, {d_ret-c_ret:+.0f} MB retention delta")
    lines.append(f"- **A pure-polars-lazy** vs **B pandas-pyarrow**: "
                 f"{a_total/b_total:.2f}× wall, {a_ret-b_ret:+.0f} MB retention delta")
    lines.append("")
...
def main():
    ...
    for pid in PATHS:
        ...
        print(f"  total {results[pid]['total_wall_s']:.1f}s  "
              f"retention {results[pid]['rss_retention_mb']:.0f}MB",
              flush=True)
