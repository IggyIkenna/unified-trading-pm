...
**Note**: Production parquets not available on worker VM. Rerun used 3 synthetic BINANCE-FUTURES
parquets (0.3–1.4 MB) vs original 9 real parquets (2.9–46.8 MB). Numbers scale with data size;
the relative ordering (D < A < B < C by retention) is consistent with the original baseline
at `results.md`. Stage 1 change (`_read_tick_data` drops `.to_pandas()`) targets Path D pattern.
Original baseline: Path C mean_peak=1861 MB, Path D mean_peak=625 MB (66% reduction on real data).

# MDPS engine benchmark — results

Versions (same across all paths):
  polars  1.40.1
  pandas  3.0.3
  pyarrow 24.0.0
  python  3.13.13

Data: 3 BINANCE-FUTURES perp trades parquets for 2026-04-15.
Each path processes ALL N instruments in a single Python process,
running each in its own subprocess to isolate cross-path arena retention.
...
Each row shows the RSS measurement AFTER each instrument completes,
post `gc.collect()`. Rising numbers = cumulative arena retention.
