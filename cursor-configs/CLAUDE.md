...
- **quickmerge lands on LDR**; **default promote is LDR→`main` DIRECT — staging DORMANT** (per-repo
  ...
  restarts ONLY on a restart-RELEVANT HEAD move — `server/`/`config/`/deps; a restart mass-disrupts the fleet, never
  widen that set casually), other services deploy via Cloud Build →
...
slot N — **verify `pwd` under `.tabs/<N>/` before your first write**; a bare `<repo>` path succeeds but is NEVER your
slot (reported not enforced every 5 min — nothing auto-cleans it, 2026-08-21 incident). **Distinct failure mode — two operators/sessions sharing ONE slot's checkout** (interactive sessions have no
...
- **Working on DATA / manifest / pipeline?** 4-state `capture_status`; canonical schema v9 but **trust the actual
  ...
  `plans/epics/security_and_cross_cutting_master.md`. **Honest
  Coverage v2 (two-layer / two-view / instrument-gates-download
