...
at the LIVE boundary, so `live−batch = (paper−batch ≈ 0) + (live−paper = execution alpha)`. **Three fill concepts, two
REALITIES** (corrected 2026-06-19): (1) **benchmark** = the strategy's ASSUMED fill, liquidity-adjusted at the SIGNAL
CANDLE'S CLOSE (`BenchmarkFillEngine`) — the deterministic yardstick, NOT the booked sim fill; (2) **smart matching** =
execution-service's job alone, the semi-realistic fill bounded by market-data fidelity (OHLCV→BBO→depth→trades→L3 MBO),
which "benchmarks around" (1) → `execution alpha = smart − benchmark`; (3) **real venue** = live only. The two realities
are the **simulated path (benchmark + execution-service smart matching, IDENTICAL in batch+paper)** vs **real-venue
(live)** — `execution-service` owns matching, paper (`PaperMatchingEngine`) and batch (Group C) MUST run the SAME
engine; the gap is batch's `GroupBRunner` stops at the benchmark (completing `GroupCRunner` is the linchpin). DeFi yield
