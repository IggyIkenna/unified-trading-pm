...
"undiagnosed" — root-caused: it is the wall-clock timeout firing mid-sweep, not a distinct crash. 14400s (4hr) is a safe
floor for a per-asset_group MVP sweep; a genuinely unscoped full sweep needs much more (many-hour run, see §1a above)
and is why the per-`--asset-group` split remains the recommended default regardless of this flag.
