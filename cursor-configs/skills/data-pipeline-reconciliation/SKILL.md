...
| -------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| **sports**     | **Corrected 2026-07-24 — two buckets, do not conflate.** The RAW-TICK bucket (`market-data-tick-sports-*`, what this skill's default `--asset-group sports` dispatch targets) uses the STANDARD `asset_group=sports`-keyed `raw_tick_data/` grammar and IS fully oracle-covered — run it normally. Only the SEPARATE reference bucket (`instruments-store-sports-*`) has **no `asset_group=` key at all** — tree `sports_reference/by_date/day={D}/pipeline_mode={m}_{s}/entity={E}/league={L}/`, `entity=` never a `data_type`, oracle-exempt (dispatch to `candidate_parquet_paths()` instead). A generic pass that skips the oracle for sports raw-tick because "sports has no oracle coverage" is following the wrong bucket's rule. See `/codex/02-data/four-surface-reconciliation-procedure.md` §4.2. |
