...
## 1.5 Five checks that a 2026-08-10 pass proved § 0-1 does NOT cover

Every one of these was a real finding that this skill's own § 0-1 flow would have missed or actively mis-fixed. Run them
before concluding a classification.

**(i) Count distinct SUBJECTS before you triage volume.** A wall of alerts is almost never N incidents. Reduce first:

```bash
grep -o 'Data-Pipeline Alert\[DP_[A-Z_]*\]' <dump> | sort | uniq -c | sort -rn   # events
grep -A1 'lifecycle-events-subVM:' <dump> | grep -v 'subVM:\|^--' | sort | uniq -c   # distinct VMs
```

Measured 2026-08-10: **108 messages / 50 min → 7 VMs + 6 data_type rows.** A messages:subjects ratio far above ~2:1
means the storm IS the bug — diagnose the repeat mechanism before any individual alert's content.

**(ii) Actuator-RETRY is not detector-REFIRE — and a cooldown is the wrong fix for it.** § 1(b) sends you to
`_RECURRING_ALERT_COOLDOWNS`. That is correct only when a _detector_ re-evaluates a standing condition. When the repeat
comes from an _actuator_ retrying (`DP_VM_PREEMPTED` INFO immediately followed by `DP_VM_PREEMPTED_NO_RELAUNCH`
CRITICAL, same VM, same minute, repeating), a cooldown silences the page while the actuator keeps burning relaunches.
**Check the actuator's own budget/dedup state persistence first.** Recurring root cause:

```bash
# Actuator budget/counter state stored on LOCAL DISK, while the monitor runs as a Cloud Run JOB
# (fresh container per execution) → the budget ALWAYS reads 0, the cap NEVER engages, every sweep retries.
rg -n 'gettempdir|/tmp|Path\(tempfile' deployment-service/scripts/recovery/*.py
gcloud run jobs describe uts-prod-dp-exit-code-monitor --region=asia-northeast1 --format="value(metadata.name)"
```

Found live 2026-08-10: `_default_preemption_budget_dir()` → `tempfile.gettempdir()`, so `_MAX_RELAUNCHES_PER_DAY=2` was
a no-op in production and the alert text's "≤2/day per prefix" promise was false. **Any actuator state that must survive
between Cloud Run Job executions belongs in GCS/Firestore, never local disk** — grep for this class whenever a
budget/cap/dedup claim appears in an alert body.

**(iii) Audit the alert TEXT against the code path for THIS VM family — canned strings lie.** Three separate false
statements shipped in live alert bodies, each actively misdirecting triage:

- `DP_VM_PREEMPTED`: _"relaunching through the Tardis/launcher concurrency guard"_ — but `mdps-*` VMs bind
  `launch-mdps-sharded-backfill.sh`, which contains **zero** Tardis references (MDPS reads MTDS output from GCS; it has
...
A wrong diagnosis hint costs more than no hint. Treat alert-text accuracy as in-scope work for this skill, not
cosmetics.

**(iv) NEVER act destructively on a vintage/scope hypothesis without a second-sample CONTROL test.** Near-miss,
2026-08-10: a `MalformedTickFieldError` appearing only on 2019 Deribit shards read as a 2019-vintage schema problem, and
the proposed remedy was to delete all Deribit 2019 data, mark it out-of-scope in UAC, and block retries. A single
control probe of a 2025 shard refuted it outright — **identical 25-column schema, identical failure**. The real bug was
adapter routing (options_chain content stored at `data_type=trades`, so `CefiTradesAdapter` runs instead of the
already-existing `CefiOptionsChainAdapter`), affecting every vintage. Deleting would have destroyed 6+ years of good
data and left the bug live.

```bash
# Control probe — parquet FOOTER only, no full download, works on multi-GiB objects
python -c "
import pyarrow.fs as fs, pyarrow.parquet as pq
f=pq.ParquetFile(fs.GcsFileSystem().open_input_file('<bucket>/<path>'))
print(f.metadata.num_rows, f.schema_arrow.names)"
```

Rule: **an absence/failure confined to one slice is a hypothesis until a control sample from a different slice is
probed.** This mirrors the codex's existing "an absence result is evidence ONLY once you've confirmed you probed the
vocabulary the WRITER actually emits" caveat — apply it to vintages, venues, and data_types too, and always before
anything irreversible.

**(v) Check the detector's POPULATION, not just its verdict.** `DP_VM_GONE_NO_CAPTURE` fired on `cefi-fwd-daily-cron-*`,
a cron LAUNCHER host whose run.log is a heartbeat log — it captures no rows by construction, so the verdict can only
ever be a false positive. Before accepting any capture-based verdict, confirm the VM is actually a capture VM
(`vm_zombie_watchdog.VM_PREFIX_TO_BUCKET` lifecycle class; live/launcher/cron hosts must be exempt, the same way
`is_live_vm` already exempts `LONG_LIVED_LIVE`).

Related standing gap worth checking whenever an MDPS/SPOT VM reports zero capture: the per-VM manifest shard rewrite is
count+time **debounced** and only guaranteed to drain on `close()`/`atexit`
(`unified_trading_library/manifest_writer/_writer_io.py`). SPOT preemption (SIGTERM → ~30s → SIGKILL) can strand that
remainder, so candles land durably in GCS while the manifest never records them — a real data-accounting loss that
_presents_ as an alerting false positive.
