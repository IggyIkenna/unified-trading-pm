...
- mechanical flags: dangling `depends_on`/`related`/`supersedes` refs; `superseded_by` set while `status: active`;
  ...
  candidate); **body over the line-cap** (500 soft / 1000 hard, per `run_hygiene_sweep.sh`); `assigned_vm` outside
...
splitting a plan over the hard line-cap; anything touching a `locked_by:` plan, kill-switch, funds isolation, or the
May-23 critical path. Interactive format — batched questions ordered P0→P1 hitting the chat box directly; recurring
classes get ONE class-level question with per-item exceptions; every question carries: the two quotes + locations, why
they conflict, which doc looks authoritative and why, and options A/B/C with the recommendation marked (never open-ended
...
- **Leave the corpus hygiene-canonical (green-gate — HARD).** After applying fixes: prettier every touched `.md`;
  ...
  **0 hard failures** before finishing (frontmatter / todo-format / todo-regression / runbook-fields all green). A plan
  you touched must never be left non-prettier, hygiene-red, or with malformed todos. Line-caps: over the HARD cap (1000)
  is a split finding (operator-gated — splitting a plan is a planning decision); over the SOFT cap (500) is a note in
  the Phase-6 report. Canonical format is the contract, same as a `quality-gates.sh`-green tree for code.
