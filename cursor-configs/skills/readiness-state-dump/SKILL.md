...
| **strategy_archetype_code**               | the archetype-code-completeness skill's own `--json` output, intersected with `satisfying_archetypes()` for this venue                                                                                                           | Fact — both sides are declared SSOTs. W1's archetype capability axis across batch/paper/live (added 2026-08-20); a venue whose only data-satisfiable archetype is not wired for a mode is not ready in that mode |
