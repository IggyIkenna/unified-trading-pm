...
def _matches(cell: ShardCell, asset_groups: set[str] | None, venues: set[str] | None) -> bool:
    if asset_groups and cell.asset_group not in asset_groups:
        return False
    return not (venues and cell.venue not in venues)
...
def _print_human(report: dict, gcs_path: str, resolved_date: str, verbose: bool) -> None:
    ...
    is_3tuple = report["grain"] == "instrument_type"
    ...
    print(f"  {'reachable coverage':<22} {pct_str}")
    print(f"  (denominator = captured + attempted_failed + expected_unattempted = {denom})")
    print()
    ...
    for ag, strays in report[f"{NOT_EXPECTED_LABEL}_tuples_by_asset_group"].items():
        ...
        print(f"=== {ag}: not-expected (stray) = {len(strays)}, Layer-1 holes (missing_tuples) = {len(holes)} ===")
        if verbose:
            for t2 in strays[:20]:
    ...
    if verbose:
        ...
        for row in report["shards"]:
            ...
            denom = row["reachable_denominator"]
            print(f"  {row['shard']:<60} reachable={pct_str:<10} denom={denom:<6} [{cs_str}]")
    else:
...
def main() -> int:
    ...
    try:
        ...
        print("This dump reports honestly when it cannot read its source; it never fabricates data.", file=sys.stderr)
        return 1
    ...
    if args.json:
        ...
        to_dict = lambda o: asdict(o) if hasattr(o, "__dataclass_fields__") else str(o)  # noqa: E731
        json.dump(report, sys.stdout, indent=2, default=to_dict)
        print()
    else:
