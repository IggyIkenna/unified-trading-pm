...
- **Multi-repo workspace** (NOT a monorepo) on `live-defi-rollout`; edit only your task's named repo. **Per-slot
  ...
  operator's live checkout, not yours; `realpath` your target before the first write. If YOUR prompt never named an
  absolute `.tabs/<N>/` path, STOP and ask — never default to the bare repo root.** 2026-08-21 incident: a wrong-CWD
  write nested one service's files in another's bare checkout, undetected 20h. Stay current `git pull --ff-only
origin live-defi-rollout`; invariant = HEAD
