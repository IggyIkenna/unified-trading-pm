...
1. Pull LDR in ALL your repos: `cd ${WORKSPACE_ROOT}/.tabs/11/<repo> && git fetch origin --quiet && git rebase origin/live-defi-rollout`
2. Re-read `harsh_orchestrator/AGENT_ONBOARDING.md` (updated boot context)
3. Read `plans/active/agent_orchestrator_slack_notifications_2026_05_19.md + work_split_2026_05_19_harsh.md § Slot 11` — this is your slot's work for today
...
**Today's assignment — Slot 11**:
agent_orchestrator_slack_notifications: P1 first (server/notifications/slack.py + unit tests — no Cloud Run needed). Then P2 (event hook wiring). P0 (--update-secrets) waits for Slot 10 P1. Then P3 staging smoke (real message to #agent-orchestrator-alerts). No human gates. (~2 cal)
