...
| 3 | 🟡 **Wave 2** — 117 UTL test-fixture sweep (pipeline_mode kwarg) — OOM-PAUSED | 🟡 OOM-PAUSED — 24 dirty UTL test files preserved in `.tabs/3/unified-trading-library`. Respawn: "check `git status`, sweep in progress, continue from there" | UTL@`547ff3c` API drift + writegate plan Phase 4 | `tab/hk/3` |
| 4 | 🔁 **Continuation** — writegate Phase 6.8 instruments-service (41 `.add()` → `record_captured()`, P0) — OOM-PAUSED | 🟡 OOM-PAUSED — 28 dirty instruments-service files preserved in `.tabs/4/instruments-service`. Respawn: "check `git status`, .add() migration in progress, continue" | `writegate_honest_coverage_endtoend_2026_05_06.md` Phase 6.8 | `tab/hk/4` |
...
| 6 | ✅ **Phase 6.5 P2 DONE** (101fc6dd) — 4 P2 items shipped. 🔁 **Wave 2** — instruments-service enrichment preflight + zero-fixture bypass bugs | ✅ DONE Phase 6.5 P2 continuation (features-service@0de7fee6 + UAC@07b4992 + UAC@466d93c + PM@101fc6dd). 🟡 OOM-PAUSED before Wave 2 STARTED — 3 dirty instruments-service files preserved. Respawn for Wave 2. | `writegate_honest_coverage_endtoend_2026_05_06.md` Phase 6.5 ✅ → instruments-service Wave 2 | `tab/hk/6` |
| 7 | ✅ **Phase 2F DONE** + UAC ICE softs pushed by main (2fb27f8). 🔁 **Wave 2 remaining** — deployment-ui honest-coverage 404 graceful message | ✅ UAC@`2fb27f8` pushed to LDR by main-orchestrator (OOM recovery). 🟡 OOM-PAUSED — 1 dirty UAC test file + 2 dirty PM files. Respawn for deployment-ui honest-coverage 404 item. | `data_status_ui_phase_2f.md` ✅ → `issues/honest_coverage_cron_vm_scheduling_2026_05_14.md` | `tab/hk/7` |
