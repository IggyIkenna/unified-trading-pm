...
| 9 | Wave 3 ✅ DONE 14:45 UTC — sports classifier ext shipped UTL@3928e3a (11 new tests → 52 total all pass: 4×PAUSED_LEAGUE + 3×PRE_SEASON + 3×POST_SEASON + 1×SOURCE_DOES_NOT_COVER); Script 3 DRY-RUN sports = 0 upgrades out of 1.87M candidates (KNOWN_COVERAGE_GAPS={} → PAUSED_LEAGUE never fires from manifest; venue/field narrows others). Issue doc updated (PM@d5bb92fd). **Side-finding (deferred)**: 117 pre-existing UTL test failures (`ManifestWriter.record_empty()` missing `pipeline_mode` kwarg from Harsh-side UTL@547ff3c API drift — unassigned, operator-pending). | ✅ DONE Wave 3 — standing by | `issues/sports_classifier_extension_followup_2026_05_13.md` | `tab/hk/9` |
