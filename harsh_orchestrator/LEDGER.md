...
#### Slot 2 — `harsh-features-consolidation-tab` 🟢 IN FLIGHT — `features_service_qg_cleanup_2026_05_11.md` Phase 1.2 (session 3, fan-out): STARTED 2026-05-11 11:05 UTC; all 4 QG-check FPs resolved (skip nothing); re-ran QG → 16 real violation categories; 3 background sub-agents A/B/C per-FAMILY; **A + C LANDED** (16→9 viols; commits in worktree, NOT pushed — slot 2 pushes after B + verify) — A=sports (gcs_reader 1306L→4 modules, google.cloud→UCI, canonical.*→sports facade, splits+~10 decomps, 11 commits), C=delta_one/cross_instrument/calendar/commodity/multi_timeframe/api/scripts (8 commits, zero deferrals). **Sub-agent B (onchain+volatility) still running.** Phase 1.2e (row e ~38 schema-prov) — group-1→UAC.internal proceeds; group-2 **Q3 ✅ RESOLVED** (~12 stay local `# CORRECT-LOCAL` + 3 `Dependency*` classes in 2 files dedup-import from UTL **root** + collapse `FeatureProcessingResult` double-def + rename delta_one `PubSubMessage`→`DeltaOnePipelineMessage`). **Q4 + Q5 ✅ FIXED by slot 1** (PM `base-service.sh` @this-cycle: `DI=` whitelists `.{domain}` facades / STEP 5.10 excludes `scripts/`). Phase 1.2e runs after A/B/C all land. New Phase 1.5 todo (lift `_get_workspace_root` ×8 → UTL, P2). Phase 1.1 ✅; Q1/Q1.3/Q2 all ✅
