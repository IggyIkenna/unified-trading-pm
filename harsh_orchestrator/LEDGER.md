...
#### Slot 2 — `harsh-features-consolidation-tab` ⚪ QUIET (session budget) — on `features_service_qg_cleanup_2026_05_11.md`: Phase 1.1 ✅ (enumeration); new Q1 ✅ ANSWERED (3 QG-check FPs → skip those rows, fixes routed to Ikenna); Phase 1.2 = the real-violation work, next session
...
#### Slot 4 — `harsh-bucket-and-adapter-tab` ⚪ QUIET but FULLY ACTIONABLE — Q4 ✅ RESOLVED = option (b) (provision env-tiered buckets + migrate data — Ikenna 2026-05-11); BOTH halves unblocked (L2 config.py migration + sports-adapter stamping); `[main → slot 4]` pings sent — needs operator nudge to pull + resume
...
- **Slot 2** `harsh-features-consolidation-tab` — ⚪ QUIET (session budget); on `features_service_qg_cleanup`: Phase 1.1 ✅; Q1 ✅ answered (3 QG-check FPs); Phase 1.2 = real-violation work, next session.
...
| 4 | `bucket_name_ssot_canonicalisation_2026_05_10.md` Q4 (P0) | yaml `${DEPLOYMENT_ENV}` env-tier on `features-*` vs flat provisioned buckets — naive resolver migration → first-write failures. | ✅ RESOLVED 2026-05-11 — operator/Ikenna picked option (b) "make reality match the yaml" (provision env-tiered buckets + migrate data; high-risk). yaml stays env-tiered; L2 config.py migration UNBLOCKED (do now, code-first); physical provisioning+migration = code_freeze Phase 2.6. `[main → slot 4]` ping sent. | — (slot 4 picks up: yaml-correctness fixes + ml-* probe + L2 config.py migration + sports stamping) |
