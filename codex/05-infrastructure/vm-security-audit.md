...
All secret access routes through `gcloud secrets versions access latest --secret=<name> --project=<id>` (Secret Manager) or VM metadata `--secret=<name>` binding. No plaintext API keys, tokens, or passwords in any launcher.
...
- File downloads: `curl -LsSf https://astral.sh/uv/install.sh | sh` (inside startup scripts embedded as heredocs — these run inside the VM, not on the host)
...
**Root cause**: Unquoted command substitutions `$( $FORCE && echo "--force" )` and `$( $DRY_RUN && echo "--dry-run" )` in a flag-aggregation loop. If output contained spaces, word splitting would produce unexpected argument boundaries.
...
**SC2211** (2 instances) — shellcheck misinterprets backtick-quoted glob patterns in shell comments as actual glob commands.
