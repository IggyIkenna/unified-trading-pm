...
- Alerts: Slack webhook (`/uts/alerts/slack-webhook-url`) + Telegram bot
  (`/uts/alerts/telegram-bot-token`, `/uts/alerts/telegram-chat-id`); SSM SecureString at runtime.
- Alert format: `🔄 Auto-rebooted <id> at <ts> — impaired for ≥15 min (StatusCheckFailed_Instance 3/3 datapoints).
  Reboot N/3 in current 24h window.`
...
account to read usage stats. On a host with 4 accounts that's 4 sequential pexpect spawns per tick (~13 s each).
The direct child was SIGTERM'd in a `finally` block, but Node.js grandchildren could orphan.
...
**No claude subprocess is spawned during a poll cycle.** `subprocess.Popen`, `subprocess.run`, and `pexpect.spawn`
are all absent from the new poll path — explicitly asserted in
