...
documented in [`firebase-split-topology.md`](firebase-split-topology.md). **There is no in-UI environment toggle.**
The env is resolved from the domain at boot and never changes at runtime.
...
`deployment-api` reads env tier from `CLOUD_DEPLOYMENT_ENV` (via `DeploymentApiConfig.deployment_env`) at boot.
All 4 Monitor endpoint responses carry an `env` field populated from `settings.DEPLOYMENT_ENV` so the UI can
badge the tier. Cloud Run injects the var; local dev sets it in `.env`.
...
`deployment-ui` resolves the tier client-side from the hostname at page load (Phase h3, same helper as trading-system-UI):
