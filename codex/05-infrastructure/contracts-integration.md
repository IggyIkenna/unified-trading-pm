...
Complete guide for using `unified-api-contracts` (external API schemas + VCR mocks) and its internal subpackage
`unified_api_contracts.internal` (internal service contracts) in services.
...
performed in the **active service repos** (market-tick-data-service/market_tick_data_service/market_interface,
execution-service, instruments-service, position-balance-monitor-service, unified-cloud-interface) in their integration
tests — not in AC. NOTE: unified-trade-execution-interface, unified-sports-execution-interface,
...
| **unified_api_contracts.internal** (subpkg) | Internal service contracts                         | Features, ML, Events, Messaging    |
...
No duplicate contract definitions in UEI.

---

## Phase 4: unified_api_contracts.internal Setup

### 4.1 Understanding unified_api_contracts.internal Structure
...
│   └── ...                     # canonical/, external/, registry/, etc.
└── tests/
    └── internal/
...
```python
from unified_api_contracts.internal.features import FeatureRequest, FeatureResponse
...
```python
from unified_api_contracts.internal.ml import InferenceRequest, InferenceResponse
...
```python
from unified_api_contracts.internal.events import ServiceEvent
...
```python
from unified_api_contracts.internal.messaging import (
    PubSubMessage,
    MessageMetadata,
)
...
```python
from unified_api_contracts.internal.messaging import PubSubMessage
...
import pytest
from unified_api_contracts.databento.schemas import DatabentoOhlcvBar
from unified_api_contracts.internal.features import FeatureRequest
...
[project]
dependencies = [
    "unified-api-contracts>=1.0.0,<2.0.0",  # Allow minor updates, block major; includes internal subpackage
]
```
