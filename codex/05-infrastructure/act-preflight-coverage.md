...
> `auto-merge-minor-fixes.yml`, `claude-api-health-monitor.yml`, `contract-drift-record.yml`,
> `downstream-fix-agent.yml`, `infra-quality-gates.yml`, `major-bump-approval.yml`, `notify-telegram.yml` (superseded by
> `notify-slack.yml`), `persist-cicd-event.yml` (now the `persist-event` composite action), `plan_health-agent.yml`
> (correct name is `plan-health-agent.yml`), `schema-changed-handler.yml`, `sit-starvation-detector.yml` (the live
> scheduled monitor is `glue-pool-starvation-monitor.yml`). All were listed here until the 2026-07-31 re-review; none
> exist in any repo.
