...
| `canonical-migration-defi-pi-range`, `canonical-migration-defi-rebuild`                                         | single-invocation categories (`defi-pi-range`, `defi-rebuild`) in `launch-canonical-migration-vm.sh`                                                      | ⚠️ NOT chunked at the shell level — no per-date frontier to checkpoint; a preempted run still replays `RESUME_START_DATE` verbatim. Lower risk than the chunked families (single VM per quarter/whole-range, not day-granular) but still an open gap                       |
