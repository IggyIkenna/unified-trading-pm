...
Replace `<package_dir>` with the Python package directory name (underscore form, e.g. `unified_trading_library.events`).
Set `MIN_COVERAGE` to `(actual_coverage - 1%)` after the first successful test run.
