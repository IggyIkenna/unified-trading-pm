...
2. **List-bucket latency**: `gsutil ls` on directories with 4k+ entries takes 4+ seconds. `vm_zombie_watchdog.py`
   walks `vm-logs/` paths; bounded directory size keeps watchdog response fast.
3. **Audit trail clarity**: snapshots / event logs older than the documented retention window have no operational
   value; deletion is itself a form of "we know what we keep and why."
...
- **Why 14 days**: covers the typical debugging window after a VM run. Beyond 14 days, logs are forensic-only and
  GCS bucket access cost > grep'ing the log.
...
- **Why 30 days**: the `deployment-api /api/repos/deploy-ready` endpoint reads only the latest snapshot per repo.
  30-day history covers ad-hoc trend queries; deeper analysis can be repointed to a BigQuery sink (post-cutover).
- **Apply / re-apply**:
  ```bash
...
- **Why 90 days**: covers cross-quarter ops debugging + post-mortem windows. Service-event JSONL is
  high-volume (every VM emits STARTED + PROGRESS + STOPPED + FAILED).
...
- **`gs://market-data-tick-*` / `gs://instruments-store-*`** — manifest snapshot retention is governed by the
  manifest consolidator's `_index/snapshots/` retention spec, not generic lifecycle.
...
- Issue doc: [`plans/active/issues/deployment_events_lifecycle_audit_2026_05_15.md`](../../plans/active/issues/deployment_events_lifecycle_audit_2026_05_15.md)
