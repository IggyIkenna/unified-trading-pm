...
On Linux with pyenv, add these to `~/.bashrc` **before** running the bootstrap. Without them `uv` auto-downloads its own
CPython into `~/.local/share/uv/python/`; all 50+ venvs point to that cache, and when the cache is cleaned every venv
breaks simultaneously. The pyenv-virtualenv hook also silently wipes `VIRTUAL_ENV` on every prompt if the global pyenv
version does not exist on disk.
...
have no `pyproject.toml` editable deps. The alignment scanner silently skips plain-string manifest deps → always reports
`"aligned": true`. No alignment action needed.
...
`setup.sh` + QG stubs from PM before running setup.

After: commit and push any changed `pyproject.toml`, `uv.lock`, `workspace-manifest.json` so agents and CI get identical
deps.
...
After Phase 2, refresh `.venv-workspace` so editable installs reflect updated dep versions. Automatic when using `--fix`
— `run-version-alignment.sh --fix` calls `sync-workspace-venv.sh` at the end. For manual refresh:
...
all repos from `workspace-manifest.json` as editable in topological order. Does NOT rebuild per-repo `.venv` (that is
Phase 2).
...
connect to local emulators use `@pytest.mark.allow_network`. Each opt-out emits a WARNING in CI logs — the count should
stay minimal and stable.
...
After running: commit the changed `.gitignore` files and any `git rm --cached` removals in each affected repo, then push
via quickmerge.
...
Validation runs before `git push` (pre-push hook) and inside `quality-gates.sh --no-fix` in CI. Any YAML syntax error
blocks the PR from passing `quality-gates-v2`.
...
**Layer 1 — Pre-commit (`check-branch-drift.sh`):** Blocks commit if local branch is behind origin. Instant. Override:
`SKIP_BRANCH_DRIFT=1` (human-only).
...
**Layer 3 — Admin force-sync:** Reads all repos' versions from remote PM manifest. Blocks if remote has bumps your local
manifest doesn't have. Override: `--force-version-override` (human-only).

**Quickmerge stage 1.6 — Dependency version canary:** Warning only (does not block). Alerts if dependencies have been
bumped on LDR/main since your last pull.
