...
- **GMX multi-chain**: currently `chain=""`; per-chain Tier-2 fan-out per writegate Phase 2.B.
...
| `02-data/per-category-bucket-layouts.md`           | Uses `category=` legacy hive vocab. `asset_group=` is canonical; `category=` is legacy-preserved.                                                                                                     |
...
| `features-sports-service/docs/ARCHITECTURE.md`                                                  | Missing 4 stub-export wiring + `_FETCH_COMPLETED_AT` cache + per-table `available_at` semantics                 |
| `features-sports-service/docs/SCHEMA_VALIDATION.md`                                             | Missing `available_at` column required + per-row stamping rules                                                 |
