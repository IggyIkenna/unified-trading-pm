...
if [ "$SKIP_CI" = true ]; then
  ...
  # PM is not a deployed package and is the SIT debouncer (not itself SIT-covered), so a staging
  # hop adds no SIT value. The main PR's quality-gates-v2 is the full per-repo gate. BOTH docs/plans
  # AND scripts/workflows target main. Downstream repos still get PM via the dep-clone fallback
  # (clone -b staging → -b main). SSOT: codex/08-workflows/ci-cd-flow.md.
  PR_BASE="main"
