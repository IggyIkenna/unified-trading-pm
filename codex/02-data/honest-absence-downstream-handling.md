...
| `empty_confirmed`  | `EXPECTED_OUTSIDE_TRANSFER_WINDOW`                | Day falls outside the transfer window for this league. Two use-cases: (1) **Sports player-transfer window closed** — football leagues have Jan + Jul windows; `transfer_records` AND Transfermarkt PLAYER_VALUES market-value data (`total_market_value_eur`/`market_value_eur`) are both expected empty outside these windows (CORRECTED 2026-08-12, operator ruling — see `is_transfer_window_open()` in UAC `canonical/domain/sports/transfer_windows.py` and plans/active/issues/transfermarkt_player_values_data_discarded_2026_08_07.md). (2) **DeFi transfer-event lookback** outside the operator-configured bounds (staking / bridging refdata).                                                                                                                                                                                                                                                                                                                                                                                                                                            | NO                                                             |
