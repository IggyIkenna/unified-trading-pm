...
| `empty_confirmed`  | `EXPECTED_OUTSIDE_TRANSFER_WINDOW`                | Day falls outside the transfer window for this league. Two use-cases: (1) **Sports player-transfer window closed** — football leagues have Jan + Jul windows; `transfer_records` AND Transfermarkt PLAYER_VALUES market-value data (`total_market_value_eur`/`market_value_eur`) are both expected empty outside these windows (CORRECTED 2026-08-12, operator ruling — see `is_transfer_window_open()` in UAC `canonical/domain/sports/transfer_windows.py` and plans/archive/2026_08/issues/transfermarkt_player_values_data_discarded_2026_08_07.md). (2) **DeFi transfer-event lookback** outside the operator-configured bounds (staking / bridging refdata).                                                                                                                                                                                                                                                                                                                                                                                                                                   | NO                                                             |
...
| `empty_confirmed`  | `STRATEGY_ENGINE_RETURNED_ZERO`                   | Sibling of `SOURCE_RETURNED_ZERO` for an internally-computed decision instead of an external fetch — a strategy engine ran its own signal-generation compute (the engine itself IS the source, no upstream fetch involved) and legitimately produced zero `strategy_instructions` for a `(client_id, strategy_id, day)` (a hold-day decision). `FetchEvidence`'s proof gate is HTTP-fetch-shaped and does not apply (only required when `reason == SOURCE_RETURNED_ZERO` literally). WITHIN the honest-coverage window — the strategy DID run and DID consider the day. Used by `strategy-service`'s "Always write instructions — even empty. This is the heartbeat from strategy to execution" design. Added UAC (`_honest_coverage_empty_reasons.py`) per `ml_strategy_manifest_coverage_gap_2026_08_03.md` todo 2 (operator ruling 2026-08-03).                                                                                                                                                                                                                                                   | NO                                                             |
...
| `empty_confirmed`  | `EXPECTED_REFERENCE_ONLY_NO_CAPTURE_PATH`         | DeFi reference-only holdings row (`instrument_type in {SPOT_ASSET, A_TOKEN, DEBT_TOKEN}`) — a derived sibling of a parent POOL/LENDING row's on-chain token leg with NO independent per-day market-data capture path, by construction, not a pipeline gap. Unlike `EXPECTED_ACQUISITION_PENDING` (wiring pending, will eventually resolve), this cell can NEVER resolve to `captured`. **WITHIN the honest-coverage window** — deliberately NOT a member of `OUT_OF_COVERAGE_WINDOW_REASONS` (unlike `EXPECTED_NOT_ENOUGH_TVL`/`EXPECTED_NO_PROVIDER_COVERAGE`/`EXPECTED_INSTRUMENT_DELISTED`); re-stamping with an out-of-window reason would silently re-exclude live instruments from the denominator. Seeded at enumerator time by `_enumerate_v2_defi` (`instruments-service@a516bd01`); retroactively backfilled for the pre-existing 215,864-cell backlog by `reclassify_defi_reference_only_eu_2026_07_21.py`. Operator decision (AskUserQuestion, 2026-07-20/21): "New in-denominator reason (Option B)". Issue: `defi_nonpool_per_instrument_eu_has_no_reconciliation_path_2026_07_20.md`. | NO                                                             |
| `empty_confirmed`  | `EXPECTED_SUBGRAPH_DEINDEXED`                     | DeFi: the venue's Graph Protocol subgraph has ZERO indexer allocations on The Graph's decentralized network (HTTP 200 + top-level `errors[]`, e.g. "subgraph not found: no allocations") — distinct from a 404 (`_SubgraphNotFoundError`) and from genuine schema drift (fixable). Permanent until a new indexer re-allocates or the code stops depending on the subgraph. Reference case: CURVE/OPTIMISM `dex_pool_swaps` (952 `attempted_failed` rows root-caused 2026-07-15, retroactively reclassified by `reclassify_defi_curve_optimism_subgraph_deindexed_2026_07_24.py`). **OUT-of-coverage-window** — not currently coverable by any retry. Issue: `defi_curve_optimism_subgraph_no_allocations_2026_07_15.md`.                                                                                                                                                                                                                                                                                                                                                                             | NO                                                             |
| `empty_confirmed`  | `EXPECTED_SUBGRAPH_STALLED_HEAD`                  | DeFi: the venue's Graph Protocol subgraph HAS indexer allocations and serves queries, but `_meta.block.timestamp` is frozen >7 days behind chain tip — a 0-row response for dates past the frozen head is a MISLEADING honest-zero (the subgraph never actually failed the query). Distinct from `EXPECTED_SUBGRAPH_DEINDEXED` (zero allocations — no query can succeed at all): this subgraph served historical data correctly up to its frozen head. Probed on the (rare) zero-rows path via a `_meta` query, bounded to that case. Reference case: PANCAKESWAP_V3/BSC `dex_pool_swaps` (subgraph=Hv1GncLY..., head frozen ~2026-04-28, >99 days behind at detection). **OUT-of-coverage-window past the frozen head** — not currently coverable by any retry unless a new indexer catches the head back up. Shipped `unified-api-contracts@2d74b345` (taxonomy) + `market-tick-data-service@5d633923` (runtime detection). Issue: `defi_dex_pool_swaps_733_row_indexer_health_findings_2026_07_27.md` (archived).                                                                                 | NO                                                             |
...
| `defi`       | `dex_pool_swaps`  | `EXPECTED_SUBGRAPH_DEINDEXED`; `EXPECTED_SUBGRAPH_STALLED_HEAD`; `EXPECTED_NOT_ENOUGH_TVL` | Feature same-day single-sample — emit `record_empty`       |
...
| **Lifecycle — permanently gone**    | `EXPECTED_INSTRUMENT_DELISTED` · `EXPECTED_PAST_SOURCE_COVERAGE_END` · `EXPECTED_DEPRECATED_DATA_TYPE` · `EXPECTED_POST_SEASON` · `EXPECTED_SUBGRAPH_DEINDEXED`                                       | Exclude from window span — data ceased at a known date (or, for `EXPECTED_SUBGRAPH_DEINDEXED`, at a known verification time). Do not extend window to compensate.                                      | NaN-fill                                                                                                                             | Skip silently — no alert. Instrument or source retired after that date.                                                                                                                                                                                                                                                                            |
| **Temporary gap / pause**           | `EXPECTED_PAUSED_LEAGUE` · `EXPECTED_REFDATA_CADENCE_CHANGE` · `EXPECTED_OUT_OF_COVERAGE_WINDOW` · `EXPECTED_PROTOCOL_PAUSED` · `EXPECTED_OUTSIDE_TRANSFER_WINDOW` · `EXPECTED_SUBGRAPH_STALLED_HEAD` | Date falls inside the calendar window span; denominator = n_valid (exclude the paused day from the numerator and denominator). Gap is expected to resolve in the future.                               | NaN-fill                                                                                                                             | Skip silently — no alert. Pause is documented and expected.                                                                                                                                                                                                                                                                                        |
| **Source-specific / no coverage**   | `EXPECTED_SOURCE_DOES_NOT_COVER_LEAGUE` · `EXPECTED_KNOWN_SOURCE_GAP` · `SOURCE_RETURNED_ZERO` · `STRATEGY_ENGINE_RETURNED_ZERO`                                                                      | Include date in window span; denominator = n_valid. `SOURCE_RETURNED_ZERO`/`STRATEGY_ENGINE_RETURNED_ZERO` warrant an optional per-service soft monitoring counter for anomaly detection — not a page. | NaN-fill                                                                                                                             | Skip. Optional soft alert on `SOURCE_RETURNED_ZERO`/`STRATEGY_ENGINE_RETURNED_ZERO` (configurable per service; not default). `strategy_instructions` specifically: execution-service already reads the 0-row parquet content directly (not gated by manifest reason) — this reason is for coverage/observability, not a new execution-path branch. |
...
| any source     | Transfer-window gate for `data_type=transfer_records` or `PLAYER_VALUES` | `EXPECTED_OUTSIDE_TRANSFER_WINDOW` if `is_transfer_window_open(league_id, day)=False` |
...
| `derivative_ticker`                                                                                                             | **REVERSED 2026-07-20 (operator ruling, `cefi/derivative_adapter.py` module docstring) — supersedes this row.** NO carry-forward: an empty window stays NaN price-like fields + `volume=0`/`trade_count=0`, honest per-bin absence (see `cefi_satellite_ao_dispatch_batch1-002`'s 2026-07-26 audit finding). LOCF here would conflate "window had nothing to aggregate" with "not yet fetched" — the adapter is deliberately NOT routed through `_finalize_session_grid` (`supports_prior_day_seed=False`). SSOT: the adapter's own module docstring + `/codex/06-coding-standards/adapter-finalization-contract.md`. | Interval close time                                 |
