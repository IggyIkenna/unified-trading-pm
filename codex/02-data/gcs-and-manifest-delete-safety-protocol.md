...
| `gcs_conditional_delete(uri, *, if_generation_match)` | `-> bool` (`True`=deleted, `False`=precondition failed or already gone) | **Prefer this over `gcs_delete_object` whenever verify and delete are two separate steps** (2026-07-27 K1/K2 delete script) — pass the `generation` from the SAME `gcs_describe_object` call that content-verified the object, closing the verify-then-delete race window (something else could replace the object between your verify and your delete). `False` means someone touched it since — treat as `failed`/refuse, never silently retry-and-delete-anyway. |
