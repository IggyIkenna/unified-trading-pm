...
instruments-service, UPI→position-balance-monitor-service) unified-cloud-interface) in their integration tests; those
interfaces hold API keys. Cassettes may be recorded and validated from interface repos. Consumers reference cassettes by
path — they do not duplicate them (HTTP only; WebSocket capture requires different tooling).
