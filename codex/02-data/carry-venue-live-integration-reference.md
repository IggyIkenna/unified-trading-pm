...
| **Aster**       | ✅ all       | `GET fapi/v1/premiumIndex`                                                                           | `lastFundingRate`             | 8h                     | `{BASE}USDT`       | Binance-compatible API; GCS backfill confirmed DONE (2026-08-03 verification, see `plans/archive/2026_08/issues/perp_funding_data_semantics_and_cadence_2026_06_16.md`).                                                                                                                              |
...
(Aster/Deribit 8× wrong) — filed `plans/archive/2026_08/issues/perp_funding_data_semantics_and_cadence_2026_06_16.md`;
that dict was **DELETED from UTL 2026-06-17** (`unified-trading-library@b587b91b`/`ed622af8`, per that doc's todo 1)
once consumers were repointed to the UAC SSOT, so there is no longer a second, disagreeing registry to reconcile
against. There is **no historical cadence tracker** (a venue changing its interval over time is invisible) — also filed
there. For new venues, prefer the **interval the API returns** over a static assumption.
