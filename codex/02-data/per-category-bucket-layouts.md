...
> BEFORE making code or doc changes informed by this doc. This doc is partially stale: uses `category=` legacy hive
> vocab; `asset_group=` is canonical for new writes (`category=` preserved on disk per CLAUDE.md asset-group section).
> The post-plan-reality doc lists the 10 cross-cutting principles codified in workspace `CLAUDE.md` (live=batch, no
> double SSOT, three-category empty-output decision, cluster validation mandatory, per-row write-time `available_at`,
