...
| **PREDICTION (post-Plan A target)**          | `raw_tick_data/by_date/day={date}/asset_group=prediction/venue={v}/data_type=prediction_canonical_question_group/canonical_question_group={cqg}/{market_id}.parquet`                                                                                                                                          | **NEW shape per predictions Plan A**: shard atom `(asset_group=prediction, venue, data_type=prediction_canonical_question_group, canonical_question_group, market_id, day)`. Bundled by canonical_question_group (e.g. BTC_UP_DOWN_HOURLY → 24/day market_ids; ELECTION_PRESIDENT_2028 → 1 over months). Per-market lifecycle bounds enforced. Plan A reconciler script splits legacy per-base_asset parquets into per-canonical-group parquets.                                                                                                                                                                     |
...
| **SPORTS (post-writegate Phase 2.B target)** | `raw_tick_data/by_date/day={date}/asset_group=sports/source={source}/data_type={dt}/league={league}/fixture={fixture_id}/{F}.parquet` (per-fixture data_types) OR `raw_tick_data/by_date/day={date}/asset_group=sports/source={source}/data_type={dt}/league={league}/{F}.parquet` (day-aggregate data_types) | **NEW shape per writegate Phase 2.B**: per-fixture data*types (`ODDS_SNAPSHOT`, `ODDS_MOVEMENT`, `ARBITRAGE`, `FIXTURE_STATS`, `FIXTURE_EVENTS`, `FIXTURE_LINEUPS`, `FIXTURE_PLAYER_STATS`, `INJURIES` when fixture-scoped) shard at `fixture_id`. Day-aggregate data_types (`STANDINGS`, `LEAGUES`, `TEAMS`, `REFEREES`, `COACHES`, `ROUNDS`) shard at day. League stays as a higher-level rollup grouping for data-status panel filtering, NOT the shard atom. Per-fixture cluster validation MANDATORY for ODDS*\*: `cluster_extractor=lambda row: row["bookmaker"]` + `SPORTS_FIXTURE_CLUSTERS` per league-tier. |
