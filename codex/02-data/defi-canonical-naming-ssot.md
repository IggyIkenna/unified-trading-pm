...
| **instrument_type**                                                      | `pool` (dex), `a_token`/`debt_token` (lending — replaces the RETIRED flat `lending`), `lst`, `spot_asset` (oracle), **`perpetual`** (DeFi DEX-pool perp: GMX) + Solana `solana_amm_pool`/`solana_vault` + Solana lending synthesized `a_token`/`debt_token` | **Lending uses the A_TOKEN/DEBT_TOKEN split** — the legacy flat `lending` is RETIRED, `a_token`/`debt_token` is THE SSOT (Aave/Spark/Compound emit real `a_token`/`debt_token` = aUSDC/variableDebtUSDC; isolated-market Morpho/Euler/Fluid/Radiant/Venus/Benqi synthesize `A{coll}-{loan}[-marketId8]`; Solana lending synthesizes the same convention — no separate `solana_lending`). **DeFi DOES have perps** (operator 2026-06-01): the DEX-pool perp GMX is `instrument_type=perpetual` for `asset_group=defi` (on-chain CLOB perps such as Hyperliquid are cefi — see the section below). IS `DEFI_ONCHAIN_INSTRUMENT_TYPES` MUST include `PERPETUAL`.                                                                                                                     |
...
manifest shape (`venue, instrument_id, data_type, day`), and rides the **cefi backfill**, not the defi path. The only
**DeFi** perp is `GMX` (ARBITRUM/AVALANCHE) — DEX-pool-shaped, in `DEFI_PERP_VENUES` (`DRIFT` + `PACIFICA` are CULLED —
purged from the registry per the 2026-07-16 taxonomy ruling). **2026-06-25 alignment:** the instruments-service capture
path (`engine/orchestrator/defi.py` `_SOLANA_DEFI_VENUES`/`_L2_DEX_PERP_VENUES`) had wrongly enumerated
...
for `lending_indices` (EVM `a_token`/`debt_token` + Solana synthesized `a_token`/`debt_token` instrument_types under one
data_type — the legacy flat `lending`/`solana_lending` are RETIRED).
