...
For **flat-per-contract** shards the filename stem == the `instrument_id` column == the manifest key (byte-identical).
For **bundles** and **defi capture-batches** the manifest key is a coarser grain and the per-instrument id lives only in
the column — see §1.
...
| ----- | --------------------------------- | ----------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------ | ---------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------- |
| **1** | flat-per-contract                 | cefi spot/perp/dated-future, tradfi equity, kalshi raw trades                                         | **`instrument_id`**                                                                                                            | one parquet per instrument (stem == id)                                                                                | non-null, == manifest key                                                                                                                    |
...
| **4** | defi capture-batch (column-level) | all defi                                                                                              | manifest keyed on the shard atom `(…venue·chain·instrument_type·data_type…)`; `instrument_id` **blank** in `_index`            | one `{venue}_{chain}_{capture_ts}.parquet` batch holding MANY instruments (capture_ts is write time, NOT the data day) | per-instrument identity is the parquet **column** only (`instrument_id` = address machine key; `canonical_instrument_id` = symbolic)         |
...
- **DeFi availability is by DATA day**, not capture time: rows land in `day={YYYY-MM-DD}` by their event/block
  timestamp; the capture-ts in the filename is a batch disambiguator; the consolidator merges the many batch files per
  shard-day into one manifest row.
