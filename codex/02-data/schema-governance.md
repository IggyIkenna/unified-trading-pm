...
```python
# SSOT: unified-trading-library/unified_trading_library/domain/timestamp_validation.py
# (the old `unified_trading_services.domain` import path is DEAD — that package no longer exists)
...
Historical record — confirmed data-loss issues in the canonical market schemas (then `canonical/domain.py`, now
`canonical/domain/market/`). All fixed as of 2026-03-06; retained for provenance.
...
The UIC Adoption Matrix tracked which terminal consumer services import each UIC schema class.

> **⚠️ RETIRED — verified 2026-07-30.** Neither `unified-api-contracts/docs/INTERNAL_ADOPTION_MATRIX.md` nor its
> generator `scripts/check_uic_adoption.py` exists in the repo any more. The orphan-schema audit below is a **dated
> 2026-03-06 snapshot**, not a live surface — do not go looking for the matrix file. Treat the categories below as
> historical context; re-derive current adoption with a plain `rg` over consumer repos if you need it.

- **Orphan threshold (as designed):** any UIC schema with 0 terminal consumer imports is an orphan and should be
  classified for adoption, removal, or deferral.
...
### Cross-cutting schemas — MIGRATION COMPLETE (verified 2026-07-30)
...
(messaging canonicals). See: `/codex/02-data/contracts-scope-and-layout.md` § Canonical type ownership.

**Current correct locations:**

Locations re-verified 2026-07-30 — `canonical/` is a PACKAGE (`canonical/domain/<area>/`), not the flat
`canonical/domain.py` / `canonical/execution.py` modules this table used to name:

| Schema                                              | Authoritative Location                                          | Import from                      |
| --------------------------------------------------- | --------------------------------------------------------------- | -------------------------------- |
| CanonicalTrade, CanonicalOrderBook, CanonicalTicker | UAC `unified_api_contracts/canonical/domain/market/__init__.py` | `unified_api_contracts`          |
| CanonicalOrder, CanonicalFill                       | UAC `unified_api_contracts/canonical/domain/execution/base.py`  | `unified_api_contracts`          |
...
> **Import surface rule (CLAUDE.md § "Writing code"):** consumers import from `unified_api_contracts.{domain}` — the
> deep `canonical.*` paths above are UAC-INTERNAL and are given here only to locate the definition, never as the import
> you should write.
