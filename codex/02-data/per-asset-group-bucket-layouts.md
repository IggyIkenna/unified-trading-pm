...
`instrument_availability/instruments.parquet` 404s. MDPS `cloud_data_provider.py` (fde923d) now dispatches on asset_group —
...
1. **SPORTS `category=sports` / `asset_group=sports` is lowercase** in the partition value (legacy `category=sports` and canonical `asset_group=sports` both lowercase) while most other hive values
