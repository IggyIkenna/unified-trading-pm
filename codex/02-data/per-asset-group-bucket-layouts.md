...
> in
> [`availability-manifest-and-data-status.md`](/codex/02-data/availability-manifest-and-data-status.md#multi-axis-correction-banner-canonical).
...
≤30-day reader-fallback window. **Legacy `category=` data + the reader-fallback was targeted for deletion ~2026-06-15**
(co-incident with the reader-fallback retirement gate per
[`service-output-emission-semantics.md`](/codex/02-data/service-output-emission-semantics.md#manifest-read-protocol-per-service_emission_state)).
**⛔ That deadline PASSED and the retirement did NOT happen — re-verified 2026-07-30:**
`market_tick_data_service/raw_tick_hive.py` still defines `RAW_TICK_ASSET_GROUP_HIVE_KEY_LEGACY = "category"` and
`market_tick_data_service/reader.py` still documents the legacy on-disk fallback. Treat the dual-vocab tolerance below
as **still in force**, and the "~2026-06-15" date as an expired intent, not a description of reality — do NOT write a
canonical-only reader on the assumption the fallback is gone. Migration scripts owning the legacy-to-canonical rekey:
[`/plans/archive/2026_05/code_freeze_migrate_backfill_sequencing_2026_05_10.md`](/plans/archive/2026_05/code_freeze_migrate_backfill_sequencing_2026_05_10.md)

- instruments-service `scripts/migrate_*_bare_to_asset_group.py`. The intent was that after 2026-06-15 the dual-vocab
  tolerance in this doc is removed and readers + writers go canonical-only — **that has not happened** (see the
  correction above); the tolerance is still load-bearing. `category=` is the **legacy** form
...
> `deployment-service/configs/cloud-providers.yaml` (authoring location; UAC ships the packaged SSOT copy at
> `unified_api_contracts/config/cloud-providers.yaml`, PM mirrors it at `/configs/cloud-providers.yaml`). Notes:
...
| -------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| CEFI                                         | `raw_tick_data/by_date/day={date}/asset_group={ag}/venue={v}/instrument_type={it}/data_type={dt}/{leaf}` — leaf is `{canonical_instrument_stem}.parquet` (one file per instrument, stem == `VENUE:TYPE:BASE-QUOTE[@MARKER]` canonical id) for the flat-per-contract majority; `ticks.parquet` is reserved for the underlying-bundle chain types only (`options_chain`/`futures_chain`, `underlying={U}/ticks.parquet`). **⛔ corrected 2026-07-26 (cefi_residual_followups_after_honest_done_2026_07_17.md Phase 2):** the prior single row claiming a uniform `ticks.parquet` leaf for ALL cefi shards was superseded by the D3/D4 wire→canonical filename cutover (`market-tick-data-service` `partitioned_writer.py::_leaf_filename` — chain-bundle types keep the `ticks.parquet` fan-in, every other instrument_type writes its own canonical-stem file). Legacy `category={cat}` data coexists on disk per dual-vocab compatibility — see preamble. Tie-breaker SSOT: [`cross-asset-canonical-target-ssot.md`](/codex/02-data/cross-asset-canonical-target-ssot.md) §0/§1. | `date × asset_group × venue × instrument_type × data_type` (legacy hive key was `category`)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              |
| TRADFI                                       | same as CEFI                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     | same                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
| DEFI                                         | `raw_tick_data/by_date/day={date}/pipeline_mode={mode}_{source}/asset_group=defi/venue={v}/chain={chain}/instrument_type={it}/data_type={dt}/{canonical_instrument_id}.parquet` (canonical form — `venue` BEFORE `chain`, `pipeline_mode={mode}_{source}` left of `asset_group=`, and the leaf is **one parquet per instrument**, stem == symbolic canonical id == manifest key; legacy chain-before-venue and `category={cat}` data coexists on disk during migration). **⛔ leaf corrected 2026-07-20, doc-reconciliation P1-09:** ~~`ticks.parquet`~~ was a THIRD defi filename form, contradicting both the retired capture-batch `{venue}_{chain}_{capture_ts}.parquet` and the canonical per-instrument stem. Tie-breaker SSOT: [`cross-asset-canonical-target-ssot.md`](/codex/02-data/cross-asset-canonical-target-ssot.md) §0/§1 pattern #1 + §8.                                                                                                                                                                                                                       | adds `pipeline_mode=` (left of `asset_group=`) + `chain=` (after `venue=`) levels                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        |
...
| **SPORTS**           | `processed/by_date/day={date}/data_type=odds_horizon_bucket/bucketed.parquet`                                                                | **`odds_horizon_bucket` (`bucket_assignment_adapter.py`) is the ONLY sports MDPS product with real prod objects** (verified live 2026-07-23) — a **completely different tree**, `processed/` not `processed_candles/`, single file per date, bookmaker-time bucketed odds feature artefacts. See `market-tick-data-service/docs/SPORTS_ODDS.md`. **`odds_movement`/`odds_snapshot`/`arbitrage_opportunity` are registered in the SAME `CandleAdapterRegistry` as CEFI/TRADFI/DEFI** (`adapters/sports/odds_movement_adapter.py` etc.) and are wired to use the generic single-derivation `processed_candles/` path below IF/WHEN they run — but a live GCS check (2026-07-23, `processed_candles/` + `processed/` prefixes, several sample dates) found **ZERO objects for all three**. Not yet root-caused: never actually dispatched (dead code) vs. dispatched-but-always-empty (a live bug) is unconfirmed — see `/plans/archive/issues/sports_mdps_derived_odds_products_zero_prod_objects_2026_07_23.md`. Do NOT assume these three have live coverage without checking GCS first. |
...
>
> **"SOURCE `data_type`" is NOT always the raw upstream fetch type — read the parameter name carefully (caught
> 2026-07-23 after a real misread).** `write_candle_parquet`'s `source_data_type` param and its docstring examples
> (`trades`, `book_snapshot_5`) describe CEFI's shape, where the `CandleAdapterRegistry` registration key genuinely IS
> the raw input type (one raw stream → one OHLCV output, `@CandleAdapterRegistry.register(CEFI, "trades")`). Asset
> groups with a 1:MANY raw→product relationship (SPORTS: one raw `TRADES` input → up to 4 distinct registered adapters —
> `odds_horizon_bucket`, `odds_movement`, `odds_snapshot`, `arbitrage_opportunity`) register each adapter under its OWN
> product name, and it is THAT name — not the literal raw fetch type — which flows through as `source_data_type` at call
> time (`CandleAdapterRegistry.get_adapter(category, data_type)` → the caller's `data_type` local IS the registration
> key, threaded straight into `write_candle_parquet(source_data_type=data_type, ...)`). So for SPORTS, the manifest/path
> `data_type` axis for e.g. `odds_movement` output is `"odds_movement"`, NOT `"trades"` — coverage tracking for the 3
> derived products (when/if they run — see the ZERO-prod-objects finding above) stays per-product, it does not collapse
> into `TRADES`'s denominator.

**Consequence**: MDPS does not produce per-venue candles for SPORTS via the legacy `odds_horizon_bucket` path. Its
`dependency_checker.py` `OUTPUT_BUCKETS` map covers SPORTS only so the dep-check can resolve; the CURRENTLY-ACTIVE
processing is `adapters/sports/bucket_assignment_adapter.py`, which writes to the legacy `processed/` tree.
...
> `/plans/archive/issues/sports_mdps_derived_odds_products_zero_prod_objects_2026_07_23.md` is archived; its own "dead
> code vs. silently-empty, unconfirmed" verdict was superseded by
> `/plans/archive/issues/mdps_t1_recon_job_oom_failing_7_days_2026_07_26.md` (Updates 4/5/7). Actual root cause: these 3
...
- Coverage roadmap (uses this doc): `/plans/archive/proper_coverage_roadmap_2026_04_20.plan.md`
