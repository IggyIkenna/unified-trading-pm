...
≤30-day reader-fallback window. **Legacy `category=` data + the reader-fallback was targeted for deletion ~2026-06-15** (co-incident with the
reader-fallback retirement gate per
...
canonical-only reader on the assumption the fallback is gone.
...
> `/plans/archive/issues/sports_mdps_derived_odds_products_zero_prod_objects_2026_07_23.md` is archived; its own "dead code vs.
