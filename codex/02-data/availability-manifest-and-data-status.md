...
| **CLOB perps (cefi axis)**           | HYPERLIQUID / ASTER / EXTENDED / LIGHTER (GMX moved to defi axis below; DRIFT / PACIFICA culled)                          | No       | Per-CEFI-instrument shape (`venue, instrument_id, data_type, day`) — NOT DeFi shape per FLAG 1 RESOLVED 2026-05-10 | hive shard on `instrument_id`             | Not required (already per-instrument shard)                                                                         |
| **DEX-pool perps (defi axis)**       | GMX                                                                                                                       | **Yes**  | DeFi-shaped `(chain, protocol, data_type, day)` — DEX-pool perp, `asset_group=defi` (NOT cefi)                     | `market_address` (row-level)              | **MANDATORY** — `expected_root_clusters=set_of_market_addresses`                                                    |
