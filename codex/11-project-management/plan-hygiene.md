...
| `check_archive_candidates.sh`     | Plan/issue docs with 0 open todos, >0 done, unlocked — should be archived (distinct from `check_terminal_status_archived.py`, which only catches a doc whose status ALREADY says resolved/complete). Ratchet-baselined (`archive_candidates_baseline.yaml`), real hard gate since 2026-07-29 (was informational-only + missed `plans/active/issues/` + used non-portable `grep -P`, all 3 fixed the same day) | 0 = count ≤ baseline               |
...
| `count_open_tasks.py`             | Not a gate — reporting only (`/open-task-count` skill). Deduped open-todo count across `plans/active/*.md`, excluding aggregator plans (master/batch/consolidated/closeout/satellite) whose todos double-count the primary plans they roll up; split by `assigned_vm` (planning = AO-dispatched vs NA = not dispatched). The honest "how much real work is left" number — a raw checkbox grep double-counts   | — (run manually / on demand)       |
