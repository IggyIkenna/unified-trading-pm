...
summary:
  ...
  plan-health-agent.yml GHA job RETIRED 2026-08-02, folded in) + archive-eligibility discipline for
...
A suite of scripts that enforce structural and metadata quality across all plans in `plans/archive/2026_08/` and
`plans/epics/`. Catches broken frontmatter, missing required fields, oversized plans, and completed plans sitting in
`archive/2026_08/` instead of `archive/`.
...
| `check_line_caps.sh`              | Plans (`plans/archive/2026_08/*.md`): soft warn >500L, hard fail >1000L, NO exemption. Epics (`plans/epics/*.md`): hard fail >2000L. Ratchet-baselined (`line_caps_baseline.yaml`), real hard gate in the sweep + prek `--precommit` (2026-07-24; the old `umbrella: true`/`locked_by`+todos escape hatch was removed the same day — a large hub doc either fits under 1000L, splits, or becomes a real epic)        | 0 = count ≤ baseline               |
| `check_todo_regression.sh`        | Every plan's open todo count ≥ count on `origin/live-defi-rollout`                                                                                                                                                                                                                                                                                                                                                   | 0 = no regressions                 |
| `check_archive_candidates.sh`     | Plan/issue docs with 0 open todos, >0 done, unlocked — should be archived (distinct from `check_terminal_status_archived.py`, which only catches a doc whose status ALREADY says resolved/complete). Ratchet-baselined (`archive_candidates_baseline.yaml`), real hard gate since 2026-07-29 (was informational-only + missed `plans/archive/issues/` + used non-portable `grep -P`, all 3 fixed the same day)       | 0 = count ≤ baseline               |
...
| `check_superseded_in_active.sh`   | No `*SUPERSEDED*` filenames or superseded `parent_epic` slugs in `archive/2026_08/`                                                                                                                                                                                                                                                                                                                                  | always 0 (soft)                    |
...
| `count_open_tasks.py`             | Not a gate — reporting only (`/open-task-count` skill). Deduped open-todo count across `plans/archive/2026_08/*.md`, excluding aggregator plans (master/batch/consolidated/closeout/satellite) whose todos double-count the primary plans they roll up; split by `assigned_vm` (planning = AO-dispatched vs NA = not dispatched). The honest "how much real work is left" number — a raw checkbox grep double-counts | — (run manually / on demand)       |
...
job's job), grace-protects any plan whose newest git change is <12h old (STEP 2), does a deep
cross-plan/codex/code-state check with grep-then-read verification (STEP 3), files judgment-only items rather than
guessing (STEP 4), and commits a single `docs(plans): daily reconciliation` unit + conditional FF-push (STEP 5).
Design-supersession decision: operator (Harsh + Ikenna), 2026-06-12 — see
`plans/archive/issues/plan_hygiene_precommit_and_agentic_resolution_2026_06_10.md`. Retirement approved:
`june_2026_vintage_audit_findings_2026_07_27.md` §5-RESOLVED #21, executed once the reconciler cleared its ≥3-green-runs
proof gate.
...
failures), and a component test (a scratch `plans/archive/2026_08/*.md` file with no frontmatter) reproduced the same
...
Active plans in `plans/archive/2026_08/<slug>_YYYY_MM_DD.md` MUST carry `parent_epic:` pointing to a live
(non-SUPERSEDED) epic slug. Active plans without `parent_epic:` are **ORPHANS** — review-blocking at PR time.
`check_frontmatter.sh` flags orphans. The `check_superseded_in_active.sh` script flags plans whose `parent_epic:` still
points to a `*SUPERSEDED*` slug.
