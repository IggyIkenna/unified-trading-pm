...
**Result archival**: move to `plans/audit/results/archive/` when (a) every gap item the result spawned is `- [x]` in
its parent active plan, (b) no remaining AMBER or RED items without a plan absorbing them. Update the Linked Results
table in the instruction file when archiving.
