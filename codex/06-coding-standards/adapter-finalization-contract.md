...
> **`cefi/derivative_adapter` REVERSED OUT 2026-07-20 (operator ruling, confirmed live in the adapter's own module
> docstring — 2026-07-26 audit, `cefi_satellite_ao_dispatch_batch1-002`) — the table row below is HISTORICAL, not
> current.** The adapter is now deliberately **NOT** routed through `_finalize_session_grid` at all — no `state_col`, no
> prior-day seed (`supports_prior_day_seed=False`). Carrying the last snapshot forward into an empty window was judged
> to conflate "window had nothing to aggregate" (honest per-bin absence) with "not yet fetched", so a window with no
> observation stays NaN price-like fields + `volume=0`/`trade_count=0` — including leading (pre-first-observation) bins.
> This makes `derivative_adapter` a SECOND intentional exception to this contract alongside `cefi/liquidations_adapter`
> (no-grid event-count design) — both are correctly non-finalizer-routed, for different reasons. Matching correction in
> `/codex/02-data/honest-absence-downstream-handling.md`'s carry-forward table.

| Adapter                                                                        | data_type           | Finalize call                                                                                       | Rationale                                                                                                                                  |
| ------------------------------------------------------------------------------ | ------------------- | --------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------ |
| `cefi/derivative_adapter` (HISTORICAL — reversed 2026-07-20, see banner above) | `derivative_ticker` | ~~`state_col="mark_price"`~~ NONE (no finalizer routing)                                            | ~~close structurally NaN; mark price is the candle price; volume→0~~ honest per-bin NaN, no LOCF (conflates empty-window with not-fetched) |
