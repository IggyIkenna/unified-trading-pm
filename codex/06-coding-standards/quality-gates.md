...
| 5.101 | `.get("key", "")` empty-string-fallback baseline ratchet — a per-repo count of empty-string-default dict-`.get()` sites (without `# noqa: qg-empty-fallback`); count <= baseline passes, count > baseline fails (a NEW site landed). Baseline-ratchet, not zero-tolerance (supersedes the old inline `codex_rg` zero-tolerance check) — same shrinking-ratchet family as DTZ/TID251/fallback-import. | (no section here — see enforcement file)                                                                                                   | `scripts/quality_gates/check_no_empty_string_fallback.py` (driver); baseline `scripts/quality_gates/no_empty_string_fallback_baseline.yaml` (per-repo `count:` + `commit:` anchor, hard-clamped DOWN-only via `--update-baseline`) | `plans/archive/issues/mtds_empty_string_fallback_codex_gate_blocking_pushes_2026_07_08.md` — origin incident (repo-wide red block, no ratchet existed yet); fix a genuinely-unsafe site fail-fast, or annotate a deliberate-safe one with `# noqa: qg-empty-fallback` + a one-line reason |
| L1    | `data_type` enum contains `LIVE_`/`BATCH_` prefixed members                                                                                                                                                                                                                                                                                                                                          | [STEP L1: DataType Mode-Prefix Ban](#step-l1-datatype-mode-prefix-ban-day-1-enable)                                                        | `scripts/quality-gates-base/base-service.sh` STEP 5.75 (ENABLED 2026-05-14)                                                                                                                                                        | "Batch = Live: Unified Pipeline Architecture" — unified DataType enum, no per-mode fork                                                                                                                                                                                                   |
...
The binding constraint is **peak RSS**, not cores. Measured heavy tier (tree-RSS, 2026-07-14 VM re-benchmark —
