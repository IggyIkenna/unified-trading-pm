...
> BEFORE making code or doc changes informed by this doc. This doc is partially stale: missing cluster validation as
> mandatory write-gate pillar at `record_captured` for bundled shards. The post-plan-reality doc lists the 10
...
- Upstream rule:
  [`02-data/sports-scheduling-and-sharding.md` §5](../02-data/sports-scheduling-and-sharding.md#5-lookahead-bias--data-crimes).
- Feature-service parallel: `FeatureWriteGate` in `unified_trading_library.feature_service_base.write_gate` — composes
  NaN/inf/leakage checks + `validate_timestamp_date_alignment`. `InstrumentsWriteGate` is the narrower raw-data
  analogue.
