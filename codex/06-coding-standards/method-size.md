...
**Enforcement**: `FUNCTION_SIZE_EXTRA_EXCLUDES` in each service's `quality-gates.sh`. Files in the allowlist are tracked technical debt with a named owner + batch (e.g. `Batch-32`). Allowlist must shrink monotonically — adding a new entry requires a TODO comment with owning batch name.
...
- Parameter-heavy signatures: introduce a frozen dataclass or `NamedTuple` to bundle >5 params that recur across helpers.
- `# fmt: off` / `# fmt: on` around irreducibly-long but semantically atomic blocks (e.g. a single multi-line dict literal).
