...
## §8 — Porting to sibling UIs (deferred)

The scanner is currently scoped to `unified-trading-system-ui`. `deployment-ui` and `user-management-ui` are candidates
for the same 3-phase rollout but are deferred until the main UI's baseline proves stable (≥2 weeks green CI on Phase 3).

Porting cost is low — the scanner is self-contained (single TS file) and the policy is identical. The port blocker is
usually the Phase 2 triage (each UI has its own backlog of historical orphans).
