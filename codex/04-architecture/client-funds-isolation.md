...
2. **Execution-service runtime gate**: **updated 2026-08-21** — `execution_service.transfer_coordinator
.TransferCoordinator` (the prior citation here) was DELETED during the walkthrough-feedback transfer-path
   consolidation (`plans/active/walkthrough_feedback_remediation_2026_08_21.md`); it had ZERO production
   construction sites workspace-wide. Its cross-client check relocated to
   `execution_service.isolation_policy.assert_transfer_intent_client_allowed()` (same behavior, preserved for
   whichever future `TransferIntent` bus consumer wires up — none exists in production today). The LIVE
   consume-time gate is `execution_service.engine.transfers.isolation.assert_fund_context_client_allowed()`, which
   guards the `TransferAdapter` path (fund-administration-service redemptions) — see the table row below. Both
   raise `CrossClientTransferForbiddenError` before any RPC call hits a venue or chain.
...
| execution-service           | `isolation_policy.assert_transfer_intent_client_allowed()` (relocated 2026-08-21 from the deleted `TransferCoordinator.execute()`; dormant, no production `TransferIntent` bus consumer) | At consume time: rejects any TransferIntent whose `client_id` != process-bound CLIENT_ID; logs alert                                                                                                                                                                                                                                                                                                                                                                             | `CrossClientTransferForbiddenError`                                       |
