...
> BEFORE making code or doc changes informed by this doc. This doc is partially stale: sports section pre-(per-fixture
> sharding); update needed to reflect per-fixture shard atom. The post-plan-reality doc lists the 10 cross-cutting
> principles codified in workspace `CLAUDE.md` (live=batch, no double SSOT, three-category empty-output decision,
> cluster validation mandatory, per-row write-time `available_at`, prediction lifecycle timing, temporary state must
> have named successor, per-VM shard isolation, etc.) plus the active plans where the canonical post-plan reality is
> being implemented (`writegate_honest_coverage_endtoend_2026_05_06.plan.md`,
