...
| --------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------ |
| **URDI** (reference data)   | Connectivity to venue APIs. Auth management. Calling `normalize_*()` from UAC. Returning `InstrumentRecord`.                                   | Domain data storage. Scheduling. Venue schemas (those are in UAC).                                           |
| **USRI** (sports reference) | Connectivity to sports data APIs (API Football, etc.). Calling `normalize_*()` from UAC. Returning `CanonicalFixture`, `CanonicalLeague`, etc. | Fixture storage. League classification logic. Team mapping data (that's UAC registry).                       |
| **UMI** (market data)       | Connectivity to market data feeds (WebSocket, REST). Raw tick/OHLCV retrieval.                                                                 | Reference data (that's URDI). Instrument discovery (that's URDI). Processing/downsampling (that's services). |
...
| ---------------------------------- | ----------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------- |
| **instruments-service**            | Calls URDI/USRI to discover instruments for a date. Joins reference data (fixtures) with market instruments. Validates. | Instruments via DataSink (Parquet, by venue/date). |
