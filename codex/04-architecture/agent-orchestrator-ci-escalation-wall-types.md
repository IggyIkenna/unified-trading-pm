...
| `cloud_build_failure`                         | A repo's Cloud Build image pipeline failing (separate from GH Actions QG)                                                                                                   | `cloud-build-failure-watcher.yml`                                                  | no                  | `cicd` (generic)                               | **no proven auto-resolution signal** — deadline→re-escalate→unresolved(pages operator) |
...
| `codex_freshness_stale` **(new, 2026-08-15)** | Daily sweep finds a codex doc authoring defect (missing/invalid `last_reviewed:`, malformed frontmatter) or staleness digest — moved OFF the `quality-gates.sh` commit path | `codex-freshness-sweep.yml` (unified-trading-pm, daily cron)                       | no (`pr_number: 0`) | `cicd` (generic)                               | `codex-freshness-sweep.yml`'s own next run concluding `success` (0 violations)         |
