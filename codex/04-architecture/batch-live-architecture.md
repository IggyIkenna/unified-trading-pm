...
| features-service (sports family)     | FSS  | `BatchHandler`          | `LiveHandler` (shipped, deployment pending) | batch / live      | unit: both modes    | **CORRECTED 2026-07-29** — `LiveHandler` is shipped + CLI-wired (`features_service/sports/cli/handlers/live_handler.py`, dispatched via `cli/main.py::get_handler_for_mode`), unit-tested (`tests/sports/unit/test_live_handler.py`); production deployment still pending on the launcher exec-dispatch gap tracked in `plans/active/issues/mdps_features_live_launcher_exec_dispatch_never_wired_2026_07_27.md`. (Previous pointer here, `plans/epics/features_and_ml_master.md` p1-todo-10, was broken/stale — that todo ID does not exist in that epic.) Full per-family live-handler status: `features-service-architecture.md` § "Live handler status per family". |
