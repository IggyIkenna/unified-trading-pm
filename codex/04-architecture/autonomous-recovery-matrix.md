...
| WS rotation completed       | No       | No             | Yes          | Log only (daily digest) |
| WS rotation failed (repeat) | Yes      | Yes (P2)       | Yes          | Rotate → venue kill ≥3  |
| Private-stream resync fail  | Yes      | Yes (CRITICAL) | Yes          | Withhold updates → kill |
...
| G8  | Silent-stale websocket → rotate → escalate to venue kill       | SHIPPED 2026-08-21        | UTL `WsSessionManager` (staleness → retry ladder → rotate; venue `WsProtocolSpec`-driven make-before-break) + `ws_rotation_request` sentinel; deployment-service `scripts/recovery/rotate_websocket.py` (Layer-0, `ActionType.ROTATE_WEBSOCKET`; ws-sourced feeds bind `rotate-websocket:<source>` in the freshness registry); MTDS live `_ws_session_bridge` (honest STALE-window marking); execution `PrivateStreamGuard` (resync-before-trust; ≥3 consecutive rotation OR resync failures emit `KILL_SWITCH_VENUE_DISCONNECT` — venue-scoped protective kill, arming autonomous, resume per this matrix) — per plan `venue_websocket_resilience_and_error_code_mapping_2026_08_21.md` |
