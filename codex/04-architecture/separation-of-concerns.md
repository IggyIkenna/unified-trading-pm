...
| **Active Service Repos** | UMI, execution-service (CeFi+DeFi+Sports), instruments-service (ref data), UFI, USRI                   | Venue connectivity + canonical routing. Each interface owns adapters for a domain (market data, execution, etc.).                       | Import from UAC (schemas + normalizers), UTL (infrastructure). Never define schemas.                                                                        |
