...
- Logs the manual decision via `unified-trading-library` lifecycle events with the operator's identity from DART
  auth.
...
1. **Strategy authoring** — register a strategy archetype config for backtest. Live = batch (CLAUDE.md "Batch =
   Live") — once authored, the same config drives batch (matching engine fills) and live (real fills) without
   divergence.
