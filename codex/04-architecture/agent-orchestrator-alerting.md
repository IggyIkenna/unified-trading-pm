...
summary: The contract for what reaches the agent-orchestrator-alerts Slack channel. Automatic backend lifecycle events
...
- **Same-plan priority inversion / starvation** — `notify_dispatch_priority_inversion` pages when a dispatched or working task holds its plan's single in-flight slot beyond the tunable threshold while a strictly higher-priority queued sibling has zero blockers. `DispatchPriorityInversionWatchdog` deduplicates by `(plan_ref, T_high.id)` and posts `notify_dispatch_priority_inversion_resolved` as the ✅ close bookend when the condition clears.
...
| `notify_dispatch_priority_inversion`                                                                                                                                                                                                                                                                                                                                                                              | **PAGE**                                 | a same-plan higher-priority queued task is starved by a long-held dispatched/working sibling; keyed episode dedup on `(plan_ref, T_high.id)` and a matching resolved close bookend                                                                                                                                                                                                                                                                                                                                                          |
...
| Orchestrator-process-external liveness (`scripts/orchestrator-liveness-guard.sh` — polls `/api/healthz` over plain HTTP, pages on N=3 consecutive failures)                            | infra craft (AO on-call: operator ikenna)            | own systemd timer (`orchestrator-liveness-guard.timer`, `OnUnitActiveSec=20`) — deliberately OUTSIDE the orchestrator process, since every other row's `notify_*` machinery lives INSIDE it and cannot fire while it's down (`ao_crash_loop_zero_alerting_and_grok_proxy_health_2026_08_20.md` Gap 1 — a 2026-08-20 crash-loop was down ~6 min with zero pages) | `bash scripts/orchestrator-liveness-guard.sh --self-test` (pure state-machine proof: below-threshold/new-page/skip/remind/resolved transitions, no network); live confirm = a page + a ✅ RESOLVED bookend in `#agent-orchestrator-alerts`, posted DIRECTLY to the Slack webhook (bypasses `server/notifications/slack.py` entirely — that path is what's unavailable when this fires) | `agent-orchestrator@38cf884b25`                                      |
