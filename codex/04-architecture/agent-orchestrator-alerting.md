...
| Orchestrator-service external liveness (`orchestrator-liveness-check.sh` — polls `/api/healthz` via `curl`, independent of the orchestrator's own process/venv/DB; pages the raw Slack webhook directly, bypassing `server/notifications/slack.py`, since that path is unavailable while the orchestrator itself is down) — closes `ao_crash_loop_zero_alerting_and_grok_proxy_health_2026_08_20.md` Gap 1 | infra craft (AO on-call: operator ikenna) | systemd `--user` timer, every 30s (`orchestrator-liveness-check.timer`, installed via `scripts/install-orchestrator-liveness-check-timer.sh`) | `bash scripts/orchestrator/orchestrator-liveness-check.sh --self-test` (9-case state-machine proof, no network needed); live confirm = a page in `#agent-orchestrator-alerts` after 3 consecutive `/api/healthz` failures (~60-90s) + a ✅ RESOLVED bookend on recovery | `agent-orchestrator/scripts/orchestrator/orchestrator-liveness-check.sh` |
