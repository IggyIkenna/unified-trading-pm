...
A silent-detection-gap incident (`/plans/active/issues/ao_context_saturation_never_alerts_2026_08_09.md` — a saturated
...
| Context-activity-silence ("silent-disarm": no context-lifecycle activity event of ANY type for a role with a live session for ≥`context_activity_silence_alert_seconds`, default 4h)   | backend_engineer craft (AO on-call: operator ikenna) | **PENDING** — detector not yet implemented                                                                                               | **PENDING**                                                                                                                                                                                                                                                                                                                                                             | tracked by `ao_context_saturation_never_alerts-0c1b3343f4c1` (todo 3 of the same issue; dispatched to slot 31, 2026-08-09T19:22 UTC, still in flight as of this registration). This row is listed now, ahead of the code landing, so the registry itself never has a silent gap about which detectors are meant to exist — see the follow-up todo below to backfill real values once it ships. |
