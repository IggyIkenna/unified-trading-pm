...
summary:
  Operator runbook for database/storage degradation (Cloud SQL / Firestore / Redis write errors, latency, pool
  exhaustion) — wait vs enter_readonly_recon_mode vs DR restore; ledger writes failing escalates to SEV0.
status: current
nature: process
asset_group: [meta]
stage: [meta]
...
tags: [runbook, incident, connectivity, database, storage, disaster-recovery]
...
last_executed: never
code_refs:
authoritative_for: [RB-CONN-004 operator runbook]
referenced_by:
  [
