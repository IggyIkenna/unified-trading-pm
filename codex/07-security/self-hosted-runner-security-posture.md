...
- **Moving a job hosted → self-hosted changes its auth model.** Hosted needs explicit `auth` steps / secrets;
  ...
  step. **Caveat, ruled failure mode (2026-07-25, recurring, open — not fixed by this posture):** "ambient ADC is
  there" assumes the shared `~/.config/gcloud` active account stays `unified-trading-sa`. A self-hosted job that itself
  calls `google-github-actions/auth` (WIF, e.g. `cloud-build-router.yml`'s `github-actions-deploy@` SA) overwrites that
  SAME shared config with a job-scoped credential that cannot outlive the job, poisoning every later shell on the host
