...
> **Status**: partially shipped (G3/G4/G5/G6 DONE; G1/G2 open — G6 found and closed same-day 2026-08-21, see § 7 for detail).
> Plan-of-record:
...
Last updated: 2026-08-21 (G3/G4/G5/G6 landed; G1/G2 still open — G6 found and closed same day).
...
| **Live-fill → `InstructionLedger` wiring**                 | **DONE (G6)**       | Found MISSING 2026-08-21 (T4 tranche): no live trade of any kind reached the real GCS `InstructionLedger` — only batch-rerun/paper-simulation runs wrote it. Operator ruling same day: route manual fills through the same writer path as batch/paper ("route to execution so it gets routed to the same place as manual trades"). Built and shipped same day — `execution-service@ee694cf46b` + `unified-trading-library@707020ff7b`: new `manual_instruction_ledger.py` builds one `TradeFillRecord` per manual fill via the same `write_run_ledger` batch/paper path, wired into EXECUTE + RECORD_ONLY. `recon_excluded` now has a real live `LedgerRow` to act on for manual fills. **Strategy-driven live fills remain unwired** — `fill_event_consumer.py` still only updates the position store, never calls the ledger writer; that gap is not part of G6's closed scope (manual fills only) and would need its own follow-up if strategy-driven live-fill ledger coverage is wanted. |
