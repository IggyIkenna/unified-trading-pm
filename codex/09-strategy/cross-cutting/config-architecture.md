...
**See also:** [Operational modes matrix](operational-modes-matrix.md) — how `DATA_MODE`, `RUNTIME_MODE`, `TESTNET_MODE`,
...
> `unified_api_contracts.internal` with fields: `source: str`, `filter: str`, `threshold: Decimal | None`,
...
> holding mode enum above. It needs to be: (a) A formal `ExecutionMode` StrEnum in `unified_api_contracts.internal` (b)
...
| Risk subscriptions     | StrategyRiskProfile (not yet wired) | `unified_api_contracts/internal/risk.py:414`           |
