...
summary: The SSOT coverage matrix — for every v2 archetype each (category, instrument_type) cell is declared
...
| CeFi × TradFi (cross-category) | crypto equity-perp + real-equity spot | PARTIAL   | Binance/OKX/Bybit equity-perp (e.g. NVDAUSDT, AAPLX) + IBKR cash-equity spot (hedge venue, operator-approved 2026-08-08) | funding-rate + dividend-adjusted basis | Crypto-venue single-stock perps/tokenized-equity map to their Databento real-equity twin via the `tracks_equity` catalogue tag — see [`cefi-capture-universe.md`](../../02-data/cefi-capture-universe.md) § "Exception — TradFi-linked perps". Dividend-adjusted NET-basis backtest (12 pairs) confirms every pair TRADEABLE — see [`carry-basis-perp.md`](archetypes/carry-basis-perp.md) § "Crypto-venue equity-perp basis variant" for the full evidence trail. Full archetype design (dispersion + overnight-gap legs) and the IBKR adapter build are still open — `plans/active/cryptovenue_equity_perps_and_tokenized_stocks_2026_06_20.md` Phase 4 / Phase 1e. |
