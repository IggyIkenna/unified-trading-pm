...
#
# Usage:
#   bash 05-verify-setup.sh --project 7
#
# Requires:
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    -h | --help)
      ...
      echo ""
      echo "Example:"
      echo "  bash 05-verify-setup.sh --project 7"
      exit 0
      ;;
    *)
...
LABEL_COUNT=$(echo "$SAMPLE_ISSUES" | jq '[.[] | select(.labels != null) | select(.labels | contains(["POST-TRADE-AND-EXECUTION"]))] | length' || echo "0")

if [ "$LABEL_COUNT" -eq 0 ]; then
  echo "  ⚠️  WARNING: No issues with POST-TRADE-AND-EXECUTION label found"
  echo "     Issues may need labels applied"
  ((CHECKS_WARNING++))
else
  echo "  ✅ PASSED: Issues have POST-TRADE-AND-EXECUTION label"
  ((CHECKS_PASSED++))
fi
