...
#
# Creates comprehensive README documentation for the Market Data Infrastructure project.
#
# Usage:
...
while [[ $# -gt 0 ]]; do
    case "$1" in
        ...
        -h|--help)
            ...
            echo ""
            echo "Example:"
            echo "  bash 06-generate-project-readme.sh --project 8 > PROJECT_README.md"
            exit 0
            ;;
        *)
...
### Project Label
- \`MARKET-DATA-INFRASTRUCTURE\` - All issues in this project
...
1. **Auto-add to project** - Issues with \`MARKET-DATA-INFRASTRUCTURE\` label auto-add
...
gh issue list --label MARKET-DATA-INFRASTRUCTURE --state all

# By priority
gh issue list --label MARKET-DATA-INFRASTRUCTURE,P0-critical --state open

# By repo
gh issue list --repo $ORG/unified-events-interface --label MARKET-DATA-INFRASTRUCTURE
\`\`\`
...
gh pr list --label MARKET-DATA-INFRASTRUCTURE --state open --search "review:required"
\`\`\`
...
- ✅ Live testing ready by Mar 10, 2026
...
**Scripts:**
- Project setup: \`unified-trading-codex/11-project-management/github-integration/scripts/projects/market-data-infrastructure/\`
