#!/usr/bin/env bash
#
# Batch execution wrapper for Unified Libraries Refactor subtasks
#
...
echo "========================================="
echo "Unified Libraries Refactor - Batch Execution"
echo "========================================="
echo ""
echo "Model: $MODEL"
...
if [ -n "$CUSTOM_ISSUES" ]; then
  ...
  if [ ! -f "$ISSUE_MANIFEST" ]; then
    ...
    echo "  cd $SCRIPT_DIR"
    echo "  python 02-create-issues.py --org IggyIkenna --epic-file ../../epic-breakdowns/epic-unified-libraries-refactor.md --apply"
    exit 1
  fi
